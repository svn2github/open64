...
static int		SetIntFromAny _ANSI_ARGS_((Tcl_Interp *interp,
			    Tcl_Obj *objPtr));
static int		SetShortFromAny _ANSI_ARGS_((Tcl_Interp *interp,
			    Tcl_Obj *objPtr));
...
static void		UpdateStringOfShort _ANSI_ARGS_((Tcl_Obj *objPtr));
...
Tcl_ObjType tclShortType = {
    "short",				/* name */
    (Tcl_FreeInternalRepProc *) NULL,   /* freeIntRepProc */
    (Tcl_DupInternalRepProc *) NULL,	/* dupIntRepProc */
    UpdateStringOfShort,			/* updateStringProc */
    SetShortFromAny			/* setFromAnyProc */
};
...
TclInitObjSubsystem()
{
    ...
    Tcl_RegisterObjType(&tclShortType);
 ...
 *	Tcl_NewShortObj to create a new integer object end up calling the
 *	debugging procedure Tcl_DbNewLongObj instead.
 *
 *	Otherwise, if the client is compiled without TCL_MEM_DEBUG defined,
 *	calls to Tcl_NewIntObj result in a call to one of the two
 *	Tcl_NewShortObj implementations below. We provide two implementations
 ...
 */
Tcl_Obj *
Tcl_NewShortObj(shortValue)
    register short shortValue;	/* Int used to initialize the new object. */
{
    register Tcl_Obj *objPtr;
    ...
    objPtr->internalRep.longValue = shortValue;
    objPtr->typePtr = &tclShortType;
    return objPtr;
}

/*
    ...
    register int *intPtr;	/* Place to store resulting int. */
{
    ...
    if (objPtr->typePtr != &tclShortType && 
    	objPtr->typePtr != &tclIntType && 
    ...
    // for short type, we doesn't care about the range, just truncate it
    if ( objPtr->typePtr == &tclShortType ) {
    	// Be careful. One 2bytes will be written to the place.
    	*(short *)intPtr = (short)w;
    	return TCL_OK;
    }
    *intPtr = (int)w;
    return TCL_OK;
...
static int
SetShortFromAny( Tcl_Interp* interp, 
				/* Tcl interpreter */
	       Tcl_Obj* objPtr )
 ...
 * UpdateStringOfShort --
 *
 *	Update the string representation for an short object.
 *	Note: This procedure does not free an existing old string rep
 *	so storage will be lost if this has not already been done. 
 *
 * Results:
 ...
 * Side effects:
 *	The object's string is set to a valid string that results from
 *	the short-to-string conversion.
 *
 *----------------------------------------------------------------------
 */

static void
UpdateStringOfShort(objPtr)
    register Tcl_Obj *objPtr;	/* Short object whose string rep to update. */
{
    char buffer[TCL_INTEGER_SPACE];
    register int len;
    
    // Question: This may need rewrite.
    //len = TclFormatInt(buffer, (objPtr->internalRep.longValue&0xffff));
    sprintf(buffer, "%hu", (short)(objPtr->internalRep.longValue&0xffff));
    len = strlen(buffer);
