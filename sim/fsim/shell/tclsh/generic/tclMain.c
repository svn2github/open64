...
static char tclLastCommandString[256] ;
				 ...
				 * execute commands. */
{
    ...
    Tcl_DString appName;
    Tcl_Obj *objPtr;
    char slCmdPrompt[32];	// command prompt buffer for Simplight
    ...
    history_file = (char *)xmalloc (2 + home_len + 14); /* strlen(".slsim-history") == 14 */
    strcpy (history_file, home);
    history_file[home_len] = '/';
#if defined (__MSDOS__)
    strcpy (history_file + home_len + 1, "_slsim-history");
#else
    strcpy (history_file + home_len + 1, ".slsim-history");
#endif
    using_history();
...
#ifdef _USE_READLINE_
	// We won't output prompt here
	{
	slCmdPrompt[0] = '\0';
    Tcl_Obj *promptCmdPtr;
    promptCmdPtr = Tcl_GetVar2Ex(interp, "sl_prompt",
	    NULL, TCL_GLOBAL_ONLY);
	if (promptCmdPtr) {
		char *p = Tcl_GetString(promptCmdPtr);
		if (p) {
			strncpy(slCmdPrompt, p, 16);
			slCmdPrompt[16] = '\0';
		}
	}
	if (!slCmdPrompt[0])
		strcpy(slCmdPrompt, "SL>");
	}
#else
	if (tty) {
	...
	/* initialize readline */
	initialize_readline();
	line = readline((const char *)slCmdPrompt);
	if (line != NULL) {
            // Insert a space before the 1st slash if it doesn't have and
	    // it is the 1st word of the line
	    char *q, *p;
	    q = strchr(line, '/');
	    if (q) {
	        p = line;
		while (isspace(*p)) p++; // skip leading space
		while (!isspace(*p) && p!=q) p++;
		if (p == q) {
		    // The first slash is in the first word, let's insert a space
		    char *newline, *r; 
		    newline = (char*)malloc(strlen(line)+2);
		    r = newline;
		    p = line;
		    while( *p ) {
		        if (p == q)
	    ...
	    if (!length) {
	        // repeat last command
		if (tclLastCommandString[0])
		    Tcl_AppendToObj(commandPtr, tclLastCommandString,
		        strlen(tclLastCommandString));
	    }
	    else {
	        Tcl_AppendToObj(commandPtr, line, length);
		strncpy(tclLastCommandString, line, 255);
		tclLastCommandString[255] = '\0';
	    }
	} else {
	    goto done;
	}
	/* Cleanup the string allocated & add it to history */
	if (line) {
	    if (*line) {
		add_history(line);
	    }
...
const char *Tcl_GetLastCommand(void)
{
	return tclLastCommandString;
}
