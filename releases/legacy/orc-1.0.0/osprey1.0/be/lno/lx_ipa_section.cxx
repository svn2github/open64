			 ...
			 IPA_LNO_READ_FILE* IPA_LNO_File)
{
  ...
  if (av->Contains_Lin_Symb()) {
    ...
    for (INTSYMB_NODE* cur = iter.First(); 
	  ...
	  for (INT i = 0; i < IPA_LNO_File->Ivar_Count(); i++) 
			      ...
			      LINEX* lx_substitute)
{
  ...
  for (INT i = 0; i <= freeze_num_terms; i++) { 
    TERM* tm = Get_term(i); 
...
LINEX* LOOPINFO::Max_value()
{
  ...
  for (INT i = 0; i <= lx_upper->Num_terms(); i++) {
...
CFG_NODE_INFO::Add_scalar_ref_passed(mINT32 id, mINT16 callsite_id)
{
  ...
  for (INT i=0; i<=scalar_array->Lastidx();++i)
    {
