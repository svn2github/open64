...
static int libdwarf_compose_begin( Dwarf_P_Debug dbg, int code, 
	size_t maxlen, int *compose_error_type)
{
   ...
   if(curblk == 0) {
	...
	newb = (struct dw_macinfo_block_s *)_dwarf_p_get_alloc(dbg,len);
	if(!newb) {
	  *compose_error_type = DW_DLE_MACINFO_MALLOC_FAIL;
	  return DW_DLV_ERROR;
	}
	newb->mb_data = (char *)newb + sizeof(struct dw_macinfo_block_s);
	...
	/* no space left in block: allocate a new block */
        size_t blen = dbg->de_current_macinfo->mb_macinfo_data_space_len*2;
        if(blen < maxlen) {
                 blen = 2*maxlen;
        }
        len = sizeof(struct dw_macinfo_block_s) + blen;
   ...
   nextchar = (unsigned char *)(curblk->mb_data + dbg->de_compose_used_len);
   *nextchar = code;
...
static void libdwarf_compose_add_string( Dwarf_P_Debug dbg, 
...
static int libdwarf_compose_add_line( Dwarf_P_Debug dbg, 
	Dwarf_Unsigned line,int *compose_error_type)
{
   ...
   /* Put the created leb number directly into the macro buffer 
      If dbg->de_compose_avail is > INT_MAX this will not work
      as the 'int' will look negative to _dwarf_pro_encode_leb128_nm!
   */

   res = _dwarf_pro_encode_leb128_nm(line,&nbytes,
		(char *)nextchar,(int)dbg->de_compose_avail);
   if(res != DW_DLV_OK) {
...
/*
   ...
   preceeding calls.
*/
static int  libdwarf_compose_complete( Dwarf_P_Debug dbg,int *compose_error_type)
{
   struct dw_macinfo_block_s *curblk = dbg->de_current_macinfo;
    ...
    Dwarf_Error*   error)
{
    ...
    length_est = COMMAND_LEN + LINE_LEN + len + len2+1;/* 1 for space 
		character we add */
    ...
    res = libdwarf_compose_add_line(dbg,line,
		&compose_error_type);
    if(res != DW_DLV_OK) {
        _dwarf_p_error(NULL, error, compose_error_type);
        return(DW_DLV_ERROR);
...
int dwarf_end_macro_file(Dwarf_P_Debug dbg,Dwarf_Error *error)
{
    size_t length_est;
    int res;
    int compose_error_type;
    ...
    Dwarf_Error *error)
{
    ...
    /*  Tthe final entry has a type code of 0 to indicate
	It is final for this CU
	Takes just 1 byte.
    */
    mac_num_bytes += 1;

    GET_CHUNK(dbg, dbg->de_elf_sects[DEBUG_MACINFO], 
	macinfo, (unsigned long)mac_num_bytes, 
	error);
    if (macinfo == NULL)
