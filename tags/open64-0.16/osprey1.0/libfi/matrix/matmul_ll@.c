...
#pragma ident "@(#) libfi/matrix/matmul_ll@.c	92.1	07/09/99 15:18:08"
...
#else
#define SUBNAME lgemmx__
#endif
/*
 * Type of constants alpha and beta
 */
#define RESULTTYPE	_f_log8

#if defined(_UNICOS)
#pragma _CRI duplicate _MATMUL_LL as MATMUL_LL@
#endif
void
	...
	DopeVectorType *MATRIX_B)
{
    ...
    SUBNAME(&MATDIM->m, &MATDIM->n, &MATDIM->k, &true, MATDIM->A,&MATDIM->inc1a,
    	    &MATDIM->inc2a, MATDIM->B, &MATDIM->inc1b, &MATDIM->inc2b,
	    &false, MATDIM->C, &MATDIM->inc1c, &MATDIM->inc2c);
    return;
}
