...
)
{
	...
	*/
	...
	cu_context = _dwarf_make_CU_Context(dbg, new_offset, error);
	  ...
	  Since _dwarf_make_CU_Context has already registered
	  an error we do not do that here: we let the lower error
	  pass thru.
	  */
	  return(DW_DLV_ERROR);
        }
...
)
{
    ...
    *has_die_child = abbrev_list->ab_has_child;
	...
	if (attr_form != 0) {
		...
		attr_form, info_ptr,cu_context->cc_length_size);
