...
--------------------------------------------------------------------*/
    /* We want the following to have an elf section number
...
common_init(Dwarf_P_Debug dbg,Dwarf_Unsigned flags)
{
    ...
    /* Revised 64 bit output, using distingushed values.
       Per 1999 dwarf2 revision
       This produces 64bit extension with ia64 objects.

       Some might want library run time selection of
       offset size. Not provided here at present.
    */ 
	...
	/* Non-MIPS, dwarf lengths and offsets are 32 bits even for
	   64bit pointer environments.
	*/
        /*  Get_REL??_isa here supports 64bit-offset dwarf.
	    For 64bit, we emit the extension bytes.
        */

    dbg->de_offset_reloc =  IS_64BIT(dbg) ? Get_REL64_isa(dbg)
		:Get_REL32_isa(dbg);
#elif defined(HAVE_OLD_DWARF2_32BIT_OFFSET)
    /* This is cygnus 32bit offset, as specified in
       pure dwarf2 v2.0.0 
    */
	...
	/* non-MIPS, dwarf lengths and offsets are 32 bits even for
	   64bit pointer environments.
	*/
        /*  Get_REL32_isa here supports 64-bit-pointer dwarf
	    with pure dwarf2 v2.0.0 32bit offsets,
	    as emitted by cygnus tools.
	    And pure 32 bit offset dwarf for 32bit pointer apps.
        */

    dbg->de_offset_reloc =  Get_REL32_isa(dbg);
#else
    /* MIPS-SGI 32 or 64, where offsets and lengths
	 are both 64 bit  for 64bit pointer objects 
	 and both 32 bit  for 32bit pointer objects.
	And a dwarf-reader must check elf info to tell which
	applies.
    */
    dbg->de_64bit_extension = 0;
    dbg->de_pointer_size = (IS_64BIT(dbg) ?  8 : 4);
