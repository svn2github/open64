...
)
{
    ...
    /* The following de_length_size is later modified
       in this routine in the important cases.
       To allow cygnus 32bit-offset dwarf2.
    */
	...
	if(ehdr64->e_machine != EM_MIPS) {
           ...
           */
	   /* 4 here supports 32bit-offset dwarf2, as emitted
		by cygnus tools, and the dwarfv2.1
		64bit extension setting.
	   */
	    ...
	    if(is_64bit && data->d_size > 10) {
	      ...
	      Dwarf_Unsigned length;
              READ_UNALIGNED(dbg,length, Dwarf_Unsigned,
                        dbg->de_debug_info, sizeof(Dwarf_Unsigned));
              READ_UNALIGNED(dbg,v1, Dwarf_Half,
                        dbg->de_debug_info+4, sizeof(Dwarf_Half));
              READ_UNALIGNED(dbg,v64, Dwarf_Half,
                        dbg->de_debug_info+8, sizeof(Dwarf_Half));
