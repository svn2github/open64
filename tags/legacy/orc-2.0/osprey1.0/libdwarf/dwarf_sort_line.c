...
)
{
    ...
    int length_size = 4;
    Dwarf_Small *orig_line_ptr; /* our local copy of
		the user's input buffer */
    Dwarf_Small *line_ptr; /* starts at orig_line_ptr,
			gets incremented thru to end of
			our copy of the input buffer */
    Dwarf_Small *new_line_ptr; /* output of
				_dwarf_update_line_sec(), used
			to update line_ptr as we pass thru
			compilation units in a .o .debug_line */

    unsigned long remaining_bytes = buffer_len; /* total length of original
		area left to be processed. Changes as we pass thru
		compilation units in a .o .debug_line */
	...
	*err_code = DW_DLE_ALLOC_FAIL;
    ...
    /*
	We must iterate thru each of a set of prologues
	and line data.
	We process each set in turn.
	If all pass, we update the passed-in buffer.
    */
    sec_res = DW_DLV_OK;
    ...
    /* all passed */ 
    if(did_change) {
	/* So update the passed in buffer 
	  orig_buffer is caller's input area.
	  orig_line_ptr is our modified copy of input area.
	*/
...
*/
static int cmpr(const void *lin, const void  *rin)
{
    ...
    Dwarf_Small **new_line_ptr) 
{
    ...
    *new_line_ptr = line_ptr_end;
	...
	Dwarf_Small *stmt_prog_entry_start = line_ptr;
		...
		instr_length = (Dwarf_Word)utmp3;
				...
				line_ptr,
				length_size);
			    /* Here we need to remember the offset
			       into the buffer
				and check to see if address went down.
			    */
    ...
    /* so now we have something to sort.
	First, finish off the last area record:
    */
    area_current->ala_length =  
		(line_ptr - orig_line_ptr) /* final offset*/
		  - area_current->ala_offset;

    /*  Build and sort a simple array of sections.
	Forcing a stable sort  by comparing on sequence number.
	We will use the sorted list to move sections of
	this part of the line table.
	Each 'section' starting with a
	DW_LNE_set_address opcode, on the
	assumption that such only get out of order where
	there was an ld-cord function rearrangement and that
	it is meaningful to restart the line info there.
    */
    {
	...
	*err_code = DW_DLE_ALLOC_FAIL;
	...
	memcpy(line_ptr,orig_line_ptr +
		ala_array[i].ala_offset,
		ala_array[i].ala_length);
