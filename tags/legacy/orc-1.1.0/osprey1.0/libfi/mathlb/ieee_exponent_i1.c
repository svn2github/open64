...
#pragma ident "@(#) libfi/mathlb/ieee_exponent_i1.c	92.1	07/09/99 11:00:36"
#include <fmath.h>
#include <fortran.h>
#ifdef	__mips
#include <fp_class.h>
...
/* IEEE_EXPONENT(X,Y_I1) returns integer(kind=1) result: */
#ifdef	_CRAYMPP
extern _f_int1 _IEEE_EXPONENT_I1_H(_f_real8 x);
#else	/* _CRAYMPP */
extern _f_int1 _IEEE_EXPONENT_I1_H(_f_real4 x);
#endif	/* _CRAYMPP */
extern _f_int1 _IEEE_EXPONENT_I1_R(_f_real8 x);
extern _f_int1 _IEEE_EXPONENT_I1_D(_f_real16 x);

#ifndef	__mips
#ifdef	_F_REAL4
/* _IEEE_EXPONENT_I1_H - IEEE EXPONENT returns the exponent part of the
 *                      32-bit argument in 8-bit integer.
 */
#ifdef	_CRAYMPP
static int __ieee_real4_i1(_f_real4 x);

_f_int1
_IEEE_EXPONENT_I1_H(_f_real8 x)
{
	int		i;
	REGISTER_8	s1;
...
/* _IEEE_EXPONENT_I1_R - IEEE EXPONENT returns the exponent part of the
 *                      64-bit argument in 8-bit integer.
 */
_f_int1
_IEEE_EXPONENT_I1_R(_f_real8 x)
{
	int		i;
	REGISTER_8	s1, s2;
...
#if _F_REAL16 == 1
/* _IEEE_EXPONENT_I1_D - IEEE EXPONENT returns the exponent part of the
 *                       128-bit argument in 8-bit integer.
 */
_f_int1
_IEEE_EXPONENT_I1_D(_f_real16 x)
{
...
_IEEE_EXPONENT_I1_H(_f_real4 x)
{
	...
	switch(fp_class_f(x)) {
		...
		case FP_NEG_DENORM:
			 ...
			 * be counted.
			 */
			return((_f_int1)(-IEEE_32_EXPO_BIAS -
