# 1 "../../common/targ_info/access/ti_latency.c"
# 1 "/usa/handong/simplnano/cmplr-open64-merge/targia32_sl1/targ_info//"
# 1 "<built-in>"
# 1 "<command line>"
# 1 "../../common/targ_info/access/ti_latency.c"
# 36 "../../common/targ_info/access/ti_latency.c"
...
# 37 "../../common/targ_info/access/ti_latency.c" 2
...
{
  return SI_RESOURCE_Name((*SI_resources_p)[id]);
}

static __inline UINT SI_RESOURCE_ID_Avail_Per_Cycle( SI_RESOURCE_ID id )
{
  return SI_RESOURCE_Avail_Per_Cycle((*SI_resources_p)[id]);
}
...
static __inline SI_RESOURCE_ID_SET SI_RESOURCE_ID_SET_Universe(void)
{
  ...
  >> (sizeof(SI_RESOURCE_ID_SET) * 8 - (*SI_resource_count_p));
}
...
{
  return (word_with_reservations & (*SI_RRW_overuse_mask_p)) != 0;
}
...
{
  SI* const info = (*SI_top_si_p)[(INT)opcode];
...
{
  SI* const info = (*SI_ID_si_p)[id];
  if ( ii > info->ii_info_size ) return info->resources_used;

  return info->ii_resources_used[ii - 1];
}
# 38 "../../common/targ_info/access/ti_latency.c" 2
# 1 "./targ_isa_properties.h" 1
# 52 "./targ_isa_properties.h"
extern const mUINT64 ISA_PROPERTIES_flags[];
# 39 "../../common/targ_info/access/ti_latency.c" 2
# 1 "./targ_isa_hazards.h" 1
# 83 "./targ_isa_hazards.h"
...
static __inline BOOL ISA_HAZARD_TOP_Has_Hazard(TOP topcode)
{
  extern mUINT8 ISA_HAZARD_hazard_index[764];
  return ISA_HAZARD_hazard_index[(INT)topcode] != 0;
}

static __inline ISA_HAZARD_INFO *ISA_HAZARD_First(TOP topcode)
{
  extern mUINT8 ISA_HAZARD_hazard_index[764];
...
static __inline ISA_HAZARD_INFO *ISA_HAZARD_Next(ISA_HAZARD_INFO *info)
{
  extern ISA_HAZARD_INFO ISA_HAZARD_hazard_info[1];
  INT index = info->next;
  return index ? ISA_HAZARD_hazard_info + index : (ISA_HAZARD_INFO *)0;
...
static __inline ISA_HAZARD ISA_HAZARD_Type(ISA_HAZARD_INFO *info)
{
  return info->type;
}

static __inline INT ISA_HAZARD_Data(ISA_HAZARD_INFO *info)
{
  return info->data;
}

static __inline INT ISA_HAZARD_Pre_Ops(ISA_HAZARD_INFO *info)
{
  return info->pre_ops;
}

static __inline INT ISA_HAZARD_Post_Ops(ISA_HAZARD_INFO *info)
{
  return info->post_ops;
}

extern void ISA_HAZARD_Initialize(void);
# 40 "../../common/targ_info/access/ti_latency.c" 2
# 1 "./targ_isa_subset.h" 1
# 41 "../../common/targ_info/access/ti_latency.c" 2
# 1 "../../common/targ_info/access/ti_errors.h" 1
# 81 "../../common/targ_info/access/ti_errors.h"
extern char TI_errmsg[80];
# 42 "../../common/targ_info/access/ti_latency.c" 2
# 1 "../../common/targ_info/access/ti_latency.h" 1
# 172 "../../common/targ_info/access/ti_latency.h"
static __inline INT TI_LATENCY_Load_Cycle(TOP opcode)
{
  return TSI_Load_Access_Time(opcode);
}

static __inline INT TI_LATENCY_Store_Cycle(TOP opcode)
{
  return TSI_Store_Available_Time(opcode);
}

static __inline INT TI_LATENCY_Operand_Access_Cycle(TOP opcode, INT opnd)
{
  return TSI_Operand_Access_Time(opcode, opnd);
}

static __inline INT TI_LATENCY_Result_Available_Cycle(TOP opcode, INT result)
{
  return TSI_Result_Available_Time(opcode, result);
}

static __inline INT TI_LATENCY_Last_Issue_Cycle(TOP opcode)
{
...
# 43 "../../common/targ_info/access/ti_latency.c" 2
# 57 "../../common/targ_info/access/ti_latency.c"
INT TI_LATENCY_Commit_Cycle(
  TOP opcode
)
{
  if ( opcode == TOP_intrncall ) return 1;
  if ( (ISA_PROPERTIES_flags[(INT)opcode] & 0x80000ULL) ) return 2;
  if ( (((BOOL) 0)) && (ISA_PROPERTIES_flags[(INT)opcode] & 0x40000000000000ULL) ) return 1;
  return 0;
}
# 75 "../../common/targ_info/access/ti_latency.c"
...
# 133 "../../common/targ_info/access/ti_latency.c"
...
# 183 "../../common/targ_info/access/ti_latency.c"
