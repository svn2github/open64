# 1 "../../libdwarf/dwarfdump/print_reloc.c"
# 1 "/usa/handong/simplnano/cmplr-open64-merge/targia32_sl1/libdwarf//"
# 1 "<built-in>"
# 1 "<command line>"
# 1 "../../libdwarf/dwarfdump/print_reloc.c"
# 40 "../../libdwarf/dwarfdump/print_reloc.c"
# 1 "../../libdwarf/dwarfdump/globals.h" 1
# 42 "../../libdwarf/dwarfdump/globals.h"
# 1 "../../libdwarf/dwarfdump/config.h" 1
# 43 "../../libdwarf/dwarfdump/globals.h" 2
# 64 "../../libdwarf/dwarfdump/globals.h"
...
# 65 "../../libdwarf/dwarfdump/globals.h" 2
# 83 "../../libdwarf/dwarfdump/globals.h"
...
# 84 "../../libdwarf/dwarfdump/globals.h" 2
...
# 85 "../../libdwarf/dwarfdump/globals.h" 2
...
# 86 "../../libdwarf/dwarfdump/globals.h" 2
...
# 88 "../../libdwarf/dwarfdump/globals.h" 2
...
# 91 "../../libdwarf/dwarfdump/globals.h" 2
...
# 97 "../../libdwarf/dwarfdump/globals.h" 2
...
int dwarf_attr_offset(Dwarf_Die ,
    ...
    Dwarf_Off * ,
    Dwarf_Error * );
# 98 "../../libdwarf/dwarfdump/globals.h" 2

typedef char * string;
typedef int boolean;
# 111 "../../libdwarf/dwarfdump/globals.h"
typedef struct {
    int checks;
    int errors;
} Dwarf_Check_Result;
...
extern void print_one_die(
 ...
 char **srcfiles,
 Dwarf_Signed cnt);
# 41 "../../libdwarf/dwarfdump/print_reloc.c" 2
# 61 "../../libdwarf/dwarfdump/print_reloc.c"
...
static char *error_msg_duplicate[ ] = {
    ".rel.debug_info" " duplicate",
    ".rel.debug_line" " duplicate",
    ".rel.debug_pubname" " duplicate",
    ".rel.debug_abbrev" " duplicate",
    ".rel.debug_aranges" " duplicate",
    ".rel.debug_frame" " duplicate",
};

static char *error_msg_null[ ] = {
    ".rel.debug_info" " null",
    ".rel.debug_line" " null",
    ".rel.debug_pubname" " null",
    ".rel.debug_abbrev" " null",
    ".rel.debug_aranges" " null",
    ".rel.debug_frame" " null",
};
# 99 "../../libdwarf/dwarfdump/print_reloc.c"
...
void print_relocinfo (Dwarf_Debug dbg)
{
 ...
 print_error(dbg, "dwarf_get_elf error",res, err);
    ...
    if ((endr_ident = elf_getident(elf, ((void *)0))) == ((void *)0)) {
 print_error(dbg, "DW_ELF_GETIDENT_ERROR",res, err);
    }
    is_64bit = (endr_ident[4] == 2);
    if (is_64bit) {
...
static void print_relocinfo_64 (Dwarf_Debug dbg, Elf *elf)
{
    ...
    if ((ehdr64 = elf64_getehdr(elf)) == ((void *)0)) {
 print_error(dbg, "DW_ELF_GETEHDR_ERROR",0, err);
    }

    while ((scn = elf_nextscn(elf, scn)) != ((void *)0)) {

 if ((shdr64 = elf64_getshdr(scn)) == ((void *)0)) {
     print_error(dbg, "DW_ELF_GETSHDR_ERROR",0, err);
 }
 if ((scn_name = elf_strptr(elf,ehdr64->e_shstrndx, shdr64->sh_name))
     == ((void *)0)) {
     print_error(dbg, "DW_ELF_STRPTR_ERROR",0, err);
 }
 if (shdr64->sh_type == 2) {
     size_t sym_size = 0;
     size_t count = 0;
     if ((sym_64 = (Elf64_Sym *) get_scndata(scn, &sym_size)) == ((void *)0)) {
  print_error(dbg, "no symbol table data",0, err);
     }
     count = sym_size / sizeof (Elf64_Sym);
     sym_64 ++;
     if ((sym_data_64 = read_64_syms(sym_64, count, elf,
         shdr64->sh_link)) == ((void *)0)) {
  print_error(dbg, "problem reading symbol table data",0, err);
     }
 }
 else if (strncmp(scn_name, ".rel.debug_", 11))
     continue;
 else if (strcmp(scn_name, ".rel.debug_info") == 0) {
     { if (sect_data[(0)].buf != ((void *)0)) { print_error(dbg, error_msg_duplicate[(0)],0, err); } if ((data = elf_getdata(scn, 0)) == ((void *)0) || data->d_size == 0) { print_error(dbg, error_msg_null[(0)],0, err); } sect_data[(0)].buf = data -> d_buf; sect_data[(0)].size = data -> d_size; }
 }
 else if (strcmp(scn_name, ".rel.debug_line") == 0) {
     { if (sect_data[(1)].buf != ((void *)0)) { print_error(dbg, error_msg_duplicate[(1)],0, err); } if ((data = elf_getdata(scn, 0)) == ((void *)0) || data->d_size == 0) { print_error(dbg, error_msg_null[(1)],0, err); } sect_data[(1)].buf = data -> d_buf; sect_data[(1)].size = data -> d_size; }
 }
 else if (strcmp(scn_name, ".rel.debug_pubname") == 0) {
     { if (sect_data[(2)].buf != ((void *)0)) { print_error(dbg, error_msg_duplicate[(2)],0, err); } if ((data = elf_getdata(scn, 0)) == ((void *)0) || data->d_size == 0) { print_error(dbg, error_msg_null[(2)],0, err); } sect_data[(2)].buf = data -> d_buf; sect_data[(2)].size = data -> d_size; }
 }
 else if (strcmp(scn_name, ".rel.debug_aranges") == 0) {
     { if (sect_data[(4)].buf != ((void *)0)) { print_error(dbg, error_msg_duplicate[(4)],0, err); } if ((data = elf_getdata(scn, 0)) == ((void *)0) || data->d_size == 0) { print_error(dbg, error_msg_null[(4)],0, err); } sect_data[(4)].buf = data -> d_buf; sect_data[(4)].size = data -> d_size; }
 }
 else if (strcmp(scn_name, ".rel.debug_abbrev") == 0) {
     { if (sect_data[(3)].buf != ((void *)0)) { print_error(dbg, error_msg_duplicate[(3)],0, err); } if ((data = elf_getdata(scn, 0)) == ((void *)0) || data->d_size == 0) { print_error(dbg, error_msg_null[(3)],0, err); } sect_data[(3)].buf = data -> d_buf; sect_data[(3)].size = data -> d_size; }
 }
 else if (strcmp(scn_name, ".rel.debug_frame") == 0) {
     { if (sect_data[(5)].buf != ((void *)0)) { print_error(dbg, error_msg_duplicate[(5)],0, err); } if ((data = elf_getdata(scn, 0)) == ((void *)0) || data->d_size == 0) { print_error(dbg, error_msg_null[(5)],0, err); } sect_data[(5)].buf = data -> d_buf; sect_data[(5)].size = data -> d_size; }
 }
    }

    for (i = 0; i < 6; i ++) {
 if (sect_data[i].buf != ((void *)0) && sect_data[i].size > 0) {
...
static void print_relocinfo_32 (Dwarf_Debug dbg, Elf *elf)
{
    ...
    if ((ehdr32 = elf32_getehdr(elf)) == ((void *)0)) {
 print_error(dbg, "DW_ELF_GETEHDR_ERROR",0, err);
    }
    while ((scn = elf_nextscn(elf, scn)) != ((void *)0)) {
 if ((shdr32 = elf32_getshdr(scn)) == ((void *)0)) {
     print_error(dbg, "DW_ELF_GETSHDR_ERROR",0, err);
 }
 ...
 if (shdr32->sh_type == 2) {
     size_t sym_size = 0;
     size_t count = 0;
     if ((sym = (Elf32_Sym *) get_scndata(scn, &sym_size)) == ((void *)0)) {
  print_error(dbg, "no symbol table data",0, err);
     }
...
# 371 "../../libdwarf/dwarfdump/print_reloc.c"
static void print_reloc_information_64(int section_no, Dwarf_Small *buf,
           Dwarf_Unsigned size)
{
    Dwarf_Unsigned off;
 ...
 printf("%5lu\t<%3lld> %-34s%s\n", (unsigned long int) (p->r_offset),
        (long long) ((p->r_info) >> 32),
        sym_data[((p->r_info) >> 32) - 1].name,
        get_reloc_type_names(((p->r_info) & 0xffffffff)));
# 398 "../../libdwarf/dwarfdump/print_reloc.c"
    }
           ...
           Dwarf_Unsigned size)
{
 ...
 printf("%5lu\t<%3d> %-34s%s\n", (unsigned long int) (p->r_offset),
        ((p->r_info) >> 8),
        sym_data[((p->r_info) >> 8) - 1].name,
        get_reloc_type_names(((p->r_info) & 0xff)));
    }
}
...
readsyms(Elf32_Sym *data, size_t num, Elf *elf, Elf32_Word link)
{
    SYM *s, *buf;
    size_t i;
    if((buf = (SYM *)calloc(num, sizeof(SYM)) ) == ((void *)0)) {
 return ((void *)0);
    }
    s = buf;
 ...
 buf->type = ((data->st_info) & 0xf);
 buf->bind = (((unsigned char) (data->st_info)) >> 4);
...
read_64_syms(Elf64_Sym *data, size_t num, Elf *elf, Elf64_Word link)
{
    ...
    SYM64 *s, *buf;
    size_t i;
    if( (buf = (SYM64 *)calloc(num, sizeof(SYM64)) ) == ((void *)0)) {
 return ((void *)0);
    }
    s = buf;
    for(i=1; i<num; i++,data++,buf++) {
