# 1 "../../libcmplrs/make_depend.c"
# 1 "/usa/handong/simplnano/cmplr-open64-merge/targia32_sl1/libcmplrs//"
# 1 "<built-in>"
# 1 "<command line>"
# 1 "../../libcmplrs/make_depend.c"
# 90 "../../libcmplrs/make_depend.c"
...
# 99 "../../libcmplrs/make_depend.c" 2

# 1 "../../include/cmplrs/make_depend.h" 1
# 51 "../../include/cmplrs/make_depend.h"
typedef struct mdrule *MDhandle;
# 70 "../../include/cmplrs/make_depend.h"
extern MDhandle MDopen(char *, char *, char *, void (*)(char *, ...));
extern void MDupdate(MDhandle, char *);
extern void MDclose(MDhandle, char *);
# 101 "../../libcmplrs/make_depend.c" 2
# 133 "../../libcmplrs/make_depend.c"
char scratch[80];
...
# 264 "../../libcmplrs/make_depend.c"
void
MDnewstab(MDhandle h)
{
   h->stab = malloc((0x40000));
   h->stabcur = h->stab;
   h->stabmax = h->stab + (0x40000);
}
# 283 "../../libcmplrs/make_depend.c"
char *
...
MDopen(char *toolname, char *filename, char *target, void (*error)(char*,...))
{
 ...
 h = malloc(sizeof *h);
 MDnewstab(h);
 table = calloc((1 << 8), sizeof *h->table);
 if (h == 0 || table == 0) {
 ...
 *tool = 0;
 next = line;
# 538 "../../libcmplrs/make_depend.c"
 while ( next<limit && *next==' ' ) {next++;}; if ( (next>=limit) ) { return 0; }
...
# 557 "../../libcmplrs/make_depend.c"
 *targ = next;


 while ( next<limit && *next!=':' && *next!='\n') {next++;}; if ( *next != ':' ) return 0;


 next++; while ( next<limit && *next==' ' ) {next++;};


 while ( next<limit && *next!='#' && *next!='\n') {next++;}; if ( (next>=limit) ) { return 0; }
...
MDclose(MDhandle h, char *target)
{
 ...
 ;
 md.f = open(md.filename, 0100|02, 0666);
 if ( md.f < 0 ) { h->error ("open (%s): %s", md.filename, strerror((*__errno_location ()))) ; goto cleanup; };

 l.l_type = 1;
 l.l_whence = 0;
 l.l_start = l.l_len = 0;
 if ( fcntl(md.f, 7, &l) < 0 )
  { h->error ("fcntl (%s) : %s", md.filename, strerror((*__errno_location ()))) ; goto cleanup; };
 if ( fstat(md.f,&md.stb) < 0 )
  { h->error ("fstat (%s) : %s", md.filename, strerror((*__errno_location ()))) ; goto cleanup; };
 ;
 ...
 md.base = md.limit = 0;
 if ( 0 < md.stb.st_size ) {
  md.base = mmap ( 0, md.stb.st_size, 0x1|0x2
    , 0x01, md.f, 0);
  if ((int) md.base < 0)
   { h->error ("mmap (%s): %s", md.filename, strerror((*__errno_location ()))) ; goto cleanup; };
 ...
 ;
  ...
  if ( 0 == nextline ) { h->error ("%s: damaged file", md.filename) ; goto cleanup; };
...
# 745 "../../libcmplrs/make_depend.c"
 if ( found ) {
  ...
  if ( 0 != ftruncate(md.f, (md.size-linelen) ) )
     { h->error ("ftruncate (%s): %s", md.filename, strerror((*__errno_location ()))) ; goto cleanup; };
 }
# 766 "../../libcmplrs/make_depend.c"
 ;
 if ( 0 == (file = fdopen(md.f,"a")))
  { h->error ("fdopen(%s): %s", md.filename, strerror((*__errno_location ()))) ; goto cleanup; };
 ...
 if ( 0 > fprintf( file, " #:%s\n", h->toolname ) )
  { h->error ("fprintf(%s): %s", md.filename, strerror((*__errno_location ()))) ; goto cleanup; };
