...
# 32 "/usr/include/sys/procfs.h" 3 4
...
# 73 "/usr/include/sys/procfs.h" 3 4
struct elf_prstatus
  {
...
boolean show_flag = 0;
boolean show_but_not_run = 0;
boolean execute_flag = 1;
boolean time_flag = 0;
boolean prelink_flag = 1;
boolean quiet_flag = 1;
int memory_flag = 0;
boolean run_m4 = 0;
static boolean ran_twice = 0;
static int f90_fe_status = 0;
static char *f90_fe_name = ((void *)0);
...
run_simple_program (char *name, char **argv, char *output)
{
 ...
 if (forkpid == 0) {
  ...
  if (((((__extension__ ({ union { __typeof(waitstatus) __in; int __i; } __u; __u.__in = (waitstatus); __u.__i; }))) & 0xff) == 0x7f)) {
   termsig = ((((__extension__ ({ union { __typeof(waitstatus) __in; int __i; } __u; __u.__in = (waitstatus); __u.__i; }))) & 0xff00) >> 8);
   error("STOPPED signal received from %s", name);
   cleanup();
   do_exit(32);

  } else if (((((__extension__ ({ union { __typeof(waitstatus) __in; int __i; } __u; __u.__in = (waitstatus); __u.__i; }))) & 0x7f) == 0)) {
          int status = ((((__extension__ ({ union { __typeof(waitstatus) __in; int __i; } __u; __u.__in = (waitstatus); __u.__i; }))) & 0xff00) >> 8);
   if (status != 0) {
   ...
   return;
  } else if((((signed char) ((((__extension__ ({ union { __typeof(waitstatus) __in; int __i; } __u; __u.__in = (waitstatus); __u.__i; }))) & 0x7f) + 1) >> 1) > 0)){
   termsig = (((__extension__ ({ union { __typeof(waitstatus) __in; int __i; } __u; __u.__in = (waitstatus); __u.__i; }))) & 0x7f);
                        switch (termsig) {
                        case 1:
                        case 2:
                        case 3:
   ...
   if (termsig == 9) {
    error("Probably caused by running out of swap space -- check %s", "/var/log/messages");
   }
   cleanup();
   do_exit(32);
  } else {
...
run_phase (phases_t phase, char *name, string_list_t *args)
{
 ...
 argc = 1;
 for (p = args->head; p != ((void *)0); p=p->next) {
 ...
 argv[0] = name;
 for (argc = 1, p = args->head; p != ((void *)0); argc++, p=p->next) {
...
# 429 "../../driver/run.c"
  ...
  if (uses_message_system && getenv("ORIG_CMD_NAME") == ((void *)0))
...
# 531 "../../driver/run.c"
  ...
  if (((((__extension__ ({ union { __typeof(waitstatus) __in; int __i; } __u; __u.__in = (waitstatus); __u.__i; }))) & 0xff) == 0x7f)) {
   ...
   if (phase == P_f90_fe && keep_listing) {
       ...
       drop_path(source_file), "T", 1);
                            if (!(stat(cif_file, &stat_buf) != 0 && (*__errno_location ()) == 2))
...
catch_signals (void)
{
    ...
    if (signal (3, ((__sighandler_t) 1)) != ((__sighandler_t) 1))
        signal (3, handler);
    if (signal (4, ((__sighandler_t) 1)) != ((__sighandler_t) 1))
        signal (4, handler);
    if (signal (5, ((__sighandler_t) 1)) != ((__sighandler_t) 1))
        signal (5, handler);
    if (signal (6, ((__sighandler_t) 1)) != ((__sighandler_t) 1))
        signal (6, handler);
    ...
    if (signal (8, ((__sighandler_t) 1)) != ((__sighandler_t) 1))
        signal (8, handler);
    if (signal (7, ((__sighandler_t) 1)) != ((__sighandler_t) 1))
        signal (7, handler);
    if (signal (11, ((__sighandler_t) 1)) != ((__sighandler_t) 1))
        signal (11, handler);
...
print_time (char *phase)
{
    ...
    utime = (double)(tm1.tms_utime + tm1.tms_cutime -
       tm0.tms_utime - tm0.tms_cutime) / (double)100;
    stime = (double)(tm1.tms_stime + tm1.tms_cstime -
       tm0.tms_stime - tm0.tms_cstime) / (double)100;
    wtime = time1 - time0;

    fprintf (stderr, "%s phase time:  %.2fu %.2fs %lu:%04.1f %.0f%%\n",
  phase, utime, stime, wtime / (60*100),
  (double)(wtime % (60*100)) / (double)100,
  (utime + stime) / ((double)wtime / (double)100) * 100.0);
}
