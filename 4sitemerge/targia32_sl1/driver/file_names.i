...
# 59 "../../driver/file_names.c" 2
# 1 "../../driver/version.h" 1
# 25 "../../driver/version.h"
...
# 60 "../../driver/file_names.c" 2

extern int (*__errno_location ());

boolean keep_flag = 0;

string_list_t *count_files = ((void *)0);
static string_list_t *temp_files = ((void *)0);
static char *tmpdir;
static char *saved_object = ((void *)0);



static string_pair_list_t *temp_obj_files = ((void *)0);
...
get_object_file (char *src)
{
 ...
 if (!(keep_flag || (ipa == 1) || remember_last_phase == P_any_as)) {
   char *obj_name = change_suffix(src, "o");
   string_pair_item_t *p;
   for (p = temp_obj_files->head; p != ((void *)0); p = p->next) {
     if (strcmp ((p->key), obj_name) == 0)
       return (p->val);
   }
 ...
 if (ipa == 1 &&
     ...
     keep_flag != 1) {
   char *p;
   src = (__extension__ ({ __const char *__old = (src); size_t __len = strlen (__old) + 1; char *__new = (char *) __builtin_alloca (__len); (char *) memcpy (__new, __old, __len); }));
   for (p = src; *p != '\0'; p++) {
     if (*p == '/')
       *p = '%';
   }
...
create_temp_file_name (char *suffix)
{
 ...
 for (p = temp_files->head; p != ((void *)0); p = p->next) {
# 146 "../../driver/file_names.c"
...
construct_name (char *src, char *suffix)
{
 if (keep_flag || current_phase == remember_last_phase) {
  ...
  if (outfile && option_was_seen(501) && get_suffix(outfile))
   srcname = outfile;
  else
   srcname = src;
...
{
 if ((report_file = getenv("PSC_CRASH_REPORT")) != ((void *)0))
  goto bail;
...
save_cpp_output (char *path)
{
 ...
 if ((ifp = fopen(path, "r")) == ((void *)0))
  goto bail;

 if ((save_dir = getenv("PSC_PROBLEM_REPORT_DIR")) == ((void *)0) &&
     (home = getenv("HOME")) != ((void *)0)) {
  asprintf(&save_dir, "%s/.ekopath-bugs", home);
 }

 if (save_dir && mkdir(save_dir, 0700) == -1 && (*__errno_location ()) != 17) {
  save_dir = ((void *)0);
 }
 ...
 if ((ofp = fopen(save_path, "w")) == ((void *)0)) {
  goto b0rked;
 }
 ...
 if (report_file) {
  ...
  if ((rfp = fopen(report_file, "r")) == ((void *)0)) {
   goto no_report;
  }

  while (fgets(buf, sizeof(buf), rfp) != ((void *)0)) {
   int len = strlen(buf);
   if (newline)
    fputs("  ", ofp);
   fputs(buf, ofp);
...
no_report:
 if (string_list_size(error_list)) {
  ...
  for (i = error_list->head; i != ((void *)0); i = i->next) {
   fprintf(ofp, "  %s\n", (i->name));
  }
 }
...
b0rked:
 fprintf(stderr, "Could not save problem report to %s: %s\n",
  save_path, strerror((*__errno_location ())));
bail:
 if (ifp != ((void *)0))
  fclose(ifp);
 if (ofp != ((void *)0))
...
cleanup (void)
{
 ...
 for (p = temp_files->head; p != ((void *)0); p = p->next) {
  ...
  if (execute_flag) {
   ...
   if (status != 0 && (*__errno_location ()) != 2) {
    internal_error("cannot unlink temp file %s", p->name);
    perror(program_name);
   }
  }
 }
 temp_files->head = temp_files->tail = ((void *)0);
