...
# 106 "../../../kgccfe/gnu/bb-reorder.c" 2


static void make_reorder_chain (void);
static basic_block make_reorder_chain_1 (basic_block, basic_block);
static basic_block maybe_duplicate_computed_goto_succ (basic_block);
...
# 133 "../../../kgccfe/gnu/bb-reorder.c"
 ...
 prev = make_reorder_chain_1 (next, prev);
  ...
  while (next);
  ((reorder_block_def) (prev)->aux)->next = ((void *)0);
}
     ...
     basic_block bb;
{
  ...
  next = e->dest;
  if (!((reorder_block_def) (next)->aux)->visited)
    return ((void *)0);
     ...
     basic_block prev;
{
  ...
  if (prev)
    {
 restart:
      ((reorder_block_def) (prev)->aux)->next = bb;
 ...
 fprintf (rtl_dump_file, "Reordering block %d after %d\n",
   bb->index, prev->index);
    ...
    {
      if (bb->prev_bb != (&entry_exit_blocks[0]))
 fancy_abort ("../../../kgccfe/gnu/bb-reorder.c", 219, __FUNCTION__);
    }
  ...
  if (any_condjump_p (bb->end)
      && (note = find_reg_note (bb->end, REG_BR_PROB, 0)) != ((void *)0))
    {
      int taken, probability;
      edge e_taken, e_fall;

      probability = ((((((note)->fld[0]).rtx))->fld[0]).rtwint);
      taken = probability > 10000 / 2;
# 250 "../../../kgccfe/gnu/bb-reorder.c"
 ...
 if (e->flags & 1)
  ...
  if (! next || next == (&entry_exit_blocks[1]) || ((reorder_block_def) (next)->aux)->visited)
    next = ((void *)0);
 ...
 if (next)
   {
     prev = make_reorder_chain_1 (next, prev);
     next = ((reorder_block_def) (e->dest)->aux)->visited ? ((void *)0) : e->dest;
   }
 else
   next = e->dest;
      }
