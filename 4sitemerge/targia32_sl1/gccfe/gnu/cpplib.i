...
static void directive_diagnostics
 (cpp_reader *, const directive *, int);
static void run_directive (cpp_reader *, int, const char *, size_t);

static const cpp_token *glue_header_name (cpp_reader *);
static const cpp_token *parse_include (cpp_reader *);
...
static struct pragma_entry *lookup_pragma_entry
  (struct pragma_entry *, const cpp_hashnode *pragma);
static struct pragma_entry *insert_pragma_entry
  (cpp_reader *, struct pragma_entry **, const cpp_hashnode *, pragma_cb);
     ...
     cpp_reader *pfile;
{
  if (pfile->directive != &dtable[T_DEFINE])
    {
      _Bool no_expand = (pfile->directive
   && ! (pfile->directive->flags & (1 << 4)));
      _Bool was_skipping = pfile->state.skipping;
     ...
     int indented;
{
  ...
  _Bool was_parsing_args = pfile->state.parsing_args;
  int skip = 1;
     ...
     cpp_reader *pfile;
{
 ...
 (*pfile->cb.undef) (pfile, pfile->directive_line, node);
      ...
      if (node->flags & (1 << 4))
 cpp_error (pfile, 0x00, "undefining \"%s\"", ((&(node)->ident)->str));
     ...
     cpp_reader *pfile;
{
 ...
 return header;
  ...
  if (header->val.str.len == 0)
    {
      cpp_error (pfile, 0x03, "empty file name in #%s", dir);
      return ((void *)0);
    }
     ...
     enum include_type type;
{
  ...
  if (type == IT_INCLUDE_NEXT && ! pfile->buffer->prev)
    {
      cpp_error (pfile, 0x00, "#include_next in primary source file");
      type = IT_INCLUDE;
    }
  else if (type == IT_IMPORT && ((pfile)->opts.warn_import))
    {
      ((pfile)->opts.warn_import) = 0;
      cpp_error (pfile, 0x00,
     ...
     cpp_reader *pfile;
{
  ...
  if (token->type != CPP_NUMBER
      ...
      &new_lineno))
    {
      cpp_error (pfile, 0x03, "\"%s\" after # is not a positive integer",
   cpp_token_as_text (pfile, token));
      return;
    }
...
do_pragma_once (pfile)
     cpp_reader *pfile;
{
  if (((pfile)->opts.warn_deprecated))
    cpp_error (pfile, 0x00, "#pragma once is obsolete");
     ...
     cpp_reader *pfile;
{
 ...
 {
   cpp_error (pfile, 0x03, "invalid #pragma GCC poison directive");
   break;
 }
     ...
     cpp_reader *pfile;
{
  ...
  if (ordering < 0)
    cpp_error (pfile, 0x00, "cannot find source %s",
        cpp_token_as_text (pfile, header));
  else if (ordering > 0)
    {
      cpp_error (pfile, 0x00, "current file is older than %s",
   cpp_token_as_text (pfile, header));
     ...
     const cpp_string *in;
{
  ...
  dest = result = __builtin_alloca(in->len + 1);
...
static void
do_sccs (pfile)
     cpp_reader *pfile __attribute__ ((__unused__));
{
}
     ...
     cpp_reader *pfile;
{
      ...
      if (ifs->next == 0 && ifs->mi_cmacro)
 {
   ...
   pfile->mi_cmacro = ifs->mi_cmacro;
      ...
      __extension__ ({ struct obstack *__o = (&pfile->buffer_ob); void *__obj = (ifs); if (__obj > (void *)__o->chunk && __obj < (void *)__o->chunk_limit) __o->next_free = __o->object_base = __obj; else (obstack_free) (__o, __obj); });
    }
}
  ...
  *answerp = 0;
  ...
  if (predicate->type == CPP_EOF)
  ...
  else if (parse_answer (pfile, answerp, type) == 0)
    {
      ...
      unsigned char *sym = __builtin_alloca(len + 1);
     ...
     cpp_reader *pfile;
{
      ...
      if (node->type == NT_ASSERTION)
 {
   ...
   new_answer->next = node->value.answers;
      ...
      ((pfile->a_buff)->cur) += (sizeof (struct answer)
         + (new_answer->count - 1)
         * sizeof (cpp_token));
      check_eol (pfile);
    }
     ...
     const char *str;
{
  ...
  count = strlen (str);
  buf = (char *) __builtin_alloca(count + 3);
  ...
  if (p)
    {
      ...
      char *buf = (char *) __builtin_alloca(count + 2);
...
cpp_push_buffer (pfile, buffer, len, from_stage3, return_at_eof)
     ...
     int return_at_eof;
{
  cpp_buffer *new = (cpp_buffer *) __extension__ ({ struct obstack *__h = (&pfile->buffer_ob); __extension__ ({ struct obstack *__o = (__h); int __len = ((sizeof(cpp_buffer))); if (__o->chunk_limit - __o->next_free < __len) _obstack_newchunk (__o, __len); __o->next_free += __len; (void) 0; }); __extension__ ({ struct obstack *__o1 = (__h); void *value; value = (void *) __o1->object_base; if (__o1->next_free == value) __o1->maybe_empty_object = 1; __o1->next_free = (((((__o1->next_free) - (char *) 0)+__o1->alignment_mask) & ~ (__o1->alignment_mask)) + (char *) 0); if (__o1->next_free - (char *)__o1->chunk > __o1->chunk_limit - (char *)__o1->chunk) __o1->next_free = __o1->chunk_limit; __o1->object_base = __o1->next_free; value; }); });
  ...
  new->from_stage3 = from_stage3 || ((pfile)->opts.traditional);
     ...
     cpp_reader *pfile;
{
  ...
  __extension__ ({ struct obstack *__o = (&pfile->buffer_ob); void *__obj = (buffer); if (__obj > (void *)__o->chunk && __obj < (void *)__o->chunk_limit) __o->next_free = __o->object_base = __obj; else (obstack_free) (__o, __obj); });
