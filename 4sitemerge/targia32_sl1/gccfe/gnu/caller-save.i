...
# 31 "../../../kgccfe/gnu/caller-save.c" 2
# 1 "../../../kgccfe/gnu/insn-config.h" 1
# 32 "../../../kgccfe/gnu/caller-save.c" 2
...
# 42 "../../../kgccfe/gnu/caller-save.c" 2


# 1 "../../../kgccfe/gnu/defaults.h" 1
# 45 "../../../kgccfe/gnu/caller-save.c" 2
# 62 "../../../kgccfe/gnu/caller-save.c"
static enum machine_mode
  regno_save_mode[176][8 / 4 + 1];




static rtx
  regno_save_mem[176][8 / 4 + 1];
...
static int
  reg_save_code[176][MAX_MACHINE_MODE];
static int
  reg_restore_code[176][MAX_MACHINE_MODE];
...
init_caller_save ()
{
 ...
 {
   for (j = 1; j <= (((target_flags & 0x00002000) ? 8 : 4) / ((target_flags & 0x00002000) ? 8 : 4)); j++)
     {
...
# 166 "../../../kgccfe/gnu/caller-save.c"
  for (i = 0; i < 176; i++)
    if ((!!((reg_class_contents [(int) ((target_flags & 0x00100000) ? M16_REGS : GR_REGS)])[(i) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((i) % ((unsigned) (8 * 8)))))))


      break;

  if (i == 176)
    fancy_abort ("../../../kgccfe/gnu/caller-save.c", 173, __FUNCTION__);

  addr_reg = gen_rtx_REG ((((target_flags & 0x00000002) && (target_flags & 0x00002000)) ? DImode : SImode), i);

  for (offset = 1 << ((8 * 4) / 2); offset; offset >>= 1)
    {
      address = gen_rtx_fmt_ee (PLUS, ((((target_flags & 0x00000002) && (target_flags & 0x00002000)) ? DImode : SImode)), (addr_reg), (gen_rtx_CONST_INT (VOIDmode, (long long) (offset))));
 ...
 break;
  ...
  saveinsn = gen_rtx_fmt_iuuBteiee (INSN, (VOIDmode), (0), (0), (0), (0), (0), (savepat), (-1), (0), (0));
  restinsn = gen_rtx_fmt_iuuBteiee (INSN, (VOIDmode), (0), (0), (0), (0), (0), (restpat), (-1), (0), (0));
  ...
  for (i = 0; i < 176; i++)
   ...
   (((test_reg)->fld[0]).rtuint) = i;
   ((test_reg)->mode = (enum machine_mode) (mode));
   ((test_mem)->mode = (enum machine_mode) (mode));


   (((saveinsn)->fld[6]).rtint) = -1;
   (((restinsn)->fld[6]).rtint) = -1;

   reg_save_code[i][mode] = ((((saveinsn)->fld[6]).rtint) >= 0 ? (((saveinsn)->fld[6]).rtint) : recog_memoized_1 (saveinsn));
   reg_restore_code[i][mode] = ((((restinsn)->fld[6]).rtint) >= 0 ? (((restinsn)->fld[6]).rtint) : recog_memoized_1 (restinsn));
...
setup_save_areas ()
{
  ...
  do { HARD_REG_ELT_TYPE *scan_tp_ = (hard_regs_used); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
  for (i = 176; i < max_regno; i++)
    if (reg_renumber[i] >= 0 && (((reg_n_info)->data.reg[i])->calls_crossed) > 0)
      {
 unsigned int regno = reg_renumber[i];
 unsigned int endregno
   = regno + mips_hard_regno_nregs (regno, ((enum machine_mode) ((cfun->emit->x_regno_reg_rtx)[i])->mode));

 for (r = regno; r < endregno; r++)
   if (call_used_regs[r])
     ((hard_regs_used)[(r) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((r) % ((unsigned) (8 * 8))));
      }
  ...
  for (i = 0; i < 176; i++)
    for (j = (((target_flags & 0x00002000) ? 8 : 4) / ((target_flags & 0x00002000) ? 8 : 4)); j > 0; j--)
      {
 int do_save = 1;
 ...
 for (k = 0; k < j; k++)
   if (! (!!((hard_regs_used)[(i + k) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((i + k) % ((unsigned) (8 * 8)))))))
     {
       do_save = 0;
       break;
     }
 ...
 regno_save_mem[i][j]
   = assign_stack_local (regno_save_mode[i][j],
    (mode_size[(int) (regno_save_mode[i][j])]), 0);
 ...
 for (k = 0; k < j; k++)
   ...
   regno_save_mem[i + k][1]
     = adjust_address_1 (regno_save_mem[i][j], regno_save_mode[i + k][1], k * ((target_flags & 0x00002000) ? 8 : 4), 0, 1);
...
save_call_clobbered_regs ()
{
  ...
  enum machine_mode save_mode [176];

  do { HARD_REG_ELT_TYPE *scan_tp_ = (hard_regs_saved); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
  n_regs_saved = 0;
 ...
 fancy_abort ("../../../kgccfe/gnu/caller-save.c", 394, __FUNCTION__);
      ...
      if ((rtx_class[(int) (code)]) == 'i')
 {
  ...
  do { HARD_REG_ELT_TYPE *scan_tp_ = (referenced_regs), *scan_fp_ = (hard_regs_saved); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
       else
  {
    do { HARD_REG_ELT_TYPE *scan_tp_ = (referenced_regs); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
    mark_referenced_regs ((((insn)->fld[5]).rtx));
    do { HARD_REG_ELT_TYPE *scan_tp_ = (referenced_regs), *scan_fp_ = (hard_regs_saved); scan_tp_[0] &= scan_fp_[0]; scan_tp_[1] &= scan_fp_[1]; scan_tp_[2] &= scan_fp_[2]; } while (0);
  }

       for (regno = 0; regno < 176; regno++)
  if ((!!((referenced_regs)[(regno) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((regno) % ((unsigned) (8 * 8)))))))
    regno += insert_restore (chain, 1, regno, (((target_flags & 0x00002000) ? 8 : 4) / ((target_flags & 0x00002000) ? 8 : 4)), save_mode);
     }
   ...
   if (code == CALL_INSN)
     {
       ...
       do { do { HARD_REG_ELT_TYPE *scan_tp_ = (hard_regs_to_save); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0); reg_set_to_hard_reg_set (&hard_regs_to_save, &chain->live_throughout); } while (0);


       for (regno = 0; regno < 176; regno++)
  if ((!!((hard_regs_to_save)[(regno) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((regno) % ((unsigned) (8 * 8)))))))
    ...
    save_mode [regno] = VOIDmode;
       ...
       do { bitmap_element *ptr_ = (&chain->live_throughout)->first; unsigned int indx_ = (176) / ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))); unsigned bit_num_ = (176) % (unsigned) (8 * 4); unsigned word_num_ = (176) / (unsigned) (8 * 4) % ((128 + (8 * 4) - 1) / (8 * 4)); while (ptr_ != 0 && ptr_->indx < indx_) ptr_ = ptr_->next; if (ptr_ != 0 && ptr_->indx != indx_) { bit_num_ = 0; word_num_ = 0; } for (; ptr_ != 0; ptr_ = ptr_->next) { for (; word_num_ < ((128 + (8 * 4) - 1) / (8 * 4)); word_num_++) { BITMAP_WORD word_ = ptr_->bits[word_num_]; if (word_ != 0) { for (; bit_num_ < (unsigned) (8 * 4); bit_num_++) { BITMAP_WORD mask_ = ((BITMAP_WORD) 1) << bit_num_; if ((word_ & mask_) != 0) { word_ &= ~ mask_; (regno) = (ptr_->indx * ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))) + word_num_ * (unsigned) (8 * 4) + bit_num_); { int r = reg_renumber[regno]; int nregs; if (r >= 0) { enum machine_mode mode; nregs = mips_hard_regno_nregs (r, ((enum machine_mode) ((cfun->emit->x_regno_reg_rtx)[regno])->mode)); mode = choose_hard_reg_mode (r, nregs); if ((mode_bitsize[(int) (mode)]) > (mode_bitsize[(int) (save_mode[r])])) save_mode[r] = mode; while (nregs-- > 0) ((hard_regs_to_save)[(r + nregs) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((r + nregs) % ((unsigned) (8 * 8)))); } else fancy_abort ("../../../kgccfe/gnu/caller-save.c", 459, __FUNCTION__); }; if (word_ == 0) break; } } } bit_num_ = 0; } word_num_ = 0; } } while (0);
# 465 "../../../kgccfe/gnu/caller-save.c"
       do { HARD_REG_ELT_TYPE *scan_tp_ = (this_insn_sets); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
       note_stores ((((insn)->fld[5]).rtx), mark_set_regs, ((void *)0));


       do { HARD_REG_ELT_TYPE *scan_tp_ = (hard_regs_to_save), *scan_fp_ = (call_fixed_reg_set); scan_tp_[0] &= ~ scan_fp_[0]; scan_tp_[1] &= ~ scan_fp_[1]; scan_tp_[2] &= ~ scan_fp_[2]; } while (0);
       do { HARD_REG_ELT_TYPE *scan_tp_ = (hard_regs_to_save), *scan_fp_ = (this_insn_sets); scan_tp_[0] &= ~ scan_fp_[0]; scan_tp_[1] &= ~ scan_fp_[1]; scan_tp_[2] &= ~ scan_fp_[2]; } while (0);
       do { HARD_REG_ELT_TYPE *scan_tp_ = (hard_regs_to_save), *scan_fp_ = (hard_regs_saved); scan_tp_[0] &= ~ scan_fp_[0]; scan_tp_[1] &= ~ scan_fp_[1]; scan_tp_[2] &= ~ scan_fp_[2]; } while (0);
       do { HARD_REG_ELT_TYPE *scan_tp_ = (hard_regs_to_save), *scan_fp_ = (call_used_reg_set); scan_tp_[0] &= scan_fp_[0]; scan_tp_[1] &= scan_fp_[1]; scan_tp_[2] &= scan_fp_[2]; } while (0);
  ...
  if ((!!((hard_regs_to_save)[(regno) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((regno) % ((unsigned) (8 * 8)))))))
    regno += insert_save (chain, 1, regno, &hard_regs_to_save, save_mode);
       ...
       n_regs_saved = 0;
       for (regno = 0; regno < 176; regno++)
  if ((!!((hard_regs_saved)[(regno) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((regno) % ((unsigned) (8 * 8)))))))
    n_regs_saved++;
     }
 }
      ...
      if (chain->next == 0 || chain->next->block > chain->block)
 {
  ...
  regno += insert_restore (chain, ((enum rtx_code) (insn)->code) == JUMP_INSN,
      regno, (((target_flags & 0x00002000) ? 8 : 4) / ((target_flags & 0x00002000) ? 8 : 4)), save_mode);
 }
    }
}
     ...
     void *data __attribute__ ((__unused__));
{
  ...
  if (((enum rtx_code) (reg)->code) == SUBREG)
    {
      rtx inner = (((reg)->fld[0]).rtx);
      if (((enum rtx_code) (inner)->code) != REG || (((inner)->fld[0]).rtuint) >= 176)
 return;
  ...
  endregno = regno + mips_hard_regno_nregs (regno, mode);

  for (i = regno; i < endregno; i++)
    ((this_insn_sets)[(i) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((i) % ((unsigned) (8 * 8))));
}
     ...
     rtx x;
{
  ...
  if (code == SET)
    mark_referenced_regs ((((x)->fld[1]).rtx));
  if (code == SET || code == CLOBBER)
    {
      x = (((x)->fld[0]).rtx);
      code = ((enum rtx_code) (x)->code);
      if (code == REG || code == PC || code == CC0
   || (code == SUBREG && ((enum rtx_code) ((((x)->fld[0]).rtx))->code) == REG
  ...
  if (code == REG)
    {
      ...
      int hardregno = (regno < 176 ? regno
         : reg_renumber[regno]);

      if (hardregno >= 0)
 {
   int nregs = mips_hard_regno_nregs (hardregno, ((enum machine_mode) (x)->mode));
   while (nregs-- > 0)
     ((referenced_regs)[(hardregno + nregs) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((hardregno + nregs) % ((unsigned) (8 * 8))));
 }



      else if (reg_equiv_mem[regno] != 0)
 mark_referenced_regs ((((reg_equiv_mem[regno])->fld[0]).rtx));
 ...
 mark_referenced_regs (reg_equiv_address[regno]);
  ...
  for (i = (rtx_length[(int) (code)]) - 1; i >= 0; i--)
    {
      if (fmt[i] == 'e')
 mark_referenced_regs ((((x)->fld[i]).rtx));
      else if (fmt[i] == 'E')
 for (j = (((((x)->fld[i]).rtvec))->num_elem) - 1; j >= 0; j--)
   mark_referenced_regs ((((((x)->fld[i]).rtvec))->elem[j]));
    }
}
# 649 "../../../kgccfe/gnu/caller-save.c"
static int
     ...
     enum machine_mode *save_mode;
{
  ...
  rtx mem;
# 671 "../../../kgccfe/gnu/caller-save.c"
  if (regno_save_mem[regno][1] == 0)
    fancy_abort ("../../../kgccfe/gnu/caller-save.c", 672, __FUNCTION__);
 ...
 continue;
      ...
      for (j = 0; j < i; j++)
 if (! (!!((hard_regs_saved)[(regno + j) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((regno + j) % ((unsigned) (8 * 8)))))))
   {
     ok = 0;
     break;
   }
 ...
 continue;
  ...
  if (save_mode [regno] != VOIDmode
      ...
      && numregs == (unsigned int) mips_hard_regno_nregs (regno, save_mode [regno]))
  ...
  for (k = 0; k < i; k++)
    {
      ((hard_regs_saved)[(regno + k) / ((unsigned) (8 * 8))] &= ~(((HARD_REG_ELT_TYPE) (1)) << ((regno + k) % ((unsigned) (8 * 8)))));
     ...
     enum machine_mode *save_mode;
{
  ...
  struct insn_chain *new;
  rtx mem;
# 748 "../../../kgccfe/gnu/caller-save.c"
  if (regno_save_mem[regno][1] == 0)
    fancy_abort ("../../../kgccfe/gnu/caller-save.c", 749, __FUNCTION__);





  for (i = (((target_flags & 0x00002000) ? 8 : 4) / ((target_flags & 0x00002000) ? 8 : 4)); i > 0; i--)
    {
      int j;
      int ok = 1;
      if (regno_save_mem[regno][i] == 0)
 continue;
      ...
      for (j = 0; j < i; j++)
 if (! (!!((*to_save)[(regno + j) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((regno + j) % ((unsigned) (8 * 8)))))))
   {
     ok = 0;
     break;
   }
...
# 823 "../../../kgccfe/gnu/caller-save.c"
 ...
 reload_insn_chain = new;
      ...
      for (link = (((chain->insn)->fld[8]).rtx); link; link = (((link)->fld[1]).rtx))
 {
  ...
  fancy_abort ("../../../kgccfe/gnu/caller-save.c", 849, __FUNCTION__);
  ...
  regno = reg_renumber[regno];
       if (regno < 0)
  continue;
       for (i = mips_hard_regno_nregs (regno, ((enum machine_mode) (reg)->mode)) - 1;
     i >= 0; i--)
  bitmap_set_bit (&new->live_throughout, regno + i);
     }
 }
      bitmap_clear (&new->dead_or_set);
      if (chain->insn == ((((basic_block_info)->data.bb[(chain->block)]))->head))
 ((((basic_block_info)->data.bb[(chain->block)]))->head) = new->insn;
    }
  else
    {
      new->next = chain->next;
 ...
 new->next->prev = new;
      ...
      note_stores ((((chain->insn)->fld[5]).rtx), add_stored_regs,
     &new->live_throughout);
      bitmap_clear (&new->dead_or_set);
      if (chain->insn == ((((basic_block_info)->data.bb[(chain->block)]))->end))
 ((((basic_block_info)->data.bb[(chain->block)]))->end) = new->insn;
    }
  new->block = chain->block;
  new->is_caller_save_insn = 1;

  (((new->insn)->fld[6]).rtint) = code;
  return new;
}
