...
# 38 "../../../kgccfe/gnu/tree-dump.c" 2
# 1 "../../../kgccfe/gnu/tree-dump.h" 1
# 31 "../../../kgccfe/gnu/tree-dump.h"
     ...
     dump_info_p di;
{
 ...
 dump_string (di, "pub");
      else if (((t)->common.protected_flag))
 dump_string (di, "prot");
      else if (((t)->common.private_flag))
 dump_string (di, "priv");
      if (((t)->common.static_flag))
 dump_string (di, "virt");

      queue_and_dump_index (di, "type", ((t)->common.type), 0);
      queue_and_dump_index (di, "base", (((t))->vec.a[4]), 0);
  ...
  if (((code_class) == '<' || (code_class) == '1' || (code_class) == '2' || (code_class) == 'e'))
    {
 ...
 default:
   fancy_abort ("../../../kgccfe/gnu/tree-dump.c", 323, __FUNCTION__);
 }
    }
 ...
 queue_and_dump_index (di, "mngl", decl_assembler_name (t), 0);
      ...
      if ((((t)->decl.locus).file))
 {
   const char *filename = strrchr ((((t)->decl.locus).file), '/');
   if (!filename)
     filename = (((t)->decl.locus).file);
   else
 ...
 queue_and_dump_index (di, "unql", ((t)->type.main_variant), 0);
  ...
  switch (code)
    ...
    case IDENTIFIER_NODE:
      dump_string_field (di, "strg", ((char *)(t)->identifier.id.str));
      dump_int (di, "lngt", ((t)->identifier.id.len));
      break;

    case TREE_LIST:
      queue_and_dump_index (di, "purp", ((t)->list.purpose), 0);
      queue_and_dump_index (di, "valu", ((t)->list.value), 0);
      queue_and_dump_index (di, "chan", ((t)->common.chain), 0);
      break;
 ...
 queue_and_dump_index (di, "csts", ((t)->type.values), 0);
      break;
    ...
    case FUNCTION_TYPE:
      ...
      queue_and_dump_index (di, "prms", ((t)->type.values), 0);
      break;

    case ARRAY_TYPE:
      queue_and_dump_index (di, "elts", ((t)->common.type), 0);
      queue_and_dump_index (di, "domn", ((t)->type.values), 0);
      break;
 ...
 dump_string (di, "union");
      ...
      queue_and_dump_index (di, "flds", ((t)->type.values), 0);
 ...
 dump_int (di, "high", (((t)->int_cst.int_cst).high));
    ...
    case STRING_CST:
      fprintf (di->stream, "strg: %-7s ", ((t)->string.pointer));
      dump_int (di, "lngt", ((t)->string.length));
      break;
    ...
    case CONSTRUCTOR:
      queue_and_dump_index (di, "elts", ((t)->exp.operands[1]), 0);
      break;
     ...
     int *flag_ptr;
{
  ...
  name = concat (dump_base_name, dump_files[phase].suffix, ((void *)0));
...
dump_end (phase, stream)
     enum tree_dump_index phase __attribute__ ((__unused__));
     FILE *stream;
{
  fclose (stream);
}
     ...
     const char *arg;
{
  ...
  for (ix = 0; ix != TDI_end; ix++)
    if ((option_value = (strncmp (arg, dump_files[ix].swtch, strlen (dump_files[ix].swtch)) ? ((void *)0) : arg + strlen (dump_files[ix].swtch))))
      {
