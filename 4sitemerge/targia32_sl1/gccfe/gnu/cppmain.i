...
# 28 "../../../kgccfe/gnu/cppmain.c" 2

static void setup_callbacks (cpp_reader *);


static void scan_translation_unit (cpp_reader *);
static void scan_translation_unit_trad (cpp_reader *);
static void account_for_newlines (cpp_reader *, const uchar *, size_t);

static int dump_macro (cpp_reader *, cpp_hashnode *, void *);

static void print_line (cpp_reader *, const struct line_map *, unsigned int, const char *);

static void maybe_print_line (cpp_reader *, const struct line_map *, unsigned int);
...
static void cb_include (cpp_reader *, unsigned int, const unsigned char *, const cpp_token *);
     ...
     FILE *out_stream;
{
  ...
  if (cpp_read_main_file (pfile, in_fname, ((void *)0)))
    {
     ...
     cpp_reader *pfile;
{
 ...
 {
   avoid_paste = 1;
   if (pfile->print.source == ((void *)0)
       || (!(pfile->print.source->flags & (1 << 0))
      ...
      if (avoid_paste)
 {
   ...
   if (pfile->print.source->flags & (1 << 0)
       || (pfile->print.prev
    && cpp_avoid_paste (pfile, pfile->print.prev, token))
       || (pfile->print.prev == ((void *)0) && token->type == CPP_HASH))
     putc_unlocked (' ', pfile->print.outf);
 }
  ...
  while (_cpp_read_logical_line_trad (pfile))
    {
      ...
      fwrite_unlocked (pfile->out.base, 1, len, pfile->print.outf);
     ...
     const char *special_flags;
{
  ...
  if (! ((pfile)->opts.no_line_commands))
    {
      size_t to_file_len = strlen (map->to_file);
      unsigned char *to_file_quoted = __builtin_alloca(to_file_len * 4 + 1);
      unsigned char *p;
      ...
      *p = '\0';
      fprintf (pfile->print.outf, "# %u \"%s\"%s",
        ((pfile->print.line) + (map)->to_line - (map)->from_line),
        to_file_quoted, special_flags);

      if (map->sysp == 2)
 fputs_unlocked (" 3 4", pfile->print.outf);
      else if (map->sysp == 1)
 fputs_unlocked (" 3", pfile->print.outf);
...
{
  maybe_print_line (pfile, pfile->print.map, line);
  fputs_unlocked ("#define ", pfile->print.outf);


  if (((pfile)->opts.dump_macros) == dump_definitions)
    fputs_unlocked ((const char *) cpp_macro_definition (pfile, node), pfile->print.outf);

  else
    fputs_unlocked ((const char *) ((&(node)->ident)->str), pfile->print.outf);
...
{
  maybe_print_line (pfile, pfile->print.map, line);
  fputs_unlocked ("#pragma ", pfile->print.outf);
