...
# 31 "../../../kgccfe/gnu/ggc-page.c" 2
# 43 "../../../kgccfe/gnu/ggc-page.c"
...
# 44 "../../../kgccfe/gnu/ggc-page.c" 2
# 171 "../../../kgccfe/gnu/ggc-page.c"
static const size_t extra_order_size_table[] = {
  sizeof (struct tree_decl),
  sizeof (struct tree_list),
  (sizeof (struct rtx_def) + ((2) - 1) * sizeof (rtunion)),
  (sizeof (struct rtx_def) + ((10) - 1) * sizeof (rtunion)),
};
# 186 "../../../kgccfe/gnu/ggc-page.c"
...
# 286 "../../../kgccfe/gnu/ggc-page.c"
typedef page_entry **page_table[((size_t) 1 << (8))];
# 303 "../../../kgccfe/gnu/ggc-page.c"
...
static __inline__ void adjust_depth (void);
     ...
     page_entry *entry;
{
  ...
  base = &G.lookup[0];
# 559 "../../../kgccfe/gnu/ggc-page.c"
  L1 = (((size_t) (p) >> (32 - (8))) & ((1 << (8)) - 1));
  L2 = (((size_t) (p) >> G.lg_pagesize) & ((1 << (32 - (8) - G.lg_pagesize)) - 1));

  if (base[L1] == ((void *)0))
    base[L1] = (page_entry **) xcalloc (((size_t) 1 << (32 - (8) - G.lg_pagesize)), sizeof (page_entry *));
...
alloc_anon (pref, size)
     char *pref __attribute__ ((__unused__));
     size_t size;
{

  char *page = (char *) mmap (pref, size, 0x1 | 0x2,
         0x02 | 0x20, -1, 0);
...
# 657 "../../../kgccfe/gnu/ggc-page.c"
static __inline__ struct page_entry *
alloc_page (order)
     unsigned order;
{
     ...
     page_entry *entry;
{
      ...
      else
 {
   ...
   fancy_abort ("../../../kgccfe/gnu/ggc-page.c", 896, __FUNCTION__);
 }
    }
  --G.by_depth_in_use;
     ...
     size_t size;
{
      ...
      if ((entry->in_use_p[word] >> bit) & 1)
 {
   ...
   hint = word * (8 * 4) + bit;
      ...
      object_offset = hint * object_size_table[order];
    }
...
# 1117 "../../../kgccfe/gnu/ggc-page.c"
  ;



  G.allocated += object_size_table[order];

  if ((0) >= 3)
    fprintf (G.debug_file,
      "Allocating object, requested size=%lu, actual=%lu at %p on %p\n",
      (unsigned long) size, (unsigned long) object_size_table[order], result,
      (void *) entry);
     ...
     unsigned order;
{
  ...
  if (object_size_table[order] > G.pagesize/2)
    {
      if (objects_per_page_table[order] != 1)
 fancy_abort ("../../../kgccfe/gnu/ggc-page.c", 1230, __FUNCTION__);

      inverse_table[order].mult = 1;
      inverse_table[order].shift = 0;
      return;
    }
...
init_ggc ()
{
  ...
  G.lg_pagesize = exact_log2_wide ((unsigned long long) (G.pagesize));
# 1271 "../../../kgccfe/gnu/ggc-page.c"
  G.debug_file = stdout;
 ...
 if ((size_t)p & (G.pagesize - 1))
   fancy_abort ("../../../kgccfe/gnu/ggc-page.c", 1289, __FUNCTION__);
      }
  ...
  for (order = (8 * 4); order < ((8 * 4) + (sizeof (extra_order_size_table) / sizeof ((extra_order_size_table)[0]))); ++order)
    {
      ...
      s = (((s) + ((__builtin_offsetof (struct max_alignment, u))) - 1) / ((__builtin_offsetof (struct max_alignment, u)))) * (__builtin_offsetof (struct max_alignment, u));
...
ggc_push_context ()
{
  ...
  if (G.context_depth >= (8 * 4))
    fancy_abort ("../../../kgccfe/gnu/ggc-page.c", 1357, __FUNCTION__);
}
     ...
     page_entry *p;
{
  ...
  num_objects = objects_per_page_table[p->order] + 1;
 ...
 p->num_free_objects -= (j & 1);
  ...
  if (p->num_free_objects >= num_objects)
    fancy_abort ("../../../kgccfe/gnu/ggc-page.c", 1395, __FUNCTION__);
}
...
# 1474 "../../../kgccfe/gnu/ggc-page.c"
      p->context_depth = depth;
    }

  adjust_depth ();
# 1493 "../../../kgccfe/gnu/ggc-page.c"
}
...
ggc_collect ()
{
  ...
  float allocated_last_gc =
    ((G.allocated_last_gc) > ((size_t)(compiler_params[(int) GGC_MIN_HEAPSIZE].value) * 1024) ? (G.allocated_last_gc) : ((size_t)(compiler_params[(int) GGC_MIN_HEAPSIZE].value) * 1024));

  float min_expand = allocated_last_gc * (compiler_params[(int) GGC_MIN_EXPAND].value) / 100;
...
ggc_print_statistics ()
{
      ...
      for (p = G.pages[i]; p; p = p->next)
 {
   allocated += p->bytes;
   in_use +=
     (objects_per_page_table[i] - p->num_free_objects) * object_size_table[i];

   overhead += (sizeof (page_entry) - sizeof (long)
         + (((((objects_per_page_table[i] + 1)) + ((8 * 4)) - 1) / ((8 * 4))) * sizeof(long)));
 }
      fprintf (stderr, "%-5lu %10lu%c %10lu%c %10lu%c\n",
        (unsigned long) object_size_table[i],
        ((unsigned long) ((allocated) < 1024*10 ? (allocated) : ((allocated) < 1024*1024*10 ? (allocated) / 1024 : (allocated) / (1024*1024)))), ((allocated) < 1024*10 ? ' ' : ((allocated) < 1024*1024*10 ? 'k' : 'M')),
        ((unsigned long) ((in_use) < 1024*10 ? (in_use) : ((in_use) < 1024*1024*10 ? (in_use) / 1024 : (in_use) / (1024*1024)))), ((in_use) < 1024*10 ? ' ' : ((in_use) < 1024*1024*10 ? 'k' : 'M')),
  ...
  fprintf (stderr, "%-5s %10lu%c %10lu%c %10lu%c\n", "Total",
    ((unsigned long) ((G.bytes_mapped) < 1024*10 ? (G.bytes_mapped) : ((G.bytes_mapped) < 1024*1024*10 ? (G.bytes_mapped) / 1024 : (G.bytes_mapped) / (1024*1024)))), ((G.bytes_mapped) < 1024*10 ? ' ' : ((G.bytes_mapped) < 1024*1024*10 ? 'k' : 'M')),
    ((unsigned long) ((G.allocated) < 1024*10 ? (G.allocated) : ((G.allocated) < 1024*1024*10 ? (G.allocated) / 1024 : (G.allocated) / (1024*1024)))), ((G.allocated) < 1024*10 ? ' ' : ((G.allocated) < 1024*1024*10 ? 'k' : 'M')),
    ((unsigned long) ((total_overhead) < 1024*10 ? (total_overhead) : ((total_overhead) < 1024*1024*10 ? (total_overhead) / 1024 : (total_overhead) / (1024*1024)))), ((total_overhead) < 1024*10 ? ' ' : ((total_overhead) < 1024*1024*10 ? 'k' : 'M')));
}
