...
# 48 "../../../kgccfe/gnu/tracer.c" 2

static int count_insns (basic_block);
static _Bool ignore_bb_p (basic_block);
static _Bool better_p (edge, edge);
     ...
     basic_block *trace;
{
  ...
  while ((e = find_best_predecessor (bb)) != ((void *)0))
    {
      ...
      if ((((reorder_block_def) (bb2)->aux)->visited || ((reorder_block_def) (bb2)->aux)->next) || (e->flags & (32 | (2 | 4 | 8)))
 ...
 fprintf (rtl_dump_file, ",%i [%i]", bb->index, bb->frequency);
  ...
  while ((e = find_best_successor (bb)) != ((void *)0))
    {
      bb = e->dest;
      if ((((reorder_block_def) (bb)->aux)->visited || ((reorder_block_def) (bb)->aux)->next) || (e->flags & (32 | (2 | 4 | 8)))
...
tail_duplicate ()
{
  ...
  if (profile_info.count_profiles_merged && flag_branch_probabilities)
    probability_cutoff = (compiler_params[(int) TRACER_MIN_BRANCH_PROBABILITY_FEEDBACK].value);
  else
    probability_cutoff = (compiler_params[(int) TRACER_MIN_BRANCH_PROBABILITY].value);
  probability_cutoff = 10000 / 100 * probability_cutoff;

  branch_ratio_cutoff =
    (10000 / 100 * (compiler_params[(int) TRACER_MIN_BRANCH_RATIO].value));
 ...
 blocks[bb->index] = fibheap_insert (heap, -bb->frequency,
  ...
  if (profile_info.count_profiles_merged && flag_branch_probabilities)
    cover_insns = (compiler_params[(int) TRACER_DYNAMIC_COVERAGE_FEEDBACK].value);
  else
    cover_insns = (compiler_params[(int) TRACER_DYNAMIC_COVERAGE].value);
  cover_insns = (weighted_insns * cover_insns + 50) / 100;
  max_dup_insns = (ninsns * (compiler_params[(int) TRACER_MAX_CODE_GROWTH].value) + 50) / 100;
 ...
 break;
      ...
      if (ignore_bb_p (bb))
 continue;
      if ((((reorder_block_def) (bb)->aux)->visited || ((reorder_block_def) (bb)->aux)->next))
 fancy_abort ("../../../kgccfe/gnu/tracer.c", 254, __FUNCTION__);
      ...
      for (pos = 1; pos < n; pos++)
 {
  ...
  fprintf (rtl_dump_file, "Duplicated %i as %i [%i]\n",
   ...
   ((reorder_block_def) (bb)->aux)->next = bb2;
   ((reorder_block_def) (bb2)->aux)->visited = 1;
   bb = bb2;
...
layout_superblocks ()
{
  basic_block end = (&entry_exit_blocks[0])->succ->dest;
  basic_block bb = (&entry_exit_blocks[0])->succ->dest->next_bb;

  while (bb != (&entry_exit_blocks[1]))
    {
      edge e, best = ((void *)0);
      while (((reorder_block_def) (end)->aux)->next)
 end = ((reorder_block_def) (end)->aux)->next;

      for (e = end->succ; e; e = e->succ_next)
 if (e->dest != (&entry_exit_blocks[1])
     && e->dest != (&entry_exit_blocks[0])->succ->dest
     && !((reorder_block_def) (e->dest)->aux)->visited
     && (!best || (((e)->src->frequency * (e)->probability + 10000 / 2) / 10000) > (((best)->src->frequency * (best)->probability + 10000 / 2) / 10000)))
   best = e;

      if (best)
 {
   ((reorder_block_def) (end)->aux)->next = best->dest;
   ((reorder_block_def) (best->dest)->aux)->visited = 1;
 }
      else
 for (; bb != (&entry_exit_blocks[1]); bb=bb->next_bb)
   {
     if (!((reorder_block_def) (bb)->aux)->visited)
       {
  ((reorder_block_def) (end)->aux)->next = bb;
  ((reorder_block_def) (bb)->aux)->visited = 1;
  break;
       }
   }
    }
