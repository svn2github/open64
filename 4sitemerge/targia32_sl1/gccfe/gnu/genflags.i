     ...
     rtx x;
{
  ...
  if (code == MATCH_OPERAND || code == MATCH_OPERATOR
      || code == MATCH_PARALLEL)
    max_opno = ((max_opno) > ((((x)->fld[0]).rtint)) ? (max_opno) : ((((x)->fld[0]).rtint)));
  ...
  for (i = 0; i < len; i++)
    {
      if (fmt[i] == 'e' || fmt[i] == 'u')
 max_operand_1 ((((x)->fld[i]).rtx));
      else if (fmt[i] == 'E')
 {
   int j;
   for (j = 0; j < (((((x)->fld[i]).rtvec))->num_elem); j++)
     max_operand_1 ((((((x)->fld[i]).rtvec))->elem[j]));
 }
    }
}
...
num_operands (insn)
     rtx insn;
{
  int len = (((((insn)->fld[1]).rtvec))->num_elem);
  int i;

  max_opno = -1;

  for (i = 0; i < len; i++)
    max_operand_1 ((((((insn)->fld[1]).rtvec))->elem[i]));
     ...
     int real, expect;
{
  ...
  if (real > expect)
    fancy_abort ("../../../kgccfe/gnu/genflags.c", 114, __FUNCTION__);
  if (real == 0)
    fancy_abort ("../../../kgccfe/gnu/genflags.c", 116, __FUNCTION__);


  fputs_unlocked ("#define GEN_", stdout);
  for (i = 0; name[i]; i++)
    putchar (_sch_toupper[(name[i]) & 0xff]);
     ...
     rtx insn;
{
  ...
  int i;
  const char *name = (((insn)->fld[0]).rtstr);
  int truth = maybe_eval_c_test ((((insn)->fld[2]).rtstr));
 ...
 gen_macro (name, num, 5);
  ...
  if (num == 0)
    fputs_unlocked ("void", stdout);
  else
    {
      for (i = 1; i < num; i++)
 fputs_unlocked ("rtx, ", stdout);
     ...
     rtx insn;
{
      ...
      for (p = (((insn)->fld[2]).rtstr); *p; p++)
 {
  ...
  __extension__ ({ struct obstack *__o = (&obstack); int __len = (sizeof (rtx)); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((&insn)), (__len)); __o->next_free += __len; (void) 0; });
}
     ...
     char **argv;
{
  ...
  progname = "genflags";
  _obstack_begin ((&obstack), 0, 0, (void *(*) (long)) xmalloc, (void (*) (void *)) free);
  ...
  while (1)
    {
      ...
      if (desc == ((void *)0))
 break;
      if (((enum rtx_code) (desc)->code) == DEFINE_INSN || ((enum rtx_code) (desc)->code) == DEFINE_EXPAND)
 gen_insn (desc);
    }


  dummy = (rtx) 0;
  __extension__ ({ struct obstack *__o = (&obstack); int __len = (sizeof (rtx)); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((&dummy)), (__len)); __o->next_free += __len; (void) 0; });
  insns = (rtx *) __extension__ ({ struct obstack *__o1 = (&obstack); void *value; value = (void *) __o1->object_base; if (__o1->next_free == value) __o1->maybe_empty_object = 1; __o1->next_free = (((((__o1->next_free) - (char *) 0)+__o1->alignment_mask) & ~ (__o1->alignment_mask)) + (char *) 0); if (__o1->next_free - (char *)__o1->chunk > __o1->chunk_limit - (char *)__o1->chunk) __o1->next_free = __o1->chunk_limit; __o1->object_base = __o1->next_free; value; });
