...
# 24 "../../../kgccfe/gnu/ra-debug.c" 2
# 1 "../../../kgccfe/gnu/insn-config.h" 1
# 25 "../../../kgccfe/gnu/ra-debug.c" 2
...
{
  { va_list ap; __builtin_va_start(ap,format); { struct Qdmy;
  struct Qdmy;
  struct Qdmy;
  if ((debug_new_regalloc & level) != 0 && rtl_dump_file != ((void *)0))
    vfprintf (rtl_dump_file, format, ap);
  } __builtin_va_end(ap); };
}
# 70 "../../../kgccfe/gnu/ra-debug.c"
static void
     ...
     rtx x;
{
  ...
  switch (code)
   ...
   if (((enum machine_mode) (x)->mode) != VOIDmode)
     fprintf (file, ":%s(", (mode_name[(int) (((enum machine_mode) (x)->mode))]));
   else
     fputs_unlocked ("(", file);
   ra_print_rtx (file, op0, 0);
   fputs_unlocked (")", file);
  ...
  if (code == IF_THEN_ELSE)
  ...
  else
    {
      ...
      fprintf (file, "%s:%s(", (rtx_name[(int) (code)]),
        (mode_name[(int) (((enum machine_mode) (x)->mode))]));
      ra_print_rtx (file, op0, 0);
      fputs_unlocked (", ", file);
      ra_print_rtx (file, op1, 0);
      fputs_unlocked (", ", file);
     ...
     rtx x;
{
       ...
       for (i = 0; i < (rtx_length[(int) (code)]); i++)
  {
    if (num)
      fputs_unlocked (", ", file);
    if (fmt[i] == 'e' && (((x)->fld[i]).rtx))

      {
        ra_print_rtx (file, (((x)->fld[i]).rtx), 0);
        num++;
      }
    else if (fmt[i] == 'w')
      {
        fprintf (file, "0x%llx", (((x)->fld[i]).rtwint));
        num++;
      }
  }
       break;
     }
      case CONST_STRING: fprintf (file, "\"%s\"", (((x)->fld[0]).rtstr)); break;
      case CONST: fputs_unlocked ("const(", file);
    ra_print_rtx (file, (((x)->fld[0]).rtx), 0);
    fputs_unlocked (")", file);
    break;
   ...
   if (regno < 176)
     {
       int i, nregs = mips_hard_regno_nregs (regno, mode);
       if (nregs > 1)
         fputs_unlocked ("[", file);
       for (i = 0; i < nregs; i++)
         {
   ...
   if (((enum rtx_code) (sub)->code) == REG
    ...
    if (reg_names[regno+i])
   ...
   else
     {
       ...
       fprintf (file, ":[%s+%d]", (mode_name[(int) (mode)]), ofs);
     }
   break;
        }
   ...
   ra_print_rtx (file, (((x)->fld[1]).rtx), 0);
   fputs_unlocked ("))", file);
   break;
  ...
  break;
   ...
   && (((sub)->fld[5]).rtint) == NOTE_INSN_DELETED_LABEL)
        fprintf (file, "(deleted uid=%d)", (((sub)->fld[0]).rtint));
      else if (((enum rtx_code) (sub)->code) == CODE_LABEL)
        fprintf (file, "L%d", (((sub)->fld[6]).rtint));
      else
        fprintf (file, "(nonlabel uid=%d)", (((sub)->fld[0]).rtint));
    }
  break;
      case SYMBOL_REF:
  fprintf (file, "sym(\"%s\")", (((x)->fld[0]).rtstr)); break;
     ...
     int with_pn;
{
  ...
  class = (rtx_class[(int) (code)]);


  if (((rtx_class[(int) (((enum rtx_code) (x)->code))]) == 'i') || code == NOTE || code == CODE_LABEL || code == BARRIER)
    {
 ...
 {
   fprintf (file, "L%d:\t; ", (((x)->fld[6]).rtint));
   if ((((x)->fld[7]).rtstr))
     fprintf (file, "(%s) ", (((x)->fld[7]).rtstr));
   switch (((enum label_kind) (((x)->jump << 1) | (x)->call)))
     ...
     default: fancy_abort ("../../../kgccfe/gnu/ra-debug.c", 381, __FUNCTION__);
     }
   fprintf (file, " [%d uses] uid=(", (((x)->fld[4]).rtint));
 }
      fprintf (file, "%d", (((x)->fld[0]).rtint));
      if (with_pn)
 fprintf (file, " %d %d", (((x)->fld[1]).rtx) ? ((((((x)->fld[1]).rtx))->fld[0]).rtint) : 0,
   (((x)->fld[2]).rtx) ? ((((((x)->fld[2]).rtx))->fld[0]).rtint) : 0);
      if (code == BARRIER)
      ...
      else if (code == NOTE)
 {
   ...
   if (ln >= (int) NOTE_INSN_BIAS && ln < (int) NOTE_INSN_MAX)
   ...
   else
     {
       ...
       if ((((x)->fld[4]).rtstr))
  fprintf (file, ":%s", (((x)->fld[4]).rtstr));
     }
 }
      else
 {
   fprintf (file, "\t");
   ra_print_rtx (file, (((x)->fld[5]).rtx), 0);
 }
      return;
    }
  switch (code)
    {
     ...
     {
       int j;
       for (j = 0; j < (((((x)->fld[0]).rtvec))->num_elem); j++)
  {
    if (j)
      fputs_unlocked ("\t;; ", file);
    ra_print_rtx (file, (((((x)->fld[0]).rtvec))->elem[j]), 0);
  }
       break;
     }
      case UNSPEC: case UNSPEC_VOLATILE:
     {
       ...
       fprintf (file, "unspec%s(%d",
         (code == UNSPEC) ? "" : "_vol", (((x)->fld[1]).rtint));
       for (j = 0; j < (((((x)->fld[0]).rtvec))->num_elem); j++)
  {
    fputs_unlocked (", ", file);
    ra_print_rtx (file, (((((x)->fld[0]).rtvec))->elem[j]), 0);
   ...
   if (((enum rtx_code) ((((x)->fld[0]).rtx))->code) == PC)
    ...
    && ((enum rtx_code) (((((((x)->fld[1]).rtx))->fld[2]).rtx))->code) == PC)
  {
    fputs_unlocked ("if ", file);
    ra_print_rtx (file, ((((((x)->fld[1]).rtx))->fld[0]).rtx), 0);
    fputs_unlocked (" jump ", file);
    ra_print_rtx (file, ((((((x)->fld[1]).rtx))->fld[1]).rtx), 0);
  }
       else
  {
    fputs_unlocked ("jump ", file);
   ...
   break;
      ...
      case TRAP_IF:
       ...
       break;
      case RESX:
       fprintf (file, "resx from region %d", (((x)->fld[0]).rtint));
       break;
     ...
     int num;
{
  ...
  if (num < 0)
    for (i = count / 2; i > 0 && (((insn)->fld[1]).rtx); i--)
      insn = (((insn)->fld[1]).rtx);
  for (i = count; i > 0 && insn; insn = (((insn)->fld[2]).rtx), i--)
    {
     ...
     rtx insn;
{
      ...
      if (((enum rtx_code) (insn)->code) == NOTE)
 {
   ...
   if ((((insn)->fld[5]).rtint) != NOTE_INSN_BASIC_BLOCK
       && (flag_ra_dump_notes
    || (((insn)->fld[5]).rtint) == NOTE_INSN_DELETED
    || (((insn)->fld[5]).rtint) == NOTE_INSN_DELETED_LABEL))
     {
       ra_print_rtx_top (file, insn, (num == 0 || !(((insn)->fld[2]).rtx)));
       num++;
     }
 }
      else
     ...
     struct df *df __attribute__ ((__unused__));
{
  ...
  int num2;
  unsigned int i;
  if (!rtl_dump_file || (debug_new_regalloc & (0x0004 | 0x0002)) == 0)
    return;
  ...
  for (def1 = 0; def1 < num_webs; def1++)
    {
      ...
      for (num2=0, def2 = 0; def2 < num_webs; def2++)
        if (def1 != def2 && ((igraph)->elms [(((def1) < (def2) ? ((def2)*((def2)-1)/2)+(def1) : ((def1)*((def1)-1)/2)+(def2))) / ((unsigned) (8 * 8))] >> (((def1) < (def2) ? ((def2)*((def2)-1)/2)+(def1) : ((def1)*((def1)-1)/2)+(def2))) % ((unsigned) (8 * 8)) & 1))
   {
     if (num1 == num)
       {
    ...
    ra_debug_msg (0x0004, "%d (SUBREG %d, %d) with ", def1,
        id2web[def1]->regno,
        (((id2web[def1]->orig_x)->fld[1]).rtuint));
         else
    ...
    (((id2web[def2]->orig_x)->fld[1]).rtuint));
     else
    ...
    if (ml->move)
      {
        ra_debug_msg (0x0004, "move: insn %d: Web %d <-- Web %d\n",
          (((ml->move->insn)->fld[0]).rtint), ml->move->target_web->id,
 ...
 {
   ra_debug_msg (0x0002, " sub %d", (((web->orig_x)->fld[1]).rtuint));
   ra_debug_msg (0x0002, " par %d", (((web)->parent_web) ? find_web_for_subweb_1 ((web)->parent_web) : (web))->id);
 }
      ra_debug_msg (0x0002, " +%d (span %d, cost ",
      web->add_hardregs, web->span_deaths);
      ra_debug_msg (0x0002, "%lld", web->spill_cost);
      ra_debug_msg (0x0002, ") (%s)", reg_class_names[web->regclass]);
      if (web->spill_temp == 1)
...
dump_igraph_machine ()
{
  ...
  if (!rtl_dump_file || (debug_new_regalloc & 0x10000) == 0)
    return;
   ...
   while (1)
     {
       unsigned int u = 0;
       int n;
       for (n = 0; n < 32 && col < 176; n++, col++)
  if ((!!((web->usable_regs)[(col) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((col) % ((unsigned) (8 * 8)))))))
    u |= 1 << n;
...
dump_constraints ()
{
  rtx insn;
  int i;
  if (!rtl_dump_file || (debug_new_regalloc & 0x0040) == 0)
    return;
  for (i = 176; i < ra_max_regno; i++)
 ...
 ra_debug_msg (0x0040, "\n");
  ...
  for (i = 176; i < ra_max_regno; i++)
    if ((cfun->emit->x_regno_reg_rtx)[i] && ((enum rtx_code) ((cfun->emit->x_regno_reg_rtx)[i])->code) == REG)
      ((((cfun->emit->x_regno_reg_rtx)[i])->fld[0]).rtuint) = i;
}
     ...
     struct df *df __attribute__ ((__unused__));
{
  struct web *web;
  struct dlist *d;
  if (!rtl_dump_file || (debug_new_regalloc & 0x0080) == 0)
    return;
     ...
     const char *prefix;
{
  ...
  for (bb = (&entry_exit_blocks[0])->next_bb; bb != (&entry_exit_blocks[1]); bb = bb->next_bb)
    {
      unsigned long long block_cost = bb->frequency;
      rtx insn, set;
      for (insn = bb->head; insn; insn = (((insn)->fld[2]).rtx))
 {
     ...
     struct web *web2;
{
  ...
  if (web1->type == PRECOLORED)
    return (!!((web2->usable_regs)[(web1->regno) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((web1->regno) % ((unsigned) (8 * 8))))));

  if (web2->type == PRECOLORED)
    return (!!((web1->usable_regs)[(web2->regno) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((web2->regno) % ((unsigned) (8 * 8))))));
     ...
     struct web *web;
{
  ...
  for (i = 0; i < web->num_defs; ++i)
    {
      ra_debug_msg (((unsigned int)-1), " %d ", (((web->defs[i]->insn)->fld[0]).rtint));
    }

  ra_debug_msg (((unsigned int)-1), "\n   use insns:");
  for (i = 0; i < web->num_uses; ++i)
    {
      ra_debug_msg (((unsigned int)-1), " %d ", (((web->uses[i]->insn)->fld[0]).rtint));
    }
  ra_debug_msg (((unsigned int)-1), "\n");
}
     ...
     struct web *web;
{
  ...
  for (def2 = 0; def2 < num_webs; def2++)
    if (((igraph)->elms [(((web->id) < (def2) ? ((def2)*((def2)-1)/2)+(web->id) : ((web->id)*((web->id)-1)/2)+(def2))) / ((unsigned) (8 * 8))] >> (((web->id) < (def2) ? ((def2)*((def2)-1)/2)+(web->id) : ((web->id)*((web->id)-1)/2)+(def2))) % ((unsigned) (8 * 8)) & 1) && web->id != def2)
      {
 if ((num % 9) == 5)
   ra_debug_msg (((unsigned int)-1), "\n             ");
 num++;
