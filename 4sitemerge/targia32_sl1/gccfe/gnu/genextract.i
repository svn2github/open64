...
# 27 "../../../kgccfe/gnu/genextract.c" 2
# 1 "../../../kgccfe/gnu/insn-config.h" 1
# 28 "../../../kgccfe/gnu/genextract.c" 2
...
# 29 "../../../kgccfe/gnu/genextract.c" 2
# 41 "../../../kgccfe/gnu/genextract.c"
struct extraction
{
  int op_count;
  char *oplocs[30];
...
static char *duplocs[4];



static int dupnums[4];
     ...
     rtx insn;
{
  ...
  if ((((((insn)->fld[1]).rtvec))->num_elem) == 1)
    walk_rtx ((((((insn)->fld[1]).rtvec))->elem[0]), "");
  else
    for (i = (((((insn)->fld[1]).rtvec))->num_elem) - 1; i >= 0; i--)
      {
 char path[2];
 ...
 walk_rtx ((((((insn)->fld[1]).rtvec))->elem[i]), path);
      }
     ...
     const char *path;
{
  ...
  switch (code)
    ...
    case MATCH_OPERAND:
    case MATCH_SCRATCH:
      oplocs[(((x)->fld[0]).rtint)] = xstrdup (path);
      op_count = ((op_count) > ((((x)->fld[0]).rtint) + 1) ? (op_count) : ((((x)->fld[0]).rtint) + 1));
      break;

    case MATCH_DUP:
      duplocs[dup_count] = xstrdup (path);
      dupnums[dup_count] = (((x)->fld[0]).rtint);
      dup_count++;
      break;
    ...
    case MATCH_OP_DUP:
      ...
      for (i = (((((x)->fld[1]).rtvec))->num_elem) - 1; i >= 0; i--)
        {
   newpath[depth] = (code == MATCH_OP_DUP ? '0' : 'a') + i;
   walk_rtx ((((((x)->fld[1]).rtvec))->elem[i]), newpath);
        }
      free (newpath);
      return;
    ...
    case MATCH_OPERATOR:
      ...
      for (i = (((((x)->fld[2]).rtvec))->num_elem) - 1; i >= 0; i--)
 {
   newpath[depth] = '0' + i;
   walk_rtx ((((((x)->fld[2]).rtvec))->elem[i]), newpath);
 }
      free (newpath);
      return;
      ...
      else if (fmt[i] == 'E')
 {
   ...
   for (j = (((((x)->fld[i]).rtvec))->num_elem) - 1; j >= 0; j--)
     {
       newpath[depth] = 'a' + j;
       walk_rtx ((((((x)->fld[i]).rtvec))->elem[j]), newpath);
     }
 }
    }
  free (newpath);
     ...
     const char *path;
{
  ...
  for (i = len - 1; i >=0 ; i--)
    {
      if ((_sch_istable[(path[i]) & 0xff] & (unsigned short)(_sch_islower)))
 printf ("XVECEXP (");
      else if ((_sch_istable[(path[i]) & 0xff] & (unsigned short)(_sch_isdigit)))
 printf ("XEXP (");
      else
 fancy_abort ("../../../kgccfe/gnu/genextract.c", 331, __FUNCTION__);
    }
 ...
 fancy_abort ("../../../kgccfe/gnu/genextract.c", 343, __FUNCTION__);
    }
}
     ...
     char **argv;
{
 ...
 {
   record_insn_name (insn_code_number, (((desc)->fld[0]).rtstr));
   gen_insn (desc);
 }

      else if (((enum rtx_code) (desc)->code) == DEFINE_PEEPHOLE)
 {
   struct code_ptr *link
