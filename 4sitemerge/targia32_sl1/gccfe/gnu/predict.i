...
# 50 "../../../kgccfe/gnu/predict.c" 2
# 1 "../../../kgccfe/gnu/insn-config.h" 1
# 51 "../../../kgccfe/gnu/predict.c" 2
...
# 65 "../../../kgccfe/gnu/predict.c" 2



static struct real_value real_zero, real_one, real_almost_one, real_br_prob_base,
...
static _Bool predicted_by_p (basic_block, enum br_predictor);
...
static void process_note_predictions (basic_block, int *, dominance_info, dominance_info);


static void process_note_prediction (basic_block, int *, dominance_info, dominance_info, int, int);


static _Bool last_basic_block_p (basic_block);
static void compute_function_frequency (void);
static void choose_function_section (void);
static _Bool can_predict_insn_p (rtx);
     ...
     basic_block bb;
{
  if (profile_info.count_profiles_merged
   ...
   / (compiler_params[(int) HOT_BB_COUNT_FRACTION].value)))
    return 0;
  if (bb->frequency < 10000 / (compiler_params[(int) HOT_BB_FREQUENCY_FRACTION].value))
    return 0;
  return 1;
}
     ...
     enum br_predictor predictor;
{
  ...
  if (!((rtx_class[(int) (((enum rtx_code) (bb->end)->code))]) == 'i'))
    return 0;
  for (note = (((bb->end)->fld[8]).rtx); note; note = (((note)->fld[1]).rtx))
    if (((enum reg_note) ((enum machine_mode) (note)->mode)) == REG_BR_PRED
 && (((((((((note)->fld[0]).rtx))->fld[0]).rtx))->fld[0]).rtwint) == (int)predictor)
      return 1;
  return 0;
}
     ...
     enum br_predictor predictor;
{
  if (!any_condjump_p (insn))
    fancy_abort ("../../../kgccfe/gnu/predict.c", 197, __FUNCTION__);
  if (!flag_guess_branch_prob)
    return;

  (((insn)->fld[8]).rtx)
    = gen_rtx_fmt_ee (EXPR_LIST, (REG_BR_PRED), (gen_rtx_fmt_ee (CONCAT, (VOIDmode), (gen_rtx_CONST_INT (VOIDmode, (long long) ((int) predictor))), (gen_rtx_CONST_INT (VOIDmode, (long long) ((int) probability))))), ((((insn)->fld[8]).rtx)));
     ...
     rtx insn;
{
  ...
  for (note = (((insn)->fld[8]).rtx); note; note = (((note)->fld[1]).rtx))
    if (((enum reg_note) ((enum machine_mode) (note)->mode)) == REG_BR_PROB)
      (((note)->fld[0]).rtx) = gen_rtx_CONST_INT (VOIDmode, (long long) (10000 - ((((((note)->fld[0]).rtx))->fld[0]).rtwint)));
    else if (((enum reg_note) ((enum machine_mode) (note)->mode)) == REG_BR_PRED)
      ((((((note)->fld[0]).rtx))->fld[1]).rtx)
 = gen_rtx_CONST_INT (VOIDmode, (long long) (10000 - (((((((((note)->fld[0]).rtx))->fld[1]).rtx))->fld[0]).rtwint)));
}
  ...
  if (bb->count)
    {
      ...
      fprintf (rtl_dump_file, "%lld", bb->count);
      if (e)
 {
   fprintf (rtl_dump_file, " hit ");
   fprintf (rtl_dump_file, "%lld", e->count);
     ...
     basic_block bb;
{
 ...
 pnote = &(((*pnote)->fld[1]).rtx);
  ...
  if (!prob_note)
    {
      (((insn)->fld[8]).rtx)
 = gen_rtx_fmt_ee (EXPR_LIST, (REG_BR_PROB), (gen_rtx_CONST_INT (VOIDmode, (long long) (combined_probability))), ((((insn)->fld[8]).rtx)));
...
expected_value_to_br_prob ()
{
 ...
 case NOTE:
   ...
   if ((((insn)->fld[5]).rtint) == NOTE_INSN_EXPECTED_VALUE)
     {
       ev = (((insn)->fld[4]).rtx);
       ev_reg = (((ev)->fld[0]).rtx);
       delete_insn (insn);
...
# 702 "../../../kgccfe/gnu/predict.c"
 ...
 fancy_abort ("../../../kgccfe/gnu/predict.c", 717, __FUNCTION__);
     ...
     basic_block bb;
{
  ...
  return (bb->next_bb == (&entry_exit_blocks[1])
   || (bb->next_bb->next_bb == (&entry_exit_blocks[1])
       && bb->succ && !bb->succ->succ_next
       && bb->succ->dest->next_bb == (&entry_exit_blocks[1])));
}
     ...
     int flags;
{
      ...
      while (next_ai != bb)
 {
  ...
  if (y == last_basic_block || !can_predict_insn_p ((((basic_block_info)->data.bb[(y)]))->end))
    return;
     ...
     dominance_info post_dominators;
{
  ...
  for (insn = bb->end; insn;
       was_bb_head |= (insn == bb->head), insn = (((insn)->fld[1]).rtx))
    {
 ...
 {
   int alg = (int) ((((insn)->fld[4]).rtint)>>8);

   process_note_prediction (bb,
       heads,
       dominators,
       post_dominators,
       alg, (int) ((((insn)->fld[4]).rtint)&0xff));
   delete_insn (insn);
 }
    }
  for (e = bb->succ; e; e = e->succ_next)
...
note_prediction_to_br_prob ()
{
  ...
  heads[(&entry_exit_blocks[0])->next_bb->index] = last_basic_block;
...
typedef struct edge_info_def
{
  ...
  struct real_value back_edge_prob;
     ...
     struct loop *loop;
{
      ...
      if (((block_info) (bb)->aux)->tovisit)
 {
  ...
  memcpy (&((block_info) (head)->aux)->frequency, &real_one, sizeof (real_one));
  last = head;
  for (bb = head; bb; bb = nextbb)
    {
      struct real_value cyclic_probability, frequency;
      ...
      if (bb != head)
 {
   ...
   for (e = bb->pred; e; e = e->pred_next)
     if (((edge_info) (e)->aux)->back_edge)
       {
  real_arithmetic (&(cyclic_probability), PLUS_EXPR, &(cyclic_probability), &(((edge_info) (e)->aux)->back_edge_prob));


       }
     else if (!(e->flags & 32))
       {
  struct real_value tmp;





  real_from_integer (&(tmp), ((global_trees[TI_DOUBLE_TYPE])->type.mode), e->probability, 0, 0);

  real_arithmetic (&(tmp), MULT_EXPR, &(tmp), &(((block_info) (e->src)->aux)->frequency));

  real_arithmetic (&(tmp), MULT_EXPR, &(tmp), &(real_inv_br_prob_base));
  real_arithmetic (&(frequency), PLUS_EXPR, &(frequency), &(tmp));
       }

   if (real_identical (&(cyclic_probability), &(real_zero)))
     memcpy (&((block_info) (bb)->aux)->frequency, &frequency, sizeof (frequency));
   else
     {
       if (real_compare (LT_EXPR, &(real_almost_one), &(cyclic_probability)))
  memcpy (&cyclic_probability, &real_almost_one, sizeof (real_zero));




       real_arithmetic (&(cyclic_probability), MINUS_EXPR, &(real_one), &(cyclic_probability));

       real_arithmetic (&(((block_info) (bb)->aux)->frequency), RDIV_EXPR, &(frequency), &(cyclic_probability));
 ...
 if (e->dest == head)
   {
     ...
     real_arithmetic (&(tmp), MULT_EXPR, &(tmp), &(((block_info) (bb)->aux)->frequency));

     real_arithmetic (&(((edge_info) (e)->aux)->back_edge_prob), MULT_EXPR, &(tmp), &(real_inv_br_prob_base));
     ...
     struct loop *first_loop;
{
 ...
 ((block_info) (bbs[i])->aux)->tovisit = 1;
      free (bbs);
      propagate_freq (loop);
    }
}
...
counts_to_freqs ()
{
  ...
  for (bb = (&entry_exit_blocks[0])->next_bb; bb != (&entry_exit_blocks[1]); bb = bb->next_bb)
    count_max = ((bb->count) > (count_max) ? (bb->count) : (count_max));
 ...
 int threshold;
{
  ...
  if (threshold > 10000)
    fancy_abort ("../../../kgccfe/gnu/predict.c", 1125, __FUNCTION__);
...
{
  basic_block bb;
  struct real_value freq_max;
  enum machine_mode double_mode = ((global_trees[TI_DOUBLE_TYPE])->type.mode);

  if (flag_branch_probabilities)
    counts_to_freqs ();
  else
    {
      real_from_integer (&(real_zero), double_mode, 0, 0, 0);
      real_from_integer (&(real_one), double_mode, 1, 0, 0);
      real_from_integer (&(real_br_prob_base), double_mode, 10000, 0, 0);
      real_from_integer (&(real_bb_freq_max), double_mode, 10000, 0, 0);
      real_from_integer (&(real_one_half), double_mode, 2, 0, 0);
      ...
      for (bb = (&entry_exit_blocks[0]); bb != ((void *)0); bb = bb->next_bb)
 {
   ...
   for (e = bb->succ; e; e = e->succ_next)
     {
       real_from_integer (&(((edge_info) (e)->aux)->back_edge_prob), double_mode, e->probability, 0, 0);

       real_arithmetic (&(((edge_info) (e)->aux)->back_edge_prob), MULT_EXPR, &(((edge_info) (e)->aux)->back_edge_prob), &(real_inv_br_prob_base));
      ...
      for (bb = (&entry_exit_blocks[0])->next_bb; bb != (&entry_exit_blocks[1]); bb = bb->next_bb)
 if (real_compare (LT_EXPR, &(freq_max), &(((block_info) (bb)->aux)->frequency)))

   memcpy (&freq_max, &((block_info) (bb)->aux)->frequency,
    sizeof (freq_max));
      ...
      for (bb = (&entry_exit_blocks[0]); bb != ((void *)0); bb = bb->next_bb)
 {
   ...
   real_arithmetic (&(tmp), MULT_EXPR, &(((block_info) (bb)->aux)->frequency), &(freq_max));

   real_arithmetic (&(tmp), PLUS_EXPR, &(tmp), &(real_one_half));
   bb->frequency = real_to_integer (&(tmp));
 }
