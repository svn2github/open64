...
# 48 "../../../kgccfe/gnu/diagnostic.c" 2
# 1 "../../../kgccfe/gnu/langhooks-def.h" 1
# 25 "../../../kgccfe/gnu/langhooks-def.h"
...
_Bool default_can_output_mi_thunk_no_vcall
  (tree, long long, long long, tree);
# 26 "../../../kgccfe/gnu/langhooks-def.h" 2
...
static char *vbuild_message_string (const char *, va_list)
...
static void diagnostic_for_decl (diagnostic_info *, tree);
...
static void output_long_unsigned_decimal (output_buffer *, long unsigned int);
...
static void output_pointer (output_buffer *, void *);
...
static void default_diagnostic_starter (diagnostic_context *, diagnostic_info *);

static void default_diagnostic_finalizer (diagnostic_context *, diagnostic_info *);


static void error_recursion (diagnostic_context *) __attribute__ ((__noreturn__));
static _Bool text_specifies_location (text_info *, location_t *);
     ...
     output_buffer *buffer;
{
  ...
  if (!((buffer)->state.ideal_maximum_length > 0)
      || (buffer)->state.prefixing_rule == DIAGNOSTICS_SHOW_PREFIX_ONCE
      || (buffer)->state.prefixing_rule == DIAGNOSTICS_SHOW_PREFIX_NEVER)
    (buffer)->state.maximum_length = (buffer)->state.ideal_maximum_length;
  else
    {
      ...
      if ((buffer)->state.ideal_maximum_length - prefix_length < 32)
 (buffer)->state.maximum_length = (buffer)->state.ideal_maximum_length + 32;
      else
 (buffer)->state.maximum_length = (buffer)->state.ideal_maximum_length;
    }
}
...
output_set_maximum_length (buffer, length)
     ...
     int length;
{
  (buffer)->state.ideal_maximum_length = length;
  set_real_maximum_length (buffer);
}
     ...
     const char *prefix;
{
  ...
  set_real_maximum_length (buffer);
  (buffer)->state.emitted_prefix_p = 0;
  (buffer)->state.indent_skip = 0;
}
     ...
     const output_buffer *buffer;
{
  ...
  if (((&buffer->obstack)->object_base) != ((&buffer->obstack)->next_free))
    p = ((const char *) ((&buffer->obstack)->next_free)) - 1;
  return p;
}
...
output_destroy_prefix (buffer)
     output_buffer *buffer;
{
  if (buffer->state.prefix != ((void *)0))
    {
      free ((char *) buffer->state.prefix);
      buffer->state.prefix = ((void *)0);
    }
}
...
output_clear_message_text (buffer)
     output_buffer *buffer;
{
  __extension__ ({ struct obstack *__o = (&buffer->obstack); void *__obj = (((&buffer->obstack)->object_base)); if (__obj > (void *)__o->chunk && __obj < (void *)__o->chunk_limit) __o->next_free = __o->object_base = __obj; else (obstack_free) (__o, __obj); });
     ...
     int maximum_length;
{
  ...
  (buffer)->stream = stderr;
  (buffer)->state.ideal_maximum_length = maximum_length;
  (buffer)->state.prefixing_rule = (&(global_dc)->buffer)->state.prefixing_rule;
  output_set_prefix (buffer, prefix);
  (buffer)->line_length = 0;
  output_clear_data (buffer);
}
     ...
     output_buffer *buffer;
{
  ...
  return ((const char *) ((&(buffer)->obstack)->object_base));
}
...
output_space_left (buffer)
     const output_buffer *buffer;
{
  return (buffer)->state.maximum_length - (buffer)->line_length;
}
     ...
     output_buffer *buffer;
{
  if (buffer->state.prefix != ((void *)0))
    {
      switch ((buffer)->state.prefixing_rule)
 {
 default:
 ...
 case DIAGNOSTICS_SHOW_PREFIX_ONCE:
   if ((buffer)->state.emitted_prefix_p)
     {
       output_indent (buffer);
       break;
     }
   (buffer)->state.indent_skip += 3;
 ...
 case DIAGNOSTICS_SHOW_PREFIX_EVERY_LINE:
   {
     ...
     (buffer)->state.emitted_prefix_p = 1;
   }
   break;
 }
    }
...
output_add_newline (buffer)
     output_buffer *buffer;
{
  __extension__ ({ struct obstack *__o = (&buffer->obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = ('\n'); (void) 0; });
  (buffer)->line_length = 0;
}
...
output_add_character (buffer, c)
     ...
     int c;
{
  if (((buffer)->state.ideal_maximum_length > 0) && output_space_left (buffer) <= 0)
    output_add_newline (buffer);
     ...
     output_buffer *buffer;
{
  ...
  __extension__ ({ struct obstack *__o = (&buffer->obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (' '); (void) 0; });
  ++(buffer)->line_length;
}
...
output_decimal (buffer, i)
     ...
     int i;
{
  do { sprintf ((buffer)->digit_buffer, "%d", i); output_add_string (buffer, (buffer)->digit_buffer); } while (0);
}
...
output_long_decimal (buffer, i)
     ...
     long int i;
{
  do { sprintf ((buffer)->digit_buffer, "%ld", i); output_add_string (buffer, (buffer)->digit_buffer); } while (0);
}
...
output_unsigned_decimal (buffer, i)
     ...
     unsigned int i;
{
  do { sprintf ((buffer)->digit_buffer, "%u", i); output_add_string (buffer, (buffer)->digit_buffer); } while (0);
}
...
output_long_unsigned_decimal (buffer, i)
     ...
     long unsigned int i;
{
  do { sprintf ((buffer)->digit_buffer, "%lu", i); output_add_string (buffer, (buffer)->digit_buffer); } while (0);
}
...
output_octal (buffer, i)
     ...
     unsigned int i;
{
  do { sprintf ((buffer)->digit_buffer, "%o", i); output_add_string (buffer, (buffer)->digit_buffer); } while (0);
}
...
output_long_octal (buffer, i)
     ...
     unsigned long int i;
{
  do { sprintf ((buffer)->digit_buffer, "%lo", i); output_add_string (buffer, (buffer)->digit_buffer); } while (0);
}
...
output_hexadecimal (buffer, i)
     ...
     unsigned int i;
{
  do { sprintf ((buffer)->digit_buffer, "%x", i); output_add_string (buffer, (buffer)->digit_buffer); } while (0);
}
...
output_long_hexadecimal (buffer, i)
     ...
     unsigned long int i;
{
  do { sprintf ((buffer)->digit_buffer, "%lx", i); output_add_string (buffer, (buffer)->digit_buffer); } while (0);
}
...
output_pointer (buffer, p)
     ...
     void *p;
{
  do { sprintf ((buffer)->digit_buffer, "%p", p); output_add_string (buffer, (buffer)->digit_buffer); } while (0);
}
...
output_append_r (buffer, start, length)
     ...
     int length;
{
  __extension__ ({ struct obstack *__o = (&buffer->obstack); int __len = (length); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((start)), (__len)); __o->next_free += __len; (void) 0; });
...
output_indent (buffer)
     output_buffer *buffer;
{
  int n = (buffer)->state.indent_skip;
  int i;
...
wrap_text (buffer, start, end)
     ...
     const char *end;
{
  _Bool is_wrapping = ((buffer)->state.ideal_maximum_length > 0);
...
output_add_identifier (buffer, id)
     ...
     tree id;
{
  output_append (buffer, ((char *)(id)->identifier.id.str),
   ((char *)(id)->identifier.id.str) + ((id)->identifier.id.len));
}
     ...
     output_buffer *buffer;
{
  ...
  fputs_unlocked (text, (buffer)->stream);
  output_clear_message_text (buffer);
}
# 520 "../../../kgccfe/gnu/diagnostic.c"
static void
     ...
     text_info *text;
{
 ...
 {
 case 'c':
   output_add_character (buffer, __builtin_va_arg(*text->args_ptr,int));
   break;

 case 'd':
 case 'i':
   if (long_integer)
     output_long_decimal (buffer, __builtin_va_arg(*text->args_ptr,long int));
   else
     output_decimal (buffer, __builtin_va_arg(*text->args_ptr,int));
   break;

 case 'o':
   if (long_integer)
     output_long_octal (buffer,
          __builtin_va_arg(*text->args_ptr,unsigned long int));
   else
     output_octal (buffer, __builtin_va_arg(*text->args_ptr,unsigned int));
   break;

 case 's':
   output_add_string (buffer, __builtin_va_arg(*text->args_ptr,const char *));
   break;

        case 'p':
          output_pointer (buffer, __builtin_va_arg(*text->args_ptr,void *));
          break;

 case 'u':
   if (long_integer)
     output_long_unsigned_decimal
       (buffer, __builtin_va_arg(*text->args_ptr,long unsigned int));
   else
     output_unsigned_decimal
       (buffer, __builtin_va_arg(*text->args_ptr,unsigned int));
   break;

 case 'x':
   if (long_integer)
     output_long_hexadecimal
       (buffer, __builtin_va_arg(*text->args_ptr,unsigned long int));
   else
     output_hexadecimal
              (buffer, __builtin_va_arg(*text->args_ptr,unsigned int));
   break;
 ...
 case '%':
   ...
   break;
        ...
        case 'H':
          {
            const location_t *locus = __builtin_va_arg(*text->args_ptr,location_t *);
 ...
 case '.':
   {
     ...
     if (*++text->format_spec != '*')
       fancy_abort ("../../../kgccfe/gnu/diagnostic.c", 620, __FUNCTION__);
     else if (*++text->format_spec != 's')
       fancy_abort ("../../../kgccfe/gnu/diagnostic.c", 622, __FUNCTION__);
     n = __builtin_va_arg(*text->args_ptr,int);
     s = __builtin_va_arg(*text->args_ptr,const char *);
 ...
 default:
   if (!buffer->format_decoder
              || !(*buffer->format_decoder) (buffer, text))
     {
       ...
       fancy_abort ("../../../kgccfe/gnu/diagnostic.c", 635, __FUNCTION__);
     }
 }
    }
}
...
build_message_string (const char *msg, ...)
{
  ...
  { va_list ap; __builtin_va_start(ap,msg); { struct Qdmy;
  struct Qdmy;
     ...
     tree decl;
{
      ...
      if (*p == '%')
 {
   if (*(p + 1) == '%')
     ...
     fancy_abort ("../../../kgccfe/gnu/diagnostic.c", 711, __FUNCTION__);
   else
     break;
 }
    }
  ...
  if (*p == '%')
    {
      const char *const n = (((decl)->decl.name)
 ...
 {
   ++p;
   if ((_sch_istable[(*(p - 1) & 0xFF) & 0xff] & (unsigned short)(_sch_isalpha)))
     break;
 }
    }
...
output_do_verbatim (buffer, text)
     ...
     text_info *text;
{
  diagnostic_prefixing_rule_t rule = (buffer)->state.prefixing_rule;
  int line_cutoff = (buffer)->state.ideal_maximum_length;


  (buffer)->state.prefixing_rule = DIAGNOSTICS_SHOW_PREFIX_NEVER;
  (buffer)->state.ideal_maximum_length = 0;

  output_format (buffer, text);

  (buffer)->state.prefixing_rule = rule;
  (buffer)->state.ideal_maximum_length = line_cutoff;
}
     ...
     diagnostic_context *context;
{
  ...
  _obstack_begin ((&context->buffer.obstack), 0, 0, (void *(*) (long)) xmalloc, (void (*) (void *)) free);


  (&context->buffer)->stream = stderr;


  (&(context)->buffer)->state.prefixing_rule = DIAGNOSTICS_SHOW_PREFIX_ONCE;
     ...
     location_t *locus;
{
  ...
  if (*p == '%' && *++p == 'H')
    {
      *locus = *__builtin_va_arg(*text->args_ptr,location_t *);
...
("note: "),
("debug: "),
# 861 "../../../kgccfe/gnu/diagnostic.c" 2

    "must-not-happen"
  };
   if (diagnostic->kind >= DK_LAST_DIAGNOSTIC_KIND)
     fancy_abort ("../../../kgccfe/gnu/diagnostic.c", 865, __FUNCTION__);
    ...
    diagnostic_t kind;
{
  switch (kind)
    {
    default:
      fancy_abort ("../../../kgccfe/gnu/diagnostic.c", 916, __FUNCTION__);
      break;
...
fatal_io_error (const char *msgid, ...)
{
  ...
  text.format_spec = (msgid);
  text.args_ptr = &ap;
  output_printf (&global_dc->buffer, "%s: %s: ", progname, xstrerror ((*__errno_location ())));
...
pedwarn_with_decl (tree decl, const char *msgid, ...)
{
  ...
  diagnostic_set_info (&diagnostic, (msgid), &ap,
                       (((decl)->decl.locus).file), (((decl)->decl.locus).line),
                       (flag_pedantic_errors ? DK_ERROR : DK_WARNING));
...
void
pedwarn_with_file_and_line (const char *file, int line, const char *msgid, ...)

{
  ...
  diagnostic_set_info (&diagnostic, (msgid), &ap, file, line,
                       (flag_pedantic_errors ? DK_ERROR : DK_WARNING));
  diagnostic_report_diagnostic (global_dc, &diagnostic);
  } __builtin_va_end(ap); };
}
     ...
     tree decl;
{
 ...
 verbatim ("%s ", ((char *)(((decl)->decl.name))->identifier.id.str));
      else
 verbatim (" %s", (*lang_hooks.decl_printable_name) (decl, 2));
      fflush (stderr);
      (&global_dc->buffer)->state.need_newline_p = 1;
  ...
  if (((context)->last_function != current_function_decl))
    {
      ...
      char *new_prefix = file ? build_message_string ("%s: ", file) : ((void *)0);

      output_set_prefix (&context->buffer, new_prefix);

      if (current_function_decl == ((void *)0))
 output_add_string (&context->buffer, ("At top level:"));
      else
 {
...
void
error_with_file_and_line (const char *file, int line, const char *msgid, ...)
...
internal_error (const char *msgid, ...)
{
  ...
  if (global_dc->internal_error != 0)
    (*global_dc->internal_error) ((msgid), &ap);
...
void
warning_with_file_and_line (const char *file, int line, const char *msgid, ...)
...
trim_filename (name)
     const char *name;
{
  static const char this_file[] = "../../../kgccfe/gnu/diagnostic.c";
     ...
     diagnostic_context *context;
{
  ...
  if ((&context->buffer)->state.need_newline_p)
    {
      output_add_newline (&context->buffer);
      (&context->buffer)->state.need_newline_p = 0;
    }
     ...
     tree node;
{
  ...
  if ((tree_code_type[(int) (((enum tree_code) (node)->common.code))] == 'd'))
    warning ("`%s' is deprecated (declared at %s:%d)",
      ...
      (((node)->decl.locus).file), (((node)->decl.locus).line));
 ...
 what = ((char *)(((((node)->type.name))->decl.name))->identifier.id.str);
