     ...
     unsigned int n_elms;
{
  ...
  size = (((n_elms) + ((unsigned) (8 * 8)) - 1) / ((unsigned) (8 * 8)));
  bytes = size * sizeof (unsigned long long);
  ...
  if (bytes > bmap->bytes)
    {
      amt = (sizeof (struct simple_bitmap_def)
     + bytes - sizeof (unsigned long long));
      bmap = (sbitmap) xrealloc ((void *) bmap, amt);
    }
  ...
  if (n_elms > bmap->n_bits)
    {
      if (def)
 {
   memset ((void *) (bmap->elms + bmap->size), -1, bytes - bmap->bytes);


   last_bit = bmap->n_bits % ((unsigned) (8 * 8));
   if (last_bit)
     bmap->elms[bmap->size - 1]
       |= ~((unsigned long long)-1 >> (((unsigned) (8 * 8)) - last_bit));


   last_bit = n_elms % ((unsigned) (8 * 8));
   if (last_bit)
     bmap->elms[size - 1]
       &= (unsigned long long)-1 >> (((unsigned) (8 * 8)) - last_bit);
 }
      else
 memset ((void *) (bmap->elms + bmap->size), 0, bytes - bmap->bytes);
    }
     ...
     unsigned int n_vecs, n_elms;
{
...

  {
    ...
    struct { char x; unsigned long long y; } align;
...
sbitmap_copy (dst, src)
     sbitmap dst, src;
{
  memcpy (dst->elms, src->elms, sizeof (unsigned long long) * dst->size);
}
...
sbitmap_equal (a, b)
     sbitmap a, b;
{
  return !memcmp (a->elms, b->elms, sizeof (unsigned long long) * a->size);
}
...
sbitmap_zero (bmap)
     sbitmap bmap;
{
  memset ((void *) bmap->elms, 0, bmap->bytes);
}
     ...
     sbitmap bmap;
{
  ...
  memset ((void *) bmap->elms, -1, bmap->bytes);
     ...
     sbitmap dst, a, b, c;
{
  ...
  for (i = 0; i < n; i++)
    {
      unsigned long long tmp = *ap++ | (*bp++ & ~*cp++);
      changed |= *dstp ^ tmp;
      *dstp++ = tmp;
    }
     ...
     sbitmap dst, a, b;
{
  ...
  if (a->size < dst_size)
    fancy_abort ("../../../kgccfe/gnu/sbitmap.c", 287, __FUNCTION__);
     ...
     sbitmap bmap;
{
  ...
  do { unsigned int word_num_; unsigned int bit_num_ = (0) % (unsigned int) ((unsigned) (8 * 8)); unsigned int size_ = (bmap)->size; unsigned long long *ptr_ = (bmap)->elms; for (word_num_ = (0) / (unsigned int) ((unsigned) (8 * 8)); word_num_ < size_; word_num_++, bit_num_ = 0) { unsigned long long word_ = ptr_[word_num_]; if (word_ != 0) for (; bit_num_ < ((unsigned) (8 * 8)); bit_num_++) { unsigned long long _mask = (unsigned long long) 1 << bit_num_; if ((word_ & _mask) != 0) { word_ &= ~ _mask; (n) = word_num_ * ((unsigned) (8 * 8)) + bit_num_; { return n; }; if (word_ == 0) break; } } } } while (0);
  return -1;
}
...
{
  int i;
  unsigned long long *ptr = bmap->elms;

  for (i = bmap->size - 1; i >= 0; i--)
    {
      unsigned long long word = ptr[i];

      if (word != 0)
 {
   unsigned int index = (i + 1) * ((unsigned) (8 * 8)) - 1;
   unsigned long long mask
     = (unsigned long long) 1 << (((unsigned) (8 * 8)) - 1);
     ...
     sbitmap bmap;
{
  ...
  for (i = n = 0; i < set_size && n < total_bits; i++)
    for (j = 0; j < ((unsigned) (8 * 8)) && n < total_bits; j++, n++)
      {
 if (n != 0 && n % 10 == 0)
   fprintf (file, " ");

 fprintf (file, "%d",
   (bmap->elms[i] & ((unsigned long long) 1 << j)) != 0);
      }
     ...
     sbitmap bmap;
{
  ...
  for (pos = 30, i = 0; i < bmap->n_bits; i++)
    if (((bmap)->elms [(i) / ((unsigned) (8 * 8))] >> (i) % ((unsigned) (8 * 8)) & 1))
      {
 if (pos > 70)
   {
     fprintf (file, "\n  ");
