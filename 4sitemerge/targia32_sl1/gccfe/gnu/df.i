...
# 160 "../../../kgccfe/gnu/df.c" 2
# 1 "../../../kgccfe/gnu/insn-config.h" 1
# 161 "../../../kgccfe/gnu/df.c" 2
...
# 170 "../../../kgccfe/gnu/df.c" 2
# 1 "../../../include/gnu/fibheap.h" 1
# 49 "../../../include/gnu/fibheap.h"
...
static __inline__ struct df_link *df_link_create (struct ref *, struct df_link *);
...
static struct ref *df_ref_create (struct df *, rtx, rtx *, rtx, enum df_ref_type, enum df_ref_flags);


static void df_ref_record_1 (struct df *, rtx, rtx *, rtx, enum df_ref_type, enum df_ref_flags);


static void df_ref_record (struct df *, rtx, rtx *, rtx, enum df_ref_type, enum df_ref_flags);


static void df_def_record_1 (struct df *, rtx, basic_block, rtx);
static void df_defs_record (struct df *, rtx, basic_block, rtx);
static void df_uses_record (struct df *, rtx *, enum df_ref_type, basic_block, rtx, enum df_ref_flags);
...
static struct ref *df_bb_regno_last_use_find (struct df *, basic_block, unsigned int);

static struct ref *df_bb_regno_first_def_find (struct df *, basic_block, unsigned int);

static struct ref *df_bb_insn_regno_last_use_find (struct df *, basic_block, rtx, unsigned int);


static struct ref *df_bb_insn_regno_first_def_find (struct df *, basic_block, rtx, unsigned int);
...
static void df_rd_transfer_function (int, int *, bitmap, bitmap, bitmap, bitmap, void *);

static void df_ru_transfer_function (int, int *, bitmap, bitmap, bitmap, bitmap, void *);

static void df_lr_transfer_function (int, int *, bitmap, bitmap, bitmap, bitmap, void *);

static void hybrid_search_bitmap (basic_block, bitmap *, bitmap *, bitmap *, bitmap *, enum df_flow_dir, enum df_confluence_op, transfer_function_bitmap, sbitmap, sbitmap, void *);




static void hybrid_search_sbitmap (basic_block, sbitmap *, sbitmap *, sbitmap *, sbitmap *, enum df_flow_dir, enum df_confluence_op, transfer_function_sbitmap, sbitmap, sbitmap, void *);




static __inline__ _Bool read_modify_subreg_p (rtx);
     ...
     unsigned int size;
{
  ...
  if (! df->insns_modified)
    {
      df->insns_modified = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
     ...
     int flags;
{
  ...
  if ((flags & 4) && df->n_regs < (unsigned int) max_reg_num ())
    dflags |= 4 | 2;
  if ((flags & 2) && df->n_uses < df->use_id)
    dflags |= 2;
  if ((flags & 1) && df->n_defs < df->def_id)
    dflags |= 1;
  ...
  for (bb = (&entry_exit_blocks[0])->next_bb; bb != (&entry_exit_blocks[1]); bb = bb->next_bb)
    {
      struct bb_info *bb_info = (&df->bbs[(bb)->index]);

      if (flags & 1 && ! bb_info->rd_in)
 {

   bb_info->rd_kill = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
   bitmap_clear (bb_info->rd_kill);
   bb_info->rd_gen = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
   bitmap_clear (bb_info->rd_gen);
   bb_info->rd_in = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
   bb_info->rd_out = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
   bb_info->rd_valid = 0;
 }
      ...
      if (flags & 2 && ! bb_info->ru_in)
 {
   ...
   bb_info->ru_kill = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
   bitmap_clear (bb_info->ru_kill);

   bb_info->ru_gen = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
   bitmap_clear (bb_info->ru_gen);
   bb_info->ru_in = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
   bb_info->ru_out = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
   bb_info->ru_valid = 0;
 }
      ...
      if (flags & 4 && ! bb_info->lr_in)
 {
   ...
   bb_info->lr_def = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
   bitmap_clear (bb_info->lr_def);
   bb_info->lr_use = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
   bitmap_clear (bb_info->lr_use);
   bb_info->lr_in = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
   bb_info->lr_out = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
   bb_info->lr_valid = 0;
 }
    }
}
     ...
     int n_regs;
{
  ...
  df->bbs_modified = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
  ...
  df->all_blocks = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
  for (bb = (&entry_exit_blocks[0])->next_bb; bb != (&entry_exit_blocks[1]); bb = bb->next_bb)
     ...
     struct df *df;
{
  ...
  __extension__ ({ struct obstack *__o = (&df_ref_obstack); void *__obj = (((void *)0)); if (__obj > (void *)__o->chunk && __obj < (void *)__o->chunk_limit) __o->next_free = __o->object_base = __obj; else (obstack_free) (__o, __obj); });
}
     ...
     unsigned int regno;
{
  ...
  use = gen_rtx_fmt_e (CLOBBER, (((enum machine_mode) (reg)->mode)), (reg));
  return use;
}
...
static __inline__ struct df_link *
     ...
     struct ref *ref;
{
      ...
      if (! link->next)
 {
   ...
   if (link->ref != ref)
     fancy_abort ("../../../kgccfe/gnu/df.c", 673, __FUNCTION__);
  ...
  if ((((ref)->type) == DF_REF_REG_DEF))
    {
      ...
      df_ref_unlink (&df->insns[(((((ref)->insn)->fld[0]).rtint))].defs, ref);
    }
  else
    {
      df_use_unlink (df, ref);
      df_ref_unlink (&df->insns[(((((ref)->insn)->fld[0]).rtint))].uses, ref);
    }
  return 1;
}
     ...
     struct ref *def;
{
  ...
  unsigned int dregno = ((((((enum rtx_code) ((def)->reg)->code) == SUBREG ? ((((def)->reg)->fld[0]).rtx) : ((def)->reg)))->fld[0]).rtuint);
     ...
     struct ref *use;
{
  ...
  unsigned int uregno = ((((((enum rtx_code) ((use)->reg)->code) == SUBREG ? ((((use)->reg)->fld[0]).rtx) : ((use)->reg)))->fld[0]).rtuint);
     ...
     enum df_ref_flags ref_flags;
{
  ...
  this_ref = (struct ref *) __extension__ ({ struct obstack *__h = (&df_ref_obstack); __extension__ ({ struct obstack *__o = (__h); int __len = ((sizeof (*this_ref))); if (__o->chunk_limit - __o->next_free < __len) _obstack_newchunk (__o, __len); __o->next_free += __len; (void) 0; }); __extension__ ({ struct obstack *__o1 = (__h); void *value; value = (void *) __o1->object_base; if (__o1->next_free == value) __o1->maybe_empty_object = 1; __o1->next_free = (((((__o1->next_free) - (char *) 0)+__o1->alignment_mask) & ~ (__o1->alignment_mask)) + (char *) 0); if (__o1->next_free - (char *)__o1->chunk > __o1->chunk_limit - (char *)__o1->chunk) __o1->next_free = __o1->chunk_limit; __o1->object_base = __o1->next_free; value; }); });
     ...
     enum df_ref_flags ref_flags;
{
  ...
  if (((enum rtx_code) (reg)->code) != REG && ((enum rtx_code) (reg)->code) != SUBREG)
    fancy_abort ("../../../kgccfe/gnu/df.c", 856, __FUNCTION__);
  ...
  if (((enum rtx_code) (reg)->code) == SUBREG
      && ((mode_size[(int) (((enum machine_mode) (reg)->mode))]) < (mode_size[(int) (word_mode)])
   || (mode_size[(int) (((enum machine_mode) (reg)->mode))])
        >= (mode_size[(int) (((enum machine_mode) ((((reg)->fld[0]).rtx))->mode))])))
    {
      loc = &(((reg)->fld[0]).rtx);
      reg = *loc;
    }

  regno = (((((enum rtx_code) (reg)->code) == SUBREG ? (((reg)->fld[0]).rtx) : reg)->fld[0]).rtuint);
  if (regno < 176)
    {
      int i;
      int endregno;
 ...
 return;
      ...
      endregno = mips_hard_regno_nregs (regno, ((enum machine_mode) (reg)->mode));
      if (((enum rtx_code) (reg)->code) == SUBREG)
        regno += subreg_regno_offset (regno, ((enum machine_mode) ((((reg)->fld[0]).rtx))->mode),
          (((reg)->fld[1]).rtuint), ((enum machine_mode) (reg)->mode));
      endregno += regno;
     ...
     rtx x;
{
  ...
  if (((enum rtx_code) (x)->code) != SUBREG)
    return 0;
  isize = (mode_size[(int) (((enum machine_mode) ((((x)->fld[0]).rtx))->mode))]);
  osize = (mode_size[(int) (((enum machine_mode) (x)->mode))]);
  if (isize <= osize)
    return 1;
  if (isize <= ((target_flags & 0x00002000) ? 8 : 4))
    return 0;
  ...
  if (((enum rtx_code) (dst)->code) == PARALLEL && ((enum machine_mode) (dst)->mode) == BLKmode)
    {
      ...
      for (i = (((((dst)->fld[0]).rtvec))->num_elem) - 1; i >= 0; i--)
 df_def_record_1 (df, (((((dst)->fld[0]).rtvec))->elem[i]), bb, insn);
      return;
    }
# 955 "../../../kgccfe/gnu/df.c"
      ...
      if (((enum rtx_code) (dst)->code) == STRICT_LOW_PART)
 {
  ...
  if (((enum rtx_code) (dst)->code) == REG
      || (((enum rtx_code) (dst)->code) == SUBREG && ((enum rtx_code) ((((dst)->fld[0]).rtx))->code) == REG))
     ...
     rtx insn;
{
 ...
 {
   code = ((enum rtx_code) ((((((x)->fld[0]).rtvec))->elem[i]))->code);
   if (code == SET || code == CLOBBER)
     df_def_record_1 (df, (((((x)->fld[0]).rtvec))->elem[i]), bb, insn);
 }
    }
}
     ...
     enum df_ref_flags flags;
{
 ...
 df_uses_record (df, &((((((x)->fld[0]).rtx))->fld[0]).rtx),
   DF_REF_REG_MEM_STORE, bb, insn, flags);
    ...
    case MEM:
      df_uses_record (df, &(((x)->fld[0]).rtx), DF_REF_REG_MEM_LOAD, bb, insn, flags);
      return;
...
# 1077 "../../../kgccfe/gnu/df.c"
 ...
 df_uses_record (df, &(((x)->fld[1]).rtx), DF_REF_REG_USE, bb, insn, 0);
 ...
 switch (((enum rtx_code) (dst)->code))
       ...
       if (read_modify_subreg_p (dst))
  {
    ...
    df_uses_record (df, &(((dst)->fld[0]).rtx), DF_REF_REG_USE, bb,
      insn, use_flags);
    break;
  }
  ...
  break;
     case MEM:
       df_uses_record (df, &(((dst)->fld[0]).rtx),
         DF_REF_REG_MEM_STORE,
         bb, insn, 0);
       break;
  ...
  fancy_abort ("../../../kgccfe/gnu/df.c", 1119, __FUNCTION__);
       use_flags = DF_REF_READ_WRITE;
     ...
     case ZERO_EXTRACT:
     case SIGN_EXTRACT:
       df_uses_record (df, &(((dst)->fld[0]).rtx), DF_REF_REG_USE, bb, insn,
         DF_REF_READ_WRITE);
       df_uses_record (df, &(((dst)->fld[1]).rtx), DF_REF_REG_USE, bb, insn, 0);
       df_uses_record (df, &(((dst)->fld[2]).rtx), DF_REF_REG_USE, bb, insn, 0);
       dst = (((dst)->fld[0]).rtx);
       break;
     default:
       fancy_abort ("../../../kgccfe/gnu/df.c", 1138, __FUNCTION__);
   }
 return;
      }
    ...
    case ASM_INPUT:
      {
# 1166 "../../../kgccfe/gnu/df.c"
 if (code == ASM_OPERANDS)
   {
     int j;

     for (j = 0; j < (((((x)->fld[3]).rtvec))->num_elem); j++)
       df_uses_record (df, &(((((x)->fld[3]).rtvec))->elem[j]),
 ...
 break;
    ...
    case POST_MODIFY:
      ...
      df_ref_record (df, (((x)->fld[0]).rtx), &(((x)->fld[0]).rtx), insn, DF_REF_REG_DEF, DF_REF_READ_WRITE);
 ...
 if (fmt[i] == 'e')
  ...
  goto retry;
     ...
     df_uses_record (df, &(((x)->fld[i]).rtx), ref_type, bb, insn, flags);
   }
 else if (fmt[i] == 'E')
   {
     int j;
     for (j = 0; j < (((((x)->fld[i]).rtvec))->num_elem); j++)
       df_uses_record (df, &(((((x)->fld[i]).rtvec))->elem[j]), ref_type,
         bb, insn, flags);
   }
      }
  }
     ...
     rtx insn;
{
  ...
  if (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i'))
    {
      ...
      df_defs_record (df, (((insn)->fld[5]).rtx), bb, insn);

      if (df->flags & 2048)
 for (note = (((insn)->fld[8]).rtx); note;
      note = (((note)->fld[1]).rtx))
   {
     switch (((enum reg_note) ((enum machine_mode) (note)->mode)))
       ...
       case REG_EQUIV:
       case REG_EQUAL:
  df_uses_record (df, &(((note)->fld[0]).rtx), DF_REF_REG_USE,
    bb, insn, 0);
       default:
  break;
       }
      ...
      if (((enum rtx_code) (insn)->code) == CALL_INSN)
 {
   ...
   for (note = (((insn)->fld[9]).rtx); note;
        note = (((note)->fld[1]).rtx))
     {
       if (((enum rtx_code) ((((note)->fld[0]).rtx))->code) == USE)
  df_uses_record (df, &((((((note)->fld[0]).rtx))->fld[0]).rtx), DF_REF_REG_USE,
    bb, insn, 0);
     }


   x = df_reg_use_gen ((0 + 29));
   df_uses_record (df, &(((x)->fld[0]).rtx), DF_REF_REG_USE, bb, insn, 0);
  ...
  if (global_regs[i])
    {
      ...
      df_uses_record (df, &(((insn)->fld[5]).rtx),
        DF_REF_REG_USE, bb, insn, 0);
  ...
  if ((!!((regs_invalidated_by_call)[(i) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((i) % ((unsigned) (8 * 8)))))))
   ...
   for (note = (((insn)->fld[9]).rtx);
     ...
     if (((enum rtx_code) ((((note)->fld[0]).rtx))->code) == CLOBBER)
       df_defs_record (df, (((note)->fld[0]).rtx), bb, insn);
 }
    }
}
     ...
     basic_block bb;
{
  ...
  for (insn = bb->end; insn && insn != (((bb->head)->fld[1]).rtx);
       insn = (((insn)->fld[1]).rtx))
    {
      struct df_link *link;
     ...
     basic_block bb;
{
  ...
  for (insn = bb->head; insn && insn != (((bb->end)->fld[2]).rtx);
       insn = (((insn)->fld[2]).rtx))
    {
      struct df_link *link;
      unsigned int uid = (((insn)->fld[0]).rtint);
     ...
     bitmap blocks;
{
  ...
  ru = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
     ...
     basic_block bb;
{
 ...
 {
   struct ref *use = use_link->ref;
   unsigned int regno = ((((((enum rtx_code) ((use)->reg)->code) == SUBREG ? ((((use)->reg)->fld[0]).rtx) : ((use)->reg)))->fld[0]).rtuint);
 ...
 {
   struct ref *def = def_link->ref;
   int dregno = ((((((enum rtx_code) ((def)->reg)->code) == SUBREG ? ((((def)->reg)->fld[0]).rtx) : ((def)->reg)))->fld[0]).rtuint);
...
df_rd_transfer_function (bb, changed, in, out, gen, kill, data)
     int bb __attribute__ ((__unused__));
     int *changed;
     ...
     basic_block bb;
{
 ...
 {
   struct ref *def = def_link->ref;
   unsigned int regno = ((((((enum rtx_code) ((def)->reg)->code) == SUBREG ? ((((def)->reg)->fld[0]).rtx) : ((def)->reg)))->fld[0]).rtuint);
   ...
   for (def2_link = df->regs[regno].defs; def2_link;
        def2_link = def2_link->next)
     {
       ...
       bitmap_clear_bit (bb_info->rd_gen, ((def2)->id));
     }
     ...
     bitmap blocks;
{
  ...
  do { unsigned int node_; do { bitmap_element *ptr_ = (blocks)->first; unsigned int indx_ = (0) / ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))); unsigned bit_num_ = (0) % (unsigned) (8 * 4); unsigned word_num_ = (0) / (unsigned) (8 * 4) % ((128 + (8 * 4) - 1) / (8 * 4)); while (ptr_ != 0 && ptr_->indx < indx_) ptr_ = ptr_->next; if (ptr_ != 0 && ptr_->indx != indx_) { bit_num_ = 0; word_num_ = 0; } for (; ptr_ != 0; ptr_ = ptr_->next) { for (; word_num_ < ((128 + (8 * 4) - 1) / (8 * 4)); word_num_++) { BITMAP_WORD word_ = ptr_->bits[word_num_]; if (word_ != 0) { for (; bit_num_ < (unsigned) (8 * 4); bit_num_++) { BITMAP_WORD mask_ = ((BITMAP_WORD) 1) << bit_num_; if ((word_ & mask_) != 0) { word_ &= ~ mask_; (node_) = (ptr_->indx * ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))) + word_num_ * (unsigned) (8 * 4) + bit_num_); {(bb) = (((basic_block_info)->data.bb[(node_)])); { df_bb_rd_local_compute (df, bb); };}; if (word_ == 0) break; } } } bit_num_ = 0; } word_num_ = 0; } } while (0); } while (0);
     ...
     basic_block bb;
{
      ...
      for (link = df->insns[uid].uses; link; link = link->next)
 {
   ...
   bitmap_set_bit (bb_info->lr_use, ((((((enum rtx_code) ((use)->reg)->code) == SUBREG ? ((((use)->reg)->fld[0]).rtx) : ((use)->reg)))->fld[0]).rtuint));
 }
    }
  bb_info->lr_valid = 1;
}
     ...
     bitmap blocks;
{
  ...
  live = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
     ...
     basic_block bb;
{
  ...
  for (insn = bb->head; ; insn = (((insn)->fld[2]).rtx))
    {
      if (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i'))
 ((df)->insns[(((insn)->fld[0]).rtint)].luid) = luid++;
      ((df)->insns[(((insn)->fld[0]).rtint)].luid) = luid;
...
# 2015 "../../../kgccfe/gnu/df.c"
 ...
 for (bb = (&entry_exit_blocks[0])->next_bb; bb != (&entry_exit_blocks[1]); bb = bb->next_bb)
   {
     in[bb->index] = (&df->bbs[(bb)->index])->rd_in;
     out[bb->index] = (&df->bbs[(bb)->index])->rd_out;
     gen[bb->index] = (&df->bbs[(bb)->index])->rd_gen;
     kill[bb->index] = (&df->bbs[(bb)->index])->rd_kill;
   }
 iterative_dataflow_bitmap (in, out, gen, kill, df->all_blocks,
       FORWARD, UNION, df_rd_transfer_function,
       df->inverse_rc_map, ((void *)0));
 free (in);
 free (out);
 free (gen);
 free (kill);
 ...
 for (bb = (&entry_exit_blocks[0])->next_bb; bb != (&entry_exit_blocks[1]); bb = bb->next_bb)
   {
     in[bb->index] = (&df->bbs[(bb)->index])->ru_in;
     out[bb->index] = (&df->bbs[(bb)->index])->ru_out;
     gen[bb->index] = (&df->bbs[(bb)->index])->ru_gen;
     kill[bb->index] = (&df->bbs[(bb)->index])->ru_kill;
   }
 iterative_dataflow_bitmap (in, out, gen, kill, df->all_blocks,
       BACKWARD, UNION, df_ru_transfer_function,
       df->inverse_rts_map, ((void *)0));
 free (in);
 free (out);
 free (gen);
 free (kill);
 ...
 for (bb = (&entry_exit_blocks[0])->next_bb; bb != (&entry_exit_blocks[1]); bb = bb->next_bb)
   {
     in[bb->index] = (&df->bbs[(bb)->index])->lr_in;
     out[bb->index] = (&df->bbs[(bb)->index])->lr_out;
     use[bb->index] = (&df->bbs[(bb)->index])->lr_use;
     def[bb->index] = (&df->bbs[(bb)->index])->lr_def;
   }
  ...
  for (i = df->def_id_save; i != df->def_id; i++)
    {
      ...
      unsigned int uid = (((((def)->insn)->fld[0]).rtint));
  ...
  for (i = df->use_id_save; i != df->use_id; i++)
    {
      ...
      unsigned int uid = (((((use)->insn)->fld[0]).rtint));
     ...
     int flags;
{
  ...
  if (df->n_bbs && df->n_bbs != (unsigned int) last_basic_block)
    fancy_abort ("../../../kgccfe/gnu/df.c", 2336, __FUNCTION__);
      ...
      else
 {
   ...
   if (! df->n_bbs)
     fancy_abort ("../../../kgccfe/gnu/df.c", 2359, __FUNCTION__);
...
df_insn_refs_unlink (df, bb, insn)
     struct df *df;
     basic_block bb __attribute__ ((__unused__));
     rtx insn;
{
  struct df_link *link;
  unsigned int uid;
     ...
     rtx insn;
{
  ...
  if (insn == bb->head)
    fancy_abort ("../../../kgccfe/gnu/df.c", 2467, __FUNCTION__);
     ...
     rtx newreg;
{
 ...
 continue;
      ...
      if (bitmap_bit_p (blocks, (((((ref)->insn)->fld[3]).bb)->index)))
 {
   ...
   if ((! link->next || ((ref)->insn)
     ...
     {
       args.insn = insn;
       for_each_rtx (&(((insn)->fld[8]).rtx), df_rtx_reg_replace, &args);
     }
 }
      else
 {
   ...
   fancy_abort ("../../../kgccfe/gnu/df.c", 2655, __FUNCTION__);
 }
    }
}
...
df_reg_replace (df, blocks, oldreg, newreg)
     ...
     rtx newreg;
{
  unsigned int oldregno = (((oldreg)->fld[0]).rtuint);
     ...
     rtx newreg;
{
  ...
  if (! ((rtx_class[(int) (((enum rtx_code) (((ref)->insn))->code))]) == 'i'))
    return 0;

  if (oldreg && oldreg != ((ref)->reg))
    fancy_abort ("../../../kgccfe/gnu/df.c", 2695, __FUNCTION__);

  if (! validate_change (((ref)->insn), ((ref)->loc), newreg, 1))
    return 0;

  df_insn_modify (df, (((((ref)->insn)->fld[3]).bb)), ((ref)->insn));
  return 1;
}
     ...
     rtx last_insn;
{
  ...
  for (insn = first_insn; ; insn = (((insn)->fld[2]).rtx))
    {
      unsigned int uid;
 ...
 fancy_abort ("../../../kgccfe/gnu/df.c", 2773, __FUNCTION__);
...
{
  rtx ret_insn;
  rtx prev_insn = (((insn)->fld[1]).rtx);


  if (insn == bb->head)
    fancy_abort ("../../../kgccfe/gnu/df.c", 2801, __FUNCTION__);
  ret_insn = emit_insn_before (pattern, insn);
  if (ret_insn == insn)
    return ret_insn;

  df_insns_modify (df, bb, (((prev_insn)->fld[2]).rtx), ret_insn);
  return ret_insn;
}
     ...
     rtx insn;
{
  ...
  df_insns_modify (df, bb, (((insn)->fld[2]).rtx), ret_insn);
  return ret_insn;
}
     ...
     rtx before_insn;
{
  ...
  for (link = df->insns[uid].defs; link; link = link->next)
    (((((link->ref)->insn)->fld[3]).bb)) = before_bb;
  for (link = df->insns[uid].uses; link; link = link->next)
    (((((link->ref)->insn)->fld[3]).bb)) = before_bb;
# 2883 "../../../kgccfe/gnu/df.c"
  ...
  for (link = df->insns[uid].defs; link; link = link->next)
    {
      ...
      if (((((((enum rtx_code) ((def)->reg)->code) == SUBREG ? ((((def)->reg)->fld[0]).rtx) : ((def)->reg)))->fld[0]).rtuint) == regno)
 return 1;
    }
  ...
  for (du_link = ((def)->chain); du_link; du_link = du_link->next)
    {
      ...
      if (bitmap_bit_p (blocks, (((((use)->insn)->fld[3]).bb)->index)))
 {
  ...
  for (link = df->insns[uid].defs; link; link = link->next)
    {
      ...
      if (bitmap_bit_p (blocks, (((((def)->insn)->fld[3]).bb)->index))
   && ! df_def_dominates_uses_p (df, def, blocks))
 return 0;
    }
  return 1;
     ...
     unsigned int regno;
{
  ...
  basic_block bb_def = def ? (((((def)->insn)->fld[3]).bb)) : 0;
  basic_block bb_use = use ? (((((use)->insn)->fld[3]).bb)) : 0;
...
df_reg_global_p (df, reg)
     ...
     rtx reg;
{
  return df_regno_bb (df, (((reg)->fld[0]).rtuint)) != 0;
}
...
df_reg_lifetime (df, reg)
     ...
     rtx reg;
{
  return df->regs[(((reg)->fld[0]).rtuint)].lifetime;
}
     ...
     rtx reg;
{
  ...
  return bitmap_bit_p (bb_info->lr_in, (((reg)->fld[0]).rtuint));
}
     ...
     rtx reg;
{
  ...
  return bitmap_bit_p (bb_info->lr_out, (((reg)->fld[0]).rtuint));
}
...
df_bb_regs_lives_compare (df, bb, reg1, reg2)
     ...
     rtx reg2;
{
  unsigned int regno1 = (((reg1)->fld[0]).rtuint);
  unsigned int regno2 = (((reg2)->fld[0]).rtuint);
  struct ref *def1;
  struct ref *use1;
  struct ref *def2;
  struct ref *use2;
  ...
  if (df_regno_bb (df, regno1) != bb
    ...
    fancy_abort ("../../../kgccfe/gnu/df.c", 3112, __FUNCTION__);

  def2 = df_bb_regno_first_def_find (df, bb, regno2);
  use1 = df_bb_regno_last_use_find (df, bb, regno1);

  if (((df)->insns[(((((def2)->insn))->fld[0]).rtint)].luid)
      > ((df)->insns[(((((use1)->insn))->fld[0]).rtint)].luid))
    return -1;

  def1 = df_bb_regno_first_def_find (df, bb, regno1);
  use2 = df_bb_regno_last_use_find (df, bb, regno2);

  if (((df)->insns[(((((def1)->insn))->fld[0]).rtint)].luid)
      > ((df)->insns[(((((use2)->insn))->fld[0]).rtint)].luid))
    return 1;
  ...
  for (link = df->insns[uid].uses; link; link = link->next)
    {
      ...
      if (((((((enum rtx_code) ((use)->reg)->code) == SUBREG ? ((((use)->reg)->fld[0]).rtx) : ((use)->reg)))->fld[0]).rtuint) == regno)
 return use;
    }
     ...
     rtx reg;
{
  ...
  def = df_bb_insn_regno_first_def_find (df, bb, insn, (((reg)->fld[0]).rtuint));

  if (! def)
    fancy_abort ("../../../kgccfe/gnu/df.c", 3246, __FUNCTION__);
  ...
  for (; link; link = link->next)
    {
      fprintf (file, "%c%d(%d) ",
        (((link->ref)->type) == DF_REF_REG_DEF) ? 'd' : 'u',
        ((link->ref)->id),
        ((((((enum rtx_code) ((link->ref)->reg)->code) == SUBREG ? ((((link->ref)->reg)->fld[0]).rtx) : ((link->ref)->reg)))->fld[0]).rtuint));
    }
  fprintf (file, "}");
}
     ...
     FILE *file;
{
   ...
   if (df->defs[j])
     {
       fprintf (file, "d%d bb %d luid %d insn %d reg %d ",
         j, (((((df->defs[j])->insn)->fld[3]).bb)->index),
         ((df)->insns[(((((df->defs[j])->insn))->fld[0]).rtint)].luid),
         (((((df->defs[j])->insn)->fld[0]).rtint)),
         ((((((enum rtx_code) ((df->defs[j])->reg)->code) == SUBREG ? ((((df->defs[j])->reg)->fld[0]).rtx) : ((df->defs[j])->reg)))->fld[0]).rtuint));
   ...
   if (df->uses[j])
     {
       fprintf (file, "u%d bb %d luid %d insn %d reg %d ",
         j, (((((df->uses[j])->insn)->fld[3]).bb)->index),
         ((df)->insns[(((((df->uses[j])->insn))->fld[0]).rtint)].luid),
         (((((df->uses[j])->insn)->fld[0]).rtint)),
         ((((((enum rtx_code) ((df->uses[j])->reg)->code) == SUBREG ? ((((df->uses[j])->reg)->fld[0]).rtx) : ((df->uses[j])->reg)))->fld[0]).rtuint));
      ...
      for (j = 0; j < df->n_regs; j++)
 {
   if (((flags & 32)
       ...
       || ((flags & 64) && reg_info[j].defs)
       || ((flags & 128) && reg_info[j].uses))
     {
     ...
     FILE *file;
{
  ...
  if (df->insns[uid].defs)
    bbi = (((((df->insns[uid].defs->ref)->insn)->fld[3]).bb)->index);
  else if (df->insns[uid].uses)
    bbi = (((((df->insns[uid].uses->ref)->insn)->fld[3]).bb)->index);
  else
    bbi = -1;

  fprintf (file, "insn %d bb %d luid %d defs ",
    uid, bbi, ((df)->insns[(((insn)->fld[0]).rtint)].luid));
     ...
     FILE *file;
{
  ...
  fprintf (file, "reg %d bb %d luid %d insn %d chain ",
    ((((((enum rtx_code) ((ref)->reg)->code) == SUBREG ? ((((ref)->reg)->fld[0]).rtx) : ((ref)->reg)))->fld[0]).rtuint),
    (((((ref)->insn)->fld[3]).bb)->index),
    ((df)->insns[(((((ref)->insn))->fld[0]).rtint)].luid),
    (((((ref)->insn))->fld[0]).rtint));
...
debug_df_reg (reg)
     rtx reg;
{
  df_regno_debug (ddf, (((reg)->fld[0]).rtuint), stderr);
}
     ...
     void *data;
{
  ...
  ((visited)->elms [(block->index) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (block->index) % ((unsigned) (8 * 8)));
  if (((pending)->elms [(block->index) / ((unsigned) (8 * 8))] >> (block->index) % ((unsigned) (8 * 8)) & 1))
    {
      if (dir == FORWARD)
 {
  ...
  {
  case UNION:
    bitmap_operation (in[i], in[i], out[e->src->index], BITMAP_IOR);
    break;
  case INTERSECTION:
    bitmap_operation (in[i], in[i], out[e->src->index], BITMAP_AND);
    break;
  }
     }
 }
      else
 {
  ...
  {
  case UNION:
    bitmap_operation (out[i], out[i], in[e->dest->index], BITMAP_IOR);
    break;
  case INTERSECTION:
    bitmap_operation (out[i], out[i], in[e->dest->index], BITMAP_AND);
    break;
  }
     }
 }
      ...
      ((pending)->elms [(i) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (i) % ((unsigned) (8 * 8))));
      if (changed)
 {
   if (dir == FORWARD)
     {
       for (e = bb->succ; e != 0; e = e->succ_next)
  {
    if (e->dest == (&entry_exit_blocks[1]) || e->dest->index == i)
      continue;
    ((pending)->elms [(e->dest->index) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (e->dest->index) % ((unsigned) (8 * 8)));
  }
     }
   else
     {
       for (e = bb->pred; e != 0; e = e->pred_next)
  {
    if (e->src == (&entry_exit_blocks[0]) || e->dest->index == i)
      continue;
    ((pending)->elms [(e->src->index) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (e->src->index) % ((unsigned) (8 * 8)));
  }
     }
 }
    }
      ...
      for (e = bb->succ; e != 0; e = e->succ_next)
 {
   if (e->dest == (&entry_exit_blocks[1]) || e->dest->index == i)
     continue;
   if (!((visited)->elms [(e->dest->index) / ((unsigned) (8 * 8))] >> (e->dest->index) % ((unsigned) (8 * 8)) & 1))
  ...
  else
    {
      for (e = bb->pred; e != 0; e = e->pred_next)
 {
   if (e->src == (&entry_exit_blocks[0]) || e->src->index == i)
     continue;
   if (!((visited)->elms [(e->src->index) / ((unsigned) (8 * 8))] >> (e->src->index) % ((unsigned) (8 * 8)) & 1))
     ...
     void *data;
{
  ...
  do { bitmap_element *ptr_ = (blocks)->first; unsigned int indx_ = (0) / ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))); unsigned bit_num_ = (0) % (unsigned) (8 * 4); unsigned word_num_ = (0) / (unsigned) (8 * 4) % ((128 + (8 * 4) - 1) / (8 * 4)); while (ptr_ != 0 && ptr_->indx < indx_) ptr_ = ptr_->next; if (ptr_ != 0 && ptr_->indx != indx_) { bit_num_ = 0; word_num_ = 0; } for (; ptr_ != 0; ptr_ = ptr_->next) { for (; word_num_ < ((128 + (8 * 4) - 1) / (8 * 4)); word_num_++) { BITMAP_WORD word_ = ptr_->bits[word_num_]; if (word_ != 0) { for (; bit_num_ < (unsigned) (8 * 4); bit_num_++) { BITMAP_WORD mask_ = ((BITMAP_WORD) 1) << bit_num_; if ((word_ & mask_) != 0) { word_ &= ~ mask_; (i) = (ptr_->indx * ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))) + word_num_ * (unsigned) (8 * 4) + bit_num_); { fibheap_insert (worklist, order[i], (void *) (size_t) i); ((pending)->elms [(i) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (i) % ((unsigned) (8 * 8))); if (dir == FORWARD) sbitmap_copy (out[i], gen[i]); else sbitmap_copy (in[i], gen[i]); }; if (word_ == 0) break; } } } bit_num_ = 0; } word_num_ = 0; } } while (0);
# 3900 "../../../kgccfe/gnu/df.c"
      ...
      while (!fibheap_empty (worklist))
 {
   ...
   if (!((visited)->elms [(bb->index) / ((unsigned) (8 * 8))] >> (bb->index) % ((unsigned) (8 * 8)) & 1))
     hybrid_search_sbitmap (bb, in, out, gen, kill, dir,
       conf_op, transfun, visited, pending, data);
 }
      if (sbitmap_first_set_bit (pending) != -1)
 {
   do { bitmap_element *ptr_ = (blocks)->first; unsigned int indx_ = (0) / ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))); unsigned bit_num_ = (0) % (unsigned) (8 * 4); unsigned word_num_ = (0) / (unsigned) (8 * 4) % ((128 + (8 * 4) - 1) / (8 * 4)); while (ptr_ != 0 && ptr_->indx < indx_) ptr_ = ptr_->next; if (ptr_ != 0 && ptr_->indx != indx_) { bit_num_ = 0; word_num_ = 0; } for (; ptr_ != 0; ptr_ = ptr_->next) { for (; word_num_ < ((128 + (8 * 4) - 1) / (8 * 4)); word_num_++) { BITMAP_WORD word_ = ptr_->bits[word_num_]; if (word_ != 0) { for (; bit_num_ < (unsigned) (8 * 4); bit_num_++) { BITMAP_WORD mask_ = ((BITMAP_WORD) 1) << bit_num_; if ((word_ & mask_) != 0) { word_ &= ~ mask_; (i) = (ptr_->indx * ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))) + word_num_ * (unsigned) (8 * 4) + bit_num_); { fibheap_insert (worklist, order[i], (void *) (size_t) i); }; if (word_ == 0) break; } } } bit_num_ = 0; } word_num_ = 0; } } while (0);
     ...
     void *data;
{
  ...
  sbitmap_zero (visited);
  worklist = fibheap_new ();
  do { bitmap_element *ptr_ = (blocks)->first; unsigned int indx_ = (0) / ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))); unsigned bit_num_ = (0) % (unsigned) (8 * 4); unsigned word_num_ = (0) / (unsigned) (8 * 4) % ((128 + (8 * 4) - 1) / (8 * 4)); while (ptr_ != 0 && ptr_->indx < indx_) ptr_ = ptr_->next; if (ptr_ != 0 && ptr_->indx != indx_) { bit_num_ = 0; word_num_ = 0; } for (; ptr_ != 0; ptr_ = ptr_->next) { for (; word_num_ < ((128 + (8 * 4) - 1) / (8 * 4)); word_num_++) { BITMAP_WORD word_ = ptr_->bits[word_num_]; if (word_ != 0) { for (; bit_num_ < (unsigned) (8 * 4); bit_num_++) { BITMAP_WORD mask_ = ((BITMAP_WORD) 1) << bit_num_; if ((word_ & mask_) != 0) { word_ &= ~ mask_; (i) = (ptr_->indx * ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))) + word_num_ * (unsigned) (8 * 4) + bit_num_); { fibheap_insert (worklist, order[i], (void *) (size_t) i); ((pending)->elms [(i) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (i) % ((unsigned) (8 * 8))); if (dir == FORWARD) bitmap_copy (out[i], gen[i]); else bitmap_copy (in[i], gen[i]); }; if (word_ == 0) break; } } } bit_num_ = 0; } word_num_ = 0; } } while (0);
# 3959 "../../../kgccfe/gnu/df.c"
  while (sbitmap_first_set_bit (pending) != -1)
    {
      while (!fibheap_empty (worklist))
 {
