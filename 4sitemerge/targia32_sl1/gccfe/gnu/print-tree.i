...
print_tree (FILE *file, tree node)
{
  ...
  table = (struct bucket **) malloc (37 * sizeof (struct bucket *));
  bzero ((char *) table, 37 * sizeof (struct bucket *));
...
debug_tree (node)
     tree node;
{
  table = (struct bucket **) xcalloc (37, sizeof (struct bucket *));
     ...
     int indent;
{
  ...
  if (indent > 0)
    fprintf (file, " ");
  fprintf (file, "%s <%s ", prefix, tree_code_name[(int) ((enum tree_code) (node)->common.code)]);
  fprintf (file, "%p", (char *) node);

  if (class == 'd')
    {
      if (((node)->decl.name))
 fprintf (file, " %s", ((char *)(((node)->decl.name))->identifier.id.str));
    }
  else if (class == 't')
    {
      if (((node)->type.name))
 {
   if (((enum tree_code) (((node)->type.name))->common.code) == IDENTIFIER_NODE)
     fprintf (file, " %s", ((char *)(((node)->type.name))->identifier.id.str));
   else if (((enum tree_code) (((node)->type.name))->common.code) == TYPE_DECL
     && ((((node)->type.name))->decl.name))
     fprintf (file, " %s",
       ((char *)(((((node)->type.name))->decl.name))->identifier.id.str));
 }
    }
  if (((enum tree_code) (node)->common.code) == IDENTIFIER_NODE)
    fprintf (file, " %s", ((char *)(node)->identifier.id.str));
 ...
 fprintf (file, "%llu", (((node)->int_cst.int_cst).low));
     ...
     int indent;
{
      ...
      if (((node)->type.name))
 {
  ...
  if (((enum tree_code) (node)->common.code) == INTEGER_CST)
    {
      if (indent <= 4)
 print_node_brief (file, "type", ((node)->common.type), indent + 4);
    }
  else
    {
      print_node (file, "type", ((node)->common.type), indent + 4);
      if (((node)->common.type))
 indent_to (file, indent + 3);
  ...
  if (((node)->common.readonly_flag))
    fputs_unlocked (" readonly", file);
  if (((node)->common.constant_flag))
 ...
 fputs_unlocked (" suppress-debug", file);
      ...
      if (((enum tree_code) (node)->common.code) == FUNCTION_DECL && ((node)->decl.inlined_function_flag))
 fputs_unlocked (" autoinline", file);
      else if (((enum tree_code) (node)->common.code) == FUNCTION_DECL && ((node)->decl.inline_flag))
 fputs_unlocked (" inline", file);
      if (((enum tree_code) (node)->common.code) == FUNCTION_DECL && (((node)->decl.built_in_class) != NOT_BUILT_IN))
 fputs_unlocked (" built-in", file);
      if (((enum tree_code) (node)->common.code) == FUNCTION_DECL && ((node)->common.unsigned_flag))
 fputs_unlocked (" built-in-nonansi", file);
 ...
 fputs_unlocked (" decl_7", file);
      ...
      print_node (file, "size", ((node)->decl.size), indent + 4);
      print_node (file, "unit size", ((node)->decl.size_unit), indent + 4);
   ...
   if (((enum tree_code) (node)->common.code) == FIELD_DECL)
     {
       ...
       fprintf (file, "%llu",
         (((unsigned long long)1) << (node)->decl.u1.a.off_align));
     }
 }
      ...
      if ((((node)->decl.pointer_alias_set) != - 1))
 {
   ...
   fprintf (file, "%lld",
     ((node)->decl.pointer_alias_set));
      ...
      if (((enum tree_code) (node)->common.code) == FIELD_DECL)
 {
   print_node (file, "offset", ((node)->decl.arguments), indent + 4);
   print_node (file, "bit offset", ((node)->decl.u2.t),
        indent + 4);
 }

      print_node_brief (file, "context", ((node)->decl.context), indent + 4);
      print_node_brief (file, "attributes",
   ((node)->decl.attributes), indent + 4);
      print_node_brief (file, "abstract_origin",
   ((node)->decl.abstract_origin), indent + 4);

      print_node (file, "arguments", ((node)->decl.arguments), indent + 4);
      print_node (file, "result", ((node)->decl.result), indent + 4);
      print_node_brief (file, "initial", ((node)->decl.initial), indent + 4);

      if(((node)->decl.decl_dst_id).block != -1 || ((node)->decl.decl_dst_id).offset != -1) {
         fprintf(file," dst idx %d %d ", ((node)->type.dst_id).block,
                        ((node)->type.dst_id).offset);
      }
 ...
 {
   print_node (file, "arg-type", ((node)->decl.initial), indent + 4);
   print_node (file, "arg-type-as-written",
        ((node)->decl.result), indent + 4);
        ...
        && ((node)->decl.u2.f) != 0)
 {
   ...
   fprintf (file, "%p", (char *) ((node)->decl.u2.f));
 }


      if (indent == 4)
 print_node (file, "chain", ((node)->common.chain), indent + 4);
      else
 print_node_brief (file, "chain", ((node)->common.chain), indent + 4);
      break;
 ...
 fputs_unlocked (" nonaliased-component", file);
        ...
        && ((node)->type.transparent_union_flag))
 fputs_unlocked (" ambient-boundedness", file);
 ...
 fputs_unlocked (" type_6", file);
      ...
      print_node (file, "size", ((node)->type.size), indent + 4);
      print_node (file, "unit size", ((node)->type.size_unit), indent + 4);
      indent_to (file, indent + 3);

      if (((node)->type.user_align))
 fprintf (file, " user");

      fprintf (file, " align %d", ((node)->type.align));
      fprintf (file, " symtab %d", ((node)->type.symtab.address));
      fprintf (file, " alias set ");
      fprintf (file, "%lld", ((node)->type.alias_set));

      print_node (file, "attributes", ((node)->type.attributes), indent + 4);
      ...
      if ((((enum tree_code) (node)->common.code) == INTEGER_TYPE || ((enum tree_code) (node)->common.code) == ENUMERAL_TYPE || ((enum tree_code) (node)->common.code) == BOOLEAN_TYPE || ((enum tree_code) (node)->common.code) == CHAR_TYPE) || ((enum tree_code) (node)->common.code) == REAL_TYPE)
 {
   ...
   print_node_brief (file, "min", ((node)->type.minval), indent + 4);
   print_node_brief (file, "max", ((node)->type.maxval), indent + 4);
 }

      if (((enum tree_code) (node)->common.code) == ENUMERAL_TYPE)
 print_node (file, "values", ((node)->type.values), indent + 4);
      else if (((enum tree_code) (node)->common.code) == ARRAY_TYPE || ((enum tree_code) (node)->common.code) == SET_TYPE)
 print_node (file, "domain", ((node)->type.values), indent + 4);
      else if (((enum tree_code) (node)->common.code) == RECORD_TYPE
        || ((enum tree_code) (node)->common.code) == UNION_TYPE
        || ((enum tree_code) (node)->common.code) == QUAL_UNION_TYPE)
 print_node (file, "fields", ((node)->type.values), indent + 4);
      else if (((enum tree_code) (node)->common.code) == FUNCTION_TYPE
        || ((enum tree_code) (node)->common.code) == METHOD_TYPE)
 {
   if (((node)->type.maxval))
     print_node_brief (file, "method basetype",
         ((node)->type.maxval), indent + 4);
   print_node (file, "arg-types", ((node)->type.values), indent + 4);
 }
 ...
 print_node_brief (file, "context", ((node)->type.context), indent + 4);

      if(((node)->type.ty_idx)) fprintf(file," ty_idx %u",((node)->type.ty_idx));
      if(((node)->type.field_ids_used)) fprintf(file," ids_used %u",
                ((node)->type.field_ids_used));
      if(((node)->type.dst_id).block != -1 || ((node)->type.dst_id).offset != -1) {
         fprintf(file," dst idx %d %d ", ((node)->type.dst_id).block,
                        ((node)->type.dst_id).offset);
      }
 ...
 indent_to (file, indent + 3);
   ...
   indent + 4);
    ...
    case 'b':
      print_node (file, "vars", ((node)->block.vars), indent + 4);
      print_node (file, "supercontext", ((node)->block.supercontext), indent + 4);
      print_node (file, "subblocks", ((node)->block.subblocks), indent + 4);
      print_node (file, "chain", (((node))->common.chain), indent + 4);
      print_node (file, "abstract_origin",
    ((node)->block.abstract_origin), indent + 4);
      break;
 ...
 {
   print_node (file, "vars", ((node)->exp.operands[0]), indent + 4);
   print_node (file, "body", ((node)->exp.operands[1]), indent + 4);
   print_node (file, "block", ((node)->exp.operands[2]), indent + 4);
   break;
 }
      ...
      for (i = 0; i < len; i++)
 {
  ...
  print_rtl (file, (struct rtx_def *) ((node)->exp.operands[i]));
       else
  fprintf (file, "(nil)");
       fprintf (file, "\n");
     }
   else
     {
       ...
       print_node (file, temp, ((node)->exp.operands[i]), indent + 4);
     }
 }

      if (((enum tree_code) (node)->common.code) == EXPR_WITH_FILE_LOCATION)
 {
   indent_to (file, indent+4);
   fprintf (file, "%s:%d:%d",
     ((((node))->exp.operands[1]) ?
      ((char *)((((node))->exp.operands[1]))->identifier.id.str) : "(no file info)"),
     (((node)->exp.complexity) >> 12), (((node)->exp.complexity) & 0xfff));
 }
      print_node (file, "chain", ((node)->common.chain), indent + 4);
      break;
 ...
 case VECTOR_CST:
   {
     ...
     for (link = vals; link; link = ((link)->common.chain), ++i)
       {
  sprintf (buf, "elt%d: ", i);
  print_node (file, buf, ((link)->list.value), indent + 4);
       }
   }
   break;

 case COMPLEX_CST:
   print_node (file, "real", ((node)->complex.real), indent + 4);
   print_node (file, "imag", ((node)->complex.imag), indent + 4);
   break;

 case STRING_CST:
   {
     const char *p = ((node)->string.pointer);
     int i = ((node)->string.length);
     fputs_unlocked (" \"", file);
     while (--i >= 0)
       {
  char ch = *p++;
 ...
 case TREE_LIST:
   print_node (file, "purpose", ((node)->list.purpose), indent + 4);
   print_node (file, "value", ((node)->list.value), indent + 4);
   print_node (file, "chain", ((node)->common.chain), indent + 4);
   break;
