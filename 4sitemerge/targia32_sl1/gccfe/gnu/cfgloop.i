...
static _Bool glb_enum_p (basic_block, void *);
static void redirect_edge_with_latch_update (edge, basic_block);
static void flow_loop_free (struct loop *);
  ...
  if (loops->parray)
    {
      ...
      if (! loops->num)
 fancy_abort ("../../../kgccfe/gnu/cfgloop.c", 210, __FUNCTION__);
     ...
     struct loop *loop;
{
 ...
 num_entries++;
  ...
  if (! num_entries)
    fancy_abort ("../../../kgccfe/gnu/cfgloop.c", 254, __FUNCTION__);
   ...
   for (e = node->pred; e; e = e->pred_next)
     {
       ...
       if (ancestor != (&entry_exit_blocks[0])
     ...
     struct loop *loop;
{
  ...
  if (ebb == (&entry_exit_blocks[0]))
    return;




  for (num = 1; ebb->pred->src != (&entry_exit_blocks[0]) && ! ebb->pred->pred_next;
       num++)
    ebb = ebb->pred->src;
     ...
     int conn_latch;
{
  ...
  insn = (((first_insn_after_basic_block_note (bb))->fld[1]).rtx);
  ...
  for (e = dummy->pred; e; e = next_e)
    {
      next_e = e->pred_next;
      if (e == except
   || !((redirect_latch && (*(int *) (e)->aux))
        || (redirect_nonlatch && !(*(int *) (e)->aux))))
 {
...
canonicalize_loop_headers ()
{
  ...
  for (header = (&entry_exit_blocks[0])->next_bb; header != (&entry_exit_blocks[1]); header = header->next_bb)
    {
      int num_latches = 0;
 ...
 (* (int *) (header)->aux) = num_latches;
    }

  if ((* (int *) ((&entry_exit_blocks[0])->succ->dest)->aux))
    {
      basic_block bb;



      bb = split_edge ((&entry_exit_blocks[0])->succ);
      ...
      if (is_heavy)
 {
   basic_block new_header =
     make_forwarder_block (header, 1, 1, heavy, 0);
   if (num_latch > 2)
     make_forwarder_block (new_header, 1, 0, ((void *)0), 1);
 }
      else
 make_forwarder_block (header, 1, 0, ((void *)0), 1);
    }
     ...
     int flags;
{
  ...
  if (! (flags & 1))
    fancy_abort ("../../../kgccfe/gnu/cfgloop.c", 777, __FUNCTION__);
      ...
      for (e = header->pred; e; e = e->pred_next)
 {
   ...
   if (e->flags & 2)
     {
       if (more_latches)
  {
    ((headers)->elms [(header->index) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (header->index) % ((unsigned) (8 * 8))));
    num_loops--;
  }
       break;
     }
   ...
   if (latch != (&entry_exit_blocks[0]) && dominated_by_p (dom, latch, header))
     {

       if (more_latches)
  fancy_abort ("../../../kgccfe/gnu/cfgloop.c", 830, __FUNCTION__);
       more_latches = 1;
       ((headers)->elms [(header->index) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (header->index) % ((unsigned) (8 * 8)));
       num_loops++;
     }
  ...
  loops->parray[0]->latch = (&entry_exit_blocks[1]);
  loops->parray[0]->header = (&entry_exit_blocks[0]);
  (&entry_exit_blocks[0])->loop_father = loops->parray[0];
  (&entry_exit_blocks[1])->loop_father = loops->parray[0];
      ...
      for (b = 0; b < n_basic_blocks; b++)
 {
   ...
   if (!((headers)->elms [(rc_order[b]) / ((unsigned) (8 * 8))] >> (rc_order[b]) % ((unsigned) (8 * 8)) & 1))
     continue;

   header = (((basic_block_info)->data.bb[(rc_order[b])]));
     ...
     const basic_block bb;
{
  ...
  if (bb == (&entry_exit_blocks[0]) || bb == (&entry_exit_blocks[1]))
    return 0;
     ...
     edge e;
{
  if (e->dest != loop->header)
    fancy_abort ("../../../kgccfe/gnu/cfgloop.c", 975, __FUNCTION__);
     ...
     const struct loop *loop;
{
  ...
  if (!loop->num_nodes)
    fancy_abort ("../../../kgccfe/gnu/cfgloop.c", 997, __FUNCTION__);

  tovisit = xcalloc (loop->num_nodes, sizeof (basic_block));
  tovisit[tv++] = loop->header;

  if (loop->latch == (&entry_exit_blocks[1]))
    {

      if (loop->num_nodes != n_basic_blocks + 2)
 fancy_abort ("../../../kgccfe/gnu/cfgloop.c", 1006, __FUNCTION__);
      for (bb = (&entry_exit_blocks[0])->next_bb; bb != (&entry_exit_blocks[1]); bb = bb->next_bb)
 tovisit[tv++] = bb;
      tovisit[tv++] = (&entry_exit_blocks[1]);
    }
  ...
  if (tv != loop->num_nodes)
    fancy_abort ("../../../kgccfe/gnu/cfgloop.c", 1019, __FUNCTION__);
  return tovisit;
}
     ...
     int flags;
{
      ...
      if (loop->header->loop_father != loop)
 {
  ...
  if (err)
    fancy_abort ("../../../kgccfe/gnu/cfgloop.c", 1173, __FUNCTION__);
}
