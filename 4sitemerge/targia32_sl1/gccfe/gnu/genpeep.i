...
# 28 "../../../kgccfe/gnu/genpeep.c" 2
# 36 "../../../kgccfe/gnu/genpeep.c"
struct link
{
  struct link *next;
  int pos;
...
gen_peephole (peep)
     rtx peep;
{
  int ninsns = (((((peep)->fld[0]).rtvec))->num_elem);
  int i;
      ...
      if (i > 0)
 {
   ...
   printf ("  if (GET_CODE (insn) == CODE_LABEL\n      || GET_CODE (insn) == BARRIER)\n    goto L%d;\n",
      ...
      match_rtx ((((((peep)->fld[0]).rtvec))->elem[i]), ((void *)0), insn_code_number);
    }




  if ((((peep)->fld[1]).rtstr) && (((peep)->fld[1]).rtstr)[0])
    printf ("  if (! (%s)) goto L%d;\n",
     (((peep)->fld[1]).rtstr), insn_code_number);
     ...
     int fail_label;
{
  ...
  switch (code)
    {
    case MATCH_OPERAND:
      if ((((x)->fld[0]).rtint) > max_opno)
 max_opno = (((x)->fld[0]).rtint);
      if ((((x)->fld[0]).rtint) >= n_operands)
 n_operands = 1 + (((x)->fld[0]).rtint);
      ...
      printf ("  operands[%d] = x;\n", (((x)->fld[0]).rtint));
      if ((((x)->fld[1]).rtstr) && (((x)->fld[1]).rtstr)[0])
 printf ("  if (! %s (x, %smode)) goto L%d;\n",
  (((x)->fld[1]).rtstr), (mode_name[(int) (((enum machine_mode) (x)->mode))]), fail_label);
      return;
 ...
 {
   link.pos = i;
   match_rtx ((((((x)->fld[1]).rtvec))->elem[i]), &link, fail_label);
 }
      return;
 ...
 printf ("  if (! %s (x, %smode)) goto L%d;\n",
  (((x)->fld[1]).rtstr), (mode_name[(int) (((enum machine_mode) (x)->mode))]), fail_label);
      link.next = path;
      link.vecelt = -1;
      for (i = 0; i < (((((x)->fld[2]).rtvec))->num_elem); i++)
 {
   link.pos = i;
   match_rtx ((((((x)->fld[2]).rtvec))->elem[i]), &link, fail_label);
 }
      return;
      ...
      for (i = 0; i < (((((x)->fld[2]).rtvec))->num_elem); i++)
 {
   ...
   match_rtx ((((((x)->fld[2]).rtvec))->elem[i]), &link, fail_label);
    ...
    case ADDRESS:
      match_rtx ((((x)->fld[0]).rtx), path, fail_label);
      return;
  ...
  if (((enum machine_mode) (x)->mode) != VOIDmode)
    {
      printf ("  if (GET_MODE (x) != %smode) goto L%d;\n",
       (mode_name[(int) (((enum machine_mode) (x)->mode))]), fail_label);
    }
  ...
  for (i = 0; i < len; i++)
    {
      link.pos = i;
      if (fmt[i] == 'e' || fmt[i] == 'u')
 match_rtx ((((x)->fld[i]).rtx), &link, fail_label);
      else if (fmt[i] == 'E')
 {
   int j;
   printf ("  if (XVECLEN (x, %d) != %d) goto L%d;\n",
    i, (((((x)->fld[i]).rtvec))->num_elem), fail_label);
   for (j = 0; j < (((((x)->fld[i]).rtvec))->num_elem); j++)
     {
       link.vecelt = j;
       match_rtx ((((((x)->fld[i]).rtvec))->elem[j]), &link, fail_label);
     }
 }
      else if (fmt[i] == 'i')
 {
     ...
     char **argv;
{
       ...
       if (((enum rtx_code) (desc)->code) == DEFINE_PEEPHOLE)
 {
   ...
   || ((enum rtx_code) (desc)->code) == DEFINE_EXPAND
   || ((enum rtx_code) (desc)->code) == DEFINE_SPLIT
