...
# 56 "../../../kgccfe/gnu/gcov.c" 2
# 92 "../../../kgccfe/gnu/gcov.c"
struct sourcefile
{
  char *name;
  int maxlineno;
...
static void scan_for_source_files (void);
static void output_data (struct sourcefile *);
static void print_usage (int) __attribute__ ((__noreturn__));
static void print_version (void) __attribute__ ((__noreturn__));
static void init_arc (struct adj_list *, int, int, struct bb_info *);
static struct adj_list *reverse_arcs (struct adj_list *);
static gcov_type *read_profile (char *, long, int);
static void create_program_flow_graph (struct bb_info_list *);
static void solve_program_flow_graph (struct bb_info_list *);
static void accumulate_branch_counts (struct coverage *, struct arcdata *);

static void calculate_branch_probs (struct bb_info *, struct line_info *, struct coverage *);


static void function_summary (struct coverage *, const char *);
static void init_line_info (struct line_info *, struct coverage *, long);

static void output_line_info (FILE *, const struct line_info *, const struct coverage *, long);

static char *make_gcov_file_name (char *);
static const char *format_hwint (long long, long long, int);
...
static void fnotice (FILE *, const char *, ...) __attribute__ ((__format__ (__printf__, 2, 3))) __attribute__ ((__nonnull__ (2)));
static void
...
static const struct option options[] =
{
  { "help", 0, ((void *)0), 'h' },
  { "version", 0, ((void *)0), 'v' },
  { "branch-probabilities", 0, ((void *)0), 'b' },
  { "branch-counts", 0, ((void *)0), 'c' },
  { "no-output", 0, ((void *)0), 'n' },
  { "long-file-names", 0, ((void *)0), 'l' },
  { "function-summaries", 0, ((void *)0), 'f' },
  { "preserve-paths", 0, ((void *)0), 'p' },
  { "object-directory", 1, ((void *)0), 'o' },
  { "object-file", 1, ((void *)0), 'o' },
};
     ...
     char **argv;
{
  ...
  while ((opt = getopt_long (argc, argv, "hvbclnfo:p", options, ((void *)0))) != -1)
    {
      switch (opt)
 {
 case 'h':
  ...
  if (object_directory && object_directory[0])
    {
      ...
      base = !stat (object_directory, &status) && ((((status.st_mode)) & 0170000) == (0040000));
 ...
 strcat (name, "/");
...

  {
    ...
    if (!fstat (fileno (bb_file), &status))
      bb_file_time = status.st_mtim.tv_sec;
  }
  ...
  ungetc (_IO_getc (bbg_file), bbg_file);
  if (feof (bbg_file))
    {
...
scan_for_source_files ()
{
  struct sourcefile *s_ptr = ((void *)0);
  char *ptr;
  long count;
  long line_num;
     ...
     char *src_name;
{
  ...
  if (output_long_names && strcmp (src_name, input_file_name))
    {
      ...
      cptr = preserve_paths ? ((void *)0) : strrchr (input_file_name, '/');
      cptr = cptr ? cptr + 1 : input_file_name;
      strcat (name, cptr);
      strcat (name, "##");
    }
  ...
  cptr = preserve_paths ? ((void *)0) : strrchr (src_name, '/');
...
{
  long block_num = 0;
  struct bb_info *block_ptr = ((void *)0);
  struct coverage function;
  struct coverage *func_ptr = ((void *)0);
  struct bb_info_list *current_graph = ((void *)0);
  int is_this_file = 0;
  char *ptr = bb_data;
  long count;
  long line_num;
      ...
      if (line_ptr->exists)
 {
   ...
   if (line_ptr->count)
     total->lines_executed++;
      ...
      for (a_ptr = line_ptr->branches, prev = ((void *)0); a_ptr; a_ptr = next)
 {
   next = a_ptr->next;
   a_ptr->next = prev;
   prev = a_ptr;
     ...
     long maxlineno;
{
  ...
  if (!source_file)
   ...
   && status.st_mtim.tv_sec > bb_file_time)
 {
      ...
      if (retval)
 {
   ...
   do
     {
       retval = fgets (string, 200, source_file);
       if (!retval)
  {
    fnotice (stderr,
    ...
    break;
       ...
       fputs_unlocked (retval, gcov_file);
     }
      ...
      if (output_branch_probs)
 {
  ...
  if (retval)
    {
      for (; (retval = fgets (string, 200, source_file)); line_num++)
 {
