...
# 34 "../../../kgccfe/gnu/sched-ebb.c" 2
# 1 "../../../kgccfe/gnu/insn-config.h" 1
# 35 "../../../kgccfe/gnu/sched-ebb.c" 2
...
static void schedule_ebb (rtx, rtx);
...
static int
new_ready (next)
     rtx next __attribute__ ((__unused__));
{
  return 1;
}
...
ebb_print_insn (insn, aligned)
     rtx insn;
     int aligned __attribute__ ((__unused__));
{
  static char tmp[80];

  sprintf (tmp, "%4d", (((insn)->fld[0]).rtint));
  return tmp;
}
...
static int
rank (insn1, insn2)
     rtx insn1 __attribute__ ((__unused__)), insn2 __attribute__ ((__unused__));
{
  return 0;
}
...
contributes_to_priority (next, insn)
     rtx next __attribute__ ((__unused__)), insn __attribute__ ((__unused__));
{
  return 1;
}
     ...
     rtx load_insn;
{
      ...
      if (((enum machine_mode) (back_link)->mode) == VOIDmode)
 {
   ...
   for (fore_link = (h_i_d[(((insn1)->fld[0]).rtint)].depend);
        fore_link;
        fore_link = (((fore_link)->fld[1]).rtx))
     {
       rtx insn2 = (((fore_link)->fld[0]).rtx);
       basic_block insn2_block = (((insn2)->fld[3]).bb);
     ...
     rtx head, tail;
{
 ...
 last_jump = insn;
    ...
    else if (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i') && last_jump != (rtx) 0)
      {
     ...
     rtx head, tail;
{
 ...
 if (((enum reg_note) ((enum machine_mode) (note)->mode)) == REG_SAVE_NOTE)
  ...
  if (sched_n_insns != n_insns)
    fancy_abort ("../../../kgccfe/gnu/sched-ebb.c", 412, __FUNCTION__);
     ...
     FILE *dump_file;
{
      ...
      for (;;)
 {
   ...
   if (bb->next_bb == (&entry_exit_blocks[1])
       || ((enum rtx_code) (bb->next_bb->head)->code) == CODE_LABEL)
     break;
   ...
   if (! e)
     break;
   if (((enum rtx_code) (tail)->code) == JUMP_INSN)
     {
       rtx x = find_reg_note (tail, REG_BR_PROB, 0);
       if (x)
  {
    int pred_val = ((((((x)->fld[0]).rtx))->fld[0]).rtwint);
    if (pred_val > 10000 / 2)
      break;
  }
     }
