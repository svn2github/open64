...
# 36 "../../../kgccfe/gnu/sched-vis.c" 2
# 46 "../../../kgccfe/gnu/sched-vis.c"
     ...
     int verbose;
{
 ...
 {
   st[1] = "-";
   op[1] = gen_rtx_CONST_INT (VOIDmode, (long long) (-((((((x)->fld[1]).rtx))->fld[0]).rtwint)));
 }
      else
 {
   st[1] = "+";
 ...
 for (i = 0; i < (((((x)->fld[0]).rtvec))->num_elem); i++)
   {
     print_pattern (tmp, (((((x)->fld[0]).rtvec))->elem[i]), verbose);
     cur = safe_concat (buf, cur, sep);
     cur = safe_concat (buf, cur, tmp);
     sep = ",";
   }
 cur = safe_concat (buf, cur, "] ");
 sprintf (tmp, "%d", (((x)->fld[1]).rtint));
 cur = safe_concat (buf, cur, tmp);
      }
      break;
    default:
     ...
     int verbose;
{
  ...
  switch (((enum rtx_code) (x)->code))
    {
    case CONST_INT:
      sprintf (t, "0x%llx", (((x)->fld[0]).rtwint));
 ...
 real_to_decimal (t, ((struct real_value *)&(((x)->fld[0]).rtwint)), sizeof (t), 0, 1);
      else
 sprintf (t, "<0x%lx,0x%lx>", (long) (((x)->fld[2]).rtwint), (long) (((x)->fld[3]).rtwint));
      cur = safe_concat (buf, cur, t);
      break;
    case CONST_STRING:
      cur = safe_concat (buf, cur, "\"");
      cur = safe_concat (buf, cur, (((x)->fld[0]).rtstr));
      cur = safe_concat (buf, cur, "\"");
      break;
    case SYMBOL_REF:
      cur = safe_concat (buf, cur, "`");
      ...
      break;
    case LABEL_REF:
      sprintf (t, "L%d", ((((((x)->fld[0]).rtx))->fld[0]).rtint));
      cur = safe_concat (buf, cur, t);
      break;
    case CONST:
      print_value (t, (((x)->fld[0]).rtx), verbose);
 ...
 {
   int c = reg_names[(((x)->fld[0]).rtuint)][0];
   if ((_sch_istable[(c) & 0xff] & (unsigned short)(_sch_isdigit)))
     cur = safe_concat (buf, cur, "%");

   cur = safe_concat (buf, cur, reg_names[(((x)->fld[0]).rtuint)]);
 }
      else
 {
   sprintf (t, "r%d", (((x)->fld[0]).rtuint));
   cur = safe_concat (buf, cur, t);
 }
      break;
    case SUBREG:
     ...
     int verbose;
{
  ...
  switch (((enum rtx_code) (x)->code))
    {
    case SET:
      print_value (t1, (((x)->fld[0]).rtx), verbose);
      print_value (t2, (((x)->fld[1]).rtx), verbose);
    ...
    case USE:
      ...
      break;
    case COND_EXEC:
      if (((enum rtx_code) ((((x)->fld[0]).rtx))->code) == NE
   && ((((((x)->fld[0]).rtx))->fld[1]).rtx) == (const_int_rtx[64]))
 print_value (t1, ((((((x)->fld[0]).rtx))->fld[0]).rtx), verbose);
      else if (((enum rtx_code) ((((x)->fld[0]).rtx))->code) == EQ
        && ((((((x)->fld[0]).rtx))->fld[1]).rtx) == (const_int_rtx[64]))
 {
   t1[0] = '!';
   print_value (t1 + 1, ((((((x)->fld[0]).rtx))->fld[0]).rtx), verbose);
 }
      else
 print_value (t1, (((x)->fld[0]).rtx), verbose);
      print_pattern (t2, (((x)->fld[1]).rtx), verbose);
      sprintf (buf, "(%s) %s", t1, t2);
      break;
    case PARALLEL:
      {
 ...
 for (i = 0; i < (((((x)->fld[0]).rtvec))->num_elem); i++)
   {
     print_pattern (t2, (((((x)->fld[0]).rtvec))->elem[i]), verbose);
 ...
 sprintf (buf, "%s}", t1);
    ...
    case SEQUENCE:
      ...
      fancy_abort ("../../../kgccfe/gnu/sched-vis.c", 701, __FUNCTION__);
      break;
    case ASM_INPUT:
      sprintf (buf, "asm {%s}", (((x)->fld[0]).rtstr));
      break;
    case ADDR_VEC:
      break;
    case ADDR_DIFF_VEC:
      print_value (buf, (((x)->fld[0]).rtx), verbose);
      break;
    case TRAP_IF:
      print_value (t1, (((x)->fld[0]).rtx), verbose);
     ...
     int verbose;
{
  ...
  switch (((enum rtx_code) (x)->code))
    {
    case INSN:
      print_pattern (t, (((x)->fld[5]).rtx), verbose);
      if (verbose)
 sprintf (buf, "%s: %s", (*current_sched_info->print_insn) (x, 1),
   t);
      else
 sprintf (buf, "%-4d %s", (((x)->fld[0]).rtint), t);
      break;
    case JUMP_INSN:
      print_pattern (t, (((x)->fld[5]).rtx), verbose);
      if (verbose)
 ...
 sprintf (buf, "%-4d %s", (((insn)->fld[0]).rtint), t);
      break;
    case CODE_LABEL:
      sprintf (buf, "L%d:", (((x)->fld[0]).rtint));
      break;
    case BARRIER:
      sprintf (buf, "i% 4d: barrier", (((x)->fld[0]).rtint));
      break;
    case NOTE:
      if ((((x)->fld[5]).rtint) > 0)
 sprintf (buf, "%4d note \"%s\" %d", (((x)->fld[0]).rtint),
   (((x)->fld[4]).rtstr), (((x)->fld[5]).rtint));
      else
 sprintf (buf, "%4d %s", (((x)->fld[0]).rtint),
   (note_insn_name[((((x)->fld[5]).rtint)) - (int) NOTE_INSN_BIAS]));
      break;
    default:
      if (verbose)
 {
 ...
 sprintf (buf, "i%-4d  <What?>", (((x)->fld[0]).rtint));
    }
}
