...
# 77 "../../../kgccfe/gnu/ssa-dce.c" 2
# 1 "../../../kgccfe/gnu/insn-config.h" 1
# 78 "../../../kgccfe/gnu/ssa-dce.c" 2
...
static void find_all_control_dependences
  (struct edge_list *el, dominance_info pdom, control_dependent_block_to_edge_map cdbte);

static void find_control_dependence
  (struct edge_list *el, int edge_index, dominance_info pdom, control_dependent_block_to_edge_map cdbte);

static basic_block find_pdom
  (dominance_info pdom, basic_block block);
static int inherently_necessary_register_1
  (rtx *current_rtx, void *data);
     ...
     size_t num_basic_blocks;
{
  ...
  c->length = num_basic_blocks - ((-3)+1);
  c->data = xmalloc ((size_t) c->length*sizeof (bitmap));
  for (i = 0; i < c->length; ++i)
    c->data[i] = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
...
set_control_dependent_block_to_edge_map_bit (c, bb, edge_index)
     ...
     int edge_index;
{
  if (bb->index - ((-3)+1) >= c->length)
    fancy_abort ("../../../kgccfe/gnu/ssa-dce.c", 185, __FUNCTION__);

  bitmap_set_bit (c->data[bb->index - ((-3)+1)],
    edge_index);
}
# 203 "../../../kgccfe/gnu/ssa-dce.c"
static void
     ...
     control_dependent_block_to_edge_map c;
{
  int i;
  for (i = 0; i < c->length; ++i)
    do { if (c->data[i]) { bitmap_clear (c->data[i]); free (c->data[i]); (c->data[i]) = 0; } } while (0);
  free ((void *) c);
}
   ...
   control_dependent_block_to_edge_map cdbte;
{
  ...
  if (((el)->index_to_edge[(edge_index)]->src) == (&entry_exit_blocks[1]))
    fancy_abort ("../../../kgccfe/gnu/ssa-dce.c", 247, __FUNCTION__);
  ending_block =
    (((el)->index_to_edge[(edge_index)]->src) == (&entry_exit_blocks[0]))
    ? (&entry_exit_blocks[0])->next_bb
    : find_pdom (pdom, ((el)->index_to_edge[(edge_index)]->src));

  for (current_block = ((el)->index_to_edge[(edge_index)]->dest);
       current_block != ending_block && current_block != (&entry_exit_blocks[1]);
...
{
  if (!block)
    fancy_abort ("../../../kgccfe/gnu/ssa-dce.c", 273, __FUNCTION__);
  if (block->index == (-3))
    fancy_abort ("../../../kgccfe/gnu/ssa-dce.c", 275, __FUNCTION__);

  if (block == (&entry_exit_blocks[0]))
    return (&entry_exit_blocks[0])->next_bb;
  else if (block == (&entry_exit_blocks[1]))
    return (&entry_exit_blocks[1]);
  else
    {
     ...
     void *data __attribute__ ((__unused__));
{
  ...
  switch (((enum rtx_code) (x)->code))
    ...
    case PC:
      ...
      break;
    case REG:
      if (((!(((((x)->fld[0]).rtuint)) < 176)) || ((0))) || x == (global_rtl[GR_PC]))
 return 0;
      else
 return !0;
      break;
     ...
     rtx current_rtx;
{
  return for_each_rtx (&current_rtx,
         &inherently_necessary_register_1, ((void *)0));
}
     ...
     void *data;
{
  ...
  if (((enum rtx_code) (dest)->code) == MEM
      || ((enum rtx_code) (dest)->code) == UNSPEC
      || ((enum rtx_code) (dest)->code) == UNSPEC_VOLATILE)
     ...
     rtx x;
{
 ...
 return 0;
      case JUMP_INSN:
 return ((((enum rtx_code) (x)->code) == JUMP_INSN) && (((enum rtx_code) ((((x)->fld[5]).rtx))->code) == ADDR_VEC || ((enum rtx_code) ((((x)->fld[5]).rtx))->code) == ADDR_DIFF_VEC)) || computed_jump_p (x) != 0;
      case INSN:
 {
   int inherently_necessary_set = 0;
   note_stores ((((x)->fld[5]).rtx),
   ...
   return (inherently_necessary_set
    || ((enum rtx_code) ((((x)->fld[5]).rtx))->code) == ASM_INPUT
    || asm_noperands ((((x)->fld[5]).rtx)) >= 0);
 }
      default:

 fancy_abort ("../../../kgccfe/gnu/ssa-dce.c", 404, __FUNCTION__);
 break;
      }
}
     ...
     void *data;
{
 ...
 {
   rtx insn = ((ssa_definition)->data.rtx[(((x)->fld[0]).rtuint)]);
   if (insn != (rtx) 0 && (((insn))->in_struct))
     {
       (((insn))->in_struct) = 0;
       do { if ((*unprocessed_instructions)->elements_used >= (*unprocessed_instructions)->num_elements) (((*unprocessed_instructions)) = varray_grow ((*unprocessed_instructions), 2 * (*unprocessed_instructions)->num_elements)); (*unprocessed_instructions)->data.rtx[(*unprocessed_instructions)->elements_used++] = (insn); } while (0);
     }
 }
      return 0;
...
{
  if (!insn)
    fancy_abort ("../../../kgccfe/gnu/ssa-dce.c", 478, __FUNCTION__);
...
ssa_eliminate_dead_code ()
{
  ...
  mark_all_insn_unnecessary ();
  unprocessed_instructions = varray_init (64, VARRAY_DATA_RTX, "unprocessed instructions");
 ...
 do { if ((unprocessed_instructions)->elements_used >= (unprocessed_instructions)->num_elements) (((unprocessed_instructions)) = varray_grow ((unprocessed_instructions), 2 * (unprocessed_instructions)->num_elements)); (unprocessed_instructions)->data.rtx[(unprocessed_instructions)->elements_used++] = (insn); } while (0);
  ...
  while (((unprocessed_instructions)->elements_used) > 0)
    {
      rtx current_instruction;
      int edge_number;

      current_instruction = ((unprocessed_instructions)->data.rtx[(unprocessed_instructions)->elements_used - 1]);
      ((unprocessed_instructions)->elements_used--);
      ...
      if (((rtx_class[(int) (((enum rtx_code) (current_instruction)->code))]) == 'i')
   && !((((enum rtx_code) (current_instruction)->code) == JUMP_INSN) && (((enum rtx_code) ((((current_instruction)->fld[5]).rtx))->code) == ADDR_VEC || ((enum rtx_code) ((((current_instruction)->fld[5]).rtx))->code) == ADDR_DIFF_VEC)))
 {

   do { bitmap_element *ptr_ = (cdbte->data[((((current_instruction)->fld[3]).bb)->index + 0) - ((-3)+1)])->first; unsigned int indx_ = (0) / ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))); unsigned bit_num_ = (0) % (unsigned) (8 * 4); unsigned word_num_ = (0) / (unsigned) (8 * 4) % ((128 + (8 * 4) - 1) / (8 * 4)); while (ptr_ != 0 && ptr_->indx < indx_) ptr_ = ptr_->next; if (ptr_ != 0 && ptr_->indx != indx_) { bit_num_ = 0; word_num_ = 0; } for (; ptr_ != 0; ptr_ = ptr_->next) { for (; word_num_ < ((128 + (8 * 4) - 1) / (8 * 4)); word_num_++) { BITMAP_WORD word_ = ptr_->bits[word_num_]; if (word_ != 0) { for (; bit_num_ < (unsigned) (8 * 4); bit_num_++) { BITMAP_WORD mask_ = ((BITMAP_WORD) 1) << bit_num_; if ((word_ & mask_) != 0) { word_ &= ~ mask_; (edge_number) = (ptr_->indx * ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))) + word_num_ * (unsigned) (8 * 4) + bit_num_); { rtx jump_insn = (((el)->index_to_edge[(edge_number)]->src))->end; if (((enum rtx_code) (jump_insn)->code) == JUMP_INSN && (((jump_insn))->in_struct)) { (((jump_insn))->in_struct) = 0; do { if ((unprocessed_instructions)->elements_used >= (unprocessed_instructions)->num_elements) (((unprocessed_instructions)) = varray_grow ((unprocessed_instructions), 2 * (unprocessed_instructions)->num_elements)); (unprocessed_instructions)->data.rtx[(unprocessed_instructions)->elements_used++] = (jump_insn); } while (0); } }; if (word_ == 0) break; } } } bit_num_ = 0; } word_num_ = 0; } } while (0);
# 560 "../../../kgccfe/gnu/ssa-dce.c"
   for_each_rtx (&current_instruction,
   &propagate_necessity_through_operand,
   (void *) &unprocessed_instructions);
# 572 "../../../kgccfe/gnu/ssa-dce.c"
   if (((current_instruction) && ((enum rtx_code) (current_instruction)->code) == INSN && ((enum rtx_code) ((((current_instruction)->fld[5]).rtx))->code) == SET && ((enum rtx_code) (((((((current_instruction)->fld[5]).rtx))->fld[1]).rtx))->code) == PHI))
     {
       rtvec phi_vec = (((((((((current_instruction)->fld[5]).rtx))->fld[1]).rtx))->fld[0]).rtvec);
       ...
       for (v = num_elem - 2; v >= 0; v -= 2)
  {
    ...
    bb = (((basic_block_info)->data.bb[((((((phi_vec)->elem[v + 1]))->fld[0]).rtwint))]));
    do { bitmap_element *ptr_ = (cdbte->data[((((bb->end)->fld[3]).bb)->index + 0) - ((-3)+1)])->first; unsigned int indx_ = (0) / ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))); unsigned bit_num_ = (0) % (unsigned) (8 * 4); unsigned word_num_ = (0) / (unsigned) (8 * 4) % ((128 + (8 * 4) - 1) / (8 * 4)); while (ptr_ != 0 && ptr_->indx < indx_) ptr_ = ptr_->next; if (ptr_ != 0 && ptr_->indx != indx_) { bit_num_ = 0; word_num_ = 0; } for (; ptr_ != 0; ptr_ = ptr_->next) { for (; word_num_ < ((128 + (8 * 4) - 1) / (8 * 4)); word_num_++) { BITMAP_WORD word_ = ptr_->bits[word_num_]; if (word_ != 0) { for (; bit_num_ < (unsigned) (8 * 4); bit_num_++) { BITMAP_WORD mask_ = ((BITMAP_WORD) 1) << bit_num_; if ((word_ & mask_) != 0) { word_ &= ~ mask_; (edge_number) = (ptr_->indx * ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))) + word_num_ * (unsigned) (8 * 4) + bit_num_); { rtx jump_insn; jump_insn = (((el)->index_to_edge[(edge_number)]->src))->end; if (((((enum rtx_code) (jump_insn)->code) == JUMP_INSN)) && (((jump_insn))->in_struct)) { (((jump_insn))->in_struct) = 0; do { if ((unprocessed_instructions)->elements_used >= (unprocessed_instructions)->num_elements) (((unprocessed_instructions)) = varray_grow ((unprocessed_instructions), 2 * (unprocessed_instructions)->num_elements)); (unprocessed_instructions)->data.rtx[(unprocessed_instructions)->elements_used++] = (jump_insn); } while (0); } }; if (word_ == 0) break; } } } bit_num_ = 0; } word_num_ = 0; } } while (0);
# 597 "../../../kgccfe/gnu/ssa-dce.c"
  }
     }
 }
    }
  ...
  { rtx insn; for (insn = get_insns (); insn != (rtx) 0; insn = (((insn)->fld[2]).rtx)) if ((((insn))->in_struct)) { { if (any_condjump_p (insn)) { basic_block bb = (((insn)->fld[3]).bb); basic_block pdom_bb = find_pdom (pdom, bb); rtx lbl; edge e; if (pdom_bb == (&entry_exit_blocks[1])) { e = bb->succ; while (e) { edge temp = e; e = e->succ_next; if ((temp->flags & 1) == 0) { if (temp->dest != (&entry_exit_blocks[1])) { rtx insn = first_insn_after_basic_block_note (temp->dest); while (((insn) && ((enum rtx_code) (insn)->code) == INSN && ((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == SET && ((enum rtx_code) (((((((insn)->fld[5]).rtx))->fld[1]).rtx))->code) == PHI)) { remove_phi_alternative ((((insn)->fld[5]).rtx), temp->src); insn = (((insn)->fld[2]).rtx); } } remove_edge (temp); } } ((insn)->code = (enum rtx_code) (NOTE)); (((insn)->fld[5]).rtint) = NOTE_INSN_DELETED; continue; } e = bb->succ; while (e) { edge temp = e; e = e->succ_next; if (temp->flags & 2) continue; if (temp->dest != (&entry_exit_blocks[1])) { rtx insn = first_insn_after_basic_block_note (temp->dest); while (((insn) && ((enum rtx_code) (insn)->code) == INSN && ((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == SET && ((enum rtx_code) (((((((insn)->fld[5]).rtx))->fld[1]).rtx))->code) == PHI)) { remove_phi_alternative ((((insn)->fld[5]).rtx), temp->src); insn = (((insn)->fld[2]).rtx); } } remove_edge (temp); } make_edge (bb, pdom_bb, 0); lbl = find_block_label (pdom_bb); ((((((insn)->fld[5]).rtx))->fld[1]).rtx) = gen_rtx_fmt_u00 (LABEL_REF, (VOIDmode), (lbl)); (((insn)->fld[6]).rtint) = -1; (((insn)->fld[9]).rtx) = lbl; (((lbl)->fld[4]).rtint)++; emit_barrier_after (insn); } else if (!(((enum rtx_code) (insn)->code) == JUMP_INSN)) delete_insn_bb (insn); }; } };
# 710 "../../../kgccfe/gnu/ssa-dce.c"
  remove_fake_edges ();
      ...
      if (bb->succ == ((void *)0))
 {
  ...
  for (insn = get_insns (); insn != (rtx) 0; insn = (((insn)->fld[2]).rtx))
    (((insn))->in_struct) = 0;
  if (((unprocessed_instructions)->elements_used) != 0)
    fancy_abort ("../../../kgccfe/gnu/ssa-dce.c", 729, __FUNCTION__);
