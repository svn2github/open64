...
static void push_token_context
  (cpp_reader *, cpp_hashnode *, const cpp_token *, unsigned int);
static void push_ptoken_context
  (cpp_reader *, cpp_hashnode *, _cpp_buff *, const cpp_token **, unsigned int);
...
static _Bool paste_tokens (cpp_reader *, const cpp_token **, const cpp_token *);

static void replace_args (cpp_reader *, cpp_hashnode *, cpp_macro *, macro_arg *);

static _cpp_buff *funlike_invocation_p (cpp_reader *, cpp_hashnode *);
static _Bool create_iso_definition (cpp_reader *, cpp_macro *);



static cpp_token *alloc_expansion_token (cpp_reader *, cpp_macro *);
static cpp_token *lex_expansion_token (cpp_reader *, cpp_macro *);
static _Bool warn_of_redefinition (cpp_reader *, const cpp_hashnode *, const cpp_macro *);

static _Bool parse_params (cpp_reader *, cpp_macro *);
static void check_trad_stringification (cpp_reader *, const cpp_macro *, const cpp_string *);
     ...
     void *v __attribute__ ((__unused__));
{
  if (node->type == NT_MACRO && !(node->flags & (1 << 2)))
   ...
   && ((lookup_line (&pfile->line_maps, macro->line))->included_from < 0))
     ...
     cpp_hashnode *node;
{
 ...
 if (node->value.builtin == BT_BASE_FILE)
   while (! ((map)->included_from < 0))
     map = (&(&pfile->line_maps)->maps[(map)->included_from]);
 ...
 *buf = '\0';
 ...
 number = pfile->cur_token[-1].line;
      number = ((number) + (pfile->map)->to_line - (pfile->map)->from_line);
      break;
     ...
     cpp_hashnode *node;
{
 ...
 return 0;
  ...
  cpp_push_buffer (pfile, buf, ustrlen (buf), 1, 1);
...
stringify_arg (pfile, arg)
     ...
     macro_arg *arg;
{
  unsigned char *dest = ((pfile->u_buff)->cur);
 ...
 backslash_count = 0;
  ...
  if ((size_t) (((pfile->u_buff)->limit) - dest) < 1)
    {
      size_t len_so_far = dest - ((pfile->u_buff)->cur);
     ...
     const cpp_token **plhs, *rhs;
{
  ...
  buf = (unsigned char *) __builtin_alloca(len);
  ...
  *end = '\0';
  ...
  cpp_push_buffer (pfile, buf, end - buf, 1, 1);
   ...
   if (!prev_ident)
     {
       ...
       if (! ((pfile)->opts.c99) && ((pfile)->opts.pedantic))
  cpp_error (pfile, 0x02,
     ...
     cpp_macro *macro;
{
  ...
  if (ctoken->type == CPP_OPEN_PAREN && !(ctoken->flags & (1 << 0)))
    {
      _Bool ok = parse_params (pfile, macro);
