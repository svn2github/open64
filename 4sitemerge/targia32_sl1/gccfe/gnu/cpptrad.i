...
# 23 "../../../kgccfe/gnu/cpptrad.c" 2
# 33 "../../../kgccfe/gnu/cpptrad.c"
struct block
{
...
static const uchar *handle_newline (cpp_reader *, const uchar *);
static const uchar *skip_escaped_newlines (cpp_reader *, const uchar *);
...
static void scan_out_logical_line (cpp_reader *pfile, cpp_macro *);
static void check_output_buffer (cpp_reader *, size_t);
static void push_replacement_text (cpp_reader *, cpp_hashnode *);
static _Bool scan_parameters (cpp_reader *, cpp_macro *);
static _Bool recursive_macro (cpp_reader *, cpp_hashnode *);
static void save_replacement_text (cpp_reader *, cpp_macro *, unsigned int);

static void maybe_start_funlike (cpp_reader *, cpp_hashnode *, const uchar *, struct fun_macro *);
     ...
     const uchar *cur;
{
  ...
  while (*cur == '\\' && (_sch_istable[(cur[1]) & 0xff] & (unsigned short)(_sch_isvsp)))
    cur = handle_newline (pfile, cur + 1);

  if (cur != orig_cur && cur == ((pfile->context)->u.trad.rlimit) && pfile->buffer->inc)
    cpp_error (pfile, 0x02, "backslash-newline at end of file");

  return cur;
}
# 175 "../../../kgccfe/gnu/cpptrad.c"
static const uchar *
     ...
     int in_define;
{
  ...
  const uchar *limit = ((pfile->context)->u.trad.rlimit);
      ...
      if (c == '/')
 {
   ...
   if (*cur == '*' && cur[1] != '/'
       && ((pfile)->opts.warn_comments))
     cpp_error_with_line (pfile, 0x00, pfile->line, 0,
     "\"/*\" within comment");
 }
      else if ((_sch_istable[(c) & 0xff] & (unsigned short)(_sch_isvsp)))
 {
  ...
  cpp_error_with_line (pfile, 0x03, from_line, 0, "unterminated comment");
  *out++ = '*';
  *out++ = '/';
  ...
  for (;;)
    {
      ...
      *out++ = c;
      ...
      if ((_sch_istable[(c) & 0xff] & (unsigned short)(_sch_isnvsp)) && c)
 continue;

      if (!c && cur - 1 != ((pfile->context)->u.trad.rlimit))
 continue;
   ...
   if (*tmp == '*')
     {
      ...
      out--;
      if (c == '\\' && (_sch_istable[(*cur) & 0xff] & (unsigned short)(_sch_isvsp)))
 {
      ...
      if (pfile->buffer->cur == pfile->buffer->rlimit)
 {
   ...
   if (stop)
     return 0;
      ...
      scan_out_logical_line (pfile, ((void *)0));
    }
     ...
     cpp_macro *macro;
{
 ...
 case '\0':
   if (cur - 1 != ((context)->u.trad.rlimit))
     continue;
 ...
 case '\\':
   if ((_sch_istable[(*cur) & 0xff] & (unsigned short)(_sch_isvsp)))
     {
       out--;
       cur = skip_escaped_newlines (pfile, cur - 1);
       continue;
 ...
 case '#':
   if (out - 1 == pfile->out.base
       ...
       && !pfile->state.in_directive)
     {
       ...
       pfile->out.cur = out;
       cur = skip_whitespace (pfile, cur, 1 );
       out = pfile->out.cur;
...
recursive_macro (pfile, node)
     ...
     cpp_hashnode *node;
{
  _Bool recursing = !!(node->flags & (1 << 5));
# 823 "../../../kgccfe/gnu/cpptrad.c"
     ...
     const cpp_macro *macro;
{
      ...
      for (exp = macro->exp.text;;)
 {
   ...
   exp += (((__builtin_offsetof (struct block, text) + (b->text_len)) + ((__builtin_offsetof (struct dummy, u)) - 1)) & ~((__builtin_offsetof (struct dummy, u)) - 1));
 }
    }
  else
    len = macro->count;
     ...
     cpp_macro *macro;
{
      ...
      if (((_sch_istable[(*cur) & 0xff] & (unsigned short)(_sch_isidst)) || ((*cur) == '$' && ((pfile)->opts.dollars_in_ident))))
 {
   ...
   if (_cpp_save_parameter (pfile, macro, lex_identifier (pfile, cur)))
     break;
   cur = skip_whitespace (pfile, ((pfile->context)->u.trad.cur),
     1 );
   if (*cur == ',')
     {
       cur++;
     ...
     unsigned int arg_index;
{
  ...
  if (macro->paramc == 0)
  ...
  else
    {
      ...
      size_t blen = (((__builtin_offsetof (struct block, text) + (len)) + ((__builtin_offsetof (struct dummy, u)) - 1)) & ~((__builtin_offsetof (struct dummy, u)) - 1));
      struct block *block;
     ...
     cpp_macro *macro;
{
      ...
      else
 {
  ...
  pfile->buffer->cur
    = skip_whitespace (pfile, ((context)->u.trad.cur),
     ...
     const cpp_macro *macro1, *macro2;
{
      ...
      for (;;)
 {
   ...
   exp1 += (((__builtin_offsetof (struct block, text) + (b1->text_len)) + ((__builtin_offsetof (struct dummy, u)) - 1)) & ~((__builtin_offsetof (struct dummy, u)) - 1));
   exp2 += (((__builtin_offsetof (struct block, text) + (b2->text_len)) + ((__builtin_offsetof (struct dummy, u)) - 1)) & ~((__builtin_offsetof (struct dummy, u)) - 1));
 }
    }
  else
    {
