...
# 31 "../../../kgccfe/gnu/cppexp.c" 2


# 1 "../../../kgccfe/gnu/defaults.h" 1
# 34 "../../../kgccfe/gnu/cppexp.c" 2
...
static _Bool num_positive (cpp_num, size_t);
static _Bool num_greater_eq (cpp_num, cpp_num, size_t);
     ...
     const cpp_token *token;
{
  ...
  if (*str == '0')
    {
      ...
      if ((*str == 'x' || *str == 'X') && (_sch_istable[(str[1]) & 0xff] & (unsigned short)(_sch_isxdigit)))
 {
   radix = 16;
   str++;
 }
 ...
 cpp_error (pfile, 0x02, "use of C99 long long integer constant");

      result |= 0x0001;
    }

  if ((result & 0x2000) && ((pfile)->opts.pedantic))
    cpp_error (pfile, 0x02, "imaginary constants are a GCC extension");
     ...
     const cpp_token *token;
{
     ...
     || token->val.node == pfile->spec_nodes.n_false))
 {
   ...
   if (((pfile)->opts.pedantic)
       && ! cpp_defined (pfile, (const uchar *)"__bool_true_false_are_defined", sizeof "__bool_true_false_are_defined" - 1))
     cpp_error (pfile, 0x02,
     ...
     cpp_reader *pfile;
{
  ...
  _Bool saw_leading_not, want_value = 1;
 ...
 case CPP_MINUS:
   ...
   break;
 case CPP_OTHER:
   if ((_sch_istable[(op.token->val.c) & 0xff] & (unsigned short)(_sch_isgraph)))
     do { cpp_error (pfile, 0x03, "invalid character '%c' in #if", op.token->val.c); goto syntax_error; } while(0);
   else
     do { cpp_error (pfile, 0x03, "invalid character '\\%03o' in #if", op.token->val.c); goto syntax_error; } while(0);
      ...
      else if (want_value)
 {
  ...
  do { cpp_error (pfile, 0x03, "void expression between '(' and ')'"); goto syntax_error; } while(0);
     }
     ...
     enum cpp_ttype op;
{
  ...
  prio = optab[op].prio - ((optab[op].flags & (1 << 1)) != 0);
  while (prio < optab[top->op].prio)
    {
      if (((pfile)->opts.warn_num_sign_change)
   && optab[top->op].flags & (1 << 2))
...
num_inequality_op (pfile, lhs, rhs, op)
     ...
     enum cpp_ttype op;
{
  _Bool gte = num_greater_eq (lhs, rhs, ((pfile)->opts.precision));
     ...
     enum cpp_ttype op;
{
  ...
  _Bool eq = (lhs.low == rhs.low && lhs.high == rhs.high);
  if (op == CPP_NOT_EQ)
    eq = !eq;
  lhs.low = eq;
  lhs.high = 0;
     ...
     enum cpp_ttype op;
{
 ...
 lhs = num_rshift (lhs, precision, n);
    ...
    case CPP_MAX:
      {
 _Bool unsignedp = lhs.unsignedp || rhs.unsignedp;
 ...
 {
   _Bool lhsp = num_positive (lhs, precision);
   result.overflow = (lhsp == num_positive (rhs, precision)
    ...
    default:
      if (((pfile)->opts.pedantic) && !pfile->state.skip_eval)
 cpp_error (pfile, 0x02,
