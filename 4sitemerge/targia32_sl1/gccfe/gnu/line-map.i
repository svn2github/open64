...
# 27 "../../../kgccfe/gnu/line-map.c" 2

static void trace_include
  (const struct line_maps *, const struct line_map *);
     ...
     unsigned int to_line;
{
      ...
      if (error || to_file == ((void *)0))
 {
   ...
   to_line = (((from)[1].from_line - 1) + ((from))->to_line - ((from))->from_line) + 1;
   sysp = from->sysp;
 }
    }
     ...
     const struct line_map *map;
{
  ...
  fprintf (stderr, ("In file included from %s:%u"),
    map->to_file, (((map)[1].from_line - 1) + ((map))->to_line - ((map))->from_line));

  while (! ((map)->included_from < 0))
    {
      map = (&(set)->maps[(map)->included_from]);
# 216 "../../../kgccfe/gnu/line-map.c"
      fprintf (stderr, (",\n                 from %s:%u"),
        map->to_file, (((map)[1].from_line - 1) + ((map))->to_line - ((map))->from_line));
    }
