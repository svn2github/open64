...
# 32 "../../../kgccfe/gnu/ra-colorize.c" 2
# 48 "../../../kgccfe/gnu/ra-colorize.c"
...
static __inline__ unsigned int simplify_p (enum node_type);
static void combine (struct web *, struct web *);
static void coalesce (void);
static void freeze_moves (struct web *);
static void freeze (void);
static void select_spill (void);
static int color_usable_p (int, HARD_REG_SET, HARD_REG_SET, enum machine_mode);

int get_free_reg (HARD_REG_SET, HARD_REG_SET, enum machine_mode);
static int get_biased_reg (HARD_REG_SET, HARD_REG_SET, HARD_REG_SET, HARD_REG_SET, enum machine_mode);
...
static void add_web_pair_cost (struct web *, struct web *, unsigned long long, unsigned int);
...
{
  if (x->next || x->prev)
    fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 108, __FUNCTION__);
  x->next = *list;
  if (*list)
    (*list)->prev = x;
  *list = x;
...
{
  if (x->prev || x->next)
    fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 121, __FUNCTION__);
  if (!*list)
    {
      *list = x;
      return;
     ...
     enum node_type type;
{
 ...
 break;
      default:
 fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 205, __FUNCTION__);
    }
  web->type = type;
}
...
reset_lists ()
{
  ...
  if ((web_lists[(int)(SIMPLIFY)]) || (web_lists[(int)(SIMPLIFY_SPILL)]) || (web_lists[(int)(SIMPLIFY_FAT)])
    ...
    fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 223, __FUNCTION__);
 ...
 put_web (web, INITIAL);
  ...
  while ((d = pop_list (&(web_lists[(int)(SPILLED)]))) != ((void *)0))
    put_web (((d)->value.web), FREE);
  while ((d = pop_list (&(web_lists[(int)(COLORED)]))) != ((void *)0))
    put_web (((d)->value.web), INITIAL);
 ...
 fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 258, __FUNCTION__);
    }
     ...
     enum node_type type;
{
  if (type == PRECOLORED)
    ...
    fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 278, __FUNCTION__);
     ...
     enum move_type type;
{
 ...
 break;
      default:
 fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 321, __FUNCTION__);
    }
  move->type = type;
}
     ...
     struct move *move;
{
  ...
  for (ml = web->moves; ml; ml = ml->next)
    if (ml->move == move)
      fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 525, __FUNCTION__);
}
     ...
     struct web *u, *v;
{
  ...
  seen = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
  for (ml = u->moves; ml; ml = ml->next)
    bitmap_set_bit (seen, (((ml->move->insn)->fld[0]).rtint));
  for (ml = v->moves; ml; ml = ml_next)
    {
      ml_next = ml->next;
      if (! bitmap_bit_p (seen, (((ml->move->insn)->fld[0]).rtint)))
        {
   ml->next = u->moves;
   u->moves = ml;
 }
     ...
     struct web *target, *source;
{
  ...
  int size;
# 585 "../../../kgccfe/gnu/ra-colorize.c"
  if (! mips_hard_regno_mode_ok[ (int)(((enum machine_mode) (target->orig_x)->mode)) ][ (source->color) ])
    return 0;


  size = mips_hard_regno_nregs (color, ((enum machine_mode) (target->orig_x)->mode));
  if (!size)
    return 0;
  ...
  for (i = size; i--;)
    if ((!!((never_use_colors)[(color + i) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((color + i) % ((unsigned) (8 * 8))))))
 || !(!!((target->usable_regs)[(color + i) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((color + i) % ((unsigned) (8 * 8))))))
# 608 "../../../kgccfe/gnu/ra-colorize.c"
 || ((sup_igraph)->elms [(target->id * num_webs + hardreg2web[color + i]->id) / ((unsigned) (8 * 8))] >> (target->id * num_webs + hardreg2web[color + i]->id) % ((unsigned) (8 * 8)) & 1))
  ...
  for (wl = target->conflict_list; wl; wl = wl->next)
    {
      ...
      if (pweb->type == SELECT || pweb->type == COALESCED)
 continue;
# 632 "../../../kgccfe/gnu/ra-colorize.c"
      if (pweb->num_conflicts < (((pweb)->type == PRECOLORED) ? 1 : (pweb)->num_freedom)
   || pweb->type == PRECOLORED
   || ((igraph)->elms [(((source->id) < (pweb->id) ? ((pweb->id)*((pweb->id)-1)/2)+(source->id) : ((source->id)*((source->id)-1)/2)+(pweb->id))) / ((unsigned) (8 * 8))] >> (((source->id) < (pweb->id) ? ((pweb->id)*((pweb->id)-1)/2)+(source->id) : ((source->id)*((source->id)-1)/2)+(pweb->id))) % ((unsigned) (8 * 8)) & 1) )
 continue;
      ...
      else
 {
   ...
   for (sl = wl->sub; sl; sl = sl->next)
     {
              if (!((igraph)->elms [(((source->id) < (sl->t->id) ? ((sl->t->id)*((sl->t->id)-1)/2)+(source->id) : ((source->id)*((source->id)-1)/2)+(sl->t->id))) / ((unsigned) (8 * 8))] >> (((source->id) < (sl->t->id) ? ((sl->t->id)*((sl->t->id)-1)/2)+(source->id) : ((source->id)*((source->id)-1)/2)+(sl->t->id))) % ((unsigned) (8 * 8)) & 1))
         return 0;
     }
        }
    }
     ...
     struct web *u, *v;
{
  ...
  if (u == v || v->type == COALESCED)
    fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 727, __FUNCTION__);
  if ((u->regno >= max_normal_pseudo) != (v->regno >= max_normal_pseudo))
    fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 729, __FUNCTION__);
      ...
      if (1)
 {
   ...
   if (u->type == PRECOLORED)
     nregs = mips_hard_regno_nregs (u->color, ((enum machine_mode) (v->orig_x)->mode));
...
# 815 "../../../kgccfe/gnu/ra-colorize.c"
  u->use_my_regs = 1;
  do { HARD_REG_ELT_TYPE *scan_tp_ = (u->usable_regs), *scan_fp_ = (v->usable_regs); scan_tp_[0] &= scan_fp_[0]; scan_tp_[1] &= scan_fp_[1]; scan_tp_[2] &= scan_fp_[2]; } while (0);
  ...
  if (!u->num_freedom)
    fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 826, __FUNCTION__);
...
coalesce ()
{
  ...
  if (source == target)
    ...
    || ((sup_igraph)->elms [(source->id * num_webs + target->id) / ((unsigned) (8 * 8))] >> (source->id * num_webs + target->id) % ((unsigned) (8 * 8)) & 1)
    || ((sup_igraph)->elms [(target->id * num_webs + source->id) / ((unsigned) (8 * 8))] >> (target->id * num_webs + source->id) % ((unsigned) (8 * 8)) & 1))
    {
      remove_move (source, m);
...
freeze ()
{
  ...
  put_web (((d)->value.web), SIMPLIFY);
  freeze_moves (((d)->value.web));
}




static unsigned long long (*spill_heuristic) (struct web *);

static unsigned long long default_spill_heuristic (struct web *);
...
select_spill ()
{
  ...
  unsigned long long best2 = (unsigned long long) -1;
  struct dlist *bestd2 = ((void *)0);
  struct dlist *d;
  for (d = (web_lists[(int)(SPILL)]); d; d = d->next)
    {
      struct web *w = ((d)->value.web);
      unsigned long long cost = spill_heuristic (w);
      ...
      else if ((w->spill_temp == 2 || w->is_coalesced) && cost < best2)
 {
  ...
  if (!bestd)
    fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 1002, __FUNCTION__);


  ((bestd)->value.web)->was_spilled = 1;
  remove_list (bestd, &(web_lists[(int)(SPILL)]));
  put_web (((bestd)->value.web), SIMPLIFY);
  freeze_moves (((bestd)->value.web));
  ra_debug_msg (0x0008, " potential spill web %3d, conflicts = %d\n",
      ((bestd)->value.web)->id, ((bestd)->value.web)->num_conflicts);
}
...
{
  if (!(!!((dont_begin_colors)[(c) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((c) % ((unsigned) (8 * 8))))))
      && (!!((free_colors)[(c) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((c) % ((unsigned) (8 * 8))))))
      && mips_hard_regno_mode_ok[ (int)(mode) ][ (c) ])
    {
      int i, size;
      size = mips_hard_regno_nregs (c, mode);
      for (i = 1; i < size && (!!((free_colors)[(c + i) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((c + i) % ((unsigned) (8 * 8)))))); i++);
      if (i == size)
 return 1;
    }
  return 0;
}
# 1048 "../../../kgccfe/gnu/ra-colorize.c"
int
     ...
     enum machine_mode mode;
{
  ...
  if (flag_ra_biased)
    {
      do { HARD_REG_ELT_TYPE *scan_tp_ = (s), *scan_fp_ = (dont_begin_colors); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
      do { HARD_REG_ELT_TYPE *scan_tp_ = (s), *scan_fp_ = (bias); scan_tp_[0] |= ~ scan_fp_[0]; scan_tp_[1] |= ~ scan_fp_[1]; scan_tp_[2] |= ~ scan_fp_[2]; } while (0);
      do { HARD_REG_ELT_TYPE *scan_tp_ = (s), *scan_fp_ = (prefer_colors); scan_tp_[0] |= ~ scan_fp_[0]; scan_tp_[1] |= ~ scan_fp_[1]; scan_tp_[2] |= ~ scan_fp_[2]; } while (0);
     ...
     int len;
{
  ...
  for (i = 0; i < 176; i++)
    {
      if (!(!!((free_colors)[(i) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((i) % ((unsigned) (8 * 8)))))))
 continue;
      for (j = 1; j < len; j++)
 if (!(!!((free_colors)[(i + j) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((i + j) % ((unsigned) (8 * 8)))))))
   break;
     ...
     HARD_REG_SET *result;
{
  ...
  do { HARD_REG_ELT_TYPE *scan_tp_ = (dont_begin); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
  for (wl = web->conflict_list; wl; wl = wl->next)
    {
      struct web *w;
      ...
      while (w)
 {
   if (ptarget->type == COLORED || ptarget->type == PRECOLORED)
     {
       ...
       unsigned int tsize = mips_hard_regno_nregs (ptarget->color, ((enum machine_mode) (w->orig_x)->mode));


       unsigned int ssize = mips_hard_regno_nregs (ptarget->color, ((enum machine_mode) (source->orig_x)->mode));
    ...
    && (mode_size[(int) (((enum machine_mode) (w->orig_x)->mode))]) >= ((target_flags & 0x00002000) ? 8 : 4))
  tofs = ((((w->orig_x)->fld[1]).rtuint) / ((target_flags & 0x00002000) ? 8 : 4));
       if ((((enum rtx_code) ((source)->orig_x)->code) == SUBREG)
    && (mode_size[(int) (((enum machine_mode) (source->orig_x)->mode))])
       >= ((target_flags & 0x00002000) ? 8 : 4))
  sofs = ((((source->orig_x)->fld[1]).rtuint) / ((target_flags & 0x00002000) ? 8 : 4));
       ...
       if (c2 >= 0)
  {
    ...
    while (c1 + sofs
    + mips_hard_regno_nregs (c1, ((enum machine_mode) (source->orig_x)->mode)) - 1
    < ptarget->color + tofs)
      c1++;
    while (c1 > 0 && c1 + sofs
    + mips_hard_regno_nregs (c1, ((enum machine_mode) (source->orig_x)->mode)) - 1
    > ptarget->color + tofs)
      c1--;
    for (; c1 <= c2; c1++)
      ((dont_begin)[(c1) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((c1) % ((unsigned) (8 * 8))));
  }
     }
  ...
  do { HARD_REG_ELT_TYPE *scan_tp_ = (*result), *scan_fp_ = (dont_begin); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
}
# 1279 "../../../kgccfe/gnu/ra-colorize.c"
static void
     ...
     int hard;
{
  ...
  struct web *fat_neighbor = ((void *)0);
  ...
  do { HARD_REG_ELT_TYPE *scan_tp_ = (bias); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
  ...
  for (wl = web->conflict_list; wl; wl = wl->next)
    {
      ...
      struct web *ptarget = alias (wl->t);
      struct sub_conflict *sl = wl->sub;
      do { HARD_REG_ELT_TYPE *scan_tp_ = (bias), *scan_fp_ = (ptarget->bias_colors); scan_tp_[0] |= scan_fp_[0]; scan_tp_[1] |= scan_fp_[1]; scan_tp_[2] |= scan_fp_[2]; } while (0);
      w = sl ? sl->t : wl->t;
      if (ptarget->type != COLORED && ptarget->type != PRECOLORED
   && !ptarget->was_spilled)
        while (w)
  ...
  if (num_fat)
    {
      do { HARD_REG_ELT_TYPE *scan_tp_ = (fat_colors), *scan_fp_ = (fats_parent->usable_regs); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
...
# 1362 "../../../kgccfe/gnu/ra-colorize.c"
      if (web->use_my_regs)
 {
   do { HARD_REG_ELT_TYPE *scan_tp_ = (colors), *scan_fp_ = (web->usable_regs); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
   do { HARD_REG_ELT_TYPE *scan_tp_ = (colors), *scan_fp_ = (usable_regs[reg_preferred_class (web->regno)]); scan_tp_[0] &= scan_fp_[0]; scan_tp_[1] &= scan_fp_[1]; scan_tp_[2] &= scan_fp_[2]; } while (0);
 ...
 do { HARD_REG_ELT_TYPE *scan_tp_ = (colors), *scan_fp_ = (usable_regs[reg_preferred_class (web->regno)]); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
      ...
      do { HARD_REG_ELT_TYPE *scan_tp_ = (call_clobbered), *scan_fp_ = (colors); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
      do { HARD_REG_ELT_TYPE *scan_tp_ = (call_clobbered), *scan_fp_ = (call_used_reg_set); scan_tp_[0] &= scan_fp_[0]; scan_tp_[1] &= scan_fp_[1]; scan_tp_[2] &= scan_fp_[2]; } while (0);
      ...
      if (c < 0)
 {
   if (web->use_my_regs)
     ...
     do { HARD_REG_ELT_TYPE *scan_tp_ = (colors), *scan_fp_ = (usable_regs [reg_alternate_class (web->regno)]); scan_tp_[0] |= scan_fp_[0]; scan_tp_[1] |= scan_fp_[1]; scan_tp_[2] |= scan_fp_[2]; } while (0);
      ...
      if (num_fat)
 {
   ...
   do { HARD_REG_ELT_TYPE *scan_tp_ = (colors1), *scan_fp_ = (fat_colors); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
   for (i = 0; i < 1 + web->add_hardregs; i++)
     ((colors1)[(c + i) / ((unsigned) (8 * 8))] &= ~(((HARD_REG_ELT_TYPE) (1)) << ((c + i) % ((unsigned) (8 * 8)))));
       ...
       if (new_long > best_long_blocks)
  {
    ...
    bestc = c;
       ...
       ((dont_begin)[(c) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((c) % ((unsigned) (8 * 8))));
      ...
      if (hard && (!web->was_spilled || web->spill_temp))
 {
       ...
       if (try->type == COALESCED)
  {
    if (alias (try)->type != PRECOLORED)
      fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 1580, __FUNCTION__);
       ...
       else
  {
   ...
   colorize_one_web (try, hard - 1);
  ...
  else
    {
      ...
      if (flag_ra_biased)
 {
   int nregs = mips_hard_regno_nregs (c, ((enum machine_mode) (web->orig_x)->mode));
   for (wl = web->conflict_list; wl; wl = wl->next)
     {
       struct web *ptarget = alias (wl->t);
       int i;
       for (i = 0; i < nregs; i++)
  ((ptarget->bias_colors)[(c + i) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((c + i) % ((unsigned) (8 * 8))));
     }
 }
    }
  ...
  while ((web_lists[(int)(SELECT)]))
    {
      ...
      web = ((d)->value.web);
      colorize_one_web (((d)->value.web), 1);
    }

  for (d = (web_lists[(int)(COALESCED)]); d; d = d->next)
    {
      struct web *a = alias (((d)->value.web));
      ((d)->value.web)->color = a->color;
    }
}
# 1697 "../../../kgccfe/gnu/ra-colorize.c"
static void
try_recolor_web (web)
     struct web *web;
{
  struct conflict_link *wl;
  unsigned long long *cost_neighbors;
  unsigned int *min_color;
  int newcol, c;
  HARD_REG_SET precolored_neighbors, spill_temps;
  HARD_REG_SET possible_begin, wide_seen;
  cost_neighbors = (unsigned long long *)
    xcalloc (176, sizeof (cost_neighbors[0]));


  min_color = (unsigned int *) xcalloc (176, sizeof (int));
  do { HARD_REG_ELT_TYPE *scan_tp_ = (possible_begin); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
  for (c = 0; c < 176; c++)
    {
      int i, nregs;
      if (!mips_hard_regno_mode_ok[ (int)(((enum machine_mode) (web->orig_x)->mode)) ][ (c) ])
 continue;
      nregs = mips_hard_regno_nregs (c, ((enum machine_mode) (web->orig_x)->mode));
      for (i = 0; i < nregs; i++)
 if (!(!!((web->usable_regs)[(c + i) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((c + i) % ((unsigned) (8 * 8)))))))
   break;
      if (i < nregs || nregs == 0)
 continue;
      ((possible_begin)[(c) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((c) % ((unsigned) (8 * 8))));
      for (; nregs--;)
 if (!min_color[c + nregs])
   min_color[c + nregs] = 1 + c;
    }
  do { HARD_REG_ELT_TYPE *scan_tp_ = (precolored_neighbors); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
  do { HARD_REG_ELT_TYPE *scan_tp_ = (spill_temps); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
  do { HARD_REG_ELT_TYPE *scan_tp_ = (wide_seen); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
  for (wl = web->conflict_list; wl; wl = wl->next)
    {
      HARD_REG_SET dont_begin;
   ...
   if (web2->type == PRECOLORED)
     {
       ...
       c2 = web2->color;
       for (; c1 <= c2; c1++)
         ((precolored_neighbors)[(c1) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((c1) % ((unsigned) (8 * 8))));
     }
   continue;
 }
 ...
 if (alias (nn->t)->add_hardregs)
   wide_p = 1;
      ...
      c1 = c1 == 0 ? web2->color : (c1 - 1);
      c2 = web2->color + mips_hard_regno_nregs (web2->color, ((enum machine_mode) (web2->orig_x)->mode)) - 1;

      for (; c1 <= c2; c1++)
 if ((!!((possible_begin)[(c1) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((c1) % ((unsigned) (8 * 8)))))))
   {
     int nregs;
     HARD_REG_SET colors;
     nregs = mips_hard_regno_nregs (c1, ((enum machine_mode) (web->orig_x)->mode));
     do { HARD_REG_ELT_TYPE *scan_tp_ = (colors), *scan_fp_ = (web2->usable_regs); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
     for (; nregs--;)
       ((colors)[(c1 + nregs) / ((unsigned) (8 * 8))] &= ~(((HARD_REG_ELT_TYPE) (1)) << ((c1 + nregs) % ((unsigned) (8 * 8)))));
     if (wide_p)
       ((wide_seen)[(c1) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((c1) % ((unsigned) (8 * 8))));
     if (get_free_reg (dont_begin, colors,
         ((enum machine_mode) (web2->orig_x)->mode)) < 0)
       {
  if (web2->spill_temp)
    ((spill_temps)[(c1) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((c1) % ((unsigned) (8 * 8))));
  else
  ...
  newcol = -1;
  for (c = 0; c < 176; c++)
    if ((!!((possible_begin)[(c) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((c) % ((unsigned) (8 * 8))))))
 && !(!!((precolored_neighbors)[(c) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((c) % ((unsigned) (8 * 8))))))
 && !(!!((spill_temps)[(c) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((c) % ((unsigned) (8 * 8))))))
 && (newcol == -1
     || cost_neighbors[c] < cost_neighbors[newcol]))
      newcol = c;
  if (newcol >= 0 && cost_neighbors[newcol] < web->spill_cost)
    {
      int nregs = mips_hard_regno_nregs (newcol, ((enum machine_mode) (web->orig_x)->mode));
      ...
      for (wl = web->conflict_list; wl; wl = wl_next)
 {
   ...
   if (web2->type == COLORED)
     {
       int nregs2 = mips_hard_regno_nregs (web2->color, ((enum machine_mode) (web2->orig_x)->mode));
  ...
  cost += web2->spill_cost;
   ...
   && nregs == 1 && !(!!((wide_seen)[(newcol) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((newcol) % ((unsigned) (8 * 8)))))))
 fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 1849, __FUNCTION__);
...
# 1881 "../../../kgccfe/gnu/ra-colorize.c"
      ;
    else
      fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 1883, __FUNCTION__);
  }
     }
 }
      free (old_colors);
...
insert_coalesced_conflicts ()
{
      ...
      for (wl = web->conflict_list; wl; wl = wl->next)
 {
   ...
   if (aweb->type == PRECOLORED)
     nregs = mips_hard_regno_nregs (aweb->color, ((enum machine_mode) (web->orig_x)->mode));
   for (i = 0; i < nregs; i++)
     {
       if (aweb->type == PRECOLORED)
  tweb = hardreg2web[i + aweb->color];
       ...
       || ((sup_igraph)->elms [(tweb->id * num_webs + wl->t->id) / ((unsigned) (8 * 8))] >> (tweb->id * num_webs + wl->t->id) % ((unsigned) (8 * 8)) & 1))
     || !(wl->t->type == PRECOLORED
          || ((sup_igraph)->elms [(wl->t->id * num_webs + tweb->id) / ((unsigned) (8 * 8))] >> (wl->t->id * num_webs + tweb->id) % ((unsigned) (8 * 8)) & 1)))

    && hard_regs_intersect_p (&tweb->usable_regs,
         &wl->t->usable_regs))
  fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 1934, __FUNCTION__);
# 1943 "../../../kgccfe/gnu/ra-colorize.c"
       if (aweb->type != PRECOLORED)
  break;
     }
 }
...
check_colors ()
{
 ...
 fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 2025, __FUNCTION__);

      for (c = 0; c < nregs; c++)
 if (!(!!((web->usable_regs)[(aweb->color + c) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((aweb->color + c) % ((unsigned) (8 * 8)))))))
   fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 2029, __FUNCTION__);
 ...
 if (wl->t->regno >= max_normal_pseudo)
 ...
 else if (!wl->sub)
   {
     ...
     int nregs2;
     if (web2->type == COLORED)
       nregs2 = mips_hard_regno_nregs (web2->color, ((enum machine_mode) (web2->orig_x)->mode));
     ...
     if (aweb->color >= web2->color + nregs2
       ...
       continue;
     fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 2053, __FUNCTION__);
   }
 else
   {
     struct sub_conflict *sl;
     ...
     for (sl = wl->sub; sl; sl = sl->next)
       {
  int ssize = mips_hard_regno_nregs (scol, ((enum machine_mode) (sl->s->orig_x)->mode));
  int tsize = mips_hard_regno_nregs (tcol, ((enum machine_mode) (sl->t->orig_x)->mode));
  int sofs = 0, tofs = 0;
         if ((((enum rtx_code) ((sl->t)->orig_x)->code) == SUBREG)
      && (mode_size[(int) (((enum machine_mode) (sl->t->orig_x)->mode))]) >= ((target_flags & 0x00002000) ? 8 : 4))
    tofs = ((((sl->t->orig_x)->fld[1]).rtuint) / ((target_flags & 0x00002000) ? 8 : 4));
         if ((((enum rtx_code) ((sl->s)->orig_x)->code) == SUBREG)
      && (mode_size[(int) (((enum machine_mode) (sl->s->orig_x)->mode))])
         >= ((target_flags & 0x00002000) ? 8 : 4))
    sofs = ((((sl->s->orig_x)->fld[1]).rtuint) / ((target_flags & 0x00002000) ? 8 : 4));
  if ((tcol + tofs >= scol + sofs + ssize)
      || (scol + sofs >= tcol + tofs + tsize))
    continue;
  fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 2077, __FUNCTION__);
       }
   }
    }
}
     ...
     struct web *web;
{
  struct dlist *d, *d_next;
  if (web->type != SPILLED)
    fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 2121, __FUNCTION__);
  for (d = (web_lists[(int)(COALESCED)]); d; d = d_next)
    {
      struct web *other = ((d)->value.web);
      d_next = d->next;
      ...
      if (other->alias == web)
 {
  ...
  do { HARD_REG_ELT_TYPE *scan_tp_ = (web->usable_regs), *scan_fp_ = (web->orig_usable_regs); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
  web->is_coalesced = 0;
  web->num_aliased = 0;
  web->was_spilled = 1;
  ...
  for (d = (web_lists[(int)(COALESCED)]); d; d = d->next)
    ((d)->value.web)->alias->is_coalesced = 1;
}
     ...
     struct web *web;
{
  ...
  unsigned int nregs = mips_hard_regno_nregs (c, ((enum machine_mode) (web->orig_x)->mode));
  if (pre->type != PRECOLORED)
    fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 2167, __FUNCTION__);
  unalias_web (web);
 ...
 continue;
      ...
      do { HARD_REG_ELT_TYPE *scan_tp_ = (regs); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
      for (i = 0; i < nregs; i++)
 ((regs)[(c + i) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((c + i) % ((unsigned) (8 * 8))));
      for (wl2 = x->conflict_list; wl2; wl2 = wl2->next)
 if (wl2->t->type == COALESCED && alias (wl2->t)->type == PRECOLORED)
   ((regs)[(alias (wl2->t)->color) / ((unsigned) (8 * 8))] &= ~(((HARD_REG_ELT_TYPE) (1)) << ((alias (wl2->t)->color) % ((unsigned) (8 * 8)))));


      for (wl2 = x->orig_conflict_list; wl2; wl2 = wl2->next)
 if (wl2->t->type == PRECOLORED)
   ((regs)[(wl2->t->color) / ((unsigned) (8 * 8))] &= ~(((HARD_REG_ELT_TYPE) (1)) << ((wl2->t->color) % ((unsigned) (8 * 8)))));


      y = ((void *)0);
      for (i = 0; i < nregs; i++)
 if ((!!((regs)[(c + i) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((c + i) % ((unsigned) (8 * 8)))))))
   {
     struct web *sub;
     y = hardreg2web[c + i];
     ((sup_igraph)->elms [(x->id * num_webs + y->id) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (x->id * num_webs + y->id) % ((unsigned) (8 * 8))));
     ((sup_igraph)->elms [(y->id * num_webs + x->id) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (y->id * num_webs + x->id) % ((unsigned) (8 * 8))));
     ((igraph)->elms [(((x->id) < (y->id) ? ((y->id)*((y->id)-1)/2)+(x->id) : ((x->id)*((x->id)-1)/2)+(y->id))) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (((x->id) < (y->id) ? ((y->id)*((y->id)-1)/2)+(x->id) : ((x->id)*((x->id)-1)/2)+(y->id))) % ((unsigned) (8 * 8))));
     for (sub = x->subreg_next; sub; sub = sub->subreg_next)
       ((igraph)->elms [(((sub->id) < (y->id) ? ((y->id)*((y->id)-1)/2)+(sub->id) : ((sub->id)*((sub->id)-1)/2)+(y->id))) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (((sub->id) < (y->id) ? ((y->id)*((y->id)-1)/2)+(sub->id) : ((sub->id)*((sub->id)-1)/2)+(y->id))) % ((unsigned) (8 * 8))));
   }
      if (!y)
 continue;
      pcl = &(x->conflict_list);
 ...
 {
   struct web *y = (*pcl)->t;
   if (y->type != PRECOLORED || !(!!((regs)[(y->color) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((y->color) % ((unsigned) (8 * 8)))))))
     pcl = &((*pcl)->next);
   else
     *pcl = (*pcl)->next;
 }
      ...
      else
 {
   ...
   *pcl = wl->next;
   if (!other->have_orig_conflicts && other->type != PRECOLORED)
     fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 2261, __FUNCTION__);
   for (owl = other->orig_conflict_list; owl; owl = owl->next)
     if (owl->t == web)
       break;
   if (owl)
     fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 2266, __FUNCTION__);
       ...
       else
  {
   ...
   if (!owl && other->type != PRECOLORED)
     fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 2282, __FUNCTION__);

   ((sup_igraph)->elms [(web->id * num_webs + other->id) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (web->id * num_webs + other->id) % ((unsigned) (8 * 8))));
   ((sup_igraph)->elms [(other->id * num_webs + web->id) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (other->id * num_webs + web->id) % ((unsigned) (8 * 8))));
   ((igraph)->elms [(((web->id) < (other->id) ? ((other->id)*((other->id)-1)/2)+(web->id) : ((web->id)*((web->id)-1)/2)+(other->id))) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (((web->id) < (other->id) ? ((other->id)*((other->id)-1)/2)+(web->id) : ((web->id)*((web->id)-1)/2)+(other->id))) % ((unsigned) (8 * 8))));
   for (sl = wl->sub; sl; sl = sl->next)
     ...
     int for_move;
{
  ...
  for (p = web_pair_list, i = 0; p; p = p->next_list)
    sorted[i++] = p;
  if (i != num_web_pairs)
    fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 2486, __FUNCTION__);
      ...
      else if (w2->type == PRECOLORED)
 {
   ...
   w2 = h;
      ...
      if (w1 != w2
   && !((sup_igraph)->elms [(w1->id * num_webs + w2->id) / ((unsigned) (8 * 8))] >> (w1->id * num_webs + w2->id) % ((unsigned) (8 * 8)) & 1)
   && !((sup_igraph)->elms [(w2->id * num_webs + w1->id) / ((unsigned) (8 * 8))] >> (w2->id * num_webs + w1->id) % ((unsigned) (8 * 8)) & 1)
   && w2->type != PRECOLORED
  ...
  || (w1->type == PRECOLORED && ok (w2, w1)))
       ...
       ((w2->prefer_colors)[(w1->color) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((w1->color) % ((unsigned) (8 * 8))));
   }
    }
  free (sorted);
}
...
aggressive_coalesce ()
{
 ...
 if (s != t
     && t->type != PRECOLORED
     && !((sup_igraph)->elms [(s->id * num_webs + t->id) / ((unsigned) (8 * 8))] >> (s->id * num_webs + t->id) % ((unsigned) (8 * 8)) & 1)
     && !((sup_igraph)->elms [(t->id * num_webs + s->id) / ((unsigned) (8 * 8))] >> (t->id * num_webs + s->id) % ((unsigned) (8 * 8)) & 1))
   {
     if ((s->type == PRECOLORED && ok (t, s))
  || s->type != PRECOLORED)
       {
         put_move (m, MV_COALESCED);
  add_web_pair_cost (s, t, (((m->insn)->fld[3]).bb)->frequency,
       0);
       }
       ...
       {
  put_move (m, CONSTRAINED);
  ((t->prefer_colors)[(s->color) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((s->color) % ((unsigned) (8 * 8))));
       }
   }
 else
   {
...
extended_coalesce_2 ()
{
  ...
  for (insn = get_insns (); insn; insn = (((insn)->fld[2]).rtx))
    if (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i') && (info = insn_df[(((insn)->fld[0]).rtint)]).num_defs)
      for (n = 0; n < info.num_defs; n++)
 {
       ...
       for (n2 = 0; n2 < info.num_uses; n2++)
  {
    ...
    if (source->type != PRECOLORED
        ...
        && !((sup_igraph)->elms [(dest->id * num_webs + source->id) / ((unsigned) (8 * 8))] >> (dest->id * num_webs + source->id) % ((unsigned) (8 * 8)) & 1)

        && !((sup_igraph)->elms [(source->id * num_webs + dest->id) / ((unsigned) (8 * 8))] >> (source->id * num_webs + dest->id) % ((unsigned) (8 * 8)) & 1)

        && hard_regs_intersect_p (&source->usable_regs,
      &dest->usable_regs))
      add_web_pair_cost (dest, source,
           (((insn)->fld[3]).bb)->frequency,
           dest->num_conflicts
...
check_uncoalesced_moves ()
{
 ...
 if (s != t
  ...
  || s->type != PRECOLORED)
     && !((sup_igraph)->elms [(s->id * num_webs + t->id) / ((unsigned) (8 * 8))] >> (s->id * num_webs + t->id) % ((unsigned) (8 * 8)) & 1)
     && !((sup_igraph)->elms [(t->id * num_webs + s->id) / ((unsigned) (8 * 8))] >> (t->id * num_webs + s->id) % ((unsigned) (8 * 8)) & 1))
   fancy_abort ("../../../kgccfe/gnu/ra-colorize.c", 2655, __FUNCTION__);
      }
}
