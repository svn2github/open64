...
static void mark_ident_hash (void *);
...
static hashnode
alloc_node (table)
     hash_table *table __attribute__ ((__unused__));
{
  return (&((struct tree_identifier *) (make_node (IDENTIFIER_NODE)))->id);
}
     ...
     int length;
{
  ...
  __extension__ ({ struct obstack *__o = (&string_stack); int __len = (length); if (__o->next_free + __len + 1 > __o->chunk_limit) _obstack_newchunk (__o, __len + 1); memcpy ((__o->next_free), ((contents)), (__len)); __o->next_free += __len; *(__o->next_free)++ = 0; (void) 0; });
