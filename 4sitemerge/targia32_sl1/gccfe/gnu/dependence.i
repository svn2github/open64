...
# 35 "../../../kgccfe/gnu/dependence.c" 2
# 62 "../../../kgccfe/gnu/dependence.c"
...
typedef struct dependence
{
  ...
  enum direction_type direction[13];
  int distance[13];
...
static void classify_dependence (subscript[], subscript[], enum complexity_type[], int*, int);

static void ziv_test (subscript[], subscript[], enum direction_type[][13], int[][13], loop*, int);


static void siv_test (subscript[], subscript[], enum direction_type[][13], int[][13], loop*, int);


static int check_subscript_induction (subscript*, subscript*, loop*);
static void gcd_test (subscript[], subscript[], enum direction_type[][13], int[][13], loop*, int);


static int find_gcd (int, int);
static void merge_dependencies (enum direction_type[][13], int[][13], int, int);
     ...
     tree exp;
{
  ...
  def_use_chain = varray_init (50, VARRAY_DATA_GENERIC, "def_use_chain");
  dep_chain = varray_init (50, VARRAY_DATA_GENERIC, "dep_chain");
  loop_chain = varray_init (50, VARRAY_DATA_GENERIC, "loop_chain");
  induction_chain = varray_init (50, VARRAY_DATA_GENERIC, "induction_chain");
  ...
  for (du_ptr = ((def_use_chain)->data.generic[(def_use_chain)->elements_used - 1]);
       ((def_use_chain)->elements_used--);
       du_ptr = ((def_use_chain)->data.generic[(def_use_chain)->elements_used - 1]))
    {
      free (du_ptr);
    }

  do { if (def_use_chain) { free (def_use_chain); def_use_chain = (varray_type) 0; } } while (0);
  do { if (loop_chain) { free (loop_chain); loop_chain = (varray_type) 0; } } while (0);
  do { if (induction_chain) { free (induction_chain); induction_chain = (varray_type) 0; } } while (0);
}
     ...
     enum def_use_type du_type;
{
 ...
 break;
      case TREE_LIST:
 build_def_use (((node)->list.value), 0);
 ...
 if (find_induction_variable (((node)->exp.operands[0]),
         ((node)->exp.operands[1]),
         ((node)->exp.operands[2]), loop_def)
     == 0)
   loop_def->status = unnormal;

 build_def_use (((node)->exp.operands[3]), 0);
 nloop--;
 current_loop = 0;
 node = ((node)->common.chain);
 break;
 ...
 if (loop_def
     && ((enum tree_code) (((node)->exp.operands[0]))->common.code) == VAR_DECL
     && have_induction_variable
        (loop_def->outer_loop,
  ((char *)(((((node)->exp.operands[0]))->decl.name))->identifier.id.str)) >= 0)
   loop_def->status = unnormal;

 if (((enum tree_code) (((node)->exp.operands[0]))->common.code) == ARRAY_REF
     || ((enum tree_code) (((node)->exp.operands[0]))->common.code) == INDIRECT_REF)
   build_def_use (((node)->exp.operands[0]), def);

 build_def_use (((node)->exp.operands[1]), use);
 node = ((node)->common.chain);
 break;
      case INDIRECT_REF:
 if (! ((node)->exp.operands[1])
     || ((enum tree_code) (((node)->exp.operands[1]))->common.code) != ARRAY_REF)
   {
     node = 0;
     break;
   }
 node = ((node)->exp.operands[1]);
      case ARRAY_REF:
 if (nloop)
   {
     int i;
     ...
     do { if ((def_use_chain)->elements_used >= (def_use_chain)->num_elements) (((def_use_chain)) = varray_grow ((def_use_chain), 2 * (def_use_chain)->num_elements)); (def_use_chain)->data.generic[(def_use_chain)->elements_used++] = (xmalloc (sizeof (def_use))); } while (0);
     du_ptr = ((def_use_chain)->data.generic[du_idx++]);
     du_ptr->type = du_type;
     ...
     du_ptr->dep = 0;
     for (array_ref = node;
   ((enum tree_code) (array_ref)->common.code) == ARRAY_REF;
   array_ref = ((array_ref)->exp.operands[0]))
       ;

     if (((enum tree_code) (array_ref)->common.code) == COMPONENT_REF)
       {
  array_ref = ((array_ref)->exp.operands[1]);
  if (! (((enum tree_code) (array_ref)->common.code) == FIELD_DECL
         && ((enum tree_code) (((array_ref)->common.type))->common.code) == ARRAY_TYPE))
    {
      node = 0;
      break;
    }
     ...
     for (i = 0;
   i < du_idx
     && strcmp (((char *)(((array_ref)->decl.name))->identifier.id.str),
         ((def_use*) (((def_use_chain)->data.generic[i])))->variable);
   ...
   i++)
     ...
     if (i != du_idx)
       {
  def_use *tmp_duc;
  for (tmp_duc = ((def_use*) (((def_use_chain)->data.generic[i])));
       tmp_duc->next;
       tmp_duc = ((def_use*)tmp_duc->next));
  tmp_duc->next = du_ptr;
       }
     else du_ptr->next = 0;
     du_ptr->variable = ((char *)(((array_ref)->decl.name))->identifier.id.str);
   }
 node = 0;
 break;
 ...
 break;
      ...
      case EXPR_STMT:
 if (((enum tree_code) (((node)->exp.operands[0]))->common.code) == MODIFY_EXPR)
   build_def_use (((node)->exp.operands[0]), def);
 node = ((node)->common.chain);
 break;

      default:
 if (tree_code_type[(int) (((enum tree_code) (node)->common.code))] == '2')
   {
     build_def_use (((node)->exp.operands[0]), use);
     build_def_use (((node)->exp.operands[1]), use);
     node = ((node)->common.chain);
   }
 else
     ...
     int nloop;
{
  ...
  do { if ((loop_chain)->elements_used >= (loop_chain)->num_elements) (((loop_chain)) = varray_grow ((loop_chain), 2 * (loop_chain)->num_elements)); (loop_chain)->data.generic[(loop_chain)->elements_used++] = (xmalloc (sizeof (loop))); } while (0);
  loop_ptr = ((loop_chain)->data.generic[(loop_chain)->elements_used - 1]);
     ...
     loop *loop_def;
{
  ...
  incr = incr_node;
  while (((enum tree_code) (incr)->common.code) == COMPOUND_EXPR)
    {
      incr_code = ((enum tree_code) (((incr)->exp.operands[0]))->common.code);
      if (incr_code == PREDECREMENT_EXPR || incr_code == POSTDECREMENT_EXPR
   || incr_code == PREINCREMENT_EXPR || incr_code == POSTINCREMENT_EXPR)
 {
   incr_node = ((incr)->exp.operands[0]);
   break;
 }
      incr_code = ((enum tree_code) (((incr)->exp.operands[1]))->common.code);
      if (incr_code == PREDECREMENT_EXPR || incr_code == POSTDECREMENT_EXPR
   || incr_code == PREINCREMENT_EXPR || incr_code == POSTINCREMENT_EXPR)
 {
   incr_node = ((incr)->exp.operands[1]);
   break;
 }
      incr = ((incr)->exp.operands[1]);
    }


  cond_node = ((cond_node)->list.value);
  incr = cond_node;
# 481 "../../../kgccfe/gnu/dependence.c"
  while (((enum tree_code) (incr)->common.code) == TRUTH_ANDIF_EXPR
  || ((enum tree_code) (incr)->common.code) == TRUTH_ORIF_EXPR)
    {
      if ((tree_code_type[(int) (((enum tree_code) (((incr)->exp.operands[0]))->common.code))] == '<') && (((enum tree_code) (((((incr)->exp.operands[0]))->exp.operands[0]))->common.code) == VAR_DECL && (((char *)(((((((incr)->exp.operands[0]))->exp.operands[0]))->decl.name))->identifier.id.str) == ((char *)(((((incr_node)->exp.operands[0]))->decl.name))->identifier.id.str))) ? 1 : 0)
   {
     cond_node = ((incr)->exp.operands[0]);
     break;
   }
      if ((tree_code_type[(int) (((enum tree_code) (((incr)->exp.operands[1]))->common.code))] == '<') && (((enum tree_code) (((((incr)->exp.operands[1]))->exp.operands[0]))->common.code) == VAR_DECL && (((char *)(((((((incr)->exp.operands[1]))->exp.operands[0]))->decl.name))->identifier.id.str) == ((char *)(((((incr_node)->exp.operands[0]))->decl.name))->identifier.id.str))) ? 1 : 0)
   {
     cond_node = ((incr)->exp.operands[1]);
     break;
   }
      incr = ((incr)->exp.operands[0]);
    }

  incr_code = ((enum tree_code) (incr_node)->common.code);
  if ((incr_code == PREDECREMENT_EXPR || incr_code == POSTDECREMENT_EXPR
       || incr_code == PREINCREMENT_EXPR || incr_code == POSTINCREMENT_EXPR)
      && tree_code_type[(int) (((enum tree_code) (cond_node)->common.code))] == '<')
    {
      if (!(tree_code_type[(int) (((enum tree_code) (cond_node)->common.code))] == '<') && (((enum tree_code) (((cond_node)->exp.operands[0]))->common.code) == VAR_DECL && (((char *)(((((cond_node)->exp.operands[0]))->decl.name))->identifier.id.str) == ((char *)(((((incr_node)->exp.operands[0]))->decl.name))->identifier.id.str))) ? 1 : 0)
 return 0;

      do { if ((induction_chain)->elements_used >= (induction_chain)->num_elements) (((induction_chain)) = varray_grow ((induction_chain), 2 * (induction_chain)->num_elements)); (induction_chain)->data.generic[(induction_chain)->elements_used++] = (xmalloc (sizeof (induction))); } while (0);
      ind_ptr = ((induction_chain)->data.generic[(induction_chain)->elements_used - 1]);
      loop_def->ind = ind_ptr;
      ind_ptr->variable = ((char *)(((((incr_node)->exp.operands[0]))->decl.name))->identifier.id.str);

      ind_ptr->increment = (((((incr_node)->exp.operands[1]))->int_cst.int_cst).low);
      if (((enum tree_code) (incr_node)->common.code) == PREDECREMENT_EXPR
   || ((enum tree_code) (incr_node)->common.code) == POSTDECREMENT_EXPR)
 ind_ptr->increment = -ind_ptr->increment;

      ind_ptr->low_bound = get_low_bound (init_node, ind_ptr->variable);
      if (((enum tree_code) (((cond_node)->exp.operands[0]))->common.code) == VAR_DECL
   && ((char *)(((((cond_node)->exp.operands[0]))->decl.name))->identifier.id.str)
      == ind_ptr->variable)
 {
   if (((enum tree_code) (((cond_node)->exp.operands[1]))->common.code) == INTEGER_CST)
     ind_ptr->high_bound =
       (((((cond_node)->exp.operands[1]))->int_cst.int_cst).low);
   else
     ind_ptr->high_bound = ind_ptr->increment < 0 ? (-2147483647 - 1) : 2147483647;
 }
      else if (((enum tree_code) (((cond_node)->exp.operands[1]))->common.code) == VAR_DECL
   && ((char *)(((((cond_node)->exp.operands[1]))->decl.name))->identifier.id.str)
        == ind_ptr->variable)
 {
   if (((enum tree_code) (((cond_node)->exp.operands[0]))->common.code) == INTEGER_CST)
     ind_ptr->high_bound =
       (((((cond_node)->exp.operands[0]))->int_cst.int_cst).low);
   else
     ind_ptr->high_bound = ind_ptr->increment < 0 ? (-2147483647 - 1) : 2147483647;
 }
      ind_ptr->next = 0;
     ...
     const char *variable;
{
  ...
  if (((enum tree_code) (node)->common.code) == SCOPE_STMT)
    node = ((node)->common.chain);

  if (! node)
    return (-2147483647 - 1);

  while (((enum tree_code) (node)->common.code) == COMPOUND_EXPR)
    {
      if (((enum tree_code) (((node)->exp.operands[0]))->common.code) == MODIFY_EXPR
   && (((enum tree_code) (((node)->exp.operands[0]))->common.code) == VAR_DECL
       && ((char *)(((((node)->exp.operands[0]))->decl.name))->identifier.id.str)
       == variable))
 break;
    }

  if (((enum tree_code) (node)->common.code) == EXPR_STMT)
    node = ((node)->exp.operands[0]);
  if (((enum tree_code) (node)->common.code) == MODIFY_EXPR
      && (((enum tree_code) (((node)->exp.operands[0]))->common.code) == VAR_DECL
   && ((char *)(((((node)->exp.operands[0]))->decl.name))->identifier.id.str)
   == variable))
    {
      return (((((node)->exp.operands[1]))->int_cst.int_cst).low);
    }
  return (-2147483647 - 1);
}
     ...
     const char *ind_var;
{
  ...
  for (loop_ptr = ((loop_chain)->data.generic[loop_idx]);
       loop_ptr && loop_idx < ((loop_chain)->num_elements);
       loop_ptr = ((loop_chain)->data.generic[++loop_idx]))
    if (loop_ptr->outer_loop == outer_loop)
      for (ind_ptr = loop_ptr->ind;
    ind_ptr && ind_idx < ((induction_chain)->num_elements);
...
link_loops ()
{
  ...
  prev_loop_ptr = ((loop_chain)->data.generic[loop_idx]);
  for (loop_ptr = ((loop_chain)->data.generic[++loop_idx]);
       loop_ptr && loop_idx < ((loop_chain)->num_elements);
       loop_ptr = ((loop_chain)->data.generic[++loop_idx]))
    {
...
get_node_dependence ()
{
  ...
  du_idx = 0;
  for (du_ptr = ((def_use_chain)->data.generic[du_idx]);
       du_ptr && du_idx < ((def_use_chain)->num_elements);
       du_ptr = ((def_use_chain)->data.generic[du_idx++]))
    {
     ...
     def_use *du;
{
    ...
    loop_ptr = ((loop_chain)->data.generic[++loop_idx]))
 {
   if (loop_ptr->outer_loop == def_ptr->outer_loop)
    ...
    ck_loop_ptr && loop_idx < ((loop_chain)->num_elements);
    ck_loop_ptr = ((loop_chain)->data.generic[++loop_idx]))
 {
      ...
      for (use_ptr = du; use_ptr; use_ptr = use_ptr->next)
 {
  ...
  have_dependence = 1;
   ...
   do { if ((dep_chain)->elements_used >= (dep_chain)->num_elements) (((dep_chain)) = varray_grow ((dep_chain), 2 * (dep_chain)->num_elements)); (dep_chain)->data.generic[(dep_chain)->elements_used++] = (xmalloc (sizeof (dependence))); } while (0);
   dep_ptr = ((dep_chain)->data.generic[(dep_chain)->elements_used - 1]);
   ...
   if (! dep_list)
     {
       ...
       do { if ((dep_chain)->elements_used >= (dep_chain)->num_elements) (((dep_chain)) = varray_grow ((dep_chain), 2 * (dep_chain)->num_elements)); (dep_chain)->data.generic[(dep_chain)->elements_used++] = (xmalloc (sizeof (dependence))); } while (0);
       dep_root_ptr = ((dep_chain)->data.generic[(dep_chain)->elements_used - 1]);
     ...
     subscript coefficients [13];
{
  ...
  for (array_ref = du->expression;
       ...
       array_ref = ((array_ref)->exp.operands[0]))
    {
      if (((enum tree_code) (((array_ref)->exp.operands[1]))->common.code) == INTEGER_CST)
 coefficients[idx].offset = (((((array_ref)->exp.operands[1]))->int_cst.int_cst).low);
      else
 if (get_one_coefficient (((array_ref)->exp.operands[1]),
     ...
     enum tree_code *type;
{
  ...
  tree_op = ((enum tree_code) (node)->common.code);
  if (type)
    *type = tree_op;
  ...
  if (tree_op == VAR_DECL)
    {
      index = have_induction_variable (du->outer_loop,
           ((char *)(((node)->decl.name))->identifier.id.str));
      if (index >= 0)
 {
   coefficients->position = index;
   coefficients->variable = ((char *)(((node)->decl.name))->identifier.id.str);
   coefficients->coefficient = 1;
   if (coefficients->offset == (-2147483647 - 1))
    ...
    {
      return get_one_coefficient (((node)->exp.operands[0]), coefficients, du,
      &tree_op_code);
    }
  else if (tree_op == PLUS_EXPR)
    {
      value = get_one_coefficient (((node)->exp.operands[0]), coefficients, du,
       &tree_op_code);
      if (tree_op_code == INTEGER_CST)
 coefficients->offset = value;

      value = get_one_coefficient (((node)->exp.operands[1]), coefficients, du,
       &tree_op_code);
      if (tree_op_code == INTEGER_CST)
 coefficients->offset = value;
 ...
 coefficients->offset = -value;
  ...
  else if (tree_op == MULT_EXPR)
    {
      ...
      value0 = get_one_coefficient (((node)->exp.operands[0]), coefficients, du,
        &tree_op_code);
      if (tree_op_code == VAR_DECL)
 value0_is_idx = 1;

      value1 = get_one_coefficient (((node)->exp.operands[1]), coefficients, du,
        &tree_op_code);
      if (tree_op_code == VAR_DECL)
 value1_is_idx = 1;
 ...
 for (ind_ptr = ck_loop_ptr->ind; ind_ptr; ind_ptr = ind_ptr->next)
  ...
  if (ind_ptr->low_bound < ind_ptr->high_bound)
    coefficients[i].offset += coefficients[i].coefficient
      * ind_ptr->low_bound;
  else if (ind_ptr->high_bound != (-2147483647 - 1))
    {
      coefficients[i].offset = coefficients[i].coefficient
        * ind_ptr->high_bound;
...
classify_dependence (icoefficients, ocoefficients, complexity, separability,
     ...
     int count;
{
  const char *iiv_used [13];
  const char *oiv_used [13];
  int ocoeff [13];
  int icoeff [13];
  int idx, cidx;

  memset (iiv_used, 0, sizeof (tree) * 13);
  memset (oiv_used, 0, sizeof (tree) * 13);
  memset (icoeff, 0, sizeof (int) * 13);
  memset (ocoeff, 0, sizeof (int) * 13);
...
ziv_test (icoefficients, ocoefficients, direction, distance, loop_ptr, sub)
     ...
     subscript ocoefficients [13];
     enum direction_type direction[13][13];
     int distance[13][13] __attribute__ ((__unused__));
     loop *loop_ptr;
     int sub;
{
     ...
     int sub;
{
  ...
  if (ocoefficients[sub].coefficient == (-2147483647 - 1))
    coef = icoefficients[sub].coefficient;
  else if (icoefficients[sub].coefficient == (-2147483647 - 1))
...
dump_array_ref (node)
     tree node;
{
  enum tree_code tree_op = ((enum tree_code) (node)->common.code);

  if (tree_op == VAR_DECL)
    {
      printf ("%s", ((char *)(((node)->decl.name))->identifier.id.str));
    }
  else if (tree_op == INTEGER_CST)
    {
      printf ("%d", (int)(((node)->int_cst.int_cst).low));
    }
  else if (tree_op == PLUS_EXPR)
    {
      dump_array_ref (((node)->exp.operands[0]));
      printf ("+");
      dump_array_ref (((node)->exp.operands[1]));
    }
  else if (tree_op == MINUS_EXPR)
    {
      dump_array_ref (((node)->exp.operands[0]));
     ...
     tree node;
{
  ...
  if (dep_chain)
    {
      if (((enum tree_code) (node)->common.code) == INDIRECT_REF && ((node)->exp.operands[1])
   && ((enum tree_code) (((node)->exp.operands[1]))->common.code) == ARRAY_REF)
 node = ((node)->exp.operands[1]);

      for (dep_ptr = ((dep_chain)->data.generic[0]);
    dep_ptr; dep_ptr = ((dep_chain)->data.generic[dep_idx++]))
 {
     ...
     int distance[13];
{
  ...
  if (((enum rtx_code) (((((((dest_rtx)->fld[5]).rtx))->fld[0]).rtx))->code) == MEM)
    {
      dest = ((((((dest_rtx)->fld[5]).rtx))->fld[0]).rtx);
      dest_idx = (((dest)->fld[2]).rtwint) - 1;
    }
  if (((enum rtx_code) (((((((src_rtx)->fld[5]).rtx))->fld[1]).rtx))->code) == MEM)
    {
      src = ((((((src_rtx)->fld[5]).rtx))->fld[1]).rtx);
      src_idx = (((src)->fld[2]).rtwint) - 1;
    }
  if (dest_idx >= 0 || src_idx >= 0)
    return 0;

  for (dep_ptr = ((dep_chain)->data.generic[-dest_idx]);
       dep_ptr; dep_ptr = dep_ptr->next)
    {
      if (dep_ptr == ((dep_chain)->data.generic[-src_idx]))
 {
...
{
  do { if (dep_chain) { free (dep_chain); dep_chain = (varray_type) 0; } } while (0);
}
