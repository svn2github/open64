...
# 25 "../../../kgccfe/gnu/ra.c" 2
# 1 "../../../kgccfe/gnu/insn-config.h" 1
# 26 "../../../kgccfe/gnu/ra.c" 2
...
HARD_REG_SET hardregs_for_mode[(int) MAX_MACHINE_MODE];
...
# 199 "../../../kgccfe/gnu/ra.c"
  unsigned int ofs;
  for (ofs = 0; ofs < ((176 + (8 * 8) - 1) / (8 * 8)); ofs++)
    {
     ...
     struct df *df;
{
 ...
 continue;
      for (n = 0, link = ((df)->insns[(((insn)->fld[0]).rtint)].defs); link; link = link->next)
        if (link->ref
     && (((((((enum rtx_code) ((link->ref)->reg)->code) == SUBREG ? ((((link->ref)->reg)->fld[0]).rtx) : ((link->ref)->reg)))->fld[0]).rtuint) >= 176
  || !(!!((never_use_colors)[(((((((enum rtx_code) ((link->ref)->reg)->code) == SUBREG ? ((((link->ref)->reg)->fld[0]).rtx) : ((link->ref)->reg)))->fld[0]).rtuint)) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((((((((enum rtx_code) ((link->ref)->reg)->code) == SUBREG ? ((((link->ref)->reg)->fld[0]).rtx) : ((link->ref)->reg)))->fld[0]).rtuint)) % ((unsigned) (8 * 8))))))))

   {
     ...
     insn_df[uid].defs[n++] = link->ref;
      ...
      for (n = 0, link = ((df)->insns[(((insn)->fld[0]).rtint)].uses); link; link = link->next)
        if (link->ref
     && (((((((enum rtx_code) ((link->ref)->reg)->code) == SUBREG ? ((((link->ref)->reg)->fld[0]).rtx) : ((link->ref)->reg)))->fld[0]).rtuint) >= 176
  || !(!!((never_use_colors)[(((((((enum rtx_code) ((link->ref)->reg)->code) == SUBREG ? ((((link->ref)->reg)->fld[0]).rtx) : ((link->ref)->reg)))->fld[0]).rtuint)) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((((((((enum rtx_code) ((link->ref)->reg)->code) == SUBREG ? ((((link->ref)->reg)->fld[0]).rtx) : ((link->ref)->reg)))->fld[0]).rtuint)) % ((unsigned) (8 * 8))))))))
  ...
  if (refs_for_insn_df + (df->def_id + df->use_id) < act_refs)
    fancy_abort ("../../../kgccfe/gnu/ra.c", 282, __FUNCTION__);
}
...
{
  struct web *w = web;
  if (size_word == (mode_size[(int) (((enum machine_mode) (web->orig_x)->mode))]))
...
{
  HARD_REG_SET c;
  do { HARD_REG_ELT_TYPE *scan_tp_ = (c), *scan_fp_ = (*a); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
  do { HARD_REG_ELT_TYPE *scan_tp_ = (c), *scan_fp_ = (*b); scan_tp_[0] &= scan_fp_[0]; scan_tp_[1] &= scan_fp_[1]; scan_tp_[2] &= scan_fp_[2]; } while (0);
     ...
     struct df *df;
{
 ...
 live_at_end[i] = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
      live_at_end += 2;
    }
  create_insn_info (df);
}
     ...
     struct df *df;
{
  ...
  __extension__ ({ struct obstack *__o = (&ra_obstack); void *__obj = (((void *)0)); if (__obj > (void *)__o->chunk && __obj < (void *)__o->chunk_limit) __o->next_free = __o->object_base = __obj; else (obstack_free) (__o, __obj); });
}
...
init_ra ()
{
  ...
  do { HARD_REG_ELT_TYPE *scan_tp_ = (never_use_colors), *scan_fp_ = (fixed_reg_set); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
  ...
  for (j = 0; j < (sizeof (eliminables) / sizeof ((eliminables)[0])); j++)
    {
      if (! (((eliminables[j].from) == 75 && (((eliminables[j].to) == (0 + 29) && ! frame_pointer_needed) || (eliminables[j].to) == ((target_flags & 0x00100000) ? 0 + 17 : 0 + 30))) || ((eliminables[j].from) != 75 && ((eliminables[j].to) == ((target_flags & 0x00100000) ? 0 + 17 : 0 + 30) || ((eliminables[j].to) == (0 + 29) && ! frame_pointer_needed && ! ((target_flags & 0x00100000) && (target_flags & 0x00002000)) && (! (target_flags & 0x00100000) || compute_frame_size (get_frame_size ()) < 32768)))))
   || (eliminables[j].to == (0 + 29) && need_fp))
 for (i = mips_hard_regno_nregs (eliminables[j].from, (((target_flags & 0x00000002) && (target_flags & 0x00002000)) ? DImode : SImode)); i--;)
   ((never_use_colors)[(eliminables[j].from + i) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((eliminables[j].from + i) % ((unsigned) (8 * 8))));
    }
  ...
  if (need_fp)
    for (i = mips_hard_regno_nregs (((target_flags & 0x00100000) ? 0 + 17 : 0 + 30), (((target_flags & 0x00000002) && (target_flags & 0x00002000)) ? DImode : SImode)); i--;)
      ((never_use_colors)[(((target_flags & 0x00100000) ? 0 + 17 : 0 + 30) + i) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((((target_flags & 0x00100000) ? 0 + 17 : 0 + 30) + i) % ((unsigned) (8 * 8))));
# 507 "../../../kgccfe/gnu/ra.c"
  for (i = mips_hard_regno_nregs ((0 + 29), (((target_flags & 0x00000002) && (target_flags & 0x00002000)) ? DImode : SImode)); i--;)
    ((never_use_colors)[((0 + 29) + i) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << (((0 + 29) + i) % ((unsigned) (8 * 8))));

  for (i = mips_hard_regno_nregs (0, (((target_flags & 0x00000002) && (target_flags & 0x00002000)) ? DImode : SImode)); i--;)
    ((never_use_colors)[(0 + i) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((0 + i) % ((unsigned) (8 * 8))));
      ...
      while (byte)
 {
  ...
  for (i = 0; i < (int) LIM_REG_CLASSES; i++)
    {
      int size;
      do { HARD_REG_ELT_TYPE *scan_tp_ = (rs), *scan_fp_ = (reg_class_contents[i]); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
      do { HARD_REG_ELT_TYPE *scan_tp_ = (rs), *scan_fp_ = (never_use_colors); scan_tp_[0] &= ~ scan_fp_[0]; scan_tp_[1] &= ~ scan_fp_[1]; scan_tp_[2] &= ~ scan_fp_[2]; } while (0);
      size = hard_regs_count (rs);
      num_free_regs[i] = size;
      do { HARD_REG_ELT_TYPE *scan_tp_ = (usable_regs[i]), *scan_fp_ = (rs); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
    }
  ...
  for (i = 0; i < (int) MAX_MACHINE_MODE; i++)
    {
      int reg, size;
      do { HARD_REG_ELT_TYPE *scan_tp_ = (rs); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
      for (reg = 0; reg < 176; reg++)
 if (mips_hard_regno_mode_ok[ (int)(i) ][ (reg) ]

     && (size = mips_hard_regno_nregs (reg, i)) != 0
     && (reg + size) <= 176)
   {
     while (size--)
       ((rs)[(reg + size) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((reg + size) % ((unsigned) (8 * 8))));
   }
      do { HARD_REG_ELT_TYPE *scan_tp_ = (hardregs_for_mode[i]), *scan_fp_ = (rs); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
    }

  for (an_unusable_color = 0; an_unusable_color < 176;
       an_unusable_color++)
    if ((!!((never_use_colors)[(an_unusable_color) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((an_unusable_color) % ((unsigned) (8 * 8)))))))
      break;
  if (an_unusable_color == 176)
    fancy_abort ("../../../kgccfe/gnu/ra.c", 561, __FUNCTION__);
     ...
     struct df *df;
{
  ...
  bitmap empty_defs = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
  bitmap empty_uses = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
 ...
 for (link = ((df)->insns[(((insn)->fld[0]).rtint)].defs); link; link = link->next)
   if (!link->ref || bitmap_bit_p (empty_defs, ((link->ref)->id))
     ...
     fancy_abort ("../../../kgccfe/gnu/ra.c", 606, __FUNCTION__);
   else
     bitmap_set_bit (b, ((link->ref)->id));

 bitmap_clear (b);
 for (link = ((df)->insns[(((insn)->fld[0]).rtint)].uses); link; link = link->next)
   if (!link->ref || bitmap_bit_p (empty_uses, ((link->ref)->id))
       || bitmap_bit_p (b, ((link->ref)->id)))
     fancy_abort ("../../../kgccfe/gnu/ra.c", 614, __FUNCTION__);
   else
     bitmap_set_bit (b, ((link->ref)->id));
      }
 ...
 if (!link->ref || bitmap_bit_p (empty_defs, ((link->ref)->id))
     || bitmap_bit_p (b, ((link->ref)->id)))
   fancy_abort ("../../../kgccfe/gnu/ra.c", 626, __FUNCTION__);
 else
   bitmap_set_bit (b, ((link->ref)->id));
 ...
 if (!link->ref || bitmap_bit_p (empty_uses, ((link->ref)->id))
     || bitmap_bit_p (b, ((link->ref)->id)))
   fancy_abort ("../../../kgccfe/gnu/ra.c", 634, __FUNCTION__);
 else
   bitmap_set_bit (b, ((link->ref)->id));
    }
...
reg_alloc ()
{
      ...
      for (e = (&entry_exit_blocks[1])->pred; e; e = e->pred_next)
 {
   ...
   last = bb->end;
   if (!((rtx_class[(int) (((enum rtx_code) (last)->code))]) == 'i') || ((enum rtx_code) ((((last)->fld[5]).rtx))->code) != USE)
     {
       rtx insns;
       start_sequence ();
       use_return_register ();
