...
# 46 "../../../kgccfe/gnu/ssa.c" 2
# 1 "../../../kgccfe/gnu/insn-config.h" 1
# 47 "../../../kgccfe/gnu/ssa.c" 2
...
static rtx ssa_rename_to_hard[176][(int) MAX_MACHINE_MODE];
...
static void ssa_rename_from_insert
  (unsigned int reg, rtx r);
static void ssa_rename_from_free
  (void);
typedef int (*srf_trav) (int regno, rtx r, sbitmap canonical_elements, partition reg_partition);
static void ssa_rename_from_traverse
  (htab_trav callback_function, sbitmap canonical_elements, partition reg_partition);
...
static int ssa_rename_from_equal
  (const void *srfp1, const void *srfp2);
...
static __inline__ rtx * phi_alternative
  (rtx, int);
static void compute_dominance_frontiers_1
  (sbitmap *frontiers, dominance_info idom, int bb, sbitmap done);
static void find_evaluations_1
  (rtx dest, rtx set, void *data);
static void find_evaluations
  (sbitmap *evals, int nregs);
static void compute_iterated_dominance_frontiers
  (sbitmap *idfs, sbitmap *frontiers, sbitmap *evals, int nregs);
static void insert_phi_node
  (int regno, int b);
static void insert_phi_nodes
  (sbitmap *idfs, sbitmap *evals, int nregs);
static void create_delayed_rename
  (struct rename_context *, rtx *);
static void apply_delayed_renames
  (struct rename_context *);
static int rename_insn_1
  (rtx *ptr, void *data);
static void rename_block
  (int b, dominance_info dom);
static void rename_registers
  (int nregs, dominance_info idom);

static __inline__ int ephi_add_node
  (rtx reg, rtx *nodes, int *n_nodes);
static int * ephi_forward
  (int t, sbitmap visited, sbitmap *succ, int *tstack);
static void ephi_backward
  (int t, sbitmap visited, sbitmap *pred, rtx *nodes);
static void ephi_create
  (int t, sbitmap visited, sbitmap *pred, sbitmap *succ, rtx *nodes);
static void eliminate_phi
  (edge e, partition reg_partition);
...
static int coalesce_if_unconflicting
  (partition p, conflict_graph conflicts, int reg1, int reg2);
static int coalesce_regs_in_copies
  (basic_block bb, partition p, conflict_graph conflicts);
static int coalesce_reg_in_phi
  (rtx, int dest_regno, int src_regno, void *data);
     ...
     rtx reg;
{
  if (!((((((reg)->fld[0]).rtuint)) < 176)))
    return ssa_rename_to_pseudo[(((reg)->fld[0]).rtuint) - 176];
  else
    return ssa_rename_to_hard[(((reg)->fld[0]).rtuint)][((enum machine_mode) (reg)->mode)];
}
...
{
  if (!((((((reg)->fld[0]).rtuint)) < 176)))
    ssa_rename_to_pseudo[(((reg)->fld[0]).rtuint) - 176] = r;
  else
    ssa_rename_to_hard[(((reg)->fld[0]).rtuint)][((enum machine_mode) (reg)->mode)] = r;
}
     ...
     int reg;
{
  ...
  return (answer == 0 ? (rtx) 0 : answer->original);
}
     ...
     unsigned int regno;
{
  ...
  return original_rtx != (rtx) 0 ? (((original_rtx)->fld[0]).rtuint) : regno;
}
...
ssa_rename_from_print ()
{
  ...
  htab_traverse (ssa_rename_from_ht, &ssa_rename_from_print_1, ((void *)0));
}
...
phi_alternative (set, c)
     ...
     int c;
{
  rtvec phi_vec = ((((((set)->fld[1]).rtx))->fld[0]).rtvec);
  int v;

  for (v = ((phi_vec)->num_elem) - 2; v >= 0; v -= 2)
    if ((((((phi_vec)->elem[v + 1]))->fld[0]).rtwint) == c)
      return &((phi_vec)->elem[v]);
     ...
     basic_block block;
{
 ...
 if (v < num_elem - 2)
   {
     ((phi_vec)->elem[v]) = ((phi_vec)->elem[num_elem - 2]);
     ((phi_vec)->elem[v + 1]) = ((phi_vec)->elem[num_elem - 1]);
   }
 ((phi_vec)->num_elem = (num_elem - 2));
 return 1;
      }
     ...
     void *data __attribute__ ((__unused__));
{
  if (((enum rtx_code) (dest)->code) == REG
      && ((!(((((dest)->fld[0]).rtuint)) < 176)) || ((0))))
    ((fe_evals[(((dest)->fld[0]).rtuint)])->elms [(fe_current_bb) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (fe_current_bb) % ((unsigned) (8 * 8)));
}
     ...
     int nregs;
{
      ...
      while (1)
 {
   if (((rtx_class[(int) (((enum rtx_code) (p)->code))]) == 'i'))
     note_stores ((((p)->fld[5]).rtx), find_evaluations_1, ((void *)0));
     ...
     sbitmap done;
{
  ...
  ((done)->elms [(bb) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (bb) % ((unsigned) (8 * 8)));
  sbitmap_zero (frontiers[bb]);




  for (c = (&entry_exit_blocks[0])->next_bb; c != (&entry_exit_blocks[1]); c = c->next_bb)
    if (get_immediate_dominator (idom, c)->index == bb
 && ! ((done)->elms [(c->index) / ((unsigned) (8 * 8))] >> (c->index) % ((unsigned) (8 * 8)) & 1))
 ...
 ((frontiers[bb])->elms [(e->dest->index) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (e->dest->index) % ((unsigned) (8 * 8)));
    }
  ...
  for (c = (&entry_exit_blocks[0])->next_bb; c != (&entry_exit_blocks[1]); c = c->next_bb)
    if (get_immediate_dominator (idom, c)->index == bb)
      {
 int x;
 do { unsigned int word_num_; unsigned int bit_num_ = (0) % (unsigned int) ((unsigned) (8 * 8)); unsigned int size_ = (frontiers[c->index])->size; unsigned long long *ptr_ = (frontiers[c->index])->elms; for (word_num_ = (0) / (unsigned int) ((unsigned) (8 * 8)); word_num_ < size_; word_num_++, bit_num_ = 0) { unsigned long long word_ = ptr_[word_num_]; if (word_ != 0) for (; bit_num_ < ((unsigned) (8 * 8)); bit_num_++) { unsigned long long _mask = (unsigned long long) 1 << bit_num_; if ((word_ & _mask) != 0) { word_ &= ~ _mask; (x) = word_num_ * ((unsigned) (8 * 8)) + bit_num_; { if (get_immediate_dominator (idom, (((basic_block_info)->data.bb[(x)])))->index != bb) ((frontiers[bb])->elms [(x) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (x) % ((unsigned) (8 * 8))); }; if (word_ == 0) break; } } } } while (0);
     ...
     int nregs;
{
      ...
      do
 {
   changed = 0;
   passes++;
   do { unsigned int word_num_; unsigned int bit_num_ = (0) % (unsigned int) ((unsigned) (8 * 8)); unsigned int size_ = (worklist)->size; unsigned long long *ptr_ = (worklist)->elms; for (word_num_ = (0) / (unsigned int) ((unsigned) (8 * 8)); word_num_ < size_; word_num_++, bit_num_ = 0) { unsigned long long word_ = ptr_[word_num_]; if (word_ != 0) for (; bit_num_ < ((unsigned) (8 * 8)); bit_num_++) { unsigned long long _mask = (unsigned long long) 1 << bit_num_; if ((word_ & _mask) != 0) { word_ &= ~ _mask; (b) = word_num_ * ((unsigned) (8 * 8)) + bit_num_; { ((worklist)->elms [(b) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (b) % ((unsigned) (8 * 8)))); sbitmap_union_of_diff (worklist, worklist, frontiers[b], idf); sbitmap_a_or_b (idf, idf, frontiers[b]); changed = 1; }; if (word_ == 0) break; } } } } while (0);
# 623 "../../../kgccfe/gnu/ssa.c"
 }
      while (changed);
    }
     ...
     int regno, bb;
{
  ...
  for (e = b->pred, i = 0; e ; e = e->pred_next, i += 2)
    if (e->src != (&entry_exit_blocks[0]))
      {
 ((vec)->elem[i + 0]) = (global_rtl[GR_PC]);
 ((vec)->elem[i + 1]) = gen_rtx_CONST_INT (VOIDmode, (long long) (e->src->index));
      }

  phi = gen_rtx_fmt_E (PHI, (VOIDmode), (vec));
  phi = gen_rtx_fmt_ee (SET, (VOIDmode), (reg), (phi));

  insn = first_insn_after_basic_block_note (b);
  end_p = (((insn)->fld[1]).rtx) == b->end;
  emit_insn_before (phi, insn);
  if (end_p)
    b->end = (((insn)->fld[1]).rtx);
}

static void
insert_phi_nodes (idfs, evals, nregs)
     sbitmap *idfs;
     sbitmap *evals __attribute__ ((__unused__));
     int nregs;
{
  int reg;
  ...
  for (reg = 0; reg < nregs; ++reg)
    ...
    {
      int b;
      do { unsigned int word_num_; unsigned int bit_num_ = (0) % (unsigned int) ((unsigned) (8 * 8)); unsigned int size_ = (idfs[reg])->size; unsigned long long *ptr_ = (idfs[reg])->elms; for (word_num_ = (0) / (unsigned int) ((unsigned) (8 * 8)); word_num_ < size_; word_num_++, bit_num_ = 0) { unsigned long long word_ = ptr_[word_num_]; if (word_ != 0) for (; bit_num_ < ((unsigned) (8 * 8)); bit_num_++) { unsigned long long _mask = (unsigned long long) 1 << bit_num_; if ((word_ & _mask) != 0) { word_ &= ~ _mask; (b) = word_num_ * ((unsigned) (8 * 8)) + bit_num_; { if (bitmap_bit_p ((((basic_block_info)->data.bb[(b)]))->global_live_at_start, reg)) insert_phi_node (reg, b); }; if (word_ == 0) break; } } } } while (0);
     ...
     rtx *reg_loc;
{
  ...
  if (((enum rtx_code) (*reg_loc)->code) != REG
      || !((!(((((*reg_loc)->fld[0]).rtuint)) < 176)) || ((0))))
    fancy_abort ("../../../kgccfe/gnu/ssa.c", 749, __FUNCTION__);
...
# 771 "../../../kgccfe/gnu/ssa.c"
static void
apply_delayed_renames (c)
       struct rename_context *c;
{
  struct rename_set_data *r;
  struct rename_set_data *last_r = ((void *)0);

  for (r = c->new_renames; r != ((void *)0); r = r->next)
    {
      int new_regno;
 ...
 fancy_abort ("../../../kgccfe/gnu/ssa.c", 785, __FUNCTION__);



      if (r->prev_reg == (rtx) 0 && !((((((r->old_reg)->fld[0]).rtuint)) < 176)))
 {
   r->new_reg = r->old_reg;

   r->prev_reg = (global_rtl[GR_PC]);
 }
      else
 r->new_reg = gen_reg_rtx (((enum machine_mode) (r->old_reg)->mode));
      new_regno = (((r->new_reg)->fld[0]).rtuint);
      ...
      if (new_regno >= (int) ssa_definition->num_elements)
 {
   ...
   ((ssa_definition) = varray_grow (ssa_definition, new_limit));
      ...
      ((ssa_definition)->data.rtx[new_regno]) = r->set_insn;
     ...
     void *data;
{
 ...
 if (((enum rtx_code) (dest)->code) == SUBREG
  ...
  > (mode_size[(int) (((enum machine_mode) ((((dest)->fld[0]).rtx))->mode))]))
     ...
     && ((!((((((((dest)->fld[0]).rtx))->fld[0]).rtuint)) < 176)) || ((0))))
   {
     destp = &(((dest)->fld[0]).rtx);
     dest = (((dest)->fld[0]).rtx);
   }
# 869 "../../../kgccfe/gnu/ssa.c"
 if (((enum rtx_code) (dest)->code) == STRICT_LOW_PART
  ...
  context->new_renames = ((void *)0);
  i = emit_insn (gen_rtx_fmt_ee (SET, (VOIDmode), (reg), (reg)));
  ...
  context->new_renames = saved_new_renames;
   ...
   && ((!(((((dest)->fld[0]).rtuint)) < 176)) || ((0))))
   {
     ...
     if (((enum rtx_code) (x)->code) == SET)
       for_each_rtx (&(((x)->fld[1]).rtx), rename_insn_1, data);
     return -1;
   }
 ...
 return 0;
    ...
    case REG:
      if (((!(((((x)->fld[0]).rtuint)) < 176)) || ((0)))
   && (((x)->fld[0]).rtuint) < ssa_max_reg_num)
 {
   rtx new_reg = ssa_rename_to_lookup (x);

   if (new_reg != (global_rtl[GR_PC]) && new_reg != (rtx) 0)
     {
       if (((enum machine_mode) (x)->mode) != ((enum machine_mode) (new_reg)->mode))
  fancy_abort ("../../../kgccfe/gnu/ssa.c", 930, __FUNCTION__);
       *ptr = new_reg;
     }
   else
     {
       ...
       *ptr = gen_reg_rtx (((enum machine_mode) (x)->mode));
     }
 }
      return -1;
 ...
 if ((((enum rtx_code) (dest)->code) == REG))
   {
     if (((!(((((dest)->fld[0]).rtuint)) < 176)) || ((0)))
  && (((dest)->fld[0]).rtuint) < ssa_max_reg_num)
       {
  rtx new_reg = ssa_rename_to_lookup (dest);
  if (new_reg != (rtx) 0 && new_reg != (global_rtl[GR_PC]))
      (((x)->fld[0]).rtx) = new_reg;
       }
...
gen_sequence ()
{
  ...
  result = gen_rtx_fmt_E (SEQUENCE, (VOIDmode), (rtvec_alloc (len)));

  for (i = 0, tem = first_insn; tem; tem = (((tem)->fld[2]).rtx), i++)
    (((((result)->fld[0]).rtvec))->elem[i]) = tem;
     ...
     dominance_info idom;
{
  ...
  edge e;
  rtx insn, next, last;
  struct rename_set_data *set_data = ((void *)0);
  basic_block c;
      ...
      if (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i'))
 {
   ...
   start_sequence ();
   for_each_rtx (&(((insn)->fld[5]).rtx), rename_insn_1, &context);
   for_each_rtx (&(((insn)->fld[8]).rtx), rename_insn_1, &context);
   ...
   if (get_insns () != (rtx) 0)
     {
       ...
       for (i = 0; i < (((((seq)->fld[0]).rtvec))->num_elem); i++)
  (((((seq)->fld[0]).rtvec))->elem[i]) = ((((((((seq)->fld[0]).rtvec))->elem[i]))->fld[5]).rtx);
       (((insn)->fld[5]).rtx) = seq;
     }
   end_sequence ();
 ...
 continue;
      ...
      while (((insn) && ((enum rtx_code) (insn)->code) == INSN && ((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == SET && ((enum rtx_code) (((((((insn)->fld[5]).rtx))->fld[1]).rtx))->code) == PHI))
 {
   rtx phi = (((insn)->fld[5]).rtx);
   rtx reg;
   ...
   reg = (((phi)->fld[0]).rtx);
   if ((((reg)->fld[0]).rtuint) >= ssa_max_reg_num)
     reg = ssa_rename_from_lookup ((((reg)->fld[0]).rtuint));
   if (reg == (rtx) 0)
     fancy_abort ("../../../kgccfe/gnu/ssa.c", 1080, __FUNCTION__);
   reg = ssa_rename_to_lookup (reg);




   if (reg == ((void *)0) || reg == (global_rtl[GR_PC]))
     {
       if (! remove_phi_alternative (phi, b))
  fancy_abort ("../../../kgccfe/gnu/ssa.c", 1089, __FUNCTION__);
     }
   else
     {
       ...
       if (((enum machine_mode) ((((phi)->fld[0]).rtx))->mode) == VOIDmode)
  (((((phi)->fld[0]).rtx))->mode = (enum machine_mode) (((enum machine_mode) (reg)->mode)));
       else if (((enum machine_mode) ((((phi)->fld[0]).rtx))->mode) != ((enum machine_mode) (reg)->mode))
  fancy_abort ("../../../kgccfe/gnu/ssa.c", 1099, __FUNCTION__);
 ...
 fancy_abort ("../../../kgccfe/gnu/ssa.c", 1124, __FUNCTION__);
...
rename_registers (nregs, idom)
     ...
     dominance_info idom;
{
  ssa_definition = varray_init (nregs * 3, VARRAY_DATA_RTX, "ssa_definition");
  ssa_rename_from_initialize ();

  ssa_rename_to_pseudo = (rtx *) __builtin_alloca(nregs * sizeof(rtx));
  memset ((char *) ssa_rename_to_pseudo, 0, nregs * sizeof(rtx));
  memset ((char *) ssa_rename_to_hard, 0,
  176 * (int) MAX_MACHINE_MODE * sizeof (rtx));
...
convert_to_ssa ()
{
  ...
  if (in_ssa_form)
    fancy_abort ("../../../kgccfe/gnu/ssa.c", 1177, __FUNCTION__);



  life_analysis (get_insns (), ((void *)0), 0);

  idom = calculate_dominance_info (CDI_DOMINATORS);

  if (rtl_dump_file)
    {
      fputs_unlocked (";; Immediate Dominators:\n", rtl_dump_file);
     ...
     int *n_nodes;
{
  ...
  for (i = *n_nodes - 1; i >= 0; --i)
    if ((((reg)->fld[0]).rtuint) == (((nodes[i])->fld[0]).rtuint))
      return i;
     ...
     int *tstack;
{
  ...
  ((visited)->elms [(t) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (t) % ((unsigned) (8 * 8)));

  do { unsigned int word_num_; unsigned int bit_num_ = (0) % (unsigned int) ((unsigned) (8 * 8)); unsigned int size_ = (succ[t])->size; unsigned long long *ptr_ = (succ[t])->elms; for (word_num_ = (0) / (unsigned int) ((unsigned) (8 * 8)); word_num_ < size_; word_num_++, bit_num_ = 0) { unsigned long long word_ = ptr_[word_num_]; if (word_ != 0) for (; bit_num_ < ((unsigned) (8 * 8)); bit_num_++) { unsigned long long _mask = (unsigned long long) 1 << bit_num_; if ((word_ & _mask) != 0) { word_ &= ~ _mask; (s) = word_num_ * ((unsigned) (8 * 8)) + bit_num_; { if (! ((visited)->elms [(s) / ((unsigned) (8 * 8))] >> (s) % ((unsigned) (8 * 8)) & 1)) tstack = ephi_forward (s, visited, succ, tstack); }; if (word_ == 0) break; } } } } while (0);
     ...
     rtx *nodes;
{
  ...
  do { unsigned int word_num_; unsigned int bit_num_ = (0) % (unsigned int) ((unsigned) (8 * 8)); unsigned int size_ = (pred[t])->size; unsigned long long *ptr_ = (pred[t])->elms; for (word_num_ = (0) / (unsigned int) ((unsigned) (8 * 8)); word_num_ < size_; word_num_++, bit_num_ = 0) { unsigned long long word_ = ptr_[word_num_]; if (word_ != 0) for (; bit_num_ < ((unsigned) (8 * 8)); bit_num_++) { unsigned long long _mask = (unsigned long long) 1 << bit_num_; if ((word_ & _mask) != 0) { word_ &= ~ _mask; (p) = word_num_ * ((unsigned) (8 * 8)) + bit_num_; { if (! ((visited)->elms [(p) / ((unsigned) (8 * 8))] >> (p) % ((unsigned) (8 * 8)) & 1)) { ephi_backward (p, visited, pred, nodes); emit_move_insn (nodes[p], nodes[t]); } }; if (word_ == 0) break; } } } } while (0);
     ...
     rtx *nodes;
{
  ...
  do { unsigned int word_num_; unsigned int bit_num_ = (0) % (unsigned int) ((unsigned) (8 * 8)); unsigned int size_ = (pred[t])->size; unsigned long long *ptr_ = (pred[t])->elms; for (word_num_ = (0) / (unsigned int) ((unsigned) (8 * 8)); word_num_ < size_; word_num_++, bit_num_ = 0) { unsigned long long word_ = ptr_[word_num_]; if (word_ != 0) for (; bit_num_ < ((unsigned) (8 * 8)); bit_num_++) { unsigned long long _mask = (unsigned long long) 1 << bit_num_; if ((word_ & _mask) != 0) { word_ &= ~ _mask; (p) = word_num_ * ((unsigned) (8 * 8)) + bit_num_; { if (! ((visited)->elms [(p) / ((unsigned) (8 * 8))] >> (p) % ((unsigned) (8 * 8)) & 1)) unvisited_predecessors = 1; else if (!reg_u) reg_u = nodes[p]; }; if (word_ == 0) break; } } } } while (0);
  ...
  if (unvisited_predecessors)
    {
      ...
      if (!reg_u)
 {
   reg_u = gen_reg_rtx (((enum machine_mode) (nodes[t])->mode));
   emit_move_insn (reg_u, nodes[t]);
 }

      do { unsigned int word_num_; unsigned int bit_num_ = (0) % (unsigned int) ((unsigned) (8 * 8)); unsigned int size_ = (pred[t])->size; unsigned long long *ptr_ = (pred[t])->elms; for (word_num_ = (0) / (unsigned int) ((unsigned) (8 * 8)); word_num_ < size_; word_num_++, bit_num_ = 0) { unsigned long long word_ = ptr_[word_num_]; if (word_ != 0) for (; bit_num_ < ((unsigned) (8 * 8)); bit_num_++) { unsigned long long _mask = (unsigned long long) 1 << bit_num_; if ((word_ & _mask) != 0) { word_ &= ~ _mask; (p) = word_num_ * ((unsigned) (8 * 8)) + bit_num_; { if (! ((visited)->elms [(p) / ((unsigned) (8 * 8))] >> (p) % ((unsigned) (8 * 8)) & 1)) { ephi_backward (p, visited, pred, nodes); emit_move_insn (nodes[p], reg_u); } }; if (word_ == 0) break; } } } } while (0);
  ...
  else
    {
      ...
      int s;
      do { unsigned int word_num_; unsigned int bit_num_ = (0) % (unsigned int) ((unsigned) (8 * 8)); unsigned int size_ = (succ[t])->size; unsigned long long *ptr_ = (succ[t])->elms; for (word_num_ = (0) / (unsigned int) ((unsigned) (8 * 8)); word_num_ < size_; word_num_++, bit_num_ = 0) { unsigned long long word_ = ptr_[word_num_]; if (word_ != 0) for (; bit_num_ < ((unsigned) (8 * 8)); bit_num_++) { unsigned long long _mask = (unsigned long long) 1 << bit_num_; if ((word_ & _mask) != 0) { word_ &= ~ _mask; (s) = word_num_ * ((unsigned) (8 * 8)) + bit_num_; { ((visited)->elms [(t) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (t) % ((unsigned) (8 * 8))); emit_move_insn (nodes[t], nodes[s]); return; }; if (word_ == 0) break; } } } } while (0);
     ...
     partition reg_partition;
{
  ...
  nodes = (rtx *) __builtin_alloca(n_nodes * sizeof(rtx));
  ...
  n_nodes = 0;
  for (; ((insn) && ((enum rtx_code) (insn)->code) == INSN && ((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == SET && ((enum rtx_code) (((((((insn)->fld[5]).rtx))->fld[1]).rtx))->code) == PHI); insn = next_nonnote_insn (insn))
    {
      rtx* preg = phi_alternative ((((insn)->fld[5]).rtx), e->src->index);
      rtx tgt = ((((((insn)->fld[5]).rtx))->fld[0]).rtx);
      rtx reg;
 ...
 continue;
      ...
      if (((enum rtx_code) (reg)->code) != REG || ((enum rtx_code) (tgt)->code) != REG)
 fancy_abort ("../../../kgccfe/gnu/ssa.c", 1429, __FUNCTION__);

      reg = (cfun->emit->x_regno_reg_rtx)[((reg_partition)->elements[((((reg)->fld[0]).rtuint))].class_element)];
      tgt = (cfun->emit->x_regno_reg_rtx)[((reg_partition)->elements[((((tgt)->fld[0]).rtuint))].class_element)];
      ...
      if (reg != tgt)
 {
   ...
   ((pred[ireg])->elms [(itgt) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (itgt) % ((unsigned) (8 * 8)));
   ((succ[itgt])->elms [(ireg) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (ireg) % ((unsigned) (8 * 8)));
 }
    }
  ...
  tstack = stack = (int *) __builtin_alloca(n_nodes * sizeof (int));

  for (i = 0; i < n_nodes; ++i)
    if (! ((visited)->elms [(i) / ((unsigned) (8 * 8))] >> (i) % ((unsigned) (8 * 8)) & 1))
     ...
     partition reg_partition;
{
  ...
  for (;
       ((phi) && ((enum rtx_code) (phi)->code) == INSN && ((enum rtx_code) ((((phi)->fld[5]).rtx))->code) == SET && ((enum rtx_code) (((((((phi)->fld[5]).rtx))->fld[1]).rtx))->code) == PHI);
       phi = next_nonnote_insn (phi))
    {
      edge e;
      int tgt_regno;
      rtx set = (((phi)->fld[5]).rtx);
      rtx tgt = (((set)->fld[0]).rtx);


      if (((enum rtx_code) (tgt)->code) != REG
   || !((!(((((tgt)->fld[0]).rtuint)) < 176)) || ((0))))
 fancy_abort ("../../../kgccfe/gnu/ssa.c", 1527, __FUNCTION__);
      tgt_regno = (((tgt)->fld[0]).rtuint);
 ...
 if ((e->flags & 2) && ((e)->src->succ->succ_next && (e)->dest->pred->pred_next))
   {
     ...
     if (((enum rtx_code) (*alt)->code) != REG
  || !((!(((((*alt)->fld[0]).rtuint)) < 176)) || ((0))))
       fancy_abort ("../../../kgccfe/gnu/ssa.c", 1545, __FUNCTION__);
     alt_regno = (((*alt)->fld[0]).rtuint);



     if (((reg_partition)->elements[(tgt_regno)].class_element)
  != ((reg_partition)->elements[(alt_regno)].class_element))
       {

  if (conflicting_hard_regs_p (tgt_regno, alt_regno))


    fancy_abort ("../../../kgccfe/gnu/ssa.c", 1557, __FUNCTION__);
     ...
     partition reg_partition;
{
  ...
  for (;
       ...
       phi = next_nonnote_insn (phi))
    {
      ...
      int tgt_regno = (((((((((phi)->fld[5]).rtx))->fld[0]).rtx))->fld[0]).rtuint);

      rtx phi2 = next_nonnote_insn (phi);


      for (;
    ((phi2) && ((enum rtx_code) (phi2)->code) == INSN && ((enum rtx_code) ((((phi2)->fld[5]).rtx))->code) == SET && ((enum rtx_code) (((((((phi2)->fld[5]).rtx))->fld[1]).rtx))->code) == PHI);
    phi2 = next_nonnote_insn (phi2))
 {
   rtx set2 = (((phi2)->fld[5]).rtx);

   int tgt2_regno = ((((((set2)->fld[0]).rtx))->fld[0]).rtuint);


   if (((reg_partition)->elements[(tgt_regno)].class_element) ==
       ((reg_partition)->elements[(tgt2_regno)].class_element))
     {
       edge e;
       ...
       for (e = b->pred; e; e = e->pred_next)
  {
    ...
    if (((enum rtx_code) (*alt)->code) != REG
        || !((!(((((*alt)->fld[0]).rtuint)) < 176)) || ((0))))
      fancy_abort ("../../../kgccfe/gnu/ssa.c", 1629, __FUNCTION__);
    if (((enum rtx_code) (*alt2)->code) != REG
        || !((!(((((*alt2)->fld[0]).rtuint)) < 176)) || ((0))))
      fancy_abort ("../../../kgccfe/gnu/ssa.c", 1632, __FUNCTION__);



    if (((reg_partition)->elements[((((*alt)->fld[0]).rtuint))].class_element)
        != ((reg_partition)->elements[((((*alt2)->fld[0]).rtuint))].class_element))
      {

        if (conflicting_hard_regs_p ((((*alt)->fld[0]).rtuint), (((*alt2)->fld[0]).rtuint)))


   fancy_abort ("../../../kgccfe/gnu/ssa.c", 1643, __FUNCTION__);

        partition_union (reg_partition,
           (((*alt)->fld[0]).rtuint), (((*alt2)->fld[0]).rtuint));
        ++changed;
      }
  }
     }
     ...
     int reg2;
{
  ...
  if (!((!((reg1) < 176)) || ((0))) || !((!((reg2) < 176)) || ((0))))
    return 0;



  reg1 = ((p)->elements[(reg1)].class_element);
  reg2 = ((p)->elements[(reg2)].class_element);
  ...
  reg = ((p)->elements[(reg1)].class_element);
     ...
     conflict_graph conflicts;
{
  ...
  for (insn = bb->head; insn != end; insn = (((insn)->fld[2]).rtx))
    {
      rtx pattern;
      rtx src;
      rtx dest;
 ...
 continue;
      ...
      if (((enum machine_mode) (src)->mode) != ((enum machine_mode) (dest)->mode))
 continue;
      ...
      changed += coalesce_if_unconflicting (p, conflicts,
         (((src)->fld[0]).rtuint), (((dest)->fld[0]).rtuint));
    }
...
compute_coalesced_reg_partition ()
{
      ...
      for (bb = (&entry_exit_blocks[1])->prev_bb; bb != (&entry_exit_blocks[0]); bb = bb->prev_bb)
 {
  ...
  do { if (phi_set) { bitmap_clear (phi_set); (phi_set) = 0; } } while (0);
     ...
     void *data;
{
  ...
  switch (((enum rtx_code) (expr)->code))
    {
    case REG:
      bitmap_set_bit (set, (((expr)->fld[0]).rtuint));

    case CONST_INT:
    case PHI:
      return 0;
    default:
      fancy_abort ("../../../kgccfe/gnu/ssa.c", 1954, __FUNCTION__);
    }
}
     ...
     regset phi_set;
{
  ...
  for (reg = 0; reg < ((ssa_definition)->num_elements); ++reg)
    if (((!((reg) < 176)) || ((0))))
      {
 rtx insn = ((ssa_definition)->data.rtx[reg]);
 rtx pattern;
 rtx src;
 ...
 if (((enum rtx_code) (src)->code) == REG)
   {
     ...
     bitmap_set_bit (phi_set, reg);
     bitmap_set_bit (phi_set, (((src)->fld[0]).rtuint));
   }
 else if (((enum rtx_code) (src)->code) == PHI)
   {
     ...
     void* data;
{
      ...
      if (((!(((((x)->fld[0]).rtuint)) < 176)) || ((0))))
 {
   ...
   unsigned int new_regno = ((reg_partition)->elements[(regno)].class_element);
   rtx canonical_element_rtx = ssa_rename_from_lookup (new_regno);

   if (canonical_element_rtx != (rtx) 0 &&
       ((((((canonical_element_rtx)->fld[0]).rtuint)) < 176)))
     {
       if ((((canonical_element_rtx)->fld[0]).rtuint) != regno)
  *ptr = canonical_element_rtx;
     }
   else if (regno != new_regno)
     {
       rtx new_reg = (cfun->emit->x_regno_reg_rtx)[new_regno];
       if (((enum machine_mode) (x)->mode) != ((enum machine_mode) (new_reg)->mode))
  fancy_abort ("../../../kgccfe/gnu/ssa.c", 2038, __FUNCTION__);
       *ptr = new_reg;
     }
 }
      return -1;
     ...
     void *data;
{
  ...
  ((canonical_elements)->elms [(((reg_partition)->elements[(reg)].class_element)) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (((reg_partition)->elements[(reg)].class_element)) % ((unsigned) (8 * 8)));
  return 1;
}
     ...
     partition reg_partition;
{
  ...
  sbitmap canonical_elements;
  int element_index;
  int already_seen[176][(int) MAX_MACHINE_MODE];
  int reg;
  int mach_mode;
  ...
  for (reg = 0; reg < 176; ++reg)
    for (mach_mode = 0; mach_mode < (int) MAX_MACHINE_MODE; ++mach_mode)
      already_seen[reg][mach_mode] = 0;


  do { unsigned int word_num_; unsigned int bit_num_ = (0) % (unsigned int) ((unsigned) (8 * 8)); unsigned int size_ = (canonical_elements)->size; unsigned long long *ptr_ = (canonical_elements)->elms; for (word_num_ = (0) / (unsigned int) ((unsigned) (8 * 8)); word_num_ < size_; word_num_++, bit_num_ = 0) { unsigned long long word_ = ptr_[word_num_]; if (word_ != 0) for (; bit_num_ < ((unsigned) (8 * 8)); bit_num_++) { unsigned long long _mask = (unsigned long long) 1 << bit_num_; if ((word_ & _mask) != 0) { word_ &= ~ _mask; (element_index) = word_num_ * ((unsigned) (8 * 8)) + bit_num_; { rtx hard_reg_rtx = ssa_rename_from_lookup (element_index); if (hard_reg_rtx != (rtx) 0 && ((((((hard_reg_rtx)->fld[0]).rtuint)) < 176)) && already_seen[(((hard_reg_rtx)->fld[0]).rtuint)][((enum machine_mode) (hard_reg_rtx)->mode)] != 0) return 0; }; if (word_ == 0) break; } } } } while (0);
# 2115 "../../../kgccfe/gnu/ssa.c"
  free(canonical_elements);
     ...
     partition reg_partition;
{
  ...
  for (b = (&entry_exit_blocks[1])->prev_bb; b != (&entry_exit_blocks[0]); b = b->prev_bb)
    {
      rtx next = b->head;
      rtx last = b->end;
      rtx insn;
      ...
      do
 {
   ...
   if (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i'))
     {
       for_each_rtx (&(((insn)->fld[5]).rtx),
       rename_equivalent_regs_in_insn,
       reg_partition);
       for_each_rtx (&(((insn)->fld[8]).rtx),
       rename_equivalent_regs_in_insn,
       reg_partition);

       if (((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == SEQUENCE)
  {
    rtx s = (((insn)->fld[5]).rtx);
    int slen = (((((s)->fld[0]).rtvec))->num_elem);
    int i;

    if (slen <= 1)
      fancy_abort ("../../../kgccfe/gnu/ssa.c", 2154, __FUNCTION__);

    (((insn)->fld[5]).rtx) = (((((s)->fld[0]).rtvec))->elem[slen-1]);
    for (i = 0; i < slen - 1; i++)
      emit_insn_before ((((((s)->fld[0]).rtvec))->elem[i]), insn);
  }
     }
...
convert_from_ssa ()
{
  ...
  if (!check_hard_regs_in_partition (reg_partition))
    ...
    fancy_abort ("../../../kgccfe/gnu/ssa.c", 2194, __FUNCTION__);
      ...
      while (1)
 {
   ...
   if (((insn) && ((enum rtx_code) (insn)->code) == INSN && ((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == SET && ((enum rtx_code) (((((((insn)->fld[5]).rtx))->fld[1]).rtx))->code) == PHI))
     {
       if (insn == bb->end)
  bb->end = (((insn)->fld[1]).rtx);
     ...
     void *data;
{
  ...
  for (e = bb->succ; e != ((void *)0); e = e->succ_next)
    {
      ...
      if (successor == (&entry_exit_blocks[0])
   || successor == (&entry_exit_blocks[1]))
 continue;
 ...
 continue;
      ...
      for ( ; ((insn) && ((enum rtx_code) (insn)->code) == INSN && ((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == SET && ((enum rtx_code) (((((((insn)->fld[5]).rtx))->fld[1]).rtx))->code) == PHI); insn = (((insn)->fld[2]).rtx))
 {
   int result;
   rtx phi_set = (((insn)->fld[5]).rtx);
   ...
   result = (*fn) (insn, ((((((phi_set)->fld[0]).rtx))->fld[0]).rtuint),
     (((phi_src)->fld[0]).rtuint), data);
     ...
     int reg2;
{
  ...
  if (((orig_reg1) < 176) && ((orig_reg2) < 176)
      && orig_reg1 != orig_reg2)
    return 1;
  if (((orig_reg1) < 176) && !((orig_reg2) < 176))
    return 1;
