...
# 156 "../../../kgccfe/gnu/gcse.c" 2
# 1 "../../../kgccfe/gnu/insn-config.h" 1
# 157 "../../../kgccfe/gnu/gcse.c" 2
...
# 164 "../../../kgccfe/gnu/gcse.c" 2
# 1 "../../../kgccfe/gnu/params.h" 1
# 44 "../../../kgccfe/gnu/params.h"
...
# 1 "../../../kgccfe/gnu/params.def" 1
# 51 "../../../kgccfe/gnu/params.def"
PARAM_MAX_INLINE_INSNS_SINGLE,
# 63 "../../../kgccfe/gnu/params.def"
PARAM_MAX_INLINE_INSNS_AUTO,
# 82 "../../../kgccfe/gnu/params.def"
PARAM_MAX_INLINE_INSNS,
# 95 "../../../kgccfe/gnu/params.def"
PARAM_MAX_INLINE_SLOPE,
# 108 "../../../kgccfe/gnu/params.def"
PARAM_MIN_INLINE_INSNS,
# 118 "../../../kgccfe/gnu/params.def"
PARAM_MAX_INLINE_INSNS_RTL,
# 129 "../../../kgccfe/gnu/params.def"
PARAM_MAX_DELAY_SLOT_INSN_SEARCH,
# 140 "../../../kgccfe/gnu/params.def"
PARAM_MAX_DELAY_SLOT_LIVE_SEARCH,
# 150 "../../../kgccfe/gnu/params.def"
...
GGC_MIN_HEAPSIZE,
# 78 "../../../kgccfe/gnu/params.h" 2

  LAST_PARAM
} compiler_param;
# 165 "../../../kgccfe/gnu/gcse.c" 2
...
# 168 "../../../kgccfe/gnu/gcse.c" 2
# 279 "../../../kgccfe/gnu/gcse.c"
static FILE *gcse_file;
# 288 "../../../kgccfe/gnu/gcse.c"
...
static char can_copy_p[(int) (int) MAX_MACHINE_MODE];
...
static void insert_expr_in_table (rtx, enum machine_mode, rtx, int, int, struct hash_table *);
...
static struct expr *lookup_set (unsigned int, rtx, struct hash_table *);
...
static void compute_local_properties (sbitmap *, sbitmap *, sbitmap *, struct hash_table *);
...
static _Bool constprop_register (rtx, rtx, rtx, int);
static struct expr *find_bypass_set (int, int);
static _Bool reg_killed_on_edge (rtx, edge);
static int bypass_block (basic_block, rtx, rtx);
static int bypass_conditional_jumps (void);
static void alloc_pre_mem (int, int);
static void free_pre_mem (void);
static void compute_pre_data (void);
static int pre_expr_reaches_here_p (basic_block, struct expr *, basic_block);
...
static int expr_reaches_here_p (struct occr *, struct expr *, basic_block, int);
...
static int delete_null_pointer_checks_1 (unsigned int *, sbitmap *, sbitmap *, struct null_pointer_info *);


static rtx process_insert_insn (struct expr *);
static int pre_edge_insert (struct edge_list *, struct expr **);
static int expr_reaches_here_p_work (struct occr *, struct expr *, basic_block, int, char *);

static int pre_expr_reaches_here_p_work (basic_block, struct expr *, basic_block, char *);
...
static __inline__ struct ls_expr * first_ls_expr (void);
static __inline__ struct ls_expr * next_ls_expr (struct ls_expr *);
...
static int store_ops_ok (rtx, basic_block);
static void find_moveable_store (rtx);
static int compute_store_table (void);
static int load_kills_store (rtx, rtx);
static int find_loads (rtx, rtx);
static int store_killed_in_insn (rtx, rtx);
static int store_killed_after (rtx, rtx, basic_block);
static int store_killed_before (rtx, rtx, basic_block);
...
static _Bool do_local_cprop (rtx, rtx, int, rtx*);
static _Bool adjust_libcall_notes (rtx, rtx, rtx, rtx*);
...
# 760 "../../../kgccfe/gnu/gcse.c"
 ...
 warning ("GCSE disabled: %d > 1000 basic blocks and %d >= 20 edges/basic block",
  ...
  if ((n_basic_blocks
       * (((max_gcse_regno) + ((unsigned) (8 * 8)) - 1) / ((unsigned) (8 * 8)))
       * sizeof (unsigned long long)) > ((size_t) (compiler_params[(int) PARAM_MAX_GCSE_MEMORY].value)))
    {
      if (warn_disabled_optimization)
...
# 802 "../../../kgccfe/gnu/gcse.c"
  ...
  changed = 1;
  while (changed && pass < (compiler_params[(int) PARAM_MAX_GCSE_PASSES].value))
    {
      changed = 0;
      if (file)
      ...
      if (file)
 {
   ...
   fflush (file);
      ...
      __extension__ ({ struct obstack *__o = (&gcse_obstack); void *__obj = (gcse_obstack_bottom); if (__obj > (void *)__o->chunk && __obj < (void *)__o->chunk_limit) __o->next_free = __o->object_base = __obj; else (obstack_free) (__o, __obj); });
      pass++;
    }
  ...
  if (file)
    {
      fprintf (file, "GCSE of %s: %d basic blocks, ",
        (cfun->name), n_basic_blocks);
      fprintf (file, "%d pass%s, %d bytes\n\n",
        pass, pass > 1 ? "es" : "", max_pass_bytes);
    }

  __extension__ ({ struct obstack *__o = (&gcse_obstack); void *__obj = (((void *)0)); if (__obj > (void *)__o->chunk && __obj < (void *)__o->chunk_limit) __o->next_free = __o->object_base = __obj; else (obstack_free) (__o, __obj); });
  free_reg_set_mem ();
...
compute_can_copy ()
{
  ...
  memset (can_copy_p, 0, (int) MAX_MACHINE_MODE);

  start_sequence ();
  for (i = 0; i < (int) MAX_MACHINE_MODE; i++)
    if ((mode_class[(int) (i)]) == MODE_CC)
      {
     ...
     rtx f;
{
 ...
 uid_cuid[(((insn)->fld[0]).rtint)] = i;
  ...
  reg_set_bitmap = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
  ...
  memset ((char *) modify_mem_list, 0, last_basic_block * sizeof (rtx));
  memset ((char *) canon_modify_mem_list, 0, last_basic_block * sizeof (rtx));
  modify_mem_list_set = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
  canon_modify_mem_list_set = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
}
     ...
     int y;
{
  ...
  size_t column_size = n * x * sizeof (unsigned long long);



  if (column_size * (((y) + ((unsigned) (8 * 8)) - 1) / ((unsigned) (8 * 8))) <= max_bitmap_memory)
    return y;



  return ((unsigned) (8 * 8)) * ((max_bitmap_memory + column_size - 1)
        / column_size);
}
# 1123 "../../../kgccfe/gnu/gcse.c"
static void
      ...
      for (expr = table->table[i]; expr != ((void *)0); expr = expr->next_same_hash)
 {
   ...
   if (antloc)
     for (occr = expr->antic_occr; occr != ((void *)0); occr = occr->next)
       {
  ((antloc[((((occr->insn)->fld[3]).bb)->index + 0)])->elms [(indx) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (indx) % ((unsigned) (8 * 8)));
  ...
  occr->deleted_p = 0;
   ...
   if (comp)
     for (occr = expr->avail_occr; occr != ((void *)0); occr = occr->next)
       {
  ((comp[((((occr->insn)->fld[3]).bb)->index + 0)])->elms [(indx) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (indx) % ((unsigned) (8 * 8)));
...
{
  free (reg_set_table);
  __extension__ ({ struct obstack *__o = (&reg_set_obstack); void *__obj = (((void *)0)); if (__obj > (void *)__o->chunk && __obj < (void *)__o->chunk_limit) __o->next_free = __o->object_base = __obj; else (obstack_free) (__o, __obj); });
}
     ...
     rtx insn;
{
 ...
 = (struct reg_set **) grealloc ((char *) reg_set_table,
  ...
  new_reg_info = (struct reg_set *) __extension__ ({ struct obstack *__h = (&reg_set_obstack); __extension__ ({ struct obstack *__o = (__h); int __len = ((sizeof (struct reg_set))); if (__o->chunk_limit - __o->next_free < __len) _obstack_newchunk (__o, __len); __o->next_free += __len; (void) 0; }); __extension__ ({ struct obstack *__o1 = (__h); void *value; value = (void *) __o1->object_base; if (__o1->next_free == value) __o1->maybe_empty_object = 1; __o1->next_free = (((((__o1->next_free) - (char *) 0)+__o1->alignment_mask) & ~ (__o1->alignment_mask)) + (char *) 0); if (__o1->next_free - (char *)__o1->chunk > __o1->chunk_limit - (char *)__o1->chunk) __o1->next_free = __o1->chunk_limit; __o1->object_base = __o1->next_free; value; }); });
...
record_set_info (dest, setter, data)
     rtx dest, setter __attribute__ ((__unused__));
     void *data;
{
  rtx record_set_insn = (rtx) data;

  if (((enum rtx_code) (dest)->code) == REG && (((dest)->fld[0]).rtuint) >= 176)
    record_one_set ((((dest)->fld[0]).rtuint), record_set_insn);
}
     ...
     rtx f;
{
  ...
  for (insn = f; insn != 0; insn = (((insn)->fld[2]).rtx))
    if (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i'))
      note_stores ((((insn)->fld[5]).rtx), record_set_info, insn);
}
     ...
     rtx x;
{
 ...
 = make_insn_raw (gen_rtx_fmt_ee (SET, (VOIDmode), (gen_rtx_REG (word_mode, 176 * 2)), ((const_int_rtx[64]))));
      ...
      (((test_insn)->fld[2]).rtx) = (((test_insn)->fld[1]).rtx) = 0;
    }



  ((((((((test_insn)->fld[5]).rtx))->fld[0]).rtx))->mode = (enum machine_mode) (((enum machine_mode) (x)->mode)));
  ((((((test_insn)->fld[5]).rtx))->fld[1]).rtx) = x;
  return ((icode = recog ((((test_insn)->fld[5]).rtx), test_insn, &num_clobbers)) >= 0
     ...
     int avail_p;
{
  ...
  switch (code)
    ...
    case REG:
      {
 struct reg_avail_info *info = &reg_avail_info[(((x)->fld[0]).rtuint)];

 if (info->last_bb != current_bb)
   return 1;
 if (avail_p)
   return info->last_set < (uid_cuid[(((insn)->fld[0]).rtint)]);
 else
   return info->first_set >= (uid_cuid[(((insn)->fld[0]).rtint)]);
      }

    case MEM:
      if (load_killed_in_block_p (current_bb, (uid_cuid[(((insn)->fld[0]).rtint)]),
      x, avail_p))
 return 0;
      else
 return oprs_unchanged_p ((((x)->fld[0]).rtx), insn, avail_p);
      ...
      if (fmt[i] == 'e')
 {
   ...
   if (i == 0)
     return oprs_unchanged_p ((((x)->fld[i]).rtx), insn, avail_p);

   else if (! oprs_unchanged_p ((((x)->fld[i]).rtx), insn, avail_p))
     return 0;
 }
      else if (fmt[i] == 'E')
 for (j = 0; j < (((((x)->fld[i]).rtvec))->num_elem); j++)
   if (! oprs_unchanged_p ((((((x)->fld[i]).rtvec))->elem[j]), insn, avail_p))
     return 0;
    }
     ...
     void *data __attribute__ ((__unused__));
{
  ...
  if (dest == gcse_mem_operand && pre_ldst_mems != ((void *)0))
    {
     ...
     int avail_p;
{
  ...
  while (list_entry)
    {
      ...
      if ((avail_p
    && (uid_cuid[((((((list_entry)->fld[0]).rtx))->fld[0]).rtint)]) < uid_limit)
   || (! avail_p
       && (uid_cuid[((((((list_entry)->fld[0]).rtx))->fld[0]).rtint)]) > uid_limit))
 {
   list_entry = (((list_entry)->fld[1]).rtx);
   continue;
 }

      setter = (((list_entry)->fld[0]).rtx);
 ...
 return 1;
      ...
      note_stores ((((setter)->fld[5]).rtx), mems_conflict_for_gcse_p, ((void *)0));
     ...
     int *do_not_record_p;
{
 ...
 repeat:
  ...
  switch (code)
    {
    case REG:
      hash += ((unsigned int) REG << 7) + (((x)->fld[0]).rtuint);
      return hash;

    case CONST_INT:
      hash += (((unsigned int) CONST_INT << 7) + (unsigned int) mode
        + (unsigned int) (((x)->fld[0]).rtwint));
      return hash;
 ...
 for (i = 2; i < (rtx_length[(int) (CONST_DOUBLE)]); i++)
   hash += (unsigned int) (((x)->fld[i]).rtwint);
      else
 hash += ((unsigned int) (((x)->fld[0]).rtwint)
   + (unsigned int) (((x)->fld[1]).rtwint));
      return hash;
 ...
 unsigned int h = 0;
 const unsigned char *p = (const unsigned char *) (((x)->fld[0]).rtstr);
      ...
      else
 {
   ...
   hash += (unsigned) code + (unsigned) ((enum machine_mode) (x)->mode)
     + hash_string_1 ((((x)->fld[0]).rtstr))
     + hash_string_1 ((((x)->fld[1]).rtstr))
  ...
  {
    hash += (hash_expr_1 ((((((x)->fld[3]).rtvec))->elem[i]),
     ((enum machine_mode) ((((((x)->fld[3]).rtvec))->elem[i]))->mode),
     do_not_record_p)
      + hash_string_1 (((((((((x)->fld[4]).rtvec))->elem[i]))->fld[0]).rtstr)));

  }

       hash += hash_string_1 (((((((((x)->fld[4]).rtvec))->elem[0]))->fld[0]).rtstr));
      ...
      if (fmt[i] == 'e')
 {
   ...
   hash += hash_expr_1 ((((x)->fld[i]).rtx), 0, do_not_record_p);
   if (*do_not_record_p)
     return 0;
 }
 ...
 for (j = 0; j < (((((x)->fld[i]).rtvec))->num_elem); j++)
   {
     hash += hash_expr_1 ((((((x)->fld[i]).rtvec))->elem[j]), 0, do_not_record_p);
     if (*do_not_record_p)
       return 0;
   }

      else if (fmt[i] == 's')
 hash += hash_string_1 ((((x)->fld[i]).rtstr));
      else if (fmt[i] == 'i')
 hash += (unsigned int) (((x)->fld[i]).rtint);
      else
 fancy_abort ("../../../kgccfe/gnu/gcse.c", 1768, __FUNCTION__);
    }

  return hash;
}
# 1781 "../../../kgccfe/gnu/gcse.c"
static unsigned int
     ...
     rtx x, y;
{
  ...
  switch (code)
    ...
    case CONST_INT:
      return (((x)->fld[0]).rtwint) == (((y)->fld[0]).rtwint);
    ...
    case MEM:
      ...
      if (((((x)->fld[1]).rtmem) == 0 ? 0 : (((x)->fld[1]).rtmem)->alias) != ((((y)->fld[1]).rtmem) == 0 ? 0 : (((y)->fld[1]).rtmem)->alias))
 return 0;
      break;
    ...
    case NE:
    case EQ:
      return ((expr_equiv_p ((((x)->fld[0]).rtx), (((y)->fld[0]).rtx))
        && expr_equiv_p ((((x)->fld[1]).rtx), (((y)->fld[1]).rtx)))
       || (expr_equiv_p ((((x)->fld[0]).rtx), (((y)->fld[1]).rtx))
    && expr_equiv_p ((((x)->fld[1]).rtx), (((y)->fld[0]).rtx))));
      ...
      if ((((((x)->fld[3]).rtvec))->num_elem))
 {
   for (i = (((((x)->fld[3]).rtvec))->num_elem) - 1; i >= 0; i--)
     if (! expr_equiv_p ((((((x)->fld[3]).rtvec))->elem[i]),
 ...
 {
 case 'e':
   if (! expr_equiv_p ((((x)->fld[i]).rtx), (((y)->fld[i]).rtx)))
     return 0;
   break;
 ...
 case 'E':
   if ((((((x)->fld[i]).rtvec))->num_elem) != (((((y)->fld[i]).rtvec))->num_elem))
     return 0;
   for (j = 0; j < (((((x)->fld[i]).rtvec))->num_elem); j++)
     if (! expr_equiv_p ((((((x)->fld[i]).rtvec))->elem[j]), (((((y)->fld[i]).rtvec))->elem[j])))
       return 0;
   break;
 ...
 default:
   fancy_abort ("../../../kgccfe/gnu/gcse.c", 1928, __FUNCTION__);
 }
    }

  return 1;
}
# 1945 "../../../kgccfe/gnu/gcse.c"
static void
  ...
  if (antic_p)
    {
      ...
      while (antic_occr && ((((antic_occr->insn)->fld[3]).bb)->index + 0) != ((((insn)->fld[3]).bb)->index + 0))
 {
  ...
  if (avail_p)
    {
      ...
      while (avail_occr && ((((avail_occr->insn)->fld[3]).bb)->index + 0) != ((((insn)->fld[3]).bb)->index + 0))
 {
     ...
     struct hash_table *table;
{
  ...
  if (((enum rtx_code) (x)->code) != SET
    ...
    fancy_abort ("../../../kgccfe/gnu/gcse.c", 2087, __FUNCTION__);

  hash = hash_set (((((((x)->fld[0]).rtx))->fld[0]).rtuint), table->size);
 ...
 last_expr->next_same_hash = cur_expr;
  ...
  while (cur_occr && ((((cur_occr->insn)->fld[3]).bb)->index + 0) != ((((insn)->fld[3]).bb)->index + 0))
    {
     ...
     struct hash_table *table;
{
 ...
 src = (((note)->fld[0]).rtx), pat = gen_rtx_fmt_ee (SET, (VOIDmode), (dest), (src));
   ...
   && ((note = find_reg_note (insn, REG_EQUIV, (rtx) 0)) == 0
       || ((enum rtx_code) ((((note)->fld[0]).rtx))->code) != MEM))
 {



   int antic_p = oprs_anticipatable_p (src, insn) && (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i') ? (((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == SET ? (((insn)->fld[5]).rtx) : single_set_2 (insn, (((insn)->fld[5]).rtx))) : (rtx) 0);
   ...
   insert_expr_in_table (src, ((enum machine_mode) (dest)->mode), insn, antic_p, avail_p, table);
     ...
     || (((enum rtx_code) (src)->code) == LABEL_REF || ((enum rtx_code) (src)->code) == SYMBOL_REF || ((enum rtx_code) (src)->code) == CONST_INT || ((enum rtx_code) (src)->code) == CONST_DOUBLE || ((enum rtx_code) (src)->code) == CONST || ((enum rtx_code) (src)->code) == HIGH || ((enum rtx_code) (src)->code) == CONST_VECTOR || ((enum rtx_code) (src)->code) == CONSTANT_P_RTX))
        ...
        && (insn == ((((basic_block_info)->data.bb[(((((insn)->fld[3]).bb)->index + 0))]))->end)
...
hash_scan_clobber (x, insn, table)
     rtx x __attribute__ ((__unused__)), insn __attribute__ ((__unused__));
     struct hash_table *table __attribute__ ((__unused__));
{
     ...
     int regno;
{
  ...
  int cuid = (uid_cuid[(((insn)->fld[0]).rtint)]);
  ...
  if (info->last_bb != current_bb)
    {
      ...
      info->first_set = cuid;
      ((reg_set_in_block[current_bb->index])->elms [(regno) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (regno) % ((unsigned) (8 * 8)));
    }
}
...
canon_list_insert (dest, unused1, v_insn)
     rtx dest __attribute__ ((__unused__));
     rtx unused1 __attribute__ ((__unused__));
     void * v_insn;
{
  rtx dest_addr, insn;
  int bb;
  ...
  dest_addr = get_addr ((((dest)->fld[0]).rtx));
  dest_addr = canon_rtx (dest_addr);
  insn = (rtx) v_insn;
  bb = ((((insn)->fld[3]).bb)->index + 0);
     ...
     rtx insn;
{
 ...
 alloc_INSN_LIST (insn, canon_modify_mem_list[bb]);
    ...
    note_stores ((((insn)->fld[5]).rtx), canon_list_insert, (void*) insn);
}
     ...
     struct hash_table *table;
{
  ...
  for (current_bb = (&entry_exit_blocks[0])->next_bb; current_bb != (&entry_exit_blocks[1]); current_bb = current_bb->next_bb)
    {
      rtx insn;
      unsigned int regno;
      int in_libcall_block;
    ...
    insn && insn != (((current_bb->end)->fld[2]).rtx);
    insn = (((insn)->fld[2]).rtx))
 {
   if (! ((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i'))
     continue;
  ...
  if (clobbers_all
   ...
   note_stores ((((insn)->fld[5]).rtx), record_last_set_info, insn);
 }
     ...
     struct hash_table *table;
{
 ...
 expr = expr->next_same_hash;
    ...
    {
      while (expr && ((((((expr->expr)->fld[0]).rtx))->fld[0]).rtuint) != regno)
 expr = expr->next_same_hash;
    }
     ...
     rtx x, insn;
{
  ...
  switch (code)
    ...
    case MEM:
      if (load_killed_in_block_p ((((insn)->fld[3]).bb),
      (uid_cuid[(((insn)->fld[0]).rtint)]), x, 0))
 return 0;
      else
 return oprs_not_set_p ((((x)->fld[0]).rtx), insn);

    case REG:
      return ! bitmap_bit_p (reg_set_bitmap, (((x)->fld[0]).rtuint));
      ...
      if (fmt[i] == 'e')
 {
   ...
   if (i == 0)
     return oprs_not_set_p ((((x)->fld[i]).rtx), insn);

   if (! oprs_not_set_p ((((x)->fld[i]).rtx), insn))
     return 0;
 }
      else if (fmt[i] == 'E')
 for (j = 0; j < (((((x)->fld[i]).rtvec))->num_elem); j++)
   if (! oprs_not_set_p ((((((x)->fld[i]).rtvec))->elem[j]), insn))
     return 0;
    }
     ...
     rtx pat, insn;
{
  ...
  if (((enum rtx_code) (dest)->code) == REG)
    bitmap_set_bit (reg_set_bitmap, (((dest)->fld[0]).rtuint));
  else if (((enum rtx_code) (dest)->code) == MEM)
    record_last_mem_set_info (insn);

  if (((enum rtx_code) ((((pat)->fld[1]).rtx))->code) == CALL)
    mark_call (insn);
}
     ...
     basic_block bb;
{
  ...
  for (this_reg = reg_set_table[regno]; this_reg; this_reg = this_reg ->next)
    if (((((this_reg->insn)->fld[3]).bb)->index + 0) != ((((insn)->fld[3]).bb)->index + 0))
      ((rd_kill[bb->index])->elms [((uid_cuid[(((this_reg->insn)->fld[0]).rtint)])) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << ((uid_cuid[(((this_reg->insn)->fld[0]).rtint)])) % ((unsigned) (8 * 8)));
}
...
compute_kill_rd ()
{
  ...
  int i;
  basic_block bb;
# 2957 "../../../kgccfe/gnu/gcse.c"
  for (bb = (&entry_exit_blocks[0])->next_bb; bb != (&entry_exit_blocks[1]); bb = bb->next_bb)
    for (cuid = 0; cuid < max_cuid; cuid++)
      if (((rd_gen[bb->index])->elms [(cuid) / ((unsigned) (8 * 8))] >> (cuid) % ((unsigned) (8 * 8)) & 1))
 {
  ...
  {
    enum rtx_code code = ((enum rtx_code) ((((((pat)->fld[0]).rtvec))->elem[i]))->code);

    if ((code == SET || code == CLOBBER)
        && ((enum rtx_code) (((((((((pat)->fld[0]).rtvec))->elem[i]))->fld[0]).rtx))->code) == REG)
      handle_rd_kill_set (insn,
     (((((((((((pat)->fld[0]).rtvec))->elem[i]))->fld[0]).rtx))->fld[0]).rtuint),
     bb);
  }
     }
   else if (((enum rtx_code) (pat)->code) == SET && ((enum rtx_code) ((((pat)->fld[0]).rtx))->code) == REG)


     handle_rd_kill_set (insn, ((((((pat)->fld[0]).rtx))->fld[0]).rtuint), bb);
 }
}
     ...
     struct hash_table *expr_hash_table;
{
 ...
 ((ae_gen[((((occr->insn)->fld[3]).bb)->index + 0)])->elms [(expr->bitmap_index) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (expr->bitmap_index) % ((unsigned) (8 * 8)));
}
     ...
     basic_block bb;
{
  ...
  switch (code)
    {
    case REG:
      return ((reg_set_in_block[bb->index])->elms [((((x)->fld[0]).rtuint)) / ((unsigned) (8 * 8))] >> ((((x)->fld[0]).rtuint)) % ((unsigned) (8 * 8)) & 1);

    case MEM:
      if (load_killed_in_block_p (bb, get_max_uid () + 1, x, 0))
 return 1;
      else
 return expr_killed_p ((((x)->fld[0]).rtx), bb);
      ...
      if (fmt[i] == 'e')
 {
   ...
   if (i == 0)
     return expr_killed_p ((((x)->fld[i]).rtx), bb);
   else if (expr_killed_p ((((x)->fld[i]).rtx), bb))
     return 1;
 }
      else if (fmt[i] == 'E')
 for (j = 0; j < (((((x)->fld[i]).rtvec))->num_elem); j++)
   if (expr_killed_p ((((((x)->fld[i]).rtvec))->elem[j]), bb))
     return 1;
    }
     ...
     struct hash_table *expr_hash_table;
{
      ...
      for (expr = expr_hash_table->table[i]; expr; expr = expr->next_same_hash)
 {
   ...
   if (((ae_gen[bb->index])->elms [(expr->bitmap_index) / ((unsigned) (8 * 8))] >> (expr->bitmap_index) % ((unsigned) (8 * 8)) & 1))
     continue;

   if (expr_killed_p (expr->expr, bb))
     ((ae_kill[bb->index])->elms [(expr->bitmap_index) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (expr->bitmap_index) % ((unsigned) (8 * 8)));
 }
}
# 3178 "../../../kgccfe/gnu/gcse.c"
static int
     ...
     char *visited;
{
      ...
      else if (pred_bb == bb)
 {
   ...
   if (check_self_loop
       && ((ae_gen[pred_bb->index])->elms [(expr->bitmap_index) / ((unsigned) (8 * 8))] >> (expr->bitmap_index) % ((unsigned) (8 * 8)) & 1)
       && ((((occr->insn)->fld[3]).bb)->index + 0) == pred_bb->index)
     return 1;
   ...
   visited[pred_bb->index] = 1;
      ...
      else if (((ae_kill[pred_bb->index])->elms [(expr->bitmap_index) / ((unsigned) (8 * 8))] >> (expr->bitmap_index) % ((unsigned) (8 * 8)) & 1))
 visited[pred_bb->index] = 1;


      else if (((ae_gen[pred_bb->index])->elms [(expr->bitmap_index) / ((unsigned) (8 * 8))] >> (expr->bitmap_index) % ((unsigned) (8 * 8)) & 1))
 {



   if (((((occr->insn)->fld[3]).bb)->index + 0) == pred_bb->index)
     return 1;
     ...
     rtx insn;
{
  ...
  if (expr->avail_occr->next == ((void *)0))
    {
      if ((((expr->avail_occr->insn)->fld[3]).bb) == bb)
 ...
 return ((void *)0);
  ...
  else
    {
      ...
      struct occr *occr;
      rtx insn_computes_expr = ((void *)0);
      int can_reach = 0;

      for (occr = expr->avail_occr; occr != ((void *)0); occr = occr->next)
 {
   if ((((occr->insn)->fld[3]).bb) == bb)
     {




       if ((uid_cuid[(((insn)->fld[0]).rtint)]) < (uid_cuid[(((occr->insn)->fld[0]).rtint)])
  ...
  return ((void *)0);
      ...
      if (insn_computes_expr == ((void *)0))
 fancy_abort ("../../../kgccfe/gnu/gcse.c", 3317, __FUNCTION__);
     ...
     rtx insn, def_insn;
{
  ...
  if (((reaching_defs[((((insn)->fld[3]).bb)->index + 0)])->elms [((uid_cuid[(((def_insn)->fld[0]).rtint)])) / ((unsigned) (8 * 8))] >> ((uid_cuid[(((def_insn)->fld[0]).rtint)])) % ((unsigned) (8 * 8)) & 1))
    return 1;

  if (((((insn)->fld[3]).bb)->index + 0) == ((((def_insn)->fld[3]).bb)->index + 0))
    {
      if ((uid_cuid[(((def_insn)->fld[0]).rtint)]) < (uid_cuid[(((insn)->fld[0]).rtint)]))
 {
   if (((enum rtx_code) ((((def_insn)->fld[5]).rtx))->code) == PARALLEL)
     return 1;
   else if (((enum rtx_code) ((((def_insn)->fld[5]).rtx))->code) == CLOBBER)
     reg = ((((((def_insn)->fld[5]).rtx))->fld[0]).rtx);
   else if (((enum rtx_code) ((((def_insn)->fld[5]).rtx))->code) == SET)
     reg = ((((((def_insn)->fld[5]).rtx))->fld[0]).rtx);
   else
     fancy_abort ("../../../kgccfe/gnu/gcse.c", 3346, __FUNCTION__);

   return ! reg_set_between_p (reg, (((def_insn)->fld[2]).rtx), insn);
 }
      else
 return 0;
    }
     ...
     int for_combine;
{
 ...
 if (((enum rtx_code) ((((this_reg->insn)->fld[5]).rtx))->code) == PARALLEL)
   return 0;

 if (!for_combine
     && (((enum rtx_code) ((((this_reg->insn)->fld[5]).rtx))->code) == CLOBBER
  || ! rtx_equal_p (((((((this_reg->insn)->fld[5]).rtx))->fld[1]).rtx),
      ((((((insn)->fld[5]).rtx))->fld[1]).rtx))))
 ...
 if (number_of_reaching_defs > 1)
   {
     ...
     if (((enum rtx_code) ((((this_reg->insn)->fld[5]).rtx))->code) == CLOBBER)
       return 0;
     else if (! rtx_equal_p (((((((this_reg->insn)->fld[5]).rtx))->fld[1]).rtx),
        ((((((insn)->fld[5]).rtx))->fld[1]).rtx)))
       return 0;
   }
     ...
     struct expr *expr;
{
  ...
  if (insn_computes_expr == ((void *)0))
    return 0;
  expr_set = (((rtx_class[(int) (((enum rtx_code) (insn_computes_expr)->code))]) == 'i') ? (((enum rtx_code) ((((insn_computes_expr)->fld[5]).rtx))->code) == SET ? (((insn_computes_expr)->fld[5]).rtx) : single_set_2 (insn_computes_expr, (((insn_computes_expr)->fld[5]).rtx))) : (rtx) 0);
  if (!expr_set)
    fancy_abort ("../../../kgccfe/gnu/gcse.c", 3429, __FUNCTION__);
  ...
  if (((enum rtx_code) ((((expr_set)->fld[1]).rtx))->code) == REG)
    {


      unsigned int regnum_for_replacing
 = ((((((expr_set)->fld[1]).rtx))->fld[0]).rtuint);
       ...
       || can_disregard_other_sets (&this_reg, insn, 0)))
 {
  ...
  if (!found_setting)
    {
      unsigned int regnum_for_replacing
 = ((((((expr_set)->fld[0]).rtx))->fld[0]).rtuint);


      if (regnum_for_replacing >= max_gcse_regno)
 fancy_abort ("../../../kgccfe/gnu/gcse.c", 3465, __FUNCTION__);

      this_reg = reg_set_table[regnum_for_replacing];



      if (this_reg->next == ((void *)0)
   || can_disregard_other_sets (&this_reg, insn, 0))
 found_setting = 1;
    }
  ...
  if (found_setting)
    {
      pat = (((insn)->fld[5]).rtx);
      if (use_src)
 to = (((expr_set)->fld[1]).rtx);
      else
 to = (((expr_set)->fld[0]).rtx);
      changed = validate_change (insn, &(((pat)->fld[1]).rtx), to, 0);
   ...
   if (gcse_file != ((void *)0))
     {
       ...
       fprintf (gcse_file, " reg %d %s insn %d\n",
         (((to)->fld[0]).rtuint), use_src ? "from" : "set in",
         (((insn_computes_expr)->fld[0]).rtint));
     }
 }
    }
  ...
  else if (1 )
    {
      ...
      to = gen_reg_rtx (((enum machine_mode) ((((expr_set)->fld[0]).rtx))->mode));




      new_insn
 = emit_insn_after (gen_rtx_fmt_ee (SET, (VOIDmode), (to), ((((expr_set)->fld[0]).rtx))),

      insn_computes_expr);


      record_one_set ((((to)->fld[0]).rtuint), new_insn);

      gcse_create_count++;
      if (gcse_file != ((void *)0))
 {
   fprintf (gcse_file, "GCSE: Creating insn %d to copy value of reg %d",
     ((((((insn_computes_expr)->fld[2]).rtx))->fld[0]).rtint),
     ((((((((((((insn_computes_expr)->fld[2]).rtx))->fld[5]).rtx))->fld[1]).rtx))->fld[0]).rtuint));
   ...
   fprintf (gcse_file, "      into newly allocated reg %d\n",
     (((to)->fld[0]).rtuint));
      ...
      changed = validate_change (insn, &(((pat)->fld[1]).rtx),
     (((((((((insn_computes_expr)->fld[2]).rtx))->fld[5]).rtx))->fld[0]).rtx),
   ...
   if (gcse_file != ((void *)0))
     {
       fprintf (gcse_file,
         ...
         (((insn)->fld[0]).rtint),
         ((((((((((((insn_computes_expr)->fld[2]).rtx))->fld[5]).rtx))->fld[0]).rtx))->fld[0]).rtuint));
...
classic_gcse ()
{
  ...
  if ((&entry_exit_blocks[0])->next_bb == (&entry_exit_blocks[1]))
    return 0;

  changed = 0;
  for (bb = (&entry_exit_blocks[0])->next_bb->next_bb; bb != (&entry_exit_blocks[1]); bb = bb->next_bb)
    {
      ...
      for (insn = bb->head;
    insn != ((void *)0) && insn != (((bb->end)->fld[2]).rtx);
    insn = (((insn)->fld[2]).rtx))
 {
   ...
   if (((enum rtx_code) (insn)->code) == INSN
       ...
       && (((((((((insn)->fld[5]).rtx))->fld[0]).rtx))->fld[0]).rtuint) >= 176)
     {
       rtx pat = (((insn)->fld[5]).rtx);
       rtx src = (((pat)->fld[1]).rtx);
       struct expr *expr;
    ...
    && ((ae_in[bb->index])->elms [(expr->bitmap_index) / ((unsigned) (8 * 8))] >> (expr->bitmap_index) % ((unsigned) (8 * 8)) & 1)
  ...
  if (gcse_file)
    {
      ...
      fprintf (gcse_file, "GCSE of %s, pass %d: %d bytes needed, %d substs,",
        (cfun->name), pass, bytes_used, gcse_subst_count);
     ...
     int set_p;
{
      ...
      if (set_p)
 {
   if ((((x)->fld[0]).rtuint) < 176)
     {
       for (bb = (&entry_exit_blocks[0])->next_bb; bb != (&entry_exit_blocks[1]); bb = bb->next_bb)
  if (((reg_set_in_block[bb->index])->elms [((((x)->fld[0]).rtuint)) / ((unsigned) (8 * 8))] >> ((((x)->fld[0]).rtuint)) % ((unsigned) (8 * 8)) & 1))
    ((bmap[bb->index])->elms [(indx) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (indx) % ((unsigned) (8 * 8)));
     }
   else
     {
       for (r = reg_set_table[(((x)->fld[0]).rtuint)]; r != ((void *)0); r = r->next)
  ((bmap[((((r->insn)->fld[3]).bb)->index + 0)])->elms [(indx) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (indx) % ((unsigned) (8 * 8)));
     }
 }
      else
 {
   ...
   while (list_entry)
     {
       ...
       if (((enum rtx_code) ((((list_entry)->fld[0]).rtx))->code) == CALL_INSN)
  {
    if (set_p)
      ((bmap[bb->index])->elms [(indx) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (indx) % ((unsigned) (8 * 8)));
    else
    ...
    break;
       ...
       dest = (((list_entry)->fld[0]).rtx);
       list_entry = (((list_entry)->fld[1]).rtx);
       dest_addr = (((list_entry)->fld[0]).rtx);
      ...
      if (fmt[i] == 'e')
 {
   ...
   compute_transp ((((x)->fld[i]).rtx), indx, bmap, set_p);
 }
      else if (fmt[i] == 'E')
 for (j = 0; j < (((((x)->fld[i]).rtvec))->num_elem); j++)
   compute_transp ((((((x)->fld[i]).rtvec))->elem[j]), indx, bmap, set_p);
    }
}
...
# 3860 "../../../kgccfe/gnu/gcse.c"
static struct reg_use reg_use_table[8];


static int reg_use_count;
# 3872 "../../../kgccfe/gnu/gcse.c"
static void
     ...
     void *data __attribute__ ((__unused__));
{
      ...
      if (fmt[i] == 'e')
 {
   ...
   find_used_regs (&(((x)->fld[i]).rtx), data);
 }
      else if (fmt[i] == 'E')
 for (j = 0; j < (((((x)->fld[i]).rtvec))->num_elem); j++)
   find_used_regs (&(((((x)->fld[i]).rtvec))->elem[j]), data);
    }
}
     ...
     rtx from, to, insn;
{
  ...
  if (success && set && (((enum rtx_code) (to)->code) == LABEL_REF || ((enum rtx_code) (to)->code) == SYMBOL_REF || ((enum rtx_code) (to)->code) == CONST_INT || ((enum rtx_code) (to)->code) == CONST_DOUBLE || ((enum rtx_code) (to)->code) == CONST || ((enum rtx_code) (to)->code) == HIGH || ((enum rtx_code) (to)->code) == CONST_VECTOR || ((enum rtx_code) (to)->code) == CONSTANT_P_RTX))
    {
      src = simplify_rtx ((((set)->fld[1]).rtx));

      if (src)
 validate_change (insn, &(((set)->fld[1]).rtx), src, 0);
    }

  if (!success && set && reg_mentioned_p (from, (((set)->fld[1]).rtx)))
    {



      src = simplify_replace_rtx ((((set)->fld[1]).rtx), from, to);

      if (!rtx_equal_p (src, (((set)->fld[1]).rtx))
   && validate_change (insn, &(((set)->fld[1]).rtx), src, 0))
 success = 1;
 ...
 note = set_unique_reg_note (insn, REG_EQUAL, copy_rtx (src));
    ...
    (((note)->fld[0]).rtx) = simplify_replace_rtx ((((note)->fld[0]).rtx), from, to);





  if (note && (((enum rtx_code) ((((note)->fld[0]).rtx))->code) == REG))
     ...
     rtx insn;
{
  ...
  struct expr *set1 = 0;
# 4002 "../../../kgccfe/gnu/gcse.c"
  while (1)
    {
      rtx src;
      struct expr *set = lookup_set (regno, (rtx) 0, &set_hash_table);



      while (set)
 {
   if (((cprop_avin[((((insn)->fld[3]).bb)->index + 0)])->elms [(set->bitmap_index) / ((unsigned) (8 * 8))] >> (set->bitmap_index) % ((unsigned) (8 * 8)) & 1))
     break;
 ...
 fancy_abort ("../../../kgccfe/gnu/gcse.c", 4022, __FUNCTION__);

      src = (((set->expr)->fld[1]).rtx);
# 4033 "../../../kgccfe/gnu/gcse.c"
      if ((((enum rtx_code) (src)->code) == LABEL_REF || ((enum rtx_code) (src)->code) == SYMBOL_REF || ((enum rtx_code) (src)->code) == CONST_INT || ((enum rtx_code) (src)->code) == CONST_DOUBLE || ((enum rtx_code) (src)->code) == CONST || ((enum rtx_code) (src)->code) == HIGH || ((enum rtx_code) (src)->code) == CONST_VECTOR || ((enum rtx_code) (src)->code) == CONSTANT_P_RTX) || oprs_not_set_p (src, insn))
 set1 = set;
     ...
     rtx src;
{
  ...
  if (setcc != ((void *)0)
    ...
    {
      rtx setcc_set = (((rtx_class[(int) (((enum rtx_code) (setcc)->code))]) == 'i') ? (((enum rtx_code) ((((setcc)->fld[5]).rtx))->code) == SET ? (((setcc)->fld[5]).rtx) : single_set_2 (setcc, (((setcc)->fld[5]).rtx))) : (rtx) 0);
      new_set = simplify_replace_rtx ((((set)->fld[1]).rtx),
          (((setcc_set)->fld[0]).rtx),
          (((setcc_set)->fld[1]).rtx));
    }
  else
  ...
  if (rtx_equal_p (new, new_set) || rtx_equal_p (new, (((set)->fld[1]).rtx)))
    return 0;
 ...
 return 0;
      if (! validate_change (jump, &(((set)->fld[1]).rtx), new, 0))
 return 0;




      if (((enum rtx_code) ((((set)->fld[1]).rtx))->code) == LABEL_REF)
  ...
  if (gcse_file != ((void *)0))
    {
      fprintf (gcse_file,
        ...
        (((from)->fld[0]).rtuint), (((jump)->fld[0]).rtint));
     ...
     int alter_jumps;
{
  ...
  if (alter_jumps
      && (sset = (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i') ? (((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == SET ? (((insn)->fld[5]).rtx) : single_set_2 (insn, (((insn)->fld[5]).rtx))) : (rtx) 0)) != ((void *)0)
      && (((insn)->fld[2]).rtx)
      && any_condjump_p ((((insn)->fld[2]).rtx)) && onlyjump_p ((((insn)->fld[2]).rtx)))
    {
      rtx dest = (((sset)->fld[0]).rtx);
      if (((((enum rtx_code) (dest)->code) == REG) || 0)
   && cprop_jump ((((insn)->fld[3]).bb), insn, (((insn)->fld[2]).rtx), from, to))
 return 1;
    }
  ...
  if (((enum rtx_code) (insn)->code) == INSN
    ...
    return cprop_jump ((((insn)->fld[3]).bb), ((void *)0), insn, from, to);
  return 0;
}
     ...
     int alter_jumps;
{
  ...
  reg_use_count = 0;
  note_uses (&(((insn)->fld[5]).rtx), find_used_regs, ((void *)0));

  note = find_reg_equal_equiv_note (insn);


  if (note)
    find_used_regs (&(((note)->fld[0]).rtx), ((void *)0));

  for (reg_used = &reg_use_table[0]; reg_use_count > 0;
       reg_used++, reg_use_count--)
    {
      unsigned int regno = (((reg_used->reg_rtx)->fld[0]).rtuint);
      rtx pat, src;
      struct expr *set;
 ...
 continue;
      ...
      if (((enum rtx_code) (pat)->code) != SET)
 fancy_abort ("../../../kgccfe/gnu/gcse.c", 4220, __FUNCTION__);
      ...
      if ((((enum rtx_code) (src)->code) == LABEL_REF || ((enum rtx_code) (src)->code) == SYMBOL_REF || ((enum rtx_code) (src)->code) == CONST_INT || ((enum rtx_code) (src)->code) == CONST_DOUBLE || ((enum rtx_code) (src)->code) == CONST || ((enum rtx_code) (src)->code) == HIGH || ((enum rtx_code) (src)->code) == CONST_VECTOR || ((enum rtx_code) (src)->code) == CONSTANT_P_RTX))
 {
       ...
       if (gcse_file != ((void *)0))
  {
    ...
    fprintf (gcse_file, "insn %d with constant ", (((insn)->fld[0]).rtint));
    print_rtl (gcse_file, src);
    fprintf (gcse_file, "\n");
  }
     }
     ...
     rtx *libcall_sp;
{
      ...
      for (l = val->locs; l; l = l->next)
 {
   ...
   if ((((enum rtx_code) (this_rtx)->code) == LABEL_REF || ((enum rtx_code) (this_rtx)->code) == SYMBOL_REF || ((enum rtx_code) (this_rtx)->code) == CONST_INT || ((enum rtx_code) (this_rtx)->code) == CONST_DOUBLE || ((enum rtx_code) (this_rtx)->code) == CONST || ((enum rtx_code) (this_rtx)->code) == HIGH || ((enum rtx_code) (this_rtx)->code) == CONST_VECTOR || ((enum rtx_code) (this_rtx)->code) == CONSTANT_P_RTX))
     newcnst = this_rtx;
   if ((((enum rtx_code) (this_rtx)->code) == REG) && (((this_rtx)->fld[0]).rtuint) >= 176
      ...
      if (newcnst && constprop_register (insn, x, newcnst, alter_jumps))
 {
   ...
   if (!adjust_libcall_notes (x, newcnst, insn, libcall_sp))
     fancy_abort ("../../../kgccfe/gnu/gcse.c", 4366, __FUNCTION__);
   if (gcse_file != ((void *)0))
     {
   ...
   if (gcse_file != ((void *)0))
     {
       fprintf (gcse_file,
         ...
         (((x)->fld[0]).rtuint), (((insn)->fld[0]).rtint));
       fprintf (gcse_file, " with reg %d\n", (((newreg)->fld[0]).rtuint));
     }
   copy_prop_count++;
   return 1;
     ...
     rtx oldreg, newval, insn, *libcall_sp;
{
 ...
 {
   if (reg_set_between_p (newval, (((insn)->fld[1]).rtx), end))
     {
       do
  {
    ...
    if (! note)
      continue;
    if (reg_mentioned_p (newval, (((note)->fld[0]).rtx)))
      return 0;
  }
       while ((end = *libcall_sp++));
       return 1;
      ...
      (((note)->fld[0]).rtx) = replace_rtx ((((note)->fld[0]).rtx), oldreg, newval);
      insn = end;
    }
  return 1;
}
     ...
     int alter_jumps;
{
      ...
      if (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i'))
 {
  ...
  fancy_abort ("../../../kgccfe/gnu/gcse.c", 4458, __FUNCTION__);
       *--libcall_sp = (((note)->fld[0]).rtx);
     }
   ...
   do
     {
       reg_use_count = 0;
       note_uses (&(((insn)->fld[5]).rtx), local_cprop_find_used_regs, ((void *)0));
       if (note)
  local_cprop_find_used_regs (&(((note)->fld[0]).rtx), ((void *)0));
     ...
     int bb;
{
  ...
  for (;;)
    {
      ...
      while (set)
 {
   if (((cprop_avout[bb])->elms [(set->bitmap_index) / ((unsigned) (8 * 8))] >> (set->bitmap_index) % ((unsigned) (8 * 8)) & 1))
     break;
   set = next_set (regno, set);
 }
 ...
 break;
      ...
      if (((enum rtx_code) (set->expr)->code) != SET)
 fancy_abort ("../../../kgccfe/gnu/gcse.c", 4618, __FUNCTION__);
     ...
     edge e;
{
  ...
  for (insn = e->insns; insn; insn = (((insn)->fld[2]).rtx))
    if (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i') && reg_set_p (reg, insn))
      return 1;

  return 0;
}
# 4663 "../../../kgccfe/gnu/gcse.c"
static int
     ...
     rtx setcc, jump;
{
      ...
      for (i = 0; i < reg_use_count; i++)
 {
   ...
   src = (((pc_set (jump))->fld[1]).rtx);

   if (setcc != ((void *)0))
       src = simplify_replace_rtx (src,
       ((((((setcc)->fld[5]).rtx))->fld[0]).rtx),
       ((((((setcc)->fld[5]).rtx))->fld[1]).rtx));
   ...
   if (new == (global_rtl[GR_PC]))
     {
       ...
       dest = edest->insns ? ((void *)0) : edest->dest;
     }
   else if (((enum rtx_code) (new)->code) == LABEL_REF)
     {
       dest = ((((((new)->fld[0]).rtx))->fld[3]).bb);
       ...
       if (gcse_file != ((void *)0))
  {
    fprintf (gcse_file, "JUMP-BYPASS: Proved reg %d in jump_insn %d equals constant ",
      regno, (((jump)->fld[0]).rtint));
    print_rtl (gcse_file, (((set->expr)->fld[1]).rtx));
...
compute_pre_data ()
{
 ...
 if (may_trap_p (e->expr))
   ((trapping_expr)->elms [(e->bitmap_index) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (e->bitmap_index) % ((unsigned) (8 * 8)));
    }
  ...
  for (pred = bb->pred; pred != ((void *)0); pred = pred->pred_next)
    {
      ...
      if (pred->src == (&entry_exit_blocks[0])

   || visited[pred_bb->index])
 ;


      else if (((comp[pred_bb->index])->elms [(expr->bitmap_index) / ((unsigned) (8 * 8))] >> (expr->bitmap_index) % ((unsigned) (8 * 8)) & 1))
 {
   ...
   visited[pred_bb->index] = 1;
      ...
      else if (! ((transp[pred_bb->index])->elms [(expr->bitmap_index) / ((unsigned) (8 * 8))] >> (expr->bitmap_index) % ((unsigned) (8 * 8)) & 1))
 visited[pred_bb->index] = 1;
     ...
     struct expr *expr;
{
  ...
  if (general_operand (exp, ((enum machine_mode) (reg)->mode)))
    ...
    fancy_abort ("../../../kgccfe/gnu/gcse.c", 5077, __FUNCTION__);
     ...
     int pre;
{
  ...
  if (pat == (rtx) 0 || ! ((rtx_class[(int) (((enum rtx_code) (pat)->code))]) == 'i'))
    fancy_abort ("../../../kgccfe/gnu/gcse.c", 5107, __FUNCTION__);

  pat_end = pat;
  while ((((pat_end)->fld[2]).rtx) != (rtx) 0)
    pat_end = (((pat_end)->fld[2]).rtx);
  ...
  if (((enum rtx_code) (insn)->code) == JUMP_INSN
   ...
   && !((antloc[bb->index])->elms [(expr->bitmap_index) / ((unsigned) (8 * 8))] >> (expr->bitmap_index) % ((unsigned) (8 * 8)) & 1)
   && !((transp[bb->index])->elms [(expr->bitmap_index) / ((unsigned) (8 * 8))] >> (expr->bitmap_index) % ((unsigned) (8 * 8)) & 1))
 fancy_abort ("../../../kgccfe/gnu/gcse.c", 5130, __FUNCTION__);




      if (((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == ADDR_VEC
   || ((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == ADDR_DIFF_VEC)
 insn = prev_real_insn (insn);
# 5155 "../../../kgccfe/gnu/gcse.c"
    ...
    && (bb->succ->succ_next || (bb->succ->flags & 2)))
    {
# 5172 "../../../kgccfe/gnu/gcse.c"
      if (pre
   && !((antloc[bb->index])->elms [(expr->bitmap_index) / ((unsigned) (8 * 8))] >> (expr->bitmap_index) % ((unsigned) (8 * 8)) & 1)
   && !((transp[bb->index])->elms [(expr->bitmap_index) / ((unsigned) (8 * 8))] >> (expr->bitmap_index) % ((unsigned) (8 * 8)) & 1))
 fancy_abort ("../../../kgccfe/gnu/gcse.c", 5175, __FUNCTION__);




      insn = find_first_parameter_load (insn, bb->head);
# 5190 "../../../kgccfe/gnu/gcse.c"
      while (((enum rtx_code) (insn)->code) == CODE_LABEL
      || (((enum rtx_code) (insn)->code) == NOTE && (((insn)->fld[5]).rtint) == NOTE_INSN_BASIC_BLOCK))
 insn = (((insn)->fld[2]).rtx);
  ...
  while (1)
    {
      if (((rtx_class[(int) (((enum rtx_code) (pat)->code))]) == 'i'))
 {
   add_label_notes ((((pat)->fld[5]).rtx), new_insn);
   note_stores ((((pat)->fld[5]).rtx), record_set_info, pat);
 }
      if (pat == pat_end)
 break;
      pat = (((pat)->fld[2]).rtx);
  ...
  for (e = 0; e < num_edges; e++)
    {
      ...
      for (i = indx = 0; i < set_size; i++, indx += ((unsigned) (8 * 8)))
 {
   unsigned long long insert = pre_insert_map[e]->elms[i];
  ...
  for (occr = expr->antic_occr; occr != ((void *)0); occr = occr->next)
    {
      ...
      if (!((inserted[e])->elms [(j) / ((unsigned) (8 * 8))] >> (j) % ((unsigned) (8 * 8)) & 1))
        {
   rtx insn;
   edge eg = ((edge_list)->index_to_edge[(e)]);
# 5276 "../../../kgccfe/gnu/gcse.c"
   if ((eg->flags & 2) == 2)
   ...
   ((inserted[e])->elms [(j) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (j) % ((unsigned) (8 * 8)));
   did_insert = 1;
   gcse_create_count++;
        }
    }
     ...
     rtx insn;
{
  ...
  if (!set)
    fancy_abort ("../../../kgccfe/gnu/gcse.c", 5321, __FUNCTION__);

  new_insn = emit_insn_after (gen_move_insn (reg, (((set)->fld[0]).rtx)), insn);
  ...
  if (gcse_file)
    fprintf (gcse_file,
      "PRE: bb %d, insn %d, copy expression %d in insn %d to reg %d\n",
       ((((insn)->fld[3]).bb)->index + 0), (((new_insn)->fld[0]).rtint), indx,
       (((insn)->fld[0]).rtint), regno);
  update_ld_motion_stores (expr);
}
...
pre_insert_copies ()
{
 ...
 for (occr = expr->antic_occr; occr != ((void *)0); occr = occr->next)
  ...
  if (((pre_redundant_insns)->elms [((uid_cuid[(((insn)->fld[0]).rtint)])) / ((unsigned) (8 * 8))] >> ((uid_cuid[(((insn)->fld[0]).rtint)])) % ((unsigned) (8 * 8)) & 1))
    continue;


  if (! pre_expr_reaches_here_p ((((avail->insn)->fld[3]).bb),
            expr,
            (((occr->insn)->fld[3]).bb)))
    continue;
...
{
  rtx new;
  rtx set = (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i') ? (((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == SET ? (((insn)->fld[5]).rtx) : single_set_2 (insn, (((insn)->fld[5]).rtx))) : (rtx) 0), set2;
  rtx note;
  rtx eqv;
  ...
  set2 = (((rtx_class[(int) (((enum rtx_code) (new)->code))]) == 'i') ? (((enum rtx_code) ((((new)->fld[5]).rtx))->code) == SET ? (((new)->fld[5]).rtx) : single_set_2 (new, (((new)->fld[5]).rtx))) : (rtx) 0);
  if (!set2 || !rtx_equal_p ((((set2)->fld[0]).rtx), dest))
    return new;
 ...
 for (occr = expr->antic_occr; occr != ((void *)0); occr = occr->next)
   {
     ...
     if (((pre_delete_map[bb->index])->elms [(indx) / ((unsigned) (8 * 8))] >> (indx) % ((unsigned) (8 * 8)) & 1))
       {
  ...
  if (! set)
    fancy_abort ("../../../kgccfe/gnu/gcse.c", 5461, __FUNCTION__);
  ...
  if (expr->reaching_reg == ((void *)0))
    expr->reaching_reg
      = gen_reg_rtx (((enum machine_mode) ((((set)->fld[0]).rtx))->mode));

  gcse_emit_move_after (expr->reaching_reg, (((set)->fld[0]).rtx), insn);
  delete_insn (insn);
  occr->deleted_p = 1;
  ((pre_redundant_insns)->elms [((uid_cuid[(((insn)->fld[0]).rtint)])) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << ((uid_cuid[(((insn)->fld[0]).rtint)])) % ((unsigned) (8 * 8)));
  changed = 1;
  gcse_subst_count++;
  ...
  if (gcse_file)
    {
      fprintf (gcse_file,
        "PRE: redundant insn %d (expression %d) in ",
          (((insn)->fld[0]).rtint), indx);
      fprintf (gcse_file, "bb %d, reaching reg is %d\n",
        bb->index, (((expr->reaching_reg)->fld[0]).rtuint));
    }
       }
   }
      }
     ...
     rtx insn;
{
 ...
 ((((((x)->fld[0]).rtx))->fld[4]).rtint)++;
  ...
  for (i = (rtx_length[(int) (code)]) - 1, fmt = (rtx_format[(int) (code)]); i >= 0; i--)
    {
      if (fmt[i] == 'e')
 add_label_notes ((((x)->fld[i]).rtx), insn);
      else if (fmt[i] == 'E')
 for (j = (((((x)->fld[i]).rtvec))->num_elem) - 1; j >= 0; j--)
   add_label_notes ((((((x)->fld[i]).rtvec))->elem[j]), insn);
    }
}
# 5661 "../../../kgccfe/gnu/gcse.c"
static void
compute_transpout ()
{
  basic_block bb;
  ...
  for (bb = (&entry_exit_blocks[0])->next_bb; bb != (&entry_exit_blocks[1]); bb = bb->next_bb)
    {
      ...
      if (((enum rtx_code) (bb->end)->code) != CALL_INSN)
 continue;
 ...
 for (expr = expr_hash_table.table[i]; expr ; expr = expr->next_same_hash)
   if (((enum rtx_code) (expr->expr)->code) == MEM)
     {
       if (((enum rtx_code) ((((expr->expr)->fld[0]).rtx))->code) == SYMBOL_REF
    && ((((((expr->expr)->fld[0]).rtx)))->unchanging))
  continue;




       ((transpout[bb->index])->elms [(expr->bitmap_index) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (expr->bitmap_index) % ((unsigned) (8 * 8))));
     }
    }
}
# 5702 "../../../kgccfe/gnu/gcse.c"
static void
invalidate_nonnull_info (x, setter, data)
     rtx x;
     rtx setter __attribute__ ((__unused__));
     void *data;
{
  unsigned int regno;
  ...
  if (((enum rtx_code) (x)->code) != REG
      || (((x)->fld[0]).rtuint) < npi->min_reg
      || (((x)->fld[0]).rtuint) >= npi->max_reg)
    return;

  regno = (((x)->fld[0]).rtuint) - npi->min_reg;

  ((npi->nonnull_local[npi->current_block->index])->elms [(regno) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (regno) % ((unsigned) (8 * 8))));
  ((npi->nonnull_killed[npi->current_block->index])->elms [(regno) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (regno) % ((unsigned) (8 * 8)));
}
...
# 5751 "../../../kgccfe/gnu/gcse.c"
  sbitmap_vector_zero (nonnull_local, last_basic_block);
  sbitmap_vector_zero (nonnull_killed, last_basic_block);

  for (current_block = (&entry_exit_blocks[0])->next_bb; current_block != (&entry_exit_blocks[1]); current_block = current_block->next_bb)
    {
      rtx insn, stop_insn;
      ...
      stop_insn = (((current_block->end)->fld[2]).rtx);
    ...
    insn = (((insn)->fld[2]).rtx))
 {
   ...
   if (!set)
     {
       note_stores ((((insn)->fld[5]).rtx), invalidate_nonnull_info, npi);
       continue;
     }
   ...
   if (((enum rtx_code) ((((set)->fld[1]).rtx))->code) == MEM
       && ((enum rtx_code) ((reg = ((((((set)->fld[1]).rtx))->fld[0]).rtx)))->code) == REG
       && (((reg)->fld[0]).rtuint) >= npi->min_reg
       && (((reg)->fld[0]).rtuint) < npi->max_reg)
     ((nonnull_local[current_block->index])->elms [((((reg)->fld[0]).rtuint) - npi->min_reg) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << ((((reg)->fld[0]).rtuint) - npi->min_reg) % ((unsigned) (8 * 8)));
   ...
   if (((enum rtx_code) ((((set)->fld[0]).rtx))->code) == MEM
       && ((enum rtx_code) ((reg = ((((((set)->fld[0]).rtx))->fld[0]).rtx)))->code) == REG
       && (((reg)->fld[0]).rtuint) >= npi->min_reg
       && (((reg)->fld[0]).rtuint) < npi->max_reg)
     ((nonnull_local[current_block->index])->elms [((((reg)->fld[0]).rtuint) - npi->min_reg) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << ((((reg)->fld[0]).rtuint) - npi->min_reg) % ((unsigned) (8 * 8)));
 ...
 continue;
      ...
      if (!((nonnull_avout[bb->index])->elms [(block_reg[bb->index] - npi->min_reg) / ((unsigned) (8 * 8))] >> (block_reg[bb->index] - npi->min_reg) % ((unsigned) (8 * 8)) & 1))
 continue;
      ...
      if (((enum rtx_code) (condition)->code) == NE)
 {
   ...
   new_jump = emit_jump_insn_after (gen_jump ((((last_insn)->fld[9]).rtx)),
        last_insn);
   (((new_jump)->fld[9]).rtx) = (((last_insn)->fld[9]).rtx);
   ((((((new_jump)->fld[9]).rtx))->fld[4]).rtint)++;
...
# 5927 "../../../kgccfe/gnu/gcse.c"
 ...
 continue;
      ...
      if (!condition
   || (((enum rtx_code) (condition)->code) != NE && ((enum rtx_code) (condition)->code) != EQ)
   || ((enum rtx_code) ((((condition)->fld[1]).rtx))->code) != CONST_INT
   || ((((condition)->fld[1]).rtx)
       != (const_tiny_rtx[0][(int) (((enum machine_mode) ((((condition)->fld[0]).rtx))->mode))])))
 continue;


      reg = (((condition)->fld[0]).rtx);
      if (((enum rtx_code) (reg)->code) != REG)
 continue;

      block_reg[bb->index] = (((reg)->fld[0]).rtuint);
    }


  for (reg = 176; reg < max_reg; reg += regs_per_pass)
    {
      npi.min_reg = reg;
      npi.max_reg = ((reg + regs_per_pass) < (max_reg) ? (reg + regs_per_pass) : (max_reg));
...
compute_code_hoist_vbeinout ()
{
      ...
      for (bb = (&entry_exit_blocks[1])->prev_bb; bb != (&entry_exit_blocks[0]); bb = bb->prev_bb)
 {
   ...
   if (bb->next_bb != (&entry_exit_blocks[1]))
     ...
     char *visited;
{
 ...
 continue;
      ...
      else if (((comp[pred_bb->index])->elms [(expr_index) / ((unsigned) (8 * 8))] >> (expr_index) % ((unsigned) (8 * 8)) & 1))
 break;
      else if (! ((transp[pred_bb->index])->elms [(expr_index) / ((unsigned) (8 * 8))] >> (expr_index) % ((unsigned) (8 * 8)) & 1))
 break;
...
hoist_code ()
{
      ...
      for (i = 0; i < hoist_vbeout[bb->index]->n_bits; i++)
 {
   ...
   if (((hoist_vbeout[bb->index])->elms [(i) / ((unsigned) (8 * 8))] >> (i) % ((unsigned) (8 * 8)) & 1)
       && ((transpout[bb->index])->elms [(i) / ((unsigned) (8 * 8))] >> (i) % ((unsigned) (8 * 8)) & 1))
     {
       ...
       for (j = 0; j < domby_len; j++)
  {
    ...
    if (!((antloc[dominated->index])->elms [(i) / ((unsigned) (8 * 8))] >> (i) % ((unsigned) (8 * 8)) & 1))
      continue;
...
# 6237 "../../../kgccfe/gnu/gcse.c"
       if (hoistable > 1)
  {
    ((hoist_exprs[bb->index])->elms [(i) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (i) % ((unsigned) (8 * 8)));
    found = 1;
  }
     }
 }
    ...
    if (hoist_expr_reaches_here_p (bb, i, dominated, ((void *)0)))
      {
        ...
        while ((((occr->insn)->fld[3]).bb) != dominated && occr)
   occr = occr->next;


        if (!occr)
   fancy_abort ("../../../kgccfe/gnu/gcse.c", 6295, __FUNCTION__);

        insn = occr->insn;

        set = (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i') ? (((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == SET ? (((insn)->fld[5]).rtx) : single_set_2 (insn, (((insn)->fld[5]).rtx))) : (rtx) 0);
        if (! set)
   fancy_abort ("../../../kgccfe/gnu/gcse.c", 6301, __FUNCTION__);
...
static __inline__ struct ls_expr *
first_ls_expr ()
{
     ...
     rtx x;
{
  ...
  if (!rtx_varies_p ((((x)->fld[0]).rtx), 0))
    return 1;

  return 0;
}
# 6556 "../../../kgccfe/gnu/gcse.c"
static void
     ...
     rtx x;
{
  ...
  for (i = (rtx_length[(int) (((enum rtx_code) (x)->code))]) - 1; i >= 0; i--)
    {
      if (fmt[i] == 'e')
 invalidate_any_buried_refs ((((x)->fld[i]).rtx));
      else if (fmt[i] == 'E')
 for (j = (((((x)->fld[i]).rtvec))->num_elem) - 1; j >= 0; j--)
   invalidate_any_buried_refs ((((((x)->fld[i]).rtvec))->elem[j]));
    }
}
...
compute_ld_motion_mems ()
{
    ...
    insn = (((insn)->fld[2]).rtx))
 {
   if ((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i')
     {
       if (((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == SET)
  {
    rtx src = ((((((insn)->fld[5]).rtx))->fld[1]).rtx);
  ...
  invalidate_any_buried_refs ((((insn)->fld[5]).rtx));
     }
 }
    }
}
...
trim_ld_motion_mems ()
{
  struct ls_expr * last = ((void *)0);
  struct ls_expr * ptr = first_ls_expr ();

  while (ptr != ((void *)0))
    {
      int del = ptr->invalid;
      struct expr * expr = ((void *)0);
     ...
     struct expr * expr;
{
  ...
  if ((mem_ptr = find_rtx_in_ldst (expr->expr)))
    {
# 6735 "../../../kgccfe/gnu/gcse.c"
      rtx list = mem_ptr->stores;

      for ( ; list != (rtx) 0; list = (((list)->fld[1]).rtx))
 {
   rtx insn = (((list)->fld[0]).rtx);
   rtx pat = (((insn)->fld[5]).rtx);
   rtx src = (((pat)->fld[1]).rtx);
   rtx reg = expr->reaching_reg;
   rtx copy, new;
   ...
   copy = gen_move_insn ( reg, (((pat)->fld[1]).rtx));
   new = emit_insn_before (copy, insn);
   record_one_set ((((reg)->fld[0]).rtuint), new);
   (((pat)->fld[1]).rtx) = reg;
     ...
     void * data __attribute__ ((__unused__));
{
  ...
  if (((enum rtx_code) (dest)->code) == REG)
    ((*regvec)->elms [((((dest)->fld[0]).rtuint)) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << ((((dest)->fld[0]).rtuint)) % ((unsigned) (8 * 8)));
}
     ...
     basic_block bb;
{
      ...
      else if (fmt[i] == 'E')
 {
   ...
   for (j = 0; j < (((((x)->fld[i]).rtvec))->num_elem); j++)
     {
       if (! store_ops_ok ((((((x)->fld[i]).rtvec))->elem[j]), bb))
  return 0;
     }
 }
    }
...
{
  struct ls_expr * ptr;
  rtx dest = (((insn)->fld[5]).rtx);

  if (((enum rtx_code) (dest)->code) != SET
      || ((enum rtx_code) ((((dest)->fld[1]).rtx))->code) == ASM_OPERANDS)
    return;
  ...
  if (((enum rtx_code) ((((dest)->fld[0]).rtx))->code) != SYMBOL_REF)
      return;

  if (rtx_varies_p ((((dest)->fld[0]).rtx), 0))
    return;
...
compute_store_table ()
{
    ...
    insn = (((insn)->fld[1]).rtx))
 {
  ...
  if (clobbers_all
      || (!!((regs_invalidated_by_call)[(regno) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((regno) % ((unsigned) (8 * 8)))))))
    ((reg_set_in_block[bb->index])->elms [(regno) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (regno) % ((unsigned) (8 * 8)));
     }
...
load_kills_store (x, store_pattern)
     rtx x, store_pattern;
{
  if (true_dependence (x, ((enum machine_mode) (x)->mode), store_pattern, rtx_addr_varies_p))
    return 1;
  return 0;
}
     ...
     rtx x, store_pattern;
{
 ...
 return 1;
  ...
  for (i = (rtx_length[(int) (((enum rtx_code) (x)->code))]) - 1; i >= 0 && !ret; i--)
    {
      if (fmt[i] == 'e')
 ret |= find_loads ((((x)->fld[i]).rtx), store_pattern);
      else if (fmt[i] == 'E')
 for (j = (((((x)->fld[i]).rtvec))->num_elem) - 1; j >= 0; j--)
   ret |= find_loads ((((((x)->fld[i]).rtvec))->elem[j]), store_pattern);
    }
  return ret;
}
  ...
  if (((enum rtx_code) (insn)->code) == CALL_INSN)
    {
      ...
      return ! (((insn))->unchanging) || pure_call_p (insn);
    }
  ...
  if (((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == SET)
    {
      ...
      if (((enum rtx_code) ((((pat)->fld[0]).rtx))->code) == MEM && !expr_equiv_p ((((pat)->fld[0]).rtx), x))

 if (find_loads ((((pat)->fld[0]).rtx), x))
   return 1;
      return find_loads ((((pat)->fld[1]).rtx), x);
    }
  else
    return find_loads ((((insn)->fld[5]).rtx), x);
}
     ...
     basic_block bb;
{
  ...
  if (!store_ops_ok ((((x)->fld[0]).rtx), bb))
    return 1;

  for ( ; insn && insn != (((last)->fld[2]).rtx); insn = (((insn)->fld[2]).rtx))
     ...
     basic_block bb;
{
  ...
  for ( ; insn && insn != (((first)->fld[1]).rtx); insn = (((insn)->fld[1]).rtx))
    if (store_killed_in_insn (x, insn))
      return 1;
  ...
  for (ptr = first_ls_expr (); ptr != ((void *)0); ptr = next_ls_expr (ptr))
    {
      ...
      for (st = store_list; st != ((void *)0); st = (((st)->fld[1]).rtx))
 {
   insn = (((st)->fld[0]).rtx);
   bb = (((insn)->fld[3]).bb);
   ...
   if (!store_killed_after (ptr->pattern, insn, bb))
     {
       ...
       if (((ae_gen[bb->index])->elms [(ptr->index) / ((unsigned) (8 * 8))] >> (ptr->index) % ((unsigned) (8 * 8)) & 1))
  {

    rtx st;
    for (st = ((ptr)->stores); st ; st = (((st)->fld[1]).rtx))
      if (((((((st)->fld[0]).rtx))->fld[3]).bb) == bb)
        break;
    if (st)
      {
   ...
   fprintf (gcse_file, "Removing redundant store:\n");
        replace_store_insn (r, (((st)->fld[0]).rtx), bb);
        (((st)->fld[0]).rtx) = insn;
        continue;
      }
  }
       ((ae_gen[bb->index])->elms [(ptr->index) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (ptr->index) % ((unsigned) (8 * 8)));
       ((ptr)->stores) = alloc_INSN_LIST (insn,
       ((ptr)->stores));
     }

   if (!store_killed_before (ptr->pattern, insn, bb))
     {
       ((st_antloc[((((insn)->fld[3]).bb)->index + 0)])->elms [(ptr->index) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (ptr->index) % ((unsigned) (8 * 8)));
       ((ptr)->loads) = alloc_INSN_LIST (insn,
       ((ptr)->loads));
     }
 }
  ...
  for (ptr = first_ls_expr (); ptr != ((void *)0); ptr = next_ls_expr (ptr))
    for (b = (&entry_exit_blocks[0])->next_bb; b != (&entry_exit_blocks[1]); b = b->next_bb)
      {
 if (store_killed_after (ptr->pattern, b->head, b))
   {
# 7213 "../../../kgccfe/gnu/gcse.c"
     ((ae_kill[b->index])->elms [(ptr->index) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (ptr->index) % ((unsigned) (8 * 8)));
   }
 else
   ((transp[b->index])->elms [(ptr->index) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (ptr->index) % ((unsigned) (8 * 8)));
      }
     ...
     basic_block bb;
{
  ...
  rtx prev = (((bb->head)->fld[1]).rtx);
  rtx before = bb->head;
  while (before != 0)
    {
      if (((enum rtx_code) (before)->code) != CODE_LABEL
   && (((enum rtx_code) (before)->code) != NOTE
       || (((before)->fld[5]).rtint) != NOTE_INSN_BASIC_BLOCK))
 break;
      prev = before;
      if (prev == bb->end)
 break;
      before = (((before)->fld[2]).rtx);
    }
  ...
  for (tmp = e->dest->pred; tmp ; tmp = tmp->pred_next)
    {
      ...
      if (index == -1)
 fancy_abort ("../../../kgccfe/gnu/gcse.c", 7297, __FUNCTION__);
      if (! ((pre_insert_map[index])->elms [(expr->index) / ((unsigned) (8 * 8))] >> (expr->index) % ((unsigned) (8 * 8)) & 1))
 break;
    }



  if (!tmp && bb != (&entry_exit_blocks[1]))
    {
      for (tmp = e->dest->pred; tmp ; tmp = tmp->pred_next)
 {
   int index = (find_edge_index ((edge_list), (tmp->src), (tmp->dest)));
   ((pre_insert_map[index])->elms [(expr->index) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (expr->index) % ((unsigned) (8 * 8))));
 }
      insert_insn_start_bb (insn, bb);
      return 0;
    }
     ...
     basic_block bb;
{
  ...
  insn = gen_move_insn (reg, ((((((del)->fld[5]).rtx))->fld[1]).rtx));
     ...
     basic_block bb;
{
  ...
  for (i = ((expr)->stores); i; i = (((i)->fld[1]).rtx))
    {
      del = (((i)->fld[0]).rtx);
      if ((((del)->fld[3]).bb) == bb)
 {
...
store_motion ()
{
  ...
  for (ptr = first_ls_expr (); ptr != ((void *)0); ptr = next_ls_expr (ptr))
    {
      for (bb = (&entry_exit_blocks[0])->next_bb; bb != (&entry_exit_blocks[1]); bb = bb->next_bb)
 if (((pre_delete_map[bb->index])->elms [(ptr->index) / ((unsigned) (8 * 8))] >> (ptr->index) % ((unsigned) (8 * 8)) & 1))
   delete_store (ptr, bb);

      for (x = 0; x < ((edge_list)->num_edges); x++)
 if (((pre_insert_map[x])->elms [(ptr->index) / ((unsigned) (8 * 8))] >> (ptr->index) % ((unsigned) (8 * 8)) & 1))
...
# 1 "../../../kgccfe/gnu/MIPS/gt-gcse.h" 1
# 25 "../../../kgccfe/gnu/MIPS/gt-gcse.h"
