...
# 34 "../../../kgccfe/gnu/genopinit.c" 2
# 65 "../../../kgccfe/gnu/genopinit.c"
     ...
     rtx insn;
{
    ...
    pp++)
 {
  ...
  break;
       case 'V':
                break;
       case 'c':
  for (op = 0; op < ((int) LAST_AND_UNUSED_RTX_CODE); op++)
    {
      for (p = (rtx_name[(int) (op)]), q = np; *p; p++, q++)
        if (*p != *q)
   break;
      ...
      if (*p == 0 && *q == 0 && (rtx_class[(int) (op)]) == '<')
        break;
    }

  if (op == ((int) LAST_AND_UNUSED_RTX_CODE))
    matches = 0;
  else
    np += strlen ((rtx_name[(int) (op)]));
  break;
       case 'a':
       case 'b':
  ...
  for (i = ((int) MAX_MACHINE_MODE) - 1; i >= 0; i--)
    {
      for (p = (mode_name[(int) (i)]), q = np; *p; p++, q++)
        if (_sch_tolower[(*p) & 0xff] != *q)
   break;
  ...
  if (i < 0)
    ...
    m1 = i, np += strlen ((mode_name[(int) (i)]));
  else
    m2 = i, np += strlen ((mode_name[(int) (i)]));

  force_int = force_partial_int = force_float = 0;
  break;

       default:
  fancy_abort ("../../../kgccfe/gnu/genopinit.c", 252, __FUNCTION__);
       }
 }

      if (matches && pp[0] == '$' && pp[1] == ')'
   && *np == 0
   && (! force_consec || (int) ((enum machine_mode)mode_wider_mode[(int) (m1)]) == m2))
 break;
    }
 ...
 switch (*++pp)
   ...
   case 'I': case 'F': case 'N':
     break;
   case 'V':
            if ((mode_class[(int) (m1)]) == MODE_FLOAT)
              printf ("v");
            break;
   case 'a':
     for (np = (mode_name[(int) (m1)]); *np; np++)
       putchar (_sch_tolower[(*np) & 0xff]);
     break;
   case 'b':
     for (np = (mode_name[(int) (m2)]); *np; np++)
       putchar (_sch_tolower[(*np) & 0xff]);
     break;
   case 'A':
     printf ("(int) %smode", (mode_name[(int) (m1)]));
     break;
   case 'B':
     printf ("(int) %smode", (mode_name[(int) (m2)]));
     break;
   case 'c':
     printf ("%s", (rtx_name[(int) (op)]));
     break;
   case 'C':
     printf ("(int) ");
     for (np = (rtx_name[(int) (op)]); *np; np++)
       putchar (_sch_toupper[(*np) & 0xff]);
     break;
   }
    }
