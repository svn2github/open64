...
static void write_units (int, struct range *, struct range *, struct range *, struct range *, struct range *);
...
{
  const char *p, *tag;
  int is_const = ((enum rtx_code) ((((attr)->fld[2]).rtx))->code) == CONST;
   ...
   if (*p == ',')
     fputs_unlocked (", ", stdout);
      ...
      printf ("extern enum attr_%s get_attr_%s PARAMS ((%s));\n\n",
       (((attr)->fld[0]).rtstr), (((attr)->fld[0]).rtstr), (is_const ? "void" : "rtx"));
    }
     ...
     char **argv;
{
 ...
 break;
      ...
      if (((enum rtx_code) (desc)->code) == DEFINE_ATTR)
 gen_attr (desc);

      else if (((enum rtx_code) (desc)->code) == DEFINE_DELAY)
        {
   if (! have_delay)
     {
   ...
   for (i = 0; i < (((((desc)->fld[1]).rtvec))->num_elem); i += 3)
     {
       if ((((((desc)->fld[1]).rtvec))->elem[i + 1]) && ! have_annul_true)
  {
    ...
    have_annul_true = 1;
       ...
       if ((((((desc)->fld[1]).rtvec))->elem[i + 2]) && ! have_annul_false)
  {
    ...
    have_annul_false = 1;
      ...
      else if (((enum rtx_code) (desc)->code) == DEFINE_FUNCTION_UNIT)
 {
   const char *name = (((desc)->fld[0]).rtstr);
  ...
  xmalloc (sizeof (struct function_unit));
   ...
   unit->issue_delay.min, unit->issue_delay.max);
      ...
      else if (((enum rtx_code) (desc)->code) == DEFINE_INSN_RESERVATION)
...
# 334 "../../../kgccfe/gnu/genattr.c"
      for (unit = units; unit; unit = unit->next)
 {
   struct range blockage;

   blockage = unit->issue_delay;
   blockage.max = ((unit->ready_cost.max - (unit->ready_cost.min - 1)) > (blockage.max) ? (unit->ready_cost.max - (unit->ready_cost.min - 1)) : (blockage.max));
   ...
   if (unit->simultaneity != 0)
     {
       ...
       blockage.min = ((unit->ready_cost.min - fill_time) > (blockage.min) ? (unit->ready_cost.min - fill_time) : (blockage.min));

       blockage.max = ((unit->ready_cost.max - fill_time) > (blockage.max) ? (unit->ready_cost.max - fill_time) : (blockage.max));
