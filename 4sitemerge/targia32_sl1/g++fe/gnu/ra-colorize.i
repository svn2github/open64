...
# 32 "../../../kg++fe/gnu/ra-colorize.c" 2
# 48 "../../../kg++fe/gnu/ra-colorize.c"
...
{
  if (x->next || x->prev)
    fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 108, __FUNCTION__);
  x->next = *list;
  if (*list)
    (*list)->prev = x;
  *list = x;
...
{
  if (x->prev || x->next)
    fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 121, __FUNCTION__);
  if (!*list)
    {
      *list = x;
      return;
     ...
     enum node_type type;
{
 ...
 break;
      default:
 fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 205, __FUNCTION__);
    }
  web->type = type;
}
...
reset_lists ()
{
  ...
  if ((web_lists[(int)(SIMPLIFY)]) || (web_lists[(int)(SIMPLIFY_SPILL)]) || (web_lists[(int)(SIMPLIFY_FAT)])
    ...
    fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 223, __FUNCTION__);
 ...
 fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 258, __FUNCTION__);
    }
     ...
     enum node_type type;
{
  if (type == PRECOLORED)
    ...
    fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 278, __FUNCTION__);
     ...
     enum move_type type;
{
 ...
 break;
      default:
 fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 321, __FUNCTION__);
    }
  move->type = type;
}
     ...
     struct move *move;
{
  ...
  for (ml = web->moves; ml; ml = ml->next)
    if (ml->move == move)
      fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 525, __FUNCTION__);
}
     ...
     struct web *u, *v;
{
  ...
  if (u == v || v->type == COALESCED)
    fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 727, __FUNCTION__);
  if ((u->regno >= max_normal_pseudo) != (v->regno >= max_normal_pseudo))
    fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 729, __FUNCTION__);
...
# 815 "../../../kg++fe/gnu/ra-colorize.c"
  ...
  if (!u->num_freedom)
    fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 826, __FUNCTION__);
...
select_spill ()
{
      ...
      else if ((w->spill_temp == 2 || w->is_coalesced) && cost < best2)
 {
  ...
  if (!bestd)
    fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 1002, __FUNCTION__);
      ...
      if (hard && (!web->was_spilled || web->spill_temp))
 {
       ...
       if (try->type == COALESCED)
  {
    if (alias (try)->type != PRECOLORED)
      fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 1580, __FUNCTION__);
     ...
     struct web *web;
{
 ...
 fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 1849, __FUNCTION__);
...
# 1881 "../../../kg++fe/gnu/ra-colorize.c"
      ;
    else
      fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 1883, __FUNCTION__);
  }
     }
 }
      free (old_colors);
      ...
      for (wl = web->conflict_list; wl; wl = wl->next)
 {
  ...
  tweb = hardreg2web[i + aweb->color];
    ...
    && hard_regs_intersect_p (&tweb->usable_regs,
         &wl->t->usable_regs))
  fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 1934, __FUNCTION__);
# 1943 "../../../kg++fe/gnu/ra-colorize.c"
       if (aweb->type != PRECOLORED)
  break;
     }
 }
...
check_colors ()
{
 ...
 fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 2025, __FUNCTION__);

      for (c = 0; c < nregs; c++)
 if (!(!!((web->usable_regs)[(aweb->color + c) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((aweb->color + c) % ((unsigned) (8 * 8)))))))
   fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 2029, __FUNCTION__);
 ...
 if (wl->t->regno >= max_normal_pseudo)
 ...
 else if (!wl->sub)
   {
     ...
     if (aweb->color >= web2->color + nregs2
       ...
       continue;
     fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 2053, __FUNCTION__);
   }
 else
   {
     struct sub_conflict *sl;
  ...
  if ((tcol + tofs >= scol + sofs + ssize)
    ...
    continue;
  fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 2077, __FUNCTION__);
       }
   }
    }
}
     ...
     struct web *web;
{
  struct dlist *d, *d_next;
  if (web->type != SPILLED)
    fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 2121, __FUNCTION__);
     ...
     struct web *web;
{
  ...
  if (pre->type != PRECOLORED)
    fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 2167, __FUNCTION__);
  unalias_web (web);
      ...
      else
 {
   ...
   *pcl = wl->next;
   if (!other->have_orig_conflicts && other->type != PRECOLORED)
     fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 2261, __FUNCTION__);
   for (owl = other->orig_conflict_list; owl; owl = owl->next)
     if (owl->t == web)
       break;
   if (owl)
     fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 2266, __FUNCTION__);
       ...
       else
  {
   ...
   if (!owl && other->type != PRECOLORED)
     fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 2282, __FUNCTION__);
     ...
     int for_move;
{
  ...
  for (p = web_pair_list, i = 0; p; p = p->next_list)
    sorted[i++] = p;
  if (i != num_web_pairs)
    fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 2486, __FUNCTION__);
...
check_uncoalesced_moves ()
{
 ...
 if (s != t
  ...
  || s->type != PRECOLORED)
     && !((sup_igraph)->elms [(s->id * num_webs + t->id) / ((unsigned) (8 * 8))] >> (s->id * num_webs + t->id) % ((unsigned) (8 * 8)) & 1)
     && !((sup_igraph)->elms [(t->id * num_webs + s->id) / ((unsigned) (8 * 8))] >> (t->id * num_webs + s->id) % ((unsigned) (8 * 8)) & 1))
   fancy_abort ("../../../kg++fe/gnu/ra-colorize.c", 2655, __FUNCTION__);
      }
}
