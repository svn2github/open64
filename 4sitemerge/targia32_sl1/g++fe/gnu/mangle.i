...
# 60 "../../../kg++fe/gnu/cp/mangle.c" 2
# 1 "../../../include/gnu/obstack.h" 1
# 61 "../../../kg++fe/gnu/cp/mangle.c" 2
...
# 64 "../../../kg++fe/gnu/cp/mangle.c" 2
# 96 "../../../kg++fe/gnu/cp/mangle.c"
...
static int decl_is_template_id (tree, tree*);



static __inline__ tree canonicalize_for_substitution (tree);
static void add_substitution (tree);
static __inline__ int is_std_substitution (tree, substitution_identifier_index_t);
static __inline__ int is_std_substitution_char (tree, substitution_identifier_index_t);
static int find_substitution (tree);



static void write_mangled_name (tree);
static void write_encoding (tree);
static void write_name (tree, int);
static void write_unscoped_name (tree);
static void write_unscoped_template_name (tree);
static void write_nested_name (tree);
static void write_prefix (tree);
static void write_template_prefix (tree);
static void write_unqualified_name (tree);
static void write_conversion_operator_name (tree);
static void write_source_name (tree);
static int hwint_to_ascii (unsigned long long, unsigned int, char *, unsigned);
static void write_number (unsigned long long, int, unsigned int);

static void write_integer_cst (tree);
static void write_real_cst (tree);
static void write_identifier (const char *);
static void write_special_name_constructor (tree);
static void write_special_name_destructor (tree);
static void write_type (tree);
static int write_CV_qualifiers_for_type (tree);
static void write_builtin_type (tree);
static void write_function_type (tree);
static void write_bare_function_type (tree, int, tree);
static void write_method_parms (tree, int, tree);
static void write_class_enum_type (tree);
static void write_template_args (tree);
static void write_expression (tree);
static void write_template_arg_literal (tree);
static void write_template_arg (tree);
static void write_template_template_arg (tree);
static void write_array_type (tree);
static void write_pointer_to_member_type (tree);
static void write_template_param (tree);
static void write_template_template_param (tree);
static void write_substitution (int);
static int discriminator_for_local_entity (tree);
static int discriminator_for_string_literal (tree, tree);
static void write_discriminator (int);
static void write_local_name (tree, tree, tree);
static void dump_substitution_candidates (void);
static const char *mangle_decl_string (tree);



static __inline__ void start_mangling (tree);
static __inline__ const char *finish_mangling (_Bool);
static tree mangle_special_for_type (tree, const char *);



static void write_java_integer_type_codes (tree);
# 248 "../../../kg++fe/gnu/cp/mangle.c"
static int
  ...
  if (((enum tree_code) (decl)->common.code) == TYPE_DECL)
    {
      ...
      if ((((((enum tree_code) (type)->common.code)) == RECORD_TYPE || (((enum tree_code) (type)->common.code)) == UNION_TYPE) && (((enum tree_code) (type)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (type)->common.code) == TYPENAME_TYPE || ((enum tree_code) (type)->common.code) == TYPEOF_TYPE || ((enum tree_code) (type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((type)->type.lang_flag_5))) && (((type)->type.lang_specific) != ((void *)0) && (((enum tree_code) (type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || (((&(((type))->type.lang_specific)->u.c)->template_info) != ((void *)0) && ((((((((((((((&(((type))->type.lang_specific)->u.c)->template_info))->list.purpose)))->decl.arguments))->list.value))->common.type)) == ((((((&(((type))->type.lang_specific)->u.c)->template_info))->list.purpose)))))))))
 {
   if (template_info != ((void *)0))


     *template_info = (((enum tree_code) (type)->common.code) == ENUMERAL_TYPE ? ((((type))->type.binfo)) : (((enum tree_code) (type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM ? ((&(((type))->type.lang_specific)->u.c) ->template_info) : (((type)->type.lang_specific) ? ((&(((type))->type.lang_specific)->u.c)->template_info) : (tree) ((void *)0))));
   return 1;
 }
    }
  else
    {
      ...
      if (((decl)->decl.lang_specific) != ((void *)0)
   && (((decl)->decl.lang_specific)->decl_flags.use_template)
   && (((((((((((((((decl))->decl.lang_specific) ->decl_flags.u.template_info))->list.purpose)))->decl.arguments))->list.value))->common.type)) == ((((((((decl))->decl.lang_specific) ->decl_flags.u.template_info))->list.purpose))))
   && ((enum tree_code) (decl)->common.code) != TEMPLATE_DECL)
 {
   if (template_info != ((void *)0))


     *template_info = ((((decl))->decl.lang_specific) ->decl_flags.u.template_info);
   return 1;
 }
    }
...
dump_substitution_candidates ()
{
  ...
  for (i = 0; i < ((G.substitutions)->elements_used); ++i)
    {
      tree el = ((G.substitutions)->data.tree[i]);
      const char *name = "???";

      if (i > 0)
 fprintf (stderr, "                    ");
      if ((tree_code_type[(int) (((enum tree_code) (el)->common.code))] == 'd'))
 name = ((char *)(((el)->decl.name))->identifier.id.str);
      else if (((enum tree_code) (el)->common.code) == TREE_LIST)
 name = ((char *)(((((el)->list.value))->decl.name))->identifier.id.str);
      else if (((el)->type.name))
 name = ((char *)(((((el)->type.name))->decl.name))->identifier.id.str);
      fprintf (stderr, " S%d_ = ", i - 1);
      if ((tree_code_type[(int) (((enum tree_code) (el)->common.code))] == 't') &&
   (((cp_type_quals (el) & 0x4) != 0)
    || ((cp_type_quals (el) & 0x2) != 0)
    || ((cp_type_quals (el) & 0x1) != 0)))
 fprintf (stderr, "CV-");
      fprintf (stderr, "%s (%s at %p)\n",
        name, tree_code_name[((enum tree_code) (el)->common.code)], (void *) el);
    }
}
# 327 "../../../kg++fe/gnu/cp/mangle.c"
     ...
     tree node;
{
  ...
  if (0)
    fprintf (stderr, "  ++ add_substitution (%s at %10p)\n",
      tree_code_name[((enum tree_code) (node)->common.code)], (void *) node);
  ...
  if (0 && c != node)
    fprintf (stderr, "  ++ using candidate (%s at %10p)\n",
      tree_code_name[((enum tree_code) (node)->common.code)], (void *) node);
  node = c;
# 378 "../../../kg++fe/gnu/cp/mangle.c"
  do { if ((G.substitutions)->elements_used >= (G.substitutions)->num_elements) (((G.substitutions)) = varray_grow ((G.substitutions), 2 * (G.substitutions)->num_elements)); (G.substitutions)->data.tree[(G.substitutions)->elements_used++] = (node); } while (0);
     ...
     substitution_identifier_index_t index;
{
  ...
  else if ((((((enum tree_code) (node)->common.code)) == RECORD_TYPE || (((enum tree_code) (node)->common.code)) == UNION_TYPE) && (((enum tree_code) (node)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (node)->common.code) == TYPENAME_TYPE || ((enum tree_code) (node)->common.code) == TYPEOF_TYPE || ((enum tree_code) (node)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((node)->type.lang_flag_5))))
    {
      type = node;
      decl = ((node)->type.name);
    }
  ...
  return ((((enum tree_code) ((((decl)->decl.context) ? ((decl)->decl.context) : global_namespace))->common.code) == NAMESPACE_DECL && ((((((decl)->decl.context) ? ((decl)->decl.context) : global_namespace))->decl.context) ? (((((decl)->decl.context) ? ((decl)->decl.context) : global_namespace))->decl.context) : global_namespace) == global_namespace && (((((decl)->decl.context) ? ((decl)->decl.context) : global_namespace))->decl.name) == cp_global_trees[CPTI_STD_IDENTIFIER])
   && ((type)->type.lang_specific)
   && (((enum tree_code) (type)->common.code) == ENUMERAL_TYPE ? ((((type))->type.binfo)) : (((enum tree_code) (type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM ? ((&(((type))->type.lang_specific)->u.c) ->template_info) : (((type)->type.lang_specific) ? ((&(((type))->type.lang_specific)->u.c)->template_info) : (tree) ((void *)0))))
   && ((((((((((enum tree_code) (type)->common.code) == ENUMERAL_TYPE ? ((((type))->type.binfo)) : (((enum tree_code) (type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM ? ((&(((type))->type.lang_specific)->u.c) ->template_info) : (((type)->type.lang_specific) ? ((&(((type))->type.lang_specific)->u.c)->template_info) : (tree) ((void *)0)))))->list.purpose))))->decl.name)
     ...
     substitution_identifier_index_t index;
{
  ...
  if ((tree_code_type[(int) (((enum tree_code) (node)->common.code))] == 'd'))
    args = (((((((node))->decl.lang_specific) ->decl_flags.u.template_info))->list.value));
  else if ((((((enum tree_code) (node)->common.code)) == RECORD_TYPE || (((enum tree_code) (node)->common.code)) == UNION_TYPE) && (((enum tree_code) (node)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (node)->common.code) == TYPENAME_TYPE || ((enum tree_code) (node)->common.code) == TYPEOF_TYPE || ((enum tree_code) (node)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((node)->type.lang_flag_5))))
    args = (((((&(((node))->type.lang_specific)->u.c)->template_info))->list.value));
  else
  ...
  return
    ((args)->vec.length) == 1
    && ((args)->vec.a[0]) == integer_types[itk_char];
}
# 479 "../../../kg++fe/gnu/cp/mangle.c"
static int
find_substitution (node)
     tree node;
{
  int i;
  int size = ((G.substitutions)->elements_used);
  tree decl;
  tree type;
  ...
  decl = (tree_code_type[(int) (((enum tree_code) (node)->common.code))] == 't') ? ((node)->type.name) : node;
  type = (tree_code_type[(int) (((enum tree_code) (node)->common.code))] == 't') ? node : ((node)->common.type);


  if (decl
      && is_std_substitution (decl, SUBID_ALLOCATOR)
      && !((&((((decl)->common.type))->type.lang_specific)->u.c)->use_template))
    {
      __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("Sa")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("Sa"))), (__len)); __o->next_free += __len; (void) 0; });
      return 1;
    }
      ...
      if ((tree_code_type[(int) (((enum tree_code) (node)->common.code))] == 't'))
 {
   ...
   if (cp_type_quals (type) == 0x0
     ...
     {
       tree args = (((((&(((type))->type.lang_specific)->u.c)->template_info))->list.value));
       if (((args)->vec.length) == 3
    && comptypes ((((args)->vec.a[0])), (integer_types[itk_char]), 0)
    && is_std_substitution_char (((args)->vec.a[1]),
            SUBID_CHAR_TRAITS)
    && is_std_substitution_char (((args)->vec.a[2]),
            SUBID_ALLOCATOR))
  {
    __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("Ss")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("Ss"))), (__len)); __o->next_free += __len; (void) 0; });
    return 1;
  }
     }
 }
 ...
 {
   __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("Sb")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("Sb"))), (__len)); __o->next_free += __len; (void) 0; });
   return 1;
 }
    }
  ...
  if ((tree_code_type[(int) (((enum tree_code) (node)->common.code))] == 't')
      ...
      && ((&((type)->type.lang_specific)->u.c)->use_template)
      && ((&(((type))->type.lang_specific)->u.c)->template_info) != ((void *)0))
    {


      tree args = (((((&(((type))->type.lang_specific)->u.c)->template_info))->list.value));
      if (((args)->vec.length) == 2
   && comptypes ((((args)->vec.a[0])), (integer_types[itk_char]), 0)
   && is_std_substitution_char (((args)->vec.a[1]),
           SUBID_CHAR_TRAITS))
 {

   tree name = (((((((&(((type))->type.lang_specific)->u.c)->template_info))->list.purpose)))->decl.name);
   if (name == subst_identifiers[SUBID_BASIC_ISTREAM])
     {
       __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("Si")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("Si"))), (__len)); __o->next_free += __len; (void) 0; });
       return 1;
     }

   else if (name == subst_identifiers[SUBID_BASIC_OSTREAM])
     {
       __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("So")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("So"))), (__len)); __o->next_free += __len; (void) 0; });
       return 1;
     }

   else if (name == subst_identifiers[SUBID_BASIC_IOSTREAM])
     {
       __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("Sd")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("Sd"))), (__len)); __o->next_free += __len; (void) 0; });
       return 1;
     }
 }
    }
  ...
  if (decl && (((enum tree_code) (decl)->common.code) == NAMESPACE_DECL && (((decl)->decl.context) ? ((decl)->decl.context) : global_namespace) == global_namespace && ((decl)->decl.name) == cp_global_trees[CPTI_STD_IDENTIFIER]))
    {
      __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("St")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("St"))), (__len)); __o->next_free += __len; (void) 0; });
      return 1;
    }
  ...
  for (i = 0; i < size; ++i)
    {
      tree candidate = ((G.substitutions)->data.tree[i]);


      if (decl == candidate
   || ((tree_code_type[(int) (((enum tree_code) (candidate)->common.code))] == 't') && type && (tree_code_type[(int) (((enum tree_code) (type)->common.code))] == 't')
       && comptypes ((type), (candidate), 0))
   || (((enum tree_code) (node)->common.code) == TREE_LIST && ((enum tree_code) (candidate)->common.code) == TREE_LIST && (((tree_code_type[(int) (((enum tree_code) (((node)->list.purpose))->common.code))] == 't') && comptypes ((((node)->list.purpose)), (((candidate)->list.purpose)), 0)) || ((node)->list.purpose) == ((candidate)->list.purpose)) && ((node)->list.value) == ((candidate)->list.value)))
 {
     ...
     tree decl;
{
  ...
  if (((decl)->decl.lang_specific)
      && ((((enum tree_code) (decl)->common.code) == FUNCTION_DECL && !(((enum tree_code) (decl)->common.code) == FUNCTION_DECL && ((decl)->decl.lang_flag_7))) && ((((decl)->decl.lang_specific) ? ((decl)->decl.lang_specific)->decl_flags.language : (((enum tree_code) (decl)->common.code) == FUNCTION_DECL ? lang_c : lang_cplusplus)) == lang_c))
      && ! (((((decl)->decl.name))->common.lang_flag_2) ? ((decl)->decl.lang_specific)->u.f.operator_code : ERROR_MARK))





    write_source_name (((decl)->decl.name));
  else

    {
      __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("_Z")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("_Z"))), (__len)); __o->next_free += __len; (void) 0; });
      write_encoding (decl);
    }
}
     ...
     tree decl;
{
  ...
  if (((decl)->decl.lang_specific) && ((((enum tree_code) (decl)->common.code) == FUNCTION_DECL && !(((enum tree_code) (decl)->common.code) == FUNCTION_DECL && ((decl)->decl.lang_flag_7))) && ((((decl)->decl.lang_specific) ? ((decl)->decl.lang_specific)->decl_flags.language : (((enum tree_code) (decl)->common.code) == FUNCTION_DECL ? lang_c : lang_cplusplus)) == lang_c)))
    {
 ...
 fn_type = ((decl)->common.type);

      write_bare_function_type (fn_type,
    (!(((decl)->decl.lang_specific)->decl_flags.constructor_attr)
     && !(((decl)->decl.lang_specific)->decl_flags.destructor_attr)
     && !(((((decl)->decl.name))->common.lang_flag_4))
     && decl_is_template_id (decl, ((void *)0))),
    decl);
    }
}
# 685 "../../../kg++fe/gnu/cp/mangle.c"
static void
     ...
     int ignore_local_scope;
{
  ...
  if (((enum tree_code) (decl)->common.code) == TYPE_DECL)
    {
      ...
      decl = ((((((decl)->common.type))->type.main_variant))->type.name);
      context = ((((((decl)->common.type))->type.main_variant))->type.context);
    }
  else
    context = (((decl)->decl.context) == ((void *)0)) ? ((void *)0) : (((decl)->decl.context) ? ((decl)->decl.context) : global_namespace);
  ...
  if (context == ((void *)0)
      ...
      || (((enum tree_code) (context)->common.code) == NAMESPACE_DECL && (((context)->decl.context) ? ((context)->decl.context) : global_namespace) == global_namespace && ((context)->decl.name) == cp_global_trees[CPTI_STD_IDENTIFIER])
      || (ignore_local_scope && ((enum tree_code) (context)->common.code) == FUNCTION_DECL))
    {
      tree template_info;
      ...
      if (decl_is_template_id (decl, &template_info))
 {
   ...
   write_unscoped_template_name ((((template_info)->list.purpose)));
   write_template_args ((((template_info)->list.value)));
 }
      else
   ...
   while (context != ((void *)0) && context != global_namespace)
     {
       ...
       if (context != ((void *)0) && (tree_code_type[(int) (((enum tree_code) (context)->common.code))] == 't'))
  context = ((context)->type.name);

       if (((enum tree_code) (context)->common.code) == FUNCTION_DECL)
  {
    ...
    return;
       ...
       context = (((context)->decl.context) ? ((context)->decl.context) : global_namespace);
     }
     ...
     tree decl;
{
  ...
  if ((((enum tree_code) (context)->common.code) == NAMESPACE_DECL && (((context)->decl.context) ? ((context)->decl.context) : global_namespace) == global_namespace && ((context)->decl.name) == cp_global_trees[CPTI_STD_IDENTIFIER]))
    {
      __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("St")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("St"))), (__len)); __o->next_free += __len; (void) 0; });
    ...
    fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 790, __FUNCTION__);
}
     ...
     tree decl;
{
  ...
  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('N')); (void) 0; });


  if (((enum tree_code) (decl)->common.code) == FUNCTION_DECL
      && (((enum tree_code) (((decl)->common.type))->common.code) == METHOD_TYPE))
    {
      if (((((enum tree_code) (((decl)->common.type))->common.code) == METHOD_TYPE) && ((cp_type_quals (((((((((decl)->common.type))->type.values))->list.value))->common.type)) & 0x2) != 0)))
 __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('V')); (void) 0; });
      if (((((enum tree_code) (((decl)->common.type))->common.code) == METHOD_TYPE) && ((cp_type_quals (((((((((decl)->common.type))->type.values))->list.value))->common.type)) & 0x1) != 0)))
 __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('K')); (void) 0; });
    }
  ...
  if (decl_is_template_id (decl, &template_info))
  ...
  else
    {
      ...
      write_prefix (((decl)->decl.context));
      write_unqualified_name (decl);
    }
  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('E')); (void) 0; });
}
     ...
     tree node;
{
 ...
 return;
  ...
  else
    {
      ...
      decl = ((node)->type.name);
      if ((((node)->type.lang_specific) != ((void *)0) && (((enum tree_code) (node)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || (((&(((node))->type.lang_specific)->u.c)->template_info) != ((void *)0) && ((((((((((((((&(((node))->type.lang_specific)->u.c)->template_info))->list.purpose)))->decl.arguments))->list.value))->common.type)) == ((((((&(((node))->type.lang_specific)->u.c)->template_info))->list.purpose)))))))))
 template_info = (((enum tree_code) (node)->common.code) == ENUMERAL_TYPE ? ((((node))->type.binfo)) : (((enum tree_code) (node)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM ? ((&(((node))->type.lang_specific)->u.c) ->template_info) : (((node)->type.lang_specific) ? ((&(((node))->type.lang_specific)->u.c)->template_info) : (tree) ((void *)0))));
    }


  if (((enum tree_code) (node)->common.code) == TEMPLATE_TYPE_PARM
  ...
  if (((enum tree_code) (node)->common.code) == TEMPLATE_TYPE_PARM
    ...
    {
      write_prefix ((((decl)->decl.context) ? ((decl)->decl.context) : global_namespace));
      write_unqualified_name (decl);
    }
...
write_template_prefix (node)
     tree node;
{
  tree decl = (tree_code_type[(int) (((enum tree_code) (node)->common.code))] == 'd') ? node : ((node)->type.name);
  tree type = (tree_code_type[(int) (((enum tree_code) (node)->common.code))] == 'd') ? ((node)->common.type) : node;
  tree context = (((decl)->decl.context) ? ((decl)->decl.context) : global_namespace);
  tree template_info;
  tree template;
  tree substitution;
  ...
  if (decl_is_template_id (decl, &template_info))
    template = (((template_info)->list.purpose));
  else if ((((type)->type.lang_specific) != ((void *)0) && (((enum tree_code) (type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || (((&(((type))->type.lang_specific)->u.c)->template_info) != ((void *)0) && ((((((((((((((&(((type))->type.lang_specific)->u.c)->template_info))->list.purpose)))->decl.arguments))->list.value))->common.type)) == ((((((&(((type))->type.lang_specific)->u.c)->template_info))->list.purpose)))))))))
    template = (((((((enum tree_code) (type)->common.code) == ENUMERAL_TYPE ? ((((type))->type.binfo)) : (((enum tree_code) (type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM ? ((&(((type))->type.lang_specific)->u.c) ->template_info) : (((type)->type.lang_specific) ? ((&(((type))->type.lang_specific)->u.c)->template_info) : (tree) ((void *)0)))))->list.purpose)));
  else

    fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 943, __FUNCTION__);
# 967 "../../../kg++fe/gnu/cp/mangle.c"
  if ((tree_code_type[(int) (((enum tree_code) (context)->common.code))] == 't'))
  ...
  if (((enum tree_code) (((template)->common.type))->common.code) == TEMPLATE_TEMPLATE_PARM
      && !(flag_abi_version == 0 || flag_abi_version >= (2)))
    G.need_abi_warning = 1;

  if (((enum tree_code) (((template)->common.type))->common.code) == TEMPLATE_TEMPLATE_PARM
      && (flag_abi_version == 0 || flag_abi_version >= (2)))
    write_template_param (((template)->common.type));
  else
    {
     ...
     tree decl;
{
  ...
  if (((decl)->decl.lang_specific) != ((void *)0) && (((decl)->decl.lang_specific)->decl_flags.constructor_attr))
    write_special_name_constructor (decl);
  else if (((decl)->decl.lang_specific) != ((void *)0) && (((decl)->decl.lang_specific)->decl_flags.destructor_attr))
    write_special_name_destructor (decl);
  else if ((((((decl)->decl.name))->common.lang_flag_4)))
    {
      ...
      if (decl_is_template_id (decl, ((void *)0)))
 {
   ...
   type = ((fn_type)->common.type);
 }
      else
 type = ((((((decl)->decl.name))->common.lang_flag_4)) ? ((((decl)->decl.name))->common.type) : (tree) ((void *)0));
      write_conversion_operator_name (type);
    }
  else if ((((((decl)->decl.name))->common.lang_flag_2) ? ((decl)->decl.lang_specific)->u.f.operator_code : ERROR_MARK))
    {
      operator_name_info_t *oni;
      if ((((decl)->decl.lang_specific)->decl_flags.assignment_operator_p))
 oni = assignment_operator_name_info;
      else
 oni = operator_name_info;

      __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen (oni[(((((decl)->decl.name))->common.lang_flag_2) ? ((decl)->decl.lang_specific)->u.f.operator_code : ERROR_MARK)].mangled_name)); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), (((oni[(((((decl)->decl.name))->common.lang_flag_2) ? ((decl)->decl.lang_specific)->u.f.operator_code : ERROR_MARK)].mangled_name))), (__len)); __o->next_free += __len; (void) 0; });
    }
  else
    write_source_name (((decl)->decl.name));
}
...
{
  __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("cv")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("cv"))), (__len)); __o->next_free += __len; (void) 0; });
  write_type (type);
}
     ...
     tree identifier;
{
  ...
  if ((((struct lang_identifier*)(identifier))->class_template_info))
    identifier = (((struct lang_identifier*)(identifier))->class_template_info);

  write_number ((((identifier)->identifier.id.len)), 1, 10);
  write_identifier (((char *)(identifier)->identifier.id.str));
}
     ...
     unsigned int base;
{
  ...
  if (!unsigned_p && (long long) number < 0)
    {
      __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('n')); (void) 0; });
      number = -((long long) number);
    }
  count = hwint_to_ascii (number, base, buffer + sizeof (buffer), 1);
  __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = ((count)); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), (((buffer + sizeof (buffer) - count))), (__len)); __o->next_free += __len; (void) 0; });
}
     ...
     tree cst;
{
  ...
  if ((((cst)->int_cst.int_cst).high) + (sign < 0))
    {


      char buffer[sizeof (long long) * 8 * 2];
      ...
      if (sizeof (long long) >= 8)
 {
   ...
   chunk *= chunk;
      ...
      type = c_common_signed_or_unsigned_type (1, ((cst)->common.type));
      base = build_int_2_wide ((unsigned long long) (chunk), (long long) (0));
      n = build_int_2_wide ((unsigned long long) ((((cst)->int_cst.int_cst).low)), (long long) ((((cst)->int_cst.int_cst).high)));
      ((n)->common.type) = ((base)->common.type) = type;
      ...
      do
 {
   ...
   c = hwint_to_ascii ((((tmp)->int_cst.int_cst).low), 10, ptr,
    done ? 1 : chunk_digits);
   ptr -= c;
   count += c;
   n = d;
      ...
      while (!done);
      __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = ((count)); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), (((ptr))), (__len)); __o->next_free += __len; (void) 0; });
    }
  else
    {

      unsigned long long low = (((cst)->int_cst.int_cst).low);
      ...
      if (sign < 0)
 {
   ...
   low = -low;
      ...
      write_number ((low), 1, 10);
    }
}
# 1214 "../../../kg++fe/gnu/cp/mangle.c"
static void
     ...
     tree value;
{
  if ((flag_abi_version == 0 || flag_abi_version >= (2)))
    {
      ...
      int words = (mode_bitsize[(int) (((type)->type.mode))]) / 32;

      real_to_target (target_real, &(*((value)->real_cst.real_cst_ptr)),
        ((type)->type.mode));
      ...
      for (; i != limit; i += dir)
 {
   ...
   __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = ((8)); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), (((buffer))), (__len)); __o->next_free += __len; (void) 0; });
 }
    }
  else
    {
      ...
      size_t i;
      for (i = 0; i < sizeof ((*((value)->real_cst.real_cst_ptr))); ++i)
 write_number (((unsigned char *) &(*((value)->real_cst.real_cst_ptr)))[i],
                       1,
                 16);
      G.need_abi_warning = 1;
    }
...
{
  ;
  __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen (identifier)); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), (((identifier))), (__len)); __o->next_free += __len; (void) 0; });
}
# 1285 "../../../kg++fe/gnu/cp/mangle.c"
static void
write_special_name_constructor (ctor)
     tree ctor;
{
  if (((((ctor)->decl.lang_specific)->decl_flags.constructor_attr) && ((ctor)->decl.name) == cp_global_trees[CPTI_COMPLETE_CTOR_IDENTIFIER])



      || ((((ctor)->decl.lang_specific)->decl_flags.constructor_attr) && !((((enum tree_code) (ctor)->common.code) == FUNCTION_DECL || ((enum tree_code) (ctor)->common.code) == TEMPLATE_DECL) && ((ctor)->decl.lang_specific) && (((ctor)->decl.lang_specific)->u.f.cloned_function) != (tree) ((void *)0))))
    __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("C1")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("C1"))), (__len)); __o->next_free += __len; (void) 0; });
  else if (((((ctor)->decl.lang_specific)->decl_flags.constructor_attr) && ((ctor)->decl.name) == cp_global_trees[CPTI_BASE_CTOR_IDENTIFIER]))
    __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("C2")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("C2"))), (__len)); __o->next_free += __len; (void) 0; });
  else
    fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 1298, __FUNCTION__);
}
# 1312 "../../../kg++fe/gnu/cp/mangle.c"
static void
write_special_name_destructor (dtor)
     tree dtor;
{
  if (((((dtor)->decl.lang_specific)->decl_flags.destructor_attr) && ((dtor)->decl.name) == cp_global_trees[CPTI_DELETING_DTOR_IDENTIFIER]))
    __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("D0")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("D0"))), (__len)); __o->next_free += __len; (void) 0; });
  else if (((((dtor)->decl.lang_specific)->decl_flags.destructor_attr) && ((dtor)->decl.name) == cp_global_trees[CPTI_COMPLETE_DTOR_IDENTIFIER])



    || ((((dtor)->decl.lang_specific)->decl_flags.destructor_attr) && !((((enum tree_code) (dtor)->common.code) == FUNCTION_DECL || ((enum tree_code) (dtor)->common.code) == TEMPLATE_DECL) && ((dtor)->decl.lang_specific) && (((dtor)->decl.lang_specific)->u.f.cloned_function) != (tree) ((void *)0))))
    __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("D1")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("D1"))), (__len)); __o->next_free += __len; (void) 0; });
  else if (((((dtor)->decl.lang_specific)->decl_flags.destructor_attr) && ((dtor)->decl.name) == cp_global_trees[CPTI_BASE_DTOR_IDENTIFIER]))
    __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("D2")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("D2"))), (__len)); __o->next_free += __len; (void) 0; });
  else
    fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 1327, __FUNCTION__);
}
     ...
     tree entity;
{
  ...
  if ((((enum tree_code) (entity)->common.code) == VAR_DECL && (((entity)->decl.context) && ((enum tree_code) (((entity)->decl.context))->common.code) == FUNCTION_DECL)) && ((entity)->decl.lang_specific))
    discriminator = ((&((entity)->decl.lang_specific)->decl_flags.u2)->discriminator);
  else if (((enum tree_code) (entity)->common.code) == TYPE_DECL)
    {

      entity = ((entity)->common.type);
      for (type = &((local_classes)->data.tree[0]); *type != entity; ++type)
        if ((((((*type)->type.name))->decl.name)) == (((((entity)->type.name))->decl.name))
            && ((*type)->type.context) == ((entity)->type.context))
   ++discriminator;
    }
...
discriminator_for_string_literal (function, string)
     tree function __attribute__ ((__unused__));
     tree string __attribute__ ((__unused__));
{
     ...
     int discriminator;
{
  ...
  if (discriminator > 0)
    {
      __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('_')); (void) 0; });
      write_number ((discriminator - 1), 1, 10);
    }
}
# 1399 "../../../kg++fe/gnu/cp/mangle.c"
static void
     ...
     tree entity;
{
  ...
  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('Z')); (void) 0; });
  write_encoding (function);
  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('E')); (void) 0; });
  if (((enum tree_code) (entity)->common.code) == STRING_CST)
    {
      __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('s')); (void) 0; });
     ...
     tree type;
{
  ...
  if (write_CV_qualifiers_for_type (type) > 0)
    ...
    write_type (((type)->type.main_variant));
 ...
 case REAL_TYPE:
   ...
   write_builtin_type (((type)->type.main_variant));
   ++is_builtin_type;
   break;

 case COMPLEX_TYPE:
   __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('C')); (void) 0; });
   write_type (((type)->common.type));
   break;
 ...
 case UNBOUND_CLASS_TEMPLATE:
   ...
   write_nested_name (((type)->common.chain));
   break;
 ...
 case POINTER_TYPE:
   ...
   if ((((enum tree_code) (type)->common.code) == POINTER_TYPE && ((enum tree_code) (((type)->common.type))->common.code) == OFFSET_TYPE))
     ...
     {
       __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('P')); (void) 0; });
       write_type (((type)->common.type));
     }
   break;

 case REFERENCE_TYPE:
   __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('R')); (void) 0; });
   write_type (((type)->common.type));
   break;
 ...
 case VECTOR_TYPE:
   __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("U8__vector")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("U8__vector"))), (__len)); __o->next_free += __len; (void) 0; });
   write_type (((type)->common.type));
   break;

 default:
   fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 1559, __FUNCTION__);
 }
    }
...
# 1589 "../../../kg++fe/gnu/cp/mangle.c"
  if (((((type)->common.readonly_flag) * 0x1) | (((type)->common.volatile_flag) * 0x2) | (((type)->type.restrict_flag) * 0x4) | ((((enum tree_code) (type)->common.code) == RECORD_TYPE && ((type)->common.type)) * 0x8)) & 0x4)
    {
      __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('r')); (void) 0; });
      ++num_qualifiers;
    }
  if (((((type)->common.readonly_flag) * 0x1) | (((type)->common.volatile_flag) * 0x2) | (((type)->type.restrict_flag) * 0x4) | ((((enum tree_code) (type)->common.code) == RECORD_TYPE && ((type)->common.type)) * 0x8)) & 0x2)
    {
      __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('V')); (void) 0; });
      ++num_qualifiers;
    }
  if (((((type)->common.readonly_flag) * 0x1) | (((type)->common.volatile_flag) * 0x2) | (((type)->type.restrict_flag) * 0x4) | ((((enum tree_code) (type)->common.code) == RECORD_TYPE && ((type)->common.type)) * 0x8)) & 0x1)
    {
      __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('K')); (void) 0; });
      ++num_qualifiers;
    }
     ...
     tree type;
{
  switch (((enum tree_code) (type)->common.code))
    ...
    case BOOLEAN_TYPE:
      __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('b')); (void) 0; });
      break;

    case INTEGER_TYPE:

      if (((type)->type.no_force_blk_flag))
 type = ((type)->type.values);



      if (type == c_global_trees[CTI_WCHAR_TYPE])
 __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('w')); (void) 0; });
      else if (((type)->type.lang_flag_3))
 ...
 iagain:
  ...
  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = ((integer_type_codes[itk])); (void) 0; });
  break;
       }

   if (itk == itk_none)
     {
       tree t = c_common_type_for_mode (((type)->type.mode),
            ((type)->common.unsigned_flag));
       if (type == t)
  {
    if (((type)->type.precision) == 128)
      __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = ((((type)->common.unsigned_flag) ? 'o' : 'n')); (void) 0; });
    else

      fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 1681, __FUNCTION__);
  }
       else
  {
    type = t;
   ...
   || type == cp_global_trees[CPTI_JAVA_FLOAT_TYPE])
 __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('f')); (void) 0; });
      else if (type == global_trees[TI_DOUBLE_TYPE]
        || type == cp_global_trees[CPTI_JAVA_DOUBLE_TYPE])
 __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('d')); (void) 0; });
      else if (type == global_trees[TI_LONG_DOUBLE_TYPE])
 __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('e')); (void) 0; });
      else
 fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 1702, __FUNCTION__);
      break;

    default:
      fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 1706, __FUNCTION__);
    }
}
  ...
  if (((enum tree_code) (type)->common.code) == METHOD_TYPE)
    {
      ...
      tree this_type = ((((((type)->type.values))->list.value))->common.type);
      write_CV_qualifiers_for_type (this_type);
    }

  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('F')); (void) 0; });
# 1746 "../../../kg++fe/gnu/cp/mangle.c"
  write_bare_function_type (type, 1,
                ((void *)0));
  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('E')); (void) 0; });
}
# 1758 "../../../kg++fe/gnu/cp/mangle.c"
static void
     ...
     tree decl;
{
  ...
  write_method_parms (((type)->type.values),
     ...
     int method_p;
{
  ...
  tree parm_decl = decl ? ((decl)->decl.arguments) : (tree) ((void *)0);



  int varargs_p = 1;
# 1802 "../../../kg++fe/gnu/cp/mangle.c"
  if (method_p)
    {
      parm_types = ((parm_types)->common.chain);
      parm_decl = parm_decl ? ((parm_decl)->common.chain) : (tree) ((void *)0);

      while (parm_decl && ((parm_decl)->decl.artificial_flag))
 {
   parm_types = ((parm_types)->common.chain);
   parm_decl = ((parm_decl)->common.chain);
 }
    }
  ...
  for (first_parm_type = parm_types;
       ...
       parm_types = ((parm_types)->common.chain))
    {
      tree parm = ((parm_types)->list.value);
      if (parm == global_trees[TI_VOID_TYPE])
 {
   ...
   (void) (((((parm_types)->common.chain) == ((void *)0)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 1830, __FUNCTION__), 0) : 0);
 }
      else
 write_type (parm);
    }
     ...
     tree args;
{
  ...
  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('I')); (void) 0; });

  if (((enum tree_code) (args)->common.code) == TREE_VEC)
    {
      int i;
      int length = ((args)->vec.length);
      (void) (((length > 0) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 1867, __FUNCTION__), 0) : 0);

      if (((enum tree_code) (((args)->vec.a[0]))->common.code) == TREE_VEC)
 {


   args = ((args)->vec.a[length - 1]);
   length = ((args)->vec.length);
 }
      for (i = 0; i < length; ++i)
 write_template_arg (((args)->vec.a[i]));
    }
  else
    {
      (void) (((((enum tree_code) (args)->common.code) == TREE_LIST) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 1881, __FUNCTION__), 0) : 0);

      while (args)
 {
   write_template_arg (((args)->list.value));
     ...
     tree expr;
{
  ...
  if (code == PTRMEM_CST)
    {
      expr = build_nt (ADDR_EXPR,
         build_nt (SCOPE_REF,
     ((((enum tree_code) ((((expr))->common.type))->common.code) == POINTER_TYPE && ((enum tree_code) ((((((expr))->common.type))->common.type))->common.code) == OFFSET_TYPE) ? (((((((expr))->common.type))->common.type))->type.maxval) : ((((((((((((expr))->common.type))->type.values))->common.type)))->common.type))->type.maxval)),
     (((ptrmem_cst_t)(expr))->member)));
      code = ((enum tree_code) (expr)->common.code);
    }
  ...
  if (code == TEMPLATE_TYPE_PARM
  ...
  else if (tree_code_type[(int) (code)] == 'c'
    || ((flag_abi_version == 0 || flag_abi_version >= (2)) && code == CONST_DECL))
  ...
  else if ((tree_code_type[(int) (((enum tree_code) (expr)->common.code))] == 'd'))
    {
      ...
      if (code == CONST_DECL)
 G.need_abi_warning = 1;
      __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('L')); (void) 0; });
      write_mangled_name (expr);
      __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('E')); (void) 0; });
    }
  else if (((enum tree_code) (expr)->common.code) == SIZEOF_EXPR
    && (tree_code_type[(int) (((enum tree_code) (((expr)->exp.operands[0]))->common.code))] == 't'))
    {
      __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("st")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("st"))), (__len)); __o->next_free += __len; (void) 0; });
      write_type (((expr)->exp.operands[0]));
    }
  else if ((flag_abi_version == 0 || flag_abi_version >= (2)) && ((enum tree_code) (expr)->common.code) == SCOPE_REF)
    {
      tree scope = ((expr)->exp.operands[0]);
      tree member = ((expr)->exp.operands[1]);
      ...
      else
 {
   ...
   __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("sr")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("sr"))), (__len)); __o->next_free += __len; (void) 0; });
   write_type (scope);


   if (((enum tree_code) (member)->common.code) == TEMPLATE_ID_EXPR)
     {
       template_args = ((member)->exp.operands[1]);
       member = ((member)->exp.operands[0]);
       if (((enum tree_code) (member)->common.code) == LOOKUP_EXPR)
  member = ((member)->exp.operands[0]);
     }
   else
     template_args = (tree) ((void *)0);

   if (((member)->common.lang_flag_4))
     write_conversion_operator_name (((member)->common.type));
   else if (((member)->common.lang_flag_2))
     {
       int i;
  ...
  if (operator_name_info[i].identifier == member)
   ...
   break;
    ...
    == member)
    {
      ...
      break;
       ...
       __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen (mangled_name)); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), (((mangled_name))), (__len)); __o->next_free += __len; (void) 0; });
     }
   else
   ...
   && ((expr)->common.type)
   && ((enum tree_code) (((expr)->common.type))->common.code) == REFERENCE_TYPE)
 {
   expr = ((expr)->exp.operands[0]);
   ...
   code = ((enum tree_code) (expr)->common.code);
      ...
      __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen (operator_name_info[(int) code].mangled_name)); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), (((operator_name_info[(int) code].mangled_name))), (__len)); __o->next_free += __len; (void) 0; });
 ...
 case CAST_EXPR:
   write_type (((expr)->common.type));
   write_expression (((((expr)->exp.operands[0]))->list.value));
   break;

 case STATIC_CAST_EXPR:
 case CONST_CAST_EXPR:
   write_type (((expr)->common.type));
   write_expression (((expr)->exp.operands[0]));
   break;
 ...
 case SCOPE_REF:
   write_type (((expr)->exp.operands[0]));
   if (((enum tree_code) (((expr)->exp.operands[1]))->common.code) == IDENTIFIER_NODE)
     write_source_name (((expr)->exp.operands[1]));
   else
     {


       G.need_abi_warning = 1;
       write_encoding (((expr)->exp.operands[1]));
     }
   break;

 default:
   for (i = 0; i < tree_code_length[(int) (code)]; ++i)
     write_expression (((expr)->exp.operands[i]));
 }
    }
}
# 2098 "../../../kg++fe/gnu/cp/mangle.c"
static void
     ...
     tree value;
{
  ...
  if (((enum tree_code) (value)->common.code) == CONST_DECL)
    write_integer_cst (((value)->decl.initial));
  else if (((enum tree_code) (value)->common.code) == INTEGER_CST)
    {
      if (comptypes ((type), (c_global_trees[CTI_BOOLEAN_TYPE]), 0))
 {
   if (value == c_global_trees[CTI_BOOLEAN_FALSE] || integer_zerop (value))
     write_number ((0), 1, 10);
   else if (value == c_global_trees[CTI_BOOLEAN_TRUE])
     write_number ((1), 1, 10);
   else
     fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 2117, __FUNCTION__);
 }
      else
 write_integer_cst (value);
    ...
    fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 2125, __FUNCTION__);

  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('E')); (void) 0; });
}
# 2137 "../../../kg++fe/gnu/cp/mangle.c"
static void
     ...
     tree node;
{
  ...
  if (code == TREE_LIST)
    {
      node = ((node)->list.value);

      if ((tree_code_type[(int) (((enum tree_code) (node)->common.code))] == 'd'))
 {
   node = ((node)->common.type);
   code = ((enum tree_code) (node)->common.code);
 }
    }
  ...
  else if ((tree_code_type[(int) (code)] == 'c' && code != PTRMEM_CST)
    || ((flag_abi_version == 0 || flag_abi_version >= (2)) && code == CONST_DECL))
 ...
 G.need_abi_warning = 1;
  ...
  else
    {
      ...
      __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('X')); (void) 0; });
...
# 2211 "../../../kg++fe/gnu/cp/mangle.c"
static void
write_array_type (type)
  tree type;
{
  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('A')); (void) 0; });
  if (((type)->type.values))
    {
      tree index_type;
      tree max;

      index_type = ((type)->type.values);


      max = ((index_type)->type.maxval);
      if (((enum tree_code) (max)->common.code) == INTEGER_CST)
 {


   max = size_binop (PLUS_EXPR, max, global_trees[TI_SIZE_ONE]);
   write_number ((tree_low_cst (max, 1)), 1, 10);
 }
      else
 write_expression (((max)->exp.operands[0]));
    }
  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('_')); (void) 0; });
  write_type (((type)->common.type));
}
...
write_pointer_to_member_type (type)
     tree type;
{
  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('M')); (void) 0; });
  write_type (((((enum tree_code) (type)->common.code) == POINTER_TYPE && ((enum tree_code) (((type)->common.type))->common.code) == OFFSET_TYPE) ? ((((type)->common.type))->type.maxval) : (((((((((type)->type.values))->common.type)))->common.type))->type.maxval)));
  write_type (((((enum tree_code) (type)->common.code) == POINTER_TYPE && ((enum tree_code) (((type)->common.type))->common.code) == OFFSET_TYPE) ? ((((type)->common.type))->common.type) : (((((((type)->type.values))->common.type)))->common.type)));
}
# 2266 "../../../kg++fe/gnu/cp/mangle.c"
static void
     ...
     tree parm;
{
  ...
  switch (((enum tree_code) (parm)->common.code))
    ...
    case TEMPLATE_TEMPLATE_PARM:
    case BOUND_TEMPLATE_TEMPLATE_PARM:
      parm_index = ((((template_parm_index*)((((parm)->type.values))))->index));
      parm_level = ((((template_parm_index*)((((parm)->type.values))))->level));
      break;

    case TEMPLATE_PARM_INDEX:
      parm_index = (((template_parm_index*)(parm))->index);
      parm_level = (((template_parm_index*)(parm))->level);
      parm_type = (((((template_parm_index*)(parm))->decl))->common.type);
      break;

    default:
      fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 2292, __FUNCTION__);
    }

  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('T')); (void) 0; });


  if (parm_index > 0)
    write_number ((parm_index - 1), 1, 10);
  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('_')); (void) 0; });
}
...
start_mangling (tree entity)
{
  ...
  G.substitutions = varray_init (1, VARRAY_DATA_TREE, "mangling substitutions");
  __extension__ ({ struct obstack *__o = (&G.name_obstack); void *__obj = (((&G.name_obstack)->object_base)); if (__obj > (void *)__o->chunk && __obj < (void *)__o->chunk_limit) __o->next_free = __o->object_base = __obj; else (obstack_free) (__o, __obj); });
}
...
finish_mangling (_Bool warn)
{
  ...
  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('\0')); (void) 0; });

  return (const char *) ((&G.name_obstack)->object_base);
}
...
init_mangle ()
{
  ...
  subst_identifiers[SUBID_ALLOCATOR] = (__builtin_constant_p ("allocator") ? get_identifier_with_length (("allocator"), (unsigned) strlen ("allocator")) : (get_identifier) ("allocator"));
  subst_identifiers[SUBID_BASIC_STRING] = (__builtin_constant_p ("basic_string") ? get_identifier_with_length (("basic_string"), (unsigned) strlen ("basic_string")) : (get_identifier) ("basic_string"));
  subst_identifiers[SUBID_CHAR_TRAITS] = (__builtin_constant_p ("char_traits") ? get_identifier_with_length (("char_traits"), (unsigned) strlen ("char_traits")) : (get_identifier) ("char_traits"));
  subst_identifiers[SUBID_BASIC_ISTREAM] = (__builtin_constant_p ("basic_istream") ? get_identifier_with_length (("basic_istream"), (unsigned) strlen ("basic_istream")) : (get_identifier) ("basic_istream"));
  subst_identifiers[SUBID_BASIC_OSTREAM] = (__builtin_constant_p ("basic_ostream") ? get_identifier_with_length (("basic_ostream"), (unsigned) strlen ("basic_ostream")) : (get_identifier) ("basic_ostream"));
  subst_identifiers[SUBID_BASIC_IOSTREAM] = (__builtin_constant_p ("basic_iostream") ? get_identifier_with_length (("basic_iostream"), (unsigned) strlen ("basic_iostream")) : (get_identifier) ("basic_iostream"));
}
     ...
     tree decl;
{
  ...
  if (((enum tree_code) (decl)->common.code) == TYPE_DECL)
    ...
    || (((enum tree_code) (decl)->common.code) == VAR_DECL
        && (((decl)->decl.context) ? ((decl)->decl.context) : global_namespace) == global_namespace)

    || (((enum tree_code) (decl)->common.code) == VAR_DECL
        && ((((decl)->decl.lang_specific) ? ((decl)->decl.lang_specific)->decl_flags.language : (((enum tree_code) (decl)->common.code) == FUNCTION_DECL ? lang_c : lang_cplusplus)) == lang_c)))
    __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen (((char *)(((decl)->decl.name))->identifier.id.str))); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), (((((char *)(((decl)->decl.name))->identifier.id.str)))), (__len)); __o->next_free += __len; (void) 0; });
  else
    {
      write_mangled_name (decl);
      if (((decl)->decl.lang_specific)
   && (((((decl)->decl.lang_specific)->decl_flags.destructor_attr) && !((((enum tree_code) (decl)->common.code) == FUNCTION_DECL || ((enum tree_code) (decl)->common.code) == TEMPLATE_DECL) && ((decl)->decl.lang_specific) && (((decl)->decl.lang_specific)->u.f.cloned_function) != (tree) ((void *)0)))
       || ((((decl)->decl.lang_specific)->decl_flags.constructor_attr) && !((((enum tree_code) (decl)->common.code) == FUNCTION_DECL || ((enum tree_code) (decl)->common.code) == TEMPLATE_DECL) && ((decl)->decl.lang_specific) && (((decl)->decl.lang_specific)->u.f.cloned_function) != (tree) ((void *)0)))))



 __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen (" *INTERNAL* ")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), (((" *INTERNAL* "))), (__len)); __o->next_free += __len; (void) 0; });
    }
...
mangle_decl (decl)
     tree decl;
{
  tree id = (__builtin_constant_p (mangle_decl_string (decl)) ? get_identifier_with_length ((mangle_decl_string (decl)), (unsigned) strlen (mangle_decl_string (decl))) : (get_identifier) (mangle_decl_string (decl)));

  ((decl)->decl.assembler_name = (id));
}
...
mangle_type (type)
     tree type;
{
  return (__builtin_constant_p (mangle_type_string (type)) ? get_identifier_with_length ((mangle_type_string (type)), (unsigned) strlen (mangle_type_string (type))) : (get_identifier) (mangle_type_string (type)));
}
     ...
     const char *code;
{
  ...
  __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("_Z")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("_Z"))), (__len)); __o->next_free += __len; (void) 0; });
  __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen (code)); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), (((code))), (__len)); __o->next_free += __len; (void) 0; });
  ...
  return (__builtin_constant_p (result) ? get_identifier_with_length ((result), (unsigned) strlen (result)) : (get_identifier) (result));
}
     ...
     tree binfo;
{
  ...
  __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("_Z")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("_Z"))), (__len)); __o->next_free += __len; (void) 0; });
  __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("TC")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("TC"))), (__len)); __o->next_free += __len; (void) 0; });
  write_type (type);
  write_integer_cst ((((binfo))->vec.a[1]));
  __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('_')); (void) 0; });
  write_type (((binfo)->common.type));
     ...
     tree vcall_offset;
{
  ...
  if (vcall_offset != 0)
    ...
    __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('h')); (void) 0; });
...
{
  return htab_hash_pointer ((((tree) val)->list.purpose));
}
...
{
  return (((tree) val1)->list.purpose) == (tree) val2;
}
...
mangle_conv_op_name_for_type (const tree type)
{
  ...
  if (conv_type_names == ((void *)0))
    conv_type_names = htab_create_alloc (31, &hash_type, &compare_type, ((void *)0), ggc_calloc, ((void *)0));

  slot = htab_find_slot_with_hash (conv_type_names, type,
       htab_hash_pointer (type), INSERT);
  if (*slot)
    return (((tree) *slot)->list.value);


  sprintf (buffer, "operator %lu",
    (unsigned long) htab_elements (conv_type_names));
  identifier = (__builtin_constant_p (buffer) ? get_identifier_with_length ((buffer), (unsigned) strlen (buffer)) : (get_identifier) (buffer));
  *slot = build_tree_list (type, identifier);


  ((identifier)->common.lang_flag_2) = 1;
  ((identifier)->common.lang_flag_4) = 1;


  ((identifier)->common.type) = type;
     ...
     tree variable;
{
  ...
  __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("_ZGV")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("_ZGV"))), (__len)); __o->next_free += __len; (void) 0; });
  if (strncmp (((char *)(((variable)->decl.name))->identifier.id.str), "_ZGR", 4) == 0)


    __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen (((char *)(((variable)->decl.name))->identifier.id.str) + 4)); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), (((((char *)(((variable)->decl.name))->identifier.id.str) + 4))), (__len)); __o->next_free += __len; (void) 0; });
  else
    write_name (variable, 0);
  return (__builtin_constant_p (finish_mangling ( 0)) ? get_identifier_with_length ((finish_mangling ( 0)), (unsigned) strlen (finish_mangling ( 0))) : (get_identifier) (finish_mangling ( 0)));
}
...
{
  start_mangling (variable);
  __extension__ ({ struct obstack *__o = (&G.name_obstack); int __len = (strlen ("_ZGR")); if (__o->next_free + __len > __o->chunk_limit) _obstack_newchunk (__o, __len); memcpy ((__o->next_free), ((("_ZGR"))), (__len)); __o->next_free += __len; (void) 0; });
  write_name (variable, 0);
  return (__builtin_constant_p (finish_mangling ( 0)) ? get_identifier_with_length ((finish_mangling ( 0)), (unsigned) strlen (finish_mangling ( 0))) : (get_identifier) (finish_mangling ( 0)));
}
...
write_java_integer_type_codes (type)
     tree type;
{
  if (type == cp_global_trees[CPTI_JAVA_INT_TYPE])
    __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('i')); (void) 0; });
  else if (type == cp_global_trees[CPTI_JAVA_SHORT_TYPE])
    __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('s')); (void) 0; });
  else if (type == cp_global_trees[CPTI_JAVA_BYTE_TYPE])
    __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('c')); (void) 0; });
  else if (type == cp_global_trees[CPTI_JAVA_CHAR_TYPE])
    __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('w')); (void) 0; });
  else if (type == cp_global_trees[CPTI_JAVA_LONG_TYPE])
    __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('x')); (void) 0; });
  else if (type == cp_global_trees[CPTI_JAVA_BOOLEAN_TYPE])
    __extension__ ({ struct obstack *__o = (&G.name_obstack); if (__o->next_free + 1 > __o->chunk_limit) _obstack_newchunk (__o, 1); *(__o->next_free)++ = (('b')); (void) 0; });
  else
    fancy_abort ("../../../kg++fe/gnu/cp/mangle.c", 2738, __FUNCTION__);
}

# 1 "../../../kg++fe/gnu/MIPS/gt-cp-mangle.h" 1
# 25 "../../../kg++fe/gnu/MIPS/gt-cp-mangle.h"
