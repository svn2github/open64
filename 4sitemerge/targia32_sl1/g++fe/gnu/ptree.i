     ...
     int indent;
{
      ...
      if ((((node)->decl.lang_flag_0)))
 {
  ...
  if (((enum tree_code) (node)->common.code) == FUNCTION_DECL
      && (((node)->decl.lang_specific)->u.f.u.pending_inline_info))
    {
      fprintf (file, " pending-inline-info ");
      fprintf (file, "%p", (((node)->decl.lang_specific)->u.f.u.pending_inline_info));
    }
  if (((enum tree_code) (node)->common.code) == TYPE_DECL
      && ((((node))->decl.lang_specific)->u.f.u.sorted_fields))
    {
      fprintf (file, " sorted-fields ");
      fprintf (file, "%p", ((((node))->decl.lang_specific)->u.f.u.sorted_fields));
    }
  if ((((enum tree_code) (node)->common.code) == FUNCTION_DECL || ((enum tree_code) (node)->common.code) == VAR_DECL)
      && ((((node))->decl.lang_specific) ->decl_flags.u.template_info))
    {
      fprintf (file, " template-info ");
      fprintf (file, "%p", ((((node))->decl.lang_specific) ->decl_flags.u.template_info));
    }
}
     ...
     int indent;
{
  switch (((enum tree_code) (node)->common.code))
    ...
    case BOUND_TEMPLATE_TEMPLATE_PARM:
      indent_to (file, indent + 3);
      fputs_unlocked ("index ", file);
      fprintf (file, "%lld", ((((template_parm_index*)((((node)->type.values))))->index)));
      fputs_unlocked (" level ", file);
      fprintf (file, "%lld", ((((template_parm_index*)((((node)->type.values))))->level)));
      fputs_unlocked (" orig_level ", file);
      fprintf (file, "%lld", ((((template_parm_index*)((((node)->type.values))))->orig_level)));
      return;

    case FUNCTION_TYPE:
    case METHOD_TYPE:
      if (((node)->type.binfo))
 print_node (file, "throws", ((node)->type.binfo), indent + 4);
      return;
  ...
  if ((((enum tree_code) (node)->common.code) == RECORD_TYPE && ((node)->type.lang_specific) && ((&((node)->type.lang_specific)->u.c)->ptrmemfunc_flag)))
    print_node (file, "ptrmemfunc fn type", (((((node)->type.values))->common.type)),
  indent + 4);

  if (! (((((enum tree_code) (node)->common.code)) == RECORD_TYPE || (((enum tree_code) (node)->common.code)) == UNION_TYPE) && (((enum tree_code) (node)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (node)->common.code) == TYPENAME_TYPE || ((enum tree_code) (node)->common.code) == TYPEOF_TYPE || ((enum tree_code) (node)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((node)->type.lang_flag_5))))
    return;

  indent_to (file, indent + 3);

  if (((node)->type.needs_constructing_flag))
    fputs_unlocked ("needs-constructor", file);
  if ((((node)->type.lang_flag_4)))
    fputs_unlocked (" needs-destructor", file);
  if ((((node)->type.lang_flag_2)))
    fputs_unlocked (" ~X()", file);
  if (((&((node)->type.lang_specific)->u.c)->h.has_default_ctor))
    fputs_unlocked (" X()", file);
  if (((&((node)->type.lang_specific)->u.c)->h.has_type_conversion))
    fputs_unlocked (" has-type-conversion", file);
  if (((&((node)->type.lang_specific)->u.c)->h.has_init_ref))
    {
      if (((&((node)->type.lang_specific)->u.c)->has_const_init_ref))
 fputs_unlocked (" X(constX&)", file);
      else
 fputs_unlocked (" X(X&)", file);
    }
  if (((&((node)->type.lang_specific)->u.c)->has_new))
    fputs_unlocked (" new", file);
  if (((&((node)->type.lang_specific)->u.c)->has_array_new))
    fputs_unlocked (" new[]", file);
  if (((&((node)->type.lang_specific)->u.c)->gets_delete) & 1)
    fputs_unlocked (" delete", file);
  if (((&((node)->type.lang_specific)->u.c)->gets_delete) & 2)
    fputs_unlocked (" delete[]", file);
  if (((&((node)->type.lang_specific)->u.c)->has_assign_ref))
    fputs_unlocked (" this=(X&)", file);
  if (((&((node)->type.lang_specific)->u.c)->has_call_overloaded))
    fputs_unlocked (" op()", file);
  if (((&((node)->type.lang_specific)->u.c)->has_array_ref_overloaded))
    fputs_unlocked (" op[]", file);
  if (((&((node)->type.lang_specific)->u.c)->has_arrow_overloaded))
    fputs_unlocked (" op->", file);
  if (((&((node)->type.lang_specific)->u.c)->h.uses_multiple_inheritance))
    fputs_unlocked (" uses-multiple-inheritance", file);

  if (((enum tree_code) (node)->common.code) == RECORD_TYPE)
    {
      fprintf (file, " n_parents %d", (((((((node)->type.binfo)))->vec.a[4]) ? (((((((node)->type.binfo)))->vec.a[4]))->vec.length) : 0)));
      fprintf (file, " use_template=%d", ((&((node)->type.lang_specific)->u.c)->use_template));
      if (((&((node)->type.lang_specific)->u.c)->interface_only))
 fprintf (file, " interface-only");
      if (((&((node)->type.lang_specific)->u.c)->interface_unknown))
 fprintf (file, " interface-unknown");
      print_node (file, "member-functions", ((&((node)->type.lang_specific)->u.c)->methods),
    indent + 4);
    }
}
...
cxx_print_identifier (file, node, indent)
     ...
     int indent;
{
  cxx_print_binding (file, (((struct lang_identifier*)(node))->namespace_bindings), "bindings");
  print_node (file, "class", (((struct lang_identifier*)(node))->class_value), indent + 4);
  cxx_print_binding (file, (((struct lang_identifier*)(node))->bindings), "local bindings");
  print_node (file, "label", (((struct lang_identifier*)(node))->x ? ((struct lang_identifier*)(node))->x->label_value : 0), indent + 4);
  print_node (file, "template", (((struct lang_identifier*)(node))->class_template_info), indent + 4);
  print_node (file, "implicit", (((struct lang_identifier*)(node))->x ? ((struct lang_identifier*)(node))->x->implicit_decl : 0), indent + 4);
  print_node (file, "error locus", (((struct lang_identifier*)(node))->x ? ((struct lang_identifier*)(node))->x->error_locus : 0), indent + 4);
}
     ...
     int indent;
{
  switch (((enum tree_code) (node)->common.code))
    {
    case OVERLOAD:
      print_node (file, "function", (((struct tree_overload*)(node))->function), indent+4);
      print_node (file, "chain", ((node)->common.chain), indent+4);
      break;
    case TEMPLATE_PARM_INDEX:
      indent_to (file, indent + 3);
      fputs_unlocked ("index ", file);
      fprintf (file, "%lld", (((template_parm_index*)(node))->index));
      fputs_unlocked (" level ", file);
      fprintf (file, "%lld", (((template_parm_index*)(node))->level));
      fputs_unlocked (" orig_level ", file);
      fprintf (file, "%lld", (((template_parm_index*)(node))->orig_level));
      break;
    default:
      break;
    }
