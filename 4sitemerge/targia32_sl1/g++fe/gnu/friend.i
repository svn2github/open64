     ...
     tree type, supplicant;
{
  ...
  if (supplicant == (tree) ((void *)0) || type == (tree) ((void *)0))
    return 0;
  ...
  if (declp)
    ...
    {
      tree list = ((((((((type)->type.main_variant))->common.chain)))->decl.initial));
      tree name = ((supplicant)->decl.name);

      for (; list ; list = ((list)->common.chain))
 {
   if (name == (((list)->list.purpose)))
     {
       tree friends = (((list)->list.value));
       for (; friends ; friends = ((friends)->common.chain))
  {
    if (((friends)->list.value) == (tree) ((void *)0))
      continue;

    if (supplicant == ((friends)->list.value))
      return 1;


    if (((enum tree_code) (supplicant)->common.code) == TEMPLATE_DECL)
      return 1;
    ...
    if (((((enum tree_code) (((supplicant)->common.type))->common.code) == METHOD_TYPE) || (((supplicant)->decl.lang_specific)->decl_flags.static_function))
        && comptypes ((((supplicant)->common.type)), (((((friends)->list.value))->common.type)), 0))

      return 1;

    if (((enum tree_code) (((friends)->list.value))->common.code) == TEMPLATE_DECL
    ...
    context && (tree_code_type[(int) (((enum tree_code) (context)->common.code))] == 't');
    context = ((context)->type.context))
 if (type == context)
   return 1;

      list = ((&(((((((((type)->type.main_variant))->common.chain)))->common.type))->type.lang_specific)->u.c)->friend_classes);
      for (; list ; list = ((list)->common.chain))
 {
   tree t = ((list)->list.value);

   if (((enum tree_code) (t)->common.code) == TEMPLATE_DECL ?
       is_specialization_of ((((((supplicant)->type.main_variant))->common.chain)), t) :
       comptypes ((supplicant), (t), 0))
     return 1;
 }
    }

  if (declp && ((((enum tree_code) (((supplicant)->common.type))->common.code) == METHOD_TYPE) || (((supplicant)->decl.lang_specific)->decl_flags.static_function)))
    context = ((supplicant)->decl.context);
  else if (! declp)

    context = decl_function_context ((((((supplicant)->type.main_variant))->common.chain)));
  else
    context = (tree) ((void *)0);
     ...
     tree type, decl;
{
  ...
  list = (((typedecl)->decl.initial));
  name = ((decl)->decl.name);
  type = ((typedecl)->common.type);
      ...
      if (name == (((list)->list.purpose)))
 {
       ...
       if (decl == ((friends)->list.value))
  {
   ...
   ((list)->list.value) = tree_cons (global_trees[TI_ERROR_MARK], decl,
      ((list)->list.value));
   return;
 }
  ...
  (((typedecl)->decl.initial))
    = tree_cons (((decl)->decl.name), build_tree_list (global_trees[TI_ERROR_MARK], decl),
   (((typedecl)->decl.initial)));
  if (!uses_template_parms (type))
    (((decl)->decl.lang_specific)->u.f.befriending_classes)
      = tree_cons ((tree) ((void *)0), type,
     (((decl)->decl.lang_specific)->u.f.befriending_classes));
}
# 197 "../../../kg++fe/gnu/cp/friend.c"
void
     ...
     tree type, friend_type;
{
  ...
  if (! (((enum tree_code) (friend_type)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (friend_type)->common.code) == TYPENAME_TYPE || ((enum tree_code) (friend_type)->common.code) == TYPEOF_TYPE || ((enum tree_code) (friend_type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((friend_type)->type.lang_flag_5)))
    {
      error ("invalid type `%T' declared `friend'", friend_type);
      return;
    }

  if (scope_chain->x_processing_template_decl > template_class_depth (type))



    {
      if ((((((enum tree_code) (friend_type)->common.code)) == RECORD_TYPE || (((enum tree_code) (friend_type)->common.code)) == UNION_TYPE) && (((enum tree_code) (friend_type)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (friend_type)->common.code) == TYPENAME_TYPE || ((enum tree_code) (friend_type)->common.code) == TYPEOF_TYPE || ((enum tree_code) (friend_type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((friend_type)->type.lang_flag_5)))
   && (((&((friend_type)->type.lang_specific)->u.c)->use_template) == 2)
   && uses_template_parms (friend_type))
 {
  ...
  else if (comptypes ((type), (friend_type), 0))
    {
  ...
  if (!is_template_friend)
    ;
  else if (((enum tree_code) (friend_type)->common.code) == TYPENAME_TYPE)
    {

      error ("typename type `%#T' declared `friend'", friend_type);
      return;
    }
  else if (((enum tree_code) (friend_type)->common.code) == TEMPLATE_TYPE_PARM)
    {

      error ("template parameter type `%T' declared `friend'", friend_type);
      return;
    }
  else if (!((&(((friend_type))->type.lang_specific)->u.c)->template_info))
    {
  ...
  if (is_template_friend)
    friend_type = (((((&(((friend_type))->type.lang_specific)->u.c)->template_info))->list.purpose));

  classes = ((&((type)->type.lang_specific)->u.c)->friend_classes);
  while (classes

  && !(((enum tree_code) (((classes)->list.value))->common.code) == TEMPLATE_DECL ?
       friend_type == ((classes)->list.value) :
       comptypes ((((classes)->list.value)), (friend_type), 0)))
    classes = ((classes)->common.chain);
  if (classes)
    warning ("`%T' is already a friend of `%T'",
  ((classes)->list.value), type);
  else
    {
      maybe_add_class_template_decl_list (type, friend_type, 1);

      ((&((type)->type.lang_specific)->u.c)->friend_classes)
 = tree_cons ((tree) ((void *)0), friend_type, ((&((type)->type.lang_specific)->u.c)->friend_classes));
      if (is_template_friend)
 friend_type = ((friend_type)->common.type);
      if (!uses_template_parms (type))
 ((&((friend_type)->type.lang_specific)->u.c)->befriending_classes)
   = tree_cons ((tree) ((void *)0), type,
         ((&((friend_type)->type.lang_specific)->u.c)->befriending_classes));
    }
}
# 313 "../../../kg++fe/gnu/cp/friend.c"
tree
     ...
     int funcdef_flag;
{
  ...
  (((decl)->decl.lang_specific)->decl_flags.friend_attr) = 1;

  if (((enum tree_code) (declarator)->common.code) == TEMPLATE_ID_EXPR)
    {
      declarator = ((declarator)->exp.operands[0]);
      if (((enum tree_code) (declarator)->common.code) == LOOKUP_EXPR)
 declarator = ((declarator)->exp.operands[0]);
      if (is_overloaded_fn (declarator))
 declarator = ((get_first_fn (declarator))->decl.name);
    }

  if (((enum tree_code) (decl)->common.code) != FUNCTION_DECL)
    fancy_abort ("../../../kg++fe/gnu/cp/friend.c", 336, __FUNCTION__);

  is_friend_template = (scope_chain->x_processing_template_decl > template_class_depth (current_scope ()));

  if (ctype)
    {
      tree cname = ((ctype)->type.name);
      if (((enum tree_code) (cname)->common.code) == TYPE_DECL)
 cname = ((cname)->decl.name);
 ...
 decl = (((((((push_template_decl (decl)))->decl.lang_specific) ->decl_flags.u.template_info))->list.purpose));
      else if (template_class_depth (scope_chain->class_type))
 ...
 add_friend (scope_chain->class_type, decl);



      else if ((((ctype)->type.size) != (tree) ((void *)0)) || ((&((ctype)->type.lang_specific)->u.c)->being_defined))
 {
 ...
 error ("member `%D' declared as friend before type `%T' defined",
  ...
  else if (((enum tree_code) (decl)->common.code) == FUNCTION_DECL)
    {
      ...
      ((decl)->decl.arguments) = parmdecls;
      if (funcdef_flag)
 (((decl)->decl.lang_specific)->u.f.context = (scope_chain->class_type));

      if (! (((decl)->decl.lang_specific)->decl_flags.use_template))
 {
       ...
       if (! explained)
  {
    ...
    explained = 1;
      ...
      add_friend (scope_chain->class_type,
    is_friend_template ? (((((((decl))->decl.lang_specific) ->decl_flags.u.template_info))->list.purpose)) : decl);
      (((decl)->decl.lang_specific)->decl_flags.friend_attr) = 1;
    }
