	.file	"cpplex.c"
	.section	.debug_abbrev,"",@progbits
.Ldebug_abbrev0:
	.section	.debug_info,"",@progbits
.Ldebug_info0:
	.section	.debug_line,"",@progbits
.Ldebug_line0:
	.text
.Ltext0:
	.section	.rodata
.LC0:
	.string	"%:"
.LC1:
	.string	"%:%:"
.LC2:
	.string	"<:"
.LC3:
	.string	":>"
.LC4:
	.string	"<%"
.LC5:
	.string	"%>"
	.section	.data.rel.ro.local,"aw",@progbits
	.align 4
	.type	digraph_spellings, @object
	.size	digraph_spellings, 24
digraph_spellings:
	.long	.LC0
	.long	.LC1
	.long	.LC2
	.long	.LC3
	.long	.LC4
	.long	.LC5
	.section	.rodata
.LC6:
	.string	"="
.LC7:
	.string	"!"
.LC8:
	.string	">"
.LC9:
	.string	"<"
.LC10:
	.string	"+"
.LC11:
	.string	"-"
.LC12:
	.string	"*"
.LC13:
	.string	"/"
.LC14:
	.string	"%"
.LC15:
	.string	"&"
.LC16:
	.string	"|"
.LC17:
	.string	"^"
.LC18:
	.string	">>"
.LC19:
	.string	"<<"
.LC20:
	.string	"<?"
.LC21:
	.string	">?"
.LC22:
	.string	"~"
.LC23:
	.string	"&&"
.LC24:
	.string	"||"
.LC25:
	.string	"?"
.LC26:
	.string	":"
.LC27:
	.string	","
.LC28:
	.string	"("
.LC29:
	.string	")"
.LC30:
	.string	"CPP_EOF"
.LC31:
	.string	"=="
.LC32:
	.string	"!="
.LC33:
	.string	">="
.LC34:
	.string	"<="
.LC35:
	.string	"+="
.LC36:
	.string	"-="
.LC37:
	.string	"*="
.LC38:
	.string	"/="
.LC39:
	.string	"%="
.LC40:
	.string	"&="
.LC41:
	.string	"|="
.LC42:
	.string	"^="
.LC43:
	.string	">>="
.LC44:
	.string	"<<="
.LC45:
	.string	"<?="
.LC46:
	.string	">?="
.LC47:
	.string	"#"
.LC48:
	.string	"##"
.LC49:
	.string	"["
.LC50:
	.string	"]"
.LC51:
	.string	"{"
.LC52:
	.string	"}"
.LC53:
	.string	";"
.LC54:
	.string	"..."
.LC55:
	.string	"++"
.LC56:
	.string	"--"
.LC57:
	.string	"->"
.LC58:
	.string	"."
.LC59:
	.string	"::"
.LC60:
	.string	"->*"
.LC61:
	.string	".*"
.LC62:
	.string	"@"
.LC63:
	.string	"CPP_NAME"
.LC64:
	.string	"CPP_NUMBER"
.LC65:
	.string	"CPP_CHAR"
.LC66:
	.string	"CPP_WCHAR"
.LC67:
	.string	"CPP_OTHER"
.LC68:
	.string	"CPP_STRING"
.LC69:
	.string	"CPP_WSTRING"
.LC70:
	.string	"CPP_HEADER_NAME"
.LC71:
	.string	"CPP_COMMENT"
.LC72:
	.string	"CPP_MACRO_ARG"
.LC73:
	.string	"CPP_PADDING"
	.section	.data.rel.ro.local
	.align 32
	.type	token_spellings, @object
	.size	token_spellings, 544
token_spellings:
	.long	0
	.long	.LC6
	.long	0
	.long	.LC7
	.long	0
	.long	.LC8
	.long	0
	.long	.LC9
	.long	0
	.long	.LC10
	.long	0
	.long	.LC11
	.long	0
	.long	.LC12
	.long	0
	.long	.LC13
	.long	0
	.long	.LC14
	.long	0
	.long	.LC15
	.long	0
	.long	.LC16
	.long	0
	.long	.LC17
	.long	0
	.long	.LC18
	.long	0
	.long	.LC19
	.long	0
	.long	.LC20
	.long	0
	.long	.LC21
	.long	0
	.long	.LC22
	.long	0
	.long	.LC23
	.long	0
	.long	.LC24
	.long	0
	.long	.LC25
	.long	0
	.long	.LC26
	.long	0
	.long	.LC27
	.long	0
	.long	.LC28
	.long	0
	.long	.LC29
	.long	5
	.long	.LC30
	.long	0
	.long	.LC31
	.long	0
	.long	.LC32
	.long	0
	.long	.LC33
	.long	0
	.long	.LC34
	.long	0
	.long	.LC35
	.long	0
	.long	.LC36
	.long	0
	.long	.LC37
	.long	0
	.long	.LC38
	.long	0
	.long	.LC39
	.long	0
	.long	.LC40
	.long	0
	.long	.LC41
	.long	0
	.long	.LC42
	.long	0
	.long	.LC43
	.long	0
	.long	.LC44
	.long	0
	.long	.LC45
	.long	0
	.long	.LC46
	.long	0
	.long	.LC47
	.long	0
	.long	.LC48
	.long	0
	.long	.LC49
	.long	0
	.long	.LC50
	.long	0
	.long	.LC51
	.long	0
	.long	.LC52
	.long	0
	.long	.LC53
	.long	0
	.long	.LC54
	.long	0
	.long	.LC55
	.long	0
	.long	.LC56
	.long	0
	.long	.LC57
	.long	0
	.long	.LC58
	.long	0
	.long	.LC59
	.long	0
	.long	.LC60
	.long	0
	.long	.LC61
	.long	0
	.long	.LC62
	.long	2
	.long	.LC63
	.long	3
	.long	.LC64
	.long	4
	.long	.LC65
	.long	4
	.long	.LC66
	.long	1
	.long	.LC67
	.long	4
	.long	.LC68
	.long	4
	.long	.LC69
	.long	4
	.long	.LC70
	.long	3
	.long	.LC71
	.long	5
	.long	.LC72
	.long	5
	.long	.LC73
	.text
.globl cpp_ideq
	.type	cpp_ideq, @function
cpp_ideq:
.LFB21:
	.file 1 "../../../kg++fe/gnu/cpplex.c"
	.loc 1 107 0
	pushl	%ebp
.LCFI0:
	movl	%esp, %ebp
.LCFI1:
	subl	$24, %esp
.LCFI2:
	.loc 1 108 0
	movl	8(%ebp), %eax
	movzbl	6(%eax), %eax
	cmpb	$57, %al
	je	.L2
	.loc 1 109 0
	movl	$0, -4(%ebp)
	jmp	.L4
.L2:
	.loc 1 111 0
	movl	12(%ebp), %edx
	movl	8(%ebp), %eax
	movl	8(%eax), %eax
	movl	(%eax), %eax
	movl	%edx, 4(%esp)
	movl	%eax, (%esp)
	call	ustrcmp
	testl	%eax, %eax
	sete	%al
	movzbl	%al, %eax
	movl	%eax, -4(%ebp)
.L4:
	movl	-4(%ebp), %eax
	.loc 1 112 0
	leave
	ret
.LFE21:
	.size	cpp_ideq, .-cpp_ideq
	.type	ustrcmp, @function
ustrcmp:
.LFB15:
	.file 2 "../../../kg++fe/gnu/cpphash.h"
	.loc 2 590 0
	pushl	%ebp
.LCFI3:
	movl	%esp, %ebp
.LCFI4:
	pushl	%ebx
.LCFI5:
	subl	$20, %esp
.LCFI6:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 2 591 0
	movl	12(%ebp), %eax
	movl	8(%ebp), %edx
	movl	%eax, 4(%esp)
	movl	%edx, (%esp)
	call	strcmp@PLT
	.loc 2 592 0
	addl	$20, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE15:
	.size	ustrcmp, .-ustrcmp
.globl seen_omp_paren
	.bss
	.type	seen_omp_paren, @object
	.size	seen_omp_paren, 1
seen_omp_paren:
	.zero	1
	.text
	.type	handle_newline, @function
handle_newline:
.LFB22:
	.loc 1 124 0
	pushl	%ebp
.LCFI7:
	movl	%esp, %ebp
.LCFI8:
	subl	$16, %esp
.LCFI9:
	.loc 1 125 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -4(%ebp)
	.loc 1 129 0
	movl	-4(%ebp), %eax
	movl	(%eax), %eax
	subl	$1, %eax
	movzbl	(%eax), %eax
	movzbl	%al, %edx
	movl	-4(%ebp), %eax
	movl	(%eax), %eax
	movzbl	(%eax), %eax
	movzbl	%al, %eax
	leal	(%edx,%eax), %eax
	cmpl	$23, %eax
	jne	.L9
	.loc 1 130 0
	movl	-4(%ebp), %eax
	movl	(%eax), %eax
	leal	1(%eax), %edx
	movl	-4(%ebp), %eax
	movl	%edx, (%eax)
.L9:
	.loc 1 132 0
	movl	-4(%ebp), %eax
	movl	(%eax), %edx
	movl	-4(%ebp), %eax
	movl	%edx, 12(%eax)
	.loc 1 133 0
	movl	-4(%ebp), %eax
	movl	$0, 32(%eax)
	.loc 1 134 0
	movl	8(%ebp), %eax
	movl	52(%eax), %eax
	leal	1(%eax), %edx
	movl	8(%ebp), %eax
	movl	%edx, 52(%eax)
	.loc 1 135 0
	leave
	ret
.LFE22:
	.size	handle_newline, .-handle_newline
	.section	.rodata
.LC74:
	.string	"trigraph ??%c converted to %c"
.LC75:
	.string	"trigraph ??%c ignored"
	.text
	.type	trigraph_p, @function
trigraph_p:
.LFB23:
	.loc 1 146 0
	pushl	%ebp
.LCFI10:
	movl	%esp, %ebp
.LCFI11:
	pushl	%edi
.LCFI12:
	pushl	%esi
.LCFI13:
	pushl	%ebx
.LCFI14:
	subl	$60, %esp
.LCFI15:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 147 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -24(%ebp)
	.loc 1 148 0
	movl	-24(%ebp), %eax
	movl	(%eax), %eax
	addl	$1, %eax
	movzbl	(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, -20(%ebp)
	.loc 1 151 0
	movl	-20(%ebp), %edx
	movl	_cpp_trigraph_map@GOT(%ebx), %eax
	movzbl	(%eax,%edx), %eax
	testb	%al, %al
	jne	.L13
	.loc 1 152 0
	movl	$0, -32(%ebp)
	jmp	.L15
.L13:
	.loc 1 154 0
	movl	8(%ebp), %eax
	movzbl	398(%eax), %eax
	testb	%al, %al
	setne	%al
	movb	%al, -13(%ebp)
	.loc 1 157 0
	movl	8(%ebp), %eax
	movzbl	408(%eax), %eax
	testb	%al, %al
	je	.L16
	movl	8(%ebp), %eax
	movzbl	14(%eax), %eax
	testb	%al, %al
	jne	.L16
	.loc 1 159 0
	cmpb	$0, -13(%ebp)
	je	.L19
	.loc 1 160 0
	movl	-20(%ebp), %edx
	movl	_cpp_trigraph_map@GOT(%ebx), %eax
	movzbl	(%eax,%edx), %eax
	movzbl	%al, %esi
	movl	-20(%ebp), %edi
	movl	-24(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, %edx
	movl	-24(%ebp), %eax
	movl	12(%eax), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	movl	%eax, %edx
	movl	-24(%ebp), %eax
	movl	32(%eax), %eax
	leal	(%edx,%eax), %eax
	leal	-1(%eax), %edx
	movl	8(%ebp), %eax
	movl	52(%eax), %ecx
	movl	%esi, 24(%esp)
	movl	%edi, 20(%esp)
	leal	.LC74@GOTOFF(%ebx), %eax
	movl	%eax, 16(%esp)
	movl	%edx, 12(%esp)
	movl	%ecx, 8(%esp)
	movl	$0, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error_with_line@PLT
	jmp	.L16
.L19:
	.loc 1 165 0
	movl	-24(%ebp), %eax
	movl	(%eax), %edx
	movl	-24(%ebp), %eax
	movl	40(%eax), %eax
	cmpl	%eax, %edx
	je	.L16
	.loc 1 167 0
	movl	-24(%ebp), %eax
	movl	(%eax), %edx
	movl	-24(%ebp), %eax
	movl	%edx, 40(%eax)
	.loc 1 168 0
	movl	-20(%ebp), %esi
	movl	-24(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, %edx
	movl	-24(%ebp), %eax
	movl	12(%eax), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	movl	%eax, %edx
	movl	-24(%ebp), %eax
	movl	32(%eax), %eax
	leal	(%edx,%eax), %eax
	leal	-1(%eax), %ecx
	movl	8(%ebp), %eax
	movl	52(%eax), %edx
	movl	%esi, 20(%esp)
	leal	.LC75@GOTOFF(%ebx), %eax
	movl	%eax, 16(%esp)
	movl	%ecx, 12(%esp)
	movl	%edx, 8(%esp)
	movl	$0, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error_with_line@PLT
.L16:
	.loc 1 174 0
	movzbl	-13(%ebp), %eax
	movl	%eax, -32(%ebp)
.L15:
	movl	-32(%ebp), %eax
	.loc 1 175 0
	addl	$60, %esp
	popl	%ebx
	popl	%esi
	popl	%edi
	popl	%ebp
	ret
.LFE23:
	.size	trigraph_p, .-trigraph_p
	.section	.rodata
	.align 4
.LC76:
	.string	"backslash and newline separated by space"
	.align 4
.LC77:
	.string	"backslash-newline at end of file"
	.text
	.type	skip_escaped_newlines, @function
skip_escaped_newlines:
.LFB24:
	.loc 1 184 0
	pushl	%ebp
.LCFI16:
	movl	%esp, %ebp
.LCFI17:
	pushl	%ebx
.LCFI18:
	subl	$36, %esp
.LCFI19:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 185 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -20(%ebp)
	.loc 1 186 0
	movl	-20(%ebp), %eax
	movl	(%eax), %eax
	subl	$1, %eax
	movzbl	(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, -16(%ebp)
	.loc 1 189 0
	movl	-20(%ebp), %eax
	movzbl	45(%eax), %eax
	testb	%al, %al
	jne	.L24
	jmp	.L44
.L25:
.L44:
.LBB2:
	.loc 1 196 0
	cmpl	$63, -16(%ebp)
	jne	.L26
	.loc 1 198 0
	movl	-20(%ebp), %eax
	movl	(%eax), %eax
	movzbl	(%eax), %eax
	cmpb	$63, %al
	jne	.L24
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	trigraph_p
	xorl	$1, %eax
	testb	%al, %al
	jne	.L24
	.loc 1 202 0
	movl	-20(%ebp), %eax
	movl	(%eax), %eax
	addl	$1, %eax
	movzbl	(%eax), %eax
	movzbl	%al, %edx
	movl	_cpp_trigraph_map@GOT(%ebx), %eax
	movzbl	(%eax,%edx), %eax
	movzbl	%al, %eax
	movl	%eax, -16(%ebp)
	.loc 1 203 0
	movl	-20(%ebp), %eax
	movl	(%eax), %eax
	leal	2(%eax), %edx
	movl	-20(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 204 0
	cmpl	$92, -16(%ebp)
	jne	.L24
.L26:
	.loc 1 208 0
	movl	-20(%ebp), %eax
	movl	(%eax), %edx
	movl	-20(%ebp), %eax
	movl	8(%eax), %eax
	cmpl	%eax, %edx
	je	.L24
	.loc 1 213 0
	movl	-20(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -12(%ebp)
.L31:
	.loc 1 215 0
	movl	-20(%ebp), %eax
	movl	(%eax), %edx
	movzbl	(%edx), %eax
	movzbl	%al, %eax
	movl	%eax, -8(%ebp)
	addl	$1, %edx
	movl	-20(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 216 0
	movzbl	-8(%ebp),%edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$2048, %eax
	testl	%eax, %eax
	je	.L32
	movl	-20(%ebp), %eax
	movl	(%eax), %edx
	movl	-20(%ebp), %eax
	movl	8(%eax), %eax
	cmpl	%eax, %edx
	jb	.L31
.L32:
	.loc 1 218 0
	movzbl	-8(%ebp),%edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$1024, %eax
	testl	%eax, %eax
	jne	.L34
	.loc 1 220 0
	movl	-20(%ebp), %edx
	movl	-12(%ebp), %eax
	movl	%eax, (%edx)
	.loc 1 221 0
	jmp	.L24
.L34:
	.loc 1 224 0
	movl	-20(%ebp), %eax
	movl	(%eax), %eax
	subl	$1, %eax
	cmpl	-12(%ebp), %eax
	je	.L36
	movl	8(%ebp), %eax
	movzbl	14(%eax), %eax
	testb	%al, %al
	jne	.L36
	.loc 1 226 0
	leal	.LC76@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$0, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L36:
	.loc 1 229 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	handle_newline
	.loc 1 230 0
	movl	-20(%ebp), %eax
	movl	(%eax), %edx
	movl	-20(%ebp), %eax
	movl	%edx, 4(%eax)
	.loc 1 231 0
	movl	-20(%ebp), %eax
	movl	(%eax), %edx
	movl	-20(%ebp), %eax
	movl	8(%eax), %eax
	cmpl	%eax, %edx
	jne	.L39
	.loc 1 233 0
	leal	.LC77@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$2, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	.loc 1 235 0
	movl	$-1, -16(%ebp)
	jmp	.L41
.L39:
	.loc 1 238 0
	movl	-20(%ebp), %eax
	movl	(%eax), %edx
	movzbl	(%edx), %eax
	movzbl	%al, %eax
	movl	%eax, -16(%ebp)
	addl	$1, %edx
	movl	-20(%ebp), %eax
	movl	%edx, (%eax)
.L41:
	.loc 1 240 0
	cmpl	$92, -16(%ebp)
	je	.L25
	cmpl	$63, -16(%ebp)
	je	.L25
.L24:
.LBE2:
	.loc 1 243 0
	movl	-16(%ebp), %eax
	.loc 1 244 0
	addl	$36, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE24:
	.size	skip_escaped_newlines, .-skip_escaped_newlines
	.type	get_effective_char, @function
get_effective_char:
.LFB25:
	.loc 1 254 0
	pushl	%ebp
.LCFI20:
	movl	%esp, %ebp
.LCFI21:
	subl	$24, %esp
.LCFI22:
	.loc 1 256 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -4(%ebp)
	.loc 1 258 0
	movl	-4(%ebp), %eax
	movl	(%eax), %edx
	movl	-4(%ebp), %eax
	movl	%edx, 4(%eax)
	.loc 1 259 0
	movl	-4(%ebp), %eax
	movl	(%eax), %edx
	movzbl	(%edx), %eax
	movzbl	%al, %eax
	movl	%eax, -8(%ebp)
	addl	$1, %edx
	movl	-4(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 260 0
	cmpl	$63, -8(%ebp)
	je	.L46
	cmpl	$92, -8(%ebp)
	jne	.L48
.L46:
	movl	$1, -20(%ebp)
	jmp	.L49
.L48:
	movl	$0, -20(%ebp)
.L49:
	movl	-20(%ebp), %eax
	testl	%eax, %eax
	je	.L50
	.loc 1 261 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	skip_escaped_newlines
	movl	%eax, -8(%ebp)
.L50:
	.loc 1 263 0
	movl	-8(%ebp), %eax
	.loc 1 264 0
	leave
	ret
.LFE25:
	.size	get_effective_char, .-get_effective_char
	.section	.rodata
.LC78:
	.string	"\"/*\" within comment"
	.text
	.type	skip_block_comment, @function
skip_block_comment:
.LFB26:
	.loc 1 272 0
	pushl	%ebp
.LCFI23:
	movl	%esp, %ebp
.LCFI24:
	pushl	%ebx
.LCFI25:
	subl	$52, %esp
.LCFI26:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 273 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -16(%ebp)
	.loc 1 274 0
	movl	$-1, -12(%ebp)
	movl	$-1, -8(%ebp)
	.loc 1 276 0
	movl	8(%ebp), %eax
	movb	$1, 14(%eax)
	.loc 1 277 0
	jmp	.L75
.L55:
	.loc 1 279 0
	movl	-12(%ebp), %eax
	movl	%eax, -8(%ebp)
	movl	-16(%ebp), %eax
	movl	(%eax), %edx
	movzbl	(%edx), %eax
	movzbl	%al, %eax
	movl	%eax, -12(%ebp)
	addl	$1, %edx
	movl	-16(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 283 0
	cmpl	$63, -12(%ebp)
	je	.L56
	cmpl	$92, -12(%ebp)
	jne	.L58
.L56:
	.loc 1 284 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	skip_escaped_newlines
	movl	%eax, -12(%ebp)
.L58:
	.loc 1 288 0
	cmpl	$47, -12(%ebp)
	jne	.L59
	.loc 1 290 0
	cmpl	$42, -8(%ebp)
	je	.L61
	.loc 1 296 0
	movl	8(%ebp), %eax
	movzbl	407(%eax), %eax
	testb	%al, %al
	je	.L54
	movl	-16(%ebp), %eax
	movl	(%eax), %eax
	movzbl	(%eax), %eax
	cmpb	$42, %al
	jne	.L54
	movl	-16(%ebp), %eax
	movl	(%eax), %eax
	addl	$1, %eax
	movzbl	(%eax), %eax
	cmpb	$47, %al
	je	.L54
	.loc 1 298 0
	movl	-16(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, %edx
	movl	-16(%ebp), %eax
	movl	12(%eax), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	movl	%eax, %edx
	movl	-16(%ebp), %eax
	movl	32(%eax), %eax
	addl	%eax, %edx
	movl	8(%ebp), %eax
	movl	52(%eax), %ecx
	leal	.LC78@GOTOFF(%ebx), %eax
	movl	%eax, 16(%esp)
	movl	%edx, 12(%esp)
	movl	%ecx, 8(%esp)
	movl	$0, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error_with_line@PLT
	jmp	.L54
.L59:
	.loc 1 302 0
	movzbl	-12(%ebp),%edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$1024, %eax
	testl	%eax, %eax
	je	.L67
	.loc 1 303 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	handle_newline
	jmp	.L54
.L67:
	.loc 1 304 0
	cmpl	$9, -12(%ebp)
	jne	.L54
	.loc 1 305 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	adjust_column
.L54:
.L75:
	.loc 1 277 0
	movl	-16(%ebp), %eax
	movl	(%eax), %edx
	movl	-16(%ebp), %eax
	movl	8(%eax), %eax
	cmpl	%eax, %edx
	jne	.L55
.L61:
	.loc 1 308 0
	movl	8(%ebp), %eax
	movb	$0, 14(%eax)
	.loc 1 309 0
	cmpl	$47, -12(%ebp)
	jne	.L70
	cmpl	$42, -8(%ebp)
	je	.L72
.L70:
	movl	$1, -24(%ebp)
	jmp	.L73
.L72:
	movl	$0, -24(%ebp)
.L73:
	movl	-24(%ebp), %eax
	.loc 1 310 0
	addl	$52, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE26:
	.size	skip_block_comment, .-skip_block_comment
	.type	skip_line_comment, @function
skip_line_comment:
.LFB27:
	.loc 1 318 0
	pushl	%ebp
.LCFI27:
	movl	%esp, %ebp
.LCFI28:
	pushl	%ebx
.LCFI29:
	subl	$20, %esp
.LCFI30:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 319 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -16(%ebp)
	.loc 1 320 0
	movl	8(%ebp), %eax
	movl	52(%eax), %eax
	movl	%eax, -12(%ebp)
	.loc 1 327 0
	movl	8(%ebp), %eax
	movb	$1, 14(%eax)
.L77:
	.loc 1 334 0
	movl	-16(%ebp), %eax
	movl	(%eax), %edx
	movl	-16(%ebp), %eax
	movl	8(%eax), %eax
	cmpl	%eax, %edx
	je	.L78
	.loc 1 353 0
	movl	-16(%ebp), %eax
	movl	(%eax), %edx
	movzbl	(%edx), %eax
	movzbl	%al, %eax
	movl	%eax, -8(%ebp)
	addl	$1, %edx
	movl	-16(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 355 0
	cmpl	$63, -8(%ebp)
	je	.L80
	cmpl	$92, -8(%ebp)
	jne	.L82
.L80:
	.loc 1 356 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	skip_escaped_newlines
	movl	%eax, -8(%ebp)
.L82:
	.loc 1 358 0
	movzbl	-8(%ebp),%edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$1024, %eax
	testl	%eax, %eax
	je	.L77
	.loc 1 361 0
	movl	-16(%ebp), %eax
	movl	(%eax), %eax
	leal	-1(%eax), %edx
	movl	-16(%ebp), %eax
	movl	%edx, (%eax)
.L78:
	.loc 1 364 0
	movl	8(%ebp), %eax
	movb	$0, 14(%eax)
	.loc 1 365 0
	movl	8(%ebp), %eax
	movl	52(%eax), %eax
	cmpl	-12(%ebp), %eax
	setne	%al
	movzbl	%al, %eax
	.loc 1 366 0
	addl	$20, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE27:
	.size	skip_line_comment, .-skip_line_comment
	.type	adjust_column, @function
adjust_column:
.LFB28:
	.loc 1 373 0
	pushl	%ebp
.LCFI31:
	movl	%esp, %ebp
.LCFI32:
	pushl	%edi
.LCFI33:
	pushl	%esi
.LCFI34:
	subl	$16, %esp
.LCFI35:
	.loc 1 374 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -16(%ebp)
	.loc 1 375 0
	movl	-16(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, %edx
	movl	-16(%ebp), %eax
	movl	12(%eax), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	movl	%eax, %edx
	movl	-16(%ebp), %eax
	movl	32(%eax), %eax
	leal	(%edx,%eax), %eax
	subl	$1, %eax
	movl	%eax, -12(%ebp)
	.loc 1 379 0
	movl	-16(%ebp), %eax
	movl	32(%eax), %ecx
	movl	8(%ebp), %eax
	movl	360(%eax), %esi
	movl	8(%ebp), %eax
	movl	360(%eax), %edx
	movl	-12(%ebp), %eax
	movl	%edx, %edi
	movl	$0, %edx
	divl	%edi
	movl	%edx, %eax
	movl	%esi, %edx
	subl	%eax, %edx
	movl	%edx, %eax
	leal	(%ecx,%eax), %eax
	leal	-1(%eax), %edx
	movl	-16(%ebp), %eax
	movl	%edx, 32(%eax)
	.loc 1 381 0
	addl	$16, %esp
	popl	%esi
	popl	%edi
	popl	%ebp
	ret
.LFE28:
	.size	adjust_column, .-adjust_column
	.section	.rodata
.LC79:
	.string	"null character(s) ignored"
.LC80:
	.string	"form feed"
.LC81:
	.string	"vertical tab"
.LC82:
	.string	"%s in preprocessing directive"
	.text
	.type	skip_whitespace, @function
skip_whitespace:
.LFB29:
	.loc 1 390 0
	pushl	%ebp
.LCFI36:
	movl	%esp, %ebp
.LCFI37:
	pushl	%ebx
.LCFI38:
	subl	$52, %esp
.LCFI39:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 391 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -12(%ebp)
	.loc 1 392 0
	movl	$0, -8(%ebp)
.L88:
	.loc 1 397 0
	cmpl	$32, 12(%ebp)
	je	.L89
	.loc 1 399 0
	cmpl	$9, 12(%ebp)
	jne	.L91
	.loc 1 400 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	adjust_column
	jmp	.L89
.L91:
	.loc 1 402 0
	cmpl	$0, 12(%ebp)
	jne	.L93
	.loc 1 404 0
	movl	-12(%ebp), %eax
	movl	(%eax), %eax
	leal	-1(%eax), %edx
	movl	-12(%ebp), %eax
	movl	8(%eax), %eax
	cmpl	%eax, %edx
	jne	.L95
	.loc 1 405 0
	movl	$0, -28(%ebp)
	jmp	.L97
.L95:
	.loc 1 406 0
	cmpl	$0, -8(%ebp)
	jne	.L89
	.loc 1 408 0
	leal	.LC79@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$0, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	.loc 1 409 0
	movl	$1, -8(%ebp)
	jmp	.L89
.L93:
	.loc 1 412 0
	movl	8(%ebp), %eax
	movzbl	8(%eax), %eax
	testb	%al, %al
	je	.L89
	movl	8(%ebp), %eax
	movzbl	425(%eax), %eax
	testb	%al, %al
	je	.L89
	.loc 1 413 0
	cmpl	$12, 12(%ebp)
	jne	.L102
	leal	.LC80@GOTOFF(%ebx), %eax
	movl	%eax, -24(%ebp)
	jmp	.L104
.L102:
	leal	.LC81@GOTOFF(%ebx), %ecx
	movl	%ecx, -24(%ebp)
.L104:
	movl	-12(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, %edx
	movl	-12(%ebp), %eax
	movl	12(%eax), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	movl	%eax, %edx
	movl	-12(%ebp), %eax
	movl	32(%eax), %eax
	leal	(%edx,%eax), %ecx
	movl	8(%ebp), %eax
	movl	52(%eax), %edx
	movl	-24(%ebp), %eax
	movl	%eax, 20(%esp)
	leal	.LC82@GOTOFF(%ebx), %eax
	movl	%eax, 16(%esp)
	movl	%ecx, 12(%esp)
	movl	%edx, 8(%esp)
	movl	$2, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error_with_line@PLT
.L89:
	.loc 1 418 0
	movl	-12(%ebp), %eax
	movl	(%eax), %edx
	movzbl	(%edx), %eax
	movzbl	%al, %eax
	movl	%eax, 12(%ebp)
	addl	$1, %edx
	movl	-12(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 421 0
	movzbl	12(%ebp),%edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$2048, %eax
	testl	%eax, %eax
	jne	.L88
	.loc 1 423 0
	movl	-12(%ebp), %eax
	movl	(%eax), %eax
	leal	-1(%eax), %edx
	movl	-12(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 424 0
	movl	$1, -28(%ebp)
.L97:
	movl	-28(%ebp), %eax
	.loc 1 425 0
	addl	$52, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE29:
	.size	skip_whitespace, .-skip_whitespace
	.type	name_p, @function
name_p:
.LFB30:
	.loc 1 433 0
	pushl	%ebp
.LCFI40:
	movl	%esp, %ebp
.LCFI41:
	subl	$20, %esp
.LCFI42:
	call	__i686.get_pc_thunk.cx
	addl	$_GLOBAL_OFFSET_TABLE_, %ecx
	.loc 1 436 0
	movl	$0, -4(%ebp)
	jmp	.L108
.L109:
	.loc 1 437 0
	movl	12(%ebp), %eax
	movl	4(%eax), %edx
	movl	-4(%ebp), %eax
	leal	(%edx,%eax), %eax
	movzbl	(%eax), %eax
	movzbl	%al, %edx
	movl	_sch_istable@GOT(%ecx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$516, %eax
	testl	%eax, %eax
	jne	.L110
	movl	12(%ebp), %eax
	movl	4(%eax), %edx
	movl	-4(%ebp), %eax
	leal	(%edx,%eax), %eax
	movzbl	(%eax), %eax
	cmpb	$36, %al
	jne	.L112
	movl	8(%ebp), %eax
	movzbl	420(%eax), %eax
	testb	%al, %al
	jne	.L110
.L112:
	.loc 1 438 0
	movl	$0, -20(%ebp)
	jmp	.L114
.L110:
	.loc 1 436 0
	addl	$1, -4(%ebp)
.L108:
	movl	12(%ebp), %eax
	movl	(%eax), %eax
	cmpl	-4(%ebp), %eax
	ja	.L109
	.loc 1 440 0
	movl	$1, -20(%ebp)
.L114:
	movl	-20(%ebp), %eax
	.loc 1 441 0
	leave
	ret
.LFE30:
	.size	name_p, .-name_p
	.section	.rodata
.LC83:
	.string	"attempt to use poisoned \"%s\""
	.align 4
.LC84:
	.string	"__VA_ARGS__ can only appear in the expansion of a C99 variadic macro"
	.text
	.type	parse_identifier, @function
parse_identifier:
.LFB31:
	.loc 1 454 0
	pushl	%ebp
.LCFI43:
	movl	%esp, %ebp
.LCFI44:
	pushl	%ebx
.LCFI45:
	subl	$36, %esp
.LCFI46:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 460 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	(%eax), %eax
	movl	%eax, -12(%ebp)
	.loc 1 461 0
	jmp	.L118
.L119:
	.loc 1 462 0
	addl	$1, -12(%ebp)
.L118:
	.loc 1 461 0
	movl	-12(%ebp), %eax
	movzbl	(%eax), %eax
	movzbl	%al, %edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$516, %eax
	testl	%eax, %eax
	jne	.L119
	.loc 1 465 0
	movl	-12(%ebp), %eax
	movzbl	(%eax), %eax
	cmpb	$63, %al
	je	.L121
	movl	-12(%ebp), %eax
	movzbl	(%eax), %eax
	cmpb	$92, %al
	je	.L121
	movl	-12(%ebp), %eax
	movzbl	(%eax), %eax
	cmpb	$36, %al
	jne	.L124
.L121:
.LBB3:
	.loc 1 469 0
	leal	-20(%ebp), %eax
	movl	%eax, 12(%esp)
	movl	$0, 8(%esp)
	movl	-12(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	parse_slow
	movl	%eax, -8(%ebp)
	.loc 1 470 0
	movl	-20(%ebp), %edx
	movl	8(%ebp), %eax
	movl	348(%eax), %ecx
	movl	$2, 12(%esp)
	movl	%edx, 8(%esp)
	movl	-8(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	%ecx, (%esp)
	call	ht_lookup@PLT
	movl	%eax, -16(%ebp)
	.loc 1 465 0
	jmp	.L125
.L124:
.LBE3:
	.loc 1 475 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	(%eax), %eax
	subl	$1, %eax
	movl	%eax, -8(%ebp)
	.loc 1 476 0
	movl	8(%ebp), %eax
	movl	(%eax), %edx
	movl	-12(%ebp), %eax
	movl	%eax, (%edx)
	.loc 1 477 0
	movl	-12(%ebp), %edx
	movl	-8(%ebp), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	movl	%eax, %edx
	movl	8(%ebp), %eax
	movl	348(%eax), %ecx
	movl	$1, 12(%esp)
	movl	%edx, 8(%esp)
	movl	-8(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	%ecx, (%esp)
	call	ht_lookup@PLT
	movl	%eax, -16(%ebp)
.L125:
	.loc 1 483 0
	movl	-16(%ebp), %eax
	movzbl	17(%eax), %eax
	movzbl	%al, %eax
	andl	$8, %eax
	testl	%eax, %eax
	je	.L126
	movl	8(%ebp), %eax
	movzbl	10(%eax), %eax
	testb	%al, %al
	jne	.L126
	movl	$1, -24(%ebp)
	jmp	.L129
.L126:
	movl	$0, -24(%ebp)
.L129:
	movl	-24(%ebp), %eax
	testl	%eax, %eax
	je	.L130
	.loc 1 487 0
	movl	-16(%ebp), %eax
	movzbl	17(%eax), %eax
	movzbl	%al, %eax
	andl	$2, %eax
	testl	%eax, %eax
	je	.L132
	movl	8(%ebp), %eax
	movzbl	16(%eax), %eax
	testb	%al, %al
	jne	.L132
	.loc 1 488 0
	movl	-16(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, 12(%esp)
	leal	.LC83@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$3, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L132:
	.loc 1 493 0
	movl	8(%ebp), %eax
	movl	476(%eax), %eax
	cmpl	-16(%ebp), %eax
	jne	.L130
	movl	8(%ebp), %eax
	movzbl	15(%eax), %eax
	testb	%al, %al
	jne	.L130
	.loc 1 495 0
	leal	.LC84@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$2, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L130:
	.loc 1 499 0
	movl	-16(%ebp), %eax
	.loc 1 500 0
	addl	$36, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE31:
	.size	parse_identifier, .-parse_identifier
	.section	.rodata
	.align 4
.LC85:
	.string	"'$' character(s) in identifier or number"
	.text
	.type	parse_slow, @function
parse_slow:
.LFB32:
	.loc 1 514 0
	pushl	%ebp
.LCFI47:
	movl	%esp, %ebp
.LCFI48:
	pushl	%edi
.LCFI49:
	pushl	%esi
.LCFI50:
	pushl	%ebx
.LCFI51:
	subl	$76, %esp
.LCFI52:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 515 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -68(%ebp)
	.loc 1 516 0
	movl	-68(%ebp), %eax
	movl	(%eax), %eax
	subl	$1, %eax
	movl	%eax, -64(%ebp)
	.loc 1 517 0
	movl	8(%ebp), %eax
	movl	348(%eax), %eax
	movl	%eax, -60(%ebp)
	.loc 1 518 0
	movl	$0, -48(%ebp)
	.loc 1 521 0
	cmpl	$2, 16(%ebp)
	jne	.L139
.LBB4:
	.loc 1 522 0
	movl	-60(%ebp), %eax
	movl	%eax, -44(%ebp)
	movl	-44(%ebp), %eax
	movl	12(%eax), %eax
	leal	1(%eax), %edx
	movl	-44(%ebp), %eax
	movl	16(%eax), %eax
	cmpl	%eax, %edx
	jbe	.L141
	movl	$1, 4(%esp)
	movl	-44(%ebp), %eax
	movl	%eax, (%esp)
	call	_obstack_newchunk@PLT
.L141:
	movl	-44(%ebp), %eax
	movl	12(%eax), %eax
	movb	$46, (%eax)
	leal	1(%eax), %edx
	movl	-44(%ebp), %eax
	movl	%edx, 12(%eax)
.L139:
.LBE4:
.LBB5:
	.loc 1 525 0
	movl	-60(%ebp), %eax
	movl	%eax, -40(%ebp)
	movl	12(%ebp), %edx
	movl	-64(%ebp), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	movl	%eax, -36(%ebp)
	movl	-40(%ebp), %eax
	movl	12(%eax), %edx
	movl	-36(%ebp), %eax
	addl	%eax, %edx
	movl	-40(%ebp), %eax
	movl	16(%eax), %eax
	cmpl	%eax, %edx
	jbe	.L143
	movl	-36(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	-40(%ebp), %eax
	movl	%eax, (%esp)
	call	_obstack_newchunk@PLT
.L143:
	movl	-36(%ebp), %ecx
	movl	-40(%ebp), %eax
	movl	12(%eax), %eax
	movl	-64(%ebp), %edx
	movl	%eax, %edi
	movl	%edx, %esi
	cld
	rep
	movsb
	movl	-40(%ebp), %eax
	movl	12(%eax), %edx
	movl	-36(%ebp), %eax
	addl	%eax, %edx
	movl	-40(%ebp), %eax
	movl	%edx, 12(%eax)
.LBE5:
	.loc 1 529 0
	movl	12(%ebp), %eax
	subl	$1, %eax
	movzbl	(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, -52(%ebp)
	.loc 1 530 0
	movl	12(%ebp), %eax
	movzbl	(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, -56(%ebp)
	addl	$1, 12(%ebp)
	.loc 1 531 0
	movl	-68(%ebp), %edx
	movl	12(%ebp), %eax
	movl	%eax, (%edx)
	jmp	.L180
.L145:
.L180:
	.loc 1 535 0
	movl	-68(%ebp), %eax
	movl	(%eax), %eax
	leal	-1(%eax), %edx
	movl	-68(%ebp), %eax
	movl	%edx, 4(%eax)
	.loc 1 536 0
	cmpl	$63, -56(%ebp)
	je	.L146
	cmpl	$92, -56(%ebp)
	jne	.L148
.L146:
	.loc 1 537 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	skip_escaped_newlines
	movl	%eax, -56(%ebp)
.L148:
	.loc 1 539 0
	movzbl	-56(%ebp),%edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$516, %eax
	testl	%eax, %eax
	jne	.L181
	cmpl	$36, -56(%ebp)
	jne	.L151
	movl	8(%ebp), %eax
	movzbl	420(%eax), %eax
	testb	%al, %al
	jne	.L181
.L151:
	.loc 1 541 0
	cmpl	$0, 16(%ebp)
	je	.L153
	.loc 1 543 0
	cmpl	$46, -56(%ebp)
	je	.L181
	cmpl	$43, -56(%ebp)
	je	.L156
	cmpl	$45, -56(%ebp)
	jne	.L153
.L156:
	cmpl	$101, -52(%ebp)
	je	.L181
	cmpl	$69, -52(%ebp)
	je	.L181
	cmpl	$112, -52(%ebp)
	je	.L160
	cmpl	$80, -52(%ebp)
	jne	.L153
.L160:
	movl	8(%ebp), %eax
	movzbl	400(%eax), %eax
	testb	%al, %al
	je	.L153
	jmp	.L181
.L149:
.L181:
	.loc 1 550 0
	movl	-56(%ebp), %eax
	movl	%eax, -52(%ebp)
.LBB6:
	.loc 1 551 0
	movl	-60(%ebp), %eax
	movl	%eax, -32(%ebp)
	movl	-32(%ebp), %eax
	movl	12(%eax), %eax
	leal	1(%eax), %edx
	movl	-32(%ebp), %eax
	movl	16(%eax), %eax
	cmpl	%eax, %edx
	jbe	.L162
	movl	$1, 4(%esp)
	movl	-32(%ebp), %eax
	movl	%eax, (%esp)
	call	_obstack_newchunk@PLT
.L162:
	movl	-32(%ebp), %eax
	movl	12(%eax), %edx
	movl	-56(%ebp), %eax
	movb	%al, (%edx)
	addl	$1, %edx
	movl	-32(%ebp), %eax
	movl	%edx, 12(%eax)
.LBE6:
	.loc 1 553 0
	cmpl	$36, -56(%ebp)
	jne	.L164
	.loc 1 554 0
	addl	$1, -48(%ebp)
.L164:
	.loc 1 556 0
	movl	-68(%ebp), %eax
	movl	(%eax), %edx
	movzbl	(%edx), %eax
	movzbl	%al, %eax
	movl	%eax, -56(%ebp)
	addl	$1, %edx
	movl	-68(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 558 0
	movzbl	-56(%ebp),%edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$516, %eax
	testl	%eax, %eax
	jne	.L149
	cmpl	$36, -56(%ebp)
	jne	.L145
	movl	8(%ebp), %eax
	movzbl	420(%eax), %eax
	testb	%al, %al
	jne	.L149
	.loc 1 559 0
	jmp	.L145
.L153:
	.loc 1 562 0
	movl	-68(%ebp), %eax
	movl	4(%eax), %edx
	movl	-68(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 567 0
	cmpl	$0, -48(%ebp)
	je	.L169
	movl	8(%ebp), %eax
	movzbl	425(%eax), %eax
	testb	%al, %al
	je	.L169
	movl	8(%ebp), %eax
	movzbl	10(%eax), %eax
	testb	%al, %al
	jne	.L169
	.loc 1 568 0
	leal	.LC85@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$2, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L169:
.LBB7:
	.loc 1 571 0
	movl	-60(%ebp), %eax
	movl	%eax, -28(%ebp)
	movl	-28(%ebp), %eax
	movl	12(%eax), %eax
	movl	%eax, %edx
	movl	-28(%ebp), %eax
	movl	8(%eax), %eax
	movl	%edx, %esi
	subl	%eax, %esi
	movl	%esi, %eax
	movl	%eax, %edx
.LBE7:
	movl	20(%ebp), %eax
	movl	%edx, (%eax)
.LBB8:
	.loc 1 572 0
	movl	-60(%ebp), %eax
	movl	%eax, -24(%ebp)
	movl	-24(%ebp), %eax
	movl	12(%eax), %eax
	leal	1(%eax), %edx
	movl	-24(%ebp), %eax
	movl	16(%eax), %eax
	cmpl	%eax, %edx
	jbe	.L173
	movl	$1, 4(%esp)
	movl	-24(%ebp), %eax
	movl	%eax, (%esp)
	call	_obstack_newchunk@PLT
.L173:
	movl	-24(%ebp), %eax
	movl	12(%eax), %eax
	movb	$0, (%eax)
	leal	1(%eax), %edx
	movl	-24(%ebp), %eax
	movl	%edx, 12(%eax)
.LBE8:
.LBB9:
	.loc 1 573 0
	movl	-60(%ebp), %eax
	movl	%eax, -20(%ebp)
	movl	-20(%ebp), %eax
	movl	8(%eax), %eax
	movl	%eax, -16(%ebp)
	movl	-20(%ebp), %eax
	movl	12(%eax), %eax
	cmpl	-16(%ebp), %eax
	jne	.L175
	movl	-20(%ebp), %eax
	movzbl	40(%eax), %edx
	orl	$2, %edx
	movb	%dl, 40(%eax)
.L175:
	movl	-20(%ebp), %eax
	movl	12(%eax), %eax
	movl	%eax, %edx
	movl	-20(%ebp), %eax
	movl	24(%eax), %eax
	addl	%eax, %edx
	movl	-20(%ebp), %eax
	movl	24(%eax), %eax
	notl	%eax
	andl	%edx, %eax
	movl	%eax, %edx
	movl	-20(%ebp), %eax
	movl	%edx, 12(%eax)
	movl	-20(%ebp), %eax
	movl	12(%eax), %eax
	movl	%eax, %edx
	movl	-20(%ebp), %eax
	movl	4(%eax), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	-20(%ebp), %eax
	movl	16(%eax), %eax
	movl	%eax, %edx
	movl	-20(%ebp), %eax
	movl	4(%eax), %eax
	movl	%edx, %esi
	subl	%eax, %esi
	movl	%esi, %eax
	cmpl	%eax, %ecx
	jle	.L177
	movl	-20(%ebp), %eax
	movl	16(%eax), %edx
	movl	-20(%ebp), %eax
	movl	%edx, 12(%eax)
.L177:
	movl	-20(%ebp), %eax
	movl	12(%eax), %edx
	movl	-20(%ebp), %eax
	movl	%edx, 8(%eax)
	movl	-16(%ebp), %eax
.LBE9:
	.loc 1 574 0
	addl	$76, %esp
	popl	%ebx
	popl	%esi
	popl	%edi
	popl	%ebp
	ret
.LFE32:
	.size	parse_slow, .-parse_slow
	.type	parse_number, @function
parse_number:
.LFB33:
	.loc 1 584 0
	pushl	%ebp
.LCFI53:
	movl	%esp, %ebp
.LCFI54:
	pushl	%edi
.LCFI55:
	pushl	%esi
.LCFI56:
	pushl	%ebx
.LCFI57:
	subl	$44, %esp
.LCFI58:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 589 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	(%eax), %eax
	movl	%eax, -24(%ebp)
	.loc 1 590 0
	jmp	.L183
.L184:
	.loc 1 591 0
	addl	$1, -24(%ebp)
.L183:
	.loc 1 590 0
	movl	-24(%ebp), %eax
	movzbl	(%eax), %eax
	movzbl	%al, %edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$516, %eax
	testl	%eax, %eax
	jne	.L184
	movl	-24(%ebp), %eax
	movzbl	(%eax), %eax
	cmpb	$46, %al
	je	.L184
	movl	-24(%ebp), %eax
	movzbl	(%eax), %eax
	cmpb	$43, %al
	je	.L187
	movl	-24(%ebp), %eax
	movzbl	(%eax), %eax
	cmpb	$45, %al
	jne	.L189
.L187:
	movl	-24(%ebp), %eax
	subl	$1, %eax
	movzbl	(%eax), %eax
	cmpb	$101, %al
	je	.L184
	movl	-24(%ebp), %eax
	subl	$1, %eax
	movzbl	(%eax), %eax
	cmpb	$69, %al
	je	.L184
	movl	-24(%ebp), %eax
	subl	$1, %eax
	movzbl	(%eax), %eax
	cmpb	$112, %al
	je	.L192
	movl	-24(%ebp), %eax
	subl	$1, %eax
	movzbl	(%eax), %eax
	cmpb	$80, %al
	jne	.L189
.L192:
	movl	8(%ebp), %eax
	movzbl	400(%eax), %eax
	testb	%al, %al
	jne	.L184
.L189:
	.loc 1 594 0
	movl	-24(%ebp), %eax
	movzbl	(%eax), %eax
	cmpb	$63, %al
	je	.L194
	movl	-24(%ebp), %eax
	movzbl	(%eax), %eax
	cmpb	$92, %al
	je	.L194
	movl	-24(%ebp), %eax
	movzbl	(%eax), %eax
	cmpb	$36, %al
	jne	.L197
.L194:
	.loc 1 595 0
	movl	12(%ebp), %eax
	movl	16(%ebp), %edx
	addl	$1, %edx
	movl	%eax, 12(%esp)
	movl	%edx, 8(%esp)
	movl	-24(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	parse_slow
	movl	%eax, %edx
	movl	12(%ebp), %eax
	movl	%edx, 4(%eax)
	.loc 1 594 0
	jmp	.L201
.L197:
.LBB10:
	.loc 1 598 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	(%eax), %eax
	subl	$1, %eax
	movl	%eax, -20(%ebp)
	.loc 1 601 0
	movl	-24(%ebp), %edx
	movl	-20(%ebp), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	addl	16(%ebp), %eax
	movl	%eax, %edx
	movl	12(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 602 0
	movl	12(%ebp), %eax
	movl	(%eax), %eax
	addl	$1, %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_unaligned_alloc@PLT
	movl	%eax, -16(%ebp)
	.loc 1 603 0
	movl	12(%ebp), %eax
	movl	(%eax), %eax
	addl	-16(%ebp), %eax
	movb	$0, (%eax)
	.loc 1 604 0
	movl	12(%ebp), %edx
	movl	-16(%ebp), %eax
	movl	%eax, 4(%edx)
	.loc 1 606 0
	cmpl	$0, 16(%ebp)
	je	.L199
	.loc 1 607 0
	movl	-16(%ebp), %eax
	movb	$46, (%eax)
	addl	$1, -16(%ebp)
.L199:
	.loc 1 608 0
	movl	-24(%ebp), %edx
	movl	-20(%ebp), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	movl	%eax, %ecx
	movl	-16(%ebp), %eax
	movl	-20(%ebp), %edx
	movl	%eax, %edi
	movl	%edx, %esi
	cld
	rep
	movsb
	.loc 1 609 0
	movl	8(%ebp), %eax
	movl	(%eax), %edx
	movl	-24(%ebp), %eax
	movl	%eax, (%edx)
.L201:
.LBE10:
	.loc 1 611 0
	addl	$44, %esp
	popl	%ebx
	popl	%esi
	popl	%edi
	popl	%ebp
	ret
.LFE33:
	.size	parse_number, .-parse_number
	.type	unescaped_terminator_p, @function
unescaped_terminator_p:
.LFB34:
	.loc 1 618 0
	pushl	%ebp
.LCFI59:
	movl	%esp, %ebp
.LCFI60:
	subl	$20, %esp
.LCFI61:
	.loc 1 622 0
	movl	8(%ebp), %eax
	movzbl	11(%eax), %eax
	testb	%al, %al
	je	.L203
	.loc 1 623 0
	movl	$1, -20(%ebp)
	jmp	.L205
.L203:
	.loc 1 625 0
	movl	8(%ebp), %eax
	movl	64(%eax), %eax
	movl	8(%eax), %eax
	movl	%eax, -8(%ebp)
	.loc 1 629 0
	movl	12(%ebp), %eax
	movl	%eax, -4(%ebp)
	jmp	.L206
.L207:
	subl	$1, -4(%ebp)
.L206:
	movl	-4(%ebp), %eax
	cmpl	-8(%ebp), %eax
	jbe	.L208
	movl	-4(%ebp), %eax
	subl	$1, %eax
	movzbl	(%eax), %eax
	cmpb	$92, %al
	je	.L207
.L208:
	.loc 1 632 0
	movl	12(%ebp), %edx
	movl	-4(%ebp), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	andl	$1, %eax
	testl	%eax, %eax
	sete	%al
	movzbl	%al, %eax
	movl	%eax, -20(%ebp)
.L205:
	movl	-20(%ebp), %eax
	.loc 1 633 0
	leave
	ret
.LFE34:
	.size	unescaped_terminator_p, .-unescaped_terminator_p
	.section	.rodata
	.align 4
.LC86:
	.string	"missing terminating %c character"
	.align 4
.LC87:
	.string	"null character(s) preserved in literal"
	.text
	.type	parse_string, @function
parse_string:
.LFB35:
	.loc 1 647 0
	pushl	%ebp
.LCFI62:
	movl	%esp, %ebp
.LCFI63:
	pushl	%ebx
.LCFI64:
	subl	$52, %esp
.LCFI65:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 648 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -28(%ebp)
	.loc 1 651 0
	movb	$0, -5(%ebp)
	.loc 1 657 0
	movl	8(%ebp), %eax
	movl	64(%eax), %eax
	movl	8(%eax), %eax
	movl	%eax, -24(%ebp)
	.loc 1 658 0
	movl	8(%ebp), %eax
	movl	64(%eax), %eax
	movl	12(%eax), %eax
	movl	%eax, -20(%ebp)
.L212:
	.loc 1 667 0
	movl	-24(%ebp), %eax
	cmpl	-20(%ebp), %eax
	jne	.L213
.LBB11:
	.loc 1 669 0
	movl	-24(%ebp), %edx
	movl	8(%ebp), %eax
	movl	64(%eax), %eax
	movl	8(%eax), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	movl	%eax, -12(%ebp)
	.loc 1 670 0
	movl	8(%ebp), %eax
	addl	$64, %eax
	movl	$2, 8(%esp)
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_extend_buff@PLT
	.loc 1 671 0
	movl	8(%ebp), %eax
	movl	64(%eax), %eax
	movl	8(%eax), %edx
	movl	-12(%ebp), %eax
	leal	(%edx,%eax), %eax
	movl	%eax, -24(%ebp)
	.loc 1 672 0
	movl	8(%ebp), %eax
	movl	64(%eax), %eax
	movl	12(%eax), %eax
	movl	%eax, -20(%ebp)
.L213:
.LBE11:
	.loc 1 691 0
	movl	-28(%ebp), %eax
	movl	(%eax), %edx
	movzbl	(%edx), %eax
	movzbl	%al, %eax
	movl	%eax, -16(%ebp)
	addl	$1, %edx
	movl	-28(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 695 0
	cmpl	$63, -16(%ebp)
	je	.L215
	cmpl	$92, -16(%ebp)
	jne	.L217
.L215:
	.loc 1 696 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	skip_escaped_newlines
	movl	%eax, -16(%ebp)
.L217:
	.loc 1 698 0
	movl	-16(%ebp), %eax
	cmpl	16(%ebp), %eax
	jne	.L218
	.loc 1 700 0
	movl	-24(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	unescaped_terminator_p
	testl	%eax, %eax
	jne	.L220
	.loc 1 701 0
	jmp	.L222
.L218:
	.loc 1 703 0
	movzbl	-16(%ebp),%edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$1024, %eax
	testl	%eax, %eax
	je	.L223
.L224:
	.loc 1 710 0
	movl	8(%ebp), %eax
	movl	388(%eax), %eax
	cmpl	$7, %eax
	jne	.L225
	cmpl	$62, 16(%ebp)
	jne	.L227
.L225:
	.loc 1 711 0
	movl	16(%ebp), %eax
	movl	%eax, 12(%esp)
	leal	.LC86@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$3, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L227:
	.loc 1 713 0
	movl	-28(%ebp), %eax
	movl	(%eax), %eax
	leal	-1(%eax), %edx
	movl	-28(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 714 0
	jmp	.L220
.L223:
	.loc 1 716 0
	cmpl	$0, -16(%ebp)
	jne	.L222
	.loc 1 718 0
	movl	-28(%ebp), %eax
	movl	(%eax), %eax
	leal	-1(%eax), %edx
	movl	-28(%ebp), %eax
	movl	8(%eax), %eax
	cmpl	%eax, %edx
	je	.L224
	.loc 1 720 0
	movzbl	-5(%ebp), %eax
	xorl	$1, %eax
	testb	%al, %al
	je	.L222
	.loc 1 722 0
	movb	$1, -5(%ebp)
	.loc 1 723 0
	leal	.LC87@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$0, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L222:
	.loc 1 735 0
	movl	-16(%ebp), %eax
	movl	%eax, %edx
	movl	-24(%ebp), %eax
	movb	%dl, (%eax)
	addl	$1, -24(%ebp)
	.loc 1 736 0
	jmp	.L212
.L220:
	.loc 1 738 0
	movl	-24(%ebp), %eax
	movb	$0, (%eax)
	.loc 1 740 0
	movl	8(%ebp), %eax
	movl	64(%eax), %eax
	movl	8(%eax), %edx
	movl	12(%ebp), %eax
	movl	%edx, 12(%eax)
	.loc 1 741 0
	movl	-24(%ebp), %edx
	movl	8(%ebp), %eax
	movl	64(%eax), %eax
	movl	8(%eax), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	movl	%eax, %edx
	movl	12(%ebp), %eax
	movl	%edx, 8(%eax)
	.loc 1 742 0
	movl	8(%ebp), %eax
	movl	64(%eax), %edx
	movl	-24(%ebp), %eax
	addl	$1, %eax
	movl	%eax, 8(%edx)
	.loc 1 743 0
	addl	$52, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE35:
	.size	parse_string, .-parse_string
	.type	save_comment, @function
save_comment:
.LFB36:
	.loc 1 752 0
	pushl	%ebp
.LCFI66:
	movl	%esp, %ebp
.LCFI67:
	pushl	%edi
.LCFI68:
	pushl	%esi
.LCFI69:
	pushl	%ebx
.LCFI70:
	subl	$28, %esp
.LCFI71:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 756 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	(%eax), %eax
	movl	%eax, %edx
	movl	16(%ebp), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	addl	$1, %eax
	movl	%eax, -20(%ebp)
	.loc 1 760 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	(%eax), %eax
	subl	$1, %eax
	movzbl	(%eax), %eax
	movzbl	%al, %edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$1024, %eax
	testl	%eax, %eax
	je	.L233
	.loc 1 761 0
	subl	$1, -20(%ebp)
.L233:
	.loc 1 769 0
	movl	8(%ebp), %eax
	movzbl	8(%eax), %eax
	testb	%al, %al
	je	.L235
	cmpl	$47, 20(%ebp)
	jne	.L235
	movl	-20(%ebp), %eax
	addl	$2, %eax
	movl	%eax, -32(%ebp)
	jmp	.L238
.L235:
	movl	-20(%ebp), %ecx
	movl	%ecx, -32(%ebp)
.L238:
	movl	-32(%ebp), %eax
	movl	%eax, -16(%ebp)
	.loc 1 771 0
	movl	-16(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_unaligned_alloc@PLT
	movl	%eax, -24(%ebp)
	.loc 1 773 0
	movl	12(%ebp), %eax
	movb	$65, 6(%eax)
	.loc 1 774 0
	movl	12(%ebp), %edx
	movl	-16(%ebp), %eax
	movl	%eax, 8(%edx)
	.loc 1 775 0
	movl	12(%ebp), %edx
	movl	-24(%ebp), %eax
	movl	%eax, 12(%edx)
	.loc 1 777 0
	movl	-24(%ebp), %eax
	movb	$47, (%eax)
	.loc 1 778 0
	movl	-20(%ebp), %ecx
	subl	$1, %ecx
	movl	-24(%ebp), %eax
	addl	$1, %eax
	movl	16(%ebp), %edx
	movl	%eax, %edi
	movl	%edx, %esi
	cld
	rep
	movsb
	.loc 1 781 0
	movl	8(%ebp), %eax
	movzbl	8(%eax), %eax
	testb	%al, %al
	je	.L242
	cmpl	$47, 20(%ebp)
	jne	.L242
	.loc 1 783 0
	movl	-24(%ebp), %eax
	addl	$1, %eax
	movb	$42, (%eax)
	.loc 1 784 0
	movl	-16(%ebp), %eax
	addl	-24(%ebp), %eax
	subl	$2, %eax
	movb	$42, (%eax)
	.loc 1 785 0
	movl	-16(%ebp), %eax
	addl	-24(%ebp), %eax
	subl	$1, %eax
	movb	$47, (%eax)
.L242:
	.loc 1 787 0
	addl	$28, %esp
	popl	%ebx
	popl	%esi
	popl	%edi
	popl	%ebp
	ret
.LFE36:
	.size	save_comment, .-save_comment
.globl _cpp_init_tokenrun
	.type	_cpp_init_tokenrun, @function
_cpp_init_tokenrun:
.LFB37:
	.loc 1 794 0
	pushl	%ebp
.LCFI72:
	movl	%esp, %ebp
.LCFI73:
	pushl	%ebx
.LCFI74:
	subl	$4, %esp
.LCFI75:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 795 0
	movl	12(%ebp), %eax
	sall	$4, %eax
	movl	%eax, (%esp)
	call	xmalloc@PLT
	movl	%eax, %edx
	movl	8(%ebp), %eax
	movl	%edx, 8(%eax)
	.loc 1 796 0
	movl	8(%ebp), %eax
	movl	8(%eax), %edx
	movl	12(%ebp), %eax
	sall	$4, %eax
	addl	%eax, %edx
	movl	8(%ebp), %eax
	movl	%edx, 12(%eax)
	.loc 1 797 0
	movl	8(%ebp), %eax
	movl	$0, (%eax)
	.loc 1 798 0
	addl	$4, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE37:
	.size	_cpp_init_tokenrun, .-_cpp_init_tokenrun
	.type	next_tokenrun, @function
next_tokenrun:
.LFB38:
	.loc 1 804 0
	pushl	%ebp
.LCFI76:
	movl	%esp, %ebp
.LCFI77:
	pushl	%ebx
.LCFI78:
	subl	$20, %esp
.LCFI79:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 805 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	testl	%eax, %eax
	jne	.L246
	.loc 1 807 0
	movl	$16, (%esp)
	call	xmalloc@PLT
	movl	%eax, %edx
	movl	8(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 808 0
	movl	8(%ebp), %eax
	movl	(%eax), %edx
	movl	8(%ebp), %eax
	movl	%eax, 4(%edx)
	.loc 1 809 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	$250, 4(%esp)
	movl	%eax, (%esp)
	call	_cpp_init_tokenrun@PLT
.L246:
	.loc 1 812 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	.loc 1 813 0
	addl	$20, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE38:
	.size	next_tokenrun, .-next_tokenrun
.globl _cpp_temp_token
	.type	_cpp_temp_token, @function
_cpp_temp_token:
.LFB39:
	.loc 1 822 0
	pushl	%ebp
.LCFI80:
	movl	%esp, %ebp
.LCFI81:
	subl	$24, %esp
.LCFI82:
	.loc 1 825 0
	movl	8(%ebp), %eax
	movl	124(%eax), %eax
	subl	$16, %eax
	movl	%eax, -8(%ebp)
	.loc 1 826 0
	movl	8(%ebp), %eax
	movl	124(%eax), %edx
	movl	8(%ebp), %eax
	movl	144(%eax), %eax
	movl	12(%eax), %eax
	cmpl	%eax, %edx
	jne	.L250
	.loc 1 828 0
	movl	8(%ebp), %eax
	movl	144(%eax), %eax
	movl	%eax, (%esp)
	call	next_tokenrun
	movl	%eax, %edx
	movl	8(%ebp), %eax
	movl	%edx, 144(%eax)
	.loc 1 829 0
	movl	8(%ebp), %eax
	movl	144(%eax), %eax
	movl	8(%eax), %edx
	movl	8(%ebp), %eax
	movl	%edx, 124(%eax)
.L250:
	.loc 1 832 0
	movl	8(%ebp), %eax
	movl	124(%eax), %eax
	movl	%eax, -4(%ebp)
	leal	16(%eax), %edx
	movl	8(%ebp), %eax
	movl	%edx, 124(%eax)
	.loc 1 833 0
	movl	-8(%ebp), %eax
	movl	(%eax), %edx
	movl	-4(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 834 0
	movl	-8(%ebp), %eax
	movzwl	4(%eax), %edx
	movl	-4(%ebp), %eax
	movw	%dx, 4(%eax)
	.loc 1 835 0
	movl	-4(%ebp), %eax
	.loc 1 836 0
	leave
	ret
.LFE39:
	.size	_cpp_temp_token, .-_cpp_temp_token
	.local	current_pragma
	.comm	current_pragma,4,4
.globl last_token_omp_hash
	.bss
	.type	last_token_omp_hash, @object
	.size	last_token_omp_hash, 1
last_token_omp_hash:
	.zero	1
	.section	.rodata
.LC88:
	.string	"pragma"
.LC89:
	.string	"omp"
.LC90:
	.string	"options"
.LC91:
	.string	"mips_frequency_hint"
.LC92:
	.string	"frequency_hint"
.LC93:
	.string	"sl2"
	.text
	.type	_cpp_omp_token, @function
_cpp_omp_token:
.LFB40:
	.loc 1 855 0
	pushl	%ebp
.LCFI83:
	movl	%esp, %ebp
.LCFI84:
	pushl	%edi
.LCFI85:
	pushl	%esi
.LCFI86:
	pushl	%ebx
.LCFI87:
	subl	$124, %esp
.LCFI88:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 856 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -32(%ebp)
	.loc 1 857 0
	movl	$0, -28(%ebp)
	.loc 1 859 0
	movl	-32(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -24(%ebp)
	.loc 1 860 0
	movl	-32(%ebp), %eax
	movl	8(%eax), %eax
	movl	%eax, -20(%ebp)
	.loc 1 862 0
	jmp	.L254
.L255:
	addl	$1, -24(%ebp)
.L254:
	movl	-24(%ebp), %eax
	movzbl	(%eax), %eax
	cmpb	$32, %al
	je	.L255
	movl	-24(%ebp), %eax
	movzbl	(%eax), %eax
	cmpb	$9, %al
	je	.L255
	.loc 1 864 0
	movl	$6, -16(%ebp)
	.loc 1 865 0
	movl	-20(%ebp), %eax
	movl	-24(%ebp), %edx
	subl	%edx, %eax
	cmpl	-16(%ebp), %eax
	jl	.L258
	movl	-16(%ebp), %edx
	movl	-24(%ebp), %eax
	movl	%eax, -52(%ebp)
	leal	.LC88@GOTOFF(%ebx), %eax
	movl	%eax, -56(%ebp)
	movl	%edx, -60(%ebp)
	cld
	movl	-60(%ebp), %ecx
	cmpl	%ecx, -60(%ebp)
	movl	-52(%ebp), %esi
	movl	-56(%ebp), %edi
	movl	-60(%ebp), %ecx
	repz
	cmpsb
	seta	%dl
	setb	%al
	movl	%edx, %ecx
	subb	%al, %cl
	movl	%ecx, %eax
	movsbl	%al,%eax
	testl	%eax, %eax
	je	.L260
.L258:
	.loc 1 866 0
	movl	$0, -48(%ebp)
	jmp	.L261
.L260:
	.loc 1 868 0
	movl	-16(%ebp), %eax
	addl	%eax, -24(%ebp)
	.loc 1 870 0
	jmp	.L262
.L263:
	addl	$1, -24(%ebp)
.L262:
	movl	-24(%ebp), %eax
	cmpl	-20(%ebp), %eax
	jae	.L264
	movl	-24(%ebp), %eax
	movzbl	(%eax), %eax
	cmpb	$32, %al
	je	.L263
	movl	-24(%ebp), %eax
	movzbl	(%eax), %eax
	cmpb	$9, %al
	je	.L263
.L264:
	.loc 1 872 0
	movl	$3, -16(%ebp)
	.loc 1 873 0
	movl	-20(%ebp), %eax
	movl	-24(%ebp), %edx
	subl	%edx, %eax
	cmpl	-16(%ebp), %eax
	jl	.L267
	movl	-16(%ebp), %edx
	movl	-24(%ebp), %eax
	movl	%eax, -64(%ebp)
	leal	.LC89@GOTOFF(%ebx), %esi
	movl	%esi, -68(%ebp)
	movl	%edx, -72(%ebp)
	cld
	movl	-72(%ebp), %edi
	cmpl	%edi, -72(%ebp)
	movl	-64(%ebp), %esi
	movl	-68(%ebp), %edi
	movl	-72(%ebp), %ecx
	repz
	cmpsb
	seta	%dl
	setb	%al
	movl	%edx, %ecx
	subb	%al, %cl
	movl	%ecx, %eax
	movsbl	%al,%eax
	testl	%eax, %eax
	jne	.L267
	.loc 1 875 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_lex_direct@PLT
	movl	%eax, -28(%ebp)
	.loc 1 876 0
	movl	$1, current_pragma@GOTOFF(%ebx)
	.loc 1 873 0
	jmp	.L270
.L267:
	.loc 1 878 0
	movl	-20(%ebp), %edx
	movl	-24(%ebp), %eax
	movl	%edx, %esi
	subl	%eax, %esi
	movl	%esi, %eax
	cmpl	$6, %eax
	jbe	.L271
	movl	-24(%ebp), %eax
	movl	%eax, -76(%ebp)
	leal	.LC90@GOTOFF(%ebx), %edi
	movl	%edi, -80(%ebp)
	movl	$7, -84(%ebp)
	cld
	movl	-76(%ebp), %esi
	movl	-80(%ebp), %edi
	movl	-84(%ebp), %ecx
	repz
	cmpsb
	seta	%dl
	setb	%al
	movl	%edx, %ecx
	subb	%al, %cl
	movl	%ecx, %eax
	movsbl	%al,%eax
	testl	%eax, %eax
	jne	.L271
	.loc 1 881 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_lex_direct@PLT
	movl	%eax, -28(%ebp)
	.loc 1 882 0
	movl	$2, current_pragma@GOTOFF(%ebx)
	.loc 1 878 0
	jmp	.L270
.L271:
	.loc 1 884 0
	movl	-20(%ebp), %edx
	movl	-24(%ebp), %eax
	movl	%edx, %esi
	subl	%eax, %esi
	movl	%esi, %eax
	cmpl	$18, %eax
	jbe	.L274
	movl	-24(%ebp), %eax
	movl	%eax, -88(%ebp)
	leal	.LC91@GOTOFF(%ebx), %edi
	movl	%edi, -92(%ebp)
	movl	$19, -96(%ebp)
	cld
	movl	-88(%ebp), %esi
	movl	-92(%ebp), %edi
	movl	-96(%ebp), %ecx
	repz
	cmpsb
	seta	%dl
	setb	%al
	movl	%edx, %ecx
	subb	%al, %cl
	movl	%ecx, %eax
	movsbl	%al,%eax
	testl	%eax, %eax
	je	.L276
.L274:
	movl	-20(%ebp), %edx
	movl	-24(%ebp), %eax
	movl	%edx, %esi
	subl	%eax, %esi
	movl	%esi, %eax
	cmpl	$13, %eax
	jbe	.L277
	movl	-24(%ebp), %eax
	movl	%eax, -100(%ebp)
	leal	.LC92@GOTOFF(%ebx), %edi
	movl	%edi, -104(%ebp)
	movl	$14, -108(%ebp)
	cld
	movl	-100(%ebp), %esi
	movl	-104(%ebp), %edi
	movl	-108(%ebp), %ecx
	repz
	cmpsb
	seta	%dl
	setb	%al
	movl	%edx, %ecx
	subb	%al, %cl
	movl	%ecx, %eax
	movsbl	%al,%eax
	testl	%eax, %eax
	jne	.L277
.L276:
	.loc 1 889 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_lex_direct@PLT
	movl	%eax, -28(%ebp)
	.loc 1 890 0
	movl	$3, current_pragma@GOTOFF(%ebx)
	.loc 1 884 0
	jmp	.L270
.L277:
	.loc 1 894 0
	movl	-20(%ebp), %edx
	movl	-24(%ebp), %eax
	movl	%edx, %esi
	subl	%eax, %esi
	movl	%esi, %eax
	cmpl	$2, %eax
	jbe	.L270
	movl	-24(%ebp), %eax
	movl	%eax, -112(%ebp)
	leal	.LC93@GOTOFF(%ebx), %edi
	movl	%edi, -116(%ebp)
	movl	$3, -120(%ebp)
	cld
	movl	-112(%ebp), %esi
	movl	-116(%ebp), %edi
	movl	-120(%ebp), %ecx
	repz
	cmpsb
	seta	%dl
	setb	%al
	movl	%edx, %ecx
	subb	%al, %cl
	movl	%ecx, %eax
	movsbl	%al,%eax
	testl	%eax, %eax
	jne	.L270
	.loc 1 896 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_lex_direct@PLT
	movl	%eax, -28(%ebp)
	.loc 1 897 0
	movl	$4, current_pragma@GOTOFF(%ebx)
.L270:
	.loc 1 901 0
	movl	-28(%ebp), %esi
	movl	%esi, -48(%ebp)
.L261:
	movl	-48(%ebp), %eax
	.loc 1 902 0
	addl	$124, %esp
	popl	%ebx
	popl	%esi
	popl	%edi
	popl	%ebp
	ret
.LFE40:
	.size	_cpp_omp_token, .-_cpp_omp_token
	.type	skip_to_end_of_line, @function
skip_to_end_of_line:
.LFB41:
	.loc 1 908 0
	pushl	%ebp
.LCFI89:
	movl	%esp, %ebp
.LCFI90:
	subl	$16, %esp
.LCFI91:
	.loc 1 909 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -8(%ebp)
	.loc 1 912 0
	jmp	.L283
.L284:
	.loc 1 914 0
	movl	-8(%ebp), %eax
	movl	(%eax), %edx
	movzbl	(%edx), %eax
	movzbl	%al, %eax
	movl	%eax, -4(%ebp)
	addl	$1, %edx
	movl	-8(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 915 0
	cmpl	$10, -4(%ebp)
	jne	.L283
	.loc 1 918 0
	movl	-8(%ebp), %eax
	movl	(%eax), %eax
	leal	-1(%eax), %edx
	movl	-8(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 919 0
	jmp	.L287
.L283:
	.loc 1 912 0
	movl	-8(%ebp), %eax
	movl	(%eax), %edx
	movl	-8(%ebp), %eax
	movl	8(%eax), %eax
	cmpl	%eax, %edx
	jne	.L284
.L287:
	.loc 1 922 0
	leave
	ret
.LFE41:
	.size	skip_to_end_of_line, .-skip_to_end_of_line
.globl _cpp_lex_token
	.type	_cpp_lex_token, @function
_cpp_lex_token:
.LFB42:
	.loc 1 933 0
	pushl	%ebp
.LCFI92:
	movl	%esp, %ebp
.LCFI93:
	pushl	%ebx
.LCFI94:
	subl	$36, %esp
.LCFI95:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	jmp	.L323
.L289:
.L323:
	.loc 1 938 0
	movl	8(%ebp), %eax
	movl	124(%eax), %edx
	movl	8(%ebp), %eax
	movl	144(%eax), %eax
	movl	12(%eax), %eax
	cmpl	%eax, %edx
	jne	.L290
	.loc 1 940 0
	movl	8(%ebp), %eax
	movl	144(%eax), %eax
	movl	%eax, (%esp)
	call	next_tokenrun
	movl	%eax, %edx
	movl	8(%ebp), %eax
	movl	%edx, 144(%eax)
	.loc 1 941 0
	movl	8(%ebp), %eax
	movl	144(%eax), %eax
	movl	8(%eax), %edx
	movl	8(%ebp), %eax
	movl	%edx, 124(%eax)
.L290:
	.loc 1 944 0
	movl	8(%ebp), %eax
	movl	148(%eax), %eax
	testl	%eax, %eax
	je	.L292
	.loc 1 946 0
	movl	8(%ebp), %eax
	movl	148(%eax), %eax
	leal	-1(%eax), %edx
	movl	8(%ebp), %eax
	movl	%edx, 148(%eax)
	.loc 1 947 0
	movl	8(%ebp), %eax
	movl	124(%eax), %eax
	movl	%eax, -12(%ebp)
	leal	16(%eax), %edx
	movl	8(%ebp), %eax
	movl	%edx, 124(%eax)
	jmp	.L294
.L292:
	.loc 1 951 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_lex_direct@PLT
	movl	%eax, -12(%ebp)
	.loc 1 953 0
	movl	in_omp_pragma@GOT(%ebx), %eax
	movzbl	(%eax), %eax
	testb	%al, %al
	je	.L294
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	(%eax), %eax
	subl	$1, %eax
	movzbl	(%eax), %eax
	cmpb	$10, %al
	jne	.L294
	.loc 1 954 0
	movl	-12(%ebp), %eax
	movl	%eax, -24(%ebp)
	jmp	.L297
.L294:
	.loc 1 959 0
	movl	-12(%ebp), %eax
	movzbl	7(%eax), %eax
	movzbl	%al, %eax
	andl	$64, %eax
	testl	%eax, %eax
	je	.L298
	movl	-12(%ebp), %eax
	movzbl	6(%eax), %eax
	cmpb	$41, %al
	jne	.L298
	movl	8(%ebp), %eax
	movzbl	18(%eax), %eax
	cmpb	$1, %al
	je	.L298
.LBB12:
	.loc 1 963 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_omp_token
	movl	%eax, -8(%ebp)
	.loc 1 964 0
	cmpl	$0, -8(%ebp)
	je	.L298
	.loc 1 966 0
	movl	flag_openmp@GOT(%ebx), %eax
	movl	(%eax), %eax
	testl	%eax, %eax
	je	.L303
	movl	current_pragma@GOTOFF(%ebx), %eax
	cmpl	$1, %eax
	jne	.L303
	.loc 1 968 0
	movl	last_token_omp_hash@GOT(%ebx), %eax
	movb	$1, (%eax)
	.loc 1 969 0
	movl	-8(%ebp), %eax
	movl	%eax, -24(%ebp)
	jmp	.L297
.L303:
	.loc 1 971 0
	movl	current_pragma@GOTOFF(%ebx), %eax
	cmpl	$2, %eax
	je	.L306
	movl	current_pragma@GOTOFF(%ebx), %eax
	cmpl	$3, %eax
	jne	.L308
.L306:
	.loc 1 974 0
	movl	last_token_omp_hash@GOT(%ebx), %eax
	movb	$1, (%eax)
	.loc 1 975 0
	movl	-8(%ebp), %eax
	movl	%eax, -24(%ebp)
	jmp	.L297
.L308:
	.loc 1 978 0
	movl	current_pragma@GOTOFF(%ebx), %eax
	cmpl	$4, %eax
	jne	.L309
	.loc 1 980 0
	movl	last_token_omp_hash@GOT(%ebx), %eax
	movb	$1, (%eax)
	.loc 1 981 0
	movl	-8(%ebp), %eax
	movl	%eax, -24(%ebp)
	jmp	.L297
.L309:
	.loc 1 986 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	skip_to_end_of_line
	.loc 1 987 0
	jmp	.L289
.L298:
.LBE12:
	.loc 1 993 0
	movl	-12(%ebp), %eax
	movzbl	7(%eax), %eax
	movzbl	%al, %eax
	andl	$64, %eax
	testl	%eax, %eax
	je	.L312
	.loc 1 997 0
	movl	-12(%ebp), %eax
	movzbl	6(%eax), %eax
	cmpb	$41, %al
	jne	.L314
	movl	8(%ebp), %eax
	movzbl	18(%eax), %eax
	cmpb	$1, %al
	je	.L314
	movl	-12(%ebp), %eax
	movzbl	7(%eax), %eax
	movzbl	%al, %eax
	andl	$1, %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_handle_directive@PLT
	testl	%eax, %eax
	jne	.L289
.L314:
	.loc 1 1004 0
	movl	8(%ebp), %eax
	movl	316(%eax), %eax
	testl	%eax, %eax
	je	.L312
	movl	8(%ebp), %eax
	movzbl	10(%eax), %eax
	testb	%al, %al
	jne	.L312
	.loc 1 1005 0
	movl	8(%ebp), %eax
	movl	316(%eax), %edx
	movl	8(%ebp), %eax
	movzbl	18(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, 8(%esp)
	movl	-12(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	*%edx
.L312:
	.loc 1 1009 0
	movl	8(%ebp), %eax
	movzbl	8(%eax), %eax
	testb	%al, %al
	jne	.L319
	.loc 1 1015 0
	movl	8(%ebp), %eax
	movb	$0, 120(%eax)
	.loc 1 1017 0
	movl	8(%ebp), %eax
	movzbl	10(%eax), %eax
	testb	%al, %al
	je	.L319
	movl	-12(%ebp), %eax
	movzbl	6(%eax), %eax
	cmpb	$24, %al
	je	.L319
	.loc 1 1019 0
	jmp	.L289
.L319:
	.loc 1 1021 0
	movl	-12(%ebp), %eax
	movl	%eax, -24(%ebp)
.L297:
	movl	-24(%ebp), %eax
	.loc 1 1022 0
	addl	$36, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE42:
	.size	_cpp_lex_token, .-_cpp_lex_token
	.section	.rodata
.LC94:
	.string	"no newline at end of file"
	.text
	.type	continue_after_nul, @function
continue_after_nul:
.LFB43:
	.loc 1 1031 0
	pushl	%ebp
.LCFI96:
	movl	%esp, %ebp
.LCFI97:
	pushl	%ebx
.LCFI98:
	subl	$36, %esp
.LCFI99:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 1032 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -12(%ebp)
	.loc 1 1033 0
	movb	$0, -5(%ebp)
	.loc 1 1035 0
	movl	-12(%ebp), %eax
	movb	$64, 36(%eax)
	.loc 1 1036 0
	movl	8(%ebp), %eax
	movzbl	433(%eax), %eax
	testb	%al, %al
	je	.L325
	.loc 1 1038 0
	movl	8(%ebp), %eax
	movzbl	8(%eax), %eax
	testb	%al, %al
	je	.L327
	.loc 1 1039 0
	movl	$0, -24(%ebp)
	jmp	.L329
.L327:
	.loc 1 1041 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_remove_overlay@PLT
	.loc 1 1042 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_read_logical_line_trad@PLT
	movb	%al, -5(%ebp)
	.loc 1 1043 0
	movl	8(%ebp), %eax
	movl	516(%eax), %eax
	movl	%eax, %edx
	movl	8(%ebp), %eax
	movl	508(%eax), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	movl	%eax, %edx
	movl	8(%ebp), %eax
	movl	508(%eax), %eax
	movl	%edx, 8(%esp)
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_overlay_buffer@PLT
	.loc 1 1045 0
	movl	8(%ebp), %eax
	movl	520(%eax), %edx
	movl	8(%ebp), %eax
	movl	%edx, 52(%eax)
	jmp	.L330
.L325:
	.loc 1 1051 0
	movl	8(%ebp), %eax
	movzbl	18(%eax), %eax
	testb	%al, %al
	jne	.L330
	.loc 1 1053 0
	movl	-12(%ebp), %eax
	movl	(%eax), %edx
	movl	-12(%ebp), %eax
	movl	12(%eax), %eax
	cmpl	%eax, %edx
	je	.L332
	.loc 1 1057 0
	movl	-12(%ebp), %eax
	movzbl	45(%eax), %eax
	testb	%al, %al
	jne	.L334
	.loc 1 1058 0
	leal	.LC94@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$2, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L334:
	.loc 1 1059 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	handle_newline
.L332:
	.loc 1 1064 0
	movl	8(%ebp), %eax
	movzbl	8(%eax), %eax
	testb	%al, %al
	jne	.L330
	movl	-12(%ebp), %eax
	movl	16(%eax), %eax
	testl	%eax, %eax
	je	.L330
	.loc 1 1066 0
	movl	-12(%ebp), %eax
	movzbl	47(%eax), %eax
	movzbl	%al, %eax
	xorl	$1, %eax
	movb	%al, -5(%ebp)
	.loc 1 1067 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_pop_buffer@PLT
.L330:
	.loc 1 1072 0
	movzbl	-5(%ebp), %eax
	movl	%eax, -24(%ebp)
.L329:
	movl	-24(%ebp), %eax
	.loc 1 1073 0
	addl	$36, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE43:
	.size	continue_after_nul, .-continue_after_nul
	.section	.rodata
.LC95:
	.string	"unterminated comment"
	.align 4
.LC96:
	.string	"C++ style comments are not allowed in ISO C90"
	.align 4
.LC97:
	.string	"(this will be reported only once per input file)"
.LC98:
	.string	"multi-line comment"
	.text
.globl _cpp_lex_direct
	.type	_cpp_lex_direct, @function
_cpp_lex_direct:
.LFB44:
	.loc 1 1100 0
	pushl	%ebp
.LCFI100:
	movl	%esp, %ebp
.LCFI101:
	pushl	%ebx
.LCFI102:
	subl	$68, %esp
.LCFI103:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 1104 0
	movl	8(%ebp), %eax
	movl	124(%eax), %eax
	movl	%eax, -24(%ebp)
	leal	16(%eax), %edx
	movl	8(%ebp), %eax
	movl	%edx, 124(%eax)
	jmp	.L540
.L340:
.L540:
	.loc 1 1107 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -32(%ebp)
	.loc 1 1108 0
	movl	-32(%ebp), %eax
	movzbl	36(%eax), %edx
	movl	-24(%ebp), %eax
	movb	%dl, 7(%eax)
	.loc 1 1109 0
	movl	-32(%ebp), %eax
	movb	$0, 36(%eax)
	jmp	.L541
.L341:
.L541:
	.loc 1 1111 0
	movl	8(%ebp), %eax
	movl	52(%eax), %edx
	movl	-24(%ebp), %eax
	movl	%edx, (%eax)
.L342:
	.loc 1 1114 0
	movl	-32(%ebp), %eax
	movl	(%eax), %edx
	movzbl	(%edx), %eax
	movzbl	%al, %eax
	movl	%eax, -36(%ebp)
	addl	$1, %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 1115 0
	movl	-32(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, %edx
	movl	-32(%ebp), %eax
	movl	12(%eax), %eax
	subw	%ax, %dx
	movl	-32(%ebp), %eax
	movl	32(%eax), %eax
	addl	%eax, %edx
	movl	-24(%ebp), %eax
	movw	%dx, 4(%eax)
	.loc 1 1118 0
	movl	in_omp_pragma@GOT(%ebx), %eax
	movzbl	(%eax), %eax
	testb	%al, %al
	je	.L343
	cmpl	$10, -36(%ebp)
	jne	.L343
	.loc 1 1120 0
	movl	-32(%ebp), %eax
	movb	$64, 36(%eax)
	.loc 1 1121 0
	movl	-24(%ebp), %eax
	movb	$57, 6(%eax)
	.loc 1 1122 0
	movl	-32(%ebp), %eax
	movl	(%eax), %eax
	leal	-1(%eax), %edx
	movl	8(%ebp), %eax
	movl	348(%eax), %eax
	movl	$1, 12(%esp)
	movl	$1, 8(%esp)
	movl	%edx, 4(%esp)
	movl	%eax, (%esp)
	call	ht_lookup@PLT
	movl	%eax, %edx
	movl	-24(%ebp), %eax
	movl	%edx, 8(%eax)
	.loc 1 1124 0
	movl	-24(%ebp), %eax
	movl	%eax, -44(%ebp)
	jmp	.L346
.L343:
	.loc 1 1128 0
	cmpl	$126, -36(%ebp)
	ja	.L347
	movl	-36(%ebp), %eax
	sall	$2, %eax
	movl	.L381@GOTOFF(%eax,%ebx), %eax
	addl	%ebx, %eax
	jmp	*%eax
	.section	.rodata
	.align 4
	.align 4
.L381:
	.long	.L348@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L348@GOTOFF
	.long	.L349@GOTOFF
	.long	.L348@GOTOFF
	.long	.L348@GOTOFF
	.long	.L349@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L347@GOTOFF
	.long	.L348@GOTOFF
	.long	.L350@GOTOFF
	.long	.L351@GOTOFF
	.long	.L352@GOTOFF
	.long	.L353@GOTOFF
	.long	.L354@GOTOFF
	.long	.L355@GOTOFF
	.long	.L351@GOTOFF
	.long	.L356@GOTOFF
	.long	.L357@GOTOFF
	.long	.L358@GOTOFF
	.long	.L359@GOTOFF
	.long	.L360@GOTOFF
	.long	.L361@GOTOFF
	.long	.L362@GOTOFF
	.long	.L363@GOTOFF
	.long	.L364@GOTOFF
	.long	.L364@GOTOFF
	.long	.L364@GOTOFF
	.long	.L364@GOTOFF
	.long	.L364@GOTOFF
	.long	.L364@GOTOFF
	.long	.L364@GOTOFF
	.long	.L364@GOTOFF
	.long	.L364@GOTOFF
	.long	.L364@GOTOFF
	.long	.L365@GOTOFF
	.long	.L366@GOTOFF
	.long	.L367@GOTOFF
	.long	.L368@GOTOFF
	.long	.L369@GOTOFF
	.long	.L370@GOTOFF
	.long	.L371@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L373@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L374@GOTOFF
	.long	.L370@GOTOFF
	.long	.L375@GOTOFF
	.long	.L376@GOTOFF
	.long	.L372@GOTOFF
	.long	.L347@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L372@GOTOFF
	.long	.L377@GOTOFF
	.long	.L378@GOTOFF
	.long	.L379@GOTOFF
	.long	.L380@GOTOFF
	.text
.L348:
	.loc 1 1131 0
	movl	-24(%ebp), %eax
	movzbl	7(%eax), %eax
	movl	%eax, %edx
	orl	$1, %edx
	movl	-24(%ebp), %eax
	movb	%dl, 7(%eax)
	.loc 1 1132 0
	movl	-36(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	skip_whitespace
	testl	%eax, %eax
	jne	.L342
	.loc 1 1136 0
	movl	-32(%ebp), %eax
	movl	(%eax), %eax
	leal	-1(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 1137 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	continue_after_nul
	testb	%al, %al
	jne	.L340
	.loc 1 1139 0
	movl	-24(%ebp), %eax
	movb	$24, 6(%eax)
	.loc 1 1140 0
	jmp	.L384
.L349:
	.loc 1 1143 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	handle_newline
	.loc 1 1144 0
	movl	-32(%ebp), %eax
	movb	$64, 36(%eax)
	.loc 1 1145 0
	movl	8(%ebp), %eax
	movzbl	8(%eax), %eax
	testb	%al, %al
	jne	.L385
	.loc 1 1147 0
	movl	8(%ebp), %eax
	movzbl	18(%eax), %eax
	cmpb	$2, %al
	jne	.L387
	.loc 1 1148 0
	movl	-32(%ebp), %eax
	movzbl	36(%eax), %eax
	movl	%eax, %edx
	orl	$1, %edx
	movl	-32(%ebp), %eax
	movb	%dl, 36(%eax)
.L387:
	.loc 1 1149 0
	movl	8(%ebp), %eax
	movl	152(%eax), %eax
	testl	%eax, %eax
	jne	.L340
	.loc 1 1151 0
	movl	8(%ebp), %edx
	subl	$-128, %edx
	movl	8(%ebp), %eax
	movl	%edx, 144(%eax)
	.loc 1 1152 0
	movl	8(%ebp), %eax
	movl	136(%eax), %eax
	movl	%eax, -24(%ebp)
	.loc 1 1153 0
	movl	-24(%ebp), %edx
	addl	$16, %edx
	movl	8(%ebp), %eax
	movl	%edx, 124(%eax)
	.loc 1 1155 0
	jmp	.L340
.L385:
	.loc 1 1157 0
	movl	-24(%ebp), %eax
	movb	$24, 6(%eax)
	.loc 1 1158 0
	jmp	.L384
.L370:
.LBB13:
	.loc 1 1165 0
	movl	8(%ebp), %eax
	movl	52(%eax), %eax
	movl	%eax, -20(%ebp)
	.loc 1 1167 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	skip_escaped_newlines
	movl	%eax, -36(%ebp)
	.loc 1 1168 0
	movl	8(%ebp), %eax
	movl	52(%eax), %eax
	cmpl	-20(%ebp), %eax
	je	.L391
	.loc 1 1170 0
	movl	-32(%ebp), %eax
	movl	(%eax), %eax
	leal	-1(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 1173 0
	jmp	.L341
.L391:
.LBE13:
	.loc 1 1178 0
	cmpl	$63, -36(%ebp)
	jne	.L393
	.loc 1 1179 0
	movl	-24(%ebp), %eax
	movb	$19, 6(%eax)
	.loc 1 1184 0
	jmp	.L384
.L393:
	.loc 1 1180 0
	cmpl	$92, -36(%ebp)
	je	.L347
	.loc 1 1183 0
	jmp	.L343
.L364:
	.loc 1 1188 0
	movl	-24(%ebp), %eax
	movb	$58, 6(%eax)
	.loc 1 1189 0
	movl	-24(%ebp), %eax
	addl	$8, %eax
	movl	$0, 8(%esp)
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	parse_number
	.loc 1 1190 0
	jmp	.L384
.L373:
.LBB14:
	.loc 1 1195 0
	movl	-32(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -16(%ebp)
	.loc 1 1197 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	movl	%eax, -36(%ebp)
	.loc 1 1198 0
	cmpl	$39, -36(%ebp)
	je	.L395
	cmpl	$34, -36(%ebp)
	jne	.L397
.L395:
	.loc 1 1200 0
	cmpl	$34, -36(%ebp)
	jne	.L398
	movb	$63, -38(%ebp)
	jmp	.L400
.L398:
	movb	$60, -38(%ebp)
.L400:
	movl	-24(%ebp), %eax
	movzbl	-38(%ebp), %edx
	movb	%dl, 6(%eax)
	.loc 1 1201 0
	movl	-36(%ebp), %eax
	movl	%eax, 8(%esp)
	movl	-24(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	parse_string
	.loc 1 1202 0
	jmp	.L384
.L397:
	.loc 1 1204 0
	movl	-32(%ebp), %edx
	movl	-16(%ebp), %eax
	movl	%eax, (%edx)
.L372:
.LBE14:
	.loc 1 1220 0
	movl	-24(%ebp), %eax
	movb	$57, 6(%eax)
	.loc 1 1221 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	parse_identifier
	movl	%eax, %edx
	movl	-24(%ebp), %eax
	movl	%edx, 8(%eax)
	.loc 1 1224 0
	movl	-24(%ebp), %eax
	movl	8(%eax), %eax
	movzbl	17(%eax), %eax
	movzbl	%al, %eax
	andl	$1, %eax
	testb	%al, %al
	je	.L384
	.loc 1 1226 0
	movl	-24(%ebp), %eax
	movzbl	7(%eax), %eax
	movl	%eax, %edx
	orl	$16, %edx
	movl	-24(%ebp), %eax
	movb	%dl, 7(%eax)
	.loc 1 1228 0
	movl	-24(%ebp), %eax
	movl	8(%eax), %eax
	movl	20(%eax), %eax
	movl	%eax, %edx
	movl	-24(%ebp), %eax
	movb	%dl, 6(%eax)
	.loc 1 1233 0
	jmp	.L384
.L351:
	.loc 1 1237 0
	cmpl	$34, -36(%ebp)
	jne	.L403
	movb	$62, -37(%ebp)
	jmp	.L405
.L403:
	movb	$59, -37(%ebp)
.L405:
	movl	-24(%ebp), %eax
	movzbl	-37(%ebp), %edx
	movb	%dl, 6(%eax)
	.loc 1 1238 0
	movl	-36(%ebp), %eax
	movl	%eax, 8(%esp)
	movl	-24(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	parse_string
	.loc 1 1239 0
	jmp	.L384
.L363:
	.loc 1 1243 0
	movl	-32(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -28(%ebp)
	.loc 1 1244 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	movl	%eax, -36(%ebp)
	.loc 1 1246 0
	cmpl	$42, -36(%ebp)
	jne	.L406
	.loc 1 1248 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	skip_block_comment
	testl	%eax, %eax
	je	.L410
	.loc 1 1249 0
	leal	.LC95@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$3, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	jmp	.L410
.L406:
	.loc 1 1251 0
	cmpl	$47, -36(%ebp)
	jne	.L411
	movl	8(%ebp), %eax
	movzbl	394(%eax), %eax
	testb	%al, %al
	jne	.L413
	movl	8(%ebp), %eax
	movl	48(%eax), %eax
	testl	%eax, %eax
	je	.L411
	movl	8(%ebp), %eax
	movl	48(%eax), %eax
	movzbl	17(%eax), %eax
	testb	%al, %al
	je	.L411
.L413:
	.loc 1 1256 0
	movl	8(%ebp), %eax
	movl	388(%eax), %eax
	testl	%eax, %eax
	jne	.L416
	movl	8(%ebp), %eax
	movzbl	425(%eax), %eax
	testb	%al, %al
	je	.L416
	movl	-32(%ebp), %eax
	movzbl	44(%eax), %eax
	testb	%al, %al
	jne	.L416
	.loc 1 1259 0
	leal	.LC96@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$2, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	.loc 1 1261 0
	leal	.LC97@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$2, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	.loc 1 1263 0
	movl	-32(%ebp), %eax
	movb	$1, 44(%eax)
.L416:
	.loc 1 1266 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	skip_line_comment
	testl	%eax, %eax
	je	.L410
	movl	8(%ebp), %eax
	movzbl	407(%eax), %eax
	testb	%al, %al
	je	.L410
	.loc 1 1267 0
	leal	.LC98@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$0, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	.loc 1 1251 0
	jmp	.L410
.L411:
	.loc 1 1269 0
	cmpl	$61, -36(%ebp)
	jne	.L423
	.loc 1 1271 0
	movl	-24(%ebp), %eax
	movb	$32, 6(%eax)
	.loc 1 1272 0
	jmp	.L384
.L423:
	.loc 1 1276 0
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 1277 0
	movl	-24(%ebp), %eax
	movb	$7, 6(%eax)
	.loc 1 1278 0
	jmp	.L384
.L410:
	.loc 1 1281 0
	movl	8(%ebp), %eax
	movzbl	13(%eax), %eax
	testb	%al, %al
	jne	.L425
	.loc 1 1283 0
	movl	-24(%ebp), %eax
	movzbl	7(%eax), %eax
	movl	%eax, %edx
	orl	$1, %edx
	movl	-24(%ebp), %eax
	movb	%dl, 7(%eax)
	.loc 1 1284 0
	jmp	.L341
.L425:
	.loc 1 1288 0
	movl	-36(%ebp), %eax
	movl	%eax, 12(%esp)
	movl	-28(%ebp), %eax
	movl	%eax, 8(%esp)
	movl	-24(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	save_comment
	.loc 1 1289 0
	jmp	.L384
.L367:
	.loc 1 1292 0
	movl	8(%ebp), %eax
	movzbl	11(%eax), %eax
	testb	%al, %al
	je	.L427
	.loc 1 1294 0
	movl	-24(%ebp), %eax
	movb	$64, 6(%eax)
	.loc 1 1295 0
	movl	$62, 8(%esp)
	movl	-24(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	parse_string
	.loc 1 1296 0
	jmp	.L384
.L427:
	.loc 1 1299 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	movl	%eax, -36(%ebp)
	.loc 1 1300 0
	cmpl	$61, -36(%ebp)
	jne	.L429
	.loc 1 1301 0
	movl	-24(%ebp), %eax
	movb	$28, 6(%eax)
	jmp	.L384
.L429:
	.loc 1 1302 0
	cmpl	$60, -36(%ebp)
	jne	.L432
	.loc 1 1303 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	cmpl	$61, %eax
	jne	.L434
	movl	-24(%ebp), %eax
	movb	$38, 6(%eax)
	jmp	.L384
.L434:
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	movl	-24(%ebp), %eax
	movb	$13, 6(%eax)
	jmp	.L384
.L432:
	.loc 1 1304 0
	cmpl	$63, -36(%ebp)
	jne	.L437
	movl	8(%ebp), %eax
	movzbl	393(%eax), %eax
	testb	%al, %al
	je	.L437
	.loc 1 1305 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	cmpl	$61, %eax
	jne	.L440
	movl	-24(%ebp), %eax
	movb	$39, 6(%eax)
	jmp	.L384
.L440:
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	movl	-24(%ebp), %eax
	movb	$14, 6(%eax)
	.loc 1 1304 0
	jmp	.L384
.L437:
	.loc 1 1306 0
	cmpl	$58, -36(%ebp)
	jne	.L443
	movl	8(%ebp), %eax
	movzbl	399(%eax), %eax
	testb	%al, %al
	je	.L443
	.loc 1 1308 0
	movl	-24(%ebp), %eax
	movb	$43, 6(%eax)
	.loc 1 1309 0
	movl	-24(%ebp), %eax
	movzbl	7(%eax), %eax
	movl	%eax, %edx
	orl	$2, %edx
	movl	-24(%ebp), %eax
	movb	%dl, 7(%eax)
	.loc 1 1306 0
	jmp	.L384
.L443:
	.loc 1 1311 0
	cmpl	$37, -36(%ebp)
	jne	.L446
	movl	8(%ebp), %eax
	movzbl	399(%eax), %eax
	testb	%al, %al
	je	.L446
	.loc 1 1313 0
	movl	-24(%ebp), %eax
	movb	$45, 6(%eax)
	.loc 1 1314 0
	movl	-24(%ebp), %eax
	movzbl	7(%eax), %eax
	movl	%eax, %edx
	orl	$2, %edx
	movl	-24(%ebp), %eax
	movb	%dl, 7(%eax)
	.loc 1 1311 0
	jmp	.L384
.L446:
	.loc 1 1318 0
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 1319 0
	movl	-24(%ebp), %eax
	movb	$3, 6(%eax)
	.loc 1 1321 0
	jmp	.L384
.L369:
	.loc 1 1324 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	movl	%eax, -36(%ebp)
	.loc 1 1325 0
	cmpl	$61, -36(%ebp)
	jne	.L449
	.loc 1 1326 0
	movl	-24(%ebp), %eax
	movb	$27, 6(%eax)
	jmp	.L384
.L449:
	.loc 1 1327 0
	cmpl	$62, -36(%ebp)
	jne	.L452
	.loc 1 1328 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	cmpl	$61, %eax
	jne	.L454
	movl	-24(%ebp), %eax
	movb	$37, 6(%eax)
	jmp	.L384
.L454:
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	movl	-24(%ebp), %eax
	movb	$12, 6(%eax)
	jmp	.L384
.L452:
	.loc 1 1329 0
	cmpl	$63, -36(%ebp)
	jne	.L457
	movl	8(%ebp), %eax
	movzbl	393(%eax), %eax
	testb	%al, %al
	je	.L457
	.loc 1 1330 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	cmpl	$61, %eax
	jne	.L460
	movl	-24(%ebp), %eax
	movb	$40, 6(%eax)
	jmp	.L384
.L460:
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	movl	-24(%ebp), %eax
	movb	$15, 6(%eax)
	.loc 1 1329 0
	jmp	.L384
.L457:
	.loc 1 1333 0
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 1334 0
	movl	-24(%ebp), %eax
	movb	$2, 6(%eax)
	.loc 1 1336 0
	jmp	.L384
.L354:
	.loc 1 1339 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	movl	%eax, -36(%ebp)
	.loc 1 1340 0
	cmpl	$61, -36(%ebp)
	jne	.L463
	.loc 1 1341 0
	movl	-24(%ebp), %eax
	movb	$33, 6(%eax)
	jmp	.L384
.L463:
	.loc 1 1342 0
	movl	8(%ebp), %eax
	movzbl	399(%eax), %eax
	testb	%al, %al
	je	.L466
	cmpl	$58, -36(%ebp)
	jne	.L466
	.loc 1 1344 0
	movl	-24(%ebp), %eax
	movzbl	7(%eax), %eax
	movl	%eax, %edx
	orl	$2, %edx
	movl	-24(%ebp), %eax
	movb	%dl, 7(%eax)
	.loc 1 1345 0
	movl	-24(%ebp), %eax
	movb	$41, 6(%eax)
	.loc 1 1346 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	cmpl	$37, %eax
	jne	.L469
.LBB15:
	.loc 1 1348 0
	movl	-32(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -12(%ebp)
	.loc 1 1350 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	cmpl	$58, %eax
	jne	.L471
	.loc 1 1351 0
	movl	-24(%ebp), %eax
	movb	$42, 6(%eax)
	jmp	.L384
.L471:
	.loc 1 1353 0
	movl	-12(%ebp), %edx
	subl	$1, %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	jmp	.L384
.L469:
.LBE15:
	.loc 1 1356 0
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 1342 0
	jmp	.L384
.L466:
	.loc 1 1358 0
	movl	8(%ebp), %eax
	movzbl	399(%eax), %eax
	testb	%al, %al
	je	.L475
	cmpl	$62, -36(%ebp)
	jne	.L475
	.loc 1 1360 0
	movl	-24(%ebp), %eax
	movzbl	7(%eax), %eax
	movl	%eax, %edx
	orl	$2, %edx
	movl	-24(%ebp), %eax
	movb	%dl, 7(%eax)
	.loc 1 1361 0
	movl	-24(%ebp), %eax
	movb	$46, 6(%eax)
	.loc 1 1358 0
	jmp	.L384
.L475:
	.loc 1 1365 0
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 1366 0
	movl	-24(%ebp), %eax
	movb	$8, 6(%eax)
	.loc 1 1368 0
	jmp	.L384
.L362:
	.loc 1 1371 0
	movl	-24(%ebp), %eax
	movb	$52, 6(%eax)
	.loc 1 1372 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	movl	%eax, -36(%ebp)
	.loc 1 1373 0
	cmpl	$46, -36(%ebp)
	jne	.L478
.LBB16:
	.loc 1 1375 0
	movl	-32(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -8(%ebp)
	.loc 1 1377 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	cmpl	$46, %eax
	jne	.L480
	.loc 1 1378 0
	movl	-24(%ebp), %eax
	movb	$48, 6(%eax)
	jmp	.L384
.L480:
	.loc 1 1380 0
	movl	-8(%ebp), %edx
	subl	$1, %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	jmp	.L384
.L478:
.LBE16:
	.loc 1 1383 0
	movzbl	-36(%ebp),%edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$4, %eax
	testl	%eax, %eax
	je	.L484
	.loc 1 1385 0
	movl	-24(%ebp), %eax
	movb	$58, 6(%eax)
	.loc 1 1386 0
	movl	-24(%ebp), %eax
	addl	$8, %eax
	movl	$1, 8(%esp)
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	parse_number
	jmp	.L384
.L484:
	.loc 1 1388 0
	cmpl	$42, -36(%ebp)
	jne	.L486
	movl	8(%ebp), %eax
	movzbl	393(%eax), %eax
	testb	%al, %al
	je	.L486
	.loc 1 1389 0
	movl	-24(%ebp), %eax
	movb	$55, 6(%eax)
	.loc 1 1388 0
	jmp	.L384
.L486:
	.loc 1 1391 0
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 1392 0
	jmp	.L384
.L359:
	.loc 1 1395 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	movl	%eax, -36(%ebp)
	.loc 1 1396 0
	cmpl	$43, -36(%ebp)
	jne	.L489
	.loc 1 1397 0
	movl	-24(%ebp), %eax
	movb	$49, 6(%eax)
	jmp	.L384
.L489:
	.loc 1 1398 0
	cmpl	$61, -36(%ebp)
	jne	.L492
	.loc 1 1399 0
	movl	-24(%ebp), %eax
	movb	$29, 6(%eax)
	jmp	.L384
.L492:
	.loc 1 1402 0
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 1403 0
	movl	-24(%ebp), %eax
	movb	$4, 6(%eax)
	.loc 1 1405 0
	jmp	.L384
.L361:
	.loc 1 1408 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	movl	%eax, -36(%ebp)
	.loc 1 1409 0
	cmpl	$62, -36(%ebp)
	jne	.L494
	.loc 1 1411 0
	movl	-24(%ebp), %eax
	movb	$51, 6(%eax)
	.loc 1 1412 0
	movl	8(%ebp), %eax
	movzbl	393(%eax), %eax
	testb	%al, %al
	je	.L384
	.loc 1 1414 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	cmpl	$42, %eax
	jne	.L498
	.loc 1 1415 0
	movl	-24(%ebp), %eax
	movb	$54, 6(%eax)
	jmp	.L384
.L498:
	.loc 1 1417 0
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	jmp	.L384
.L494:
	.loc 1 1420 0
	cmpl	$45, -36(%ebp)
	jne	.L501
	.loc 1 1421 0
	movl	-24(%ebp), %eax
	movb	$50, 6(%eax)
	jmp	.L384
.L501:
	.loc 1 1422 0
	cmpl	$61, -36(%ebp)
	jne	.L503
	.loc 1 1423 0
	movl	-24(%ebp), %eax
	movb	$30, 6(%eax)
	jmp	.L384
.L503:
	.loc 1 1426 0
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 1427 0
	movl	-24(%ebp), %eax
	movb	$5, 6(%eax)
	.loc 1 1429 0
	jmp	.L384
.L355:
	.loc 1 1432 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	movl	%eax, -36(%ebp)
	.loc 1 1433 0
	cmpl	$38, -36(%ebp)
	jne	.L505
	.loc 1 1434 0
	movl	-24(%ebp), %eax
	movb	$17, 6(%eax)
	jmp	.L384
.L505:
	.loc 1 1435 0
	cmpl	$61, -36(%ebp)
	jne	.L508
	.loc 1 1436 0
	movl	-24(%ebp), %eax
	movb	$34, 6(%eax)
	jmp	.L384
.L508:
	.loc 1 1439 0
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 1440 0
	movl	-24(%ebp), %eax
	movb	$9, 6(%eax)
	.loc 1 1442 0
	jmp	.L384
.L378:
	.loc 1 1445 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	movl	%eax, -36(%ebp)
	.loc 1 1446 0
	cmpl	$124, -36(%ebp)
	jne	.L510
	.loc 1 1447 0
	movl	-24(%ebp), %eax
	movb	$18, 6(%eax)
	jmp	.L384
.L510:
	.loc 1 1448 0
	cmpl	$61, -36(%ebp)
	jne	.L513
	.loc 1 1449 0
	movl	-24(%ebp), %eax
	movb	$35, 6(%eax)
	jmp	.L384
.L513:
	.loc 1 1452 0
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 1453 0
	movl	-24(%ebp), %eax
	movb	$10, 6(%eax)
	.loc 1 1455 0
	jmp	.L384
.L365:
	.loc 1 1458 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	movl	%eax, -36(%ebp)
	.loc 1 1459 0
	cmpl	$58, -36(%ebp)
	jne	.L515
	movl	8(%ebp), %eax
	movzbl	393(%eax), %eax
	testb	%al, %al
	je	.L515
	.loc 1 1460 0
	movl	-24(%ebp), %eax
	movb	$53, 6(%eax)
	.loc 1 1459 0
	jmp	.L384
.L515:
	.loc 1 1461 0
	cmpl	$62, -36(%ebp)
	jne	.L519
	movl	8(%ebp), %eax
	movzbl	399(%eax), %eax
	testb	%al, %al
	je	.L519
	.loc 1 1463 0
	movl	-24(%ebp), %eax
	movzbl	7(%eax), %eax
	movl	%eax, %edx
	orl	$2, %edx
	movl	-24(%ebp), %eax
	movb	%dl, 7(%eax)
	.loc 1 1464 0
	movl	-24(%ebp), %eax
	movb	$44, 6(%eax)
	.loc 1 1461 0
	jmp	.L384
.L519:
	.loc 1 1468 0
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 1469 0
	movl	-24(%ebp), %eax
	movb	$20, 6(%eax)
	.loc 1 1471 0
	jmp	.L384
.L358:
	.loc 1 1473 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	cmpl	$61, %eax
	jne	.L522
	movl	-24(%ebp), %eax
	movb	$31, 6(%eax)
	jmp	.L384
.L522:
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	movl	-24(%ebp), %eax
	movb	$6, 6(%eax)
	jmp	.L384
.L368:
	.loc 1 1474 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	cmpl	$61, %eax
	jne	.L525
	movl	-24(%ebp), %eax
	movb	$25, 6(%eax)
	jmp	.L384
.L525:
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	movl	-24(%ebp), %eax
	movb	$0, 6(%eax)
	jmp	.L384
.L350:
	.loc 1 1475 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	cmpl	$61, %eax
	jne	.L528
	movl	-24(%ebp), %eax
	movb	$26, 6(%eax)
	jmp	.L384
.L528:
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	movl	-24(%ebp), %eax
	movb	$1, 6(%eax)
	jmp	.L384
.L376:
	.loc 1 1476 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	cmpl	$61, %eax
	jne	.L531
	movl	-24(%ebp), %eax
	movb	$36, 6(%eax)
	jmp	.L384
.L531:
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	movl	-24(%ebp), %eax
	movb	$11, 6(%eax)
	jmp	.L384
.L352:
	.loc 1 1477 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	get_effective_char
	cmpl	$35, %eax
	jne	.L534
	movl	-24(%ebp), %eax
	movb	$42, 6(%eax)
	jmp	.L384
.L534:
	movl	-32(%ebp), %eax
	movl	4(%eax), %edx
	movl	-32(%ebp), %eax
	movl	%edx, (%eax)
	movl	-24(%ebp), %eax
	movb	$41, 6(%eax)
	jmp	.L384
.L380:
	.loc 1 1479 0
	movl	-24(%ebp), %eax
	movb	$16, 6(%eax)
	jmp	.L384
.L360:
	.loc 1 1480 0
	movl	-24(%ebp), %eax
	movb	$21, 6(%eax)
	jmp	.L384
.L356:
	.loc 1 1481 0
	movl	-24(%ebp), %eax
	movb	$22, 6(%eax)
	.loc 1 1483 0
	movl	in_omp_pragma@GOT(%ebx), %eax
	movzbl	(%eax), %eax
	testb	%al, %al
	je	.L384
	movl	seen_omp_paren@GOT(%ebx), %eax
	movb	$1, (%eax)
	.loc 1 1485 0
	jmp	.L384
.L357:
	.loc 1 1486 0
	movl	-24(%ebp), %eax
	movb	$23, 6(%eax)
	jmp	.L384
.L374:
	.loc 1 1487 0
	movl	-24(%ebp), %eax
	movb	$43, 6(%eax)
	jmp	.L384
.L375:
	.loc 1 1488 0
	movl	-24(%ebp), %eax
	movb	$44, 6(%eax)
	jmp	.L384
.L377:
	.loc 1 1489 0
	movl	-24(%ebp), %eax
	movb	$45, 6(%eax)
	jmp	.L384
.L379:
	.loc 1 1490 0
	movl	-24(%ebp), %eax
	movb	$46, 6(%eax)
	jmp	.L384
.L366:
	.loc 1 1491 0
	movl	-24(%ebp), %eax
	movb	$47, 6(%eax)
	jmp	.L384
.L371:
	.loc 1 1494 0
	movl	-24(%ebp), %eax
	movb	$56, 6(%eax)
	jmp	.L384
.L353:
	.loc 1 1497 0
	movl	8(%ebp), %eax
	movzbl	420(%eax), %eax
	testb	%al, %al
	jne	.L372
.L347:
	.loc 1 1503 0
	movl	-24(%ebp), %eax
	movb	$61, 6(%eax)
	.loc 1 1504 0
	movl	-36(%ebp), %eax
	movl	%eax, %edx
	movl	-24(%ebp), %eax
	movb	%dl, 8(%eax)
.L384:
	.loc 1 1508 0
	movl	-24(%ebp), %eax
	movl	%eax, -44(%ebp)
.L346:
	movl	-44(%ebp), %eax
	.loc 1 1509 0
	addl	$68, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE44:
	.size	_cpp_lex_direct, .-_cpp_lex_direct
.globl cpp_token_len
	.type	cpp_token_len, @function
cpp_token_len:
.LFB45:
	.loc 1 1516 0
	pushl	%ebp
.LCFI104:
	movl	%esp, %ebp
.LCFI105:
	subl	$20, %esp
.LCFI106:
	call	__i686.get_pc_thunk.cx
	addl	$_GLOBAL_OFFSET_TABLE_, %ecx
	.loc 1 1519 0
	movl	8(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	movl	token_spellings@GOTOFF(%ecx,%eax,8), %eax
	movl	%eax, -20(%ebp)
	cmpl	$2, -20(%ebp)
	je	.L544
	cmpl	$2, -20(%ebp)
	jb	.L543
	cmpl	$4, -20(%ebp)
	ja	.L543
	jmp	.L545
.L543:
	.loc 1 1521 0
	movl	$0, -4(%ebp)
	jmp	.L546
.L545:
	.loc 1 1523 0
	movl	8(%ebp), %eax
	movl	8(%eax), %eax
	movl	%eax, -4(%ebp)
	jmp	.L546
.L544:
	.loc 1 1524 0
	movl	8(%ebp), %eax
	movl	8(%eax), %eax
	movl	4(%eax), %eax
	movl	%eax, -4(%ebp)
.L546:
	.loc 1 1527 0
	movl	-4(%ebp), %eax
	addl	$5, %eax
	.loc 1 1528 0
	leave
	ret
.LFE45:
	.size	cpp_token_len, .-cpp_token_len
	.section	.rodata
.LC99:
	.string	"unknown string token %s\n"
.LC100:
	.string	"unspellable token %s"
	.text
.globl cpp_spell_token
	.type	cpp_spell_token, @function
cpp_spell_token:
.LFB46:
	.loc 1 1539 0
	pushl	%ebp
.LCFI107:
	movl	%esp, %ebp
.LCFI108:
	pushl	%edi
.LCFI109:
	pushl	%esi
.LCFI110:
	pushl	%ebx
.LCFI111:
	subl	$60, %esp
.LCFI112:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 1540 0
	movl	12(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	movl	token_spellings@GOTOFF(%ebx,%eax,8), %eax
	movl	%eax, -52(%ebp)
	cmpl	$5, -52(%ebp)
	ja	.L549
	movl	-52(%ebp), %eax
	sall	$2, %eax
	movl	.L556@GOTOFF(%eax,%ebx), %eax
	addl	%ebx, %eax
	jmp	*%eax
	.section	.rodata
	.align 4
	.align 4
.L556:
	.long	.L550@GOTOFF
	.long	.L551@GOTOFF
	.long	.L552@GOTOFF
	.long	.L553@GOTOFF
	.long	.L554@GOTOFF
	.long	.L555@GOTOFF
	.text
.L550:
.LBB17:
	.loc 1 1547 0
	movl	12(%ebp), %eax
	movzbl	7(%eax), %eax
	movzbl	%al, %eax
	andl	$2, %eax
	testl	%eax, %eax
	je	.L557
	.loc 1 1548 0
	movl	12(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	subl	$41, %eax
	movl	digraph_spellings@GOTOFF(%ebx,%eax,4), %eax
	movl	%eax, -32(%ebp)
	jmp	.L561
.L557:
	.loc 1 1550 0
	movl	12(%ebp), %eax
	movzbl	7(%eax), %eax
	movzbl	%al, %eax
	andl	$16, %eax
	testl	%eax, %eax
	jne	.L552
	.loc 1 1553 0
	movl	12(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	movl	4+token_spellings@GOTOFF(%ebx,%eax,8), %eax
	movl	%eax, -32(%ebp)
	.loc 1 1555 0
	jmp	.L561
.L562:
	.loc 1 1556 0
	movl	16(%ebp), %edx
	movzbl	-13(%ebp), %eax
	movb	%al, (%edx)
	addl	$1, 16(%ebp)
.L561:
	.loc 1 1555 0
	movl	-32(%ebp), %eax
	movzbl	(%eax), %eax
	movb	%al, -13(%ebp)
	cmpb	$0, -13(%ebp)
	setne	%al
	addl	$1, -32(%ebp)
	testb	%al, %al
	jne	.L562
	.loc 1 1558 0
	jmp	.L549
.L551:
.LBE17:
	.loc 1 1561 0
	movl	12(%ebp), %eax
	movzbl	8(%eax), %edx
	movl	16(%ebp), %eax
	movb	%dl, (%eax)
	addl	$1, 16(%ebp)
	.loc 1 1562 0
	jmp	.L549
.L552:
	.loc 1 1566 0
	movl	12(%ebp), %eax
	movl	8(%eax), %eax
	movl	4(%eax), %ecx
	movl	12(%ebp), %eax
	movl	8(%eax), %eax
	movl	(%eax), %edx
	movl	16(%ebp), %eax
	movl	%eax, %edi
	movl	%edx, %esi
	cld
	rep
	movsb
	.loc 1 1567 0
	movl	12(%ebp), %eax
	movl	8(%eax), %eax
	movl	4(%eax), %eax
	addl	%eax, 16(%ebp)
	.loc 1 1568 0
	jmp	.L549
.L553:
	.loc 1 1571 0
	movl	12(%ebp), %eax
	movl	8(%eax), %ecx
	movl	12(%ebp), %eax
	movl	12(%eax), %edx
	movl	16(%ebp), %eax
	movl	%eax, %edi
	movl	%edx, %esi
	cld
	rep
	movsb
	.loc 1 1572 0
	movl	12(%ebp), %eax
	movl	8(%eax), %eax
	addl	%eax, 16(%ebp)
	.loc 1 1573 0
	jmp	.L549
.L554:
.LBB18:
	.loc 1 1578 0
	movl	12(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	subl	$59, %eax
	movl	%eax, -56(%ebp)
	cmpl	$5, -56(%ebp)
	ja	.L564
	movl	-56(%ebp), %eax
	sall	$2, %eax
	movl	.L570@GOTOFF(%eax,%ebx), %eax
	addl	%ebx, %eax
	jmp	*%eax
	.section	.rodata
	.align 4
	.align 4
.L570:
	.long	.L565@GOTOFF
	.long	.L566@GOTOFF
	.long	.L564@GOTOFF
	.long	.L567@GOTOFF
	.long	.L568@GOTOFF
	.long	.L569@GOTOFF
	.text
.L567:
	.loc 1 1580 0
	movl	$34, -28(%ebp)
	movl	$34, -24(%ebp)
	movl	$0, -20(%ebp)
	jmp	.L571
.L568:
	.loc 1 1581 0
	movl	$34, -28(%ebp)
	movl	$34, -24(%ebp)
	movl	$76, -20(%ebp)
	jmp	.L571
.L565:
	.loc 1 1582 0
	movl	$39, -28(%ebp)
	movl	$39, -24(%ebp)
	movl	$0, -20(%ebp)
	jmp	.L571
.L566:
	.loc 1 1583 0
	movl	$39, -28(%ebp)
	movl	$39, -24(%ebp)
	movl	$76, -20(%ebp)
	jmp	.L571
.L569:
	.loc 1 1584 0
	movl	$60, -28(%ebp)
	movl	$62, -24(%ebp)
	movl	$0, -20(%ebp)
	jmp	.L571
.L564:
	.loc 1 1586 0
	movl	12(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	movl	4+token_spellings@GOTOFF(%ebx,%eax,8), %eax
	movl	%eax, 12(%esp)
	leal	.LC99@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$4, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	.loc 1 1588 0
	movl	16(%ebp), %eax
	movl	%eax, -48(%ebp)
	jmp	.L572
.L571:
	.loc 1 1590 0
	cmpl	$0, -20(%ebp)
	je	.L573
	movl	-20(%ebp), %eax
	movl	%eax, %edx
	movl	16(%ebp), %eax
	movb	%dl, (%eax)
	addl	$1, 16(%ebp)
.L573:
	.loc 1 1591 0
	movl	-28(%ebp), %eax
	movl	%eax, %edx
	movl	16(%ebp), %eax
	movb	%dl, (%eax)
	addl	$1, 16(%ebp)
	.loc 1 1592 0
	movl	12(%ebp), %eax
	movl	8(%eax), %ecx
	movl	12(%ebp), %eax
	movl	12(%eax), %edx
	movl	16(%ebp), %eax
	movl	%eax, %edi
	movl	%edx, %esi
	cld
	rep
	movsb
	.loc 1 1593 0
	movl	12(%ebp), %eax
	movl	8(%eax), %eax
	addl	%eax, 16(%ebp)
	.loc 1 1594 0
	movl	-24(%ebp), %eax
	movl	%eax, %edx
	movl	16(%ebp), %eax
	movb	%dl, (%eax)
	addl	$1, 16(%ebp)
	.loc 1 1596 0
	jmp	.L549
.L555:
.LBE18:
	.loc 1 1599 0
	movl	12(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	movl	4+token_spellings@GOTOFF(%ebx,%eax,8), %eax
	movl	%eax, 12(%esp)
	leal	.LC100@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$4, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L549:
	.loc 1 1603 0
	movl	16(%ebp), %eax
	movl	%eax, -48(%ebp)
.L572:
	movl	-48(%ebp), %eax
	.loc 1 1604 0
	addl	$60, %esp
	popl	%ebx
	popl	%esi
	popl	%edi
	popl	%ebp
	ret
.LFE46:
	.size	cpp_spell_token, .-cpp_spell_token
.globl cpp_token_as_text
	.type	cpp_token_as_text, @function
cpp_token_as_text:
.LFB47:
	.loc 1 1612 0
	pushl	%ebp
.LCFI113:
	movl	%esp, %ebp
.LCFI114:
	pushl	%ebx
.LCFI115:
	subl	$36, %esp
.LCFI116:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 1613 0
	movl	12(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_token_len@PLT
	movl	%eax, -16(%ebp)
	.loc 1 1614 0
	movl	-16(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_unaligned_alloc@PLT
	movl	%eax, -12(%ebp)
	.loc 1 1616 0
	movl	-12(%ebp), %eax
	movl	%eax, 8(%esp)
	movl	12(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_spell_token@PLT
	movl	%eax, -8(%ebp)
	.loc 1 1617 0
	movl	-8(%ebp), %eax
	movb	$0, (%eax)
	.loc 1 1619 0
	movl	-12(%ebp), %eax
	.loc 1 1620 0
	addl	$36, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE47:
	.size	cpp_token_as_text, .-cpp_token_as_text
.globl cpp_type2name
	.type	cpp_type2name, @function
cpp_type2name:
.LFB48:
	.loc 1 1627 0
	pushl	%ebp
.LCFI117:
	movl	%esp, %ebp
.LCFI118:
	call	__i686.get_pc_thunk.cx
	addl	$_GLOBAL_OFFSET_TABLE_, %ecx
	.loc 1 1628 0
	movl	8(%ebp), %eax
	movl	4+token_spellings@GOTOFF(%ecx,%eax,8), %eax
	.loc 1 1629 0
	popl	%ebp
	ret
.LFE48:
	.size	cpp_type2name, .-cpp_type2name
	.section	.rodata
.LC101:
	.string	"impossible STRING token %s\n"
	.text
.globl cpp_output_token
	.type	cpp_output_token, @function
cpp_output_token:
.LFB49:
	.loc 1 1638 0
	pushl	%ebp
.LCFI119:
	movl	%esp, %ebp
.LCFI120:
	pushl	%ebx
.LCFI121:
	subl	$68, %esp
.LCFI122:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 1639 0
	movl	8(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	movl	token_spellings@GOTOFF(%ebx,%eax,8), %eax
	movl	%eax, -40(%ebp)
	cmpl	$4, -40(%ebp)
	ja	.L604
	movl	-40(%ebp), %eax
	sall	$2, %eax
	movl	.L587@GOTOFF(%eax,%ebx), %eax
	addl	%ebx, %eax
	jmp	*%eax
	.section	.rodata
	.align 4
	.align 4
.L587:
	.long	.L582@GOTOFF
	.long	.L583@GOTOFF
	.long	.L584@GOTOFF
	.long	.L585@GOTOFF
	.long	.L586@GOTOFF
	.text
.L582:
.LBB19:
	.loc 1 1646 0
	movl	8(%ebp), %eax
	movzbl	7(%eax), %eax
	movzbl	%al, %eax
	andl	$2, %eax
	testl	%eax, %eax
	je	.L588
	.loc 1 1647 0
	movl	8(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	subl	$41, %eax
	movl	digraph_spellings@GOTOFF(%ebx,%eax,4), %eax
	movl	%eax, -24(%ebp)
	jmp	.L590
.L588:
	.loc 1 1649 0
	movl	8(%ebp), %eax
	movzbl	7(%eax), %eax
	movzbl	%al, %eax
	andl	$16, %eax
	testl	%eax, %eax
	jne	.L584
	.loc 1 1652 0
	movl	8(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	movl	4+token_spellings@GOTOFF(%ebx,%eax,8), %eax
	movl	%eax, -24(%ebp)
.L590:
	.loc 1 1654 0
	movl	-24(%ebp), %eax
	movzbl	(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, -20(%ebp)
.L592:
	.loc 1 1656 0
	movl	12(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	-20(%ebp), %eax
	movl	%eax, (%esp)
	call	putc_unlocked@PLT
	.loc 1 1657 0
	addl	$1, -24(%ebp)
	movl	-24(%ebp), %eax
	movzbl	(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, -20(%ebp)
	cmpl	$0, -20(%ebp)
	jne	.L592
	.loc 1 1659 0
	jmp	.L604
.L583:
.LBE19:
	.loc 1 1662 0
	movl	8(%ebp), %eax
	movzbl	8(%eax), %eax
	movzbl	%al, %edx
	movl	12(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	%edx, (%esp)
	call	putc_unlocked@PLT
	.loc 1 1663 0
	jmp	.L604
.L584:
	.loc 1 1667 0
	movl	8(%ebp), %eax
	movl	8(%eax), %eax
	movl	4(%eax), %ecx
	movl	8(%ebp), %eax
	movl	8(%eax), %eax
	movl	(%eax), %edx
	movl	12(%ebp), %eax
	movl	%eax, 12(%esp)
	movl	%ecx, 8(%esp)
	movl	$1, 4(%esp)
	movl	%edx, (%esp)
	call	fwrite_unlocked@PLT
	.loc 1 1668 0
	jmp	.L604
.L585:
	.loc 1 1671 0
	movl	8(%ebp), %eax
	movl	8(%eax), %edx
	movl	8(%ebp), %eax
	movl	12(%eax), %ecx
	movl	12(%ebp), %eax
	movl	%eax, 12(%esp)
	movl	%edx, 8(%esp)
	movl	$1, 4(%esp)
	movl	%ecx, (%esp)
	call	fwrite_unlocked@PLT
	.loc 1 1672 0
	jmp	.L604
.L586:
.LBB20:
	.loc 1 1677 0
	movl	8(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	subl	$59, %eax
	movl	%eax, -44(%ebp)
	cmpl	$5, -44(%ebp)
	ja	.L594
	movl	-44(%ebp), %eax
	sall	$2, %eax
	movl	.L600@GOTOFF(%eax,%ebx), %eax
	addl	%ebx, %eax
	jmp	*%eax
	.section	.rodata
	.align 4
	.align 4
.L600:
	.long	.L595@GOTOFF
	.long	.L596@GOTOFF
	.long	.L594@GOTOFF
	.long	.L597@GOTOFF
	.long	.L598@GOTOFF
	.long	.L599@GOTOFF
	.text
.L597:
	.loc 1 1679 0
	movl	$34, -16(%ebp)
	movl	$34, -12(%ebp)
	movl	$0, -8(%ebp)
	jmp	.L601
.L598:
	.loc 1 1680 0
	movl	$34, -16(%ebp)
	movl	$34, -12(%ebp)
	movl	$76, -8(%ebp)
	jmp	.L601
.L595:
	.loc 1 1681 0
	movl	$39, -16(%ebp)
	movl	$39, -12(%ebp)
	movl	$0, -8(%ebp)
	jmp	.L601
.L596:
	.loc 1 1682 0
	movl	$39, -16(%ebp)
	movl	$39, -12(%ebp)
	movl	$76, -8(%ebp)
	jmp	.L601
.L599:
	.loc 1 1683 0
	movl	$60, -16(%ebp)
	movl	$62, -12(%ebp)
	movl	$0, -8(%ebp)
	jmp	.L601
.L594:
	.loc 1 1685 0
	movl	8(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	movl	4+token_spellings@GOTOFF(%ebx,%eax,8), %edx
	movl	stderr@GOT(%ebx), %eax
	movl	(%eax), %ecx
	movl	%edx, 8(%esp)
	leal	.LC101@GOTOFF(%ebx), %eax
	movl	%eax, 4(%esp)
	movl	%ecx, (%esp)
	call	fprintf@PLT
	.loc 1 1686 0
	jmp	.L604
.L601:
	.loc 1 1688 0
	cmpl	$0, -8(%ebp)
	je	.L602
	movl	12(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	-8(%ebp), %eax
	movl	%eax, (%esp)
	call	putc_unlocked@PLT
.L602:
	.loc 1 1689 0
	movl	12(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	-16(%ebp), %eax
	movl	%eax, (%esp)
	call	putc_unlocked@PLT
	.loc 1 1690 0
	movl	8(%ebp), %eax
	movl	8(%eax), %edx
	movl	8(%ebp), %eax
	movl	12(%eax), %ecx
	movl	12(%ebp), %eax
	movl	%eax, 12(%esp)
	movl	%edx, 8(%esp)
	movl	$1, 4(%esp)
	movl	%ecx, (%esp)
	call	fwrite_unlocked@PLT
	.loc 1 1691 0
	movl	12(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	-12(%ebp), %eax
	movl	%eax, (%esp)
	call	putc_unlocked@PLT
.L604:
.LBE20:
	.loc 1 1699 0
	addl	$68, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE49:
	.size	cpp_output_token, .-cpp_output_token
.globl _cpp_equiv_tokens
	.type	_cpp_equiv_tokens, @function
_cpp_equiv_tokens:
.LFB50:
	.loc 1 1705 0
	pushl	%ebp
.LCFI123:
	movl	%esp, %ebp
.LCFI124:
	pushl	%edi
.LCFI125:
	pushl	%esi
.LCFI126:
	pushl	%ebx
.LCFI127:
	subl	$28, %esp
.LCFI128:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 1706 0
	movl	8(%ebp), %eax
	movzbl	6(%eax), %edx
	movl	12(%ebp), %eax
	movzbl	6(%eax), %eax
	cmpb	%al, %dl
	jne	.L606
	movl	8(%ebp), %eax
	movzbl	7(%eax), %edx
	movl	12(%ebp), %eax
	movzbl	7(%eax), %eax
	cmpb	%al, %dl
	jne	.L606
	.loc 1 1707 0
	movl	8(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	movl	token_spellings@GOTOFF(%ebx,%eax,8), %eax
	movl	%eax, -28(%ebp)
	cmpl	$5, -28(%ebp)
	ja	.L609
	movl	-28(%ebp), %eax
	sall	$2, %eax
	movl	.L614@GOTOFF(%eax,%ebx), %eax
	addl	%ebx, %eax
	jmp	*%eax
	.section	.rodata
	.align 4
	.align 4
.L614:
	.long	.L609@GOTOFF
	.long	.L610@GOTOFF
	.long	.L611@GOTOFF
	.long	.L612@GOTOFF
	.long	.L612@GOTOFF
	.long	.L613@GOTOFF
	.text
.L609:
	.loc 1 1711 0
	movl	$1, -24(%ebp)
	jmp	.L615
.L610:
	.loc 1 1713 0
	movl	8(%ebp), %eax
	movzbl	8(%eax), %edx
	movl	12(%ebp), %eax
	movzbl	8(%eax), %eax
	cmpb	%al, %dl
	sete	%al
	movzbl	%al, %eax
	movl	%eax, -24(%ebp)
	jmp	.L615
.L613:
	.loc 1 1715 0
	movl	8(%ebp), %eax
	movzbl	6(%eax), %eax
	cmpb	$66, %al
	jne	.L616
	movl	8(%ebp), %eax
	movl	8(%eax), %edx
	movl	12(%ebp), %eax
	movl	8(%eax), %eax
	cmpl	%eax, %edx
	jne	.L618
.L616:
	movl	$1, -20(%ebp)
	jmp	.L619
.L618:
	movl	$0, -20(%ebp)
.L619:
	movl	-20(%ebp), %eax
	movl	%eax, -24(%ebp)
	jmp	.L615
.L611:
	.loc 1 1717 0
	movl	8(%ebp), %eax
	movl	8(%eax), %edx
	movl	12(%ebp), %eax
	movl	8(%eax), %eax
	cmpl	%eax, %edx
	sete	%al
	movzbl	%al, %eax
	movl	%eax, -24(%ebp)
	jmp	.L615
.L612:
	.loc 1 1720 0
	movl	8(%ebp), %eax
	movl	8(%eax), %edx
	movl	12(%ebp), %eax
	movl	8(%eax), %eax
	cmpl	%eax, %edx
	jne	.L620
	movl	8(%ebp), %eax
	movl	8(%eax), %ecx
	movl	12(%ebp), %eax
	movl	12(%eax), %edx
	movl	8(%ebp), %eax
	movl	12(%eax), %eax
	movl	%eax, -32(%ebp)
	movl	%edx, -36(%ebp)
	movl	%ecx, -40(%ebp)
	cld
	movl	-40(%ebp), %ecx
	cmpl	%ecx, -40(%ebp)
	movl	-32(%ebp), %esi
	movl	-36(%ebp), %edi
	movl	-40(%ebp), %ecx
	repz
	cmpsb
	seta	%dl
	setb	%al
	movl	%edx, %ecx
	subb	%al, %cl
	movl	%ecx, %eax
	movsbl	%al,%eax
	testl	%eax, %eax
	jne	.L620
	movl	$1, -16(%ebp)
	jmp	.L623
.L620:
	movl	$0, -16(%ebp)
.L623:
	movl	-16(%ebp), %esi
	movl	%esi, -24(%ebp)
	jmp	.L615
.L606:
	.loc 1 1725 0
	movl	$0, -24(%ebp)
.L615:
	movl	-24(%ebp), %eax
	.loc 1 1726 0
	addl	$28, %esp
	popl	%ebx
	popl	%esi
	popl	%edi
	popl	%ebp
	ret
.LFE50:
	.size	_cpp_equiv_tokens, .-_cpp_equiv_tokens
.globl cpp_avoid_paste
	.type	cpp_avoid_paste, @function
cpp_avoid_paste:
.LFB51:
	.loc 1 1736 0
	pushl	%ebp
.LCFI129:
	movl	%esp, %ebp
.LCFI130:
	pushl	%ebx
.LCFI131:
	subl	$72, %esp
.LCFI132:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 1737 0
	movl	12(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, -16(%ebp)
	movl	16(%ebp), %eax
	movzbl	6(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, -12(%ebp)
	.loc 1 1740 0
	movl	12(%ebp), %eax
	movzbl	7(%eax), %eax
	movzbl	%al, %eax
	andl	$16, %eax
	testl	%eax, %eax
	je	.L626
	.loc 1 1741 0
	movl	$57, -16(%ebp)
.L626:
	.loc 1 1742 0
	movl	16(%ebp), %eax
	movzbl	7(%eax), %eax
	movzbl	%al, %eax
	andl	$16, %eax
	testl	%eax, %eax
	je	.L628
	.loc 1 1743 0
	movl	$57, -12(%ebp)
.L628:
	.loc 1 1745 0
	movl	$-1, -8(%ebp)
	.loc 1 1746 0
	movl	16(%ebp), %eax
	movzbl	7(%eax), %eax
	movzbl	%al, %eax
	andl	$2, %eax
	testl	%eax, %eax
	je	.L630
	.loc 1 1747 0
	movl	-12(%ebp), %eax
	subl	$41, %eax
	movl	digraph_spellings@GOTOFF(%ebx,%eax,4), %eax
	movzbl	(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, -8(%ebp)
	jmp	.L632
.L630:
	.loc 1 1748 0
	movl	-12(%ebp), %eax
	movl	token_spellings@GOTOFF(%ebx,%eax,8), %eax
	testl	%eax, %eax
	jne	.L632
	.loc 1 1749 0
	movl	-12(%ebp), %eax
	movl	4+token_spellings@GOTOFF(%ebx,%eax,8), %eax
	movzbl	(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, -8(%ebp)
.L632:
	.loc 1 1752 0
	movl	-16(%ebp), %eax
	cmpl	$15, %eax
	jg	.L634
	cmpl	$61, -8(%ebp)
	jne	.L634
	.loc 1 1753 0
	movl	$1, -68(%ebp)
	jmp	.L637
.L634:
	.loc 1 1755 0
	cmpl	$61, -16(%ebp)
	ja	.L638
	movl	-16(%ebp), %eax
	sall	$2, %eax
	movl	.L654@GOTOFF(%eax,%ebx), %eax
	addl	%ebx, %eax
	jmp	*%eax
	.section	.rodata
	.align 4
	.align 4
.L654:
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L639@GOTOFF
	.long	.L640@GOTOFF
	.long	.L641@GOTOFF
	.long	.L642@GOTOFF
	.long	.L638@GOTOFF
	.long	.L643@GOTOFF
	.long	.L644@GOTOFF
	.long	.L645@GOTOFF
	.long	.L646@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L647@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L648@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L649@GOTOFF
	.long	.L650@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L651@GOTOFF
	.long	.L652@GOTOFF
	.long	.L638@GOTOFF
	.long	.L638@GOTOFF
	.long	.L653@GOTOFF
	.text
.L639:
	.loc 1 1757 0
	cmpl	$62, -8(%ebp)
	je	.L655
	cmpl	$63, -8(%ebp)
	jne	.L657
.L655:
	movl	$1, -64(%ebp)
	jmp	.L658
.L657:
	movl	$0, -64(%ebp)
.L658:
	movl	-64(%ebp), %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L640:
	.loc 1 1758 0
	cmpl	$60, -8(%ebp)
	je	.L659
	cmpl	$63, -8(%ebp)
	je	.L659
	cmpl	$37, -8(%ebp)
	je	.L659
	cmpl	$58, -8(%ebp)
	jne	.L663
.L659:
	movl	$1, -60(%ebp)
	jmp	.L664
.L663:
	movl	$0, -60(%ebp)
.L664:
	movl	-60(%ebp), %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L641:
	.loc 1 1759 0
	cmpl	$43, -8(%ebp)
	sete	%al
	movzbl	%al, %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L642:
	.loc 1 1760 0
	cmpl	$45, -8(%ebp)
	je	.L665
	cmpl	$62, -8(%ebp)
	jne	.L667
.L665:
	movl	$1, -56(%ebp)
	jmp	.L668
.L667:
	movl	$0, -56(%ebp)
.L668:
	movl	-56(%ebp), %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L643:
	.loc 1 1761 0
	cmpl	$47, -8(%ebp)
	je	.L669
	cmpl	$42, -8(%ebp)
	jne	.L671
.L669:
	movl	$1, -52(%ebp)
	jmp	.L672
.L671:
	movl	$0, -52(%ebp)
.L672:
	movl	-52(%ebp), %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L644:
	.loc 1 1762 0
	cmpl	$58, -8(%ebp)
	je	.L673
	cmpl	$62, -8(%ebp)
	jne	.L675
.L673:
	movl	$1, -48(%ebp)
	jmp	.L676
.L675:
	movl	$0, -48(%ebp)
.L676:
	movl	-48(%ebp), %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L645:
	.loc 1 1763 0
	cmpl	$38, -8(%ebp)
	sete	%al
	movzbl	%al, %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L646:
	.loc 1 1764 0
	cmpl	$124, -8(%ebp)
	sete	%al
	movzbl	%al, %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L647:
	.loc 1 1765 0
	cmpl	$58, -8(%ebp)
	je	.L677
	cmpl	$62, -8(%ebp)
	jne	.L679
.L677:
	movl	$1, -44(%ebp)
	jmp	.L680
.L679:
	movl	$0, -44(%ebp)
.L680:
	movl	-44(%ebp), %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L649:
	.loc 1 1766 0
	cmpl	$42, -8(%ebp)
	sete	%al
	movzbl	%al, %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L650:
	.loc 1 1767 0
	cmpl	$46, -8(%ebp)
	je	.L681
	cmpl	$37, -8(%ebp)
	je	.L681
	cmpl	$58, -12(%ebp)
	jne	.L684
.L681:
	movl	$1, -40(%ebp)
	jmp	.L685
.L684:
	movl	$0, -40(%ebp)
.L685:
	movl	-40(%ebp), %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L648:
	.loc 1 1768 0
	cmpl	$35, -8(%ebp)
	je	.L686
	cmpl	$37, -8(%ebp)
	jne	.L688
.L686:
	movl	$1, -36(%ebp)
	jmp	.L689
.L688:
	movl	$0, -36(%ebp)
.L689:
	movl	-36(%ebp), %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L651:
	.loc 1 1769 0
	cmpl	$58, -12(%ebp)
	jne	.L690
	movl	16(%ebp), %eax
	addl	$8, %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	name_p
	testl	%eax, %eax
	jne	.L692
.L690:
	cmpl	$57, -12(%ebp)
	je	.L692
	cmpl	$59, -12(%ebp)
	je	.L692
	cmpl	$62, -12(%ebp)
	jne	.L695
.L692:
	movl	$1, -32(%ebp)
	jmp	.L696
.L695:
	movl	$0, -32(%ebp)
.L696:
	movl	-32(%ebp), %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L652:
	.loc 1 1773 0
	cmpl	$58, -12(%ebp)
	je	.L697
	cmpl	$57, -12(%ebp)
	je	.L697
	cmpl	$46, -8(%ebp)
	je	.L697
	cmpl	$43, -8(%ebp)
	je	.L697
	cmpl	$45, -8(%ebp)
	jne	.L702
.L697:
	movl	$1, -28(%ebp)
	jmp	.L703
.L702:
	movl	$0, -28(%ebp)
.L703:
	movl	-28(%ebp), %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L653:
	.loc 1 1775 0
	movl	8(%ebp), %eax
	movzbl	395(%eax), %eax
	testb	%al, %al
	je	.L704
	movl	12(%ebp), %eax
	movzbl	8(%eax), %eax
	cmpb	$64, %al
	jne	.L704
	cmpl	$57, -12(%ebp)
	je	.L707
	cmpl	$62, -12(%ebp)
	jne	.L704
.L707:
	movl	$1, -24(%ebp)
	jmp	.L709
.L704:
	movl	$0, -24(%ebp)
.L709:
	movl	-24(%ebp), %eax
	movl	%eax, -68(%ebp)
	jmp	.L637
.L638:
	.loc 1 1781 0
	movl	$0, -68(%ebp)
.L637:
	movl	-68(%ebp), %eax
	.loc 1 1782 0
	addl	$72, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE51:
	.size	cpp_avoid_paste, .-cpp_avoid_paste
.globl cpp_output_line
	.type	cpp_output_line, @function
cpp_output_line:
.LFB52:
	.loc 1 1791 0
	pushl	%ebp
.LCFI133:
	movl	%esp, %ebp
.LCFI134:
	pushl	%ebx
.LCFI135:
	subl	$36, %esp
.LCFI136:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 1794 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_get_token@PLT
	movl	%eax, -8(%ebp)
	.loc 1 1795 0
	jmp	.L717
.L713:
	.loc 1 1797 0
	movl	12(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	-8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_output_token@PLT
	.loc 1 1798 0
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_get_token@PLT
	movl	%eax, -8(%ebp)
	.loc 1 1799 0
	movl	-8(%ebp), %eax
	movzbl	7(%eax), %eax
	movzbl	%al, %eax
	andl	$1, %eax
	testb	%al, %al
	je	.L712
	.loc 1 1800 0
	movl	12(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	$32, (%esp)
	call	putc_unlocked@PLT
.L712:
.L717:
	.loc 1 1795 0
	movl	-8(%ebp), %eax
	movzbl	6(%eax), %eax
	cmpb	$24, %al
	jne	.L713
	.loc 1 1803 0
	movl	12(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	$10, (%esp)
	call	putc_unlocked@PLT
	.loc 1 1804 0
	addl	$36, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE52:
	.size	cpp_output_line, .-cpp_output_line
	.type	hex_digit_value, @function
hex_digit_value:
.LFB53:
	.loc 1 1810 0
	pushl	%ebp
.LCFI137:
	movl	%esp, %ebp
.LCFI138:
	pushl	%ebx
.LCFI139:
	subl	$4, %esp
.LCFI140:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 1811 0
	movl	8(%ebp), %eax
	movzbl	%al, %edx
	movl	_hex_value@GOT(%ebx), %eax
	movzbl	(%eax,%edx), %eax
	cmpb	$99, %al
	je	.L719
	.loc 1 1812 0
	movl	8(%ebp), %eax
	movzbl	%al, %edx
	movl	_hex_value@GOT(%ebx), %eax
	movzbl	(%eax,%edx), %eax
	movzbl	%al, %eax
	movl	%eax, -8(%ebp)
	jmp	.L718
.L719:
	.loc 1 1814 0
	call	abort@PLT
.L718:
	.loc 1 1815 0
	movl	-8(%ebp), %eax
	addl	$4, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE53:
	.size	hex_digit_value, .-hex_digit_value
	.section	.rodata
	.align 4
.LC102:
	.string	"the meaning of '\\%c' is different in traditional C"
	.align 4
.LC103:
	.string	"incomplete universal-character-name"
	.align 4
.LC104:
	.string	"non-hex digit '%c' in universal-character-name"
	.align 4
.LC105:
	.string	"universal-character-name out of range"
	.text
	.type	maybe_read_ucs, @function
maybe_read_ucs:
.LFB54:
	.loc 1 1845 0
	pushl	%ebp
.LCFI141:
	movl	%esp, %ebp
.LCFI142:
	pushl	%esi
.LCFI143:
	pushl	%ebx
.LCFI144:
	subl	$48, %esp
.LCFI145:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 1846 0
	movl	12(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -24(%ebp)
	.loc 1 1847 0
	movl	$0, -20(%ebp)
	.loc 1 1848 0
	movl	20(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -16(%ebp)
	.loc 1 1851 0
	movl	8(%ebp), %eax
	movzbl	393(%eax), %eax
	testb	%al, %al
	jne	.L723
	movl	8(%ebp), %eax
	movzbl	423(%eax), %eax
	testb	%al, %al
	jne	.L723
	.loc 1 1852 0
	movl	$1, -32(%ebp)
	jmp	.L726
.L723:
	.loc 1 1854 0
	movl	8(%ebp), %eax
	movzbl	411(%eax), %eax
	testb	%al, %al
	je	.L727
	.loc 1 1855 0
	movl	-16(%ebp), %eax
	movl	%eax, 12(%esp)
	leal	.LC102@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$0, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L727:
	.loc 1 1858 0
	cmpl	$117, -16(%ebp)
	jne	.L729
	movl	$4, -28(%ebp)
	jmp	.L731
.L729:
	movl	$8, -28(%ebp)
.L731:
	movl	-28(%ebp), %eax
	movl	%eax, -12(%ebp)
	.loc 1 1860 0
	movl	16(%ebp), %edx
	movl	-24(%ebp), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	cmpl	-12(%ebp), %eax
	jae	.L735
	.loc 1 1862 0
	leal	.LC103@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$3, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	.loc 1 1864 0
	movl	16(%ebp), %eax
	movl	%eax, -24(%ebp)
	jmp	.L734
.L736:
	.loc 1 1870 0
	movl	-24(%ebp), %eax
	movzbl	(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, -16(%ebp)
	.loc 1 1871 0
	movzbl	-16(%ebp),%edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$256, %eax
	testl	%eax, %eax
	je	.L737
	.loc 1 1872 0
	movl	-20(%ebp), %eax
	movl	%eax, %esi
	sall	$4, %esi
	movl	-16(%ebp), %eax
	movl	%eax, (%esp)
	call	hex_digit_value
	leal	(%esi,%eax), %eax
	movl	%eax, -20(%ebp)
	.loc 1 1868 0
	subl	$1, -12(%ebp)
	addl	$1, -24(%ebp)
	jmp	.L735
.L737:
	.loc 1 1875 0
	movl	-16(%ebp), %eax
	movl	%eax, 12(%esp)
	leal	.LC104@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$3, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	.loc 1 1878 0
	jmp	.L734
.L735:
	.loc 1 1868 0
	cmpl	$0, -12(%ebp)
	jne	.L736
.L734:
	.loc 1 1888 0
	cmpl	$159, -20(%ebp)
	jbe	.L739
	movl	-20(%ebp), %eax
	testl	%eax, %eax
	js	.L739
	cmpl	$55295, -20(%ebp)
	jbe	.L744
	cmpl	$57343, -20(%ebp)
	jbe	.L739
	jmp	.L744
.L739:
	.loc 1 1894 0
	cmpl	$36, -20(%ebp)
	je	.L744
	cmpl	$64, -20(%ebp)
	je	.L744
	cmpl	$96, -20(%ebp)
	je	.L744
	.loc 1 1897 0
	cmpl	$0, -12(%ebp)
	jne	.L744
	.loc 1 1898 0
	leal	.LC105@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$3, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L744:
	.loc 1 1901 0
	movl	12(%ebp), %edx
	movl	-24(%ebp), %eax
	movl	%eax, (%edx)
	.loc 1 1902 0
	movl	20(%ebp), %edx
	movl	-20(%ebp), %eax
	movl	%eax, (%edx)
	.loc 1 1903 0
	movl	$0, -32(%ebp)
.L726:
	movl	-32(%ebp), %eax
	.loc 1 1904 0
	addl	$48, %esp
	popl	%ebx
	popl	%esi
	popl	%ebp
	ret
.LFE54:
	.size	maybe_read_ucs, .-maybe_read_ucs
	.section	.rodata
	.align 4
.LC106:
	.string	"the meaning of '\\a' is different in traditional C"
	.align 4
.LC107:
	.string	"non-ISO-standard escape sequence, '\\%c'"
	.align 4
.LC108:
	.string	"the meaning of '\\x' is different in traditional C"
	.align 4
.LC109:
	.string	"\\x used with no following hex digits"
	.align 4
.LC110:
	.string	"hex escape sequence out of range"
	.align 4
.LC111:
	.string	"octal escape sequence out of range"
.LC112:
	.string	"unknown escape sequence '\\%c'"
	.align 4
.LC113:
	.string	"unknown escape sequence: '\\%03o'"
	.align 4
.LC114:
	.string	"escape sequence out of range for its type"
	.text
.globl cpp_parse_escape
	.type	cpp_parse_escape, @function
cpp_parse_escape:
.LFB55:
	.loc 1 1917 0
	pushl	%ebp
.LCFI146:
	movl	%esp, %ebp
.LCFI147:
	pushl	%esi
.LCFI148:
	pushl	%ebx
.LCFI149:
	subl	$80, %esp
.LCFI150:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 1918 0
	movl	$0, -40(%ebp)
	.loc 1 1919 0
	movl	12(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -44(%ebp)
	.loc 1 1923 0
	cmpl	$0, 20(%ebp)
	je	.L751
	.loc 1 1924 0
	movl	8(%ebp), %eax
	movl	456(%eax), %eax
	movl	%eax, -32(%ebp)
	jmp	.L753
.L751:
	.loc 1 1926 0
	movl	8(%ebp), %eax
	movl	448(%eax), %eax
	movl	%eax, -32(%ebp)
.L753:
	.loc 1 1927 0
	cmpl	$31, -32(%ebp)
	ja	.L754
	.loc 1 1928 0
	movl	-32(%ebp), %ecx
	movl	$1, %eax
	sall	%cl, %eax
	subl	$1, %eax
	movl	%eax, -36(%ebp)
	jmp	.L756
.L754:
	.loc 1 1930 0
	movl	$-1, -36(%ebp)
.L756:
	.loc 1 1932 0
	movl	-44(%ebp), %edx
	movzbl	(%edx), %eax
	movzbl	%al, %eax
	movl	%eax, -48(%ebp)
	leal	1(%edx), %eax
	movl	%eax, -44(%ebp)
	.loc 1 1933 0
	movl	-48(%ebp), %eax
	subl	$34, %eax
	movl	%eax, -60(%ebp)
	cmpl	$89, -60(%ebp)
	ja	.L757
	movl	-60(%ebp), %eax
	sall	$2, %eax
	movl	.L771@GOTOFF(%eax,%ebx), %eax
	addl	%ebx, %eax
	jmp	*%eax
	.section	.rodata
	.align 4
	.align 4
.L771:
	.long	.L772@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L759@GOTOFF
	.long	.L757@GOTOFF
	.long	.L772@GOTOFF
	.long	.L759@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L760@GOTOFF
	.long	.L760@GOTOFF
	.long	.L760@GOTOFF
	.long	.L760@GOTOFF
	.long	.L760@GOTOFF
	.long	.L760@GOTOFF
	.long	.L760@GOTOFF
	.long	.L760@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L772@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L761@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L762@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L759@GOTOFF
	.long	.L772@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L763@GOTOFF
	.long	.L764@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L761@GOTOFF
	.long	.L765@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L766@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L767@GOTOFF
	.long	.L757@GOTOFF
	.long	.L768@GOTOFF
	.long	.L762@GOTOFF
	.long	.L769@GOTOFF
	.long	.L757@GOTOFF
	.long	.L770@GOTOFF
	.long	.L757@GOTOFF
	.long	.L757@GOTOFF
	.long	.L759@GOTOFF
	.text
.L764:
	.loc 1 1936 0
	movl	$8, -48(%ebp)
	jmp	.L772
.L765:
	.loc 1 1937 0
	movl	$12, -48(%ebp)
	jmp	.L772
.L766:
	.loc 1 1938 0
	movl	$10, -48(%ebp)
	jmp	.L772
.L767:
	.loc 1 1939 0
	movl	$13, -48(%ebp)
	jmp	.L772
.L768:
	.loc 1 1940 0
	movl	$9, -48(%ebp)
	jmp	.L772
.L769:
	.loc 1 1941 0
	movl	$11, -48(%ebp)
	jmp	.L772
.L759:
	.loc 1 1946 0
	movl	8(%ebp), %eax
	movzbl	425(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, -40(%ebp)
	.loc 1 1947 0
	jmp	.L772
.L763:
	.loc 1 1950 0
	movl	8(%ebp), %eax
	movzbl	411(%eax), %eax
	testb	%al, %al
	je	.L773
	.loc 1 1951 0
	leal	.LC106@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$0, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L773:
	.loc 1 1953 0
	movl	$7, -48(%ebp)
	.loc 1 1954 0
	jmp	.L772
.L761:
	.loc 1 1957 0
	movl	8(%ebp), %eax
	movzbl	425(%eax), %eax
	testb	%al, %al
	je	.L775
	.loc 1 1958 0
	movl	-48(%ebp), %eax
	movl	%eax, 12(%esp)
	leal	.LC107@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$2, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L775:
	.loc 1 1960 0
	movl	$27, -48(%ebp)
	.loc 1 1961 0
	jmp	.L772
.L762:
	.loc 1 1964 0
	leal	-48(%ebp), %eax
	movl	%eax, 12(%esp)
	movl	16(%ebp), %eax
	movl	%eax, 8(%esp)
	leal	-44(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	maybe_read_ucs
	movl	%eax, -40(%ebp)
	.loc 1 1965 0
	jmp	.L772
.L770:
	.loc 1 1968 0
	movl	8(%ebp), %eax
	movzbl	411(%eax), %eax
	testb	%al, %al
	je	.L777
	.loc 1 1969 0
	leal	.LC108@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$0, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L777:
.LBB21:
	.loc 1 1973 0
	movl	$0, -28(%ebp)
	movl	$0, -24(%ebp)
	.loc 1 1974 0
	movl	$0, -20(%ebp)
	.loc 1 1976 0
	jmp	.L779
.L780:
	.loc 1 1978 0
	movl	-44(%ebp), %eax
	movzbl	(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, -48(%ebp)
	.loc 1 1979 0
	movl	-48(%ebp), %eax
	movzbl	%al,%edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$256, %eax
	testl	%eax, %eax
	je	.L781
	.loc 1 1981 0
	movl	-44(%ebp), %eax
	addl	$1, %eax
	movl	%eax, -44(%ebp)
	.loc 1 1982 0
	movl	-28(%ebp), %eax
	andl	$268435455, %eax
	xorl	-28(%ebp), %eax
	orl	%eax, -24(%ebp)
	.loc 1 1983 0
	movl	-28(%ebp), %eax
	movl	%eax, %esi
	sall	$4, %esi
	movl	-48(%ebp), %eax
	movl	%eax, (%esp)
	call	hex_digit_value
	leal	(%esi,%eax), %eax
	movl	%eax, -28(%ebp)
	.loc 1 1984 0
	movl	$1, -20(%ebp)
.L779:
	.loc 1 1976 0
	movl	-44(%ebp), %eax
	cmpl	16(%ebp), %eax
	jb	.L780
.L781:
	.loc 1 1987 0
	cmpl	$0, -20(%ebp)
	jne	.L783
	.loc 1 1988 0
	leal	.LC109@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$3, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L783:
	.loc 1 1991 0
	movl	-36(%ebp), %eax
	andl	-28(%ebp), %eax
	cmpl	-28(%ebp), %eax
	setne	%al
	movzbl	%al, %eax
	orl	-24(%ebp), %eax
	testl	%eax, %eax
	je	.L785
	.loc 1 1993 0
	leal	.LC110@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$2, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	.loc 1 1995 0
	movl	-36(%ebp), %eax
	andl	%eax, -28(%ebp)
.L785:
	.loc 1 1997 0
	movl	-28(%ebp), %eax
	movl	%eax, -48(%ebp)
	.loc 1 1999 0
	jmp	.L772
.L760:
.LBE21:
.LBB22:
	.loc 1 2004 0
	movl	$0, -16(%ebp)
	.loc 1 2005 0
	movl	-48(%ebp), %eax
	subl	$48, %eax
	movl	%eax, -12(%ebp)
	.loc 1 2007 0
	jmp	.L787
.L788:
	.loc 1 2009 0
	movl	-44(%ebp), %eax
	movzbl	(%eax), %eax
	movzbl	%al, %eax
	movl	%eax, -48(%ebp)
	.loc 1 2010 0
	movl	-48(%ebp), %eax
	cmpl	$47, %eax
	jbe	.L789
	movl	-48(%ebp), %eax
	cmpl	$55, %eax
	ja	.L789
	.loc 1 2012 0
	movl	-44(%ebp), %eax
	addl	$1, %eax
	movl	%eax, -44(%ebp)
	.loc 1 2013 0
	movl	-12(%ebp), %eax
	leal	0(,%eax,8), %edx
	movl	-48(%ebp), %eax
	leal	(%edx,%eax), %eax
	subl	$48, %eax
	movl	%eax, -12(%ebp)
.L787:
	.loc 1 2007 0
	movl	-44(%ebp), %eax
	cmpl	16(%ebp), %eax
	jae	.L789
	addl	$1, -16(%ebp)
	cmpl	$2, -16(%ebp)
	jbe	.L788
.L789:
	.loc 1 2016 0
	movl	-36(%ebp), %eax
	andl	-12(%ebp), %eax
	cmpl	-12(%ebp), %eax
	je	.L793
	.loc 1 2018 0
	leal	.LC111@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$2, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	.loc 1 2020 0
	movl	-36(%ebp), %eax
	andl	%eax, -12(%ebp)
.L793:
	.loc 1 2022 0
	movl	-12(%ebp), %eax
	movl	%eax, -48(%ebp)
	.loc 1 2024 0
	jmp	.L772
.L757:
.LBE22:
	.loc 1 2027 0
	movl	$1, -40(%ebp)
.L772:
	.loc 1 2031 0
	cmpl	$0, -40(%ebp)
	je	.L795
	.loc 1 2033 0
	movl	-48(%ebp), %eax
	movzbl	%al,%edx
	movl	_sch_istable@GOT(%ebx), %eax
	movzwl	(%eax,%edx,2), %eax
	movzwl	%ax, %eax
	andl	$172, %eax
	testl	%eax, %eax
	je	.L797
	.loc 1 2034 0
	movl	-48(%ebp), %eax
	movl	%eax, 12(%esp)
	leal	.LC112@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$2, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	jmp	.L795
.L797:
	.loc 1 2037 0
	movl	-48(%ebp), %eax
	movl	%eax, 12(%esp)
	leal	.LC113@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$2, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L795:
	.loc 1 2041 0
	movl	-48(%ebp), %eax
	cmpl	-36(%ebp), %eax
	jbe	.L799
	.loc 1 2043 0
	leal	.LC114@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$2, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	.loc 1 2044 0
	movl	-48(%ebp), %eax
	andl	-36(%ebp), %eax
	movl	%eax, -48(%ebp)
.L799:
	.loc 1 2047 0
	movl	-44(%ebp), %edx
	movl	12(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 2048 0
	movl	-48(%ebp), %eax
	.loc 1 2049 0
	addl	$80, %esp
	popl	%ebx
	popl	%esi
	popl	%ebp
	ret
.LFE55:
	.size	cpp_parse_escape, .-cpp_parse_escape
	.section	.rodata
.LC115:
	.string	"empty character constant"
	.align 4
.LC116:
	.string	"character constant too long for its type"
	.align 4
.LC117:
	.string	"multi-character character constant"
	.text
.globl cpp_interpret_charconst
	.type	cpp_interpret_charconst, @function
cpp_interpret_charconst:
.LFB56:
	.loc 1 2062 0
	pushl	%ebp
.LCFI151:
	movl	%esp, %ebp
.LCFI152:
	pushl	%ebx
.LCFI153:
	subl	$68, %esp
.LCFI154:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 2063 0
	movl	12(%ebp), %eax
	movl	12(%eax), %eax
	movl	%eax, -40(%ebp)
	.loc 1 2064 0
	movl	12(%ebp), %eax
	movl	8(%eax), %eax
	movl	%eax, %edx
	movl	-40(%ebp), %eax
	leal	(%edx,%eax), %eax
	movl	%eax, -36(%ebp)
	.loc 1 2065 0
	movl	$0, -32(%ebp)
	.loc 1 2067 0
	movl	$0, -12(%ebp)
	.loc 1 2075 0
	movl	12(%ebp), %eax
	movzbl	6(%eax), %eax
	cmpb	$59, %al
	jne	.L803
	.loc 1 2077 0
	movl	8(%ebp), %eax
	movl	448(%eax), %eax
	movl	%eax, -28(%ebp)
	.loc 1 2078 0
	movl	8(%ebp), %eax
	movl	452(%eax), %eax
	movl	$0, %edx
	divl	-28(%ebp)
	movl	%eax, -24(%ebp)
	.loc 1 2079 0
	movl	8(%ebp), %eax
	movzbl	460(%eax), %eax
	movb	%al, -5(%ebp)
	jmp	.L805
.L803:
	.loc 1 2083 0
	movl	8(%ebp), %eax
	movl	456(%eax), %eax
	movl	%eax, -28(%ebp)
	.loc 1 2084 0
	movl	$1, -24(%ebp)
	.loc 1 2085 0
	movl	8(%ebp), %eax
	movzbl	461(%eax), %eax
	movb	%al, -5(%ebp)
.L805:
	.loc 1 2088 0
	cmpl	$31, -28(%ebp)
	ja	.L806
	.loc 1 2089 0
	movl	-28(%ebp), %ecx
	movl	$1, %eax
	sall	%cl, %eax
	subl	$1, %eax
	movl	%eax, -16(%ebp)
	jmp	.L831
.L806:
	.loc 1 2091 0
	movl	$-1, -16(%ebp)
	.loc 1 2093 0
	jmp	.L831
.L810:
	.loc 1 2112 0
	movl	-40(%ebp), %edx
	movzbl	(%edx), %eax
	movzbl	%al, %eax
	movl	%eax, -20(%ebp)
	leal	1(%edx), %eax
	movl	%eax, -40(%ebp)
	.loc 1 2115 0
	cmpl	$92, -20(%ebp)
	jne	.L811
	.loc 1 2116 0
	movl	12(%ebp), %eax
	movzbl	6(%eax), %eax
	cmpb	$60, %al
	sete	%al
	movzbl	%al, %eax
	movl	%eax, 12(%esp)
	movl	-36(%ebp), %eax
	movl	%eax, 8(%esp)
	leal	-40(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_parse_escape@PLT
	movl	%eax, -20(%ebp)
.L811:
	.loc 1 2123 0
	addl	$1, -32(%ebp)
	.loc 1 2126 0
	movl	-16(%ebp), %eax
	andl	%eax, -20(%ebp)
	.loc 1 2127 0
	cmpl	$31, -28(%ebp)
	ja	.L813
	.loc 1 2128 0
	movl	-28(%ebp), %ecx
	movl	-12(%ebp), %eax
	sall	%cl, %eax
	orl	-20(%ebp), %eax
	movl	%eax, -12(%ebp)
	jmp	.L809
.L813:
	.loc 1 2130 0
	movl	-20(%ebp), %eax
	movl	%eax, -12(%ebp)
.L809:
.L831:
	.loc 1 2093 0
	movl	-40(%ebp), %eax
	cmpl	-36(%ebp), %eax
	jb	.L810
	.loc 1 2133 0
	cmpl	$0, -32(%ebp)
	jne	.L816
	.loc 1 2134 0
	leal	.LC115@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$3, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	jmp	.L818
.L816:
	.loc 1 2135 0
	cmpl	$1, -32(%ebp)
	jbe	.L818
	.loc 1 2138 0
	movb	$0, -5(%ebp)
	.loc 1 2140 0
	movl	-32(%ebp), %eax
	cmpl	-24(%ebp), %eax
	jbe	.L820
	.loc 1 2142 0
	movl	-24(%ebp), %eax
	movl	%eax, -32(%ebp)
	.loc 1 2143 0
	leal	.LC116@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$0, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
	jmp	.L818
.L820:
	.loc 1 2146 0
	movl	8(%ebp), %eax
	movzbl	410(%eax), %eax
	testb	%al, %al
	je	.L818
	.loc 1 2147 0
	leal	.LC117@GOTOFF(%ebx), %eax
	movl	%eax, 8(%esp)
	movl	$0, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	cpp_error@PLT
.L818:
	.loc 1 2153 0
	cmpl	$1, -32(%ebp)
	jbe	.L823
	.loc 1 2154 0
	movl	-28(%ebp), %eax
	imull	-24(%ebp), %eax
	movl	%eax, -28(%ebp)
.L823:
	.loc 1 2155 0
	cmpl	$31, -28(%ebp)
	ja	.L825
	.loc 1 2157 0
	movl	-28(%ebp), %ecx
	movl	$1, %eax
	sall	%cl, %eax
	subl	$1, %eax
	movl	%eax, -16(%ebp)
	.loc 1 2158 0
	cmpb	$0, -5(%ebp)
	jne	.L827
	movl	-28(%ebp), %eax
	subl	$1, %eax
	movl	%eax, %ecx
	movl	$1, %eax
	sall	%cl, %eax
	andl	-12(%ebp), %eax
	testl	%eax, %eax
	jne	.L829
.L827:
	.loc 1 2159 0
	movl	-16(%ebp), %eax
	andl	%eax, -12(%ebp)
	.loc 1 2158 0
	jmp	.L825
.L829:
	.loc 1 2161 0
	movl	-16(%ebp), %eax
	notl	%eax
	orl	%eax, -12(%ebp)
.L825:
	.loc 1 2164 0
	movl	16(%ebp), %edx
	movl	-32(%ebp), %eax
	movl	%eax, (%edx)
	.loc 1 2165 0
	movzbl	-5(%ebp), %edx
	movl	20(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 2166 0
	movl	-12(%ebp), %eax
	.loc 1 2167 0
	addl	$68, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE56:
	.size	cpp_interpret_charconst, .-cpp_interpret_charconst
	.type	new_buff, @function
new_buff:
.LFB57:
	.loc 1 2189 0
	pushl	%ebp
.LCFI155:
	movl	%esp, %ebp
.LCFI156:
	pushl	%ebx
.LCFI157:
	subl	$20, %esp
.LCFI158:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 2193 0
	cmpl	$7999, 8(%ebp)
	ja	.L833
	.loc 1 2194 0
	movl	$8000, 8(%ebp)
.L833:
	.loc 1 2195 0
	movl	8(%ebp), %eax
	addl	$3, %eax
	andl	$-4, %eax
	movl	%eax, 8(%ebp)
	.loc 1 2197 0
	movl	8(%ebp), %eax
	addl	$16, %eax
	movl	%eax, (%esp)
	call	xmalloc@PLT
	movl	%eax, -8(%ebp)
	.loc 1 2198 0
	movl	8(%ebp), %eax
	addl	-8(%ebp), %eax
	movl	%eax, -12(%ebp)
	.loc 1 2199 0
	movl	-12(%ebp), %edx
	movl	-8(%ebp), %eax
	movl	%eax, 4(%edx)
	.loc 1 2200 0
	movl	-12(%ebp), %edx
	movl	-8(%ebp), %eax
	movl	%eax, 8(%edx)
	.loc 1 2201 0
	movl	8(%ebp), %eax
	movl	%eax, %edx
	addl	-8(%ebp), %edx
	movl	-12(%ebp), %eax
	movl	%edx, 12(%eax)
	.loc 1 2202 0
	movl	-12(%ebp), %eax
	movl	$0, (%eax)
	.loc 1 2203 0
	movl	-12(%ebp), %eax
	.loc 1 2204 0
	addl	$20, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE57:
	.size	new_buff, .-new_buff
.globl _cpp_release_buff
	.type	_cpp_release_buff, @function
_cpp_release_buff:
.LFB58:
	.loc 1 2211 0
	pushl	%ebp
.LCFI159:
	movl	%esp, %ebp
.LCFI160:
	subl	$16, %esp
.LCFI161:
	.loc 1 2212 0
	movl	12(%ebp), %eax
	movl	%eax, -4(%ebp)
	.loc 1 2214 0
	jmp	.L837
.L838:
	.loc 1 2215 0
	movl	-4(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -4(%ebp)
.L837:
	.loc 1 2214 0
	movl	-4(%ebp), %eax
	movl	(%eax), %eax
	testl	%eax, %eax
	jne	.L838
	.loc 1 2216 0
	movl	8(%ebp), %eax
	movl	68(%eax), %edx
	movl	-4(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 2217 0
	movl	8(%ebp), %edx
	movl	12(%ebp), %eax
	movl	%eax, 68(%edx)
	.loc 1 2218 0
	leave
	ret
.LFE58:
	.size	_cpp_release_buff, .-_cpp_release_buff
.globl _cpp_get_buff
	.type	_cpp_get_buff, @function
_cpp_get_buff:
.LFB59:
	.loc 1 2225 0
	pushl	%ebp
.LCFI162:
	movl	%esp, %ebp
.LCFI163:
	subl	$24, %esp
.LCFI164:
	.loc 1 2228 0
	movl	8(%ebp), %eax
	addl	$68, %eax
	movl	%eax, -8(%ebp)
.L842:
.LBB23:
	.loc 1 2232 0
	movl	-8(%ebp), %eax
	movl	(%eax), %eax
	testl	%eax, %eax
	jne	.L843
	.loc 1 2233 0
	movl	12(%ebp), %eax
	movl	%eax, (%esp)
	call	new_buff
	movl	%eax, -20(%ebp)
	jmp	.L845
.L843:
	.loc 1 2234 0
	movl	-8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -12(%ebp)
	.loc 1 2235 0
	movl	-12(%ebp), %eax
	movl	12(%eax), %eax
	movl	%eax, %edx
	movl	-12(%ebp), %eax
	movl	4(%eax), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	movl	%eax, -4(%ebp)
	.loc 1 2238 0
	movl	-4(%ebp), %eax
	cmpl	12(%ebp), %eax
	jb	.L846
	movl	12(%ebp), %edx
	movl	%edx, %eax
	addl	%eax, %eax
	addl	%edx, %eax
	shrl	%eax
	addl	$8000, %eax
	cmpl	-4(%ebp), %eax
	jae	.L848
.L846:
.LBE23:
	.loc 1 2228 0
	movl	-8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -8(%ebp)
	.loc 1 2240 0
	jmp	.L842
.L848:
	.loc 1 2242 0
	movl	-12(%ebp), %eax
	movl	(%eax), %edx
	movl	-8(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 2243 0
	movl	-12(%ebp), %eax
	movl	$0, (%eax)
	.loc 1 2244 0
	movl	-12(%ebp), %eax
	movl	4(%eax), %edx
	movl	-12(%ebp), %eax
	movl	%edx, 8(%eax)
	.loc 1 2245 0
	movl	-12(%ebp), %eax
	movl	%eax, -20(%ebp)
.L845:
	movl	-20(%ebp), %eax
	.loc 1 2246 0
	leave
	ret
.LFE59:
	.size	_cpp_get_buff, .-_cpp_get_buff
.globl _cpp_append_extend_buff
	.type	_cpp_append_extend_buff, @function
_cpp_append_extend_buff:
.LFB60:
	.loc 1 2257 0
	pushl	%ebp
.LCFI165:
	movl	%esp, %ebp
.LCFI166:
	pushl	%edi
.LCFI167:
	pushl	%esi
.LCFI168:
	pushl	%ebx
.LCFI169:
	subl	$28, %esp
.LCFI170:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 2258 0
	movl	12(%ebp), %eax
	movl	12(%eax), %eax
	movl	%eax, %edx
	movl	12(%ebp), %eax
	movl	8(%eax), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	addl	%eax, %eax
	addl	16(%ebp), %eax
	movl	%eax, -20(%ebp)
	.loc 1 2259 0
	movl	-20(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_get_buff@PLT
	movl	%eax, -16(%ebp)
	.loc 1 2261 0
	movl	12(%ebp), %edx
	movl	-16(%ebp), %eax
	movl	%eax, (%edx)
	.loc 1 2262 0
	movl	12(%ebp), %eax
	movl	12(%eax), %eax
	movl	%eax, %edx
	movl	12(%ebp), %eax
	movl	8(%eax), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	movl	%eax, %ecx
	movl	12(%ebp), %eax
	movl	8(%eax), %edx
	movl	-16(%ebp), %eax
	movl	4(%eax), %eax
	movl	%eax, %edi
	movl	%edx, %esi
	cld
	rep
	movsb
	.loc 1 2263 0
	movl	-16(%ebp), %eax
	.loc 1 2264 0
	addl	$28, %esp
	popl	%ebx
	popl	%esi
	popl	%edi
	popl	%ebp
	ret
.LFE60:
	.size	_cpp_append_extend_buff, .-_cpp_append_extend_buff
.globl _cpp_extend_buff
	.type	_cpp_extend_buff, @function
_cpp_extend_buff:
.LFB61:
	.loc 1 2276 0
	pushl	%ebp
.LCFI171:
	movl	%esp, %ebp
.LCFI172:
	pushl	%edi
.LCFI173:
	pushl	%esi
.LCFI174:
	pushl	%ebx
.LCFI175:
	subl	$28, %esp
.LCFI176:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 2277 0
	movl	12(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -20(%ebp)
	.loc 1 2278 0
	movl	-20(%ebp), %eax
	movl	12(%eax), %eax
	movl	%eax, %edx
	movl	-20(%ebp), %eax
	movl	8(%eax), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	addl	%eax, %eax
	addl	16(%ebp), %eax
	movl	%eax, -16(%ebp)
	.loc 1 2280 0
	movl	-16(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_get_buff@PLT
	movl	%eax, -24(%ebp)
	.loc 1 2281 0
	movl	-20(%ebp), %eax
	movl	12(%eax), %eax
	movl	%eax, %edx
	movl	-20(%ebp), %eax
	movl	8(%eax), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	movl	%eax, %ecx
	movl	-20(%ebp), %eax
	movl	8(%eax), %edx
	movl	-24(%ebp), %eax
	movl	4(%eax), %eax
	movl	%eax, %edi
	movl	%edx, %esi
	cld
	rep
	movsb
	.loc 1 2282 0
	movl	-24(%ebp), %edx
	movl	-20(%ebp), %eax
	movl	%eax, (%edx)
	.loc 1 2283 0
	movl	12(%ebp), %edx
	movl	-24(%ebp), %eax
	movl	%eax, (%edx)
	.loc 1 2284 0
	addl	$28, %esp
	popl	%ebx
	popl	%esi
	popl	%edi
	popl	%ebp
	ret
.LFE61:
	.size	_cpp_extend_buff, .-_cpp_extend_buff
.globl _cpp_free_buff
	.type	_cpp_free_buff, @function
_cpp_free_buff:
.LFB62:
	.loc 1 2290 0
	pushl	%ebp
.LCFI177:
	movl	%esp, %ebp
.LCFI178:
	pushl	%ebx
.LCFI179:
	subl	$20, %esp
.LCFI180:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 2293 0
	jmp	.L855
.L856:
	.loc 1 2295 0
	movl	8(%ebp), %eax
	movl	(%eax), %eax
	movl	%eax, -8(%ebp)
	.loc 1 2296 0
	movl	8(%ebp), %eax
	movl	4(%eax), %eax
	movl	%eax, (%esp)
	call	free@PLT
	.loc 1 2293 0
	movl	-8(%ebp), %eax
	movl	%eax, 8(%ebp)
.L855:
	cmpl	$0, 8(%ebp)
	jne	.L856
	.loc 1 2298 0
	addl	$20, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE62:
	.size	_cpp_free_buff, .-_cpp_free_buff
.globl _cpp_unaligned_alloc
	.type	_cpp_unaligned_alloc, @function
_cpp_unaligned_alloc:
.LFB63:
	.loc 1 2305 0
	pushl	%ebp
.LCFI181:
	movl	%esp, %ebp
.LCFI182:
	pushl	%ebx
.LCFI183:
	subl	$36, %esp
.LCFI184:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 2306 0
	movl	8(%ebp), %eax
	movl	64(%eax), %eax
	movl	%eax, -12(%ebp)
	.loc 1 2307 0
	movl	-12(%ebp), %eax
	movl	8(%eax), %eax
	movl	%eax, -8(%ebp)
	.loc 1 2309 0
	movl	-12(%ebp), %eax
	movl	12(%eax), %eax
	movl	%eax, %edx
	movl	-8(%ebp), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	cmpl	12(%ebp), %eax
	jae	.L860
	.loc 1 2311 0
	movl	12(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_get_buff@PLT
	movl	%eax, -12(%ebp)
	.loc 1 2312 0
	movl	8(%ebp), %eax
	movl	64(%eax), %edx
	movl	-12(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 2313 0
	movl	8(%ebp), %edx
	movl	-12(%ebp), %eax
	movl	%eax, 64(%edx)
	.loc 1 2314 0
	movl	-12(%ebp), %eax
	movl	8(%eax), %eax
	movl	%eax, -8(%ebp)
.L860:
	.loc 1 2317 0
	movl	12(%ebp), %eax
	movl	%eax, %edx
	addl	-8(%ebp), %edx
	movl	-12(%ebp), %eax
	movl	%edx, 8(%eax)
	.loc 1 2318 0
	movl	-8(%ebp), %eax
	.loc 1 2319 0
	addl	$36, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE63:
	.size	_cpp_unaligned_alloc, .-_cpp_unaligned_alloc
.globl _cpp_aligned_alloc
	.type	_cpp_aligned_alloc, @function
_cpp_aligned_alloc:
.LFB64:
	.loc 1 2335 0
	pushl	%ebp
.LCFI185:
	movl	%esp, %ebp
.LCFI186:
	pushl	%ebx
.LCFI187:
	subl	$36, %esp
.LCFI188:
	call	__i686.get_pc_thunk.bx
	addl	$_GLOBAL_OFFSET_TABLE_, %ebx
	.loc 1 2336 0
	movl	8(%ebp), %eax
	movl	60(%eax), %eax
	movl	%eax, -12(%ebp)
	.loc 1 2337 0
	movl	-12(%ebp), %eax
	movl	8(%eax), %eax
	movl	%eax, -8(%ebp)
	.loc 1 2339 0
	movl	-12(%ebp), %eax
	movl	12(%eax), %eax
	movl	%eax, %edx
	movl	-8(%ebp), %eax
	movl	%edx, %ecx
	subl	%eax, %ecx
	movl	%ecx, %eax
	cmpl	12(%ebp), %eax
	jae	.L864
	.loc 1 2341 0
	movl	12(%ebp), %eax
	movl	%eax, 4(%esp)
	movl	8(%ebp), %eax
	movl	%eax, (%esp)
	call	_cpp_get_buff@PLT
	movl	%eax, -12(%ebp)
	.loc 1 2342 0
	movl	8(%ebp), %eax
	movl	60(%eax), %edx
	movl	-12(%ebp), %eax
	movl	%edx, (%eax)
	.loc 1 2343 0
	movl	8(%ebp), %edx
	movl	-12(%ebp), %eax
	movl	%eax, 60(%edx)
	.loc 1 2344 0
	movl	-12(%ebp), %eax
	movl	8(%eax), %eax
	movl	%eax, -8(%ebp)
.L864:
	.loc 1 2347 0
	movl	12(%ebp), %eax
	movl	%eax, %edx
	addl	-8(%ebp), %edx
	movl	-12(%ebp), %eax
	movl	%edx, 8(%eax)
	.loc 1 2348 0
	movl	-8(%ebp), %eax
	.loc 1 2349 0
	addl	$36, %esp
	popl	%ebx
	popl	%ebp
	ret
.LFE64:
	.size	_cpp_aligned_alloc, .-_cpp_aligned_alloc
	.section	.debug_frame,"",@progbits
.Lframe0:
	.long	.LECIE0-.LSCIE0
.LSCIE0:
	.long	0xffffffff
	.byte	0x1
	.string	""
	.uleb128 0x1
	.sleb128 -4
	.byte	0x8
	.byte	0xc
	.uleb128 0x4
	.uleb128 0x4
	.byte	0x88
	.uleb128 0x1
	.align 4
.LECIE0:
.LSFDE0:
	.long	.LEFDE0-.LASFDE0
.LASFDE0:
	.long	.Lframe0
	.long	.LFB21
	.long	.LFE21-.LFB21
	.byte	0x4
	.long	.LCFI0-.LFB21
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI1-.LCFI0
	.byte	0xd
	.uleb128 0x5
	.align 4
.LEFDE0:
.LSFDE2:
	.long	.LEFDE2-.LASFDE2
.LASFDE2:
	.long	.Lframe0
	.long	.LFB15
	.long	.LFE15-.LFB15
	.byte	0x4
	.long	.LCFI3-.LFB15
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI4-.LCFI3
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI6-.LCFI4
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE2:
.LSFDE4:
	.long	.LEFDE4-.LASFDE4
.LASFDE4:
	.long	.Lframe0
	.long	.LFB22
	.long	.LFE22-.LFB22
	.byte	0x4
	.long	.LCFI7-.LFB22
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI8-.LCFI7
	.byte	0xd
	.uleb128 0x5
	.align 4
.LEFDE4:
.LSFDE6:
	.long	.LEFDE6-.LASFDE6
.LASFDE6:
	.long	.Lframe0
	.long	.LFB23
	.long	.LFE23-.LFB23
	.byte	0x4
	.long	.LCFI10-.LFB23
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI11-.LCFI10
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI15-.LCFI11
	.byte	0x83
	.uleb128 0x5
	.byte	0x86
	.uleb128 0x4
	.byte	0x87
	.uleb128 0x3
	.align 4
.LEFDE6:
.LSFDE8:
	.long	.LEFDE8-.LASFDE8
.LASFDE8:
	.long	.Lframe0
	.long	.LFB24
	.long	.LFE24-.LFB24
	.byte	0x4
	.long	.LCFI16-.LFB24
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI17-.LCFI16
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI19-.LCFI17
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE8:
.LSFDE10:
	.long	.LEFDE10-.LASFDE10
.LASFDE10:
	.long	.Lframe0
	.long	.LFB25
	.long	.LFE25-.LFB25
	.byte	0x4
	.long	.LCFI20-.LFB25
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI21-.LCFI20
	.byte	0xd
	.uleb128 0x5
	.align 4
.LEFDE10:
.LSFDE12:
	.long	.LEFDE12-.LASFDE12
.LASFDE12:
	.long	.Lframe0
	.long	.LFB26
	.long	.LFE26-.LFB26
	.byte	0x4
	.long	.LCFI23-.LFB26
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI24-.LCFI23
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI26-.LCFI24
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE12:
.LSFDE14:
	.long	.LEFDE14-.LASFDE14
.LASFDE14:
	.long	.Lframe0
	.long	.LFB27
	.long	.LFE27-.LFB27
	.byte	0x4
	.long	.LCFI27-.LFB27
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI28-.LCFI27
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI30-.LCFI28
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE14:
.LSFDE16:
	.long	.LEFDE16-.LASFDE16
.LASFDE16:
	.long	.Lframe0
	.long	.LFB28
	.long	.LFE28-.LFB28
	.byte	0x4
	.long	.LCFI31-.LFB28
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI32-.LCFI31
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI35-.LCFI32
	.byte	0x86
	.uleb128 0x4
	.byte	0x87
	.uleb128 0x3
	.align 4
.LEFDE16:
.LSFDE18:
	.long	.LEFDE18-.LASFDE18
.LASFDE18:
	.long	.Lframe0
	.long	.LFB29
	.long	.LFE29-.LFB29
	.byte	0x4
	.long	.LCFI36-.LFB29
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI37-.LCFI36
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI39-.LCFI37
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE18:
.LSFDE20:
	.long	.LEFDE20-.LASFDE20
.LASFDE20:
	.long	.Lframe0
	.long	.LFB30
	.long	.LFE30-.LFB30
	.byte	0x4
	.long	.LCFI40-.LFB30
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI41-.LCFI40
	.byte	0xd
	.uleb128 0x5
	.align 4
.LEFDE20:
.LSFDE22:
	.long	.LEFDE22-.LASFDE22
.LASFDE22:
	.long	.Lframe0
	.long	.LFB31
	.long	.LFE31-.LFB31
	.byte	0x4
	.long	.LCFI43-.LFB31
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI44-.LCFI43
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI46-.LCFI44
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE22:
.LSFDE24:
	.long	.LEFDE24-.LASFDE24
.LASFDE24:
	.long	.Lframe0
	.long	.LFB32
	.long	.LFE32-.LFB32
	.byte	0x4
	.long	.LCFI47-.LFB32
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI48-.LCFI47
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI52-.LCFI48
	.byte	0x83
	.uleb128 0x5
	.byte	0x86
	.uleb128 0x4
	.byte	0x87
	.uleb128 0x3
	.align 4
.LEFDE24:
.LSFDE26:
	.long	.LEFDE26-.LASFDE26
.LASFDE26:
	.long	.Lframe0
	.long	.LFB33
	.long	.LFE33-.LFB33
	.byte	0x4
	.long	.LCFI53-.LFB33
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI54-.LCFI53
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI58-.LCFI54
	.byte	0x83
	.uleb128 0x5
	.byte	0x86
	.uleb128 0x4
	.byte	0x87
	.uleb128 0x3
	.align 4
.LEFDE26:
.LSFDE28:
	.long	.LEFDE28-.LASFDE28
.LASFDE28:
	.long	.Lframe0
	.long	.LFB34
	.long	.LFE34-.LFB34
	.byte	0x4
	.long	.LCFI59-.LFB34
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI60-.LCFI59
	.byte	0xd
	.uleb128 0x5
	.align 4
.LEFDE28:
.LSFDE30:
	.long	.LEFDE30-.LASFDE30
.LASFDE30:
	.long	.Lframe0
	.long	.LFB35
	.long	.LFE35-.LFB35
	.byte	0x4
	.long	.LCFI62-.LFB35
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI63-.LCFI62
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI65-.LCFI63
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE30:
.LSFDE32:
	.long	.LEFDE32-.LASFDE32
.LASFDE32:
	.long	.Lframe0
	.long	.LFB36
	.long	.LFE36-.LFB36
	.byte	0x4
	.long	.LCFI66-.LFB36
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI67-.LCFI66
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI71-.LCFI67
	.byte	0x83
	.uleb128 0x5
	.byte	0x86
	.uleb128 0x4
	.byte	0x87
	.uleb128 0x3
	.align 4
.LEFDE32:
.LSFDE34:
	.long	.LEFDE34-.LASFDE34
.LASFDE34:
	.long	.Lframe0
	.long	.LFB37
	.long	.LFE37-.LFB37
	.byte	0x4
	.long	.LCFI72-.LFB37
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI73-.LCFI72
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI75-.LCFI73
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE34:
.LSFDE36:
	.long	.LEFDE36-.LASFDE36
.LASFDE36:
	.long	.Lframe0
	.long	.LFB38
	.long	.LFE38-.LFB38
	.byte	0x4
	.long	.LCFI76-.LFB38
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI77-.LCFI76
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI79-.LCFI77
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE36:
.LSFDE38:
	.long	.LEFDE38-.LASFDE38
.LASFDE38:
	.long	.Lframe0
	.long	.LFB39
	.long	.LFE39-.LFB39
	.byte	0x4
	.long	.LCFI80-.LFB39
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI81-.LCFI80
	.byte	0xd
	.uleb128 0x5
	.align 4
.LEFDE38:
.LSFDE40:
	.long	.LEFDE40-.LASFDE40
.LASFDE40:
	.long	.Lframe0
	.long	.LFB40
	.long	.LFE40-.LFB40
	.byte	0x4
	.long	.LCFI83-.LFB40
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI84-.LCFI83
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI88-.LCFI84
	.byte	0x83
	.uleb128 0x5
	.byte	0x86
	.uleb128 0x4
	.byte	0x87
	.uleb128 0x3
	.align 4
.LEFDE40:
.LSFDE42:
	.long	.LEFDE42-.LASFDE42
.LASFDE42:
	.long	.Lframe0
	.long	.LFB41
	.long	.LFE41-.LFB41
	.byte	0x4
	.long	.LCFI89-.LFB41
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI90-.LCFI89
	.byte	0xd
	.uleb128 0x5
	.align 4
.LEFDE42:
.LSFDE44:
	.long	.LEFDE44-.LASFDE44
.LASFDE44:
	.long	.Lframe0
	.long	.LFB42
	.long	.LFE42-.LFB42
	.byte	0x4
	.long	.LCFI92-.LFB42
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI93-.LCFI92
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI95-.LCFI93
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE44:
.LSFDE46:
	.long	.LEFDE46-.LASFDE46
.LASFDE46:
	.long	.Lframe0
	.long	.LFB43
	.long	.LFE43-.LFB43
	.byte	0x4
	.long	.LCFI96-.LFB43
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI97-.LCFI96
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI99-.LCFI97
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE46:
.LSFDE48:
	.long	.LEFDE48-.LASFDE48
.LASFDE48:
	.long	.Lframe0
	.long	.LFB44
	.long	.LFE44-.LFB44
	.byte	0x4
	.long	.LCFI100-.LFB44
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI101-.LCFI100
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI103-.LCFI101
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE48:
.LSFDE50:
	.long	.LEFDE50-.LASFDE50
.LASFDE50:
	.long	.Lframe0
	.long	.LFB45
	.long	.LFE45-.LFB45
	.byte	0x4
	.long	.LCFI104-.LFB45
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI105-.LCFI104
	.byte	0xd
	.uleb128 0x5
	.align 4
.LEFDE50:
.LSFDE52:
	.long	.LEFDE52-.LASFDE52
.LASFDE52:
	.long	.Lframe0
	.long	.LFB46
	.long	.LFE46-.LFB46
	.byte	0x4
	.long	.LCFI107-.LFB46
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI108-.LCFI107
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI112-.LCFI108
	.byte	0x83
	.uleb128 0x5
	.byte	0x86
	.uleb128 0x4
	.byte	0x87
	.uleb128 0x3
	.align 4
.LEFDE52:
.LSFDE54:
	.long	.LEFDE54-.LASFDE54
.LASFDE54:
	.long	.Lframe0
	.long	.LFB47
	.long	.LFE47-.LFB47
	.byte	0x4
	.long	.LCFI113-.LFB47
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI114-.LCFI113
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI116-.LCFI114
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE54:
.LSFDE56:
	.long	.LEFDE56-.LASFDE56
.LASFDE56:
	.long	.Lframe0
	.long	.LFB48
	.long	.LFE48-.LFB48
	.byte	0x4
	.long	.LCFI117-.LFB48
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI118-.LCFI117
	.byte	0xd
	.uleb128 0x5
	.align 4
.LEFDE56:
.LSFDE58:
	.long	.LEFDE58-.LASFDE58
.LASFDE58:
	.long	.Lframe0
	.long	.LFB49
	.long	.LFE49-.LFB49
	.byte	0x4
	.long	.LCFI119-.LFB49
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI120-.LCFI119
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI122-.LCFI120
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE58:
.LSFDE60:
	.long	.LEFDE60-.LASFDE60
.LASFDE60:
	.long	.Lframe0
	.long	.LFB50
	.long	.LFE50-.LFB50
	.byte	0x4
	.long	.LCFI123-.LFB50
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI124-.LCFI123
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI128-.LCFI124
	.byte	0x83
	.uleb128 0x5
	.byte	0x86
	.uleb128 0x4
	.byte	0x87
	.uleb128 0x3
	.align 4
.LEFDE60:
.LSFDE62:
	.long	.LEFDE62-.LASFDE62
.LASFDE62:
	.long	.Lframe0
	.long	.LFB51
	.long	.LFE51-.LFB51
	.byte	0x4
	.long	.LCFI129-.LFB51
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI130-.LCFI129
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI132-.LCFI130
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE62:
.LSFDE64:
	.long	.LEFDE64-.LASFDE64
.LASFDE64:
	.long	.Lframe0
	.long	.LFB52
	.long	.LFE52-.LFB52
	.byte	0x4
	.long	.LCFI133-.LFB52
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI134-.LCFI133
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI136-.LCFI134
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE64:
.LSFDE66:
	.long	.LEFDE66-.LASFDE66
.LASFDE66:
	.long	.Lframe0
	.long	.LFB53
	.long	.LFE53-.LFB53
	.byte	0x4
	.long	.LCFI137-.LFB53
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI138-.LCFI137
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI140-.LCFI138
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE66:
.LSFDE68:
	.long	.LEFDE68-.LASFDE68
.LASFDE68:
	.long	.Lframe0
	.long	.LFB54
	.long	.LFE54-.LFB54
	.byte	0x4
	.long	.LCFI141-.LFB54
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI142-.LCFI141
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI145-.LCFI142
	.byte	0x83
	.uleb128 0x4
	.byte	0x86
	.uleb128 0x3
	.align 4
.LEFDE68:
.LSFDE70:
	.long	.LEFDE70-.LASFDE70
.LASFDE70:
	.long	.Lframe0
	.long	.LFB55
	.long	.LFE55-.LFB55
	.byte	0x4
	.long	.LCFI146-.LFB55
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI147-.LCFI146
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI150-.LCFI147
	.byte	0x83
	.uleb128 0x4
	.byte	0x86
	.uleb128 0x3
	.align 4
.LEFDE70:
.LSFDE72:
	.long	.LEFDE72-.LASFDE72
.LASFDE72:
	.long	.Lframe0
	.long	.LFB56
	.long	.LFE56-.LFB56
	.byte	0x4
	.long	.LCFI151-.LFB56
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI152-.LCFI151
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI154-.LCFI152
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE72:
.LSFDE74:
	.long	.LEFDE74-.LASFDE74
.LASFDE74:
	.long	.Lframe0
	.long	.LFB57
	.long	.LFE57-.LFB57
	.byte	0x4
	.long	.LCFI155-.LFB57
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI156-.LCFI155
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI158-.LCFI156
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE74:
.LSFDE76:
	.long	.LEFDE76-.LASFDE76
.LASFDE76:
	.long	.Lframe0
	.long	.LFB58
	.long	.LFE58-.LFB58
	.byte	0x4
	.long	.LCFI159-.LFB58
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI160-.LCFI159
	.byte	0xd
	.uleb128 0x5
	.align 4
.LEFDE76:
.LSFDE78:
	.long	.LEFDE78-.LASFDE78
.LASFDE78:
	.long	.Lframe0
	.long	.LFB59
	.long	.LFE59-.LFB59
	.byte	0x4
	.long	.LCFI162-.LFB59
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI163-.LCFI162
	.byte	0xd
	.uleb128 0x5
	.align 4
.LEFDE78:
.LSFDE80:
	.long	.LEFDE80-.LASFDE80
.LASFDE80:
	.long	.Lframe0
	.long	.LFB60
	.long	.LFE60-.LFB60
	.byte	0x4
	.long	.LCFI165-.LFB60
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI166-.LCFI165
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI170-.LCFI166
	.byte	0x83
	.uleb128 0x5
	.byte	0x86
	.uleb128 0x4
	.byte	0x87
	.uleb128 0x3
	.align 4
.LEFDE80:
.LSFDE82:
	.long	.LEFDE82-.LASFDE82
.LASFDE82:
	.long	.Lframe0
	.long	.LFB61
	.long	.LFE61-.LFB61
	.byte	0x4
	.long	.LCFI171-.LFB61
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI172-.LCFI171
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI176-.LCFI172
	.byte	0x83
	.uleb128 0x5
	.byte	0x86
	.uleb128 0x4
	.byte	0x87
	.uleb128 0x3
	.align 4
.LEFDE82:
.LSFDE84:
	.long	.LEFDE84-.LASFDE84
.LASFDE84:
	.long	.Lframe0
	.long	.LFB62
	.long	.LFE62-.LFB62
	.byte	0x4
	.long	.LCFI177-.LFB62
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI178-.LCFI177
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI180-.LCFI178
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE84:
.LSFDE86:
	.long	.LEFDE86-.LASFDE86
.LASFDE86:
	.long	.Lframe0
	.long	.LFB63
	.long	.LFE63-.LFB63
	.byte	0x4
	.long	.LCFI181-.LFB63
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI182-.LCFI181
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI184-.LCFI182
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE86:
.LSFDE88:
	.long	.LEFDE88-.LASFDE88
.LASFDE88:
	.long	.Lframe0
	.long	.LFB64
	.long	.LFE64-.LFB64
	.byte	0x4
	.long	.LCFI185-.LFB64
	.byte	0xe
	.uleb128 0x8
	.byte	0x85
	.uleb128 0x2
	.byte	0x4
	.long	.LCFI186-.LCFI185
	.byte	0xd
	.uleb128 0x5
	.byte	0x4
	.long	.LCFI188-.LCFI186
	.byte	0x83
	.uleb128 0x3
	.align 4
.LEFDE88:
	.file 3 "../../../kg++fe/gnu/cpplib.h"
	.file 4 "../../../kg++fe/gnu/hashtable.h"
	.file 5 "/usr/include/sys/types.h"
	.file 6 "/usr/include/bits/types.h"
	.file 7 "../../../kg++fe/gnu/line-map.h"
	.file 8 "/usr/include/stdio.h"
	.file 9 "../../../include/gnu/obstack.h"
	.file 10 "/usr/lib/gcc/i486-linux-gnu/4.1.2/include/stddef.h"
	.file 11 "/usr/include/libio.h"
	.file 12 "../../../include/gnu/safe-ctype.h"
	.file 13 "../../../include/gnu/libiberty.h"
	.text
.Letext0:
	.section	.debug_loc,"",@progbits
.Ldebug_loc0:
.LLST0:
	.long	.LFB21-.Ltext0
	.long	.LCFI0-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI0-.Ltext0
	.long	.LCFI1-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI1-.Ltext0
	.long	.LFE21-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST1:
	.long	.LFB15-.Ltext0
	.long	.LCFI3-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI3-.Ltext0
	.long	.LCFI4-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI4-.Ltext0
	.long	.LFE15-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST2:
	.long	.LFB22-.Ltext0
	.long	.LCFI7-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI7-.Ltext0
	.long	.LCFI8-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI8-.Ltext0
	.long	.LFE22-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST3:
	.long	.LFB23-.Ltext0
	.long	.LCFI10-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI10-.Ltext0
	.long	.LCFI11-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI11-.Ltext0
	.long	.LFE23-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST4:
	.long	.LFB24-.Ltext0
	.long	.LCFI16-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI16-.Ltext0
	.long	.LCFI17-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI17-.Ltext0
	.long	.LFE24-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST5:
	.long	.LFB25-.Ltext0
	.long	.LCFI20-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI20-.Ltext0
	.long	.LCFI21-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI21-.Ltext0
	.long	.LFE25-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST6:
	.long	.LFB26-.Ltext0
	.long	.LCFI23-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI23-.Ltext0
	.long	.LCFI24-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI24-.Ltext0
	.long	.LFE26-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST7:
	.long	.LFB27-.Ltext0
	.long	.LCFI27-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI27-.Ltext0
	.long	.LCFI28-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI28-.Ltext0
	.long	.LFE27-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST8:
	.long	.LFB28-.Ltext0
	.long	.LCFI31-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI31-.Ltext0
	.long	.LCFI32-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI32-.Ltext0
	.long	.LFE28-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST9:
	.long	.LFB29-.Ltext0
	.long	.LCFI36-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI36-.Ltext0
	.long	.LCFI37-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI37-.Ltext0
	.long	.LFE29-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST10:
	.long	.LFB30-.Ltext0
	.long	.LCFI40-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI40-.Ltext0
	.long	.LCFI41-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI41-.Ltext0
	.long	.LFE30-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST11:
	.long	.LFB31-.Ltext0
	.long	.LCFI43-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI43-.Ltext0
	.long	.LCFI44-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI44-.Ltext0
	.long	.LFE31-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST12:
	.long	.LFB32-.Ltext0
	.long	.LCFI47-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI47-.Ltext0
	.long	.LCFI48-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI48-.Ltext0
	.long	.LFE32-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST13:
	.long	.LFB33-.Ltext0
	.long	.LCFI53-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI53-.Ltext0
	.long	.LCFI54-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI54-.Ltext0
	.long	.LFE33-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST14:
	.long	.LFB34-.Ltext0
	.long	.LCFI59-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI59-.Ltext0
	.long	.LCFI60-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI60-.Ltext0
	.long	.LFE34-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST15:
	.long	.LFB35-.Ltext0
	.long	.LCFI62-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI62-.Ltext0
	.long	.LCFI63-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI63-.Ltext0
	.long	.LFE35-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST16:
	.long	.LFB36-.Ltext0
	.long	.LCFI66-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI66-.Ltext0
	.long	.LCFI67-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI67-.Ltext0
	.long	.LFE36-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST17:
	.long	.LFB37-.Ltext0
	.long	.LCFI72-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI72-.Ltext0
	.long	.LCFI73-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI73-.Ltext0
	.long	.LFE37-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST18:
	.long	.LFB38-.Ltext0
	.long	.LCFI76-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI76-.Ltext0
	.long	.LCFI77-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI77-.Ltext0
	.long	.LFE38-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST19:
	.long	.LFB39-.Ltext0
	.long	.LCFI80-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI80-.Ltext0
	.long	.LCFI81-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI81-.Ltext0
	.long	.LFE39-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST20:
	.long	.LFB40-.Ltext0
	.long	.LCFI83-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI83-.Ltext0
	.long	.LCFI84-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI84-.Ltext0
	.long	.LFE40-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST21:
	.long	.LFB41-.Ltext0
	.long	.LCFI89-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI89-.Ltext0
	.long	.LCFI90-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI90-.Ltext0
	.long	.LFE41-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST22:
	.long	.LFB42-.Ltext0
	.long	.LCFI92-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI92-.Ltext0
	.long	.LCFI93-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI93-.Ltext0
	.long	.LFE42-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST23:
	.long	.LFB43-.Ltext0
	.long	.LCFI96-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI96-.Ltext0
	.long	.LCFI97-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI97-.Ltext0
	.long	.LFE43-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST24:
	.long	.LFB44-.Ltext0
	.long	.LCFI100-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI100-.Ltext0
	.long	.LCFI101-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI101-.Ltext0
	.long	.LFE44-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST25:
	.long	.LFB45-.Ltext0
	.long	.LCFI104-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI104-.Ltext0
	.long	.LCFI105-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI105-.Ltext0
	.long	.LFE45-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST26:
	.long	.LFB46-.Ltext0
	.long	.LCFI107-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI107-.Ltext0
	.long	.LCFI108-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI108-.Ltext0
	.long	.LFE46-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST27:
	.long	.LFB47-.Ltext0
	.long	.LCFI113-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI113-.Ltext0
	.long	.LCFI114-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI114-.Ltext0
	.long	.LFE47-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST28:
	.long	.LFB48-.Ltext0
	.long	.LCFI117-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI117-.Ltext0
	.long	.LCFI118-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI118-.Ltext0
	.long	.LFE48-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST29:
	.long	.LFB49-.Ltext0
	.long	.LCFI119-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI119-.Ltext0
	.long	.LCFI120-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI120-.Ltext0
	.long	.LFE49-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST30:
	.long	.LFB50-.Ltext0
	.long	.LCFI123-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI123-.Ltext0
	.long	.LCFI124-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI124-.Ltext0
	.long	.LFE50-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST31:
	.long	.LFB51-.Ltext0
	.long	.LCFI129-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI129-.Ltext0
	.long	.LCFI130-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI130-.Ltext0
	.long	.LFE51-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST32:
	.long	.LFB52-.Ltext0
	.long	.LCFI133-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI133-.Ltext0
	.long	.LCFI134-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI134-.Ltext0
	.long	.LFE52-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST33:
	.long	.LFB53-.Ltext0
	.long	.LCFI137-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI137-.Ltext0
	.long	.LCFI138-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI138-.Ltext0
	.long	.LFE53-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST34:
	.long	.LFB54-.Ltext0
	.long	.LCFI141-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI141-.Ltext0
	.long	.LCFI142-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI142-.Ltext0
	.long	.LFE54-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST35:
	.long	.LFB55-.Ltext0
	.long	.LCFI146-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI146-.Ltext0
	.long	.LCFI147-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI147-.Ltext0
	.long	.LFE55-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST36:
	.long	.LFB56-.Ltext0
	.long	.LCFI151-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI151-.Ltext0
	.long	.LCFI152-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI152-.Ltext0
	.long	.LFE56-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST37:
	.long	.LFB57-.Ltext0
	.long	.LCFI155-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI155-.Ltext0
	.long	.LCFI156-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI156-.Ltext0
	.long	.LFE57-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST38:
	.long	.LFB58-.Ltext0
	.long	.LCFI159-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI159-.Ltext0
	.long	.LCFI160-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI160-.Ltext0
	.long	.LFE58-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST39:
	.long	.LFB59-.Ltext0
	.long	.LCFI162-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI162-.Ltext0
	.long	.LCFI163-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI163-.Ltext0
	.long	.LFE59-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST40:
	.long	.LFB60-.Ltext0
	.long	.LCFI165-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI165-.Ltext0
	.long	.LCFI166-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI166-.Ltext0
	.long	.LFE60-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST41:
	.long	.LFB61-.Ltext0
	.long	.LCFI171-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI171-.Ltext0
	.long	.LCFI172-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI172-.Ltext0
	.long	.LFE61-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST42:
	.long	.LFB62-.Ltext0
	.long	.LCFI177-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI177-.Ltext0
	.long	.LCFI178-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI178-.Ltext0
	.long	.LFE62-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST43:
	.long	.LFB63-.Ltext0
	.long	.LCFI181-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI181-.Ltext0
	.long	.LCFI182-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI182-.Ltext0
	.long	.LFE63-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
.LLST44:
	.long	.LFB64-.Ltext0
	.long	.LCFI185-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 4
	.long	.LCFI185-.Ltext0
	.long	.LCFI186-.Ltext0
	.value	0x2
	.byte	0x74
	.sleb128 8
	.long	.LCFI186-.Ltext0
	.long	.LFE64-.Ltext0
	.value	0x2
	.byte	0x75
	.sleb128 8
	.long	0x0
	.long	0x0
	.section	.debug_info
	.long	0x3cbb
	.value	0x2
	.long	.Ldebug_abbrev0
	.byte	0x4
	.uleb128 0x1
	.long	.Ldebug_line0
	.long	.Letext0
	.long	.Ltext0
	.string	"GNU C 4.1.2 (Ubuntu 4.1.2-0ubuntu4)"
	.byte	0x1
	.string	"../../../kg++fe/gnu/cpplex.c"
	.string	"/usa/handong/simplnano/cmplr-open64-merge/targia32_sl1/g++fe/gnu"
	.uleb128 0x2
	.byte	0x4
	.long	0xa1
	.uleb128 0x3
	.long	0xa6
	.uleb128 0x4
	.string	"char"
	.byte	0x1
	.byte	0x8
	.uleb128 0x4
	.string	"int"
	.byte	0x4
	.byte	0x5
	.uleb128 0x5
	.long	.LASF0
	.byte	0x4
	.byte	0x7
	.uleb128 0x5
	.long	.LASF0
	.byte	0x4
	.byte	0x7
	.uleb128 0x6
	.string	"size_t"
	.byte	0xa
	.byte	0xd6
	.long	0xb5
	.uleb128 0x4
	.string	"unsigned char"
	.byte	0x1
	.byte	0x8
	.uleb128 0x4
	.string	"short unsigned int"
	.byte	0x2
	.byte	0x7
	.uleb128 0x4
	.string	"long unsigned int"
	.byte	0x4
	.byte	0x7
	.uleb128 0x4
	.string	"signed char"
	.byte	0x1
	.byte	0x6
	.uleb128 0x4
	.string	"short int"
	.byte	0x2
	.byte	0x5
	.uleb128 0x4
	.string	"long long int"
	.byte	0x8
	.byte	0x5
	.uleb128 0x4
	.string	"long long unsigned int"
	.byte	0x8
	.byte	0x7
	.uleb128 0x6
	.string	"__quad_t"
	.byte	0x6
	.byte	0x3b
	.long	0x129
	.uleb128 0x6
	.string	"__u_quad_t"
	.byte	0x6
	.byte	0x3c
	.long	0x13a
	.uleb128 0x6
	.string	"__dev_t"
	.byte	0x6
	.byte	0x89
	.long	0x164
	.uleb128 0x6
	.string	"__ino_t"
	.byte	0x6
	.byte	0x8c
	.long	0xf8
	.uleb128 0x6
	.string	"__off_t"
	.byte	0x6
	.byte	0x90
	.long	0x1a3
	.uleb128 0x4
	.string	"long int"
	.byte	0x4
	.byte	0x5
	.uleb128 0x6
	.string	"__off64_t"
	.byte	0x6
	.byte	0x91
	.long	0x154
	.uleb128 0x7
	.byte	0x4
	.uleb128 0x2
	.byte	0x4
	.long	0xa6
	.uleb128 0x6
	.string	"FILE"
	.byte	0x8
	.byte	0x2e
	.long	0x1d4
	.uleb128 0x8
	.long	0x44f
	.long	.LASF1
	.byte	0x94
	.byte	0x8
	.byte	0x2e
	.uleb128 0x9
	.string	"_flags"
	.byte	0xb
	.value	0x10c
	.long	0xae
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0x9
	.string	"_IO_read_ptr"
	.byte	0xb
	.value	0x111
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0x9
	.string	"_IO_read_end"
	.byte	0xb
	.value	0x112
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0x9
	.string	"_IO_read_base"
	.byte	0xb
	.value	0x113
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.uleb128 0x9
	.string	"_IO_write_base"
	.byte	0xb
	.value	0x114
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0x9
	.string	"_IO_write_ptr"
	.byte	0xb
	.value	0x115
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0x14
	.uleb128 0x9
	.string	"_IO_write_end"
	.byte	0xb
	.value	0x116
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0x18
	.uleb128 0x9
	.string	"_IO_buf_base"
	.byte	0xb
	.value	0x117
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0x1c
	.uleb128 0x9
	.string	"_IO_buf_end"
	.byte	0xb
	.value	0x118
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0x20
	.uleb128 0x9
	.string	"_IO_save_base"
	.byte	0xb
	.value	0x11a
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0x24
	.uleb128 0x9
	.string	"_IO_backup_base"
	.byte	0xb
	.value	0x11b
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0x28
	.uleb128 0x9
	.string	"_IO_save_end"
	.byte	0xb
	.value	0x11c
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0x2c
	.uleb128 0x9
	.string	"_markers"
	.byte	0xb
	.value	0x11e
	.long	0x4c7
	.byte	0x2
	.byte	0x23
	.uleb128 0x30
	.uleb128 0x9
	.string	"_chain"
	.byte	0xb
	.value	0x120
	.long	0x4cd
	.byte	0x2
	.byte	0x23
	.uleb128 0x34
	.uleb128 0x9
	.string	"_fileno"
	.byte	0xb
	.value	0x122
	.long	0xae
	.byte	0x2
	.byte	0x23
	.uleb128 0x38
	.uleb128 0x9
	.string	"_flags2"
	.byte	0xb
	.value	0x126
	.long	0xae
	.byte	0x2
	.byte	0x23
	.uleb128 0x3c
	.uleb128 0x9
	.string	"_old_offset"
	.byte	0xb
	.value	0x128
	.long	0x194
	.byte	0x2
	.byte	0x23
	.uleb128 0x40
	.uleb128 0x9
	.string	"_cur_column"
	.byte	0xb
	.value	0x12c
	.long	0xe2
	.byte	0x2
	.byte	0x23
	.uleb128 0x44
	.uleb128 0x9
	.string	"_vtable_offset"
	.byte	0xb
	.value	0x12d
	.long	0x10d
	.byte	0x2
	.byte	0x23
	.uleb128 0x46
	.uleb128 0x9
	.string	"_shortbuf"
	.byte	0xb
	.value	0x12e
	.long	0x4d3
	.byte	0x2
	.byte	0x23
	.uleb128 0x47
	.uleb128 0x9
	.string	"_lock"
	.byte	0xb
	.value	0x132
	.long	0x4e3
	.byte	0x2
	.byte	0x23
	.uleb128 0x48
	.uleb128 0x9
	.string	"_offset"
	.byte	0xb
	.value	0x13b
	.long	0x1af
	.byte	0x2
	.byte	0x23
	.uleb128 0x4c
	.uleb128 0x9
	.string	"__pad1"
	.byte	0xb
	.value	0x144
	.long	0x1c0
	.byte	0x2
	.byte	0x23
	.uleb128 0x54
	.uleb128 0x9
	.string	"__pad2"
	.byte	0xb
	.value	0x145
	.long	0x1c0
	.byte	0x2
	.byte	0x23
	.uleb128 0x58
	.uleb128 0x9
	.string	"__pad3"
	.byte	0xb
	.value	0x146
	.long	0x1c0
	.byte	0x2
	.byte	0x23
	.uleb128 0x5c
	.uleb128 0x9
	.string	"__pad4"
	.byte	0xb
	.value	0x147
	.long	0x1c0
	.byte	0x2
	.byte	0x23
	.uleb128 0x60
	.uleb128 0x9
	.string	"__pad5"
	.byte	0xb
	.value	0x148
	.long	0xc3
	.byte	0x2
	.byte	0x23
	.uleb128 0x64
	.uleb128 0x9
	.string	"_mode"
	.byte	0xb
	.value	0x14a
	.long	0xae
	.byte	0x2
	.byte	0x23
	.uleb128 0x68
	.uleb128 0x9
	.string	"_unused2"
	.byte	0xb
	.value	0x14c
	.long	0x4e9
	.byte	0x2
	.byte	0x23
	.uleb128 0x6c
	.byte	0x0
	.uleb128 0xa
	.long	0x45f
	.long	0xa6
	.uleb128 0xb
	.long	0xbc
	.byte	0x3
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x465
	.uleb128 0x2
	.byte	0x4
	.long	0x46b
	.uleb128 0x3
	.long	0xd1
	.uleb128 0x2
	.byte	0x4
	.long	0xd1
	.uleb128 0xc
	.string	"_IO_lock_t"
	.byte	0xb
	.byte	0xb0
	.uleb128 0xd
	.long	0x4c7
	.string	"_IO_marker"
	.byte	0xc
	.byte	0xb
	.byte	0xb6
	.uleb128 0xe
	.string	"_next"
	.byte	0xb
	.byte	0xb7
	.long	0x4c7
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"_sbuf"
	.byte	0xb
	.byte	0xb8
	.long	0x4cd
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xe
	.string	"_pos"
	.byte	0xb
	.byte	0xbc
	.long	0xae
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x484
	.uleb128 0x2
	.byte	0x4
	.long	0x1d4
	.uleb128 0xa
	.long	0x4e3
	.long	0xa6
	.uleb128 0xb
	.long	0xbc
	.byte	0x0
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x476
	.uleb128 0xa
	.long	0x4f9
	.long	0xa6
	.uleb128 0xb
	.long	0xbc
	.byte	0x27
	.byte	0x0
	.uleb128 0x6
	.string	"ino_t"
	.byte	0x5
	.byte	0x32
	.long	0x185
	.uleb128 0x6
	.string	"dev_t"
	.byte	0x5
	.byte	0x3e
	.long	0x176
	.uleb128 0x2
	.byte	0x4
	.long	0xae
	.uleb128 0xd
	.long	0x561
	.string	"_obstack_chunk"
	.byte	0xc
	.byte	0x9
	.byte	0xa2
	.uleb128 0xf
	.long	.LASF2
	.byte	0x9
	.byte	0xa3
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"prev"
	.byte	0x9
	.byte	0xa4
	.long	0x561
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xe
	.string	"contents"
	.byte	0x9
	.byte	0xa5
	.long	0x44f
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x519
	.uleb128 0x10
	.long	0x694
	.string	"obstack"
	.byte	0x2c
	.byte	0x8
	.value	0x31b
	.uleb128 0xe
	.string	"chunk_size"
	.byte	0x9
	.byte	0xaa
	.long	0x1a3
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"chunk"
	.byte	0x9
	.byte	0xab
	.long	0x561
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xe
	.string	"object_base"
	.byte	0x9
	.byte	0xac
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0xe
	.string	"next_free"
	.byte	0x9
	.byte	0xad
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.uleb128 0xe
	.string	"chunk_limit"
	.byte	0x9
	.byte	0xae
	.long	0x1c2
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0xe
	.string	"temp"
	.byte	0x9
	.byte	0xaf
	.long	0xae
	.byte	0x2
	.byte	0x23
	.uleb128 0x14
	.uleb128 0xe
	.string	"alignment_mask"
	.byte	0x9
	.byte	0xb0
	.long	0xae
	.byte	0x2
	.byte	0x23
	.uleb128 0x18
	.uleb128 0xe
	.string	"chunkfun"
	.byte	0x9
	.byte	0xb5
	.long	0x6a9
	.byte	0x2
	.byte	0x23
	.uleb128 0x1c
	.uleb128 0xe
	.string	"freefun"
	.byte	0x9
	.byte	0xb6
	.long	0x6c0
	.byte	0x2
	.byte	0x23
	.uleb128 0x20
	.uleb128 0xe
	.string	"extra_arg"
	.byte	0x9
	.byte	0xb7
	.long	0x1c0
	.byte	0x2
	.byte	0x23
	.uleb128 0x24
	.uleb128 0x11
	.string	"use_extra_arg"
	.byte	0x9
	.byte	0xbd
	.long	0xb5
	.byte	0x4
	.byte	0x1
	.byte	0x1f
	.byte	0x2
	.byte	0x23
	.uleb128 0x28
	.uleb128 0x11
	.string	"maybe_empty_object"
	.byte	0x9
	.byte	0xbe
	.long	0xb5
	.byte	0x4
	.byte	0x1
	.byte	0x1e
	.byte	0x2
	.byte	0x23
	.uleb128 0x28
	.uleb128 0x11
	.string	"alloc_failed"
	.byte	0x9
	.byte	0xc2
	.long	0xb5
	.byte	0x4
	.byte	0x1
	.byte	0x1d
	.byte	0x2
	.byte	0x23
	.uleb128 0x28
	.byte	0x0
	.uleb128 0x12
	.long	0x6a9
	.byte	0x1
	.long	0x561
	.uleb128 0x13
	.long	0x1c0
	.uleb128 0x13
	.long	0x1a3
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x694
	.uleb128 0x14
	.long	0x6c0
	.byte	0x1
	.uleb128 0x13
	.long	0x1c0
	.uleb128 0x13
	.long	0x561
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x6af
	.uleb128 0x8
	.long	0x704
	.long	.LASF3
	.byte	0xc
	.byte	0x4
	.byte	0x19
	.uleb128 0xe
	.string	"str"
	.byte	0x4
	.byte	0x1c
	.long	0x465
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"len"
	.byte	0x4
	.byte	0x1d
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xe
	.string	"hash_value"
	.byte	0x4
	.byte	0x1e
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.byte	0x0
	.uleb128 0x15
	.long	.LASF4
	.byte	0x4
	.byte	0x29
	.long	0x70f
	.uleb128 0xd
	.long	0x7ad
	.string	"ht"
	.byte	0x48
	.byte	0x4
	.byte	0x29
	.uleb128 0xe
	.string	"stack"
	.byte	0x4
	.byte	0x32
	.long	0x567
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"entries"
	.byte	0x4
	.byte	0x34
	.long	0x7c3
	.byte	0x2
	.byte	0x23
	.uleb128 0x2c
	.uleb128 0xe
	.string	"alloc_node"
	.byte	0x4
	.byte	0x36
	.long	0x7df
	.byte	0x2
	.byte	0x23
	.uleb128 0x30
	.uleb128 0xe
	.string	"nslots"
	.byte	0x4
	.byte	0x38
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x34
	.uleb128 0xe
	.string	"nelements"
	.byte	0x4
	.byte	0x39
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x38
	.uleb128 0xf
	.long	.LASF5
	.byte	0x4
	.byte	0x3c
	.long	0xbad
	.byte	0x2
	.byte	0x23
	.uleb128 0x3c
	.uleb128 0xe
	.string	"searches"
	.byte	0x4
	.byte	0x3f
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x40
	.uleb128 0xe
	.string	"collisions"
	.byte	0x4
	.byte	0x40
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x44
	.byte	0x0
	.uleb128 0x6
	.string	"hashnode"
	.byte	0x4
	.byte	0x2a
	.long	0x7bd
	.uleb128 0x2
	.byte	0x4
	.long	0x6c6
	.uleb128 0x2
	.byte	0x4
	.long	0x7ad
	.uleb128 0x12
	.long	0x7d9
	.byte	0x1
	.long	0x7ad
	.uleb128 0x13
	.long	0x7d9
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x704
	.uleb128 0x2
	.byte	0x4
	.long	0x7c9
	.uleb128 0x16
	.long	0xbad
	.long	.LASF6
	.value	0x210
	.byte	0x4
	.byte	0x28
	.uleb128 0x17
	.long	.LASF7
	.byte	0x2
	.value	0x151
	.long	0x235e
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0x9
	.string	"overlaid_buffer"
	.byte	0x2
	.value	0x154
	.long	0x235e
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0x9
	.string	"state"
	.byte	0x2
	.value	0x157
	.long	0x20d5
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0x17
	.long	.LASF8
	.byte	0x2
	.value	0x15a
	.long	0xc6d
	.byte	0x2
	.byte	0x23
	.uleb128 0x18
	.uleb128 0x9
	.string	"map"
	.byte	0x2
	.value	0x15b
	.long	0x1cf3
	.byte	0x2
	.byte	0x23
	.uleb128 0x30
	.uleb128 0x9
	.string	"line"
	.byte	0x2
	.value	0x15c
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x34
	.uleb128 0x9
	.string	"directive_line"
	.byte	0x2
	.value	0x15f
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x38
	.uleb128 0x9
	.string	"a_buff"
	.byte	0x2
	.value	0x162
	.long	0x20cf
	.byte	0x2
	.byte	0x23
	.uleb128 0x3c
	.uleb128 0x9
	.string	"u_buff"
	.byte	0x2
	.value	0x163
	.long	0x20cf
	.byte	0x2
	.byte	0x23
	.uleb128 0x40
	.uleb128 0x9
	.string	"free_buffs"
	.byte	0x2
	.value	0x164
	.long	0x20cf
	.byte	0x2
	.byte	0x23
	.uleb128 0x44
	.uleb128 0x9
	.string	"base_context"
	.byte	0x2
	.value	0x167
	.long	0x1ff5
	.byte	0x2
	.byte	0x23
	.uleb128 0x48
	.uleb128 0x9
	.string	"context"
	.byte	0x2
	.value	0x168
	.long	0x2364
	.byte	0x2
	.byte	0x23
	.uleb128 0x64
	.uleb128 0x17
	.long	.LASF9
	.byte	0x2
	.value	0x16b
	.long	0x2370
	.byte	0x2
	.byte	0x23
	.uleb128 0x68
	.uleb128 0x9
	.string	"next_include_file"
	.byte	0x2
	.value	0x170
	.long	0x238c
	.byte	0x2
	.byte	0x23
	.uleb128 0x6c
	.uleb128 0x9
	.string	"mi_cmacro"
	.byte	0x2
	.value	0x173
	.long	0x2398
	.byte	0x2
	.byte	0x23
	.uleb128 0x70
	.uleb128 0x9
	.string	"mi_ind_cmacro"
	.byte	0x2
	.value	0x174
	.long	0x2398
	.byte	0x2
	.byte	0x23
	.uleb128 0x74
	.uleb128 0x9
	.string	"mi_valid"
	.byte	0x2
	.value	0x175
	.long	0xcf1
	.byte	0x2
	.byte	0x23
	.uleb128 0x78
	.uleb128 0x9
	.string	"cur_token"
	.byte	0x2
	.value	0x178
	.long	0x1ee1
	.byte	0x2
	.byte	0x23
	.uleb128 0x7c
	.uleb128 0x9
	.string	"base_run"
	.byte	0x2
	.value	0x179
	.long	0x1f93
	.byte	0x3
	.byte	0x23
	.uleb128 0x80
	.uleb128 0x9
	.string	"cur_run"
	.byte	0x2
	.value	0x179
	.long	0x1fe4
	.byte	0x3
	.byte	0x23
	.uleb128 0x90
	.uleb128 0x9
	.string	"lookaheads"
	.byte	0x2
	.value	0x17a
	.long	0xb5
	.byte	0x3
	.byte	0x23
	.uleb128 0x94
	.uleb128 0x9
	.string	"keep_tokens"
	.byte	0x2
	.value	0x17d
	.long	0xb5
	.byte	0x3
	.byte	0x23
	.uleb128 0x98
	.uleb128 0x9
	.string	"errors"
	.byte	0x2
	.value	0x180
	.long	0xb5
	.byte	0x3
	.byte	0x23
	.uleb128 0x9c
	.uleb128 0x9
	.string	"macro_buffer"
	.byte	0x2
	.value	0x183
	.long	0x470
	.byte	0x3
	.byte	0x23
	.uleb128 0xa0
	.uleb128 0x9
	.string	"macro_buffer_len"
	.byte	0x2
	.value	0x184
	.long	0xb5
	.byte	0x3
	.byte	0x23
	.uleb128 0xa4
	.uleb128 0x9
	.string	"all_include_files"
	.byte	0x2
	.value	0x187
	.long	0x23b2
	.byte	0x3
	.byte	0x23
	.uleb128 0xa8
	.uleb128 0x9
	.string	"max_include_len"
	.byte	0x2
	.value	0x18b
	.long	0xb5
	.byte	0x3
	.byte	0x23
	.uleb128 0xac
	.uleb128 0x9
	.string	"first_unused_line"
	.byte	0x2
	.value	0x18e
	.long	0xb5
	.byte	0x3
	.byte	0x23
	.uleb128 0xb0
	.uleb128 0x9
	.string	"date"
	.byte	0x2
	.value	0x191
	.long	0x1ee7
	.byte	0x3
	.byte	0x23
	.uleb128 0xb4
	.uleb128 0x9
	.string	"time"
	.byte	0x2
	.value	0x192
	.long	0x1ee7
	.byte	0x3
	.byte	0x23
	.uleb128 0xb8
	.uleb128 0x9
	.string	"avoid_paste"
	.byte	0x2
	.value	0x195
	.long	0x1448
	.byte	0x3
	.byte	0x23
	.uleb128 0xbc
	.uleb128 0x9
	.string	"eof"
	.byte	0x2
	.value	0x196
	.long	0x1448
	.byte	0x3
	.byte	0x23
	.uleb128 0xcc
	.uleb128 0x9
	.string	"deps"
	.byte	0x2
	.value	0x199
	.long	0x23bf
	.byte	0x3
	.byte	0x23
	.uleb128 0xdc
	.uleb128 0x9
	.string	"hash_ob"
	.byte	0x2
	.value	0x19d
	.long	0x567
	.byte	0x3
	.byte	0x23
	.uleb128 0xe0
	.uleb128 0x9
	.string	"buffer_ob"
	.byte	0x2
	.value	0x1a1
	.long	0x567
	.byte	0x3
	.byte	0x23
	.uleb128 0x10c
	.uleb128 0x9
	.string	"pragmas"
	.byte	0x2
	.value	0x1a5
	.long	0x23d4
	.byte	0x3
	.byte	0x23
	.uleb128 0x138
	.uleb128 0x9
	.string	"cb"
	.byte	0x2
	.value	0x1a8
	.long	0x163d
	.byte	0x3
	.byte	0x23
	.uleb128 0x13c
	.uleb128 0x17
	.long	.LASF4
	.byte	0x2
	.value	0x1ab
	.long	0x23da
	.byte	0x3
	.byte	0x23
	.uleb128 0x15c
	.uleb128 0x9
	.string	"op_stack"
	.byte	0x2
	.value	0x1ae
	.long	0x23e5
	.byte	0x3
	.byte	0x23
	.uleb128 0x160
	.uleb128 0x9
	.string	"op_limit"
	.byte	0x2
	.value	0x1ae
	.long	0x23e5
	.byte	0x3
	.byte	0x23
	.uleb128 0x164
	.uleb128 0x9
	.string	"opts"
	.byte	0x2
	.value	0x1b1
	.long	0xe8e
	.byte	0x3
	.byte	0x23
	.uleb128 0x168
	.uleb128 0x17
	.long	.LASF10
	.byte	0x2
	.value	0x1b5
	.long	0x220a
	.byte	0x3
	.byte	0x23
	.uleb128 0x1d0
	.uleb128 0x9
	.string	"print"
	.byte	0x2
	.value	0x1b8
	.long	0x2268
	.byte	0x3
	.byte	0x23
	.uleb128 0x1e0
	.uleb128 0x9
	.string	"our_hashtable"
	.byte	0x2
	.value	0x1bb
	.long	0xd1
	.byte	0x3
	.byte	0x23
	.uleb128 0x1f8
	.uleb128 0x9
	.string	"out"
	.byte	0x2
	.value	0x1c4
	.long	0x230b
	.byte	0x3
	.byte	0x23
	.uleb128 0x1fc
	.uleb128 0x9
	.string	"saved_line"
	.byte	0x2
	.value	0x1c8
	.long	0xb5
	.byte	0x3
	.byte	0x23
	.uleb128 0x20c
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x7e5
	.uleb128 0x18
	.long	0xbe8
	.string	"lc_reason"
	.byte	0x4
	.byte	0x7
	.byte	0x1f
	.uleb128 0x19
	.string	"LC_ENTER"
	.sleb128 0
	.uleb128 0x19
	.string	"LC_LEAVE"
	.sleb128 1
	.uleb128 0x19
	.string	"LC_RENAME"
	.sleb128 2
	.byte	0x0
	.uleb128 0xd
	.long	0xc6d
	.string	"line_map"
	.byte	0x14
	.byte	0x7
	.byte	0x2a
	.uleb128 0xe
	.string	"to_file"
	.byte	0x7
	.byte	0x2b
	.long	0x9b
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"to_line"
	.byte	0x7
	.byte	0x2c
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xe
	.string	"from_line"
	.byte	0x7
	.byte	0x2d
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0xe
	.string	"included_from"
	.byte	0x7
	.byte	0x2e
	.long	0xae
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.uleb128 0x11
	.string	"reason"
	.byte	0x7
	.byte	0x2f
	.long	0xbb3
	.byte	0x4
	.byte	0x8
	.byte	0x18
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0xe
	.string	"sysp"
	.byte	0x7
	.byte	0x30
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x11
	.byte	0x0
	.uleb128 0x8
	.long	0xceb
	.long	.LASF8
	.byte	0x18
	.byte	0x7
	.byte	0x35
	.uleb128 0xe
	.string	"maps"
	.byte	0x7
	.byte	0x36
	.long	0xceb
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"allocated"
	.byte	0x7
	.byte	0x37
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xe
	.string	"used"
	.byte	0x7
	.byte	0x38
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0xe
	.string	"last_listed"
	.byte	0x7
	.byte	0x3d
	.long	0xae
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.uleb128 0xe
	.string	"depth"
	.byte	0x7
	.byte	0x40
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0xe
	.string	"trace_includes"
	.byte	0x7
	.byte	0x43
	.long	0xcf1
	.byte	0x2
	.byte	0x23
	.uleb128 0x14
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0xbe8
	.uleb128 0x4
	.string	"_Bool"
	.byte	0x1
	.byte	0x2
	.uleb128 0x15
	.long	.LASF6
	.byte	0x3
	.byte	0x28
	.long	0x7e5
	.uleb128 0x15
	.long	.LASF11
	.byte	0x3
	.byte	0x2a
	.long	0xd10
	.uleb128 0x8
	.long	0xe8e
	.long	.LASF11
	.byte	0x58
	.byte	0x3
	.byte	0x2a
	.uleb128 0x9
	.string	"cur"
	.byte	0x2
	.value	0x114
	.long	0x465
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0x9
	.string	"backup_to"
	.byte	0x2
	.value	0x115
	.long	0x465
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0x17
	.long	.LASF12
	.byte	0x2
	.value	0x116
	.long	0x465
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0x9
	.string	"line_base"
	.byte	0x2
	.value	0x117
	.long	0x465
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.uleb128 0x9
	.string	"prev"
	.byte	0x2
	.value	0x119
	.long	0x22e4
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0x9
	.string	"buf"
	.byte	0x2
	.value	0x11b
	.long	0x465
	.byte	0x2
	.byte	0x23
	.uleb128 0x14
	.uleb128 0x9
	.string	"inc"
	.byte	0x2
	.value	0x11f
	.long	0x22f9
	.byte	0x2
	.byte	0x23
	.uleb128 0x18
	.uleb128 0x17
	.long	.LASF13
	.byte	0x2
	.value	0x123
	.long	0x2305
	.byte	0x2
	.byte	0x23
	.uleb128 0x1c
	.uleb128 0x9
	.string	"col_adjust"
	.byte	0x2
	.value	0x126
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x20
	.uleb128 0x9
	.string	"saved_flags"
	.byte	0x2
	.value	0x129
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x24
	.uleb128 0x9
	.string	"last_Wtrigraphs"
	.byte	0x2
	.value	0x12d
	.long	0x465
	.byte	0x2
	.byte	0x23
	.uleb128 0x28
	.uleb128 0x9
	.string	"warned_cplusplus_comments"
	.byte	0x2
	.value	0x133
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x2c
	.uleb128 0x9
	.string	"from_stage3"
	.byte	0x2
	.value	0x138
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x2d
	.uleb128 0x9
	.string	"search_cached"
	.byte	0x2
	.value	0x13c
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x2e
	.uleb128 0x9
	.string	"return_at_eof"
	.byte	0x2
	.value	0x141
	.long	0xcf1
	.byte	0x2
	.byte	0x23
	.uleb128 0x2f
	.uleb128 0x9
	.string	"dir"
	.byte	0x2
	.value	0x145
	.long	0x1c21
	.byte	0x2
	.byte	0x23
	.uleb128 0x30
	.uleb128 0x17
	.long	.LASF14
	.byte	0x2
	.value	0x148
	.long	0x1ee7
	.byte	0x2
	.byte	0x23
	.uleb128 0x50
	.uleb128 0x9
	.string	"saved_rlimit"
	.byte	0x2
	.value	0x148
	.long	0x1ee7
	.byte	0x2
	.byte	0x23
	.uleb128 0x54
	.byte	0x0
	.uleb128 0x8
	.long	0x1448
	.long	.LASF15
	.byte	0x68
	.byte	0x3
	.byte	0x2b
	.uleb128 0xe
	.string	"tabstop"
	.byte	0x3
	.byte	0xe5
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"pending"
	.byte	0x3
	.byte	0xe8
	.long	0x1c1b
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xe
	.string	"quote_include"
	.byte	0x3
	.byte	0xeb
	.long	0x1c9f
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0xe
	.string	"bracket_include"
	.byte	0x3
	.byte	0xec
	.long	0x1c9f
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.uleb128 0xe
	.string	"map_list"
	.byte	0x3
	.byte	0xf0
	.long	0x1cba
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0xe
	.string	"include_prefix"
	.byte	0x3
	.byte	0xf4
	.long	0x9b
	.byte	0x2
	.byte	0x23
	.uleb128 0x14
	.uleb128 0xe
	.string	"include_prefix_len"
	.byte	0x3
	.byte	0xf5
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x18
	.uleb128 0xe
	.string	"lang"
	.byte	0x3
	.byte	0xf8
	.long	0x1a99
	.byte	0x2
	.byte	0x23
	.uleb128 0x1c
	.uleb128 0xe
	.string	"verbose"
	.byte	0x3
	.byte	0xfb
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x20
	.uleb128 0xe
	.string	"cplusplus"
	.byte	0x3
	.byte	0xfe
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x21
	.uleb128 0x9
	.string	"cplusplus_comments"
	.byte	0x3
	.value	0x101
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x22
	.uleb128 0x9
	.string	"objc"
	.byte	0x3
	.value	0x105
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x23
	.uleb128 0x9
	.string	"discard_comments"
	.byte	0x3
	.value	0x108
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x24
	.uleb128 0x9
	.string	"discard_comments_in_macro_exp"
	.byte	0x3
	.value	0x10c
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x25
	.uleb128 0x9
	.string	"trigraphs"
	.byte	0x3
	.value	0x10f
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x26
	.uleb128 0x9
	.string	"digraphs"
	.byte	0x3
	.value	0x112
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x27
	.uleb128 0x9
	.string	"extended_numbers"
	.byte	0x3
	.value	0x115
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x28
	.uleb128 0x9
	.string	"print_include_names"
	.byte	0x3
	.value	0x118
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x29
	.uleb128 0x9
	.string	"pedantic_errors"
	.byte	0x3
	.value	0x11b
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x2a
	.uleb128 0x9
	.string	"inhibit_warnings"
	.byte	0x3
	.value	0x11e
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x2b
	.uleb128 0x9
	.string	"warn_deprecated"
	.byte	0x3
	.value	0x121
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x2c
	.uleb128 0x9
	.string	"warn_system_headers"
	.byte	0x3
	.value	0x124
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x2d
	.uleb128 0x9
	.string	"inhibit_errors"
	.byte	0x3
	.value	0x128
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x2e
	.uleb128 0x9
	.string	"warn_comments"
	.byte	0x3
	.value	0x12b
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x2f
	.uleb128 0x9
	.string	"warn_trigraphs"
	.byte	0x3
	.value	0x12e
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x30
	.uleb128 0x9
	.string	"warn_import"
	.byte	0x3
	.value	0x131
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x31
	.uleb128 0x9
	.string	"warn_multichar"
	.byte	0x3
	.value	0x134
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x32
	.uleb128 0x9
	.string	"warn_traditional"
	.byte	0x3
	.value	0x138
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x33
	.uleb128 0x9
	.string	"warn_long_long"
	.byte	0x3
	.value	0x13b
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x34
	.uleb128 0x9
	.string	"warn_endif_labels"
	.byte	0x3
	.value	0x13e
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x35
	.uleb128 0x9
	.string	"warn_num_sign_change"
	.byte	0x3
	.value	0x142
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x36
	.uleb128 0x9
	.string	"warnings_are_errors"
	.byte	0x3
	.value	0x145
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x37
	.uleb128 0x9
	.string	"no_output"
	.byte	0x3
	.value	0x149
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x38
	.uleb128 0x9
	.string	"remap"
	.byte	0x3
	.value	0x14d
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x39
	.uleb128 0x9
	.string	"no_line_commands"
	.byte	0x3
	.value	0x150
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x3a
	.uleb128 0x9
	.string	"ignore_srcdir"
	.byte	0x3
	.value	0x154
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x3b
	.uleb128 0x9
	.string	"dollars_in_ident"
	.byte	0x3
	.value	0x157
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x3c
	.uleb128 0x9
	.string	"warn_undef"
	.byte	0x3
	.value	0x15a
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x3d
	.uleb128 0x9
	.string	"warn_unused_macros"
	.byte	0x3
	.value	0x15d
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x3e
	.uleb128 0x9
	.string	"c99"
	.byte	0x3
	.value	0x160
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x3f
	.uleb128 0x9
	.string	"std"
	.byte	0x3
	.value	0x163
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x40
	.uleb128 0x9
	.string	"pedantic"
	.byte	0x3
	.value	0x166
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x41
	.uleb128 0x9
	.string	"preprocessed"
	.byte	0x3
	.value	0x16a
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x42
	.uleb128 0x9
	.string	"no_standard_includes"
	.byte	0x3
	.value	0x16d
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x43
	.uleb128 0x9
	.string	"no_standard_cplusplus_includes"
	.byte	0x3
	.value	0x170
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x44
	.uleb128 0x9
	.string	"dump_macros"
	.byte	0x3
	.value	0x173
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x45
	.uleb128 0x9
	.string	"dump_includes"
	.byte	0x3
	.value	0x176
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x46
	.uleb128 0x9
	.string	"show_column"
	.byte	0x3
	.value	0x179
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x47
	.uleb128 0x9
	.string	"operator_names"
	.byte	0x3
	.value	0x17c
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x48
	.uleb128 0x9
	.string	"traditional"
	.byte	0x3
	.value	0x17f
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x49
	.uleb128 0x9
	.string	"deps"
	.byte	0x3
	.value	0x190
	.long	0x1ba4
	.byte	0x2
	.byte	0x23
	.uleb128 0x4c
	.uleb128 0x9
	.string	"precision"
	.byte	0x3
	.value	0x196
	.long	0xc3
	.byte	0x2
	.byte	0x23
	.uleb128 0x54
	.uleb128 0x9
	.string	"char_precision"
	.byte	0x3
	.value	0x196
	.long	0xc3
	.byte	0x2
	.byte	0x23
	.uleb128 0x58
	.uleb128 0x9
	.string	"int_precision"
	.byte	0x3
	.value	0x196
	.long	0xc3
	.byte	0x2
	.byte	0x23
	.uleb128 0x5c
	.uleb128 0x9
	.string	"wchar_precision"
	.byte	0x3
	.value	0x196
	.long	0xc3
	.byte	0x2
	.byte	0x23
	.uleb128 0x60
	.uleb128 0x9
	.string	"unsigned_char"
	.byte	0x3
	.value	0x199
	.long	0xcf1
	.byte	0x2
	.byte	0x23
	.uleb128 0x64
	.uleb128 0x9
	.string	"unsigned_wchar"
	.byte	0x3
	.value	0x199
	.long	0xcf1
	.byte	0x2
	.byte	0x23
	.uleb128 0x65
	.uleb128 0x9
	.string	"stdc_0_in_system_headers"
	.byte	0x3
	.value	0x19c
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x66
	.byte	0x0
	.uleb128 0x15
	.long	.LASF16
	.byte	0x3
	.byte	0x2c
	.long	0x1453
	.uleb128 0x8
	.long	0x14ad
	.long	.LASF16
	.byte	0x10
	.byte	0x3
	.byte	0x2c
	.uleb128 0xe
	.string	"line"
	.byte	0x3
	.byte	0xb6
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"col"
	.byte	0x3
	.byte	0xb7
	.long	0xe2
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0x11
	.string	"type"
	.byte	0x3
	.byte	0xb8
	.long	0x16f2
	.byte	0x4
	.byte	0x8
	.byte	0x8
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xe
	.string	"flags"
	.byte	0x3
	.byte	0xb9
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x7
	.uleb128 0xe
	.string	"val"
	.byte	0x3
	.byte	0xc2
	.long	0x1b0d
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.byte	0x0
	.uleb128 0x15
	.long	.LASF17
	.byte	0x3
	.byte	0x2d
	.long	0x14b8
	.uleb128 0x8
	.long	0x14e2
	.long	.LASF17
	.byte	0x8
	.byte	0x3
	.byte	0x2d
	.uleb128 0xe
	.string	"len"
	.byte	0x3
	.byte	0xa5
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"text"
	.byte	0x3
	.byte	0xa6
	.long	0x465
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.byte	0x0
	.uleb128 0x15
	.long	.LASF18
	.byte	0x3
	.byte	0x2e
	.long	0x14ed
	.uleb128 0x8
	.long	0x1584
	.long	.LASF18
	.byte	0x18
	.byte	0x3
	.byte	0x2e
	.uleb128 0x9
	.string	"ident"
	.byte	0x3
	.value	0x1e6
	.long	0x6c6
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0x9
	.string	"arg_index"
	.byte	0x3
	.value	0x1e7
	.long	0xe2
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.uleb128 0x9
	.string	"directive_index"
	.byte	0x3
	.value	0x1e8
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0xe
	.uleb128 0x9
	.string	"rid_code"
	.byte	0x3
	.value	0x1e9
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0xf
	.uleb128 0x1a
	.string	"type"
	.byte	0x3
	.value	0x1ea
	.long	0x1d91
	.byte	0x4
	.byte	0x8
	.byte	0x18
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0x9
	.string	"flags"
	.byte	0x3
	.value	0x1eb
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x11
	.uleb128 0x9
	.string	"value"
	.byte	0x3
	.value	0x1f7
	.long	0x1e44
	.byte	0x2
	.byte	0x23
	.uleb128 0x14
	.byte	0x0
	.uleb128 0x15
	.long	.LASF19
	.byte	0x3
	.byte	0x2f
	.long	0x158f
	.uleb128 0x8
	.long	0x163d
	.long	.LASF19
	.byte	0x14
	.byte	0x3
	.byte	0x2f
	.uleb128 0xe
	.string	"params"
	.byte	0x2
	.byte	0x49
	.long	0x1ef2
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"exp"
	.byte	0x2
	.byte	0x52
	.long	0x1ebe
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xe
	.string	"line"
	.byte	0x2
	.byte	0x55
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0xe
	.string	"count"
	.byte	0x2
	.byte	0x58
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.uleb128 0xe
	.string	"paramc"
	.byte	0x2
	.byte	0x5b
	.long	0xe2
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0x11
	.string	"fun_like"
	.byte	0x2
	.byte	0x5e
	.long	0xb5
	.byte	0x4
	.byte	0x1
	.byte	0xf
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0x11
	.string	"variadic"
	.byte	0x2
	.byte	0x61
	.long	0xb5
	.byte	0x4
	.byte	0x1
	.byte	0xe
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0x11
	.string	"syshdr"
	.byte	0x2
	.byte	0x64
	.long	0xb5
	.byte	0x4
	.byte	0x1
	.byte	0xd
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0x11
	.string	"used"
	.byte	0x2
	.byte	0x67
	.long	0xb5
	.byte	0x4
	.byte	0x1
	.byte	0xc
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.byte	0x0
	.uleb128 0x8
	.long	0x16f2
	.long	.LASF20
	.byte	0x20
	.byte	0x3
	.byte	0x30
	.uleb128 0x9
	.string	"line_change"
	.byte	0x3
	.value	0x1a3
	.long	0x1cdc
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0x9
	.string	"file_change"
	.byte	0x3
	.value	0x1a4
	.long	0x1cfe
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0x9
	.string	"include"
	.byte	0x3
	.value	0x1a5
	.long	0x1d1f
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0x9
	.string	"define"
	.byte	0x3
	.value	0x1a7
	.long	0x1d3b
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.uleb128 0x9
	.string	"undef"
	.byte	0x3
	.value	0x1a8
	.long	0x1d3b
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0x9
	.string	"ident"
	.byte	0x3
	.value	0x1a9
	.long	0x1d62
	.byte	0x2
	.byte	0x23
	.uleb128 0x14
	.uleb128 0x9
	.string	"def_pragma"
	.byte	0x3
	.value	0x1aa
	.long	0x1d79
	.byte	0x2
	.byte	0x23
	.uleb128 0x18
	.uleb128 0x9
	.string	"register_builtins"
	.byte	0x3
	.value	0x1ad
	.long	0x1d8b
	.byte	0x2
	.byte	0x23
	.uleb128 0x1c
	.byte	0x0
	.uleb128 0x18
	.long	0x1a99
	.string	"cpp_ttype"
	.byte	0x4
	.byte	0x3
	.byte	0x97
	.uleb128 0x19
	.string	"CPP_EQ"
	.sleb128 0
	.uleb128 0x19
	.string	"CPP_NOT"
	.sleb128 1
	.uleb128 0x19
	.string	"CPP_GREATER"
	.sleb128 2
	.uleb128 0x19
	.string	"CPP_LESS"
	.sleb128 3
	.uleb128 0x19
	.string	"CPP_PLUS"
	.sleb128 4
	.uleb128 0x19
	.string	"CPP_MINUS"
	.sleb128 5
	.uleb128 0x19
	.string	"CPP_MULT"
	.sleb128 6
	.uleb128 0x19
	.string	"CPP_DIV"
	.sleb128 7
	.uleb128 0x19
	.string	"CPP_MOD"
	.sleb128 8
	.uleb128 0x19
	.string	"CPP_AND"
	.sleb128 9
	.uleb128 0x19
	.string	"CPP_OR"
	.sleb128 10
	.uleb128 0x19
	.string	"CPP_XOR"
	.sleb128 11
	.uleb128 0x19
	.string	"CPP_RSHIFT"
	.sleb128 12
	.uleb128 0x19
	.string	"CPP_LSHIFT"
	.sleb128 13
	.uleb128 0x19
	.string	"CPP_MIN"
	.sleb128 14
	.uleb128 0x19
	.string	"CPP_MAX"
	.sleb128 15
	.uleb128 0x19
	.string	"CPP_COMPL"
	.sleb128 16
	.uleb128 0x19
	.string	"CPP_AND_AND"
	.sleb128 17
	.uleb128 0x19
	.string	"CPP_OR_OR"
	.sleb128 18
	.uleb128 0x19
	.string	"CPP_QUERY"
	.sleb128 19
	.uleb128 0x19
	.string	"CPP_COLON"
	.sleb128 20
	.uleb128 0x19
	.string	"CPP_COMMA"
	.sleb128 21
	.uleb128 0x19
	.string	"CPP_OPEN_PAREN"
	.sleb128 22
	.uleb128 0x19
	.string	"CPP_CLOSE_PAREN"
	.sleb128 23
	.uleb128 0x19
	.string	"CPP_EOF"
	.sleb128 24
	.uleb128 0x19
	.string	"CPP_EQ_EQ"
	.sleb128 25
	.uleb128 0x19
	.string	"CPP_NOT_EQ"
	.sleb128 26
	.uleb128 0x19
	.string	"CPP_GREATER_EQ"
	.sleb128 27
	.uleb128 0x19
	.string	"CPP_LESS_EQ"
	.sleb128 28
	.uleb128 0x19
	.string	"CPP_PLUS_EQ"
	.sleb128 29
	.uleb128 0x19
	.string	"CPP_MINUS_EQ"
	.sleb128 30
	.uleb128 0x19
	.string	"CPP_MULT_EQ"
	.sleb128 31
	.uleb128 0x19
	.string	"CPP_DIV_EQ"
	.sleb128 32
	.uleb128 0x19
	.string	"CPP_MOD_EQ"
	.sleb128 33
	.uleb128 0x19
	.string	"CPP_AND_EQ"
	.sleb128 34
	.uleb128 0x19
	.string	"CPP_OR_EQ"
	.sleb128 35
	.uleb128 0x19
	.string	"CPP_XOR_EQ"
	.sleb128 36
	.uleb128 0x19
	.string	"CPP_RSHIFT_EQ"
	.sleb128 37
	.uleb128 0x19
	.string	"CPP_LSHIFT_EQ"
	.sleb128 38
	.uleb128 0x19
	.string	"CPP_MIN_EQ"
	.sleb128 39
	.uleb128 0x19
	.string	"CPP_MAX_EQ"
	.sleb128 40
	.uleb128 0x19
	.string	"CPP_HASH"
	.sleb128 41
	.uleb128 0x19
	.string	"CPP_PASTE"
	.sleb128 42
	.uleb128 0x19
	.string	"CPP_OPEN_SQUARE"
	.sleb128 43
	.uleb128 0x19
	.string	"CPP_CLOSE_SQUARE"
	.sleb128 44
	.uleb128 0x19
	.string	"CPP_OPEN_BRACE"
	.sleb128 45
	.uleb128 0x19
	.string	"CPP_CLOSE_BRACE"
	.sleb128 46
	.uleb128 0x19
	.string	"CPP_SEMICOLON"
	.sleb128 47
	.uleb128 0x19
	.string	"CPP_ELLIPSIS"
	.sleb128 48
	.uleb128 0x19
	.string	"CPP_PLUS_PLUS"
	.sleb128 49
	.uleb128 0x19
	.string	"CPP_MINUS_MINUS"
	.sleb128 50
	.uleb128 0x19
	.string	"CPP_DEREF"
	.sleb128 51
	.uleb128 0x19
	.string	"CPP_DOT"
	.sleb128 52
	.uleb128 0x19
	.string	"CPP_SCOPE"
	.sleb128 53
	.uleb128 0x19
	.string	"CPP_DEREF_STAR"
	.sleb128 54
	.uleb128 0x19
	.string	"CPP_DOT_STAR"
	.sleb128 55
	.uleb128 0x19
	.string	"CPP_ATSIGN"
	.sleb128 56
	.uleb128 0x19
	.string	"CPP_NAME"
	.sleb128 57
	.uleb128 0x19
	.string	"CPP_NUMBER"
	.sleb128 58
	.uleb128 0x19
	.string	"CPP_CHAR"
	.sleb128 59
	.uleb128 0x19
	.string	"CPP_WCHAR"
	.sleb128 60
	.uleb128 0x19
	.string	"CPP_OTHER"
	.sleb128 61
	.uleb128 0x19
	.string	"CPP_STRING"
	.sleb128 62
	.uleb128 0x19
	.string	"CPP_WSTRING"
	.sleb128 63
	.uleb128 0x19
	.string	"CPP_HEADER_NAME"
	.sleb128 64
	.uleb128 0x19
	.string	"CPP_COMMENT"
	.sleb128 65
	.uleb128 0x19
	.string	"CPP_MACRO_ARG"
	.sleb128 66
	.uleb128 0x19
	.string	"CPP_PADDING"
	.sleb128 67
	.uleb128 0x19
	.string	"N_TTYPES"
	.sleb128 68
	.byte	0x0
	.uleb128 0x18
	.long	0x1b0d
	.string	"c_lang"
	.byte	0x4
	.byte	0x3
	.byte	0x9f
	.uleb128 0x19
	.string	"CLK_GNUC89"
	.sleb128 0
	.uleb128 0x19
	.string	"CLK_GNUC99"
	.sleb128 1
	.uleb128 0x19
	.string	"CLK_STDC89"
	.sleb128 2
	.uleb128 0x19
	.string	"CLK_STDC94"
	.sleb128 3
	.uleb128 0x19
	.string	"CLK_STDC99"
	.sleb128 4
	.uleb128 0x19
	.string	"CLK_GNUCXX"
	.sleb128 5
	.uleb128 0x19
	.string	"CLK_CXX98"
	.sleb128 6
	.uleb128 0x19
	.string	"CLK_ASM"
	.sleb128 7
	.byte	0x0
	.uleb128 0x1b
	.long	0x1b52
	.byte	0x8
	.byte	0x3
	.byte	0xbc
	.uleb128 0x1c
	.string	"node"
	.byte	0x3
	.byte	0xbd
	.long	0x1b52
	.uleb128 0x1c
	.string	"source"
	.byte	0x3
	.byte	0xbe
	.long	0x1b58
	.uleb128 0x1c
	.string	"str"
	.byte	0x3
	.byte	0xbf
	.long	0x14b8
	.uleb128 0x1c
	.string	"arg_no"
	.byte	0x3
	.byte	0xc0
	.long	0xb5
	.uleb128 0x1c
	.string	"c"
	.byte	0x3
	.byte	0xc1
	.long	0xd1
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x14e2
	.uleb128 0x2
	.byte	0x4
	.long	0x1b5e
	.uleb128 0x3
	.long	0x1448
	.uleb128 0x6
	.string	"cppchar_t"
	.byte	0x3
	.byte	0xd4
	.long	0xb5
	.uleb128 0x1d
	.long	0x1ba4
	.byte	0x4
	.byte	0x3
	.value	0x185
	.uleb128 0x19
	.string	"DEPS_NONE"
	.sleb128 0
	.uleb128 0x19
	.string	"DEPS_USER"
	.sleb128 1
	.uleb128 0x19
	.string	"DEPS_SYSTEM"
	.sleb128 2
	.byte	0x0
	.uleb128 0x1e
	.long	0x1c0d
	.byte	0x8
	.byte	0x3
	.value	0x183
	.uleb128 0x9
	.string	"style"
	.byte	0x3
	.value	0x185
	.long	0x1b74
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0x9
	.string	"missing_files"
	.byte	0x3
	.value	0x188
	.long	0xcf1
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0x9
	.string	"phony_targets"
	.byte	0x3
	.value	0x18c
	.long	0xcf1
	.byte	0x2
	.byte	0x23
	.uleb128 0x5
	.uleb128 0x9
	.string	"ignore_main_file"
	.byte	0x3
	.value	0x18f
	.long	0xcf1
	.byte	0x2
	.byte	0x23
	.uleb128 0x6
	.byte	0x0
	.uleb128 0x1f
	.string	"cpp_pending"
	.byte	0x1
	.uleb128 0x2
	.byte	0x4
	.long	0x1c0d
	.uleb128 0xd
	.long	0x1c9f
	.string	"search_path"
	.byte	0x20
	.byte	0x3
	.byte	0xeb
	.uleb128 0xf
	.long	.LASF21
	.byte	0x2
	.byte	0x86
	.long	0x1c9f
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"name"
	.byte	0x2
	.byte	0x8a
	.long	0x9b
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xe
	.string	"len"
	.byte	0x2
	.byte	0x8b
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0xe
	.string	"ino"
	.byte	0x2
	.byte	0x8e
	.long	0x4f9
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.uleb128 0xe
	.string	"dev"
	.byte	0x2
	.byte	0x8f
	.long	0x506
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0xe
	.string	"sysp"
	.byte	0x2
	.byte	0x91
	.long	0xae
	.byte	0x2
	.byte	0x23
	.uleb128 0x18
	.uleb128 0xe
	.string	"name_map"
	.byte	0x2
	.byte	0x94
	.long	0x1f5e
	.byte	0x2
	.byte	0x23
	.uleb128 0x1c
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1c21
	.uleb128 0x1f
	.string	"file_name_map_list"
	.byte	0x1
	.uleb128 0x2
	.byte	0x4
	.long	0x1ca5
	.uleb128 0x14
	.long	0x1cd6
	.byte	0x1
	.uleb128 0x13
	.long	0x1cd6
	.uleb128 0x13
	.long	0x1b58
	.uleb128 0x13
	.long	0xae
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0xcfa
	.uleb128 0x2
	.byte	0x4
	.long	0x1cc0
	.uleb128 0x14
	.long	0x1cf3
	.byte	0x1
	.uleb128 0x13
	.long	0x1cd6
	.uleb128 0x13
	.long	0x1cf3
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1cf9
	.uleb128 0x3
	.long	0xbe8
	.uleb128 0x2
	.byte	0x4
	.long	0x1ce2
	.uleb128 0x14
	.long	0x1d1f
	.byte	0x1
	.uleb128 0x13
	.long	0x1cd6
	.uleb128 0x13
	.long	0xb5
	.uleb128 0x13
	.long	0x465
	.uleb128 0x13
	.long	0x1b58
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1d04
	.uleb128 0x14
	.long	0x1d3b
	.byte	0x1
	.uleb128 0x13
	.long	0x1cd6
	.uleb128 0x13
	.long	0xb5
	.uleb128 0x13
	.long	0x1b52
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1d25
	.uleb128 0x14
	.long	0x1d57
	.byte	0x1
	.uleb128 0x13
	.long	0x1cd6
	.uleb128 0x13
	.long	0xb5
	.uleb128 0x13
	.long	0x1d57
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1d5d
	.uleb128 0x3
	.long	0x14ad
	.uleb128 0x2
	.byte	0x4
	.long	0x1d41
	.uleb128 0x14
	.long	0x1d79
	.byte	0x1
	.uleb128 0x13
	.long	0x1cd6
	.uleb128 0x13
	.long	0xb5
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1d68
	.uleb128 0x14
	.long	0x1d8b
	.byte	0x1
	.uleb128 0x13
	.long	0x1cd6
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1d7f
	.uleb128 0x20
	.long	0x1dc9
	.string	"node_type"
	.byte	0x4
	.byte	0x3
	.value	0x1c8
	.uleb128 0x19
	.string	"NT_VOID"
	.sleb128 0
	.uleb128 0x19
	.string	"NT_MACRO"
	.sleb128 1
	.uleb128 0x19
	.string	"NT_ASSERTION"
	.sleb128 2
	.byte	0x0
	.uleb128 0x20
	.long	0x1e44
	.string	"builtin_type"
	.byte	0x4
	.byte	0x3
	.value	0x1d1
	.uleb128 0x19
	.string	"BT_SPECLINE"
	.sleb128 0
	.uleb128 0x19
	.string	"BT_DATE"
	.sleb128 1
	.uleb128 0x19
	.string	"BT_FILE"
	.sleb128 2
	.uleb128 0x19
	.string	"BT_BASE_FILE"
	.sleb128 3
	.uleb128 0x19
	.string	"BT_INCLUDE_LEVEL"
	.sleb128 4
	.uleb128 0x19
	.string	"BT_TIME"
	.sleb128 5
	.uleb128 0x19
	.string	"BT_STDC"
	.sleb128 6
	.uleb128 0x19
	.string	"BT_PRAGMA"
	.sleb128 7
	.byte	0x0
	.uleb128 0x21
	.long	0x1e92
	.byte	0x4
	.byte	0x3
	.value	0x1ee
	.uleb128 0x22
	.string	"macro"
	.byte	0x3
	.value	0x1ef
	.long	0x1e92
	.uleb128 0x22
	.string	"answers"
	.byte	0x3
	.value	0x1f0
	.long	0x1ea1
	.uleb128 0x22
	.string	"operator_code"
	.byte	0x3
	.value	0x1f2
	.long	0x16f2
	.uleb128 0x22
	.string	"builtin"
	.byte	0x3
	.value	0x1f6
	.long	0x1dc9
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1584
	.uleb128 0x1f
	.string	"answer"
	.byte	0x1
	.uleb128 0x2
	.byte	0x4
	.long	0x1e98
	.uleb128 0x6
	.string	"uchar"
	.byte	0x2
	.byte	0x20
	.long	0xd1
	.uleb128 0x4
	.string	"double"
	.byte	0x8
	.byte	0x4
	.uleb128 0x1b
	.long	0x1ee1
	.byte	0x4
	.byte	0x2
	.byte	0x4f
	.uleb128 0x1c
	.string	"tokens"
	.byte	0x2
	.byte	0x50
	.long	0x1ee1
	.uleb128 0x1c
	.string	"text"
	.byte	0x2
	.byte	0x51
	.long	0x1ee7
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1448
	.uleb128 0x2
	.byte	0x4
	.long	0x1eed
	.uleb128 0x3
	.long	0x1ea7
	.uleb128 0x2
	.byte	0x4
	.long	0x1b52
	.uleb128 0x15
	.long	.LASF22
	.byte	0x2
	.byte	0x6f
	.long	0x1f03
	.uleb128 0x8
	.long	0x1f48
	.long	.LASF22
	.byte	0x10
	.byte	0x2
	.byte	0x6f
	.uleb128 0xf
	.long	.LASF21
	.byte	0x2
	.byte	0x72
	.long	0x1f48
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xf
	.long	.LASF23
	.byte	0x2
	.byte	0x73
	.long	0x470
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xe
	.string	"cur"
	.byte	0x2
	.byte	0x73
	.long	0x470
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0xf
	.long	.LASF2
	.byte	0x2
	.byte	0x73
	.long	0x470
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1f03
	.uleb128 0x1f
	.string	"file_name_map"
	.byte	0x1
	.uleb128 0x2
	.byte	0x4
	.long	0x1f4e
	.uleb128 0x23
	.long	0x1f8d
	.string	"utoken"
	.byte	0x4
	.byte	0x2
	.byte	0x9b
	.uleb128 0x24
	.long	.LASF24
	.byte	0x2
	.byte	0x9c
	.long	0x1b58
	.uleb128 0x1c
	.string	"ptoken"
	.byte	0x2
	.byte	0x9d
	.long	0x1f8d
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1b58
	.uleb128 0x15
	.long	.LASF25
	.byte	0x2
	.byte	0xa1
	.long	0x1f9e
	.uleb128 0x8
	.long	0x1fe4
	.long	.LASF25
	.byte	0x10
	.byte	0x2
	.byte	0xa1
	.uleb128 0xf
	.long	.LASF21
	.byte	0x2
	.byte	0xa4
	.long	0x1fe4
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"prev"
	.byte	0x2
	.byte	0xa4
	.long	0x1fe4
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xf
	.long	.LASF23
	.byte	0x2
	.byte	0xa5
	.long	0x1ee1
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0xf
	.long	.LASF2
	.byte	0x2
	.byte	0xa5
	.long	0x1ee1
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1f93
	.uleb128 0x15
	.long	.LASF26
	.byte	0x2
	.byte	0xae
	.long	0x1ff5
	.uleb128 0x8
	.long	0x205c
	.long	.LASF26
	.byte	0x1c
	.byte	0x2
	.byte	0xae
	.uleb128 0xf
	.long	.LASF21
	.byte	0x2
	.byte	0xb2
	.long	0x20c9
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"prev"
	.byte	0x2
	.byte	0xb2
	.long	0x20c9
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xe
	.string	"u"
	.byte	0x2
	.byte	0xc5
	.long	0x20a9
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0xf
	.long	.LASF27
	.byte	0x2
	.byte	0xc9
	.long	0x20cf
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0xe
	.string	"macro"
	.byte	0x2
	.byte	0xcc
	.long	0x1b52
	.byte	0x2
	.byte	0x23
	.uleb128 0x14
	.uleb128 0xe
	.string	"direct_p"
	.byte	0x2
	.byte	0xcf
	.long	0xcf1
	.byte	0x2
	.byte	0x23
	.uleb128 0x18
	.byte	0x0
	.uleb128 0x25
	.long	0x2084
	.byte	0x8
	.byte	0x2
	.byte	0xba
	.uleb128 0xe
	.string	"first"
	.byte	0x2
	.byte	0xbb
	.long	0x1f64
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"last"
	.byte	0x2
	.byte	0xbc
	.long	0x1f64
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.byte	0x0
	.uleb128 0x25
	.long	0x20a9
	.byte	0x8
	.byte	0x2
	.byte	0xc1
	.uleb128 0xe
	.string	"cur"
	.byte	0x2
	.byte	0xc2
	.long	0x1ee7
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xf
	.long	.LASF12
	.byte	0x2
	.byte	0xc3
	.long	0x1ee7
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.byte	0x0
	.uleb128 0x1b
	.long	0x20c9
	.byte	0x8
	.byte	0x2
	.byte	0xb5
	.uleb128 0x1c
	.string	"iso"
	.byte	0x2
	.byte	0xbd
	.long	0x205c
	.uleb128 0x1c
	.string	"trad"
	.byte	0x2
	.byte	0xc4
	.long	0x2084
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1fea
	.uleb128 0x2
	.byte	0x4
	.long	0x1ef8
	.uleb128 0xd
	.long	0x220a
	.string	"lexer_state"
	.byte	0x10
	.byte	0x2
	.byte	0xd3
	.uleb128 0xe
	.string	"in_directive"
	.byte	0x2
	.byte	0xd5
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"directive_wants_padding"
	.byte	0x2
	.byte	0xda
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x1
	.uleb128 0xe
	.string	"skipping"
	.byte	0x2
	.byte	0xdd
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x2
	.uleb128 0xe
	.string	"angled_headers"
	.byte	0x2
	.byte	0xe0
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x3
	.uleb128 0xe
	.string	"in_expression"
	.byte	0x2
	.byte	0xe3
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xe
	.string	"save_comments"
	.byte	0x2
	.byte	0xe7
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x5
	.uleb128 0xe
	.string	"lexing_comment"
	.byte	0x2
	.byte	0xea
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x6
	.uleb128 0xe
	.string	"va_args_ok"
	.byte	0x2
	.byte	0xed
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x7
	.uleb128 0xe
	.string	"poisoned_ok"
	.byte	0x2
	.byte	0xf0
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0xe
	.string	"prevent_expansion"
	.byte	0x2
	.byte	0xf3
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x9
	.uleb128 0xe
	.string	"parsing_args"
	.byte	0x2
	.byte	0xf6
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0xa
	.uleb128 0xe
	.string	"skip_eval"
	.byte	0x2
	.byte	0xf9
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.byte	0x0
	.uleb128 0x8
	.long	0x2268
	.long	.LASF10
	.byte	0x10
	.byte	0x2
	.byte	0xfe
	.uleb128 0xe
	.string	"n_defined"
	.byte	0x2
	.byte	0xff
	.long	0x1b52
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0x9
	.string	"n_true"
	.byte	0x2
	.value	0x100
	.long	0x1b52
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0x9
	.string	"n_false"
	.byte	0x2
	.value	0x101
	.long	0x1b52
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0x9
	.string	"n__VA_ARGS__"
	.byte	0x2
	.value	0x102
	.long	0x1b52
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.byte	0x0
	.uleb128 0x10
	.long	0x22de
	.string	"printer"
	.byte	0x18
	.byte	0x2
	.value	0x108
	.uleb128 0x9
	.string	"outf"
	.byte	0x2
	.value	0x109
	.long	0x22de
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0x9
	.string	"map"
	.byte	0x2
	.value	0x10a
	.long	0x1cf3
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0x9
	.string	"prev"
	.byte	0x2
	.value	0x10b
	.long	0x1b58
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0x9
	.string	"source"
	.byte	0x2
	.value	0x10c
	.long	0x1b58
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.uleb128 0x9
	.string	"line"
	.byte	0x2
	.value	0x10d
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0x9
	.string	"printed"
	.byte	0x2
	.value	0x10e
	.long	0xd1
	.byte	0x2
	.byte	0x23
	.uleb128 0x14
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1c8
	.uleb128 0x2
	.byte	0x4
	.long	0xd10
	.uleb128 0x1f
	.string	"include_file"
	.byte	0x1
	.uleb128 0x2
	.byte	0x4
	.long	0x22ea
	.uleb128 0x26
	.long	.LASF13
	.byte	0x1
	.uleb128 0x2
	.byte	0x4
	.long	0x22ff
	.uleb128 0x1e
	.long	0x2358
	.byte	0x10
	.byte	0x2
	.value	0x1bf
	.uleb128 0x17
	.long	.LASF23
	.byte	0x2
	.value	0x1c0
	.long	0x2358
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0x17
	.long	.LASF2
	.byte	0x2
	.value	0x1c1
	.long	0x2358
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0x9
	.string	"cur"
	.byte	0x2
	.value	0x1c2
	.long	0x2358
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0x9
	.string	"first_line"
	.byte	0x2
	.value	0x1c3
	.long	0xb5
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1ea7
	.uleb128 0x2
	.byte	0x4
	.long	0xd05
	.uleb128 0x2
	.byte	0x4
	.long	0x1ff5
	.uleb128 0x26
	.long	.LASF9
	.byte	0x1
	.uleb128 0x2
	.byte	0x4
	.long	0x2376
	.uleb128 0x3
	.long	0x236a
	.uleb128 0x1f
	.string	"pending_option"
	.byte	0x1
	.uleb128 0x2
	.byte	0x4
	.long	0x2392
	.uleb128 0x2
	.byte	0x4
	.long	0x237b
	.uleb128 0x2
	.byte	0x4
	.long	0x239e
	.uleb128 0x3
	.long	0x14e2
	.uleb128 0x1f
	.string	"splay_tree_s"
	.byte	0x1
	.uleb128 0x2
	.byte	0x4
	.long	0x23a3
	.uleb128 0x1f
	.string	"deps"
	.byte	0x1
	.uleb128 0x2
	.byte	0x4
	.long	0x23b8
	.uleb128 0x1f
	.string	"pragma_entry"
	.byte	0x1
	.uleb128 0x2
	.byte	0x4
	.long	0x23c5
	.uleb128 0x2
	.byte	0x4
	.long	0x70f
	.uleb128 0x1f
	.string	"op"
	.byte	0x1
	.uleb128 0x2
	.byte	0x4
	.long	0x23e0
	.uleb128 0x18
	.long	0x2456
	.string	"spell_type"
	.byte	0x4
	.byte	0x1
	.byte	0x2d
	.uleb128 0x19
	.string	"SPELL_OPERATOR"
	.sleb128 0
	.uleb128 0x19
	.string	"SPELL_CHAR"
	.sleb128 1
	.uleb128 0x19
	.string	"SPELL_IDENT"
	.sleb128 2
	.uleb128 0x19
	.string	"SPELL_NUMBER"
	.sleb128 3
	.uleb128 0x19
	.string	"SPELL_STRING"
	.sleb128 4
	.uleb128 0x19
	.string	"SPELL_NONE"
	.sleb128 5
	.byte	0x0
	.uleb128 0xd
	.long	0x2490
	.string	"token_spelling"
	.byte	0x8
	.byte	0x1
	.byte	0x37
	.uleb128 0xe
	.string	"category"
	.byte	0x1
	.byte	0x38
	.long	0x23eb
	.byte	0x2
	.byte	0x23
	.uleb128 0x0
	.uleb128 0xe
	.string	"name"
	.byte	0x1
	.byte	0x39
	.long	0x465
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.byte	0x0
	.uleb128 0x27
	.long	0x24d2
	.byte	0x1
	.string	"cpp_ideq"
	.byte	0x1
	.byte	0x6b
	.byte	0x1
	.long	0xae
	.long	.LFB21
	.long	.LFE21
	.long	.LLST0
	.uleb128 0x28
	.long	.LASF24
	.byte	0x1
	.byte	0x69
	.long	0x1b58
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x29
	.string	"string"
	.byte	0x1
	.byte	0x6a
	.long	0x9b
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.byte	0x0
	.uleb128 0x2a
	.long	0x2510
	.string	"ustrcmp"
	.byte	0x2
	.value	0x24e
	.byte	0x1
	.long	0xae
	.long	.LFB15
	.long	.LFE15
	.long	.LLST1
	.uleb128 0x2b
	.string	"s1"
	.byte	0x2
	.value	0x24d
	.long	0x1ee7
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"s2"
	.byte	0x2
	.value	0x24d
	.long	0x1ee7
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.byte	0x0
	.uleb128 0x2c
	.long	0x2550
	.string	"handle_newline"
	.byte	0x1
	.byte	0x7c
	.byte	0x1
	.long	.LFB22
	.long	.LFE22
	.long	.LLST2
	.uleb128 0x28
	.long	.LASF5
	.byte	0x1
	.byte	0x7b
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2d
	.long	.LASF7
	.byte	0x1
	.byte	0x7d
	.long	0x235e
	.byte	0x2
	.byte	0x91
	.sleb128 -12
	.byte	0x0
	.uleb128 0x2e
	.long	0x25b5
	.string	"trigraph_p"
	.byte	0x1
	.byte	0x92
	.byte	0x1
	.long	0xcf1
	.long	.LFB23
	.long	.LFE23
	.long	.LLST3
	.uleb128 0x28
	.long	.LASF5
	.byte	0x1
	.byte	0x91
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2d
	.long	.LASF7
	.byte	0x1
	.byte	0x93
	.long	0x235e
	.byte	0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x2f
	.string	"from_char"
	.byte	0x1
	.byte	0x94
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.uleb128 0x2f
	.string	"accept"
	.byte	0x1
	.byte	0x95
	.long	0xcf1
	.byte	0x2
	.byte	0x91
	.sleb128 -21
	.byte	0x0
	.uleb128 0x2e
	.long	0x2636
	.string	"skip_escaped_newlines"
	.byte	0x1
	.byte	0xb8
	.byte	0x1
	.long	0x1b63
	.long	.LFB24
	.long	.LFE24
	.long	.LLST4
	.uleb128 0x28
	.long	.LASF5
	.byte	0x1
	.byte	0xb7
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2d
	.long	.LASF7
	.byte	0x1
	.byte	0xb9
	.long	0x235e
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.uleb128 0x2d
	.long	.LASF21
	.byte	0x1
	.byte	0xba
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.uleb128 0x30
	.long	.LBB2
	.long	.LBE2
	.uleb128 0x2d
	.long	.LASF14
	.byte	0x1
	.byte	0xbf
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x2f
	.string	"next1"
	.byte	0x1
	.byte	0xc0
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.byte	0x0
	.byte	0x0
	.uleb128 0x2e
	.long	0x268d
	.string	"get_effective_char"
	.byte	0x1
	.byte	0xfe
	.byte	0x1
	.long	0x1b63
	.long	.LFB25
	.long	.LFE25
	.long	.LLST5
	.uleb128 0x28
	.long	.LASF5
	.byte	0x1
	.byte	0xfd
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2d
	.long	.LASF21
	.byte	0x1
	.byte	0xff
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.uleb128 0x31
	.long	.LASF7
	.byte	0x1
	.value	0x100
	.long	0x235e
	.byte	0x2
	.byte	0x91
	.sleb128 -12
	.byte	0x0
	.uleb128 0x2a
	.long	0x26f6
	.string	"skip_block_comment"
	.byte	0x1
	.value	0x110
	.byte	0x1
	.long	0xae
	.long	.LFB26
	.long	.LFE26
	.long	.LLST6
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x10f
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x31
	.long	.LASF7
	.byte	0x1
	.value	0x111
	.long	0x235e
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.uleb128 0x33
	.string	"c"
	.byte	0x1
	.value	0x112
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x33
	.string	"prevc"
	.byte	0x1
	.value	0x112
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.byte	0x0
	.uleb128 0x2a
	.long	0x2771
	.string	"skip_line_comment"
	.byte	0x1
	.value	0x13e
	.byte	0x1
	.long	0xae
	.long	.LFB27
	.long	.LFE27
	.long	.LLST7
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x13d
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x31
	.long	.LASF7
	.byte	0x1
	.value	0x13f
	.long	0x235e
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.uleb128 0x33
	.string	"orig_line"
	.byte	0x1
	.value	0x140
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x33
	.string	"c"
	.byte	0x1
	.value	0x141
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.uleb128 0x34
	.string	"at_eof"
	.byte	0x1
	.value	0x16a
	.long	.L78
	.byte	0x0
	.uleb128 0x35
	.long	0x27c2
	.string	"adjust_column"
	.byte	0x1
	.value	0x175
	.byte	0x1
	.long	.LFB28
	.long	.LFE28
	.long	.LLST8
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x174
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x31
	.long	.LASF7
	.byte	0x1
	.value	0x176
	.long	0x235e
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.uleb128 0x33
	.string	"col"
	.byte	0x1
	.value	0x177
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.byte	0x0
	.uleb128 0x2a
	.long	0x2829
	.string	"skip_whitespace"
	.byte	0x1
	.value	0x186
	.byte	0x1
	.long	0xae
	.long	.LFB29
	.long	.LFE29
	.long	.LLST9
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x184
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"c"
	.byte	0x1
	.value	0x185
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x31
	.long	.LASF7
	.byte	0x1
	.value	0x187
	.long	0x235e
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x33
	.string	"warned"
	.byte	0x1
	.value	0x188
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.byte	0x0
	.uleb128 0x2a
	.long	0x2878
	.string	"name_p"
	.byte	0x1
	.value	0x1b1
	.byte	0x1
	.long	0xae
	.long	.LFB30
	.long	.LFE30
	.long	.LLST10
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x1af
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"string"
	.byte	0x1
	.value	0x1b0
	.long	0x1d57
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x33
	.string	"i"
	.byte	0x1
	.value	0x1b2
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -12
	.byte	0x0
	.uleb128 0x2a
	.long	0x28f8
	.string	"parse_identifier"
	.byte	0x1
	.value	0x1c6
	.byte	0x1
	.long	0x1b52
	.long	.LFB31
	.long	.LFE31
	.long	.LLST11
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x1c5
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x31
	.long	.LASF28
	.byte	0x1
	.value	0x1c7
	.long	0x1b52
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.uleb128 0x33
	.string	"cur"
	.byte	0x1
	.value	0x1c8
	.long	0x1ee7
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x31
	.long	.LASF23
	.byte	0x1
	.value	0x1c8
	.long	0x1ee7
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.uleb128 0x30
	.long	.LBB3
	.long	.LBE3
	.uleb128 0x33
	.string	"len"
	.byte	0x1
	.value	0x1d3
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.byte	0x0
	.byte	0x0
	.uleb128 0x2a
	.long	0x2a92
	.string	"parse_slow"
	.byte	0x1
	.value	0x202
	.byte	0x1
	.long	0x2358
	.long	.LFB32
	.long	.LFE32
	.long	.LLST12
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x1fe
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"cur"
	.byte	0x1
	.value	0x1ff
	.long	0x1ee7
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x2b
	.string	"number_p"
	.byte	0x1
	.value	0x200
	.long	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 8
	.uleb128 0x2b
	.string	"plen"
	.byte	0x1
	.value	0x201
	.long	0x2a92
	.byte	0x2
	.byte	0x91
	.sleb128 12
	.uleb128 0x31
	.long	.LASF7
	.byte	0x1
	.value	0x203
	.long	0x235e
	.byte	0x3
	.byte	0x91
	.sleb128 -76
	.uleb128 0x31
	.long	.LASF23
	.byte	0x1
	.value	0x204
	.long	0x1ee7
	.byte	0x3
	.byte	0x91
	.sleb128 -72
	.uleb128 0x33
	.string	"stack"
	.byte	0x1
	.value	0x205
	.long	0x2a98
	.byte	0x3
	.byte	0x91
	.sleb128 -68
	.uleb128 0x33
	.string	"c"
	.byte	0x1
	.value	0x206
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -64
	.uleb128 0x33
	.string	"prevc"
	.byte	0x1
	.value	0x206
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -60
	.uleb128 0x33
	.string	"saw_dollar"
	.byte	0x1
	.value	0x206
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -56
	.uleb128 0x36
	.long	0x29e1
	.long	.LBB4
	.long	.LBE4
	.uleb128 0x33
	.string	"__o"
	.byte	0x1
	.value	0x20a
	.long	0x2a98
	.byte	0x2
	.byte	0x91
	.sleb128 -52
	.byte	0x0
	.uleb128 0x36
	.long	0x2a0f
	.long	.LBB5
	.long	.LBE5
	.uleb128 0x33
	.string	"__o"
	.byte	0x1
	.value	0x20d
	.long	0x2a98
	.byte	0x2
	.byte	0x91
	.sleb128 -48
	.uleb128 0x33
	.string	"__len"
	.byte	0x1
	.value	0x20d
	.long	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -44
	.byte	0x0
	.uleb128 0x36
	.long	0x2a2c
	.long	.LBB6
	.long	.LBE6
	.uleb128 0x33
	.string	"__o"
	.byte	0x1
	.value	0x227
	.long	0x2a98
	.byte	0x2
	.byte	0x91
	.sleb128 -40
	.byte	0x0
	.uleb128 0x36
	.long	0x2a49
	.long	.LBB7
	.long	.LBE7
	.uleb128 0x33
	.string	"__o"
	.byte	0x1
	.value	0x23b
	.long	0x2a98
	.byte	0x2
	.byte	0x91
	.sleb128 -36
	.byte	0x0
	.uleb128 0x36
	.long	0x2a66
	.long	.LBB8
	.long	.LBE8
	.uleb128 0x33
	.string	"__o"
	.byte	0x1
	.value	0x23c
	.long	0x2a98
	.byte	0x2
	.byte	0x91
	.sleb128 -32
	.byte	0x0
	.uleb128 0x30
	.long	.LBB9
	.long	.LBE9
	.uleb128 0x33
	.string	"__o1"
	.byte	0x1
	.value	0x23d
	.long	0x2a98
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.uleb128 0x33
	.string	"value"
	.byte	0x1
	.value	0x23d
	.long	0x1c0
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.byte	0x0
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0xb5
	.uleb128 0x2
	.byte	0x4
	.long	0x567
	.uleb128 0x35
	.long	0x2b34
	.string	"parse_number"
	.byte	0x1
	.value	0x248
	.byte	0x1
	.long	.LFB33
	.long	.LFE33
	.long	.LLST13
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x245
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"number"
	.byte	0x1
	.value	0x246
	.long	0x2b34
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x2b
	.string	"leading_period"
	.byte	0x1
	.value	0x247
	.long	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 8
	.uleb128 0x33
	.string	"cur"
	.byte	0x1
	.value	0x249
	.long	0x1ee7
	.byte	0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x30
	.long	.LBB10
	.long	.LBE10
	.uleb128 0x31
	.long	.LASF23
	.byte	0x1
	.value	0x256
	.long	0x1ee7
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.uleb128 0x33
	.string	"dest"
	.byte	0x1
	.value	0x257
	.long	0x2358
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.byte	0x0
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x14ad
	.uleb128 0x2a
	.long	0x2bab
	.string	"unescaped_terminator_p"
	.byte	0x1
	.value	0x26a
	.byte	0x1
	.long	0xae
	.long	.LFB34
	.long	.LFE34
	.long	.LLST14
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x268
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"dest"
	.byte	0x1
	.value	0x269
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x33
	.string	"start"
	.byte	0x1
	.value	0x26b
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.uleb128 0x33
	.string	"temp"
	.byte	0x1
	.value	0x26b
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -12
	.byte	0x0
	.uleb128 0x35
	.long	0x2c8a
	.string	"parse_string"
	.byte	0x1
	.value	0x287
	.byte	0x1
	.long	.LFB35
	.long	.LFE35
	.long	.LLST15
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x284
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x32
	.long	.LASF24
	.byte	0x1
	.value	0x285
	.long	0x1ee1
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x2b
	.string	"terminator"
	.byte	0x1
	.value	0x286
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 8
	.uleb128 0x31
	.long	.LASF7
	.byte	0x1
	.value	0x288
	.long	0x235e
	.byte	0x2
	.byte	0x91
	.sleb128 -36
	.uleb128 0x33
	.string	"dest"
	.byte	0x1
	.value	0x289
	.long	0x470
	.byte	0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x31
	.long	.LASF2
	.byte	0x1
	.value	0x289
	.long	0x470
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.uleb128 0x33
	.string	"c"
	.byte	0x1
	.value	0x28a
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.uleb128 0x33
	.string	"warned_nulls"
	.byte	0x1
	.value	0x28b
	.long	0xcf1
	.byte	0x2
	.byte	0x91
	.sleb128 -13
	.uleb128 0x34
	.string	"unterminated"
	.byte	0x1
	.value	0x2cf
	.long	.L224
	.uleb128 0x30
	.long	.LBB11
	.long	.LBE11
	.uleb128 0x33
	.string	"len_so_far"
	.byte	0x1
	.value	0x29d
	.long	0xc3
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.byte	0x0
	.byte	0x0
	.uleb128 0x35
	.long	0x2d19
	.string	"save_comment"
	.byte	0x1
	.value	0x2f0
	.byte	0x1
	.long	.LFB36
	.long	.LFE36
	.long	.LLST16
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x2ec
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x32
	.long	.LASF24
	.byte	0x1
	.value	0x2ed
	.long	0x1ee1
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x2b
	.string	"from"
	.byte	0x1
	.value	0x2ee
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 8
	.uleb128 0x2b
	.string	"type"
	.byte	0x1
	.value	0x2ef
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 12
	.uleb128 0x31
	.long	.LASF7
	.byte	0x1
	.value	0x2f1
	.long	0x470
	.byte	0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x33
	.string	"len"
	.byte	0x1
	.value	0x2f2
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.uleb128 0x33
	.string	"clen"
	.byte	0x1
	.value	0x2f2
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.byte	0x0
	.uleb128 0x37
	.long	0x2d63
	.byte	0x1
	.string	"_cpp_init_tokenrun"
	.byte	0x1
	.value	0x31a
	.byte	0x1
	.long	.LFB37
	.long	.LFE37
	.long	.LLST17
	.uleb128 0x2b
	.string	"run"
	.byte	0x1
	.value	0x318
	.long	0x1fe4
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"count"
	.byte	0x1
	.value	0x319
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.byte	0x0
	.uleb128 0x2a
	.long	0x2d9a
	.string	"next_tokenrun"
	.byte	0x1
	.value	0x324
	.byte	0x1
	.long	0x1fe4
	.long	.LFB38
	.long	.LFE38
	.long	.LLST18
	.uleb128 0x2b
	.string	"run"
	.byte	0x1
	.value	0x323
	.long	0x1fe4
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.byte	0x0
	.uleb128 0x38
	.long	0x2df2
	.byte	0x1
	.string	"_cpp_temp_token"
	.byte	0x1
	.value	0x336
	.byte	0x1
	.long	0x1ee1
	.long	.LFB39
	.long	.LFE39
	.long	.LLST19
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x335
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x33
	.string	"old"
	.byte	0x1
	.value	0x337
	.long	0x1ee1
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.uleb128 0x31
	.long	.LASF28
	.byte	0x1
	.value	0x337
	.long	0x1ee1
	.byte	0x2
	.byte	0x91
	.sleb128 -12
	.byte	0x0
	.uleb128 0x20
	.long	0x2e34
	.string	"pragma_type"
	.byte	0x4
	.byte	0x1
	.value	0x348
	.uleb128 0x19
	.string	"INVALID"
	.sleb128 0
	.uleb128 0x19
	.string	"OMP"
	.sleb128 1
	.uleb128 0x19
	.string	"OPTIONS"
	.sleb128 2
	.uleb128 0x19
	.string	"EXEC_FREQ"
	.sleb128 3
	.uleb128 0x19
	.string	"SL2"
	.sleb128 4
	.byte	0x0
	.uleb128 0x2a
	.long	0x2eb5
	.string	"_cpp_omp_token"
	.byte	0x1
	.value	0x357
	.byte	0x1
	.long	0x1ee1
	.long	.LFB40
	.long	.LFE40
	.long	.LLST20
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x356
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x31
	.long	.LASF7
	.byte	0x1
	.value	0x358
	.long	0x235e
	.byte	0x2
	.byte	0x91
	.sleb128 -40
	.uleb128 0x31
	.long	.LASF28
	.byte	0x1
	.value	0x359
	.long	0x1ee1
	.byte	0x2
	.byte	0x91
	.sleb128 -36
	.uleb128 0x33
	.string	"c"
	.byte	0x1
	.value	0x35b
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x31
	.long	.LASF12
	.byte	0x1
	.value	0x35c
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.uleb128 0x33
	.string	"len"
	.byte	0x1
	.value	0x360
	.long	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.byte	0x0
	.uleb128 0x35
	.long	0x2f0a
	.string	"skip_to_end_of_line"
	.byte	0x1
	.value	0x38c
	.byte	0x1
	.long	.LFB41
	.long	.LFE41
	.long	.LLST21
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x38b
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x31
	.long	.LASF7
	.byte	0x1
	.value	0x38d
	.long	0x235e
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.uleb128 0x33
	.string	"c"
	.byte	0x1
	.value	0x38e
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -12
	.byte	0x0
	.uleb128 0x38
	.long	0x2f6f
	.byte	0x1
	.string	"_cpp_lex_token"
	.byte	0x1
	.value	0x3a5
	.byte	0x1
	.long	0x1b58
	.long	.LFB42
	.long	.LFE42
	.long	.LLST22
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x3a4
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x31
	.long	.LASF28
	.byte	0x1
	.value	0x3a6
	.long	0x1ee1
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x30
	.long	.LBB12
	.long	.LBE12
	.uleb128 0x33
	.string	"omp_res"
	.byte	0x1
	.value	0x3c3
	.long	0x1ee1
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.byte	0x0
	.byte	0x0
	.uleb128 0x2a
	.long	0x2fca
	.string	"continue_after_nul"
	.byte	0x1
	.value	0x407
	.byte	0x1
	.long	0xcf1
	.long	.LFB43
	.long	.LFE43
	.long	.LLST23
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x406
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x31
	.long	.LASF7
	.byte	0x1
	.value	0x408
	.long	0x235e
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x33
	.string	"more"
	.byte	0x1
	.value	0x409
	.long	0xcf1
	.byte	0x2
	.byte	0x91
	.sleb128 -13
	.byte	0x0
	.uleb128 0x38
	.long	0x3136
	.byte	0x1
	.string	"_cpp_lex_direct"
	.byte	0x1
	.value	0x44c
	.byte	0x1
	.long	0x1ee1
	.long	.LFB44
	.long	.LFE44
	.long	.LLST24
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x44b
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x33
	.string	"c"
	.byte	0x1
	.value	0x44d
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -44
	.uleb128 0x31
	.long	.LASF7
	.byte	0x1
	.value	0x44e
	.long	0x235e
	.byte	0x2
	.byte	0x91
	.sleb128 -40
	.uleb128 0x33
	.string	"comment_start"
	.byte	0x1
	.value	0x44f
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -36
	.uleb128 0x31
	.long	.LASF28
	.byte	0x1
	.value	0x450
	.long	0x1ee1
	.byte	0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x34
	.string	"fresh_line"
	.byte	0x1
	.value	0x472
	.long	.L340
	.uleb128 0x34
	.string	"update_tokens_line"
	.byte	0x1
	.value	0x495
	.long	.L341
	.uleb128 0x34
	.string	"skipped_white"
	.byte	0x1
	.value	0x46d
	.long	.L342
	.uleb128 0x34
	.string	"trigraph"
	.byte	0x1
	.value	0x49f
	.long	.L343
	.uleb128 0x34
	.string	"random_char"
	.byte	0x1
	.value	0x5dd
	.long	.L347
	.uleb128 0x34
	.string	"start_ident"
	.byte	0x1
	.value	0x5da
	.long	.L372
	.uleb128 0x36
	.long	0x30e2
	.long	.LBB13
	.long	.LBE13
	.uleb128 0x33
	.string	"line"
	.byte	0x1
	.value	0x48d
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.byte	0x0
	.uleb128 0x36
	.long	0x30ff
	.long	.LBB14
	.long	.LBE14
	.uleb128 0x33
	.string	"pos"
	.byte	0x1
	.value	0x4ab
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.byte	0x0
	.uleb128 0x36
	.long	0x311c
	.long	.LBB15
	.long	.LBE15
	.uleb128 0x33
	.string	"pos"
	.byte	0x1
	.value	0x544
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.byte	0x0
	.uleb128 0x30
	.long	.LBB16
	.long	.LBE16
	.uleb128 0x33
	.string	"pos"
	.byte	0x1
	.value	0x55f
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.byte	0x0
	.byte	0x0
	.uleb128 0x38
	.long	0x317d
	.byte	0x1
	.string	"cpp_token_len"
	.byte	0x1
	.value	0x5ec
	.byte	0x1
	.long	0xb5
	.long	.LFB45
	.long	.LFE45
	.long	.LLST25
	.uleb128 0x32
	.long	.LASF24
	.byte	0x1
	.value	0x5eb
	.long	0x1b58
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x33
	.string	"len"
	.byte	0x1
	.value	0x5ed
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -12
	.byte	0x0
	.uleb128 0x38
	.long	0x3245
	.byte	0x1
	.string	"cpp_spell_token"
	.byte	0x1
	.value	0x603
	.byte	0x1
	.long	0x470
	.long	.LFB46
	.long	.LFE46
	.long	.LLST26
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x600
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x32
	.long	.LASF24
	.byte	0x1
	.value	0x601
	.long	0x1b58
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x32
	.long	.LASF7
	.byte	0x1
	.value	0x602
	.long	0x470
	.byte	0x2
	.byte	0x91
	.sleb128 8
	.uleb128 0x39
	.long	.LASF29
	.byte	0x1
	.value	0x61c
	.long	.L552
	.uleb128 0x36
	.long	0x320a
	.long	.LBB17
	.long	.LBE17
	.uleb128 0x31
	.long	.LASF30
	.byte	0x1
	.value	0x608
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -40
	.uleb128 0x33
	.string	"c"
	.byte	0x1
	.value	0x609
	.long	0xd1
	.byte	0x2
	.byte	0x91
	.sleb128 -21
	.byte	0x0
	.uleb128 0x30
	.long	.LBB18
	.long	.LBE18
	.uleb128 0x33
	.string	"left"
	.byte	0x1
	.value	0x629
	.long	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -36
	.uleb128 0x33
	.string	"right"
	.byte	0x1
	.value	0x629
	.long	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x33
	.string	"tag"
	.byte	0x1
	.value	0x629
	.long	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.byte	0x0
	.byte	0x0
	.uleb128 0x38
	.long	0x32bf
	.byte	0x1
	.string	"cpp_token_as_text"
	.byte	0x1
	.value	0x64c
	.byte	0x1
	.long	0x470
	.long	.LFB47
	.long	.LFE47
	.long	.LLST27
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x64a
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x32
	.long	.LASF24
	.byte	0x1
	.value	0x64b
	.long	0x1b58
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x33
	.string	"len"
	.byte	0x1
	.value	0x64d
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.uleb128 0x33
	.string	"start"
	.byte	0x1
	.value	0x64e
	.long	0x470
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x33
	.string	"end"
	.byte	0x1
	.value	0x64e
	.long	0x470
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.byte	0x0
	.uleb128 0x38
	.long	0x32f8
	.byte	0x1
	.string	"cpp_type2name"
	.byte	0x1
	.value	0x65b
	.byte	0x1
	.long	0x9b
	.long	.LFB48
	.long	.LFE48
	.long	.LLST28
	.uleb128 0x2b
	.string	"type"
	.byte	0x1
	.value	0x65a
	.long	0x16f2
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.byte	0x0
	.uleb128 0x37
	.long	0x33ad
	.byte	0x1
	.string	"cpp_output_token"
	.byte	0x1
	.value	0x666
	.byte	0x1
	.long	.LFB49
	.long	.LFE49
	.long	.LLST29
	.uleb128 0x32
	.long	.LASF24
	.byte	0x1
	.value	0x664
	.long	0x1b58
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"fp"
	.byte	0x1
	.value	0x665
	.long	0x22de
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x39
	.long	.LASF29
	.byte	0x1
	.value	0x681
	.long	.L584
	.uleb128 0x36
	.long	0x3372
	.long	.LBB19
	.long	.LBE19
	.uleb128 0x31
	.long	.LASF30
	.byte	0x1
	.value	0x66b
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x33
	.string	"c"
	.byte	0x1
	.value	0x66c
	.long	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.byte	0x0
	.uleb128 0x30
	.long	.LBB20
	.long	.LBE20
	.uleb128 0x33
	.string	"left"
	.byte	0x1
	.value	0x68c
	.long	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.uleb128 0x33
	.string	"right"
	.byte	0x1
	.value	0x68c
	.long	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x33
	.string	"tag"
	.byte	0x1
	.value	0x68c
	.long	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.byte	0x0
	.byte	0x0
	.uleb128 0x38
	.long	0x33f4
	.byte	0x1
	.string	"_cpp_equiv_tokens"
	.byte	0x1
	.value	0x6a9
	.byte	0x1
	.long	0xae
	.long	.LFB50
	.long	.LFE50
	.long	.LLST30
	.uleb128 0x2b
	.string	"a"
	.byte	0x1
	.value	0x6a8
	.long	0x1b58
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"b"
	.byte	0x1
	.value	0x6a8
	.long	0x1b58
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.byte	0x0
	.uleb128 0x38
	.long	0x3479
	.byte	0x1
	.string	"cpp_avoid_paste"
	.byte	0x1
	.value	0x6c8
	.byte	0x1
	.long	0xae
	.long	.LFB51
	.long	.LFE51
	.long	.LLST31
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x6c6
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"token1"
	.byte	0x1
	.value	0x6c7
	.long	0x1b58
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x2b
	.string	"token2"
	.byte	0x1
	.value	0x6c7
	.long	0x1b58
	.byte	0x2
	.byte	0x91
	.sleb128 8
	.uleb128 0x33
	.string	"a"
	.byte	0x1
	.value	0x6c9
	.long	0x16f2
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.uleb128 0x33
	.string	"b"
	.byte	0x1
	.value	0x6c9
	.long	0x16f2
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x33
	.string	"c"
	.byte	0x1
	.value	0x6ca
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.byte	0x0
	.uleb128 0x37
	.long	0x34cc
	.byte	0x1
	.string	"cpp_output_line"
	.byte	0x1
	.value	0x6ff
	.byte	0x1
	.long	.LFB52
	.long	.LFE52
	.long	.LLST32
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x6fd
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"fp"
	.byte	0x1
	.value	0x6fe
	.long	0x22de
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x31
	.long	.LASF24
	.byte	0x1
	.value	0x700
	.long	0x1b58
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.byte	0x0
	.uleb128 0x2a
	.long	0x3503
	.string	"hex_digit_value"
	.byte	0x1
	.value	0x712
	.byte	0x1
	.long	0xb5
	.long	.LFB53
	.long	.LFE53
	.long	.LLST33
	.uleb128 0x2b
	.string	"c"
	.byte	0x1
	.value	0x711
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.byte	0x0
	.uleb128 0x2a
	.long	0x35a4
	.string	"maybe_read_ucs"
	.byte	0x1
	.value	0x735
	.byte	0x1
	.long	0xae
	.long	.LFB54
	.long	.LFE54
	.long	.LLST34
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x731
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"pstr"
	.byte	0x1
	.value	0x732
	.long	0x45f
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x32
	.long	.LASF2
	.byte	0x1
	.value	0x733
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 8
	.uleb128 0x2b
	.string	"pc"
	.byte	0x1
	.value	0x734
	.long	0x35a4
	.byte	0x2
	.byte	0x91
	.sleb128 12
	.uleb128 0x33
	.string	"p"
	.byte	0x1
	.value	0x736
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x33
	.string	"code"
	.byte	0x1
	.value	0x737
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.uleb128 0x33
	.string	"c"
	.byte	0x1
	.value	0x738
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.uleb128 0x33
	.string	"length"
	.byte	0x1
	.value	0x738
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x1b63
	.uleb128 0x38
	.long	0x36d3
	.byte	0x1
	.string	"cpp_parse_escape"
	.byte	0x1
	.value	0x77d
	.byte	0x1
	.long	0x1b63
	.long	.LFB55
	.long	.LFE55
	.long	.LLST35
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x779
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"pstr"
	.byte	0x1
	.value	0x77a
	.long	0x45f
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x32
	.long	.LASF2
	.byte	0x1
	.value	0x77b
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 8
	.uleb128 0x2b
	.string	"wide"
	.byte	0x1
	.value	0x77c
	.long	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 12
	.uleb128 0x33
	.string	"unknown"
	.byte	0x1
	.value	0x77e
	.long	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -48
	.uleb128 0x33
	.string	"str"
	.byte	0x1
	.value	0x77f
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -52
	.uleb128 0x33
	.string	"c"
	.byte	0x1
	.value	0x780
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -56
	.uleb128 0x33
	.string	"mask"
	.byte	0x1
	.value	0x780
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -44
	.uleb128 0x33
	.string	"width"
	.byte	0x1
	.value	0x781
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -40
	.uleb128 0x36
	.long	0x36aa
	.long	.LBB21
	.long	.LBE21
	.uleb128 0x33
	.string	"i"
	.byte	0x1
	.value	0x7b5
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -36
	.uleb128 0x33
	.string	"overflow"
	.byte	0x1
	.value	0x7b5
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x33
	.string	"digits_found"
	.byte	0x1
	.value	0x7b6
	.long	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.byte	0x0
	.uleb128 0x30
	.long	.LBB22
	.long	.LBE22
	.uleb128 0x33
	.string	"count"
	.byte	0x1
	.value	0x7d4
	.long	0xc3
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.uleb128 0x33
	.string	"i"
	.byte	0x1
	.value	0x7d5
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.byte	0x0
	.byte	0x0
	.uleb128 0x38
	.long	0x37ec
	.byte	0x1
	.string	"cpp_interpret_charconst"
	.byte	0x1
	.value	0x80e
	.byte	0x1
	.long	0x1b63
	.long	.LFB56
	.long	.LFE56
	.long	.LLST36
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x80a
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x32
	.long	.LASF24
	.byte	0x1
	.value	0x80b
	.long	0x1b58
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x2b
	.string	"pchars_seen"
	.byte	0x1
	.value	0x80c
	.long	0x2a92
	.byte	0x2
	.byte	0x91
	.sleb128 8
	.uleb128 0x2b
	.string	"unsignedp"
	.byte	0x1
	.value	0x80d
	.long	0x513
	.byte	0x2
	.byte	0x91
	.sleb128 12
	.uleb128 0x33
	.string	"str"
	.byte	0x1
	.value	0x80f
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -48
	.uleb128 0x31
	.long	.LASF2
	.byte	0x1
	.value	0x810
	.long	0x465
	.byte	0x2
	.byte	0x91
	.sleb128 -44
	.uleb128 0x33
	.string	"chars_seen"
	.byte	0x1
	.value	0x811
	.long	0xb5
	.byte	0x2
	.byte	0x91
	.sleb128 -40
	.uleb128 0x33
	.string	"width"
	.byte	0x1
	.value	0x812
	.long	0xc3
	.byte	0x2
	.byte	0x91
	.sleb128 -36
	.uleb128 0x33
	.string	"max_chars"
	.byte	0x1
	.value	0x812
	.long	0xc3
	.byte	0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x33
	.string	"c"
	.byte	0x1
	.value	0x813
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.uleb128 0x33
	.string	"mask"
	.byte	0x1
	.value	0x813
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.uleb128 0x31
	.long	.LASF28
	.byte	0x1
	.value	0x813
	.long	0x1b63
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x33
	.string	"unsigned_p"
	.byte	0x1
	.value	0x814
	.long	0xcf1
	.byte	0x2
	.byte	0x91
	.sleb128 -13
	.byte	0x0
	.uleb128 0x3a
	.long	0x3837
	.long	.LASF31
	.byte	0x1
	.value	0x88d
	.byte	0x1
	.long	0x20cf
	.long	.LFB57
	.long	.LFE57
	.long	.LLST37
	.uleb128 0x2b
	.string	"len"
	.byte	0x1
	.value	0x88c
	.long	0xc3
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x31
	.long	.LASF28
	.byte	0x1
	.value	0x88e
	.long	0x20cf
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x31
	.long	.LASF23
	.byte	0x1
	.value	0x88f
	.long	0x470
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.byte	0x0
	.uleb128 0x37
	.long	0x388d
	.byte	0x1
	.string	"_cpp_release_buff"
	.byte	0x1
	.value	0x8a3
	.byte	0x1
	.long	.LFB58
	.long	.LFE58
	.long	.LLST38
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x8a1
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x32
	.long	.LASF27
	.byte	0x1
	.value	0x8a2
	.long	0x20cf
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x33
	.string	"end"
	.byte	0x1
	.value	0x8a4
	.long	0x20cf
	.byte	0x2
	.byte	0x91
	.sleb128 -12
	.byte	0x0
	.uleb128 0x38
	.long	0x390f
	.byte	0x1
	.string	"_cpp_get_buff"
	.byte	0x1
	.value	0x8b1
	.byte	0x1
	.long	0x20cf
	.long	.LFB59
	.long	.LFE59
	.long	.LLST39
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x8af
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"min_size"
	.byte	0x1
	.value	0x8b0
	.long	0xc3
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x31
	.long	.LASF28
	.byte	0x1
	.value	0x8b2
	.long	0x20cf
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x33
	.string	"p"
	.byte	0x1
	.value	0x8b2
	.long	0x390f
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.uleb128 0x30
	.long	.LBB23
	.long	.LBE23
	.uleb128 0x33
	.string	"size"
	.byte	0x1
	.value	0x8b6
	.long	0xc3
	.byte	0x2
	.byte	0x91
	.sleb128 -12
	.byte	0x0
	.byte	0x0
	.uleb128 0x2
	.byte	0x4
	.long	0x20cf
	.uleb128 0x38
	.long	0x3994
	.byte	0x1
	.string	"_cpp_append_extend_buff"
	.byte	0x1
	.value	0x8d1
	.byte	0x1
	.long	0x20cf
	.long	.LFB60
	.long	.LFE60
	.long	.LLST40
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x8ce
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x32
	.long	.LASF27
	.byte	0x1
	.value	0x8cf
	.long	0x20cf
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x32
	.long	.LASF32
	.byte	0x1
	.value	0x8d0
	.long	0xc3
	.byte	0x2
	.byte	0x91
	.sleb128 8
	.uleb128 0x33
	.string	"size"
	.byte	0x1
	.value	0x8d2
	.long	0xc3
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.uleb128 0x31
	.long	.LASF31
	.byte	0x1
	.value	0x8d3
	.long	0x20cf
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.byte	0x0
	.uleb128 0x37
	.long	0x3a1e
	.byte	0x1
	.string	"_cpp_extend_buff"
	.byte	0x1
	.value	0x8e4
	.byte	0x1
	.long	.LFB61
	.long	.LFE61
	.long	.LLST41
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x8e1
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"pbuff"
	.byte	0x1
	.value	0x8e2
	.long	0x390f
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x32
	.long	.LASF32
	.byte	0x1
	.value	0x8e3
	.long	0xc3
	.byte	0x2
	.byte	0x91
	.sleb128 8
	.uleb128 0x31
	.long	.LASF31
	.byte	0x1
	.value	0x8e5
	.long	0x20cf
	.byte	0x2
	.byte	0x91
	.sleb128 -32
	.uleb128 0x33
	.string	"old_buff"
	.byte	0x1
	.value	0x8e5
	.long	0x20cf
	.byte	0x2
	.byte	0x91
	.sleb128 -28
	.uleb128 0x33
	.string	"size"
	.byte	0x1
	.value	0x8e6
	.long	0xc3
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.byte	0x0
	.uleb128 0x37
	.long	0x3a62
	.byte	0x1
	.string	"_cpp_free_buff"
	.byte	0x1
	.value	0x8f2
	.byte	0x1
	.long	.LFB62
	.long	.LFE62
	.long	.LLST42
	.uleb128 0x32
	.long	.LASF27
	.byte	0x1
	.value	0x8f1
	.long	0x20cf
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x31
	.long	.LASF21
	.byte	0x1
	.value	0x8f3
	.long	0x20cf
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.byte	0x0
	.uleb128 0x38
	.long	0x3ace
	.byte	0x1
	.string	"_cpp_unaligned_alloc"
	.byte	0x1
	.value	0x901
	.byte	0x1
	.long	0x470
	.long	.LFB63
	.long	.LFE63
	.long	.LLST43
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x8ff
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"len"
	.byte	0x1
	.value	0x900
	.long	0xc3
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x31
	.long	.LASF27
	.byte	0x1
	.value	0x902
	.long	0x20cf
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x31
	.long	.LASF28
	.byte	0x1
	.value	0x903
	.long	0x470
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.byte	0x0
	.uleb128 0x38
	.long	0x3b38
	.byte	0x1
	.string	"_cpp_aligned_alloc"
	.byte	0x1
	.value	0x91f
	.byte	0x1
	.long	0x470
	.long	.LFB64
	.long	.LFE64
	.long	.LLST44
	.uleb128 0x32
	.long	.LASF5
	.byte	0x1
	.value	0x91d
	.long	0x1cd6
	.byte	0x2
	.byte	0x91
	.sleb128 0
	.uleb128 0x2b
	.string	"len"
	.byte	0x1
	.value	0x91e
	.long	0xc3
	.byte	0x2
	.byte	0x91
	.sleb128 4
	.uleb128 0x31
	.long	.LASF27
	.byte	0x1
	.value	0x920
	.long	0x20cf
	.byte	0x2
	.byte	0x91
	.sleb128 -20
	.uleb128 0x31
	.long	.LASF28
	.byte	0x1
	.value	0x921
	.long	0x470
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.byte	0x0
	.uleb128 0xa
	.long	0x3b48
	.long	0x465
	.uleb128 0xb
	.long	0xbc
	.byte	0x5
	.byte	0x0
	.uleb128 0x2f
	.string	"digraph_spellings"
	.byte	0x1
	.byte	0x3c
	.long	0x3b67
	.byte	0x5
	.byte	0x3
	.long	digraph_spellings
	.uleb128 0x3
	.long	0x3b38
	.uleb128 0xa
	.long	0x3b7c
	.long	0x2456
	.uleb128 0xb
	.long	0xbc
	.byte	0x43
	.byte	0x0
	.uleb128 0x2f
	.string	"token_spellings"
	.byte	0x1
	.byte	0x41
	.long	0x3b99
	.byte	0x5
	.byte	0x3
	.long	token_spellings
	.uleb128 0x3
	.long	0x3b6c
	.uleb128 0x33
	.string	"current_pragma"
	.byte	0x1
	.value	0x353
	.long	0x2df2
	.byte	0x5
	.byte	0x3
	.long	current_pragma
	.uleb128 0x3b
	.string	"stderr"
	.byte	0x8
	.byte	0x90
	.long	0x4cd
	.byte	0x1
	.byte	0x1
	.uleb128 0xa
	.long	0x3bdb
	.long	0xe2
	.uleb128 0xb
	.long	0xbc
	.byte	0xff
	.byte	0x0
	.uleb128 0x3b
	.string	"_sch_istable"
	.byte	0xc
	.byte	0x48
	.long	0x3bf1
	.byte	0x1
	.byte	0x1
	.uleb128 0x3
	.long	0x3bcb
	.uleb128 0xa
	.long	0x3c06
	.long	0xa6
	.uleb128 0xb
	.long	0xbc
	.byte	0xff
	.byte	0x0
	.uleb128 0x3c
	.string	"_hex_value"
	.byte	0xd
	.value	0x101
	.long	0x3c1b
	.byte	0x1
	.byte	0x1
	.uleb128 0x3
	.long	0x3bf6
	.uleb128 0xa
	.long	0x3c30
	.long	0xd1
	.uleb128 0xb
	.long	0xbc
	.byte	0xff
	.byte	0x0
	.uleb128 0x3c
	.string	"_cpp_trigraph_map"
	.byte	0x2
	.value	0x1e1
	.long	0x3c4c
	.byte	0x1
	.byte	0x1
	.uleb128 0x3
	.long	0x3c20
	.uleb128 0x3b
	.string	"in_omp_pragma"
	.byte	0x1
	.byte	0x73
	.long	0xcf1
	.byte	0x1
	.byte	0x1
	.uleb128 0x3d
	.string	"seen_omp_paren"
	.byte	0x1
	.byte	0x74
	.long	0xcf1
	.byte	0x1
	.byte	0x5
	.byte	0x3
	.long	seen_omp_paren
	.uleb128 0x3e
	.string	"last_token_omp_hash"
	.byte	0x1
	.value	0x354
	.long	0xcf1
	.byte	0x1
	.byte	0x5
	.byte	0x3
	.long	last_token_omp_hash
	.uleb128 0x3c
	.string	"flag_openmp"
	.byte	0x1
	.value	0x39c
	.long	0xae
	.byte	0x1
	.byte	0x1
	.byte	0x0
	.section	.debug_abbrev
	.uleb128 0x1
	.uleb128 0x11
	.byte	0x1
	.uleb128 0x10
	.uleb128 0x6
	.uleb128 0x12
	.uleb128 0x1
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x25
	.uleb128 0x8
	.uleb128 0x13
	.uleb128 0xb
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x1b
	.uleb128 0x8
	.byte	0x0
	.byte	0x0
	.uleb128 0x2
	.uleb128 0xf
	.byte	0x0
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.byte	0x0
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x26
	.byte	0x0
	.uleb128 0x49
	.uleb128 0x13
	.byte	0x0
	.byte	0x0
	.uleb128 0x4
	.uleb128 0x24
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3e
	.uleb128 0xb
	.byte	0x0
	.byte	0x0
	.uleb128 0x5
	.uleb128 0x24
	.byte	0x0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3e
	.uleb128 0xb
	.byte	0x0
	.byte	0x0
	.uleb128 0x6
	.uleb128 0x16
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.byte	0x0
	.byte	0x0
	.uleb128 0x7
	.uleb128 0xf
	.byte	0x0
	.uleb128 0xb
	.uleb128 0xb
	.byte	0x0
	.byte	0x0
	.uleb128 0x8
	.uleb128 0x13
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.byte	0x0
	.byte	0x0
	.uleb128 0x9
	.uleb128 0xd
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x38
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0xa
	.uleb128 0x1
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x49
	.uleb128 0x13
	.byte	0x0
	.byte	0x0
	.uleb128 0xb
	.uleb128 0x21
	.byte	0x0
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2f
	.uleb128 0xb
	.byte	0x0
	.byte	0x0
	.uleb128 0xc
	.uleb128 0x16
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.byte	0x0
	.byte	0x0
	.uleb128 0xd
	.uleb128 0x13
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.byte	0x0
	.byte	0x0
	.uleb128 0xe
	.uleb128 0xd
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x38
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0xf
	.uleb128 0xd
	.byte	0x0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x38
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0x10
	.uleb128 0x13
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.byte	0x0
	.byte	0x0
	.uleb128 0x11
	.uleb128 0xd
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0xd
	.uleb128 0xb
	.uleb128 0xc
	.uleb128 0xb
	.uleb128 0x38
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0x12
	.uleb128 0x15
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x27
	.uleb128 0xc
	.uleb128 0x49
	.uleb128 0x13
	.byte	0x0
	.byte	0x0
	.uleb128 0x13
	.uleb128 0x5
	.byte	0x0
	.uleb128 0x49
	.uleb128 0x13
	.byte	0x0
	.byte	0x0
	.uleb128 0x14
	.uleb128 0x15
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x27
	.uleb128 0xc
	.byte	0x0
	.byte	0x0
	.uleb128 0x15
	.uleb128 0x16
	.byte	0x0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.byte	0x0
	.byte	0x0
	.uleb128 0x16
	.uleb128 0x13
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0xb
	.uleb128 0x5
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.byte	0x0
	.byte	0x0
	.uleb128 0x17
	.uleb128 0xd
	.byte	0x0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x38
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0x18
	.uleb128 0x4
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.byte	0x0
	.byte	0x0
	.uleb128 0x19
	.uleb128 0x28
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x1c
	.uleb128 0xd
	.byte	0x0
	.byte	0x0
	.uleb128 0x1a
	.uleb128 0xd
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0xd
	.uleb128 0xb
	.uleb128 0xc
	.uleb128 0xb
	.uleb128 0x38
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0x1b
	.uleb128 0x17
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.byte	0x0
	.byte	0x0
	.uleb128 0x1c
	.uleb128 0xd
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.byte	0x0
	.byte	0x0
	.uleb128 0x1d
	.uleb128 0x4
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.byte	0x0
	.byte	0x0
	.uleb128 0x1e
	.uleb128 0x13
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.byte	0x0
	.byte	0x0
	.uleb128 0x1f
	.uleb128 0x13
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3c
	.uleb128 0xc
	.byte	0x0
	.byte	0x0
	.uleb128 0x20
	.uleb128 0x4
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.byte	0x0
	.byte	0x0
	.uleb128 0x21
	.uleb128 0x17
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.byte	0x0
	.byte	0x0
	.uleb128 0x22
	.uleb128 0xd
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x49
	.uleb128 0x13
	.byte	0x0
	.byte	0x0
	.uleb128 0x23
	.uleb128 0x17
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.byte	0x0
	.byte	0x0
	.uleb128 0x24
	.uleb128 0xd
	.byte	0x0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.byte	0x0
	.byte	0x0
	.uleb128 0x25
	.uleb128 0x13
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.byte	0x0
	.byte	0x0
	.uleb128 0x26
	.uleb128 0x13
	.byte	0x0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3c
	.uleb128 0xc
	.byte	0x0
	.byte	0x0
	.uleb128 0x27
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3f
	.uleb128 0xc
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0xc
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.uleb128 0x40
	.uleb128 0x6
	.byte	0x0
	.byte	0x0
	.uleb128 0x28
	.uleb128 0x5
	.byte	0x0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0x29
	.uleb128 0x5
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0x2a
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x27
	.uleb128 0xc
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.uleb128 0x40
	.uleb128 0x6
	.byte	0x0
	.byte	0x0
	.uleb128 0x2b
	.uleb128 0x5
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0x2c
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0xc
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.uleb128 0x40
	.uleb128 0x6
	.byte	0x0
	.byte	0x0
	.uleb128 0x2d
	.uleb128 0x34
	.byte	0x0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0x2e
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0xc
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.uleb128 0x40
	.uleb128 0x6
	.byte	0x0
	.byte	0x0
	.uleb128 0x2f
	.uleb128 0x34
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0x30
	.uleb128 0xb
	.byte	0x1
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.byte	0x0
	.byte	0x0
	.uleb128 0x31
	.uleb128 0x34
	.byte	0x0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0x32
	.uleb128 0x5
	.byte	0x0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0x33
	.uleb128 0x34
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0x34
	.uleb128 0xa
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x11
	.uleb128 0x1
	.byte	0x0
	.byte	0x0
	.uleb128 0x35
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x27
	.uleb128 0xc
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.uleb128 0x40
	.uleb128 0x6
	.byte	0x0
	.byte	0x0
	.uleb128 0x36
	.uleb128 0xb
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.byte	0x0
	.byte	0x0
	.uleb128 0x37
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3f
	.uleb128 0xc
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x27
	.uleb128 0xc
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.uleb128 0x40
	.uleb128 0x6
	.byte	0x0
	.byte	0x0
	.uleb128 0x38
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3f
	.uleb128 0xc
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x27
	.uleb128 0xc
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.uleb128 0x40
	.uleb128 0x6
	.byte	0x0
	.byte	0x0
	.uleb128 0x39
	.uleb128 0xa
	.byte	0x0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x11
	.uleb128 0x1
	.byte	0x0
	.byte	0x0
	.uleb128 0x3a
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x1
	.uleb128 0x13
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x27
	.uleb128 0xc
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.uleb128 0x40
	.uleb128 0x6
	.byte	0x0
	.byte	0x0
	.uleb128 0x3b
	.uleb128 0x34
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x3f
	.uleb128 0xc
	.uleb128 0x3c
	.uleb128 0xc
	.byte	0x0
	.byte	0x0
	.uleb128 0x3c
	.uleb128 0x34
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x3f
	.uleb128 0xc
	.uleb128 0x3c
	.uleb128 0xc
	.byte	0x0
	.byte	0x0
	.uleb128 0x3d
	.uleb128 0x34
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x3f
	.uleb128 0xc
	.uleb128 0x2
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.uleb128 0x3e
	.uleb128 0x34
	.byte	0x0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0x5
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x3f
	.uleb128 0xc
	.uleb128 0x2
	.uleb128 0xa
	.byte	0x0
	.byte	0x0
	.byte	0x0
	.section	.debug_pubnames,"",@progbits
	.long	0x206
	.value	0x2
	.long	.Ldebug_info0
	.long	0x3cbf
	.long	0x2490
	.string	"cpp_ideq"
	.long	0x2d19
	.string	"_cpp_init_tokenrun"
	.long	0x2d9a
	.string	"_cpp_temp_token"
	.long	0x2f0a
	.string	"_cpp_lex_token"
	.long	0x2fca
	.string	"_cpp_lex_direct"
	.long	0x3136
	.string	"cpp_token_len"
	.long	0x317d
	.string	"cpp_spell_token"
	.long	0x3245
	.string	"cpp_token_as_text"
	.long	0x32bf
	.string	"cpp_type2name"
	.long	0x32f8
	.string	"cpp_output_token"
	.long	0x33ad
	.string	"_cpp_equiv_tokens"
	.long	0x33f4
	.string	"cpp_avoid_paste"
	.long	0x3479
	.string	"cpp_output_line"
	.long	0x35aa
	.string	"cpp_parse_escape"
	.long	0x36d3
	.string	"cpp_interpret_charconst"
	.long	0x3837
	.string	"_cpp_release_buff"
	.long	0x388d
	.string	"_cpp_get_buff"
	.long	0x3915
	.string	"_cpp_append_extend_buff"
	.long	0x3994
	.string	"_cpp_extend_buff"
	.long	0x3a1e
	.string	"_cpp_free_buff"
	.long	0x3a62
	.string	"_cpp_unaligned_alloc"
	.long	0x3ace
	.string	"_cpp_aligned_alloc"
	.long	0x3c68
	.string	"seen_omp_paren"
	.long	0x3c85
	.string	"last_token_omp_hash"
	.long	0x0
	.section	.debug_aranges,"",@progbits
	.long	0x1c
	.value	0x2
	.long	.Ldebug_info0
	.byte	0x4
	.byte	0x0
	.value	0x0
	.value	0x0
	.long	.Ltext0
	.long	.Letext0-.Ltext0
	.long	0x0
	.long	0x0
	.section	.debug_str,"",@progbits
.LASF8:
	.string	"line_maps"
.LASF12:
	.string	"rlimit"
.LASF19:
	.string	"cpp_macro"
.LASF21:
	.string	"next"
.LASF10:
	.string	"spec_nodes"
.LASF5:
	.string	"pfile"
.LASF29:
	.string	"spell_ident"
.LASF17:
	.string	"cpp_string"
.LASF24:
	.string	"token"
.LASF20:
	.string	"cpp_callbacks"
.LASF14:
	.string	"saved_cur"
.LASF26:
	.string	"cpp_context"
.LASF32:
	.string	"min_extra"
.LASF11:
	.string	"cpp_buffer"
.LASF9:
	.string	"directive"
.LASF30:
	.string	"spelling"
.LASF22:
	.string	"_cpp_buff"
.LASF28:
	.string	"result"
.LASF27:
	.string	"buff"
.LASF13:
	.string	"if_stack"
.LASF16:
	.string	"cpp_token"
.LASF15:
	.string	"cpp_options"
.LASF2:
	.string	"limit"
.LASF6:
	.string	"cpp_reader"
.LASF3:
	.string	"ht_identifier"
.LASF1:
	.string	"_IO_FILE"
.LASF7:
	.string	"buffer"
.LASF4:
	.string	"hash_table"
.LASF18:
	.string	"cpp_hashnode"
.LASF25:
	.string	"tokenrun"
.LASF23:
	.string	"base"
.LASF31:
	.string	"new_buff"
.LASF0:
	.string	"unsigned int"
	.ident	"GCC: (GNU) 4.1.2 (Ubuntu 4.1.2-0ubuntu4)"
	.section	.text.__i686.get_pc_thunk.cx,"axG",@progbits,__i686.get_pc_thunk.cx,comdat
.globl __i686.get_pc_thunk.cx
	.hidden	__i686.get_pc_thunk.cx
	.type	__i686.get_pc_thunk.cx, @function
__i686.get_pc_thunk.cx:
	movl	(%esp), %ecx
	ret
	.section	.text.__i686.get_pc_thunk.bx,"axG",@progbits,__i686.get_pc_thunk.bx,comdat
.globl __i686.get_pc_thunk.bx
	.hidden	__i686.get_pc_thunk.bx
	.type	__i686.get_pc_thunk.bx, @function
__i686.get_pc_thunk.bx:
	movl	(%esp), %ebx
	ret
	.section	.note.GNU-stack,"",@progbits
