...
# 34 "../../../kg++fe/gnu/cp/error.c" 2
# 1 "../../../kg++fe/gnu/langhooks-def.h" 1
# 25 "../../../kg++fe/gnu/langhooks-def.h"
# 1 "../../../kg++fe/gnu/hooks.h" 1
# 31 "../../../kg++fe/gnu/hooks.h"
# 1 "../../../kg++fe/gnu/rtl.h" 1
# 25 "../../../kg++fe/gnu/rtl.h"
struct function;
...
_Bool default_can_output_mi_thunk_no_vcall
  (tree, long long, long long, tree);
# 26 "../../../kg++fe/gnu/langhooks-def.h" 2

struct diagnostic_context;



extern long long hook_get_alias_set_0 (tree);
# 44 "../../../kg++fe/gnu/langhooks-def.h"
extern void lhd_do_nothing (void);
extern void lhd_do_nothing_t (tree);
extern void lhd_do_nothing_i (int);
extern void lhd_do_nothing_f (struct function *);
extern int lhd_decode_option (int, char **);
extern long long lhd_get_alias_set (tree);
...
extern _Bool lhd_can_use_bit_fields_p (void);
extern _Bool lhd_warn_unused_global_decl (tree);
extern void lhd_incomplete_type_error (tree, tree);
extern tree lhd_type_promotes_to (tree);
extern _Bool lhd_decl_ok_for_sibcall (tree);
extern tree lhd_expr_size (tree);


tree lhd_tree_inlining_walk_subtrees (tree *, int *, walk_tree_fn, void *, void *);


int lhd_tree_inlining_cannot_inline_tree_fn (tree *);
int lhd_tree_inlining_disregard_inline_limits (tree);
tree lhd_tree_inlining_add_pending_fn_decls (void *, tree);
int lhd_tree_inlining_tree_chain_matters_p (tree);
int lhd_tree_inlining_auto_var_in_fn_p (tree, tree);
tree lhd_tree_inlining_copy_res_decl_for_inlining (tree, tree, tree, void *, int *, void *);


int lhd_tree_inlining_anon_aggr_type_p (tree);
int lhd_tree_inlining_start_inlining (tree);
void lhd_tree_inlining_end_inlining (tree);
tree lhd_tree_inlining_convert_parm_for_inlining (tree, tree, tree);

void write_global_declarations (void);
# 178 "../../../kg++fe/gnu/langhooks-def.h"
int lhd_tree_dump_dump_tree (void *, tree);
int lhd_tree_dump_type_quals (tree);
# 35 "../../../kg++fe/gnu/cp/error.c" 2

# 1 "../../../kg++fe/gnu/defaults.h" 1
# 37 "../../../kg++fe/gnu/cp/error.c" 2


enum pad { none, before, after };
# 62 "../../../kg++fe/gnu/cp/error.c"
...
static void print_instantiation_partial_context (diagnostic_context *, tree, const char *, int);
...
static _Bool cp_printer (output_buffer *, text_info *);
static void print_non_consecutive_character (output_buffer *, int);
static void print_integer (output_buffer *, long long);
...
init_error ()
{
  ...
  (global_dc)->end_diagnostic = cp_diagnostic_finalizer;
  (&(global_dc)->buffer)->format_decoder = cp_printer;

  init_output_buffer (scratch_buffer, ((void *)0), 0);
}
     ...
     int flags;
{
  ...
  else if ((((enum tree_code) (scope)->common.code) == ARRAY_TYPE || ((enum tree_code) (scope)->common.code) == RECORD_TYPE || ((enum tree_code) (scope)->common.code) == UNION_TYPE || ((enum tree_code) (scope)->common.code) == QUAL_UNION_TYPE || ((enum tree_code) (scope)->common.code) == SET_TYPE))
    {
      dump_type (scope, f);
      output_add_string ((scratch_buffer), "::");
    }
  else if ((flags & (1)) && ((enum tree_code) (scope)->common.code) == FUNCTION_DECL)
    {
     ...
     enum pad p;
{
  ...
  int ix;
  int quals = ((((t)->common.readonly_flag) * 0x1) | (((t)->common.volatile_flag) * 0x2) | (((t)->type.restrict_flag) * 0x4) | ((((enum tree_code) (t)->common.code) == RECORD_TYPE && ((t)->common.type)) * 0x8));
        ...
        if (masks[ix] & quals)
          {
            ...
            p = before;
            output_add_string ((scratch_buffer), (names[ix]));
          }
      if (do_after)
        output_add_space (scratch_buffer);
    }
...
dump_template_argument (arg, flags)
     ...
     int flags;
{
  if ((tree_code_type[(int) (((enum tree_code) (arg)->common.code))] == 't') || ((enum tree_code) (arg)->common.code) == TEMPLATE_DECL)
    dump_type (arg, flags & ~(1 << 3));
  else
    dump_expr (arg, (flags | (1 << 9)) & ~(1 << 3));
}
  ...
  for (i = 0; i< n; ++i)
    {
      if (need_comma)
        output_add_string ((scratch_buffer), ", ");
      dump_template_argument (((args)->vec.a[i]), flags);
      need_comma = 1;
    }
}
...
dump_template_parameter (parm, flags)
     ...
     int flags;
{
  tree p = ((parm)->list.value);
  tree a = ((parm)->list.purpose);

  if (((enum tree_code) (p)->common.code) == TYPE_DECL)
    {
      if (flags & (1 << 2))
        {
          ...
          if (((p)->decl.name))
            {
              output_add_space (scratch_buffer);
              output_add_string ((scratch_buffer), ((char *)(((p)->decl.name))->identifier.id.str));
            }
        }
      else if (((p)->decl.name))
        output_add_string ((scratch_buffer), ((char *)(((p)->decl.name))->identifier.id.str));
      else
        output_add_string ((scratch_buffer), ("{template default argument error}"));
    }
  else
    dump_decl (p, flags | (1 << 2));

  if ((flags & (1 << 5)) && a != (tree) ((void *)0))
    {
      output_add_string (scratch_buffer, " = ");
      if (((enum tree_code) (p)->common.code) == TYPE_DECL || ((enum tree_code) (p)->common.code) == TEMPLATE_DECL)
        dump_type (a, flags & ~(1 << 1));
      else
        dump_expr (a, flags | (1 << 9));
    }
}
     ...
     tree parms, args;
{
  ...
  while (parms)
    {
      tree p = ((parms)->list.value);
      int lvl = ((long long) (((((parms)->list.purpose))->int_cst.int_cst).low));
      int arg_idx = 0;
      int i;

      for (i = 0; i < ((p)->vec.length); ++i)
 {
   tree arg = (tree) ((void *)0);


   if ((((args) != (tree) ((void *)0) && ((enum tree_code) (args)->common.code) == TREE_VEC && ((args)->vec.length) > 0 && ((args)->vec.a[0]) != (tree) ((void *)0) && ((enum tree_code) (((args)->vec.a[0]))->common.code) == TREE_VEC) ? ((args)->vec.length) : 1) >= lvl)
     {
       tree lvl_args = (((args) != (tree) ((void *)0) && ((enum tree_code) (args)->common.code) == TREE_VEC && ((args)->vec.length) > 0 && ((args)->vec.a[0]) != (tree) ((void *)0) && ((enum tree_code) (((args)->vec.a[0]))->common.code) == TREE_VEC) ? ((args)->vec.a[(lvl) - 1]) : (args));
       if (((lvl_args) == (tree) ((void *)0) ? 0 : (((enum tree_code) (lvl_args)->common.code) == TREE_VEC ? ((lvl_args)->vec.length) : list_length (lvl_args))) > arg_idx)
  arg = ((lvl_args)->vec.a[arg_idx]);
     }

   if (need_comma)
     output_add_string ((scratch_buffer), ", ");
   dump_template_parameter (((p)->vec.a[i]), (0));
   output_add_string (scratch_buffer, " = ");
   if (arg)
     dump_template_argument (arg, (0));
   else
     output_add_string ((scratch_buffer), ("<missing>"));
     ...
     int flags;
{
  ...
  switch (((enum tree_code) (t)->common.code))
    {
    case LANG_TYPE:
      output_add_string ((scratch_buffer), ("<unknown type>"));
      break;
    ...
    case IDENTIFIER_NODE:
      output_add_string ((scratch_buffer), ((char *)(t)->identifier.id.str));
      break;

    case TREE_VEC:
      dump_type (((t)->common.type), flags);
      break;
    ...
    case TYPE_DECL:
      if (flags & (1 << 1))
        {
          dump_type (((t)->decl.result)
                     ? ((t)->decl.result) : ((t)->common.type), flags);
          break;
        }
 ...
 tree elt = ((t)->common.type);
 elt = c_common_type_for_mode (((elt)->type.mode), ((elt)->common.unsigned_flag));
 dump_type (elt, flags);
      }
      break;

    case INTEGER_TYPE:
      if (!((((t)->type.main_variant))->common.unsigned_flag) && ((t)->common.unsigned_flag))
 output_add_string (scratch_buffer, "unsigned ");
      else if (((((t)->type.main_variant))->common.unsigned_flag) && !((t)->common.unsigned_flag))
 ...
 type = flags & (1 << 1) ? ((t)->type.main_variant) : t;
 if (((type)->type.name) && (((((type)->type.name))->decl.name)))
   output_add_string ((scratch_buffer), ((char *)((((((type)->type.name))->decl.name)))->identifier.id.str));
 else



   output_add_string ((scratch_buffer), ("<anonymous>"));
      }
      break;

    case TEMPLATE_TEMPLATE_PARM:

      if ((((((t)->type.name))->decl.name)))
 output_add_string ((scratch_buffer), ((char *)((((((t)->type.name))->decl.name)))->identifier.id.str));
      else
 output_add_string ((scratch_buffer), ("<anonymous template template parameter>"));

      break;

    case BOUND_TEMPLATE_TEMPLATE_PARM:
      {
 tree args = (((((((enum tree_code) (t)->common.code) == ENUMERAL_TYPE ? ((((t))->type.binfo)) : (((enum tree_code) (t)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM ? ((&(((t))->type.lang_specific)->u.c) ->template_info) : (((t)->type.lang_specific) ? ((&(((t))->type.lang_specific)->u.c)->template_info) : (tree) ((void *)0)))))->list.value)));
 output_add_string ((scratch_buffer), ((char *)((((((t)->type.name))->decl.name)))->identifier.id.str));
 print_non_consecutive_character ((scratch_buffer), '<');
        dump_template_argument_list (args, flags);
 print_non_consecutive_character ((scratch_buffer), '>');
      }
      break;
 ...
 output_add_string ((scratch_buffer), ("<anonymous template type parameter>"));
    ...
    case UNBOUND_CLASS_TEMPLATE:
      dump_type (((t)->type.context), flags);
      output_add_string ((scratch_buffer), "::");
      output_add_string ((scratch_buffer), ("template "));
      dump_type (((((t)->type.name))->decl.name), flags);
      break;

    case TYPEOF_TYPE:
      output_add_string (scratch_buffer, "__typeof (");
      dump_expr (((t)->type.values), flags & ~(1 << 9));
      output_add_character ((scratch_buffer), ')');
      break;

    default:
      sorry ("`%s' not supported by %s", tree_code_name[(int) ((enum tree_code) (t)->common.code)], __FUNCTION__);


    case ERROR_MARK:
      output_add_string ((scratch_buffer), ("<type error>"));
      break;
    }
}
...
dump_typename (t, flags)
     ...
     int flags;
{
  tree ctx = ((t)->type.context);

  if (((enum tree_code) (ctx)->common.code) == TYPENAME_TYPE)
    dump_typename (ctx, flags);
  else
    dump_type (ctx, flags & ~(1 << 3));
  output_add_string ((scratch_buffer), "::");
  dump_decl ((((t)->type.values)), flags);
}
     ...
     tree t;
{
  if (((enum tree_code) (t)->common.code) == ENUMERAL_TYPE)
    return "enum";
  else if (((enum tree_code) (t)->common.code) == UNION_TYPE)
    return "union";
  else if (((t)->type.lang_specific) && ((&((t)->type.lang_specific)->u.c)->declared_class))
    return "class";
  else
    return "struct";
}
     ...
     int flags;
{
  ...
  if (flags & (1 << 3))
    {
      output_add_string ((scratch_buffer), (variety));
      output_add_space (scratch_buffer);
    }
  ...
  if (name)
    {
      typdef = !((name)->decl.artificial_flag);
      tmplate = !typdef && ((enum tree_code) (t)->common.code) != ENUMERAL_TYPE
                && ((t)->type.lang_specific) && ((&(((t))->type.lang_specific)->u.c)->template_info)
                && ((((&((t)->type.lang_specific)->u.c)->use_template) == 2)
                    || ((enum tree_code) ((((((&(((t))->type.lang_specific)->u.c)->template_info))->list.purpose)))->common.code) != TEMPLATE_DECL
                    || (((((((((&(((t))->type.lang_specific)->u.c)->template_info))->list.purpose)))->decl.lang_specific)->decl_flags.use_template) == 2)
      ...
      dump_scope ((((name)->decl.context) ? ((name)->decl.context) : global_namespace), flags | (1));
      if (tmplate)
        {


          tree tpl = (((((&(((t))->type.lang_specific)->u.c)->template_info))->list.purpose));

          while (((((tpl))->decl.lang_specific) ->decl_flags.u.template_info))
            tpl = (((((((tpl))->decl.lang_specific) ->decl_flags.u.template_info))->list.purpose));
          name = tpl;
        }
      name = ((name)->decl.name);
    }

  if (name == 0 || (((char *)(name)->identifier.id.str)[0] == '.' && ((char *)(name)->identifier.id.str)[1] == '_'))
    {
      if (flags & (1 << 3))
        output_add_string ((scratch_buffer), ("<anonymous>"));
      else
    ...
    output_add_string ((scratch_buffer), ((char *)(name)->identifier.id.str));
  if (tmplate)
    dump_template_parms ((((enum tree_code) (t)->common.code) == ENUMERAL_TYPE ? ((((t))->type.binfo)) : (((enum tree_code) (t)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM ? ((&(((t))->type.lang_specific)->u.c) ->template_info) : (((t)->type.lang_specific) ? ((&(((t))->type.lang_specific)->u.c)->template_info) : (tree) ((void *)0)))),
                         !((&((t)->type.lang_specific)->u.c)->use_template),
                         flags & ~(1 << 7));
}
# 602 "../../../kg++fe/gnu/cp/error.c"
static enum pad
dump_type_prefix (t, flags)
     tree t;
     int flags;
{
 ...
 if (!(((enum tree_code) (t)->common.code) == POINTER_TYPE && ((enum tree_code) (((t)->common.type))->common.code) == OFFSET_TYPE))
   {
     if (((enum tree_code) (sub)->common.code) == ARRAY_TYPE)
              {
                output_add_space (scratch_buffer);
                output_add_character ((scratch_buffer), '(');
              }
            output_add_character
              (scratch_buffer, "&*"[((enum tree_code) (t)->common.code) == POINTER_TYPE]);
    ...
    case OFFSET_TYPE:
    offset_type:
      padding = dump_type_prefix (((t)->common.type), flags);
      if (((enum tree_code) (t)->common.code) == OFFSET_TYPE)
 {
   if (padding != none)
     output_add_space (scratch_buffer);
   dump_type (((t)->type.maxval), flags);
   output_add_string ((scratch_buffer), "::");
 }
      output_add_character (scratch_buffer, '*');
    ...
    case METHOD_TYPE:
      ...
      output_add_character ((scratch_buffer), '(');
      padding = none;
      dump_aggr_type (((t)->type.maxval), flags);
      output_add_string ((scratch_buffer), "::");
      break;
    ...
    case ERROR_MARK:
      output_add_string ((scratch_buffer), ("<typeprefixerror>"));
      break;
    }
  return padding;
}
     ...
     int flags;
{
 ...
 output_add_character ((scratch_buffer), ')');
      dump_type_suffix (((t)->common.type), flags);
      break;
 ...
 tree arg;
        output_add_character ((scratch_buffer), ')');
 arg = ((t)->type.values);
 if (((enum tree_code) (t)->common.code) == METHOD_TYPE)
   arg = ((arg)->common.chain);



 dump_parameters (arg, flags & ~(1 << 5));

 if (((enum tree_code) (t)->common.code) == METHOD_TYPE)
   dump_qualifiers
     (((((((t)->type.values))->list.value))->common.type), before);
 dump_exception_spec (((t)->type.binfo), flags);
 dump_type_suffix (((t)->common.type), flags);
 break;
      }
    ...
    case ARRAY_TYPE:
      ...
      if (((t)->type.values))
 {
   if (host_integerp (((((t)->type.values))->type.maxval), 0))
     print_integer
              (scratch_buffer,
               tree_low_cst (((((t)->type.values))->type.maxval), 0) + 1);
   else if (((enum tree_code) (((((t)->type.values))->type.maxval))->common.code) == MINUS_EXPR)
     dump_expr (((((((t)->type.values))->type.maxval))->exp.operands[0]),
                flags & ~(1 << 9));
   else
     dump_expr (fold (build_binary_op(PLUS_EXPR, ((((t)->type.values))->type.maxval), global_trees[TI_INTEGER_ONE], 1)),
     ...
     tree t;
{
  ...
  if ((((t)->decl.lang_specific)->decl_flags.global_ctor_p))
    p = "initializers";
  else if ((((t)->decl.lang_specific)->decl_flags.global_dtor_p))
    p = "destructors";
  else
    fancy_abort ("../../../kg++fe/gnu/cp/error.c", 815, __FUNCTION__);
     ...
     int flags;
{
  ...
  if (!((t)->decl.initial) || ((enum tree_code) (((t)->decl.initial))->common.code) != TEMPLATE_PARM_INDEX)
    dump_scope ((((t)->decl.context) ? ((t)->decl.context) : global_namespace), flags);
  if (((t)->decl.name))
    dump_decl (((t)->decl.name), flags);
  else
    output_add_string ((scratch_buffer), ("<anonymous>"));
  if (flags & (1 << 2))
     ...
     int flags;
{
  ...
  switch (((enum tree_code) (t)->common.code))
   ...
   {
     if ((flags & (1 << 2))
         && ((enum tree_code) (((t)->common.type))->common.code) == TEMPLATE_TYPE_PARM)
 ...
 output_add_string (scratch_buffer, "typedef ");
      dump_simple_decl (t, ((t)->decl.result)
   ? ((t)->decl.result) : ((t)->common.type),
                 flags);
      break;

    case VAR_DECL:
      if (((t)->decl.name) && (((char *)(((t)->decl.name))->identifier.id.str)[1] == 'v' && ((char *)(((t)->decl.name))->identifier.id.str)[2] == 't' && ((char *)(((t)->decl.name))->identifier.id.str)[3] == '.'))
 {
   output_add_string (scratch_buffer, "vtable for ");
   (void) ((((tree_code_type[(int) (((enum tree_code) (((t)->decl.context))->common.code))] == 't')) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/error.c", 878, __FUNCTION__), 0) : 0);
   dump_type (((t)->decl.context), flags);
   break;
 }

    case FIELD_DECL:
    case PARM_DECL:
      dump_simple_decl (t, ((t)->common.type), flags);
      break;
    ...
    case NAMESPACE_DECL:
      dump_scope ((((t)->decl.context) ? ((t)->decl.context) : global_namespace), flags);
      if (((t)->decl.name) == anonymous_namespace_name)
 output_add_string ((scratch_buffer), ("<unnamed>"));
      else
 output_add_string ((scratch_buffer), ((char *)(((t)->decl.name))->identifier.id.str));
      break;

    case SCOPE_REF:
      dump_decl (((t)->exp.operands[0]), flags & ~(1 << 2));
      output_add_string ((scratch_buffer), "::");
      dump_decl (((t)->exp.operands[1]), flags);
      break;

    case ARRAY_REF:
      dump_decl (((t)->exp.operands[0]), flags);
      output_add_character ((scratch_buffer), '[');
      dump_decl (((t)->exp.operands[1]), flags);
      output_add_character ((scratch_buffer), ']');
      break;
    ...
    case TYPE_EXPR:
      fancy_abort ("../../../kg++fe/gnu/cp/error.c", 922, __FUNCTION__);
      break;
      ...
      if (((t)->common.chain))
 {
   ...
   else if (((t)->decl.context))
     {
       dump_decl (((t)->decl.context), flags);
       output_add_string (scratch_buffer, "::");
     }
   dump_decl (((t)->decl.name), flags);
   break;
    ...
    case FUNCTION_DECL:
      if ((((t)->decl.lang_specific)->decl_flags.global_ctor_p) || (((t)->decl.lang_specific)->decl_flags.global_dtor_p))
 dump_global_iord (t);
      else if (! ((t)->decl.lang_specific))
 output_add_string ((scratch_buffer), ("<internal>"));
      else
 ...
 print_non_consecutive_character ((scratch_buffer), '<');
 for (args = ((t)->exp.operands[1]); args; args = ((args)->common.chain))
   {
     dump_template_argument (((args)->list.value), flags);
     if (((args)->common.chain))
       output_add_string ((scratch_buffer), ", ");
   }
 print_non_consecutive_character ((scratch_buffer), '>');
      }
    ...
    case CONST_DECL:
      if ((((t)->common.type) != (tree) ((void *)0) && (((enum tree_code) (((t)->common.type))->common.code)) == ENUMERAL_TYPE)
   || (((t)->decl.initial) &&
       ((enum tree_code) (((t)->decl.initial))->common.code) == TEMPLATE_PARM_INDEX))
 dump_simple_decl (t, ((t)->common.type), flags);
      else if (((t)->decl.name))
 dump_decl (((t)->decl.name), flags);
      else if (((t)->decl.initial))
 dump_expr (((t)->decl.initial), flags | (1 << 9));
      else
 output_add_string ((scratch_buffer), ("enumerator"));
      break;

    case USING_DECL:
      output_add_string (scratch_buffer, "using ");
      dump_type (((t)->decl.initial), flags);
      output_add_string ((scratch_buffer), "::");
      dump_decl (((t)->decl.name), flags);
      break;
    ...
    case ERROR_MARK:
      output_add_string ((scratch_buffer), ("<declaration error>"));
      break;
    }
}
...
dump_template_decl (t, flags)
     ...
     int flags;
{
  tree orig_parms = ((t)->decl.arguments);
  tree parms;
  int i;
  ...
  if (flags & (1 << 7))
    {
      for (parms = orig_parms = nreverse (orig_parms);
           ...
           parms = ((parms)->common.chain))
        {
   tree inner_parms = ((parms)->list.value);
          int len = ((inner_parms)->vec.length);
          ...
          for (i = 0; i < len; i++)
            {
              if (i)
                output_add_string ((scratch_buffer), ", ");
              dump_template_parameter (((inner_parms)->vec.a[i]), flags);
            }
          print_non_consecutive_character ((scratch_buffer), '>');
          output_add_space (scratch_buffer);
        }
 ...
 output_add_string (scratch_buffer, "class ");
  ...
  if (((enum tree_code) (((t)->decl.result))->common.code) == TYPE_DECL)
    dump_type (((t)->common.type),
               ((flags & ~(1 << 3)) | (1 << 8)
                | (flags & (1 << 2) ? (1 << 3) : 0)));
  else if (((enum tree_code) (((t)->decl.result))->common.code) == VAR_DECL)
    dump_decl (((t)->decl.result), flags | (1 << 8));
  else if (((t)->common.type) == (tree) ((void *)0))
    fancy_abort ("../../../kg++fe/gnu/cp/error.c", 1085, __FUNCTION__);
  else
    switch ((((enum tree_code) (((t)->common.type))->common.code)))
    {
      case METHOD_TYPE:
      case FUNCTION_TYPE:
        dump_function_decl (t, flags | (1 << 8));
        break;
      default:
     ...
     int flags;
{
  tree fntype;
  tree parmtypes;
  tree cname = (tree) ((void *)0);
  tree template_args = (tree) ((void *)0);
  tree template_parms = (tree) ((void *)0);
  int show_return = flags & (1 << 4) || flags & (1 << 2);

  if (((enum tree_code) (t)->common.code) == TEMPLATE_DECL)
    t = ((t)->decl.result);


  if ((((t)->decl.lang_specific)->decl_flags.use_template) && ((((t))->decl.lang_specific) ->decl_flags.u.template_info))
    {
      tree tmpl;

      template_args = (((((((t))->decl.lang_specific) ->decl_flags.u.template_info))->list.value));
      tmpl = most_general_template (t);
      if (tmpl && ((enum tree_code) (tmpl)->common.code) == TEMPLATE_DECL)
 {
   template_parms = ((tmpl)->decl.arguments);
   t = tmpl;
 }
    }

  fntype = ((t)->common.type);
  parmtypes = skip_artificial_parms_for ((t), ((((t)->common.type))->type.values));

  if ((((t)->decl.context) && (tree_code_type[(int) (((enum tree_code) (((t)->decl.context))->common.code))] == 't')))
    cname = ((t)->decl.context);

  else if (((enum tree_code) (fntype)->common.code) == METHOD_TYPE)
    cname = ((((parmtypes)->list.value))->common.type);

  if (!(flags & (1 << 2)))
            ;
  else if ((((t)->decl.lang_specific)->decl_flags.static_function))
    output_add_string ((scratch_buffer), ("static "));
  else if (((t)->decl.virtual_flag))
    output_add_string ((scratch_buffer), ("virtual "));


  if (show_return)
    show_return = !(((((t)->decl.name))->common.lang_flag_4)) && !(((t)->decl.lang_specific)->decl_flags.constructor_attr)
                  && !(((t)->decl.lang_specific)->decl_flags.destructor_attr);
  if (show_return)
    {
      dump_type_prefix (((fntype)->common.type), flags);
      output_add_space (scratch_buffer);
    }
 ...
 dump_qualifiers (((((((fntype)->type.values))->list.value))->common.type),
    before);

      if (flags & (1 << 6))
 dump_exception_spec (((fntype)->type.binfo), flags);

      if (show_return)
 dump_type_suffix (((fntype)->common.type), flags);
    }


  if (template_parms != (tree) ((void *)0) && template_args != (tree) ((void *)0))
    {
     ...
     int flags;
{
  ...
  for (first = 1; parmtypes != global_trees[TI_VOID_LIST_NODE];
       parmtypes = ((parmtypes)->common.chain))
    {
      ...
      dump_type (((parmtypes)->list.value), flags);

      if ((flags & (1 << 5)) && ((parmtypes)->list.purpose))
        {
          output_add_string (scratch_buffer, " = ");
          dump_expr (((parmtypes)->list.purpose), flags | (1 << 9));
        }
    }
     ...
     int flags;
{
  ...
  if ((((t)->decl.lang_specific)->decl_flags.constructor_attr)
      || (((t)->decl.lang_specific)->decl_flags.destructor_attr))
    name = constructor_name (((t)->decl.context));
  ...
  else if ((((((t)->decl.name))->common.lang_flag_4)))
    {
      ...
      output_add_string (scratch_buffer, "operator ");
      dump_type (((((t)->common.type))->common.type), flags);
    }
  else if (((name)->common.lang_flag_2))
    output_add_string ((scratch_buffer), ((char *)(name)->identifier.id.str));
  else
    dump_decl (name, flags);

  if (((t)->decl.lang_specific) && ((((t))->decl.lang_specific) ->decl_flags.u.template_info)
      && !(((((t))->decl.lang_specific) ->decl_flags.u.template_info) && !(((t)->decl.lang_specific)->decl_flags.use_template))
      && (((((t)->decl.lang_specific)->decl_flags.use_template) == 2)
   || ((enum tree_code) ((((((((t))->decl.lang_specific) ->decl_flags.u.template_info))->list.purpose)))->common.code) != TEMPLATE_DECL
   || (((((((((((t))->decl.lang_specific) ->decl_flags.u.template_info))->list.purpose)))->decl.lang_specific)->decl_flags.use_template) == 2)
   || (((((((((((((((t))->decl.lang_specific) ->decl_flags.u.template_info))->list.purpose)))->decl.arguments))->list.value))->common.type)) == ((((((((t))->decl.lang_specific) ->decl_flags.u.template_info))->list.purpose))))))
    dump_template_parms (((((t))->decl.lang_specific) ->decl_flags.u.template_info), !(((t)->decl.lang_specific)->decl_flags.use_template), flags);
}
...
dump_template_parms (info, primary, flags)
     ...
     int flags;
{
  tree args = info ? (((info)->list.value)) : (tree) ((void *)0);

  if (primary && flags & (1 << 8))
    return;
  flags &= ~((1 << 3) | (1 << 8));
  print_non_consecutive_character ((scratch_buffer), '<');
  ...
  if (args && !primary)
    {
      ...
      if (((enum tree_code) (args)->common.code) == TREE_VEC)
       ...
       && ((enum tree_code) (((args)->vec.a[0]))->common.code) == TREE_VEC)
     args = ((args)->vec.a[((args)->vec.length) - 1]);

          len = ((args)->vec.length);
        }
      else if (((enum tree_code) (args)->common.code) == TREE_LIST)
        len = -1;
      while (ix != len && args)
        {
          tree arg;
          ...
          if (!arg)
            output_add_string ((scratch_buffer), ("<template parameter error>"));
          else
            dump_template_argument (arg, flags);
          need_comma = 1;
        }
    ...
    {
      tree tpl = (((info)->list.purpose));
      tree parms = ((tpl)->decl.arguments);
      int len, ix;

      parms = ((enum tree_code) (parms)->common.code) == TREE_LIST ? ((parms)->list.value) : (tree) ((void *)0);
      len = parms ? ((parms)->vec.length) : 0;

      for (ix = 0; ix != len; ix++)
        {
          tree parm = ((((parms)->vec.a[ix]))->list.value);

          if (ix)
            output_add_string ((scratch_buffer), ", ");

          dump_decl (parm, flags & ~(1 << 2));
        }
    }
  print_non_consecutive_character ((scratch_buffer), '>');
}
     ...
     int flags;
{
  while (l)
    {
      dump_expr (((l)->list.value), flags | (1 << 9));
      l = ((l)->common.chain);
      if (l)
 output_add_string ((scratch_buffer), ", ");
    }
     ...
     int flags;
{
 ...
 (void) (((type != 0) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/error.c", 1474, __FUNCTION__), 0) : 0);


 if (((enum tree_code) (type)->common.code) == ENUMERAL_TYPE)
   {
     tree values = ((type)->type.values);

     for (; values;
          values = ((values)->common.chain))
       if (tree_int_cst_equal (((values)->list.value), t))
         break;

     if (values)
       output_add_string ((scratch_buffer), ((char *)(((values)->list.purpose))->identifier.id.str));
     else
       {
 ...
 else if (type == c_global_trees[CTI_BOOLEAN_TYPE])
   {
     if (t == c_global_trees[CTI_BOOLEAN_FALSE] || integer_zerop (t))
       output_add_string ((scratch_buffer), ("false"));
     else if (t == c_global_trees[CTI_BOOLEAN_TRUE])
       output_add_string ((scratch_buffer), ("true"));
   }
 else if (type == integer_types[itk_char])
   {
     output_add_character (scratch_buffer, '\'');
     if (host_integerp (t, ((type)->common.unsigned_flag)))
       dump_char (tree_low_cst (t, ((type)->common.unsigned_flag)));
     else
       output_printf (scratch_buffer, "\\x%x",
        (unsigned int) (((t)->int_cst.int_cst).low));
     output_add_character (scratch_buffer, '\'');
   }
 else
   {
     ...
     if (! host_integerp (t, 0))
       {
         ...
         if (tree_int_cst_sgn (val) < 0)
           {
      output_add_character (scratch_buffer, '-');
      val = build_int_2_wide ((unsigned long long) (-(((val)->int_cst.int_cst).low)), (long long) (~(((val)->int_cst.int_cst).high) + !(((val)->int_cst.int_cst).low)));
      ...
      sprintf (format, "%%x%%0%dx", (8 * 4) / 4);
           sprintf (digit_buffer, format, (((val)->int_cst.int_cst).high),
             (((val)->int_cst.int_cst).low));
           output_add_string (scratch_buffer, digit_buffer);
         }
       }
     else
       print_integer (scratch_buffer, (((t)->int_cst.int_cst).low));
   }
      }
      break;

    case REAL_CST:
      real_to_decimal (digit_buffer, &(*((t)->real_cst.real_cst_ptr)),
    ...
    case PTRMEM_CST:
      output_add_character (scratch_buffer, '&');
      dump_type (((((enum tree_code) ((((t))->common.type))->common.code) == POINTER_TYPE && ((enum tree_code) ((((((t))->common.type))->common.type))->common.code) == OFFSET_TYPE) ? (((((((t))->common.type))->common.type))->type.maxval) : ((((((((((((t))->common.type))->type.values))->common.type)))->common.type))->type.maxval)), flags);
      output_add_string ((scratch_buffer), "::");
      output_add_string ((scratch_buffer), ((char *)((((((ptrmem_cst_t)(t))->member))->decl.name))->identifier.id.str));

      break;

    case STRING_CST:
      {
 const char *p = ((t)->string.pointer);
 int len = ((t)->string.length) - 1;
 int i;
 ...
 output_add_character (scratch_buffer, '\"');
    ...
    case COMPOUND_EXPR:
      output_add_character ((scratch_buffer), '(');
      dump_expr (((t)->exp.operands[0]), flags | (1 << 9));
      output_add_string ((scratch_buffer), ", ");
      dump_expr (((t)->exp.operands[1]), flags | (1 << 9));
      output_add_character ((scratch_buffer), ')');
      break;
    ...
    case COND_EXPR:
      ...
      dump_expr (((t)->exp.operands[1]), flags | (1 << 9));
      output_add_string (scratch_buffer, " : ");
      dump_expr (((t)->exp.operands[2]), flags | (1 << 9));
      output_add_character ((scratch_buffer), ')');
      break;
 ...
 if (((enum tree_code) (((t)->exp.operands[0]))->common.code) == ADDR_EXPR)
   fn = ((((t)->exp.operands[0]))->exp.operands[0]);

 if (fn && ((enum tree_code) (fn)->common.code) == FUNCTION_DECL)
   {
     if ((((fn)->decl.lang_specific)->decl_flags.constructor_attr))
       output_add_string ((scratch_buffer), ((char *)((((((((t)->common.type))->type.name))->decl.name)))->identifier.id.str));

     else
       dump_decl (fn, 0);
   }
 else
   dump_expr (((t)->exp.operands[0]), 0);
      }
      output_add_character ((scratch_buffer), '(');
      if (((t)->exp.operands[1]))
 dump_expr_list (((((t)->exp.operands[1]))->common.chain), flags);
      output_add_character ((scratch_buffer), ')');
      break;
 ...
 tree args = ((t)->exp.operands[1]);

 if (((enum tree_code) (fn)->common.code) == ADDR_EXPR)
   fn = ((fn)->exp.operands[0]);

 if (((fn)->common.type) != (tree) ((void *)0) && (((enum tree_code) (((fn)->common.type))->common.code)) == METHOD_TYPE)
   {
     tree ob = ((args)->list.value);
     if (((enum tree_code) (ob)->common.code) == ADDR_EXPR)
       {
  dump_expr (((ob)->exp.operands[0]), flags | (1 << 9));
  output_add_character (scratch_buffer, '.');
       }
     else if (((enum tree_code) (ob)->common.code) != PARM_DECL
       || strcmp (((char *)(((ob)->decl.name))->identifier.id.str), "this"))
       {
  dump_expr (ob, flags | (1 << 9));
  output_add_string (scratch_buffer, "->");
       }
     args = ((args)->common.chain);
   }
 dump_expr (fn, flags | (1 << 9));
 output_add_character ((scratch_buffer), '(');
 ...
 output_add_character ((scratch_buffer), ')');
    ...
    case NEW_EXPR:
      {
 tree type = ((t)->exp.operands[1]);
 tree init = ((t)->exp.operands[2]);
 ...
 if (((t)->exp.operands[0]))
   {
     output_add_character ((scratch_buffer), '(');
     dump_expr_list (((t)->exp.operands[0]), flags);
     output_add_string (scratch_buffer, ") ");
   }
 if (((enum tree_code) (type)->common.code) == ARRAY_REF)
   type = build_cplus_array_type
     (((type)->exp.operands[0]),
      build_index_type (fold (build (MINUS_EXPR, integer_types[itk_int],
         ((type)->exp.operands[1]),
 ...
 if (init)
   {
     ...
     else if (init == c_global_trees[CTI_VOID_ZERO])
 ...
 dump_expr (((t)->exp.operands[1]), flags | (1 << 9));
    ...
    case NE_EXPR:
    case EXACT_DIV_EXPR:
      dump_binary_op (operator_name_info[(int) ((enum tree_code) (t)->common.code)].name, t, flags);
      break;
    ...
    case COMPONENT_REF:
      {
 tree ob = ((t)->exp.operands[0]);
 if (((enum tree_code) (ob)->common.code) == INDIRECT_REF)
   {
     ob = ((ob)->exp.operands[0]);
     if (((enum tree_code) (ob)->common.code) != PARM_DECL
  || strcmp (((char *)(((ob)->decl.name))->identifier.id.str), "this"))
       {
  dump_expr (ob, flags | (1 << 9));
  output_add_string (scratch_buffer, "->");
 ...
 dump_expr (((t)->exp.operands[1]), flags & ~(1 << 9));
    ...
    case CONVERT_EXPR:
      if (((t)->common.type) && (((enum tree_code) (((t)->common.type))->common.code) == VOID_TYPE))
 {
   output_add_character ((scratch_buffer), '(');
   dump_type (((t)->common.type), flags);
   output_add_character ((scratch_buffer), ')');
   dump_expr (((t)->exp.operands[0]), flags);
 }
      else
 dump_unary_op ("+", t, flags);
    ...
    case ADDR_EXPR:
      if (((enum tree_code) (((t)->exp.operands[0]))->common.code) == FUNCTION_DECL
   || ((enum tree_code) (((t)->exp.operands[0]))->common.code) == STRING_CST



   || (((t)->common.type)
       && ((enum tree_code) (((t)->common.type))->common.code) == REFERENCE_TYPE))
 dump_expr (((t)->exp.operands[0]), flags | (1 << 9));
      else
 dump_unary_op ("&", t, flags);
      break;
 ...
 {
   t = ((t)->exp.operands[0]);
   (void) (((((enum tree_code) (t)->common.code) == CALL_EXPR) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/error.c", 1800, __FUNCTION__), 0) : 0);
   dump_expr (((t)->exp.operands[0]), flags | (1 << 9));
   output_add_character ((scratch_buffer), '(');
   dump_expr_list (((((t)->exp.operands[1]))->common.chain), flags);
   output_add_character ((scratch_buffer), ')');
      ...
      else
 {
   if (((t)->exp.operands[0]) != (tree) ((void *)0)
       && ((((t)->exp.operands[0]))->common.type)
       && (((enum tree_code) (((((t)->exp.operands[0]))->common.type))->common.code)) == REFERENCE_TYPE)
     dump_expr (((t)->exp.operands[0]), flags);
   else
     dump_unary_op ("*", t, flags);
 }
    ...
    case PREINCREMENT_EXPR:
      dump_unary_op (operator_name_info [(int)((enum tree_code) (t)->common.code)].name, t, flags);
      break;
    ...
    case POSTINCREMENT_EXPR:
      output_add_character ((scratch_buffer), '(');
      dump_expr (((t)->exp.operands[0]), flags | (1 << 9));
      output_add_string ((scratch_buffer), (operator_name_info[(int)((enum tree_code) (t)->common.code)].name));
    ...
    case NON_LVALUE_EXPR:
      ...
      if (((t)->common.type) != (tree) ((void *)0) && (((enum tree_code) (((t)->common.type))->common.code)) == POINTER_TYPE)
 {
   tree next = ((((t)->common.type))->common.type);

   while (((enum tree_code) (next)->common.code) == POINTER_TYPE)
     next = ((next)->common.type);

   if (((enum tree_code) (next)->common.code) == FUNCTION_TYPE)
     {
       if (flags & (1 << 9))
         output_add_character ((scratch_buffer), '(');
       output_add_character (scratch_buffer, '*');
 ...
 {
   tree idx = build_ptrmemfunc_access_expr (t, cp_global_trees[CPTI_PFN_IDENTIFIER]);
   ...
   if (integer_zerop (idx))
   ...
   else if (host_integerp (idx, 0))
     {
       ...
       t = (((((((((t)->common.type))->type.values))->common.type)))->common.type);
       t = ((t)->type.maxval);
       virtuals = (((((((t)->type.main_variant))->type.binfo)))->vec.a[3]);
       ...
       if (virtuals)
  {
    ...
    break;
        ...
        {
          output_add_character (scratch_buffer, '{');
          dump_expr_list ((((t))->exp.operands[1]), flags);
          output_add_character (scratch_buffer, '}');
        }
      break;
 ...
 if (is_dummy_object (ob))
   {
     ...
     if (((enum tree_code) (t)->common.code) == FUNCTION_DECL)
       ...
       dump_expr (t, flags | (1 << 9));
     else if ((((enum tree_code) (t)->common.code) == BASELINK))
       dump_expr (((((enum tree_code) (((((t))->exp.operands[1])))->common.code) == OVERLOAD) ? (((struct tree_overload*)(((((t))->exp.operands[1]))))->function) : (((((t))->exp.operands[1])))),
    flags | (1 << 9));
     else
       dump_decl (t, flags);
   }
 ...
 break;
    ...
    case TEMPLATE_PARM_INDEX:
      dump_decl ((((template_parm_index*)(t))->decl), flags & ~(1 << 2));
      break;
    ...
    case SCOPE_REF:
      dump_type (((t)->exp.operands[0]), flags);
      output_add_string ((scratch_buffer), "::");
      dump_expr (((t)->exp.operands[1]), flags | (1 << 9));
      break;
      ...
      else
 {
   ...
   output_add_character ((scratch_buffer), ')');
    ...
    case LOOKUP_EXPR:
      output_add_string ((scratch_buffer), ((char *)(((t)->exp.operands[0]))->identifier.id.str));
      break;
    ...
    case SIZEOF_EXPR:
    case ALIGNOF_EXPR:
      if (((enum tree_code) (t)->common.code) == SIZEOF_EXPR)
 output_add_string (scratch_buffer, "sizeof (");
      else
 {
   (void) (((((enum tree_code) (t)->common.code) == ALIGNOF_EXPR) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/error.c", 2022, __FUNCTION__), 0) : 0);
   output_add_string (scratch_buffer, "__alignof__ (");
 }
      if ((tree_code_type[(int) (((enum tree_code) (((t)->exp.operands[0]))->common.code))] == 't'))
 dump_type (((t)->exp.operands[0]), flags);
      else
 dump_unary_op ("*", t, flags | (1 << 9));
      output_add_character ((scratch_buffer), ')');
      break;

    case DEFAULT_ARG:
      output_add_string ((scratch_buffer), ("<unparsed>"));
      break;
    ...
    case PSEUDO_DTOR_EXPR:
      dump_expr (((t)->exp.operands[2]), flags);
      output_add_character (scratch_buffer, '.');
      dump_type (((t)->exp.operands[0]), flags);
      output_add_string (scratch_buffer, "::~");
      dump_type (((t)->exp.operands[1]), flags);
      break;
    ...
    case BASELINK:
      output_add_string ((scratch_buffer), ((char *)(((get_first_fn (t))->decl.name))->identifier.id.str));
      break;

    case TREE_LIST:
      if (((t)->list.value) && ((enum tree_code) (((t)->list.value))->common.code) == FUNCTION_DECL)
 {
   output_add_string ((scratch_buffer), ((char *)(((((t)->list.value))->decl.name))->identifier.id.str));
   break;
 }
    ...
    case ERROR_MARK:
      output_add_string ((scratch_buffer), ("<expression error>"));
      break;
    }
}
     ...
     int flags;
{
  ...
  output_add_space (scratch_buffer);
  if (opstring)
    output_add_string ((scratch_buffer), (opstring));
  else
    output_add_string ((scratch_buffer), ("<unknown operator>"));
  output_add_space (scratch_buffer);
  dump_expr (((t)->exp.operands[1]), flags | (1 << 9));
  output_add_character ((scratch_buffer), ')');
}
...
{
  if (v >= 2)
    return decl_as_string (decl, (1 << 2));

  output_clear_message_text (scratch_buffer);

  if (v == 1 && (((decl)->decl.context) && (tree_code_type[(int) (((enum tree_code) (((decl)->decl.context))->common.code))] == 't')))
    {
      dump_type ((((decl)->decl.context) ? ((decl)->decl.context) : global_namespace), (0));
      output_add_string ((scratch_buffer), "::");
    }

  if (((enum tree_code) (decl)->common.code) == FUNCTION_DECL)
    dump_function_name (decl, (0));
  else
    dump_decl (((decl)->decl.name), (0));
...
cp_file_of (t)
     tree t;
{
  if (((enum tree_code) (t)->common.code) == PARM_DECL && ((t)->decl.context))
    return (((((t)->decl.context))->decl.locus).file);
  else if ((tree_code_type[(int) (((enum tree_code) (t)->common.code))] == 't'))
    return ((((((((t)->type.main_variant))->common.chain)))->decl.locus).file);
  else if (((enum tree_code) (t)->common.code) == OVERLOAD)
    return ((((((struct tree_overload*)(t))->function))->decl.locus).file);
  else
    return (((t)->decl.locus).file);
}
     ...
     tree t;
{
  int line = 0;
  if (((enum tree_code) (t)->common.code) == PARM_DECL && ((t)->decl.context))
    line = (((((t)->decl.context))->decl.locus).line);
  if (((enum tree_code) (t)->common.code) == TYPE_DECL && ((t)->decl.artificial_flag)
      && (((((((t)->common.type))->type.main_variant))->common.chain)))
    t = ((t)->common.type);

  if ((tree_code_type[(int) (((enum tree_code) (t)->common.code))] == 't'))
    line = ((((((((t)->type.main_variant))->common.chain)))->decl.locus).line);
  else if (((enum tree_code) (t)->common.code) == OVERLOAD)
    line = ((((((struct tree_overload*)(t))->function))->decl.locus).line);
  else
    line = (((t)->decl.locus).line);
     ...
     int verbose;
{
  ...
  if (((enum tree_code) (decl)->common.code) == TYPE_DECL || ((enum tree_code) (decl)->common.code) == RECORD_TYPE
      || ((enum tree_code) (decl)->common.code) == UNION_TYPE || ((enum tree_code) (decl)->common.code) == ENUMERAL_TYPE)
    flags = (1 << 3);
  if (verbose)
    flags |= (1 << 2);
     ...
     int v __attribute__ ((__unused__));
{
  switch (c)
    ...
    default:
      fancy_abort ("../../../kg++fe/gnu/cp/error.c", 2331, __FUNCTION__);
      return 0;
    }
}
     ...
     int v __attribute__ ((__unused__));
{
  ...
  return id ? ((char *)(id)->identifier.id.str) : "{unknown}";
}
     ...
     int verbose;
{
  ...
  if ((tree_code_type[(int) (((enum tree_code) (((p)->list.value))->common.code))] == 't'))
    return type_as_string (p, flags);

  output_clear_message_text (scratch_buffer);
  for (; p; p = ((p)->common.chain))
    {
      if (((p)->list.value) == cp_global_trees[CPTI_NULL])
 output_add_string ((scratch_buffer), ("NULL"));
      else
 dump_type (error_type (((p)->list.value)), flags);
      if (((p)->common.chain))
 output_add_string ((scratch_buffer), ", ");
    }
  return output_finalize_message (scratch_buffer);
     ...
     diagnostic_info *diagnostic;
{
  if (((context)->last_function != current_function_decl))
    {
      const char *old_prefix = (&context->buffer)->state.prefix;
  ...
  if (((((enum tree_code) (((fn)->common.type))->common.code) == METHOD_TYPE) || (((fn)->decl.lang_specific)->decl_flags.static_function)))
    {
      ...
      else if (((((fn)->decl.lang_specific)->decl_flags.constructor_attr) && copy_fn_p (fn) > 0))
        return "copy constructor";
      else if ((((fn)->decl.lang_specific)->decl_flags.constructor_attr))
        return "constructor";
      else if ((((fn)->decl.lang_specific)->decl_flags.destructor_attr))
        return "destructor";
      else
     ...
     diagnostic_context *context;
{
  ...
  if (p)
    {
      if (current_function_decl != (((p))->exp.operands[0])
      ...
      else
 {
   if (current_function_decl == (((p))->exp.operands[0]))
     ...
     output_verbatim (&context->buffer,
                             ...
                             decl_as_string ((((p))->exp.operands[0]),
                                             (1 << 2) | (1 << 4)));

   line = (((p)->exp.complexity) >> 12);
   file = ((char *)((((p))->exp.operands[1]))->identifier.id.str);
   p = ((p)->common.chain);
 }
    }
  ...
  for (; t; t = ((t)->common.chain))
    {
      output_verbatim
        (&context->buffer, "%s:%d:   instantiated from `%s'\n", file, line,
         decl_as_string ((((t))->exp.operands[0]), (1 << 2) | (1 << 4)));
      line = (((t)->exp.complexity) >> 12);
      file = ((char *)((((t))->exp.operands[1]))->identifier.id.str);
    }
     ...
     text_info *text;
{
  ...
  switch (*text->format_spec)
    {
    case 'A': result = args_to_string (__builtin_va_arg(*text->args_ptr,tree), verbose); break;
    case 'C': result = code_to_string (__builtin_va_arg(*text->args_ptr,enum tree_code), verbose); break;
    case 'D': result = decl_to_string (__builtin_va_arg(*text->args_ptr,tree), verbose); break;
    case 'E': result = expr_to_string (__builtin_va_arg(*text->args_ptr,tree), verbose); break;
    case 'F': result = fndecl_to_string (__builtin_va_arg(*text->args_ptr,tree), verbose); break;
    case 'L': result = language_to_string (__builtin_va_arg(*text->args_ptr,enum languages), verbose); break;
    case 'O': result = op_to_string (__builtin_va_arg(*text->args_ptr,enum tree_code), verbose); break;
    case 'P': result = parm_to_string (__builtin_va_arg(*text->args_ptr,int), verbose); break;
    case 'Q': result = assop_to_string (__builtin_va_arg(*text->args_ptr,enum tree_code), verbose); break;
    case 'T': result = type_to_string (__builtin_va_arg(*text->args_ptr,tree), verbose); break;
    case 'V': result = cv_to_string (__builtin_va_arg(*text->args_ptr,tree), verbose); break;
...
print_integer (buffer, i)
     ...
     long long i;
{
  sprintf (digit_buffer, "%lld", (long long) i);
     ...
     va_list ap;
{
      ...
      if (*f == '%')
 {
   ...
   switch (*f)
     ...
     case '%': break;
     case 'd': (void) __builtin_va_arg(ap,int); break;
     case 's': (void) __builtin_va_arg(ap,char *); break;
     case 'L': (void) __builtin_va_arg(ap,enum languages); break;
     case 'C':
     case 'O':
     case 'Q': (void) __builtin_va_arg(ap,enum tree_code); break;
     ...
     case 'T':
     case 'V':
       t = __builtin_va_arg(ap,tree);
       if (!here || plus)
  here = t;
       break;

     default:
       (global_dc)->diagnostic_count[(int) (DK_ERROR)] = 0;
...
cp_error_at (const char *msgid, ...)
{
  ...
  { va_list ap; __builtin_va_start(ap,msgid); { struct Qdmy;
  struct Qdmy;
