...
# 41 "../../../kg++fe/gnu/cp/class.c" 2

# 1 "../../../kg++fe/gnu/defaults.h" 1
# 43 "../../../kg++fe/gnu/cp/class.c" 2
...
static int field_decl_cmp (const tree *, const tree *);
static int method_name_cmp (const tree *, const tree *);
static void add_implicitly_declared_members (tree, int, int, int);
static tree fixed_type_or_null (tree, int *, int *);
static tree resolve_address_of_overloaded_function (tree, tree, int, int, int, tree);
...
static _Bool layout_empty_base (tree, tree, splay_tree, tree);
...
static _Bool type_requires_array_cookie (tree);
static _Bool contains_empty_class_p (tree);
static tree dfs_base_derived_from (tree, void *);
static _Bool base_derived_from (tree, tree);
static int empty_base_at_nonzero_offset_p (tree, tree, splay_tree);
static tree end_of_base (tree);
static tree get_vcall_index (tree, tree);
# 254 "../../../kg++fe/gnu/cp/class.c"
tree
build_base_path (code, expr, binfo, nonnull)
     ...
     int nonnull;
{
  tree v_binfo = (tree) ((void *)0);
  tree d_binfo = (tree) ((void *)0);
  tree probe;
  tree offset;
  tree target_type;
  tree null_test = ((void *)0);
  tree ptr_target_type;
  int fixed_type_p;
  int want_pointer = ((enum tree_code) (((expr)->common.type))->common.code) == POINTER_TYPE;

  if (expr == global_trees[TI_ERROR_MARK] || binfo == global_trees[TI_ERROR_MARK] || !binfo)
    return global_trees[TI_ERROR_MARK];

  for (probe = binfo; probe; probe = (((probe))->vec.a[0]))
    {
      d_binfo = probe;
      if (!v_binfo && ((probe)->common.static_flag))
 v_binfo = probe;
    }

  probe = ((((expr)->common.type))->type.main_variant);
  if (want_pointer)
    probe = ((((probe)->common.type))->type.main_variant);

  (void) (((code == MINUS_EXPR ? comptypes ((((binfo)->common.type)), (probe), 0) : code == PLUS_EXPR ? comptypes ((((d_binfo)->common.type)), (probe), 0) : 0) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 289, __FUNCTION__), 0) : 0);
  ...
  if (code == MINUS_EXPR && v_binfo)
    {
      ...
      ((binfo)->common.type), ((d_binfo)->common.type), ((v_binfo)->common.type));
      return global_trees[TI_ERROR_MARK];
    }
  ...
  if (fixed_type_p <= 0 && ((expr)->common.side_effects_flag))
    expr = save_expr (expr);

  if (want_pointer && !nonnull)
    null_test = build (EQ_EXPR, c_global_trees[CTI_BOOLEAN_TYPE], expr, global_trees[TI_INTEGER_ZERO]);

  offset = (((binfo))->vec.a[1]);
  ...
  if (v_binfo && fixed_type_p <= 0)
    {
      ...
      if (fixed_type_p < 0 && (cfun->language)->in_base_initializer)
 {


   tree derived = v_binfo;

   while ((((derived))->vec.a[0]))
     derived = (((derived))->vec.a[0]);

   v_offset = build (PLUS_EXPR, (((cfun->language)->x_vtt_parm)->common.type),
       (cfun->language)->x_vtt_parm, ((derived)->vec.a[9]));

   v_offset = build1 (INDIRECT_REF,
        ((((((derived)->common.type))->type.minval))->common.type),
        v_offset);
 }
      else
 v_offset = build_vfield_ref (build_indirect_ref (expr, ((void *)0)),
         ((((expr)->common.type))->common.type));

      v_binfo = binfo_for_vbase (((v_binfo)->common.type), ((d_binfo)->common.type));

      v_offset = build (PLUS_EXPR, ((v_offset)->common.type),
   v_offset, ((v_binfo)->vec.a[5]));
      v_offset = build1 (NOP_EXPR,
    build_pointer_type (global_trees[TI_PTRDIFF_TYPE]),
    v_offset);
      v_offset = build_indirect_ref (v_offset, ((void *)0));
      ((v_offset)->common.constant_flag) = 1;

      offset = cp_convert (global_trees[TI_PTRDIFF_TYPE],
      size_diffop (offset, (((v_binfo))->vec.a[1])));

      if (!integer_zerop (offset))
 v_offset = build (code, global_trees[TI_PTRDIFF_TYPE], v_offset, offset);

      if (fixed_type_p < 0)



 offset = build (COND_EXPR, global_trees[TI_PTRDIFF_TYPE],
 ...
 offset = v_offset;
  ...
  target_type = code == PLUS_EXPR ? ((binfo)->common.type) : ((d_binfo)->common.type);

  target_type = cp_build_qualified_type_real ((target_type), (cp_type_quals (((((expr)->common.type))->common.type))), tf_error | tf_warning);
  ...
  if (null_test)
    ...
    build1 (NOP_EXPR, target_type, global_trees[TI_INTEGER_ZERO]),
    expr);
...
tree
convert_to_base (tree object, tree type, _Bool check_access)
{
  tree binfo;

  binfo = lookup_base (((object)->common.type), type,
         check_access ? ba_check : ba_ignore,
         ((void *)0));
  if (!binfo || binfo == global_trees[TI_ERROR_MARK])
    return global_trees[TI_ERROR_MARK];
...
convert_to_base_statically (tree expr, tree base)
{
  ...
  if (!comptypes ((expr_type), (((base)->common.type)), 0))
    {
      tree pointer_type;

      pointer_type = build_pointer_type (expr_type);
      expr = build_unary_op (ADDR_EXPR, expr, 1);
      if (!integer_zerop ((((base))->vec.a[1])))
   expr = build (PLUS_EXPR, pointer_type, expr,
   build_nop (pointer_type, (((base))->vec.a[1])));
      expr = build_nop (build_pointer_type (((base)->common.type)), expr);
      expr = build1 (INDIRECT_REF, ((base)->common.type), expr);
    }
     ...
     tree array_ref, instance, idx;
{
  ...
  basetype = ((instance)->common.type);
  if (((enum tree_code) (basetype)->common.code) == REFERENCE_TYPE)
    basetype = ((basetype)->common.type);

  vtable = get_vtbl_decl_for_binfo (((basetype)->type.binfo));
  first_fn = (((((basetype)->type.binfo)))->vec.a[2]);

  i = fold (build_array_ref (first_fn, idx));
  i = fold (build_c_cast (global_trees[TI_PTRDIFF_TYPE],
     build_unary_op (ADDR_EXPR, i, 0)));
  i2 = fold (build_array_ref (vtable, build_int_2_wide ((unsigned long long) (0), (long long) (0))));
  i2 = fold (build_c_cast (global_trees[TI_PTRDIFF_TYPE],
      build_unary_op (ADDR_EXPR, i2, 0)));
  i = fold (build_binary_op(MINUS_EXPR, i, i2, 1));

  if (((enum tree_code) (i)->common.code) != INTEGER_CST)
    fancy_abort ("../../../kg++fe/gnu/cp/class.c", 463, __FUNCTION__);

  return build (VTABLE_REF, ((array_ref)->common.type), array_ref, vtable, i);
}
...
{
  tree aref;
  tree vtbl = (tree) ((void *)0);




  int cdtorp = 0;
  tree fixed_type = fixed_type_or_null (instance, ((void *)0), &cdtorp);

  tree basetype = ((instance)->common.type);
  if (((enum tree_code) (basetype)->common.code) == REFERENCE_TYPE)
    basetype = ((basetype)->common.type);
     ...
     tree instance, idx;
{
  ...
  if (0)
    aref = build1 (NOP_EXPR, ((aref)->common.type),
     ...
     tree vtable_type;
{
  ...
  ((decl)->decl.context) = class_type;
  ((decl)->decl.artificial_flag) = 1;
  ((decl)->common.static_flag) = 1;
  ((decl)->common.readonly_flag) = 1;
  ((decl)->decl.virtual_flag) = 1;
  ((decl)->decl.u1.a.align) = ((((target_flags & 0x00000002) && (target_flags & 0x00002000)) ? DImode : SImode) == DImode ? 64 : 32);
     ...
     int complete;
{
  ...
  if (((&((type)->type.lang_specific)->u.c)->vtables))
    return ((&((type)->type.lang_specific)->u.c)->vtables);

  decl = build_vtable (type, get_vtable_name (type), global_trees[TI_VOID_TYPE]);
  ((&((type)->type.lang_specific)->u.c)->vtables) = decl;



  ((decl)->decl.u1.a.align) = ((((global_trees[TI_DOUBLE_TYPE])->type.align)) > (((decl)->decl.u1.a.align)) ? (((global_trees[TI_DOUBLE_TYPE])->type.align)) : (((decl)->decl.u1.a.align)));
     ...
     tree binfo;
{
  ...
  copies = copy_list ((((binfo))->vec.a[3]));
  for (t = copies; t; t = ((t)->common.chain))
     ...
     tree binfo, type;
{
  ...
  if (binfo)
    {
      if (((((((binfo)->common.static_flag) ? binfo_for_vbase (((binfo)->common.type), type) : (binfo)))->common.lang_flag_4)))


 return 0;

      virtuals = copy_virtuals (binfo);
      ((decl)->common.type) = ((get_vtbl_decl_for_binfo (binfo))->common.type);
      ((decl)->decl.size) = ((((decl)->common.type))->type.size);
      ((decl)->decl.size_unit) = ((((decl)->common.type))->type.size_unit);
    }
  else
    {
      (void) (((((enum tree_code) (((decl)->common.type))->common.code) == VOID_TYPE) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 672, __FUNCTION__), 0) : 0);

      virtuals = (tree) ((void *)0);
    }
# 683 "../../../kg++fe/gnu/cp/class.c"
  (((((type)->type.binfo)))->vec.a[2]) = decl;
  (((((type)->type.binfo)))->vec.a[3]) = virtuals;
  (((((((((type)->type.binfo))->common.static_flag) ? binfo_for_vbase (((((type)->type.binfo))->common.type), type) : (((type)->type.binfo))))->common.lang_flag_4)) = 1, (void) (((!(((((type)->type.binfo))->vec.a[10]) != (tree) ((void *)0)) || ((((type)->type.binfo))->common.static_flag)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 685, __FUNCTION__), 0) : 0), (void) (((((&((((((type)->type.binfo))->common.type))->type.lang_specific)->u.c)->vfields) != (tree) ((void *)0)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 685, __FUNCTION__), 0) : 0));
  return 1;
}
# 703 "../../../kg++fe/gnu/cp/class.c"
static int
build_secondary_vtable (binfo, for_type)
     tree binfo, for_type;
{
  (void) (((binfo == (((binfo)->common.static_flag) ? binfo_for_vbase (((binfo)->common.type), for_type) : (binfo))) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 707, __FUNCTION__), 0) : 0);

  if (((((((binfo)->common.static_flag) ? binfo_for_vbase (((binfo)->common.type), for_type) : (binfo)))->common.lang_flag_4)))
  ...
  (((((((binfo)->common.static_flag) ? binfo_for_vbase (((binfo)->common.type), for_type) : (binfo)))->common.lang_flag_4)) = 1, (void) (((!(((binfo)->vec.a[10]) != (tree) ((void *)0)) || ((binfo)->common.static_flag)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 716, __FUNCTION__), 0) : 0), (void) (((((&((((binfo)->common.type))->type.lang_specific)->u.c)->vfields) != (tree) ((void *)0)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 716, __FUNCTION__), 0) : 0));


  (((binfo))->vec.a[3]) = copy_virtuals (binfo);



  (((binfo))->vec.a[2]) = (tree) ((void *)0);
  return 1;
}
...
make_new_vtable (t, binfo)
     ...
     tree binfo;
{
  if (binfo == ((t)->type.binfo))





    return build_primary_vtable (((((((t)->type.minval))->decl.context))->type.binfo),
     t);
  else
     ...
     tree *virtuals;
{
  ...
  if (fndecl != (((v)->list.value))
      || !tree_int_cst_equal (delta, (((v)->list.purpose))))
    {
      tree base_fndecl;
      ...
      if (make_new_vtable (t, binfo))
 {
   ...
   *virtuals = (((binfo))->vec.a[3]);
   while ((((*virtuals)->list.value)) != (((v)->list.value)))
     *virtuals = ((*virtuals)->common.chain);
   v = *virtuals;
 }

      base_fndecl = (((v)->list.value));
      (((v)->list.purpose)) = delta;
      (((v)->common.type)) = (tree) ((void *)0);
      (((v)->list.value)) = fndecl;
    }
}
...
add_method (type, method, error_p)
     ...
     int error_p;
{
  int using = (((method)->decl.context) != type);
  int len;
  int slot;
  tree method_vec;
  int template_conv_p = (((enum tree_code) (method)->common.code) == TEMPLATE_DECL
    && (((method)->decl.lang_specific)->decl_flags.template_conv_p));

  if (!((&((type)->type.lang_specific)->u.c)->methods))
# 823 "../../../kg++fe/gnu/cp/class.c"
    ((&((type)->type.lang_specific)->u.c)->methods) = make_tree_vec (8);

  method_vec = ((&((type)->type.lang_specific)->u.c)->methods);
  len = ((method_vec)->vec.length);


  if (((((method)->decl.lang_specific)->decl_flags.constructor_attr) && !((((enum tree_code) (method)->common.code) == FUNCTION_DECL || ((enum tree_code) (method)->common.code) == TEMPLATE_DECL) && ((method)->decl.lang_specific) && (((method)->decl.lang_specific)->u.f.cloned_function) != (tree) ((void *)0))))
    slot = 0;
  else if (((((method)->decl.lang_specific)->decl_flags.destructor_attr) && !((((enum tree_code) (method)->common.code) == FUNCTION_DECL || ((enum tree_code) (method)->common.code) == TEMPLATE_DECL) && ((method)->decl.lang_specific) && (((method)->decl.lang_specific)->u.f.cloned_function) != (tree) ((void *)0))))
    slot = 1;
  else
    {
 ...
 {
   tree m = ((method_vec)->vec.a[slot]);

   if (!m)
     break;
   m = ((((enum tree_code) (m)->common.code) == OVERLOAD) ? (((struct tree_overload*)(m))->function) : (m));

   if (template_conv_p)
     {
       have_template_convs_p = (((enum tree_code) (m)->common.code) == TEMPLATE_DECL
           && (((m)->decl.lang_specific)->decl_flags.template_conv_p));
  ...
  {
    slot = len - 1;
    if (((method_vec)->vec.a[slot]))
      slot++;
  }
       break;
     }
   if (((m)->decl.name) == ((method)->decl.name))
     break;
 }
      ...
      if (slot == len)
 {
   ...
   memcpy (&((new_vec)->vec.a[0]), &((method_vec)->vec.a[0]),
    len * sizeof (tree));
   len = new_len;
   method_vec = ((&((type)->type.lang_specific)->u.c)->methods) = new_vec;
 }

      if ((((((method)->decl.name))->common.lang_flag_4)) && !((method_vec)->vec.a[slot]))
 {
  ...
  if (!(((((((((enum tree_code) (fn)->common.code) == OVERLOAD) ? (((struct tree_overload*)(fn))->function) : (fn)))->decl.name))->common.lang_flag_4)))
  ...
  ;
   else
     {
       ...
       memmove (&((method_vec)->vec.a[slot + 1]),
         &((method_vec)->vec.a[slot]),
         (len - slot - 1) * sizeof (tree));
       ((method_vec)->vec.a[slot]) = (tree) ((void *)0);
     }
 }
    }
    ...
    fns = ((((enum tree_code) (fns)->common.code) == OVERLOAD) ? ((fns)->common.chain) : (tree) ((void *)0)))
 {
   ...
   if (((enum tree_code) (fn)->common.code) != ((enum tree_code) (method)->common.code))
     continue;
# 965 "../../../kg++fe/gnu/cp/class.c"
   parms1 = ((((fn)->common.type))->type.values);
   parms2 = ((((method)->common.type))->type.values);




   if (! (((fn)->decl.lang_specific)->decl_flags.static_function)
       && ! (((method)->decl.lang_specific)->decl_flags.static_function)
       && (((((((((parms1)->list.value))->common.type))->common.readonly_flag) * 0x1) | (((((((parms1)->list.value))->common.type))->common.volatile_flag) * 0x2) | (((((((parms1)->list.value))->common.type))->type.restrict_flag) * 0x4) | ((((enum tree_code) (((((parms1)->list.value))->common.type))->common.code) == RECORD_TYPE && ((((((parms1)->list.value))->common.type))->common.type)) * 0x8))
    != ((((((((parms2)->list.value))->common.type))->common.readonly_flag) * 0x1) | (((((((parms2)->list.value))->common.type))->common.volatile_flag) * 0x2) | (((((((parms2)->list.value))->common.type))->type.restrict_flag) * 0x4) | ((((enum tree_code) (((((parms2)->list.value))->common.type))->common.code) == RECORD_TYPE && ((((((parms2)->list.value))->common.type))->common.type)) * 0x8))))
     same = 0;


   if (((enum tree_code) (fn)->common.code) == TEMPLATE_DECL
       && !comp_template_parms (((fn)->decl.arguments),
           ((method)->decl.arguments)))
     same = 0;

   if (! (((fn)->decl.lang_specific)->decl_flags.static_function))
     parms1 = ((parms1)->common.chain);
   if (! (((method)->decl.lang_specific)->decl_flags.static_function))
     parms2 = ((parms2)->common.chain);

   if (same && compparms (parms1, parms2)
       && (!(((((fn)->decl.name))->common.lang_flag_4))
    || comptypes ((((((fn)->common.type))->common.type)), (((((method)->common.type))->common.type)), 0)))

     {
       if (using && ((fn)->decl.context) == type)
  ...
  ((method_vec)->vec.a[slot])
    = build_overload (method, ((method_vec)->vec.a[slot]));


  if (!(((method)->decl.lang_specific)->decl_flags.constructor_attr)
      && !(((method)->decl.lang_specific)->decl_flags.destructor_attr))
    push_class_level_binding (((method)->decl.name),
         ((method_vec)->vec.a[slot]));
}
# 1039 "../../../kg++fe/gnu/cp/class.c"
static tree
     ...
     tree field, fields;
{
  ...
  if (((field)->decl.name) == 0)
    {
      if (! ((((((enum tree_code) (((field)->common.type))->common.code)) == RECORD_TYPE || (((enum tree_code) (((field)->common.type))->common.code)) == UNION_TYPE) && (((enum tree_code) (((field)->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((field)->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((field)->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((field)->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((field)->common.type))->type.lang_flag_5))) && (&((((field)->common.type))->type.lang_specific)->u.c)->anon_aggr))
 return fields;

      for (x = ((((field)->common.type))->type.values); x; x = ((x)->common.chain))
 fields = delete_duplicate_fields_1 (x, fields);
      return fields;
    }
  else
    {
      for (x = fields; x; prev = x, x = ((x)->common.chain))
 {
   if (((x)->decl.name) == 0)
     {
       if (! ((((((enum tree_code) (((x)->common.type))->common.code)) == RECORD_TYPE || (((enum tree_code) (((x)->common.type))->common.code)) == UNION_TYPE) && (((enum tree_code) (((x)->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((x)->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((x)->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((x)->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((x)->common.type))->type.lang_flag_5))) && (&((((x)->common.type))->type.lang_specific)->u.c)->anon_aggr))
  continue;
       ((((x)->common.type))->type.values)
  = delete_duplicate_fields_1 (field, ((((x)->common.type))->type.values));
       if (((((x)->common.type))->type.values) == 0)
  {
    if (prev == 0)
   ...
   else if (((enum tree_code) (field)->common.code) == USING_DECL)




     ;
   else if (((field)->decl.name) == ((x)->decl.name))
     {
       if (((enum tree_code) (field)->common.code) == CONST_DECL
    && ((enum tree_code) (x)->common.code) == CONST_DECL)
  cp_error_at ("duplicate enum value `%D'", x);
       else if (((enum tree_code) (field)->common.code) == CONST_DECL
         || ((enum tree_code) (x)->common.code) == CONST_DECL)
  cp_error_at ("duplicate field `%D' (as enum and non-enum)",
        x);
       else if ((((enum tree_code) (field)->common.code) == TYPE_DECL || (((enum tree_code) (field)->common.code) == TEMPLATE_DECL && ((enum tree_code) (((field)->decl.result))->common.code) == TYPE_DECL && !(((enum tree_code) (field)->common.code) == TEMPLATE_DECL && (((field)->decl.lang_flag_0) && (((enum tree_code) (field)->common.code) == CONST_DECL || ((enum tree_code) (field)->common.code) == PARM_DECL || ((enum tree_code) (field)->common.code) == TYPE_DECL || ((enum tree_code) (field)->common.code) == TEMPLATE_DECL)))))
         && (((enum tree_code) (x)->common.code) == TYPE_DECL || (((enum tree_code) (x)->common.code) == TEMPLATE_DECL && ((enum tree_code) (((x)->decl.result))->common.code) == TYPE_DECL && !(((enum tree_code) (x)->common.code) == TEMPLATE_DECL && (((x)->decl.lang_flag_0) && (((enum tree_code) (x)->common.code) == CONST_DECL || ((enum tree_code) (x)->common.code) == PARM_DECL || ((enum tree_code) (x)->common.code) == TYPE_DECL || ((enum tree_code) (x)->common.code) == TEMPLATE_DECL))))))
  {
    if (comptypes ((((field)->common.type)), (((x)->common.type)), 0))
      continue;
         ...
         || (((enum tree_code) (x)->common.code) == TYPE_DECL || (((enum tree_code) (x)->common.code) == TEMPLATE_DECL && ((enum tree_code) (((x)->decl.result))->common.code) == TYPE_DECL && !(((enum tree_code) (x)->common.code) == TEMPLATE_DECL && (((x)->decl.lang_flag_0) && (((enum tree_code) (x)->common.code) == CONST_DECL || ((enum tree_code) (x)->common.code) == PARM_DECL || ((enum tree_code) (x)->common.code) == TYPE_DECL || ((enum tree_code) (x)->common.code) == TEMPLATE_DECL))))))
  {
    ...
    if ((((enum tree_code) (field)->common.code) == TYPE_DECL
...
{
  tree x;
  for (x = fields; x && ((x)->common.chain); x = ((x)->common.chain))
    ((x)->common.chain) = delete_duplicate_fields_1 (x, ((x)->common.chain));
}
     ...
     tree access;
{
  ...
  if (!((fdecl)->decl.lang_specific))
    retrofit_lang_decl (fdecl);

  if ((((enum tree_code) (fdecl)->common.code) == VAR_DECL && (((fdecl)->decl.context) && ((enum tree_code) (((fdecl)->decl.context))->common.code) == FUNCTION_DECL)))
    fancy_abort ("../../../kg++fe/gnu/cp/class.c", 1142, __FUNCTION__);

  elem = purpose_member (t, ((&((fdecl)->decl.lang_specific)->decl_flags.u2)->access));
  if (elem)
    {
      if (((elem)->list.value) != access)
 {
   if (((enum tree_code) (((fdecl)->common.type))->common.code) == FUNCTION_DECL)
     cp_error_at ("conflicting access specifications for method `%D', ignored", ((fdecl)->common.type));
   else
     error ("conflicting access specifications for field `%s', ignored",
     ((char *)(((fdecl)->decl.name))->identifier.id.str));
 }
      else
 {
   ...
   ;
  ...
  else
    {
      ...
      ((&((fdecl)->decl.lang_specific)->decl_flags.u2)->access) = tree_cons (t, access, ((&((fdecl)->decl.lang_specific)->decl_flags.u2)->access));
      return 1;
    }
  return 0;
}
...
handle_using_decl (using_decl, t)
     ...
     tree t;
{
  tree ctype = ((using_decl)->decl.initial);
  tree name = ((using_decl)->decl.name);
  tree access
    = ((using_decl)->common.private_flag) ? cp_global_trees[CPTI_ACCESS_PRIVATE]
    : ((using_decl)->common.protected_flag) ? cp_global_trees[CPTI_ACCESS_PROTECTED]
    : cp_global_trees[CPTI_ACCESS_PUBLIC];
  tree fdecl, binfo;
  tree flist = (tree) ((void *)0);
  tree old_value;

  if (ctype == global_trees[TI_ERROR_MARK])
    return;

  binfo = lookup_base (t, ctype, ba_any, ((void *)0));
  if (! binfo)
    {
  ...
  if ((((enum tree_code) (fdecl)->common.code) == BASELINK))

    fdecl = ((((fdecl))->exp.operands[1]));

  old_value = (((struct lang_identifier*)(name))->class_value);
  if (old_value)
    {
      if (is_overloaded_fn (old_value))
 old_value = ((((enum tree_code) (old_value)->common.code) == OVERLOAD) ? (((struct tree_overload*)(old_value))->function) : (old_value));

      if ((tree_code_type[(int) (((enum tree_code) (old_value)->common.code))] == 'd') && ((old_value)->decl.context) == t)
         ;
      else
 old_value = (tree) ((void *)0);
    }
      ...
      else
 {
   ...
   cp_error_at ("  because of local method `%#D' with same name",
         ((((enum tree_code) (old_value)->common.code) == OVERLOAD) ? (((struct tree_overload*)(old_value))->function) : (old_value)));
   return;
 }
    }
  else if (!((old_value)->decl.artificial_flag))
    {
      cp_error_at ("`%D' invalid in `%#T'", using_decl, t);
  ...
  if (flist)
    for (; flist; flist = ((((enum tree_code) (flist)->common.code) == OVERLOAD) ? ((flist)->common.chain) : (tree) ((void *)0)))
      {
 add_method (t, ((((enum tree_code) (flist)->common.code) == OVERLOAD) ? (((struct tree_overload*)(flist))->function) : (flist)), 0);
 alter_access (t, ((((enum tree_code) (flist)->common.code) == OVERLOAD) ? (((struct tree_overload*)(flist))->function) : (flist)), access);
      }
  else
    alter_access (t, fdecl, access);
}
     ...
     int *no_const_asn_ref_p;
{
  ...
  binfos = ((((t)->type.binfo))->vec.a[4]);
  n_baseclasses = (((((((t)->type.binfo)))->vec.a[4]) ? (((((((t)->type.binfo)))->vec.a[4]))->vec.length) : 0));
  seen_non_virtual_nearly_empty_base_p = 0;


  ((&((t)->type.lang_specific)->u.c)->non_aggregate) |= (n_baseclasses != 0);
  ...
  for (i = 0; i < n_baseclasses; ++i)
    {
      ...
      base_binfo = ((binfos)->vec.a[i]);
      basetype = ((base_binfo)->common.type);
      ...
      if (!(((basetype)->type.size) != (tree) ((void *)0)))
 {
   ...
   if (i == n_baseclasses-1)
     ((binfos)->vec.a[i]) = (tree) ((void *)0);
   ((binfos)->vec.length) -= 1;
   n_baseclasses -= 1;
   for (j = i; j+1 < n_baseclasses; j++)
     ((binfos)->vec.a[j]) = ((binfos)->vec.a[j+1]);
   continue;
 }
      ...
      if (warn_ecpp && ! (((basetype)->common.lang_flag_2))
   && (((basetype)->type.lang_flag_2)))
 warning ("base class `%#T' has a non-virtual destructor",
      ...
      if (! ((&((basetype)->type.lang_specific)->u.c)->has_const_init_ref))
 *cant_have_const_ctor_p = 1;
      if (((&((basetype)->type.lang_specific)->u.c)->has_assign_ref)
   && !((&((basetype)->type.lang_specific)->u.c)->h.has_const_assign_ref))
 *no_const_asn_ref_p = 1;



      if ((((basetype)->type.lang_flag_1))
   && ! ((&((basetype)->type.lang_specific)->u.c)->h.has_default_ctor))
 {
   *cant_have_default_ctor_p = 1;
   if (! (((t)->type.lang_flag_1)))
            pedwarn ("base `%T' with only non-default constructor in class without a constructor",
                        basetype);
 }

      if (((base_binfo)->common.static_flag))

 ;
      else if (((&((basetype)->type.lang_specific)->u.c)->nearly_empty_p))
 {
   if (seen_non_virtual_nearly_empty_base_p)


     ((&((t)->type.lang_specific)->u.c)->nearly_empty_p) = 0;
   else
 ...
 ((&((t)->type.lang_specific)->u.c)->nearly_empty_p) = 0;
      ...
      ((t)->type.needs_constructing_flag) |= ((basetype)->type.needs_constructing_flag);
      (((t)->type.lang_flag_4))
 |= (((basetype)->type.lang_flag_4));
      ((&((t)->type.lang_specific)->u.c)->has_complex_assign_ref)
 |= ((&((basetype)->type.lang_specific)->u.c)->has_complex_assign_ref);
      ((&((t)->type.lang_specific)->u.c)->has_complex_init_ref) |= ((&((basetype)->type.lang_specific)->u.c)->has_complex_init_ref);
      ((&((t)->type.lang_specific)->u.c)->has_call_overloaded) |= ((&((basetype)->type.lang_specific)->u.c)->has_call_overloaded);
      ((&((t)->type.lang_specific)->u.c)->has_array_ref_overloaded) |= ((&((basetype)->type.lang_specific)->u.c)->has_array_ref_overloaded);
      ((&((t)->type.lang_specific)->u.c)->has_arrow_overloaded) |= ((&((basetype)->type.lang_specific)->u.c)->has_arrow_overloaded);
      (((t)->common.lang_flag_2)) |= (((basetype)->common.lang_flag_2));
      ((&((t)->type.lang_specific)->u.c)->contains_empty_class_p)
 |= ((&((basetype)->type.lang_specific)->u.c)->contains_empty_class_p);
    }
}
...
force_canonical_binfo_r (to, from, type, mappings)
     ...
     tree mappings;
{
  int i, n_baseclasses = ((((from))->vec.a[4]) ? (((((from))->vec.a[4]))->vec.length) : 0);

  (void) (((to != from) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 1399, __FUNCTION__), 0) : 0);
  ((to)->common.used_flag)
          = ((from)->common.used_flag);
  ((from)->common.used_flag) = 0;
  ((to)->common.lang_flag_0) = ((from)->common.lang_flag_0);
  ((from)->common.lang_flag_0) = 0;
  ((to)->common.lang_flag_2) = ((from)->common.lang_flag_2);
  ((from)->common.lang_flag_2) = 0;
  if ((((from)->vec.a[10]) != (tree) ((void *)0)))
    {
      tree primary = ((from)->vec.a[10]);
      tree assoc;
      ...
      if (assoc)
        primary = ((assoc)->list.value);
      ((to)->vec.a[10]) = primary;
      ((from)->vec.a[10]) = (tree) ((void *)0);
    }
  (void) (((comptypes ((((to)->common.type)), (((from)->common.type)), 0)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 1421, __FUNCTION__), 0) : 0);

  mappings = tree_cons (from, to, mappings);

  if ((((&((((from)->common.type))->type.lang_specific)->u.c)->primary_base) != (tree) ((void *)0))
      && ((((&((((from)->common.type))->type.lang_specific)->u.c)->primary_base))->common.static_flag))
    {
      tree from_primary = get_primary_binfo (from);

      if (((from_primary)->vec.a[10]) == from)
 force_canonical_binfo (get_primary_binfo (to), from_primary,
  ...
  for (i = 0; i != n_baseclasses; i++)
    {
      tree from_binfo = (((((from))->vec.a[4]))->vec.a[(i)]);
      tree to_binfo = (((((to))->vec.a[4]))->vec.a[(i)]);

      if (((from_binfo)->common.static_flag))
        {
   if ((((from_binfo)->vec.a[10]) != (tree) ((void *)0)) &&
       purpose_member (((from_binfo)->vec.a[10]), mappings))
...
force_canonical_binfo (to, from, type, mappings)
     ...
     tree mappings;
{
  tree assoc = purpose_member (((to)->common.type),
                 ((&((type)->type.lang_specific)->u.c)->vbases));
  if (((assoc)->list.value) != to)
    {
      ((assoc)->list.value) = to;
      force_canonical_binfo_r (to, from, type, mappings);
    }
...
mark_primary_virtual_base (base_binfo, type)
     ...
     tree type;
{
  tree shared_binfo = binfo_for_vbase (((base_binfo)->common.type), type);

  if ((((shared_binfo)->vec.a[10]) != (tree) ((void *)0)))
    {
  ...
  if (base_binfo != shared_binfo)
    force_canonical_binfo (base_binfo, shared_binfo, type, ((void *)0));
     ...
     void *data;
{
  ...
  if (!((binfo)->common.lang_flag_0)
      && (((&((((binfo)->common.type))->type.lang_specific)->u.c)->primary_base) != (tree) ((void *)0)))
    {
      ...
      for (vbase = binfo; !((vbase)->common.static_flag);
    vbase = (((vbase))->vec.a[0]))
 continue;
      unshared_base = get_original_base (binfo,
      binfo_for_vbase (((vbase)->common.type),
         t));
      (void) (((unshared_base != binfo) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 1533, __FUNCTION__), 0) : 0);
      ((binfo)->common.lang_flag_2) = ((unshared_base)->common.lang_flag_2);
      if (!((binfo)->common.lang_flag_2))
 ((get_primary_binfo (binfo))->vec.a[10]) = binfo;
    }
  ...
  if ((((binfo)->vec.a[10]) != (tree) ((void *)0)) && ((binfo)->common.static_flag) &&
      binfo_for_vbase (((binfo)->common.type), t) == binfo)
    {
      tree delta = size_diffop ((((((binfo)->vec.a[10])))->vec.a[1]),
    (((binfo))->vec.a[1]));
      if (!integer_zerop (delta))
 propagate_binfo_offsets (binfo, delta, t);
    }

  ((binfo)->common.lang_flag_0) = 0;
  return ((void *)0);
}
     ...
     tree type;
{
  ...
  for (binfo = ((type)->type.binfo); binfo; binfo = ((binfo)->common.chain))
    {
      tree base_binfo;

      if (!(((&((((binfo)->common.type))->type.lang_specific)->u.c)->primary_base) != (tree) ((void *)0)))
      ...
      if (base_binfo)
        ((base_binfo)->vec.a[10]) = binfo;
      else
 ((binfo)->common.lang_flag_2) = 1;

      ((binfo)->common.lang_flag_0) = 1;
    }
# 1598 "../../../kg++fe/gnu/cp/class.c"
  dfs_walk (((type)->type.binfo), dfs_unshared_virtual_bases, ((void *)0), type);
}
     ...
     tree binfo;
{
  ...
  ((&((t)->type.lang_specific)->u.c)->primary_base) = binfo;
  basetype = ((binfo)->common.type);
  (((((t)->type.binfo)))->vec.a[2]) = (((((basetype)->type.binfo)))->vec.a[2]);
  (((((t)->type.binfo)))->vec.a[3]) = (((((basetype)->type.binfo)))->vec.a[3]);
  ((t)->type.minval) = ((basetype)->type.minval);
}
...
determine_primary_base (t)
     tree t;
{
  int i, n_baseclasses = (((((((t)->type.binfo)))->vec.a[4]) ? (((((((t)->type.binfo)))->vec.a[4]))->vec.length) : 0));
  tree vbases;
  tree type_binfo;
  ...
  type_binfo = ((t)->type.binfo);

  for (i = 0; i < n_baseclasses; i++)
    {
      tree base_binfo = (((((type_binfo))->vec.a[4]))->vec.a[(i)]);
      tree basetype = ((base_binfo)->common.type);

      if (((((basetype)->common.lang_flag_2)) || (((basetype)->common.lang_flag_3))))
 {
   ...
   if (!(((&((t)->type.lang_specific)->u.c)->primary_base) != (tree) ((void *)0)))
     {
       set_primary_base (t, base_binfo);
       ((&((t)->type.lang_specific)->u.c)->vfields) = copy_list (((&((basetype)->type.lang_specific)->u.c)->vfields));
     }
   else
     {
       tree vfields;
       ...
       for (vfields = ((&((basetype)->type.lang_specific)->u.c)->vfields);
     vfields;
     vfields = ((vfields)->common.chain))
  if (((vfields)->list.purpose) == (tree) ((void *)0)
      || ! ((((vfields)->list.purpose))->common.static_flag))
    ((&((t)->type.lang_specific)->u.c)->vfields)
  ...
  if (!((t)->type.minval))
    ((&((t)->type.lang_specific)->u.c)->primary_base) = (tree) ((void *)0);



  for (vbases = ((&((t)->type.lang_specific)->u.c)->vbases);
       vbases;
       vbases = ((vbases)->common.chain))
    {
      tree vbase_binfo = ((vbases)->list.value);
      ...
      for (i = 0; i < n_baseclasses; ++i)
 {
   tree basetype = (((((((((t)->type.binfo)))->vec.a[4]))->vec.a[(i)]))->common.type);
   tree v;

   for (v = ((&((basetype)->type.lang_specific)->u.c)->vbases);
        v;
        v = ((v)->common.chain))
     {
       tree base_vbase = ((v)->list.value);

       if ((((base_vbase)->vec.a[10]) != (tree) ((void *)0))
    && comptypes ((((base_vbase)->common.type)), (((vbase_binfo)->common.type)), 0))

  {
    ((vbase_binfo)->common.used_flag) = 1;
    break;
  }
     }
   ...
   if (((vbase_binfo)->common.used_flag))
     break;
 }
    }
  ...
  if (!(((&((t)->type.lang_specific)->u.c)->primary_base) != (tree) ((void *)0)))
    {
      ...
      tree candidate = (tree) ((void *)0);
      tree base_binfo;


      for (base_binfo = ((t)->type.binfo);
    base_binfo;
    base_binfo = ((base_binfo)->common.chain))
 {
   tree basetype = ((base_binfo)->common.type);

   if (((base_binfo)->common.static_flag)
       && ((&((basetype)->type.lang_specific)->u.c)->nearly_empty_p))
     {


       if (!((base_binfo)->common.used_flag))
  {
      ...
      if (candidate)
 {
   ...
   ((&((t)->type.lang_specific)->u.c)->vfields)
     = copy_list (((&((((candidate)->common.type))->type.lang_specific)->u.c)->vfields));
 }
    }
     ...
     tree t;
{
  ...
  tree variants = ((t)->type.next_variant);
  while (variants)
    {


      (((variants)->type.lang_flag_1)) = (((t)->type.lang_flag_1));
      (((variants)->type.lang_flag_2)) = (((t)->type.lang_flag_2));
      ((variants)->type.needs_constructing_flag) = ((t)->type.needs_constructing_flag);
      (((variants)->type.lang_flag_4))
 = (((t)->type.lang_flag_4));

      (((variants)->common.lang_flag_1))
 = (((t)->common.lang_flag_1));
      (((variants)->common.lang_flag_2)) = (((t)->common.lang_flag_2));
      (((variants)->common.lang_flag_3)) = (((t)->common.lang_flag_3));

      ((variants)->type.minval) = ((t)->type.minval);
      ((variants)->type.maxval) = ((t)->type.maxval);
      ((variants)->type.values) = ((t)->type.values);
      ((variants)->type.size) = ((t)->type.size);
      ((variants)->type.size_unit) = ((t)->type.size_unit);
      variants = ((variants)->type.next_variant);
    }

  if (n_baseclasses && (((t)->common.lang_flag_2)))
# 1802 "../../../kg++fe/gnu/cp/class.c"
      get_pure_virtuals (t);

  if (n_baseclasses)
    {

      tree binfo = ((t)->type.binfo);
      tree binfos = (((binfo))->vec.a[4]);
      tree basetype;

      for (i = n_baseclasses-1; i >= 0; i--)
 {
   basetype = ((((binfos)->vec.a[i]))->common.type);

   ((&((t)->type.lang_specific)->u.c)->h.has_type_conversion) |= ((&((basetype)->type.lang_specific)->u.c)->h.has_type_conversion);
 }
    }
  ...
  if (! (((&((t)->type.lang_specific)->u.c)->h.has_init_ref) && ! ((&((t)->type.lang_specific)->u.c)->has_complex_init_ref)) || (((t)->type.lang_flag_4)))
    {
      tree variants;
      (((((((t)->type.main_variant))->common.chain)))->decl.mode) = BLKmode;
      for (variants = t; variants; variants = ((variants)->type.next_variant))
 {
   ((variants)->type.mode) = BLKmode;
   ((variants)->common.addressable_flag) = 1;
 }
    }
}
# 1842 "../../../kg++fe/gnu/cp/class.c"
static void
     ...
     tree t;
{
  ...
  if (!warn_ctor_dtor_privacy
      ...
      || (((&((t)->type.lang_specific)->u.c)->friend_classes)
   || ((((((((t)->type.main_variant))->common.chain)))->decl.initial)))
    ...
    return;
# 1878 "../../../kg++fe/gnu/cp/class.c"
  for (fn = ((t)->type.maxval); fn; fn = ((fn)->common.chain))


    if (!((fn)->decl.artificial_flag))
      {
 if (!((fn)->common.private_flag))
   {
     if ((((fn)->decl.lang_specific)->decl_flags.static_function))
 ...
 else if (!(((fn)->decl.lang_specific)->decl_flags.constructor_attr) && !(((fn)->decl.lang_specific)->decl_flags.destructor_attr))
   has_member_fn = 1;
      }
  ...
  if (!has_nonprivate_method && has_member_fn)
    {
      ...
      int i;
      tree binfos = (((((t)->type.binfo)))->vec.a[4]);
      for (i = 0; i < (((((((t)->type.binfo)))->vec.a[4]) ? (((((((t)->type.binfo)))->vec.a[4]))->vec.length) : 0)); i++)
 if (((((binfos)->vec.a[i]))->common.public_flag)
     || ((((binfos)->vec.a[i]))->common.protected_flag))
   {
     has_nonprivate_method = 1;
     break;
   }
      if (!has_nonprivate_method)
 {
  ...
  if ((((t)->type.lang_flag_2)))
    {
      tree dtor = ((((&((t)->type.lang_specific)->u.c)->methods))->vec.a[1]);

      if (((dtor)->common.private_flag))
 {
...
# 1952 "../../../kg++fe/gnu/cp/class.c"
      if (!((&((t)->type.lang_specific)->u.c)->h.has_init_ref))
 nonprivate_ctor = 1;
      else
 for (fn = ((((&((t)->type.lang_specific)->u.c)->methods))->vec.a[0]);
      fn;
      fn = ((((enum tree_code) (fn)->common.code) == OVERLOAD) ? ((fn)->common.chain) : (tree) ((void *)0)))
   {
     tree ctor = ((((enum tree_code) (fn)->common.code) == OVERLOAD) ? (((struct tree_overload*)(fn))->function) : (fn));
     ...
     const tree *x, *y;
{
  if (((*x)->decl.name) == ((*y)->decl.name))
    ...
    return (((enum tree_code) (*y)->common.code) == TYPE_DECL || (((enum tree_code) (*y)->common.code) == TEMPLATE_DECL && ((enum tree_code) (((*y)->decl.result))->common.code) == TYPE_DECL && !(((enum tree_code) (*y)->common.code) == TEMPLATE_DECL && (((*y)->decl.lang_flag_0) && (((enum tree_code) (*y)->common.code) == CONST_DECL || ((enum tree_code) (*y)->common.code) == PARM_DECL || ((enum tree_code) (*y)->common.code) == TYPE_DECL || ((enum tree_code) (*y)->common.code) == TEMPLATE_DECL))))) - (((enum tree_code) (*x)->common.code) == TYPE_DECL || (((enum tree_code) (*x)->common.code) == TEMPLATE_DECL && ((enum tree_code) (((*x)->decl.result))->common.code) == TYPE_DECL && !(((enum tree_code) (*x)->common.code) == TEMPLATE_DECL && (((*x)->decl.lang_flag_0) && (((enum tree_code) (*x)->common.code) == CONST_DECL || ((enum tree_code) (*x)->common.code) == PARM_DECL || ((enum tree_code) (*x)->common.code) == TYPE_DECL || ((enum tree_code) (*x)->common.code) == TEMPLATE_DECL)))));
...
method_name_cmp (m1, m2)
     const tree *m1, *m2;
{
  if (*m1 == (tree) ((void *)0) && *m2 == (tree) ((void *)0))
    return 0;
  if (*m1 == (tree) ((void *)0))
    return -1;
  if (*m2 == (tree) ((void *)0))
    return 1;
  if (((((((enum tree_code) (*m1)->common.code) == OVERLOAD) ? (((struct tree_overload*)(*m1))->function) : (*m1)))->decl.name) < ((((((enum tree_code) (*m2)->common.code) == OVERLOAD) ? (((struct tree_overload*)(*m2))->function) : (*m2)))->decl.name))
    return -1;
  return 1;
}
# 2033 "../../../kg++fe/gnu/cp/class.c"
static void
     ...
     tree t;
{
  ...
  if (!((t)->type.maxval))
    {
      ...
      (((t)->type.lang_flag_2)) = 0;
      ((&((t)->type.lang_specific)->u.c)->methods) = (tree) ((void *)0);
      return;
    }

  method_vec = ((&((t)->type.lang_specific)->u.c)->methods);
  (void) (((method_vec != (tree) ((void *)0)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 2052, __FUNCTION__), 0) : 0);
  len = ((method_vec)->vec.length);



  for (fn_fields = ((t)->type.maxval); fn_fields;
       fn_fields = ((fn_fields)->common.chain))

    (((fn_fields)->decl.lang_flag_3)) = 0;

  if ((((t)->type.lang_flag_2)) && !(((((&((t)->type.lang_specific)->u.c)->methods))->vec.a[1])))
  ...
  while (len > 2 && ((method_vec)->vec.a[len-1]) == (tree) ((void *)0))
    len--;
  ((method_vec)->vec.length) = len;
 ...
 break;
  ...
  if (len - slot > 1)
    qsort (&((method_vec)->vec.a[slot]), len-slot, sizeof (tree),
...
# 2109 "../../../kg++fe/gnu/cp/class.c"
  if (((&((t)->type.lang_specific)->u.c)->methods))
    {
      tree method_vec = ((&((t)->type.lang_specific)->u.c)->methods);
      int i, len = ((method_vec)->vec.length);
      for (i = 0; i < len; i++)
 {
   tree unchain = ((method_vec)->vec.a[i]);
   while (unchain != (tree) ((void *)0))
     {
       ((((((enum tree_code) (unchain)->common.code) == OVERLOAD) ? (((struct tree_overload*)(unchain))->function) : (unchain)))->common.chain) = (tree) ((void *)0);
       unchain = ((((enum tree_code) (unchain)->common.code) == OVERLOAD) ? ((unchain)->common.chain) : (tree) ((void *)0));
     }
 }
    }

  if (((t)->type.lang_specific))
    {
      tree binfo = ((t)->type.binfo);
      int interface_only = ((&((t)->type.lang_specific)->u.c)->interface_only);
      int interface_unknown = ((&((t)->type.lang_specific)->u.c)->interface_unknown);
      tree template_info = ((&(((t))->type.lang_specific)->u.c)->template_info);
      int use_template = ((&((t)->type.lang_specific)->u.c)->use_template);

      memset ((char *) ((t)->type.lang_specific), 0, sizeof (struct lang_type));
      (((binfo))->vec.a[4]) = (tree) ((void *)0);

      ((t)->type.lang_specific)->u.h.is_lang_type_class = 1;
      ((t)->type.binfo) = binfo;
      ((&((t)->type.lang_specific)->u.c)->interface_only) = interface_only;
      ((&((t)->type.lang_specific)->u.c)->interface_unknown = !!(interface_unknown));
      ((&((t)->type.lang_specific)->u.c)->redefined) = 1;
      ((&(((t))->type.lang_specific)->u.c)->template_info) = template_info;
      ((&((t)->type.lang_specific)->u.c)->use_template) = use_template;
      ((&((t)->type.lang_specific)->u.c)->decl_list) = (tree) ((void *)0);
    }
  ((t)->type.size) = (tree) ((void *)0);
  ((t)->type.mode) = VOIDmode;
  ((t)->type.values) = (tree) ((void *)0);
  ((t)->type.maxval) = (tree) ((void *)0);
  ((t)->type.minval) = (tree) ((void *)0);
  ((t)->type.context) = (tree) ((void *)0);


  ((t)->type.lang_flag_0) = 0;
  ((t)->type.lang_flag_1) = 0;
  ((t)->type.lang_flag_2) = 0;
  ((t)->type.lang_flag_3) = 0;
  ((t)->type.lang_flag_4) = 0;
  ((t)->type.lang_flag_5) = 0;
  ((t)->type.lang_flag_6) = 0;

  (((t)->type.lang_flag_5) = (1));
}
     ...
     int n;
{
  ...
  atype = build_cplus_array_type (cp_global_trees[CPTI_VTABLE_ENTRY_TYPE],
      build_index_type (size_int_wide ((long long) (n - 1), SIZETYPE)));
  layout_type (atype);


  vtable = get_vtbl_decl_for_binfo (binfo);
  if (!comptypes ((((vtable)->common.type)), (atype), 0))
    {
      ((vtable)->common.type) = atype;
      ((vtable)->decl.size) = ((vtable)->decl.size_unit) = (tree) ((void *)0);
      layout_decl (vtable, 0);



      ((vtable)->decl.u1.a.align) = ((((global_trees[TI_DOUBLE_TYPE])->type.align)) > (((vtable)->decl.u1.a.align)) ? (((global_trees[TI_DOUBLE_TYPE])->type.align)) : (((vtable)->decl.u1.a.align)));
     ...
     tree fndecl, base_fndecl;
{
  ...
  if ((((base_fndecl)->decl.lang_specific)->decl_flags.destructor_attr) && (((fndecl)->decl.lang_specific)->decl_flags.destructor_attr)
  ...
  if (((fndecl)->decl.name) == ((base_fndecl)->decl.name))
    {
      tree types, base_types;
      types = ((((fndecl)->common.type))->type.values);
      base_types = ((((base_fndecl)->common.type))->type.values);
      if ((((((((((base_types)->list.value))->common.type))->common.readonly_flag) * 0x1) | (((((((base_types)->list.value))->common.type))->common.volatile_flag) * 0x2) | (((((((base_types)->list.value))->common.type))->type.restrict_flag) * 0x4) | ((((enum tree_code) (((((base_types)->list.value))->common.type))->common.code) == RECORD_TYPE && ((((((base_types)->list.value))->common.type))->common.type)) * 0x8))
    == ((((((((types)->list.value))->common.type))->common.readonly_flag) * 0x1) | (((((((types)->list.value))->common.type))->common.volatile_flag) * 0x2) | (((((((types)->list.value))->common.type))->type.restrict_flag) * 0x4) | ((((enum tree_code) (((((types)->list.value))->common.type))->common.code) == RECORD_TYPE && ((((((types)->list.value))->common.type))->common.type)) * 0x8)))
   && compparms (((base_types)->common.chain), ((types)->common.chain)))
 return 1;
    }
  return 0;
}
...
dfs_base_derived_from (tree binfo, void *data)
{
  ...
  if (comptypes ((((base)->common.type)), (((binfo)->common.type)), 0)
      && tree_int_cst_equal ((((base))->vec.a[1]), (((binfo))->vec.a[1])))
    return global_trees[TI_ERROR_MARK];
...
{
  return dfs_walk (derived, dfs_base_derived_from, ((void *)0), base) != (tree) ((void *)0);
}
     ...
     void *data;
{
  ...
  if (comptypes ((((binfo)->common.type)), (((ffod->declaring_base)->common.type)), 0)

      && tree_int_cst_equal ((((binfo))->vec.a[1]),
        (((ffod->declaring_base))->vec.a[1])))
    {
      tree path;
      tree method;
      ...
      while (!comptypes ((((((path)->list.value))->common.type)), (ffod->most_derived_type), 0))

 path = ((path)->common.chain);
      while (path)
 {
   method = look_for_overrides_here (((((path)->list.value))->common.type),
         ffod->fn);
   if (method)
     {
       path = ((path)->list.value);
       break;
     }
   ...
   while (*candidate)
     {
       ...
       if (base_derived_from (((*candidate)->list.value), path))
    return (tree) ((void *)0);

       if (base_derived_from (path, ((*candidate)->list.value)))
  *candidate = ((*candidate)->common.chain);
       else
  candidate = &((*candidate)->common.chain);
     }
...
# 2358 "../../../kg++fe/gnu/cp/class.c"
  ffod.fn = fn;
  ffod.declaring_base = binfo;
  ffod.most_derived_type = ((derived)->common.type);
  ffod.candidates = (tree) ((void *)0);
  ...
  if (!ffod.candidates || ((ffod.candidates)->common.chain))
    {
...
get_vcall_index (tree fn, tree type)
{
  ...
  for (v = ((&((type)->type.lang_specific)->u.c)->vcall_indices); v; v = ((v)->common.chain))
    if (((((fn)->decl.lang_specific)->decl_flags.destructor_attr) && (((((v)->list.purpose))->decl.lang_specific)->decl_flags.destructor_attr))
 || same_signature_p (fn, ((v)->list.purpose)))
      break;


  (void) (((v) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 2393, __FUNCTION__), 0) : 0);
     ...
     tree *virtuals;
{
  ...
  for (b = binfo; ; b = get_primary_binfo (b))
    {
      if (look_for_overrides_here (((b)->common.type), fn))
 break;


      if (((b)->common.lang_flag_2))
 lost = 1;
    }
  first_defn = b;


  overrider = find_final_overrider (((t)->type.binfo), b, fn);
  if (overrider == global_trees[TI_ERROR_MARK])
    return;



  check_final_overrider (((overrider)->list.purpose), fn);



  virtual_base = (tree) ((void *)0);
  ...
  for (; b; b = (((b))->vec.a[0]))
    {
      ...
      if (comptypes ((((b)->common.type)), (((((overrider)->list.value))->common.type)), 0))
 ...
 break;
      ...
      if (!virtual_base && ((b)->common.static_flag))
 virtual_base = b;
    }
  ...
  if (virtual_base)
    ...
    delta = size_diffop ((((virtual_base))->vec.a[1]),
    (((first_defn))->vec.a[1]));
  else if (lost)
    ...
    delta = global_trees[TI_SIZE_ZERO];
  else



    delta = size_diffop ((((((overrider)->list.value)))->vec.a[1]),
    (((binfo))->vec.a[1]));

  modify_vtable_entry (t,
         binfo,
         ((overrider)->list.purpose),
         delta,
         virtuals);

  if (virtual_base)
    (((*virtuals)->common.type))
      = get_vcall_index (((overrider)->list.purpose),
    ((virtual_base)->common.type));
}
     ...
     void *data;
{
  if (
      ...
      (!(((binfo)->vec.a[10]) != (tree) ((void *)0)) || ((binfo)->common.static_flag))
      ...
      && ((&((((binfo)->common.type))->type.lang_specific)->u.c)->vfields))
    {
      ...
      for (virtuals = (((binfo))->vec.a[3]),
      old_virtuals = (((((((binfo)->common.type))->type.binfo)))->vec.a[3]);
    virtuals;
    virtuals = ((virtuals)->common.chain),
      old_virtuals = ((old_virtuals)->common.chain))
 update_vtable_entry_for_fn (t,
        binfo,
        (((old_virtuals)->list.value)),
        &virtuals);
    }
     ...
     tree virtuals;
{
  ...
  for (fnsp = &virtuals; *fnsp; )
    {
      tree fn = ((*fnsp)->list.value);

      if (!value_member (fn, (((binfo))->vec.a[3]))
   || ((fn)->decl.vindex) == global_trees[TI_ERROR_MARK])
 {


   (((*fnsp)->list.purpose)) = global_trees[TI_INTEGER_ZERO];
   (((*fnsp)->common.type)) = (tree) ((void *)0);


   fnsp = &((*fnsp)->common.chain);
 }
      else
...
{
  tree methods;
  tree base_fndecls = (tree) ((void *)0);
  int n_baseclasses = (((((((t)->type.binfo)))->vec.a[4]) ? (((((((t)->type.binfo)))->vec.a[4]))->vec.length) : 0));
  int i;


  i = lookup_fnfields_1 (t, name);
  if (i != -1)
    for (methods = ((((&((t)->type.lang_specific)->u.c)->methods))->vec.a[i]);
  methods;
  methods = ((((enum tree_code) (methods)->common.code) == OVERLOAD) ? ((methods)->common.chain) : (tree) ((void *)0)))
      {
 tree method = ((((enum tree_code) (methods)->common.code) == OVERLOAD) ? (((struct tree_overload*)(methods))->function) : (methods));

 if (((enum tree_code) (method)->common.code) == FUNCTION_DECL
     && ((method)->decl.vindex))
   base_fndecls = tree_cons ((tree) ((void *)0), method, base_fndecls);
      }
     ...
     tree decl, ctype;
{
  if (((enum tree_code) (decl)->common.code) == TEMPLATE_DECL)
    ...
    return;
  if (((((decl)->decl.lang_specific)->decl_flags.destructor_attr)
       || ((((decl)->decl.name))->common.lang_flag_1))
      && look_for_overrides (ctype, decl)
      && !(((decl)->decl.lang_specific)->decl_flags.static_function))



    ((decl)->decl.vindex) = decl;

  if (((decl)->decl.virtual_flag))
    {
      if (!((decl)->decl.vindex))
 ((decl)->decl.vindex) = global_trees[TI_ERROR_MARK];
...
{
  tree method_vec = ((&((t)->type.lang_specific)->u.c)->methods);
  int n_methods = method_vec ? ((method_vec)->vec.length) : 0;
  int i;


  for (i = 2; i < n_methods && ((method_vec)->vec.a[i]); ++i)
    {
      tree fns;
      tree name;
      tree fndecl;
      ...
      name = ((((((enum tree_code) (((method_vec)->vec.a[i]))->common.code) == OVERLOAD) ? (((struct tree_overload*)(((method_vec)->vec.a[i])))->function) : (((method_vec)->vec.a[i]))))->decl.name);

      base_fndecls = (tree) ((void *)0);


      for (j = 0; j < (((((((t)->type.binfo)))->vec.a[4]) ? (((((((t)->type.binfo)))->vec.a[4]))->vec.length) : 0)); j++)
 {
   tree basetype = (((((((((t)->type.binfo)))->vec.a[4]))->vec.a[(j)]))->common.type);
   base_fndecls = chainon (get_basefndecls (name, basetype),
      base_fndecls);
 }
 ...
 continue;
      ...
      for (fns = ((method_vec)->vec.a[i]); fns; fns = ((((enum tree_code) (fns)->common.code) == OVERLOAD) ? ((fns)->common.chain) : (tree) ((void *)0)))
 {
   fndecl = ((((enum tree_code) (fns)->common.code) == OVERLOAD) ? (((struct tree_overload*)(fns))->function) : (fns));
   if (((fndecl)->decl.vindex))
     {
  ...
  if (same_signature_p (fndecl, ((*prev)->list.value)))
      ...
      while (base_fndecls)
 {
   ...
   cp_warning_at ("`%D' was hidden", ((base_fndecls)->list.value));
   cp_warning_at ("  by `%D'",
    ((((enum tree_code) (((method_vec)->vec.a[i]))->common.code) == OVERLOAD) ? (((struct tree_overload*)(((method_vec)->vec.a[i])))->function) : (((method_vec)->vec.a[i]))));
   base_fndecls = ((base_fndecls)->common.chain);
 }
    }
}
     ...
     tree t;
{
 ...
 {
   tree elt = ((((field)->common.type))->type.values);
   for (; elt; elt = ((elt)->common.chain))
     {
       ...
       if (((elt)->decl.artificial_flag)
    && (!(((enum tree_code) (elt)->common.code) == TYPE_DECL && ((elt)->decl.lang_flag_2))
        || (((((((enum tree_code) (((elt)->common.type))->common.code)) == RECORD_TYPE || (((enum tree_code) (((elt)->common.type))->common.code)) == UNION_TYPE) && (((enum tree_code) (((elt)->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((elt)->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((elt)->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((elt)->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((elt)->common.type))->type.lang_flag_5))) || ((enum tree_code) (((elt)->common.type))->common.code) == ENUMERAL_TYPE) && (((char *)(((((((((((elt)->common.type))->type.main_variant))->type.name))->decl.name))))->identifier.id.str)[0] == '.' && ((char *)(((((((((((elt)->common.type))->type.main_variant))->type.name))->decl.name))))->identifier.id.str)[1] == '_'))))
  continue;

       if (((elt)->decl.name) == constructor_name (t))
  cp_pedwarn_at ("ISO C++ forbids member `%D' with same name as enclosing class",
          elt);

       if (((enum tree_code) (elt)->common.code) != FIELD_DECL)
  {
    ...
    continue;
       ...
       if (((elt)->common.private_flag))
  cp_pedwarn_at ("private member `%#D' in anonymous union",
          elt);
       else if (((elt)->common.protected_flag))
  cp_pedwarn_at ("protected member `%#D' in anonymous union",
          elt);

       ((elt)->common.private_flag) = ((field)->common.private_flag);
       ((elt)->common.protected_flag) = ((field)->common.protected_flag);
     }
 }
    }
}
# 2795 "../../../kg++fe/gnu/cp/class.c"
void
     ...
     int friend_p;
{
  ...
  if (((&(((type))->type.lang_specific)->u.c)->template_info))
    ((&((type)->type.lang_specific)->u.c)->decl_list)
      = tree_cons (friend_p ? (tree) ((void *)0) : type,
     t, ((&((type)->type.lang_specific)->u.c)->decl_list));
}
# 2817 "../../../kg++fe/gnu/cp/class.c"
static void
...
{
  tree default_fn;
  tree implicit_fns = (tree) ((void *)0);
  tree virtual_dtor = (tree) ((void *)0);
  tree *f;

  ++adding_implicit_members;


  if ((((t)->type.lang_flag_4)) && !(((t)->type.lang_flag_2)))
    {
      default_fn = implicitly_declare_fn (sfk_destructor, t, 0);
      check_for_override (default_fn, t);


      if (default_fn == global_trees[TI_VOID_TYPE])
 (((t)->type.lang_flag_4)) = 0;
      else
 {
   ((default_fn)->common.chain) = implicit_fns;
   implicit_fns = default_fn;

   if (((default_fn)->decl.vindex))
     virtual_dtor = default_fn;
 }
    }
  else

    (((t)->type.lang_flag_4)) |= (((t)->type.lang_flag_2));


  if (! (((t)->type.lang_flag_1)) && ! cant_have_default_ctor)
    {
  ...
  if (! ((&((t)->type.lang_specific)->u.c)->h.has_init_ref) && ! ((t)->type.lang_flag_3))
    {
 ...
 = implicitly_declare_fn (sfk_copy_constructor, t,
      ...
      ((&((t)->type.lang_specific)->u.c)->has_implicit_copy_constructor) = 1;

    }


  if (! ((&((t)->type.lang_specific)->u.c)->has_assign_ref) && ! ((t)->type.lang_flag_3))
    {
      default_fn
 = implicitly_declare_fn (sfk_assignment_operator, t,
  ...
  for (f = &implicit_fns; *f; f = &((*f)->common.chain))
    {
      add_method (t, *f, 0);
      maybe_add_class_template_decl_list (scope_chain->class_type, *f, 0);
    }
  if ((flag_abi_version == 0 || flag_abi_version >= (2)))



    ((t)->type.maxval) = chainon (((t)->type.maxval), implicit_fns);
  else
    {
     ...
     tree fields;
{
  tree x;
  int n_fields = 0;
  for (x = fields; x; x = ((x)->common.chain))
    {
      if (((enum tree_code) (x)->common.code) == FIELD_DECL && ((((((enum tree_code) (((x)->common.type))->common.code)) == RECORD_TYPE || (((enum tree_code) (((x)->common.type))->common.code)) == UNION_TYPE) && (((enum tree_code) (((x)->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((x)->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((x)->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((x)->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((x)->common.type))->type.lang_flag_5))) && (&((((x)->common.type))->type.lang_specific)->u.c)->anon_aggr))
 n_fields += count_fields (((((x)->common.type))->type.values));
      else
 n_fields += 1;
    }
  return n_fields;
     ...
     int idx;
{
  ...
  for (x = fields; x; x = ((x)->common.chain))
    {
      if (((enum tree_code) (x)->common.code) == FIELD_DECL && ((((((enum tree_code) (((x)->common.type))->common.code)) == RECORD_TYPE || (((enum tree_code) (((x)->common.type))->common.code)) == UNION_TYPE) && (((enum tree_code) (((x)->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((x)->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((x)->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((x)->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((x)->common.type))->type.lang_flag_5))) && (&((((x)->common.type))->type.lang_specific)->u.c)->anon_aggr))
 idx = add_fields_to_vec (((((x)->common.type))->type.values), field_vec, idx);
      else
 ((field_vec)->vec.a[idx++]) = x;
    }
  return idx;
}
     ...
     tree field;
{
  ...
  if (((field)->decl.initial)
      && ! (((enum tree_code) (((field)->common.type))->common.code) == INTEGER_TYPE || ((enum tree_code) (((field)->common.type))->common.code) == ENUMERAL_TYPE || ((enum tree_code) (((field)->common.type))->common.code) == BOOLEAN_TYPE || ((enum tree_code) (((field)->common.type))->common.code) == CHAR_TYPE))
    {
      cp_error_at ("bit-field `%#D' with non-integral type", field);
      w = global_trees[TI_ERROR_MARK];
    }


  if (((field)->decl.initial))
    {
      w = ((field)->decl.initial);


      while ((((enum tree_code) (w)->common.code) == NOP_EXPR || ((enum tree_code) (w)->common.code) == CONVERT_EXPR || ((enum tree_code) (w)->common.code) == NON_LVALUE_EXPR) && ((w)->exp.operands[0]) != global_trees[TI_ERROR_MARK] && (((((w)->common.type))->type.mode) == ((((((w)->exp.operands[0]))->common.type))->type.mode))) (w) = ((w)->exp.operands[0]);


      if (((enum tree_code) (w)->common.code) == CONST_DECL)
 w = ((w)->decl.initial);
      else
 w = decl_constant_value (w);

      if (((enum tree_code) (w)->common.code) != INTEGER_CST)
 {
      ...
      else if (tree_int_cst_sgn (w) < 0)
 {
   ...
   w = global_trees[TI_ERROR_MARK];
      ...
      else if (integer_zerop (w) && ((field)->decl.name) != 0)
 {
   cp_error_at ("zero width for bit-field `%D'", field);
   w = global_trees[TI_ERROR_MARK];
 }
      else if (compare_tree_int (w, ((type)->type.precision)) > 0
 ...
 cp_warning_at ("width of `%D' exceeds its type", field);
      ...
      min_precision (((type)->type.minval),
 ...
 cp_warning_at ("`%D' is too small to hold all values of `%#T'",
  ...
  ((field)->decl.initial) = (tree) ((void *)0);

  if (w != global_trees[TI_ERROR_MARK])
    {
      ((field)->decl.size) = convert (sizetype_tab[(int) BITSIZETYPE], w);
      ((field)->decl.bit_field_flag) = 1;

      if (integer_zerop (w)
   && ! (* targetm.ms_bitfield_layout_p) (((field)->decl.context)))
 {

   ((field)->decl.u1.a.align) = ((((field)->decl.u1.a.align)) > (32) ? (((field)->decl.u1.a.align)) : (32));



   if (1)
     {
       ((field)->decl.u1.a.align) = ((((field)->decl.u1.a.align)) > (((type)->type.align)) ? (((field)->decl.u1.a.align)) : (((type)->type.align)));

       ((field)->decl.user_align) |= ((type)->type.user_align);
     }
  ...
  else
    {
      ...
      ((field)->decl.bit_field_flag) = 0;
      ((((field))->decl.lang_flag_4) = 0);
      ((field)->decl.u1.a.align) = ((((field)->decl.u1.a.align)) > (((type)->type.align)) ? (((field)->decl.u1.a.align)) : (((type)->type.align)));
      ((field)->decl.user_align) |= ((type)->type.user_align);
    }
}
...
{
  tree type = strip_array_types (((field)->common.type));



  if ((((enum tree_code) (type)->common.code) == UNION_TYPE && ((((((enum tree_code) (type)->common.code)) == RECORD_TYPE || (((enum tree_code) (type)->common.code)) == UNION_TYPE) && (((enum tree_code) (type)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (type)->common.code) == TYPENAME_TYPE || ((enum tree_code) (type)->common.code) == TYPEOF_TYPE || ((enum tree_code) (type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((type)->type.lang_flag_5))) && (&((type)->type.lang_specific)->u.c)->anon_aggr)))
    ;
  ...
  else if (((((((enum tree_code) (type)->common.code)) == RECORD_TYPE || (((enum tree_code) (type)->common.code)) == UNION_TYPE) && (((enum tree_code) (type)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (type)->common.code) == TYPENAME_TYPE || ((enum tree_code) (type)->common.code) == TYPEOF_TYPE || ((enum tree_code) (type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((type)->type.lang_flag_5))) && (&((type)->type.lang_specific)->u.c)->anon_aggr))
    {
      ...
      for (fields = ((type)->type.values); fields; fields = ((fields)->common.chain))
 if (((enum tree_code) (fields)->common.code) == FIELD_DECL && !((((field))->decl.lang_flag_4) == 1))
 ...
 {
   ((t)->type.needs_constructing_flag) |= ((type)->type.needs_constructing_flag);
   (((t)->type.lang_flag_4))
     |= (((type)->type.lang_flag_4));
   ((&((t)->type.lang_specific)->u.c)->has_complex_assign_ref) |= ((&((type)->type.lang_specific)->u.c)->has_complex_assign_ref);
   ((&((t)->type.lang_specific)->u.c)->has_complex_init_ref) |= ((&((type)->type.lang_specific)->u.c)->has_complex_init_ref);
 }

      if (!((&((type)->type.lang_specific)->u.c)->has_const_init_ref))
 *cant_have_const_ctor = 1;

      if (!((&((type)->type.lang_specific)->u.c)->h.has_const_assign_ref))
 *no_const_asn_ref = 1;

      if ((((type)->type.lang_flag_1))
   && ! ((&((type)->type.lang_specific)->u.c)->h.has_default_ctor))
 *cant_have_default_ctor = 1;
    }
  if (((field)->decl.initial) != (tree) ((void *)0))
    {


      if (((enum tree_code) (t)->common.code) == UNION_TYPE && any_default_members != 0)
 cp_error_at ("multiple fields in union `%T' initialized");
      *any_default_members = 1;
    }
}
# 3164 "../../../kg++fe/gnu/cp/class.c"
static void
     ...
     int *no_const_asn_ref_p)
{
  ...
  delete_duplicate_fields (((t)->type.values));


  *access_decls = (tree) ((void *)0);
  ...
  for (field = &((t)->type.values); *field; field = next)
    {
      tree x = *field;
      ...
      if (((enum tree_code) (x)->common.code) == FIELD_DECL)
 {
   ((x)->decl.regdecl_flag) |= ((t)->type.packed_flag);

   if (((((x))->decl.lang_flag_4) == 1) && integer_zerop (((x)->decl.initial)))
   ...
   else
     {
       ...
       ((&((t)->type.lang_specific)->u.c)->empty_p) = 0;

       ((&((t)->type.lang_specific)->u.c)->nearly_empty_p) = 0;


       element_type = strip_array_types (type);
       if ((((((enum tree_code) (element_type)->common.code)) == RECORD_TYPE || (((enum tree_code) (element_type)->common.code)) == UNION_TYPE) && (((enum tree_code) (element_type)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (element_type)->common.code) == TYPENAME_TYPE || ((enum tree_code) (element_type)->common.code) == TYPEOF_TYPE || ((enum tree_code) (element_type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((element_type)->type.lang_flag_5)))
    && ((&((element_type)->type.lang_specific)->u.c)->contains_empty_class_p))
  ((&((t)->type.lang_specific)->u.c)->contains_empty_class_p) = 1;
     }
 }

      if (((enum tree_code) (x)->common.code) == USING_DECL)
 {

   *field = ((x)->common.chain);


   *access_decls = tree_cons ((tree) ((void *)0), x, *access_decls);
 ...
 continue;
      ...
      if (current_function_decl && ((x)->common.static_flag))
 ...
 continue;
      ...
      ((x)->decl.nonlocal_flag) = 1;

      if (((enum tree_code) (x)->common.code) == CONST_DECL)
 continue;
      ...
      if (((enum tree_code) (x)->common.code) == VAR_DECL)
 {
   ...
   continue;
      ...
      if (((x)->common.private_flag) || ((x)->common.protected_flag))
 ((&((t)->type.lang_specific)->u.c)->non_aggregate) = 1;



      if (((enum tree_code) (type)->common.code) == REFERENCE_TYPE)
  {
   ((&((t)->type.lang_specific)->u.c)->non_pod_class) = 1;
   if (((x)->decl.initial) == (tree) ((void *)0))
     ((&((t)->type.lang_specific)->u.c)->h.ref_needs_init = (1));
   ...
   *cant_have_default_ctor_p = 1;
   ((&((t)->type.lang_specific)->u.c)->has_complex_assign_ref) = 1;

   if (! (((t)->type.lang_flag_1)) && ((&((t)->type.lang_specific)->u.c)->non_aggregate)
       && extra_warnings)
 ...
 has_pointers = 1;
      ...
      if ((((x)->decl.lang_flag_0)) || (cp_has_mutable_p (type)))
 ((&((t)->type.lang_specific)->u.c)->has_mutable) = 1;
 ...
 ((&((t)->type.lang_specific)->u.c)->non_pod_class) = 1;
      ...
      if (! zero_init_p (type))
 ((&((t)->type.lang_specific)->u.c)->non_zero_init) = 1;


      if (((cp_type_quals (type) & 0x1) != 0))
 {
   ((t)->type.lang_flag_0) = 1;
   if (((x)->decl.initial) == (tree) ((void *)0))
     ((&((t)->type.lang_specific)->u.c)->h.const_needs_init = (1));
 ...
 {
   ((t)->type.lang_flag_0) |= ((type)->type.lang_flag_0);
   ((&((t)->type.lang_specific)->u.c)->h.const_needs_init = ((((t)->type.lang_specific) ? (&((t)->type.lang_specific)->u.c)->h.const_needs_init : 0) | (((type)->type.lang_specific) ? (&((type)->type.lang_specific)->u.c)->h.const_needs_init : 0)));
      ...
      if (((x)->decl.name) == constructor_name (t)
   && (((t)->type.lang_flag_1)))
 cp_pedwarn_at ("field `%#D' with same name as class", x);



      if (((((x))->decl.lang_flag_4) == 1))
 ...
 check_field_decl (x, t,
  ...
  if (has_pointers && warn_ecpp && (((t)->type.lang_flag_1))
      && ! (((&((t)->type.lang_specific)->u.c)->h.has_init_ref) && ((&((t)->type.lang_specific)->u.c)->has_assign_ref)))
    {
      ...
      if (! ((&((t)->type.lang_specific)->u.c)->h.has_init_ref))
 {
   ...
   if (! ((&((t)->type.lang_specific)->u.c)->has_assign_ref))
     warning ("  or `operator=(const %T&)'", t);
 }
      else if (! ((&((t)->type.lang_specific)->u.c)->has_assign_ref))
     ...
     splay_tree offsets;
{
  ...
  for (t = (tree) n->value; t; t = ((t)->common.chain))
    if (comptypes ((((t)->list.value)), (type), 0))
      return 1;

  return 0;
}
# 3463 "../../../kg++fe/gnu/cp/class.c"
static int
...
{
  int r = 0;
  tree type_binfo = (tree) ((void *)0);



  if (max_offset && ((((max_offset)->int_cst.int_cst).high) < (((offset)->int_cst.int_cst).high) || ((((max_offset)->int_cst.int_cst).high) == (((offset)->int_cst.int_cst).high) && (((max_offset)->int_cst.int_cst).low) < (((offset)->int_cst.int_cst).low))))
    return 0;
  ...
  if ((((((enum tree_code) (type)->common.code)) == RECORD_TYPE || (((enum tree_code) (type)->common.code)) == UNION_TYPE) && (((enum tree_code) (type)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (type)->common.code) == TYPENAME_TYPE || ((enum tree_code) (type)->common.code) == TYPEOF_TYPE || ((enum tree_code) (type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((type)->type.lang_flag_5))))
    {
      ...
      if (!((&((type)->type.lang_specific)->u.c)->contains_empty_class_p))
 return 0;
 ...
 return r;
      ...
      if (!type_binfo)
 type_binfo = ((type)->type.binfo);
      for (i = 0; i < ((((type_binfo))->vec.a[4]) ? (((((type_binfo))->vec.a[4]))->vec.length) : 0); ++i)
 {
   tree binfo_offset;

   binfo = (((((type_binfo))->vec.a[4]))->vec.a[(i)]);

   if ((flag_abi_version == 0 || flag_abi_version >= (2))
       && ((binfo)->common.static_flag))
     continue;
   ...
   if (!(flag_abi_version == 0 || flag_abi_version >= (2)))
   ...
   else
     {
       ...
       orig_binfo = (((((((type)->type.binfo)))->vec.a[4]))->vec.a[(i)]);
       binfo_offset = size_binop (PLUS_EXPR,
      offset,
      (((orig_binfo))->vec.a[1]));
     }
   ...
   r = walk_subobject_offsets (binfo,
          ...
          offsets,
          max_offset,
          ((flag_abi_version == 0 || flag_abi_version >= (2))
           ? 0 : vbases_p));
   if (r)
     return r;
 }
      ...
      if ((flag_abi_version == 0 || flag_abi_version >= (2)))
 {
   ...
   if (vbases_p)
     for (vbase = ((&((type)->type.lang_specific)->u.c)->vbases);
   vbase;
   vbase = ((vbase)->common.chain))
       {
  binfo = ((vbase)->list.value);
  r = walk_subobject_offsets (binfo,
         f,
  ...
  if (r)
    return r;
   ...
   else
     {
       ...
       if (vbase && ((vbase)->common.static_flag))
  {
    tree derived = type_binfo;
    while ((((derived))->vec.a[0]))
      derived = (((derived))->vec.a[0]);
    derived = ((derived)->common.type);
    vbase = binfo_for_vbase (((vbase)->common.type), derived);

    if (((vbase)->vec.a[10]) == type_binfo)
      {
 ...
 if (((enum tree_code) (field)->common.code) == FIELD_DECL && !((field)->decl.artificial_flag))
   {
     ...
     if ((flag_abi_version == 0 || flag_abi_version >= (2)))
       ...
       field_offset = ((field)->decl.arguments);

     r = walk_subobject_offsets (((field)->common.type),
     f,
     size_binop (PLUS_EXPR,
          offset,
          field_offset),
 ...
 return 0;
    ...
    ((flag_abi_version == 0 || flag_abi_version >= (2))
     ? !((((((domain)->type.maxval))->int_cst.int_cst).high) < (((index)->int_cst.int_cst).high) || ((((((domain)->type.maxval))->int_cst.int_cst).high) == (((index)->int_cst.int_cst).high) && (((((domain)->type.maxval))->int_cst.int_cst).low) < (((index)->int_cst.int_cst).low)))
     : ((((index)->int_cst.int_cst).high) < (((((domain)->type.maxval))->int_cst.int_cst).high) || ((((index)->int_cst.int_cst).high) == (((((domain)->type.maxval))->int_cst.int_cst).high) && (((index)->int_cst.int_cst).low) < (((((domain)->type.maxval))->int_cst.int_cst).low))));
    index = size_binop (PLUS_EXPR, index, global_trees[TI_SIZE_ONE]))
 {
   r = walk_subobject_offsets (((type)->common.type),
          f,
          offset,
          offsets,
          max_offset,
   ...
   offset = size_binop (PLUS_EXPR, offset,
          ((((type)->common.type))->type.size_unit));
...
# 3764 "../../../kg++fe/gnu/cp/class.c"
      if (!(flag_abi_version == 0 || flag_abi_version >= (2)) && binfo && ((binfo)->common.static_flag))
 break;
        ...
        offsets, field_p))
 {
   ...
   *rli = old_rli;
   ...
   rli->bitpos
     ...
     size_int_wide ((long long) (binfo ? ((((&((type)->type.lang_specific)->u.c)->as_base))->type.align) : ((type)->type.align)), BITSIZETYPE));
 ...
 break;
  ...
  if (binfo && (((((enum tree_code) (((binfo)->common.type))->common.code)) == RECORD_TYPE || (((enum tree_code) (((binfo)->common.type))->common.code)) == UNION_TYPE) && (((enum tree_code) (((binfo)->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((binfo)->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((binfo)->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((binfo)->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((binfo)->common.type))->type.lang_flag_5))))




    propagate_binfo_offsets (binfo,
        size_diffop (convert (sizetype_tab[(int) SSIZETYPE], offset),
...
empty_base_at_nonzero_offset_p (tree type,
    tree offset,
    splay_tree offsets __attribute__ ((__unused__)))
{
...
{
  tree alignment;
  tree basetype = ((binfo)->common.type);
  _Bool atend = 0;


  (void) (((is_empty_class (basetype)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 3829, __FUNCTION__), 0) : 0);
  alignment = size_int_wide ((long long) ((((((&((basetype)->type.lang_specific)->u.c)->as_base))->type.align) / 8)), SSIZETYPE);

  if ((flag_abi_version == 0 || flag_abi_version >= (2)))
    (((binfo))->vec.a[1]) = global_trees[TI_SIZE_ZERO];
  if (warn_abi && !integer_zerop ((((binfo))->vec.a[1])))
  ...
  if (layout_conflict_p (binfo,
                 ...
                 0))
    {
      ...
      atend = 1;
      propagate_binfo_offsets (binfo, convert (sizetype_tab[(int) SSIZETYPE], eoc), t);
      while (1)
 {
   if (!layout_conflict_p (binfo,
      (((binfo))->vec.a[1]),
  ...
  if (!is_empty_class (basetype))
    {
      ...
      decl = build_decl (FIELD_DECL, (tree) ((void *)0), ((&((basetype)->type.lang_specific)->u.c)->as_base));
      ((decl)->decl.artificial_flag) = 1;
      ((decl)->decl.context) = t;
      ((decl)->decl.size) = ((((&((basetype)->type.lang_specific)->u.c)->as_base))->type.size);
      ((decl)->decl.size_unit) = ((((&((basetype)->type.lang_specific)->u.c)->as_base))->type.size_unit);
      ((decl)->decl.u1.a.align) = ((((&((basetype)->type.lang_specific)->u.c)->as_base))->type.align);
      ((decl)->decl.user_align) = ((((&((basetype)->type.lang_specific)->u.c)->as_base))->type.user_align);
      ...
      ((decl)->common.chain) = *next_field;
      *next_field = decl;
      next_field = &((decl)->common.chain);
    }
  else
    {
      tree eoc;
      ...
      eoc = round_up (rli_size_unit_so_far (rli),
        (((((&((basetype)->type.lang_specific)->u.c)->as_base))->type.align) / 8));
      atend = layout_empty_base (binfo, eoc, offsets, t);


      if (!((binfo)->common.static_flag) && ((&((t)->type.lang_specific)->u.c)->nearly_empty_p))
 {
   if (atend)
     ((&((t)->type.lang_specific)->u.c)->nearly_empty_p) = 0;
     ...
     splay_tree offsets, tree *next_field)
{
  ...
  if ((((&((t)->type.lang_specific)->u.c)->primary_base) != (tree) ((void *)0)))
    next_field = build_base_field (rli, ((&((t)->type.lang_specific)->u.c)->primary_base),
       offsets, next_field);
  ...
  for (i = 0; i < n_baseclasses; ++i)
    {
      ...
      base_binfo = (((((((t)->type.binfo)))->vec.a[4]))->vec.a[(i)]);



      if (base_binfo == ((&((t)->type.lang_specific)->u.c)->primary_base))
 continue;
      ...
      if (((base_binfo)->common.static_flag)
   && !(((base_binfo)->vec.a[10]) != (tree) ((void *)0)))
 continue;
     ...
     tree t;
{
  ...
  for (x = ((t)->type.maxval); x; x = ((x)->common.chain))
    {

      if (((x)->decl.assembler_name != (tree) ((void *)0))
   && (((struct lang_identifier*)(decl_assembler_name (x)))->x ? ((struct lang_identifier*)(decl_assembler_name (x)))->x->error_locus : 0))
 continue;

      check_for_override (x, t);
      if ((((x)->decl.lang_specific)->decl_flags.pure_virtual) && ! ((x)->decl.vindex))
      ...
      if (((x)->decl.vindex))
 {
   ...
   if ((((x)->decl.lang_specific)->decl_flags.pure_virtual))
     ((&((t)->type.lang_specific)->u.c)->pure_virtuals)
       = tree_cons ((tree) ((void *)0), x, ((&((t)->type.lang_specific)->u.c)->pure_virtuals));
 }
    }
}
     ...
     tree name;
{
  ...
  (((clone)->decl.lang_specific)->u.f.cloned_function) = fn;
  ((clone)->decl.abstract_origin) = fn;

  ((clone)->decl.name) = name;
  ((clone)->decl.assembler_name = ((tree) ((void *)0)));

  (((clone)->decl.lang_specific)->u.f.u.pending_inline_info) = ((void *)0);
  (((clone)->decl.lang_specific)->u.f.pending_inline_p) = 0;

  (((clone)->decl.lang_specific)->decl_flags.deferred) = 0;


  if (name == cp_global_trees[CPTI_BASE_DTOR_IDENTIFIER])
    {
      ((clone)->decl.virtual_flag) = 0;
      if (((enum tree_code) (clone)->common.code) != TEMPLATE_DECL)
 ((clone)->decl.vindex) = (tree) ((void *)0);
    }



  if ((((clone)->decl.lang_specific)->decl_flags.has_in_charge_parm_p))
    {
      tree basetype;
      tree parmtypes;
      tree exceptions;

      exceptions = ((((clone)->common.type))->type.binfo);
      basetype = ((((clone)->common.type))->type.maxval);
      parmtypes = ((((clone)->common.type))->type.values);

      parmtypes = ((parmtypes)->common.chain);

      parmtypes = ((parmtypes)->common.chain);

      if ((((fn)->decl.lang_specific)->decl_flags.has_vtt_parm_p)
   && ! ((((((clone)->decl.context))->common.lang_flag_3)) && (((((clone)->decl.lang_specific)->decl_flags.constructor_attr) && ((clone)->decl.name) == cp_global_trees[CPTI_BASE_CTOR_IDENTIFIER]) || ((((clone)->decl.lang_specific)->decl_flags.destructor_attr) && ((clone)->decl.name) == cp_global_trees[CPTI_BASE_DTOR_IDENTIFIER]))))
 parmtypes = ((parmtypes)->common.chain);


      ((clone)->common.type)
 = build_cplus_method_type (basetype,
       ((((clone)->common.type))->common.type),
       parmtypes);
      if (exceptions)
 ((clone)->common.type) = build_exception_variant (((clone)->common.type),
           exceptions);
    }
  ...
  if (((enum tree_code) (clone)->common.code) != TEMPLATE_DECL)
    {
      ((clone)->decl.arguments) = copy_list (((clone)->decl.arguments));

      if ((((clone)->decl.lang_specific)->decl_flags.has_in_charge_parm_p))
 {
   ((((clone)->decl.arguments))->common.chain)
     = ((((((clone)->decl.arguments))->common.chain))->common.chain);
   (((clone)->decl.lang_specific)->decl_flags.has_in_charge_parm_p) = 0;
 }

      if ((((fn)->decl.lang_specific)->decl_flags.has_vtt_parm_p))
 {
   if (((((((clone)->decl.context))->common.lang_flag_3)) && (((((clone)->decl.lang_specific)->decl_flags.constructor_attr) && ((clone)->decl.name) == cp_global_trees[CPTI_BASE_CTOR_IDENTIFIER]) || ((((clone)->decl.lang_specific)->decl_flags.destructor_attr) && ((clone)->decl.name) == cp_global_trees[CPTI_BASE_DTOR_IDENTIFIER]))))
     (((clone)->decl.lang_specific)->decl_flags.has_vtt_parm_p) = 1;
   else
     {
       ((((clone)->decl.arguments))->common.chain)
  = ((((((clone)->decl.arguments))->common.chain))->common.chain);
       (((clone)->decl.lang_specific)->decl_flags.has_vtt_parm_p) = 0;
     }
 }

      for (parms = ((clone)->decl.arguments); parms; parms = ((parms)->common.chain))
 {
   ((parms)->decl.context) = clone;
   cxx_dup_lang_specific_decl (parms);
 }
    }


  ((clone)->decl.rtl = ((rtx) 0));
  rest_of_decl_compilation (clone, ((void *)0), 1, at_eof);


  ((clone)->common.chain) = ((fn)->common.chain);
  ((fn)->common.chain) = clone;
  ...
  if (((enum tree_code) (clone)->common.code) == TEMPLATE_DECL)
    {
      ...
      ((clone)->decl.result)
 = build_clone (((clone)->decl.result), name);
      result = ((clone)->decl.result);
      ((((result))->decl.lang_specific) ->decl_flags.u.template_info) = copy_node (((((result))->decl.lang_specific) ->decl_flags.u.template_info));
      (((((((result))->decl.lang_specific) ->decl_flags.u.template_info))->list.purpose)) = clone;
    }
  else if ((((fn)->decl.lang_specific)->decl_flags.deferred))
    defer_fn (clone);
     ...
     int update_method_vec_p;
{
  ...
  if (((fn)->common.chain)
      && (((((fn)->common.chain))->decl.lang_specific)->u.f.cloned_function))
    return;

  if (((((fn)->decl.lang_specific)->decl_flags.constructor_attr) && !((((enum tree_code) (fn)->common.code) == FUNCTION_DECL || ((enum tree_code) (fn)->common.code) == TEMPLATE_DECL) && ((fn)->decl.lang_specific) && (((fn)->decl.lang_specific)->u.f.cloned_function) != (tree) ((void *)0))))
    {


      clone = build_clone (fn, cp_global_trees[CPTI_COMPLETE_CTOR_IDENTIFIER]);
      if (update_method_vec_p)
 add_method (((clone)->decl.context), clone, 0);



      {
 tree type = ((((clone)->common.type))->type.maxval);
 if (!((&((type)->type.lang_specific)->u.c)->has_implicit_copy_constructor) &&
     ((((clone)->decl.lang_specific)->decl_flags.constructor_attr) && copy_fn_p (clone) > 0)) {
   if (!((((clone)->decl.lang_specific)->decl_flags.constructor_attr) && ((clone)->decl.name) == cp_global_trees[CPTI_COMPLETE_CTOR_IDENTIFIER]))
     fancy_abort ("../../../kg++fe/gnu/cp/class.c", 4200, __FUNCTION__);
   ((&((type)->type.lang_specific)->u.c)->copy_constructor) = clone;
 }
      }


      clone = build_clone (fn, cp_global_trees[CPTI_BASE_CTOR_IDENTIFIER]);
      if (update_method_vec_p)
 add_method (((clone)->decl.context), clone, 0);
    }
  else
    {
      (void) (((((((fn)->decl.lang_specific)->decl_flags.destructor_attr) && !((((enum tree_code) (fn)->common.code) == FUNCTION_DECL || ((enum tree_code) (fn)->common.code) == TEMPLATE_DECL) && ((fn)->decl.lang_specific) && (((fn)->decl.lang_specific)->u.f.cloned_function) != (tree) ((void *)0)))) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 4212, __FUNCTION__), 0) : 0);
# 4223 "../../../kg++fe/gnu/cp/class.c"
      if (((fn)->decl.virtual_flag))
 {
   clone = build_clone (fn, cp_global_trees[CPTI_DELETING_DTOR_IDENTIFIER]);
   if (update_method_vec_p)
     add_method (((clone)->decl.context), clone, 0);
 }
      clone = build_clone (fn, cp_global_trees[CPTI_COMPLETE_DTOR_IDENTIFIER]);
      if (update_method_vec_p)
 add_method (((clone)->decl.context), clone, 0);
      clone = build_clone (fn, cp_global_trees[CPTI_BASE_DTOR_IDENTIFIER]);
      if (update_method_vec_p)
 add_method (((clone)->decl.context), clone, 0);
    }


  ((fn)->decl.abstract_flag) = 1;
}
# 4248 "../../../kg++fe/gnu/cp/class.c"
void
     ...
     tree decl;
{
  ...
  for (clone = ((decl)->common.chain); clone && (((clone)->decl.lang_specific)->u.f.cloned_function);
       clone = ((clone)->common.chain))
    {
      tree orig_clone_parms = ((((clone)->common.type))->type.values);
      tree orig_decl_parms = ((((decl)->common.type))->type.values);
      tree decl_parms, clone_parms;

      clone_parms = orig_clone_parms;


      orig_clone_parms = ((orig_clone_parms)->common.chain);
      orig_decl_parms = ((orig_decl_parms)->common.chain);
 ...
 orig_decl_parms = ((orig_decl_parms)->common.chain);
      ...
      clone_parms = orig_clone_parms;
      if ((((clone)->decl.lang_specific)->decl_flags.has_vtt_parm_p))
 clone_parms = ((clone_parms)->common.chain);

      for (decl_parms = orig_decl_parms; decl_parms;
    decl_parms = ((decl_parms)->common.chain),
      clone_parms = ((clone_parms)->common.chain))
 {
   (void) (((comptypes ((((decl_parms)->common.type)), (((clone_parms)->common.type)), 0)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 4281, __FUNCTION__), 0) : 0);


   if (((decl_parms)->list.purpose) && !((clone_parms)->list.purpose))
     {


       tree exceptions = ((((clone)->common.type))->type.binfo);
       tree basetype = ((((clone)->common.type))->type.maxval);
       tree type;
       ...
       if ((((clone)->decl.lang_specific)->decl_flags.has_vtt_parm_p))
  {
    clone_parms = tree_cons (((orig_clone_parms)->list.purpose),
        ((orig_clone_parms)->list.value),
        clone_parms);
    ((clone_parms)->common.type) = ((orig_clone_parms)->common.type);
  }
  ...
  type = build_exception_variant (type, exceptions);
       ((clone)->common.type) = type;

       clone_parms = (tree) ((void *)0);
       break;
     }
 }
      (void) (((!clone_parms) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 4311, __FUNCTION__), 0) : 0);
    }
}
     ...
     tree t;
{
  ...
  for (fns = (((((&((t)->type.lang_specific)->u.c)->methods))->vec.a[0])); fns; fns = ((((enum tree_code) (fns)->common.code) == OVERLOAD) ? ((fns)->common.chain) : (tree) ((void *)0)))
    clone_function_decl (((((enum tree_code) (fns)->common.code) == OVERLOAD) ? (((struct tree_overload*)(fns))->function) : (fns)), 1);
  for (fns = (((((&((t)->type.lang_specific)->u.c)->methods))->vec.a[1])); fns; fns = ((((enum tree_code) (fns)->common.code) == OVERLOAD) ? ((fns)->common.chain) : (tree) ((void *)0)))
    clone_function_decl (((((enum tree_code) (fns)->common.code) == OVERLOAD) ? (((struct tree_overload*)(fns))->function) : (fns)), 1);
}
     ...
     tree t;
{
  ...
  fieldsp = &((t)->type.values);
  while (*fieldsp)
    {
      if (((enum tree_code) (*fieldsp)->common.code) == FIELD_DECL
   && ((((*fieldsp))->decl.lang_flag_4) == 1)
   && ((*fieldsp)->decl.initial))
 *fieldsp = ((*fieldsp)->common.chain);
      else
 fieldsp = &((*fieldsp)->common.chain);
    }
}
     ...
     tree type;
{
  ...
  fns = lookup_fnfields (((type)->type.binfo),
    (operator_name_info[(int) (VEC_DELETE_EXPR)].identifier),
                0);


  if (!fns || fns == global_trees[TI_ERROR_MARK])
    return 0;

  for (fns = ((((fns))->exp.operands[1])); fns; fns = ((((enum tree_code) (fns)->common.code) == OVERLOAD) ? ((fns)->common.chain) : (tree) ((void *)0)))
    {
      tree fn;
      tree second_parm;
      ...
      second_parm = ((((((fn)->common.type))->type.values))->common.chain);
      if (second_parm == global_trees[TI_VOID_LIST_NODE])
 return 0;



      if (((second_parm)->common.chain) == global_trees[TI_VOID_LIST_NODE]
   && comptypes ((((second_parm)->list.value)), (sizetype_tab[(int) SIZETYPE]), 0))
...
check_bases_and_members (tree t)
{
  ...
  if (!((((t)->common.lang_flag_2)) || (((t)->common.lang_flag_3))))
    ((&((t)->type.lang_specific)->u.c)->nearly_empty_p) = 0;



  ((&((t)->type.lang_specific)->u.c)->has_complex_init_ref)
    |= (((&((t)->type.lang_specific)->u.c)->h.has_init_ref)
 || (((t)->common.lang_flag_3))
 || (((t)->common.lang_flag_2)));
  ((t)->type.needs_constructing_flag)
    |= ((((t)->type.lang_flag_1))
 || (((t)->common.lang_flag_3))
 || (((t)->common.lang_flag_2)));
  ((&((t)->type.lang_specific)->u.c)->non_aggregate) |= ((((t)->type.lang_flag_1))
      || (((t)->common.lang_flag_2)));
  ((&((t)->type.lang_specific)->u.c)->non_pod_class)
    |= (((&((t)->type.lang_specific)->u.c)->non_aggregate) || (((t)->type.lang_flag_2))
 || ((&((t)->type.lang_specific)->u.c)->has_assign_ref));
  ((&((t)->type.lang_specific)->u.c)->has_real_assign_ref) |= ((&((t)->type.lang_specific)->u.c)->has_assign_ref);
  ((&((t)->type.lang_specific)->u.c)->has_complex_assign_ref)
    |= ((&((t)->type.lang_specific)->u.c)->has_assign_ref) || ((((t)->common.lang_flag_2)) || (((t)->common.lang_flag_3)));
  ...
  for (; access_decls; access_decls = ((access_decls)->common.chain))
    handle_using_decl (((access_decls)->list.value), t);
  ...
  ((t)->type.lang_specific)->u.c.vec_new_uses_cookie
    = type_requires_array_cookie (t);
}
# 4501 "../../../kg++fe/gnu/cp/class.c"
static tree
     ...
     tree *virtuals_p;
{
  ...
  for (fn = ((t)->type.maxval); fn; fn = ((fn)->common.chain))
    if (((fn)->decl.vindex) && !((((fn)->decl.lang_specific)->decl_flags.destructor_attr) && !((((enum tree_code) (fn)->common.code) == FUNCTION_DECL || ((enum tree_code) (fn)->common.code) == TEMPLATE_DECL) && ((fn)->decl.lang_specific) && (((fn)->decl.lang_specific)->u.f.cloned_function) != (tree) ((void *)0)))
 && ((enum tree_code) (((fn)->decl.vindex))->common.code) != INTEGER_CST)
      {
 tree new_virtual = make_node (TREE_LIST);

 (((new_virtual)->list.value)) = fn;
 (((new_virtual)->list.purpose)) = global_trees[TI_INTEGER_ZERO];

 ((new_virtual)->common.chain) = *virtuals_p;
 *virtuals_p = new_virtual;
  ...
  if (!((t)->type.minval) && (*virtuals_p || ((((t)->common.lang_flag_2)) || (((t)->common.lang_flag_3)))))
    {
# 4547 "../../../kg++fe/gnu/cp/class.c"
      tree field;

      field = build_decl (FIELD_DECL, get_vfield_name (t), cp_global_trees[CPTI_VTBL_PTR_TYPE]);
      ((field)->decl.assembler_name = ((__builtin_constant_p (".vf") ? get_identifier_with_length ((".vf"), (unsigned) strlen (".vf")) : (get_identifier) (".vf"))));
      ((field)->decl.virtual_flag) = 1;
      ((field)->decl.artificial_flag) = 1;
      ((field)->decl.context) = t;
      ((field)->decl.vindex) = t;
      ((field)->decl.u1.a.align) = ((cp_global_trees[CPTI_VTBL_PTR_TYPE])->type.align);
      ((field)->decl.user_align) = ((cp_global_trees[CPTI_VTBL_PTR_TYPE])->type.user_align);
      ...
      if ((((((((t)->type.binfo)))->vec.a[4]) ? (((((((t)->type.binfo)))->vec.a[4]))->vec.length) : 0)))
...
fixup_pending_inline (fn)
     tree fn;
{
  if ((((fn)->decl.lang_specific)->u.f.u.pending_inline_info))
    {
      tree args = ((fn)->decl.arguments);
      while (args)
 {
   ((args)->decl.context) = fn;
...
fixup_inline_methods (type)
     tree type;
{
  tree method = ((type)->type.maxval);

  if (method && ((enum tree_code) (method)->common.code) == TREE_VEC)
    {
      if (((method)->vec.a[1]))
 method = ((method)->vec.a[1]);
      else if (((method)->vec.a[0]))
 method = ((method)->vec.a[0]);
      else
 method = ((method)->vec.a[2]);
    }


  for (; method; method = ((method)->common.chain))
    fixup_pending_inline (method);


  for (method = ((&((type)->type.lang_specific)->u.c)->pure_virtuals);
       method;
       method = ((method)->common.chain))
    fixup_pending_inline (((method)->list.value));
  ((&((type)->type.lang_specific)->u.c)->pure_virtuals) = (tree) ((void *)0);
}
     ...
     tree t;
{
  ...
  (((binfo))->vec.a[1])
    = convert (sizetype_tab[(int) SIZETYPE],
        size_binop (PLUS_EXPR,
      convert (sizetype_tab[(int) SSIZETYPE], (((binfo))->vec.a[1])),
      offset));
  ...
  for (i = -1; i < ((((binfo))->vec.a[4]) ? (((((binfo))->vec.a[4]))->vec.length) : 0); ++i)
    {
      tree base_binfo;
 ...
 {
   base_binfo = (((((binfo))->vec.a[4]))->vec.a[(i)]);
   ...
   if (base_binfo == primary_binfo)
     continue;
      ...
      if (((base_binfo)->common.static_flag)
   && (((base_binfo)->vec.a[10]) != binfo
       || base_binfo != binfo_for_vbase (((base_binfo)->common.type), t)))
 continue;
  ...
  if (((binfo)->common.static_flag))
    {
      ...
      vbase = binfo_for_vbase (((binfo)->common.type), t);
      if (vbase != binfo)
 {
   offset = size_diffop ((((vbase))->vec.a[1]), (((binfo))->vec.a[1]));
   propagate_binfo_offsets (binfo, offset, t);
 }
    }
...
layout_virtual_bases (record_layout_info rli, splay_tree offsets)
{
  ...
  if ((((((((t)->type.binfo)))->vec.a[4]) ? (((((((t)->type.binfo)))->vec.a[4]))->vec.length) : 0)) == 0)
    return;
  ...
  if (!(flag_abi_version == 0 || flag_abi_version >= (2)))
    {
      ...
      finish_record_layout (rli, 0);


      if (! ((t)->type.packed_flag))
 ((t)->type.align) = ((((t)->type.align)) > (8) ? (((t)->type.align)) : (8));

      rli->offset = ((t)->type.size_unit);
      rli->bitpos = global_trees[TI_BITSIZE_ZERO];
      rli->record_align = ((t)->type.align);
    }



  next_field = &((t)->type.values);
  while (*next_field)
    next_field = &((*next_field)->common.chain);




  for (vbases = ((t)->type.binfo);
       vbases;
       vbases = ((vbases)->common.chain))
    {
      tree vbase;

      if (!((vbases)->common.static_flag))
 continue;

      vbase = binfo_for_vbase (((vbases)->common.type), t);

      if (!(((vbase)->vec.a[10]) != (tree) ((void *)0)))
 {
   tree basetype = ((vbase)->common.type);



   next_field = build_base_field (rli, vbase,
      offsets, next_field);
# 4776 "../../../kg++fe/gnu/cp/class.c"
   if (warn_abi
       && first_vbase
       && (tree_int_cst_lt
    (size_binop (CEIL_DIV_EXPR,
          round_up (((((&((t)->type.lang_specific)->u.c)->as_base))->type.size),
      ((((&((basetype)->type.lang_specific)->u.c)->as_base))->type.align)),
  ...
  dfs_walk (((t)->type.binfo), dfs_set_offset_for_unshared_vbases, ((void *)0), t);
}
...
end_of_base (tree binfo)
{
  ...
  if (is_empty_class (((binfo)->common.type)))



    size = ((((binfo)->common.type))->type.size_unit);
  else
    size = ((((&((((binfo)->common.type))->type.lang_specific)->u.c)->as_base))->type.size_unit);

  return size_binop (PLUS_EXPR, (((binfo))->vec.a[1]), size);
}
...
{
  tree result = global_trees[TI_SIZE_ZERO];
  tree binfo;
  tree offset;
  int i;
  ...
  for (i = 0; i < (((((((t)->type.binfo)))->vec.a[4]) ? (((((((t)->type.binfo)))->vec.a[4]))->vec.length) : 0)); ++i)
    {
      binfo = (((((((t)->type.binfo)))->vec.a[4]))->vec.a[(i)]);

      if (!include_virtuals_p
   && ((binfo)->common.static_flag)
   && ((binfo)->vec.a[10]) != ((t)->type.binfo))
 continue;

      offset = end_of_base (binfo);
      if ((((unsigned long long) (((result)->int_cst.int_cst).high) < (unsigned long long) (((offset)->int_cst.int_cst).high)) || (((unsigned long long) (((result)->int_cst.int_cst).high) == (unsigned long long) (((offset)->int_cst.int_cst).high)) && (((result)->int_cst.int_cst).low) < (((offset)->int_cst.int_cst).low))))
 result = offset;
    }


  if ((flag_abi_version == 0 || flag_abi_version >= (2)) && include_virtuals_p)
    for (binfo = ((&((t)->type.lang_specific)->u.c)->vbases);
  binfo;
  binfo = ((binfo)->common.chain))
      {
 offset = end_of_base (((binfo)->list.value));
 if ((((unsigned long long) (((result)->int_cst.int_cst).high) < (unsigned long long) (((offset)->int_cst.int_cst).high)) || (((unsigned long long) (((result)->int_cst.int_cst).high) == (unsigned long long) (((offset)->int_cst.int_cst).high)) && (((result)->int_cst.int_cst).low) < (((offset)->int_cst.int_cst).low))))
   result = offset;
      }
     ...
     tree t;
{
  ...
  for (i = 0; i < (((((((t)->type.binfo)))->vec.a[4]) ? (((((((t)->type.binfo)))->vec.a[4]))->vec.length) : 0)); ++i)
    {
      basetype = (((((((((t)->type.binfo)))->vec.a[4]))->vec.a[(i)]))->common.type);

      if (!lookup_base (t, basetype, ba_ignore | ba_quiet, ((void *)0)))
 warning ("direct base `%T' inaccessible in `%T' due to ambiguity",
  ...
  vbases = ((vbases)->common.chain))
      {
 basetype = ((((vbases)->list.value))->common.type);
...
include_empty_classes (record_layout_info rli)
{
  ...
  eoc = end_of_class (rli->t,
        ((&((rli->t)->type.lang_specific)->u.c)->as_base) != (tree) ((void *)0));
  rli_size = rli_size_unit_so_far (rli);
  if (((enum tree_code) (rli_size)->common.code) == INTEGER_CST
      && (((unsigned long long) (((rli_size)->int_cst.int_cst).high) < (unsigned long long) (((eoc)->int_cst.int_cst).high)) || (((unsigned long long) (((rli_size)->int_cst.int_cst).high) == (unsigned long long) (((eoc)->int_cst.int_cst).high)) && (((rli_size)->int_cst.int_cst).low) < (((eoc)->int_cst.int_cst).low))))
    {
      rli->bitpos = round_up (rli->bitpos, 8);
      rli->bitpos
 = size_binop (PLUS_EXPR,
        rli->bitpos,
      ...
      convert (sizetype_tab[(int) BITSIZETYPE],
        size_binop (MINUS_EXPR,
             eoc, rli_size)),
      size_int_wide ((long long) (8), BITSIZETYPE)));
      normalize_rli (rli);
    }
}
...
layout_class_type (tree t, tree *virtuals_p)
{
  ...
  _Bool last_field_was_bitfield = 0;
  ...
  non_static_data_members = ((t)->type.values);
  ...
  if (vptr)
    {
      ((vptr)->common.chain) = ((t)->type.values);
      ((t)->type.values) = vptr;
      next_field = &((vptr)->common.chain);
  ...
  for (field = non_static_data_members; field; field = ((field)->common.chain))
    {
      tree type;
      tree padding;
...
# 5016 "../../../kg++fe/gnu/cp/class.c"
   ...
   && ((((((type)->type.size))->int_cst.int_cst).high) < (((((field)->decl.size))->int_cst.int_cst).high) || ((((((type)->type.size))->int_cst.int_cst).high) == (((((field)->decl.size))->int_cst.int_cst).high) && (((((type)->type.size))->int_cst.int_cst).low) < (((((field)->decl.size))->int_cst.int_cst).low))))
 {
   ...
   for (itk = itk_char; itk != itk_none; ++itk)
     if (((((((field)->decl.size))->int_cst.int_cst).high) < (((((integer_types[itk])->type.size))->int_cst.int_cst).high) || ((((((field)->decl.size))->int_cst.int_cst).high) == (((((integer_types[itk])->type.size))->int_cst.int_cst).high) && (((((field)->decl.size))->int_cst.int_cst).low) < (((((integer_types[itk])->type.size))->int_cst.int_cst).low))))
   ...
   if ((flag_abi_version == 0 || flag_abi_version >= (2)) && ((enum tree_code) (t)->common.code) == UNION_TYPE)


     padding = ((field)->decl.size);
   else
     {
       if (warn_abi && ((enum tree_code) (t)->common.code) == UNION_TYPE)
  warning ("size assigned to `%T' may not be "
    "ABI-compliant and may change in a future "
    "version of GCC",
    t);
       padding = size_binop (MINUS_EXPR, ((field)->decl.size),
        ((integer_type)->type.size));
     }
   ((field)->decl.size) = ((integer_type)->type.size);
   ((field)->decl.u1.a.align) = ((integer_type)->type.align);
   ((field)->decl.user_align) = ((integer_type)->type.user_align);
   layout_nonempty_base_or_field (rli, field, (tree) ((void *)0),
      empty_base_offsets);



   ((field)->decl.size) = ((type)->type.size);

   if ((flag_abi_version == 0 || flag_abi_version >= (2)))
     ((field)->decl.mode) = ((type)->type.mode);
   else if (warn_abi
     && ((field)->decl.mode) != ((type)->type.mode))
      ...
      else
 {
   ...
   layout_nonempty_base_or_field (rli, field, (tree) ((void *)0),
      ...
      if ((flag_abi_version == 0 || flag_abi_version >= (2)))
 record_subobject_offsets (((field)->common.type),
      byte_position(field),
      empty_base_offsets,
                   1);
 ...
 cp_warning_at ("offset of `%D' is not ABI-compliant and may change in a future version of GCC",
      ...
      if (warn_abi
   && !tree_int_cst_equal (((field)->decl.arguments),
      byte_position (field))
   && contains_empty_class_p (((field)->common.type)))
      ...
      if (padding)
 {
   ...
   ((padding_field)->decl.bit_field_flag) = 1;
   ((padding_field)->decl.size) = padding;
   ((padding_field)->decl.u1.a.align) = 1;
   ((padding_field)->decl.user_align) = 0;
   layout_nonempty_base_or_field (rli, padding_field,
      (tree) ((void *)0),
      empty_base_offsets);
 }

      last_field_was_bitfield = ((((field))->decl.lang_flag_4) == 1);
    }

  if ((flag_abi_version == 0 || flag_abi_version >= (2)) && !integer_zerop (rli->bitpos))
    {
  ...
  if (((&((t)->type.lang_specific)->u.c)->non_pod_class) || ((&((t)->type.lang_specific)->u.c)->empty_p))
    {
      base_t = make_node (((enum tree_code) (t)->common.code));




      if (!(flag_abi_version == 0 || flag_abi_version >= (2)) && ((&((t)->type.lang_specific)->u.c)->empty_p))
 {
   ((base_t)->type.size) = global_trees[TI_BITSIZE_ZERO];
   ((base_t)->type.size_unit) = global_trees[TI_SIZE_ZERO];
 ...
 {
   tree eoc;
# 5185 "../../../kg++fe/gnu/cp/class.c"
   eoc = end_of_class (t, 0);
   ((base_t)->type.size_unit)
     = size_binop (MAX_EXPR,
     convert (sizetype_tab[(int) SIZETYPE],
       size_binop (CEIL_DIV_EXPR,
            rli_size_so_far (rli),
   ...
   ((base_t)->type.size)
     ...
     rli_size_so_far (rli),
     size_binop (MULT_EXPR,
          convert (sizetype_tab[(int) BITSIZETYPE], eoc),
          size_int_wide ((long long) (8), BITSIZETYPE)));
 }
      ((base_t)->type.align) = rli->record_align;
      ((base_t)->type.user_align) = ((t)->type.user_align);


      next_field = &((base_t)->type.values);
      for (field = ((t)->type.values); field; field = ((field)->common.chain))
 if (((enum tree_code) (field)->common.code) == FIELD_DECL)
   {
     *next_field = build_decl (FIELD_DECL,
          ((field)->decl.name),
          ((field)->common.type));
     ((*next_field)->decl.context) = base_t;
     ((*next_field)->decl.arguments) = ((field)->decl.arguments);
     ((*next_field)->decl.u2.t)
       = ((field)->decl.u2.t);

     ((*next_field)->decl.size) = ((field)->decl.size);
            ((*next_field)->decl.size_unit) = ((field)->decl.size_unit);

     next_field = &((*next_field)->common.chain);
   }


      ((&((t)->type.lang_specific)->u.c)->as_base) = base_t;
      ((base_t)->type.context) = t;
    }
  else
    ((&((t)->type.lang_specific)->u.c)->as_base) = t;


  if (((&((t)->type.lang_specific)->u.c)->empty_p))
    ((&((t)->type.lang_specific)->u.c)->contains_empty_class_p) = 1;




  layout_decl ((((((t)->type.main_variant))->common.chain)), 0);
  ...
  if (integer_zerop (rli_size_unit_so_far (rli)) && ((&((t)->type.lang_specific)->u.c)->empty_p))
    place_field (rli,
   build_decl (FIELD_DECL, (tree) ((void *)0), integer_types[itk_char]));
...
key_method (tree type)
{
  ...
  if (((type)->type.lang_flag_3)
      ...
      || (((&((type)->type.lang_specific)->u.c)->use_template) & 1)
      || ((&((type)->type.lang_specific)->u.c)->interface_unknown == 0))
    return (tree) ((void *)0);

  for (method = ((type)->type.maxval); method != (tree) ((void *)0);
       method = ((method)->common.chain))
    if (((method)->decl.vindex) != (tree) ((void *)0)
 && ! (((method)->decl.lang_specific)->decl_flags.base.declared_inline)
 && ! (((method)->decl.lang_specific)->decl_flags.pure_virtual))
      return method;
     ...
     tree t;
{
  ...
  tree virtuals = (tree) ((void *)0);
  int n_fields = 0;
  tree vfield;

  if ((((t)->type.size) != (tree) ((void *)0)))
    {
      if ((((enum tree_code) (t)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (t)->common.code) == TYPENAME_TYPE || ((enum tree_code) (t)->common.code) == TYPEOF_TYPE || ((enum tree_code) (t)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((t)->type.lang_flag_5)))
 error ("redefinition of `%#T'", t);
      else
 fancy_abort ("../../../kg++fe/gnu/cp/class.c", 5305, __FUNCTION__);
      popclass ();
      return;
    }
  ...
  ((t)->type.size) = (tree) ((void *)0);
  ((&((t)->type.lang_specific)->u.c)->got_semicolon) = 0;
  ((&((t)->type.lang_specific)->u.c)->primary_base) = (tree) ((void *)0);
  ...
  ((&((t)->type.lang_specific)->u.c)->empty_p) = 1;
  ((&((t)->type.lang_specific)->u.c)->nearly_empty_p) = 1;
  ((&((t)->type.lang_specific)->u.c)->contains_empty_class_p) = 0;
  ...
  check_bases_and_members (t);
    ...
    if (((((t)->common.lang_flag_2)) || (((t)->common.lang_flag_3))))
    {
      ((&((t)->type.lang_specific)->u.c)->key_method) = key_method (t);



      if (((&((t)->type.lang_specific)->u.c)->key_method) == (tree) ((void *)0))
 cp_global_trees[CPTI_KEYED_CLASSES] = tree_cons ((tree) ((void *)0), t, cp_global_trees[CPTI_KEYED_CLASSES]);
    }
  ...
  vfield = ((t)->type.minval);
  if (vfield && (((&((t)->type.lang_specific)->u.c)->primary_base) != (tree) ((void *)0)))
    {
      tree primary = ((&((t)->type.lang_specific)->u.c)->primary_base);

      (void) (((comptypes ((((vfield)->decl.context)), (((primary)->common.type)), 0)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 5350, __FUNCTION__), 0) : 0);



      (void) (((integer_zerop (((vfield)->decl.arguments))) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 5353, __FUNCTION__), 0) : 0);

      (void) (((integer_zerop ((((primary))->vec.a[1]))) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 5355, __FUNCTION__), 0) : 0);


      vfield = copy_decl (vfield);
      ((vfield)->decl.context) = t;
      ((t)->type.minval) = vfield;
    }
  else
    (void) (((!vfield || ((vfield)->decl.context) == t) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 5362, __FUNCTION__), 0) : 0);

  virtuals = modify_all_vtables (t, nreverse (virtuals));



  if (((t)->type.minval) && !(((&((t)->type.lang_specific)->u.c)->primary_base) != (tree) ((void *)0)))
    ((&((t)->type.lang_specific)->u.c)->vfields)
      = chainon (((&((t)->type.lang_specific)->u.c)->vfields), build_tree_list ((tree) ((void *)0), t));


  if (virtuals || ((((t)->common.lang_flag_2)) || (((t)->common.lang_flag_3))))
    {

      if (!(((&((t)->type.lang_specific)->u.c)->primary_base) != (tree) ((void *)0)))
 build_primary_vtable ((tree) ((void *)0), t);
      else if (! ((((((((t)->type.binfo))->common.static_flag) ? binfo_for_vbase (((((t)->type.binfo))->common.type), t) : (((t)->type.binfo))))->common.lang_flag_4)))


 build_primary_vtable (((&((t)->type.lang_specific)->u.c)->primary_base), t);
    }
  ...
  if (((((t)->common.lang_flag_2)) || (((t)->common.lang_flag_3))))
    {
      ...
      if ((((((t)->type.binfo)))->vec.a[2]))
 (void) ((((((((((t)->type.binfo)))->vec.a[2]))->decl.virtual_flag)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 5391, __FUNCTION__), 0) : 0);

      if (!(((&((t)->type.lang_specific)->u.c)->primary_base) != (tree) ((void *)0)))
 (void) ((((((((t)->type.binfo)))->vec.a[3]) == (tree) ((void *)0)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 5394, __FUNCTION__), 0) : 0);



      (((((t)->type.binfo)))->vec.a[3]) = chainon ((((((t)->type.binfo)))->vec.a[3]), virtuals);


      for (vindex = 0, fn = (((((t)->type.binfo)))->vec.a[3]);
    fn;
    fn = ((fn)->common.chain),
      vindex += (0
   ? 0 : 1))
 if (((enum tree_code) ((((((fn)->list.value)))->decl.vindex))->common.code) != INTEGER_CST)
   (((((fn)->list.value)))->decl.vindex) = build_shared_int_cst (vindex);
    }
  ...
  for (x = ((t)->type.values); x; x = ((x)->common.chain))
    if (((enum tree_code) (x)->common.code) == VAR_DECL && ((x)->common.static_flag)
 && comptypes ((((((x)->common.type))->type.main_variant)), (t), 0))
      ((x)->decl.mode) = ((t)->type.mode);
# 5425 "../../../kg++fe/gnu/cp/class.c"
  n_fields = count_fields (((t)->type.values));
  if (n_fields > 7)
    {
      tree field_vec = make_tree_vec (n_fields);
      add_fields_to_vec (((t)->type.values), field_vec, 0);
      qsort (&((field_vec)->vec.a[0]), n_fields, sizeof (tree),
      (int (*)(const void *, const void *))field_decl_cmp);
      if (! (((((((t)->type.main_variant))->common.chain)))->decl.lang_specific))
 retrofit_lang_decl ((((((t)->type.main_variant))->common.chain)));
      (((((((((t)->type.main_variant))->common.chain))))->decl.lang_specific)->u.f.u.sorted_fields) = field_vec;
    }

  if ((((t)->type.lang_flag_1)))
    {
      tree vfields = ((&((t)->type.lang_specific)->u.c)->vfields);

      for (vfields = ((&((t)->type.lang_specific)->u.c)->vfields);
    vfields; vfields = ((vfields)->common.chain))



 if (((vfields)->list.purpose))
   ((vfields)->common.addressable_flag) = 1;
    }
  ...
  if (warn_nonvdtor && (((t)->common.lang_flag_2)) && (((t)->type.lang_flag_2))
      && ((((((&((t)->type.lang_specific)->u.c)->methods))->vec.a[1]))->decl.vindex) == (tree) ((void *)0))
  ...
  rest_of_type_compilation (t, ! (decl_function_context ((((((t)->type.main_variant))->common.chain))) != (tree) ((void *)0)));
}
     ...
     tree t;
{
  ...
  ((t)->type.maxval) = nreverse (((t)->type.maxval));
  ((&((t)->type.lang_specific)->u.c)->decl_list) = nreverse (((&((t)->type.lang_specific)->u.c)->decl_list));



  prev = (tree) ((void *)0);
  for (x = ((t)->type.values);
       x && ((enum tree_code) (x)->common.code) != TYPE_DECL;
       x = next)
    {
      next = ((x)->common.chain);
      ((x)->common.chain) = prev;
     ...
     tree t, attributes;
{
  ...
  input_filename = (((((t)->type.name))->decl.locus).file);
  lineno = (((((t)->type.name))->decl.locus).line);

  if (scope_chain->x_processing_template_decl)
    {
      finish_struct_methods (t);
      ((t)->type.size) = global_trees[TI_BITSIZE_ZERO];
    }
  else
    finish_struct_1 (t);
  ...
  ((&((t)->type.lang_specific)->u.c)->being_defined) = 0;
  ...
  if (scope_chain->x_processing_template_decl && at_function_scope_p ())
    add_stmt (build_min (TAG_DEFN, t));

  return t;
}
# 5559 "../../../kg++fe/gnu/cp/class.c"
static tree
fixed_type_or_null (instance, nonnull, cdtorp)
     ...
     int *cdtorp;
{
  switch (((enum tree_code) (instance)->common.code))
    {
    case INDIRECT_REF:
      if ((((enum tree_code) (((instance)->common.type))->common.code) == POINTER_TYPE || ((enum tree_code) (((instance)->common.type))->common.code) == REFERENCE_TYPE))
 return (tree) ((void *)0);
      else
 return fixed_type_or_null (((instance)->exp.operands[0]),
       nonnull, cdtorp);

    case CALL_EXPR:

      if ((((instance)->common.lang_flag_4)))
 {
   if (nonnull)
     *nonnull = 1;
   return ((instance)->common.type);
 }
      return (tree) ((void *)0);
      ...
      if ((((instance)->common.lang_flag_4)))
 {
   ...
   return ((instance)->common.type);
      ...
      return fixed_type_or_null (((instance)->exp.operands[0]), nonnull, cdtorp);
    ...
    case PLUS_EXPR:
    case MINUS_EXPR:
      if (((enum tree_code) (((instance)->exp.operands[0]))->common.code) == ADDR_EXPR)
 return fixed_type_or_null (((instance)->exp.operands[0]), nonnull, cdtorp);
      if (((enum tree_code) (((instance)->exp.operands[1]))->common.code) == INTEGER_CST)
 ...
 *nonnull = 1;
    ...
    case COMPONENT_REF:
      return fixed_type_or_null (((instance)->exp.operands[1]), nonnull, cdtorp);

    case VAR_DECL:
    case FIELD_DECL:
      if (((enum tree_code) (((instance)->common.type))->common.code) == ARRAY_TYPE
   && (((enum tree_code) (((((instance)->common.type))->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((((instance)->common.type))->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((((instance)->common.type))->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((((instance)->common.type))->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((((instance)->common.type))->common.type))->type.lang_flag_5)))
 {
   if (nonnull)
     *nonnull = 1;
   return ((((instance)->common.type))->common.type);
 }

    case TARGET_EXPR:
    case PARM_DECL:
    case RESULT_DECL:
      if ((((enum tree_code) (((instance)->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((instance)->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((instance)->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((instance)->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((instance)->common.type))->type.lang_flag_5)))
 {
   if (nonnull)
     *nonnull = 1;
   return ((instance)->common.type);
 }
      else if (instance == (cfun ? (cfun->language)->x_current_class_ptr : (tree) ((void *)0)))
        {
          if (nonnull)
            *nonnull = 1;


          if (((current_function_decl)->decl.lang_specific)
              && ((((current_function_decl)->decl.lang_specific)->decl_flags.constructor_attr)
                  || (((current_function_decl)->decl.lang_specific)->decl_flags.destructor_attr)))
            {
              if (cdtorp)
                *cdtorp = 1;
              return ((((instance)->common.type))->common.type);
      ...
      else if (((enum tree_code) (((instance)->common.type))->common.code) == REFERENCE_TYPE)
        {
   ...
   if (((enum tree_code) (instance)->common.code) == VAR_DECL
       && ((instance)->decl.initial)
       && !((((instance))->decl.lang_flag_4)))
     {
       tree type;
       ((((instance))->decl.lang_flag_4)) = 1;
       type = fixed_type_or_null (((instance)->decl.initial),
      nonnull, cdtorp);
       ((((instance))->decl.lang_flag_4)) = 0;
       return type;
     }
 }
      return (tree) ((void *)0);
...
# 5694 "../../../kg++fe/gnu/cp/class.c"
int
resolves_to_fixed_type_p (instance, nonnull)
     tree instance;
     int *nonnull;
{
  tree t = ((instance)->common.type);
  int cdtorp = 0;

  tree fixed = fixed_type_or_null (instance, nonnull, &cdtorp);
  if (fixed == (tree) ((void *)0))
    return 0;
  if ((((enum tree_code) (t)->common.code) == POINTER_TYPE || ((enum tree_code) (t)->common.code) == REFERENCE_TYPE))
    t = ((t)->common.type);
  if (!comptypes ((((t)->type.main_variant)), (((fixed)->type.main_variant)), 0))
    return 0;
  return cdtorp ? -1 : 1;
}
...
init_class_processing ()
{
  ...
  local_classes = varray_init (8, VARRAY_DATA_TREE, "local_classes");

  cp_global_trees[CPTI_ACCESS_DEFAULT] = build_int_2_wide ((unsigned long long) (0), (long long) (0));
  cp_global_trees[CPTI_ACCESS_PUBLIC] = build_int_2_wide ((unsigned long long) (ak_public), (long long) (0));
  cp_global_trees[CPTI_ACCESS_PROTECTED] = build_int_2_wide ((unsigned long long) (ak_protected), (long long) (0));
  cp_global_trees[CPTI_ACCESS_PRIVATE] = build_int_2_wide ((unsigned long long) (ak_private), (long long) (0));
  cp_global_trees[CPTI_ACCESS_DEFAULT_VIRTUAL] = build_int_2_wide ((unsigned long long) (4), (long long) (0));
  cp_global_trees[CPTI_ACCESS_PUBLIC_VIRTUAL] = build_int_2_wide ((unsigned long long) (4 | ak_public), (long long) (0));
  cp_global_trees[CPTI_ACCESS_PROTECTED_VIRTUAL] = build_int_2_wide ((unsigned long long) (4 | ak_protected), (long long) (0));
  cp_global_trees[CPTI_ACCESS_PRIVATE_VIRTUAL] = build_int_2_wide ((unsigned long long) (4 | ak_private), (long long) (0));

  ridpointers[(int) RID_PUBLIC] = cp_global_trees[CPTI_ACCESS_PUBLIC];
  ridpointers[(int) RID_PRIVATE] = cp_global_trees[CPTI_ACCESS_PRIVATE];
  ridpointers[(int) RID_PROTECTED] = cp_global_trees[CPTI_ACCESS_PROTECTED];
}
# 5771 "../../../kg++fe/gnu/cp/class.c"
void
     ...
     int modify;
{
 ...
 = (class_stack_node_t) xrealloc (current_class_stack,
  ...
  current_class_stack[current_class_depth].name = scope_chain->class_name;
  current_class_stack[current_class_depth].type = scope_chain->class_type;
  current_class_stack[current_class_depth].access = scope_chain->access_specifier;
  current_class_stack[current_class_depth].names_used = 0;
  current_class_depth++;


  scope_chain->class_name = ((type)->type.name);
  if (((enum tree_code) (scope_chain->class_name)->common.code) == TYPE_DECL)
    scope_chain->class_name = ((scope_chain->class_name)->decl.name);
  scope_chain->class_type = type;



  scope_chain->access_specifier = (((&((type)->type.lang_specific)->u.c)->declared_class)
         ? cp_global_trees[CPTI_ACCESS_PRIVATE]
         : cp_global_trees[CPTI_ACCESS_PUBLIC]);

  if (scope_chain->x_previous_class_type != (tree) ((void *)0)
      && (type != scope_chain->x_previous_class_type
   || !(((scope_chain->x_previous_class_type)->type.size) != (tree) ((void *)0)))
  ...
  if (modify)
    {
      if (type != scope_chain->x_previous_class_type || current_class_depth > 1)
      ...
      else
 {
   ...
   set_class_shadows (scope_chain->x_previous_class_values);
   for (item = scope_chain->x_previous_class_values; item; item = ((item)->common.chain))
     {
       tree id = ((item)->list.purpose);
       tree decl = ((item)->common.type);

       push_class_binding (id, decl);
       if (((enum tree_code) (decl)->common.code) == TYPE_DECL)
  set_identifier_type_value (id, ((decl)->common.type));
     }
   unuse_fields (type);
 }

      cxx_remember_type_decls (((&((type)->type.lang_specific)->u.c)->nested_udts));
    }
}
...
invalidate_class_lookup_cache ()
{
  ...
  for (t = scope_chain->x_previous_class_values; t; t = ((t)->common.chain))
    (((struct lang_identifier*)(((t)->list.purpose)))->class_value) = (tree) ((void *)0);

  scope_chain->x_previous_class_values = (tree) ((void *)0);
  scope_chain->x_previous_class_type = (tree) ((void *)0);
}
...
popclass ()
{
  ...
  current_class_depth--;
  scope_chain->class_name = current_class_stack[current_class_depth].name;
  scope_chain->class_type = current_class_stack[current_class_depth].type;
  scope_chain->access_specifier = current_class_stack[current_class_depth].access;
     ...
     tree t;
{
  ...
  if ((lookup_base ((scope_chain->class_type), t, ba_any, ((void *)0)) != (tree) ((void *)0)))
    return scope_chain->class_type;

  for (i = current_class_depth - 1; i > 0; --i)
    if ((lookup_base ((current_class_stack[i].type), t, ba_any, ((void *)0)) != (tree) ((void *)0)))
      return current_class_stack[i].type;

  return (tree) ((void *)0);
}
# 5932 "../../../kg++fe/gnu/cp/class.c"
void
     ...
     int modify;
{
  ...
  context = (((((((type)->type.main_variant))->common.chain)))->decl.context);

  if (context && (((((enum tree_code) (context)->common.code)) == RECORD_TYPE || (((enum tree_code) (context)->common.code)) == UNION_TYPE) && (((enum tree_code) (context)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (context)->common.code) == TYPENAME_TYPE || ((enum tree_code) (context)->common.code) == TYPEOF_TYPE || ((enum tree_code) (context)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((context)->type.lang_flag_5))))
...
pop_nested_class ()
{
  tree context = (((((((scope_chain->class_type)->type.main_variant))->common.chain)))->decl.context);
...
current_lang_depth ()
{
  return ((scope_chain->lang_base)->elements_used);
}
...
push_lang_context (name)
     tree name;
{
  do { if ((scope_chain->lang_base)->elements_used >= (scope_chain->lang_base)->num_elements) (((scope_chain->lang_base)) = varray_grow ((scope_chain->lang_base), 2 * (scope_chain->lang_base)->num_elements)); (scope_chain->lang_base)->data.tree[(scope_chain->lang_base)->elements_used++] = (scope_chain->lang_name); } while (0);

  if (name == cp_global_trees[CPTI_LANG_NAME_CPLUSPLUS])
    {
      scope_chain->lang_name = name;
    }
  else if (name == cp_global_trees[CPTI_LANG_NAME_JAVA])
    {
      scope_chain->lang_name = name;
      ...
      ((((cp_global_trees[CPTI_JAVA_BYTE_TYPE])->type.name))->decl.ignored_flag) = 0;
      ((((cp_global_trees[CPTI_JAVA_SHORT_TYPE])->type.name))->decl.ignored_flag) = 0;
      ((((cp_global_trees[CPTI_JAVA_INT_TYPE])->type.name))->decl.ignored_flag) = 0;
      ((((cp_global_trees[CPTI_JAVA_LONG_TYPE])->type.name))->decl.ignored_flag) = 0;
      ((((cp_global_trees[CPTI_JAVA_FLOAT_TYPE])->type.name))->decl.ignored_flag) = 0;
      ((((cp_global_trees[CPTI_JAVA_DOUBLE_TYPE])->type.name))->decl.ignored_flag) = 0;
      ((((cp_global_trees[CPTI_JAVA_CHAR_TYPE])->type.name))->decl.ignored_flag) = 0;
      ((((cp_global_trees[CPTI_JAVA_BOOLEAN_TYPE])->type.name))->decl.ignored_flag) = 0;
    }
  else if (name == cp_global_trees[CPTI_LANG_NAME_C])
    {
      scope_chain->lang_name = name;
    }
  else
    error ("language string `\"%s\"' not recognized", ((char *)(name)->identifier.id.str));
}
...
pop_lang_context ()
{
  scope_chain->lang_name = ((scope_chain->lang_base)->data.tree[(scope_chain->lang_base)->elements_used - 1]);
  ((scope_chain->lang_base)->elements_used--);
}
# 6031 "../../../kg++fe/gnu/cp/class.c"
static tree
resolve_address_of_overloaded_function (target_type,
     ...
     tree explicit_targs;
{
# 6071 "../../../kg++fe/gnu/cp/class.c"
  int is_ptrmem = 0;
  int is_reference = 0;



  tree matches = (tree) ((void *)0);
  tree fn;




  (void) (((!(((enum tree_code) (target_type)->common.code) == POINTER_TYPE && (((enum tree_code) (((target_type)->common.type))->common.code) == METHOD_TYPE))) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 6084, __FUNCTION__), 0) : 0);



  if (((enum tree_code) (overload)->common.code) == COMPONENT_REF)
    overload = ((overload)->exp.operands[1]);


  if ((((enum tree_code) (target_type)->common.code) == POINTER_TYPE && ((enum tree_code) (((target_type)->common.type))->common.code) == FUNCTION_TYPE))
                      ;
  else if ((((enum tree_code) (target_type)->common.code) == RECORD_TYPE && ((target_type)->type.lang_specific) && ((&((target_type)->type.lang_specific)->u.c)->ptrmemfunc_flag)))

    is_ptrmem = 1;
  else if (((enum tree_code) (target_type)->common.code) == FUNCTION_TYPE)
    {
    ...
    {
      if (complain)
 error ("cannot resolve overloaded function `%D' based on conversion to type `%T'",

    (((((struct tree_overload*)(overload))->function))->decl.name), target_type);
      return global_trees[TI_ERROR_MARK];
    }
  ...
  if (!template_only)
    {
      ...
      for (fns = overload; fns; fns = ((fns)->common.chain))
 {
   tree fn = (((struct tree_overload*)(fns))->function);
   tree fntype;
   ...
   if (can_convert_arg (target_type, fntype, fn))
     matches = tree_cons (fn, (tree) ((void *)0), matches);
 }
    }
  ...
  if (!matches)
    {
      ...
      if (is_ptrmem)
 target_fn_type
   = (((((((target_type)->type.values))->common.type)))->common.type);
      else
 target_fn_type = ((target_type)->common.type);
      target_arg_types = ((target_fn_type)->type.values);
      target_ret_type = ((target_fn_type)->common.type);


      if (((enum tree_code) (target_fn_type)->common.code) == METHOD_TYPE)
 target_arg_types = ((target_arg_types)->common.chain);
      ...
      for (fns = overload; fns; fns = ((fns)->common.chain))
 {
   ...
   targs = make_tree_vec (((((((fn)->decl.arguments))->list.value))->vec.length));
   ...
   if (instantiation == global_trees[TI_ERROR_MARK])

     continue;


   instantiation_type = ((instantiation)->common.type);
   if (is_ptrmem)
     instantiation_type =
      ...
      if (matches)
 {
   ...
   if (match != global_trees[TI_ERROR_MARK])
     matches = tree_cons (match, (tree) ((void *)0), (tree) ((void *)0));
 }
    }


  if (matches == (tree) ((void *)0))
    {

      if (complain)
 {
    error ("no matches converting function `%D' to type `%#T'",
      (((((struct tree_overload*)(overload))->function))->decl.name),
      target_type);




          for (; overload; overload = ((((enum tree_code) (overload)->common.code) == OVERLOAD) ? ((overload)->common.chain) : (tree) ((void *)0)))
            matches = tree_cons ((tree) ((void *)0), ((((enum tree_code) (overload)->common.code) == OVERLOAD) ? (((struct tree_overload*)(overload))->function) : (overload)),
     matches);
      ...
      if (complain)
 {
   ...
   for (match = matches; match; match = ((match)->common.chain))
     ((match)->list.value) = ((match)->list.purpose);
  ...
  if ((((enum tree_code) (target_type)->common.code) == POINTER_TYPE && ((enum tree_code) (((target_type)->common.type))->common.code) == FUNCTION_TYPE) || (((enum tree_code) (target_type)->common.code) == RECORD_TYPE && ((target_type)->type.lang_specific) && ((&((target_type)->type.lang_specific)->u.c)->ptrmemfunc_flag)))
     ...
     tsubst_flags_t flags;
{
  ...
  if (((enum tree_code) (lhstype)->common.code) == LANG_TYPE)
    {
      if (complain)
 error ("not enough type information");
  ...
  if (((rhs)->common.type) != (tree) ((void *)0) && ! (type_unknown_p (rhs)))
    {
      if (comptypes (lhstype, ((rhs)->common.type), strict))
 return rhs;
      if (complain)
 error ("argument of type `%T' does not match `%T'",
  ...
  if (((enum tree_code) (rhs)->common.code) == BASELINK)
    rhs = ((((rhs))->exp.operands[1]));



  if (((enum tree_code) (rhs)->common.code) != OVERLOAD)
    rhs = copy_node (rhs);
  ...
  switch (((enum tree_code) (rhs)->common.code))
    {
    case TYPE_EXPR:
    case CONVERT_EXPR:
    case SAVE_EXPR:
    case CONSTRUCTOR:
    case BUFFER_REF:
      fancy_abort ("../../../kg++fe/gnu/cp/class.c", 6356, __FUNCTION__);
      return global_trees[TI_ERROR_MARK];
 ...
 new_rhs = instantiate_type (build_pointer_type (lhstype),
        ((rhs)->exp.operands[0]), flags);
 if (new_rhs == global_trees[TI_ERROR_MARK])
   return global_trees[TI_ERROR_MARK];

 ((rhs)->common.type) = lhstype;
 ((rhs)->exp.operands[0]) = new_rhs;
 return rhs;
      }

    case NOP_EXPR:
      rhs = copy_node (((rhs)->exp.operands[0]));
      ((rhs)->common.type) = cp_global_trees[CPTI_UNKNOWN_TYPE];
      return instantiate_type (lhstype, rhs, flags);

    case COMPONENT_REF:
      return instantiate_type (lhstype, ((rhs)->exp.operands[1]), flags);

    case OFFSET_REF:
      rhs = ((rhs)->exp.operands[1]);
      if ((((enum tree_code) (rhs)->common.code) == BASELINK))
 return instantiate_type (lhstype, ((((rhs))->exp.operands[1])),
                          flags | allow_ptrmem);



      (void) (((((enum tree_code) (rhs)->common.code) == TEMPLATE_ID_EXPR) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 6390, __FUNCTION__), 0) : 0);



    case TEMPLATE_ID_EXPR:
      {
 tree fns = ((rhs)->exp.operands[0]);
 tree args = ((rhs)->exp.operands[1]);
 ...
 resolve_address_of_overloaded_function (lhstype,
    ...
    case TREE_LIST:
      ...
      (void) ((((((enum tree_code) (rhs)->common.code) == BASELINK)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 6419, __FUNCTION__), 0) : 0);

      return instantiate_type (lhstype, ((((rhs))->exp.operands[1])), flags);

    case CALL_EXPR:

      fancy_abort ("../../../kg++fe/gnu/cp/class.c", 6425, __FUNCTION__);
      return global_trees[TI_ERROR_MARK];
    ...
    case COMPOUND_EXPR:
      ((rhs)->exp.operands[0])
 = instantiate_type (lhstype, ((rhs)->exp.operands[0]), flags);
      if (((rhs)->exp.operands[0]) == global_trees[TI_ERROR_MARK])
 return global_trees[TI_ERROR_MARK];
      ((rhs)->exp.operands[1])
 = instantiate_type (lhstype, ((rhs)->exp.operands[1]), flags);
      if (((rhs)->exp.operands[1]) == global_trees[TI_ERROR_MARK])
 return global_trees[TI_ERROR_MARK];
 ...
 error ("not enough type information");
    ...
    case COND_EXPR:
      if (type_unknown_p (((rhs)->exp.operands[0])))
 {
   if (complain)
     error ("not enough type information");
   return global_trees[TI_ERROR_MARK];
 ...
 return global_trees[TI_ERROR_MARK];
      ((rhs)->exp.operands[2])
 = instantiate_type (lhstype, ((rhs)->exp.operands[2]), flags);
      if (((rhs)->exp.operands[2]) == global_trees[TI_ERROR_MARK])
 return global_trees[TI_ERROR_MARK];
 ...
 return global_trees[TI_ERROR_MARK];
    ...
    case ADDR_EXPR:
    {
      if (((rhs)->common.lang_flag_0))
        flags |= tf_ptrmem_ok;

      return instantiate_type (lhstype, ((rhs)->exp.operands[0]), flags);
    }
    case ENTRY_VALUE_EXPR:
      fancy_abort ("../../../kg++fe/gnu/cp/class.c", 6532, __FUNCTION__);
      return global_trees[TI_ERROR_MARK];
    ...
    default:
      fancy_abort ("../../../kg++fe/gnu/cp/class.c", 6539, __FUNCTION__);
      return global_trees[TI_ERROR_MARK];
    }
}
     ...
     tree type;
{
  ...
  while ((((binfo))->vec.a[4])
  && (((((((((((binfo))->vec.a[4]))->vec.a[(0)]))->common.type))->common.lang_flag_2)) || ((((((((((binfo))->vec.a[4]))->vec.a[(0)]))->common.type))->common.lang_flag_3)))
  && ! (((((((binfo))->vec.a[4]))->vec.a[(0)]))->common.static_flag))
    binfo = (((((binfo))->vec.a[4]))->vec.a[(0)]);

  type = ((binfo)->common.type);
  buf = (char *) __builtin_alloca(sizeof ("_vptr.%s") + ((((((((type)->type.name))->decl.name)))->identifier.id.len)) + 2);

  sprintf (buf, "_vptr.%s",
    ((char *)(constructor_name (type))->identifier.id.str));
...
{
  tree name = constructor_name (scope_chain->class_type);
  tree value = build_lang_decl (TYPE_DECL, name, scope_chain->class_type);
  tree saved_cas;

  ((value)->decl.nonlocal_flag) = 1;
  ((value)->decl.context) = scope_chain->class_type;
  ((value)->decl.artificial_flag) = 1;
  (((value)->decl.lang_flag_4) = 1);

  if (scope_chain->x_processing_template_decl)
    value = push_template_decl (value);

  saved_cas = scope_chain->access_specifier;
  scope_chain->access_specifier = cp_global_trees[CPTI_ACCESS_PUBLIC];
  finish_member_declaration (value);
  scope_chain->access_specifier = saved_cas;
}
     ...
     tree type;
{
  ...
  if ((flag_abi_version == 0 || flag_abi_version >= (2)))
    return ((&((type)->type.lang_specific)->u.c)->empty_p);
  else
    return integer_zerop (((((&((type)->type.lang_specific)->u.c)->as_base))->type.size));
}
  ...
  if ((((((enum tree_code) (type)->common.code)) == RECORD_TYPE || (((enum tree_code) (type)->common.code)) == UNION_TYPE) && (((enum tree_code) (type)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (type)->common.code) == TYPENAME_TYPE || ((enum tree_code) (type)->common.code) == TYPEOF_TYPE || ((enum tree_code) (type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((type)->type.lang_flag_5))))
    {
      ...
      for (i = 0; i < (((((((type)->type.binfo)))->vec.a[4]) ? (((((((type)->type.binfo)))->vec.a[4]))->vec.length) : 0)); ++i)
 if (contains_empty_class_p ((((((((((type)->type.binfo)))->vec.a[4]))->vec.a[(i)]))->common.type)))
   return 1;
      for (field = ((type)->type.values); field; field = ((field)->common.chain))
 if (((enum tree_code) (field)->common.code) == FIELD_DECL
     && !((field)->decl.artificial_flag)
     && is_empty_class (((field)->common.type)))
   return 1;
    }
  else if (((enum tree_code) (type)->common.code) == ARRAY_TYPE)
    return contains_empty_class_p (((type)->common.type));
  return 0;
}
     ...
     tree type;
{
  ...
  while (node && ((enum tree_code) (node)->common.code) != NAMESPACE_DECL)
    {
      switch (tree_code_type[(int) (((enum tree_code) (node)->common.code))])
 {
 case 'd':
   node = ((node)->decl.context);
   break;

 case 't':
   if (node != type)
     return node;
   node = ((node)->type.context);
   break;

 default:
   fancy_abort ("../../../kg++fe/gnu/cp/class.c", 6685, __FUNCTION__);
 }
    }
  return (tree) ((void *)0);
}
     ...
     tree base, type;
{
  while (type)
    {
      if (lookup_base (type, base, ba_any, ((void *)0)))
 return 1;
     ...
     tree decl;
{
  ...
  if ((((struct lang_identifier*)(name))->class_value))
    return;
  ...
  if (n)
    {
     ...
     ((((((enum tree_code) (decl)->common.code) == OVERLOAD) ? (((struct tree_overload*)(decl))->function) : (decl)))->decl.name),
     (tree) n->value);
    }
}
     ...
     tree binfo;
{
  ...
  decl = (((binfo))->vec.a[2]);
  if (decl && ((enum tree_code) (decl)->common.code) == PLUS_EXPR)
    {
      (void) (((((enum tree_code) (((decl)->exp.operands[0]))->common.code) == ADDR_EXPR) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 6782, __FUNCTION__), 0) : 0);

      decl = ((((decl)->exp.operands[0]))->exp.operands[0]);
    }
  if (decl)
    (void) (((((enum tree_code) (decl)->common.code) == VAR_DECL) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 6786, __FUNCTION__), 0) : 0);
  return decl;
}
...
{
  tree cons = (tree) data;
  tree primary_base = ((cons)->list.purpose);

  if (((binfo)->common.static_flag)
      && comptypes ((((binfo)->common.type)), (primary_base), 0))



    ((cons)->list.value) = tree_cons (((void *)0), binfo, ((cons)->list.value));

  return (tree) ((void *)0);
}
# 6819 "../../../kg++fe/gnu/cp/class.c"
tree
     ...
     tree binfo;
{
  ...
  primary_base = ((&((((binfo)->common.type))->type.lang_specific)->u.c)->primary_base);
  if (!primary_base)
    return (tree) ((void *)0);



  if (!((primary_base)->common.static_flag))
    {
      int i;



      for (i = 0; i < ((((binfo))->vec.a[4]) ? (((((binfo))->vec.a[4]))->vec.length) : 0); ++i)
 {
   tree base_binfo = (((((binfo))->vec.a[4]))->vec.a[(i)]);

   if (comptypes ((((base_binfo)->common.type)), (((primary_base)->common.type)), 0))

     return base_binfo;
 }


      fancy_abort ("../../../kg++fe/gnu/cp/class.c", 6849, __FUNCTION__);
    }
  ...
  virtuals = build_tree_list (((primary_base)->common.type), (tree) ((void *)0));
  dfs_walk (binfo, dfs_get_primary_binfo, ((void *)0), virtuals);
  virtuals = ((virtuals)->list.value);


  (void) (((virtuals) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 6863, __FUNCTION__), 0) : 0);

  if (((virtuals)->common.chain))
    {
    ...
    (((complete_binfo))->vec.a[0]);
    complete_binfo = (((complete_binfo))->vec.a[0]))
 continue;
      canonical = binfo_for_vbase (((primary_base)->common.type),
       ((complete_binfo)->common.type));

      for (; virtuals; virtuals = ((virtuals)->common.chain))
 {
   result = ((virtuals)->list.value);

   if (canonical == result)
     {


       (void) (((!((virtuals)->common.chain)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 6889, __FUNCTION__), 0) : 0);
       break;
     }
 }
    }
     ...
     int indent;
{
  ...
  fprintf (stream, "%s (0x%lx) ",
    type_as_string (binfo, (0)),
    (unsigned long) binfo);
  fprintf (stream, "%lld",
    tree_low_cst ((((binfo))->vec.a[1]), 0));
  if (is_empty_class (((binfo)->common.type)))
    fprintf (stream, " empty");
  else if (((&((((binfo)->common.type))->type.lang_specific)->u.c)->nearly_empty_p))
    fprintf (stream, " nearly-empty");
  if (((binfo)->common.static_flag))
    {
      tree canonical = binfo_for_vbase (((binfo)->common.type), t);
  ...
  if (((binfo)->vec.a[10]))
    {
      ...
      fprintf (stream, " primary-for %s (0x%lx)",
        type_as_string (((binfo)->vec.a[10]),
          (0)),
        (unsigned long)((binfo)->vec.a[10]));
    }
  if (((binfo)->common.lang_flag_2))
    {
      indented = maybe_indent_hierarchy (stream, indent + 3, indented);
     ...
     tree t;
{
  ...
  fprintf (stream, "   size=%lu align=%lu\n",
    (unsigned long)(tree_low_cst (((t)->type.size), 0) / 8),
    (unsigned long)(((t)->type.align) / 8));
  dump_class_hierarchy_r (stream, flags, t, ((t)->type.binfo), 0);
  fprintf (stream, "\n");
     ...
     tree decl;
{
  ...
  int ix;
  long long elt;
  tree size = ((((((decl)->common.type))->type.values))->type.maxval);

  elt = (tree_low_cst (((((((decl)->common.type))->common.type))->type.size), 0)
  / 8);
  fprintf (stream, "%s:", decl_as_string (decl, (0)));
  fprintf (stream, " %s entries",
    expr_as_string (size_binop (PLUS_EXPR, size, global_trees[TI_SIZE_ONE]),
      (0)));
  fprintf (stream, "\n");

  for (ix = 0, inits = ((((decl)->decl.initial))->exp.operands[1]);
       inits; ix++, inits = ((inits)->common.chain))
    fprintf (stream, "%-4ld  %s\n", (long)(ix * elt),
      expr_as_string (((inits)->list.value), (0)));
}
     ...
     tree vtable;
{
  ...
  if (!(flags & (1 << 1)))
    {
      int ctor_vtbl_p = ((t)->type.binfo) != binfo;
      ...
      if (ctor_vtbl_p)
 {
   ...
   fprintf (stream, " in %s", type_as_string (t, (0)));
 }
      fprintf (stream, "\n");
     ...
     tree t;
{
  ...
  list = build_tree_list ((((((t)->type.binfo)))->vec.a[2]), (tree) ((void *)0));
  accumulate_vtbl_inits (((t)->type.binfo), ((t)->type.binfo),
    ((t)->type.binfo), t, list);


  for (vbase = ((t)->type.binfo); vbase; vbase = ((vbase)->common.chain))
    {
      tree real_base;

      if (!((vbase)->common.static_flag))
 continue;



      real_base = binfo_for_vbase (((vbase)->common.type), t);
  ...
  for (i = 0; i < ((((((t)->type.binfo)))->vec.a[4]) ? (((((((t)->type.binfo)))->vec.a[4]))->vec.length) : 0); ++i)
    {
      tree base = (((((((t)->type.binfo)))->vec.a[4]))->vec.a[(i)]);
      if (((base)->common.static_flag))
 {
   vbase = binfo_for_vbase (((base)->common.type), t);
   ((base)->vec.a[5]) = ((vbase)->vec.a[5]);
 }
    }

  if ((((((t)->type.binfo)))->vec.a[2]))
    initialize_vtable (((t)->type.binfo), ((list)->list.value));
}
     ...
     tree inits;
{
  ...
  dump_vtable (((binfo)->common.type), binfo, decl);
}
  ...
  tree inits;
{
  ...
  ((decl)->decl.initial) = build_nt (CONSTRUCTOR, (tree) ((void *)0), inits);
  (((((decl)->decl.initial))->common.lang_flag_4)) = 1;
  cp_finish_decl (decl, ((decl)->decl.initial), (tree) ((void *)0), 0);
  ((decl)->decl.context) = context;
}
# 7207 "../../../kg++fe/gnu/cp/class.c"
static void
build_vtt (t)
     tree t;
{
  ...
  inits = (tree) ((void *)0);
  index = global_trees[TI_SIZE_ZERO];
  build_vtt_inits (((t)->type.binfo), t, &inits, &index);
  ...
  type = build_index_type (size_int_wide ((long long) (list_length (inits) - 1), SIZETYPE));
  type = build_cplus_array_type (global_trees[TI_CONST_PTR_TYPE], type);


  vtt = build_vtable (t, get_vtt_name (t), type);
  initialize_array (vtt, inits);

  ((vtt)->common.chain) = ((((&((t)->type.lang_specific)->u.c)->vtables))->common.chain);
  ((((&((t)->type.lang_specific)->u.c)->vtables))->common.chain) = vtt;
     ...
     tree binfo;
{
  ...
  if (comptypes ((((base_binfo)->common.type)), (((binfo)->common.type)), 0))
    return binfo;
  if (((base_binfo)->common.static_flag))
    return binfo_for_vbase (((base_binfo)->common.type), ((binfo)->common.type));
  derived = get_original_base ((((base_binfo))->vec.a[0]), binfo);

  for (ix = 0; ix != ((((derived))->vec.a[4]) ? (((((derived))->vec.a[4]))->vec.length) : 0); ix++)
    if (comptypes ((((base_binfo)->common.type)), ((((((((derived))->vec.a[4]))->vec.a[(ix)]))->common.type)), 0))

      return (((((derived))->vec.a[4]))->vec.a[(ix)]);
  fancy_abort ("../../../kg++fe/gnu/cp/class.c", 7262, __FUNCTION__);
  return ((void *)0);
}
     ...
     tree binfo;
{
  ...
  while (1)
    {
      vt = (((binfo))->vec.a[2]);
      if (((enum tree_code) (vt)->common.code) == TREE_LIST)
 vt = ((vt)->list.value);
      if (((enum tree_code) (vt)->common.code) == TREE_VEC)
 binfo = vt;
      else
 break;
    }
     ...
     tree *index;
{
  ...
  int top_level_p = comptypes ((((binfo)->common.type)), (t), 0);


  if (!(((((binfo)->common.type))->common.lang_flag_3)))
    return inits;
  ...
  *inits = build_tree_list ((tree) ((void *)0), init);
  inits = &((*inits)->common.chain);
  if (top_level_p)
    {
      (void) (((!((binfo)->vec.a[9])) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 7332, __FUNCTION__), 0) : 0);
      ((binfo)->vec.a[9]) = *index;
    }
  *index = size_binop (PLUS_EXPR, *index, ((global_trees[TI_PTR_TYPE])->type.size_unit));


  for (i = 0; i < ((((binfo))->vec.a[4]) ? (((((binfo))->vec.a[4]))->vec.length) : 0); ++i)
    {
      b = (((((binfo))->vec.a[4]))->vec.a[(i)]);
      if (!((b)->common.static_flag))
 inits = build_vtt_inits ((((((binfo))->vec.a[4]))->vec.a[(i)]), t,
     inits, index);
    }
  ...
  secondary_vptrs = tree_cons (t, (tree) ((void *)0), ((binfo)->common.type));
  ((secondary_vptrs)->common.type) = *index;
  ((secondary_vptrs)->common.unsigned_flag) = top_level_p;
  ((secondary_vptrs)->common.used_flag) = 0;
  ...
  dfs_walk_real (binfo,
   ...
   secondary_vptrs);
  ((secondary_vptrs)->common.used_flag) = 1;
  dfs_walk (binfo, dfs_unmark, dfs_ctor_vtable_bases_queue_p,
            secondary_vptrs);

  *index = ((secondary_vptrs)->common.type);




  secondary_vptrs = ((secondary_vptrs)->list.value);
  if (secondary_vptrs)
    {
      *inits = nreverse (secondary_vptrs);
      inits = &((secondary_vptrs)->common.chain);
      (void) (((*inits == (tree) ((void *)0)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 7373, __FUNCTION__), 0) : 0);
    }


  if (top_level_p)
    for (b = ((((binfo)->common.type))->type.binfo); b; b = ((b)->common.chain))
      {
 tree vbase;
 ...
 vbase = binfo_for_vbase (((b)->common.type), t);
 inits = build_vtt_inits (vbase, t, inits, index);
      }

  if (!top_level_p)
    {
      tree data = tree_cons (t, binfo, (tree) ((void *)0));
      ((data)->common.unsigned_flag) = 0;
      ((data)->common.used_flag) = 0;
     ...
     void *data;
{
  ...
  l = (tree) data;
  t = ((l)->common.chain);
  top_level_p = ((l)->common.unsigned_flag);

  (((binfo)->common.static_flag) ? ((((((enum tree_code) (((binfo)->common.type))->common.code)) == RECORD_TYPE || (((enum tree_code) (((binfo)->common.type))->common.code)) == UNION_TYPE) && (((enum tree_code) (((binfo)->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((binfo)->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((binfo)->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((binfo)->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((binfo)->common.type))->type.lang_flag_5))) ? (void) ((&((((binfo)->common.type))->type.lang_specific)->u.c)->marks |= (1 << (0))) : (void) (((((binfo)->common.type))->type.alias_set) |= (1 << (0)))) : (void)(((binfo)->common.lang_flag_0) = 1));


  if (!((((binfo)->common.type))->type.minval))
    return (tree) ((void *)0);


  if (comptypes ((((binfo)->common.type)), (t), 0))
    return (tree) ((void *)0);


  if (!((binfo)->common.static_flag) && (((binfo)->vec.a[10]) != (tree) ((void *)0)))
    return (tree) ((void *)0);
  ...
  if (top_level_p)
    {
      (void) (((!((binfo)->vec.a[9])) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 7449, __FUNCTION__), 0) : 0);
      ((binfo)->vec.a[9]) = index;
    }
  ((l)->common.type) = size_binop (PLUS_EXPR, index,
         ((global_trees[TI_PTR_TYPE])->type.size_unit));


  if (top_level_p && ((binfo)->common.static_flag))
    {



      while (((binfo)->vec.a[10]))
        binfo = ((binfo)->vec.a[10]);
    }
  init = binfo_ctor_vtable (binfo);
  ((l)->list.value) = tree_cons ((tree) ((void *)0), init, ((l)->list.value));
     ...
     void *data;
{
  if (((binfo)->common.static_flag))
    ...
    binfo = binfo_for_vbase (((binfo)->common.type), (((tree) data)->list.purpose));

  if (!(((binfo)->common.static_flag) ? ((((((((enum tree_code) (((binfo)->common.type))->common.code)) == RECORD_TYPE || (((enum tree_code) (((binfo)->common.type))->common.code)) == UNION_TYPE) && (((enum tree_code) (((binfo)->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((binfo)->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((binfo)->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((binfo)->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((binfo)->common.type))->type.lang_flag_5))) ? (&((((binfo)->common.type))->type.lang_specific)->u.c)->marks : ((unsigned) ((((binfo)->common.type))->type.alias_set))) & (1 << (0))) != 0) : ((binfo)->common.lang_flag_0)) == (((tree) data)->common.used_flag))
    return (tree) ((void *)0);
  return binfo;
}
...
dfs_fixup_binfo_vtbls (binfo, data)
     ...
     void *data;
{
  (((binfo)->common.static_flag) ? ((((((enum tree_code) (((binfo)->common.type))->common.code)) == RECORD_TYPE || (((enum tree_code) (((binfo)->common.type))->common.code)) == UNION_TYPE) && (((enum tree_code) (((binfo)->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((binfo)->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((binfo)->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((binfo)->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((binfo)->common.type))->type.lang_flag_5))) ? (void) ((&((((binfo)->common.type))->type.lang_specific)->u.c)->marks &= ~(1 << (0))) : (void) (((((binfo)->common.type))->type.alias_set) &= ~(1 << (0)))) : (void)(((binfo)->common.lang_flag_0) = 0));
  ...
  if ((((binfo))->vec.a[2])
      && ((enum tree_code) ((((binfo))->vec.a[2]))->common.code) == TREE_LIST
      && ((((((binfo))->vec.a[2]))->list.purpose)
   == (((tree) data)->list.value)))
    (((binfo))->vec.a[2]) = (((((binfo))->vec.a[2]))->common.chain);
     ...
     tree t;
{
  ...
  if (namespace_binding ((id), global_namespace))
    return;

  (void) (((!comptypes ((((binfo)->common.type)), (t), 0)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 7536, __FUNCTION__), 0) : 0);



  vtbl = build_vtable (t, id, global_trees[TI_PTR_TYPE]);
  list = build_tree_list (vtbl, (tree) ((void *)0));
  accumulate_vtbl_inits (binfo, ((((binfo)->common.type))->type.binfo),
    binfo, t, list);



  for (vbase = ((((binfo)->common.type))->type.binfo);
       vbase;
       vbase = ((vbase)->common.chain))
    {
      tree b;
      ...
      if (!((vbase)->common.static_flag))
 continue;
      b = binfo_for_vbase (((vbase)->common.type), t);
      orig_base = binfo_for_vbase (((vbase)->common.type), ((binfo)->common.type));

      accumulate_vtbl_inits (b, orig_base, binfo, t, list);
    }
  inits = ((list)->list.value);


  type = build_index_type (size_int_wide ((long long) (list_length (inits) - 1), SIZETYPE));
  type = build_cplus_array_type (cp_global_trees[CPTI_VTABLE_ENTRY_TYPE], type);
  ((vtbl)->common.type) = type;


  ((&((t)->type.lang_specific)->u.c)->vtables) = chainon (((&((t)->type.lang_specific)->u.c)->vtables), vtbl);
  initialize_array (vtbl, inits);
  dump_vtable (t, binfo, vtbl);
}
# 7584 "../../../kg++fe/gnu/cp/class.c"
static void
...
{
  int i;
  int ctor_vtbl_p = !comptypes ((((rtti_binfo)->common.type)), (t), 0);

  (void) (((comptypes ((((binfo)->common.type)), (((orig_binfo)->common.type)), 0)) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 7597, __FUNCTION__), 0) : 0);




  if (!((((((binfo)->common.type))->common.lang_flag_2)) || (((((binfo)->common.type))->common.lang_flag_3))))
    return;



  if (ctor_vtbl_p
      && !(((((binfo)->common.type))->common.lang_flag_3))
      && !binfo_via_virtual (orig_binfo, ((rtti_binfo)->common.type)))
    return;


  ((inits)->list.value)
    = chainon (((inits)->list.value),
     ...
     tree l;
{
  ...
  if (ctor_vtbl_p
      && ((orig_binfo)->common.static_flag) && (((orig_binfo)->vec.a[10]) != (tree) ((void *)0)))
    {
# 7668 "../../../kg++fe/gnu/cp/class.c"
      tree b = ((binfo)->vec.a[10]);
 ...
 {
   last = b;
   if (((b)->common.static_flag) || b == rtti_binfo)
     break;
 }


      if (b == (tree) ((void *)0))
 for (b = last; b; b = (((b))->vec.a[0]))
   if (((b)->common.static_flag) || b == rtti_binfo)
     break;
      ...
      if (b == rtti_binfo
   || (b && binfo_for_vbase (((b)->common.type),
        ((rtti_binfo)->common.type))))
 ...
 vtbl = last;
  ...
  else if (!((((((orig_binfo)->common.static_flag) ? binfo_for_vbase (((orig_binfo)->common.type), ((rtti_binfo)->common.type)) : (orig_binfo)))->common.lang_flag_4)))
    return inits;
  ...
  if (!vtbl)
    {
      ...
      vtbl = build1 (ADDR_EXPR,
       cp_global_trees[CPTI_VTBL_PTR_TYPE],
       vtbl);
      ((vtbl)->common.constant_flag) = 1;
      index = size_binop (PLUS_EXPR,
     size_int_wide ((long long) (non_fn_entries), SIZETYPE),
     size_int_wide ((long long) (list_length (((l)->list.value))), SIZETYPE));
      index = size_binop (MULT_EXPR,
     ((cp_global_trees[CPTI_VTABLE_ENTRY_TYPE])->type.size_unit),
     index);
      vtbl = build (PLUS_EXPR, ((vtbl)->common.type), vtbl, index);
      ((vtbl)->common.constant_flag) = 1;
    }
  ...
  if (ctor_vtbl_p)
    ...
    (((binfo))->vec.a[2]) = tree_cons (rtti_binfo, vtbl, (((binfo))->vec.a[2]));
  else if ((((binfo)->vec.a[10]) != (tree) ((void *)0)) && ((binfo)->common.static_flag))
    inits = (tree) ((void *)0);
  else
     ...
     int *non_fn_entries_p;
{
  ...
  vid.primary_vtbl_p = (binfo == ((t)->type.binfo));
  vid.ctor_vtbl_p = !comptypes ((((rtti_binfo)->common.type)), (t), 0);
  vid.generate_vcall_entries = 1;

  vid.index = size_int_wide ((long long) (-3 * 1), SSIZETYPE);
  ...
  vid.fns = varray_init (32, VARRAY_DATA_TREE, "fns");

  build_vcall_and_vbase_vtbl_entries (binfo, &vid);


  for (vbase = ((&((t)->type.lang_specific)->u.c)->vbases);
       vbase;
       vbase = ((vbase)->common.chain))
    (((((vbase)->list.value))->common.static_flag) ? ((((((enum tree_code) (((((vbase)->list.value))->common.type))->common.code)) == RECORD_TYPE || (((enum tree_code) (((((vbase)->list.value))->common.type))->common.code)) == UNION_TYPE) && (((enum tree_code) (((((vbase)->list.value))->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((((vbase)->list.value))->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((((vbase)->list.value))->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((((vbase)->list.value))->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((((vbase)->list.value))->common.type))->type.lang_flag_5))) ? (void) ((&((((((vbase)->list.value))->common.type))->type.lang_specific)->u.c)->marks &= ~(1 << (2))) : (void) (((((((vbase)->list.value))->common.type))->type.alias_set) &= ~(1 << (2)))) : (((((vbase)->list.value))->common.lang_flag_3) = 0));
  ...
  if (1 > 1)
    {
      ...
      for (prev = &vid.inits; (cur = *prev); prev = &((cur)->common.chain))
 {
   tree add = cur;
   int i;

   for (i = 1; i < 1; ++i)
     add = tree_cons ((tree) ((void *)0),
        build1 (NOP_EXPR, cp_global_trees[CPTI_VTABLE_ENTRY_TYPE],
         global_trees[TI_NULL_POINTER]),
        add);
   *prev = add;
 }
    }
  ...
  vfun_inits = (tree) ((void *)0);
  for (v = (((orig_binfo))->vec.a[3]); v; v = ((v)->common.chain))
    {
      tree delta;
      tree vcall_index;
      tree fn;
...
# 7850 "../../../kg++fe/gnu/cp/class.c"
      for (b = binfo; ; b = get_primary_binfo (b))
 {
   ...
   if (((b)->common.lang_flag_2))
     {
       init = global_trees[TI_SIZE_ZERO];
       break;
     }
 }
      ...
      if (! init)
 {
   ...
   delta = (((v)->list.purpose));
   vcall_index = (((v)->common.type));

   (void) (((((enum tree_code) (delta)->common.code) == INTEGER_CST) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 7872, __FUNCTION__), 0) : 0);
   (void) (((((enum tree_code) (fn)->common.code) == FUNCTION_DECL) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 7873, __FUNCTION__), 0) : 0);



   if ((((fn)->decl.lang_specific)->decl_flags.pure_virtual))
     fn = cp_global_trees[CPTI_ABORT_FNDECL];
   else if (!integer_zerop (delta) || vcall_index)
     fn = make_thunk (fn, delta, vcall_index);


   init = build1 (ADDR_EXPR, cp_global_trees[CPTI_VTABLE_ENTRY_TYPE], fn);
 ...
 {
   int i;
   if (init == global_trees[TI_SIZE_ZERO])
     for (i = 0; i < 0; ++i)
       vfun_inits = tree_cons ((tree) ((void *)0), init, vfun_inits);
   else
     for (i = 0; i < 0; ++i)
       {
  tree fdesc = build (FDESC_EXPR, cp_global_trees[CPTI_VTABLE_ENTRY_TYPE],
        ((init)->exp.operands[0]),
        build_int_2_wide ((unsigned long long) (i), (long long) (0)));
  ((fdesc)->common.constant_flag) = 1;

  vfun_inits = tree_cons ((tree) ((void *)0), fdesc, vfun_inits);
       }
 }
      else
        vfun_inits = tree_cons ((tree) ((void *)0), init, vfun_inits);
     ...
     vtbl_init_data *vid;
{
  ...
  while ((((non_primary_binfo))->vec.a[0]))
    {
      tree b;
      ...
      if (((non_primary_binfo)->common.static_flag))
 {
   non_primary_binfo = vid->binfo;
   break;
 }

      b = (((non_primary_binfo))->vec.a[0]);
 ...
 continue;
      ...
      if ((((b)->common.static_flag) ? ((((((((enum tree_code) (((b)->common.type))->common.code)) == RECORD_TYPE || (((enum tree_code) (((b)->common.type))->common.code)) == UNION_TYPE) && (((enum tree_code) (((b)->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((b)->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((b)->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((b)->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((b)->common.type))->type.lang_flag_5))) ? (&((((b)->common.type))->type.lang_specific)->u.c)->marks : ((unsigned) ((((b)->common.type))->type.alias_set))) & (1 << (2))) != 0) : ((b)->common.lang_flag_3)))
 continue;
      (((b)->common.static_flag) ? ((((((enum tree_code) (((b)->common.type))->common.code)) == RECORD_TYPE || (((enum tree_code) (((b)->common.type))->common.code)) == UNION_TYPE) && (((enum tree_code) (((b)->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((b)->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((b)->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((b)->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((b)->common.type))->type.lang_flag_5))) ? (void) ((&((((b)->common.type))->type.lang_specific)->u.c)->marks |= (1 << (2))) : (void) (((((b)->common.type))->type.alias_set) |= (1 << (2)))) : (((b)->common.lang_flag_3) = 1));
      ...
      if (binfo != ((t)->type.binfo))
 {
   ...
   orig_vbase = binfo_for_vbase (((vbase)->common.type),
     ((binfo)->common.type));


   if (!tree_int_cst_equal (delta,
       ((orig_vbase)->vec.a[5])))
     fancy_abort ("../../../kg++fe/gnu/cp/class.c", 8028, __FUNCTION__);
 }
      ...
      delta = size_diffop ((((b))->vec.a[1]), (((non_primary_binfo))->vec.a[1]));
 ...
 = build_tree_list ((tree) ((void *)0),
      ...
      delta)));
      vid->last_init = &((*vid->last_init)->common.chain);
    }
}
     ...
     vtbl_init_data *vid;
{
  ...
  if (((binfo)->common.static_flag) || binfo == ((vid->derived)->type.binfo))
    {
# 8082 "../../../kg++fe/gnu/cp/class.c"
      vid->vbase = binfo;
     ...
     vtbl_init_data *vid;
{
  ...
  if (((binfo)->common.static_flag) && vid->vbase != binfo)
    return;
     ...
     vtbl_init_data* vid;
{
  ...
  if ((flag_abi_version == 0 || flag_abi_version >= (2)))
    {
      ...
      for (orig_fn = ((((binfo)->common.type))->type.maxval);
    orig_fn;
    orig_fn = ((orig_fn)->common.chain))
 if (((orig_fn)->decl.vindex))
      ...
      while ((((non_primary_binfo))->vec.a[0]))
 {
  ...
  fancy_abort ("../../../kg++fe/gnu/cp/class.c", 8174, __FUNCTION__);
       non_primary_binfo = vid->binfo;
       break;
     }
 ...
 non_primary_binfo = get_original_base
          (non_primary_binfo, ((((vid->rtti_binfo)->common.type))->type.binfo));

      for (base_virtuals = (((binfo))->vec.a[3]),
      derived_virtuals = (((non_primary_binfo))->vec.a[3]),
      orig_virtuals = (((((((binfo)->common.type))->type.binfo)))->vec.a[3]);
    base_virtuals;
    base_virtuals = ((base_virtuals)->common.chain),
      derived_virtuals = ((derived_virtuals)->common.chain),
      orig_virtuals = ((orig_virtuals)->common.chain))
 {
   tree orig_fn;



   orig_fn = (((orig_virtuals)->list.value));




   if (!comptypes ((((orig_fn)->decl.context)), (((binfo)->common.type)), 0))
     continue;
...
add_vcall_offset (tree orig_fn, tree binfo, vtbl_init_data *vid)
{
  ...
  for (i = 0; i < ((vid->fns)->elements_used); ++i)
    {
      tree derived_entry;

      derived_entry = ((vid->fns)->data.tree[i]);
      if (same_signature_p (derived_entry, orig_fn)


   || ((((derived_entry)->decl.lang_specific)->decl_flags.destructor_attr)
       && (((orig_fn)->decl.lang_specific)->decl_flags.destructor_attr)))
 return;
    }
  ...
  if (vid->binfo == ((vid->derived)->type.binfo))
    ((&((vid->derived)->type.lang_specific)->u.c)->vcall_indices)
  ...
  do { if ((vid->fns)->elements_used >= (vid->fns)->num_elements) (((vid->fns)) = varray_grow ((vid->fns), 2 * (vid->fns)->num_elements)); (vid->fns)->data.tree[(vid->fns)->elements_used++] = (orig_fn); } while (0);
 ...
 vcall_offset = build1 (NOP_EXPR, cp_global_trees[CPTI_VTABLE_ENTRY_TYPE],
      ...
      else
 {
   ...
   vcall_offset = size_diffop ((((base))->vec.a[1]),
          (((vid->binfo))->vec.a[1]));
   vcall_offset = fold (build1 (NOP_EXPR, cp_global_trees[CPTI_VTABLE_ENTRY_TYPE],
           vcall_offset));
 }

      *vid->last_init = build_tree_list ((tree) ((void *)0), vcall_offset);
      vid->last_init = &((*vid->last_init)->common.chain);
    }
}
     ...
     vtbl_init_data *vid;
{
  ...
  basetype = ((binfo)->common.type);
  t = ((vid->rtti_binfo)->common.type);



  b = binfo;
  while ((((&((((b)->common.type))->type.lang_specific)->u.c)->primary_base) != (tree) ((void *)0))
         && !((b)->common.lang_flag_2))
    {
      tree primary_base;

      primary_base = get_primary_binfo (b);
      (void) (((((primary_base)->vec.a[10]) == b) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/class.c", 8314, __FUNCTION__), 0) : 0);
      b = primary_base;
    }
  offset = size_diffop ((((vid->rtti_binfo))->vec.a[1]), (((b))->vec.a[1]));


  if (flag_rtti)
    decl = build_address (get_tinfo_decl (t));
  else
    decl = global_trees[TI_INTEGER_ZERO];



  init = build_nop (cp_global_trees[CPTI_VTABLE_ENTRY_TYPE], decl);
  *vid->last_init = build_tree_list ((tree) ((void *)0), init);
  vid->last_init = &((*vid->last_init)->common.chain);




  init = build_nop (cp_global_trees[CPTI_VTABLE_ENTRY_TYPE], offset);
  *vid->last_init = build_tree_list ((tree) ((void *)0), init);
  vid->last_init = &((*vid->last_init)->common.chain);
}
