...
# 24 "../../../kg++fe/gnu/cp/g++spec.c" 2
# 46 "../../../kg++fe/gnu/cp/g++spec.c"
void
     ...
     int *in_added_libraries;
{
       ...
       if (argc == 2)
  {
    ...
    library = 0;
     ...
     saw_speclang = 1;
       ...
       && (char *)strchr ("bBVDUoeTuIYmLiA", argv[i][1]) != ((void *)0))
      || strcmp (argv[i], "-Xlinker") == 0
      || strcmp (argv[i], "-Tdata") == 0))
     quote = argv[i];
   else if (library != 0 && ((argv[i][2] == '\0'
       && (char *) strchr ("cSEM", argv[i][1]) != ((void *)0))
      || strcmp (argv[i], "-MM") == 0
     ...
     shared_libgcc = 0;
   else if ((!strcmp (&argv[i][1], "Tdata") || !strcmp (&argv[i][1], "Ttext") || !strcmp (&argv[i][1], "Tbss") || !strcmp (&argv[i][1], "include") || !strcmp (&argv[i][1], "imacros") || !strcmp (&argv[i][1], "aux-info") || !strcmp (&argv[i][1], "idirafter") || !strcmp (&argv[i][1], "iprefix") || !strcmp (&argv[i][1], "iwithprefix") || !strcmp (&argv[i][1], "iwithprefixbefore") || !strcmp (&argv[i][1], "isystem") || !strcmp (&argv[i][1], "-param") || !strcmp (&argv[i][1], "specs") || !strcmp (&argv[i][1], "MF") || !strcmp (&argv[i][1], "MT") || !strcmp (&argv[i][1], "MQ")))
     i++;
   else
  ...
  while (i < argc)
    {
      ...
      if (!saw_math && (args[i] & (1<<2)) && library)
 {
   --j;
   saw_math = argv[i];
 }

      if (!saw_libc && (args[i] & (1<<3)) && library)
 {
   --j;
   saw_libc = argv[i];
 }
      ...
      if (args[i] & (1<<1))
 {
  ...
  if (library)
    {
      arglist[j++] = saw_profile_flag ? "-lstdc++" : "-lstdc++";
      added_libraries++;
  ...
  if (saw_math)
    ...
    {
      arglist[j++] = saw_profile_flag ? "-lm" : "-lm";
      added_libraries++;
    }
  if (saw_libc)
    arglist[j++] = saw_libc;
