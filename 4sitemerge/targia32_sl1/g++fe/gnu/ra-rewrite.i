...
# 34 "../../../kg++fe/gnu/ra-rewrite.c" 2
# 1 "../../../kg++fe/gnu/MIPS/insn-config.h" 1
# 35 "../../../kg++fe/gnu/ra-rewrite.c" 2
...
static unsigned long long spill_prop_savings (struct web *, sbitmap);
...
static _Bool is_partly_live_1 (sbitmap, struct web *);
...
static void reloads_to_loads (struct rewrite_info *, struct ref **, unsigned int, struct web **);
...
static unsigned long long deleted_move_cost;
     ...
     sbitmap coalesce, spilled;
{
 ...
 if ((((spilled)->elms [(s->id) / ((unsigned) (8 * 8))] >> (s->id) % ((unsigned) (8 * 8)) & 1) && ((coalesce)->elms [(t->id) / ((unsigned) (8 * 8))] >> (t->id) % ((unsigned) (8 * 8)) & 1))
     || (((spilled)->elms [(t->id) / ((unsigned) (8 * 8))] >> (t->id) % ((unsigned) (8 * 8)) & 1) && ((coalesce)->elms [(s->id) / ((unsigned) (8 * 8))] >> (s->id) % ((unsigned) (8 * 8)) & 1)))
   {
     struct conflict_link *wl;
     if (((sup_igraph)->elms [(s->id * num_webs + t->id) / ((unsigned) (8 * 8))] >> (s->id * num_webs + t->id) % ((unsigned) (8 * 8)) & 1)
  || ((sup_igraph)->elms [(t->id * num_webs + s->id) / ((unsigned) (8 * 8))] >> (t->id * num_webs + s->id) % ((unsigned) (8 * 8)) & 1)
  || s->pattern || t->pattern)
       continue;

     deleted_move_insns++;
     deleted_move_cost += (((m->insn)->fld[3]).bb)->frequency + 1;
     ((m->insn)->code = (enum rtx_code) (NOTE));
     (((m->insn)->fld[5]).rtint) = NOTE_INSN_DELETED;
     df_insn_modify (df, (((m->insn)->fld[3]).bb), m->insn);

     m->target_web->target_of_spilled_move = 1;
     if (s == t)

       continue;
# 122 "../../../kg++fe/gnu/ra-rewrite.c"
     if (t->type != SPILLED || s->type != SPILLED)
       fancy_abort ("../../../kg++fe/gnu/ra-rewrite.c", 123, __FUNCTION__);
...
spill_prop_savings (web, spilled)
     ...
     sbitmap spilled;
{
  unsigned long long savings = 0;
  struct move_list *ml;
  struct move *m;
  unsigned int cost;
  if (web->pattern)
    return 0;
  cost = 1 + ((((mips_tune == PROCESSOR_R4000) || (mips_tune == PROCESSOR_R6000)) ? 6 : 4) + memory_move_secondary_cost ((((enum machine_mode) (web->orig_x)->mode)), (web->regclass), (1)));
  cost += 1 + ((((mips_tune == PROCESSOR_R4000) || (mips_tune == PROCESSOR_R6000)) ? 6 : 4) + memory_move_secondary_cost ((((enum machine_mode) (web->orig_x)->mode)), (web->regclass), (0)));
  for (ml = wl_moves; ml; ml = ml->next)
    if ((m = ml->move) != ((void *)0))
      {
 struct web *s = alias (m->source_web);
 ...
 if (s != web || !((spilled)->elms [(t->id) / ((unsigned) (8 * 8))] >> (t->id) % ((unsigned) (8 * 8)) & 1) || t->pattern
     || ((sup_igraph)->elms [(s->id * num_webs + t->id) / ((unsigned) (8 * 8))] >> (s->id * num_webs + t->id) % ((unsigned) (8 * 8)) & 1)
     || ((sup_igraph)->elms [(t->id * num_webs + s->id) / ((unsigned) (8 * 8))] >> (t->id * num_webs + s->id) % ((unsigned) (8 * 8)) & 1))
   continue;
 savings += (((m->insn)->fld[3]).bb)->frequency * cost;
      }
  return savings;
}
     ...
     sbitmap list, processed;
{
 ...
 if (s != web || t->type != COLORED || ((processed)->elms [(t->id) / ((unsigned) (8 * 8))] >> (t->id) % ((unsigned) (8 * 8)) & 1))
   continue;
 ((list)->elms [(t->id) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (t->id) % ((unsigned) (8 * 8)));
 ((processed)->elms [(t->id) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (t->id) % ((unsigned) (8 * 8)));
      }
}
# 232 "../../../kg++fe/gnu/ra-rewrite.c"
static int
     ...
     sbitmap to_prop, spilled, processed;
{
  ...
  do { unsigned int word_num_; unsigned int bit_num_ = (0) % (unsigned int) ((unsigned) (8 * 8)); unsigned int size_ = (to_prop)->size; unsigned long long *ptr_ = (to_prop)->elms; for (word_num_ = (0) / (unsigned int) ((unsigned) (8 * 8)); word_num_ < size_; word_num_++, bit_num_ = 0) { unsigned long long word_ = ptr_[word_num_]; if (word_ != 0) for (; bit_num_ < ((unsigned) (8 * 8)); bit_num_++) { unsigned long long _mask = (unsigned long long) 1 << bit_num_; if ((word_ & _mask) != 0) { word_ &= ~ _mask; (id) = word_num_ * ((unsigned) (8 * 8)) + bit_num_; { spill_prop_insert (id2web[id], list, processed); }; if (word_ == 0) break; } } } } while (0);
  ...
  while ((id = sbitmap_first_set_bit (list)) >= 0)
    {
      ...
      ((list)->elms [(id) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (id) % ((unsigned) (8 * 8))));
      if (spill_prop_savings (web, spilled) >= web->spill_cost)
 {
   ...
   web->color = -1;
   put_web (web, SPILLED);
   ((spilled)->elms [(id) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (id) % ((unsigned) (8 * 8)));
   ((to_prop)->elms [(id) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (id) % ((unsigned) (8 * 8)));
   spill_prop_insert (web, list, processed);
   again = 1;
 }
    }
...
spill_coalprop ()
{
  ...
  for (d = (web_lists[(int)(SPILLED)]); d; d = d->next)
    ((spilled)->elms [(((d)->value.web)->id) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (((d)->value.web)->id) % ((unsigned) (8 * 8)));
...
{
  struct dlist *d;
  unsigned long long *costs = (unsigned long long *)
    xmalloc (176 * sizeof (costs[0]));
  for (d = (web_lists[(int)(SPILLED)]); d; d = d->next)
    {
      ...
      int bestc, c;
      HARD_REG_SET avail;
      memset (costs, 0, 176 * sizeof (costs[0]));
      for (wl = web->conflict_list; wl; wl = wl->next)
 {
   ...
   if (pweb->type == COLORED || pweb->type == PRECOLORED)
     costs[pweb->color] += pweb->spill_cost;
      ...
      do { HARD_REG_ELT_TYPE *scan_tp_ = (avail), *scan_fp_ = (web->usable_regs); scan_tp_[0] = scan_fp_[0]; scan_tp_[1] = scan_fp_[1]; scan_tp_[2] = scan_fp_[2]; } while (0);
      if (web->crosses_call)
 {


   for (c = 0; c < 176; c++)
     if ((!!((call_used_reg_set)[(c) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((c) % ((unsigned) (8 * 8)))))))
       costs[c] += 1000;
 }
      bestc = -1;
      for (c = 0; c < 176; c++)
 if ((bestc < 0 || costs[bestc] > costs[c])
            && (!!((avail)[(c) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((c) % ((unsigned) (8 * 8))))))
     && mips_hard_regno_mode_ok[ (int)(((enum machine_mode) ((cfun->emit->x_regno_reg_rtx)[web->regno])->mode)) ][ (c) ])
   {
     int i, size;
     size = mips_hard_regno_nregs (c, ((enum machine_mode) ((cfun->emit->x_regno_reg_rtx)[web->regno])->mode));
     for (i = 1; i < size
   && (!!((avail)[(c + i) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((c + i) % ((unsigned) (8 * 8)))))); i++);
     if (i == size)
       bestc = c;
   }
      web->color = bestc;
...
static unsigned long long spill_load_cost;
static unsigned long long spill_store_cost;
static unsigned long long spill_remat_cost;




static bitmap useless_defs;
# 398 "../../../kg++fe/gnu/ra-rewrite.c"
static void
     ...
     bitmap new_deaths;
{
  unsigned int i;
  struct dlist *d;
  bitmap b = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
 ...
 if (flag_ra_spill_every_use)
  ...
  if (!((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i'))
    continue;
         ...
         if (bitmap_bit_p (b, (((insn)->fld[0]).rtint)))
    continue;
         bitmap_set_bit (b, (((insn)->fld[0]).rtint));
  ...
  emit_insn_before (insns, insn);
         ...
         if (bb->head == insn)
    bb->head = (((prev)->fld[2]).rtx);
  for (insn = (((insn)->fld[1]).rtx); insn != prev;
       insn = (((insn)->fld[1]).rtx))
    {
      ((((insn)->fld[3]).bb) = bb);
 ...
 if (slot)
       ...
       if (insns)
  {
    ...
    if (bb->end == insn)
      bb->end = (((following)->fld[1]).rtx);
    for (insn = insns; insn != following; insn = (((insn)->fld[2]).rtx))
      {
        ((((insn)->fld[3]).bb) = bb);
        df_insn_modify (df, bb, insn);
      }
...
# 523 "../../../kg++fe/gnu/ra-rewrite.c"
       bitmap_set_bit (new_deaths, ((((((following)->fld[1]).rtx))->fld[0]).rtint));
     }
      }
...
{
  rtx base1, base2;
  long long ofs1 = 0, ofs2 = 0;
  int size1 = (mode_size[(int) (((enum machine_mode) (s1)->mode))]);
  int size2 = (mode_size[(int) (((enum machine_mode) (s2)->mode))]);
  if (((enum rtx_code) (s1)->code) == SUBREG)
    ofs1 = (((s1)->fld[1]).rtuint), s1 = (((s1)->fld[0]).rtx);
  if (((enum rtx_code) (s2)->code) == SUBREG)
    ofs2 = (((s2)->fld[1]).rtuint), s2 = (((s2)->fld[0]).rtx);
  ...
  if (((enum rtx_code) (s1)->code) == REG && ((enum rtx_code) (s2)->code) == REG)
    {
      if ((((s1)->fld[0]).rtuint) != (((s2)->fld[0]).rtuint))
 return 0;
 ...
 return 0;
  ...
  if (((enum rtx_code) (s1)->code) != MEM || ((enum rtx_code) (s2)->code) != MEM)
    fancy_abort ("../../../kg++fe/gnu/ra-rewrite.c", 585, __FUNCTION__);
  s1 = (((s1)->fld[0]).rtx);
  s2 = (((s2)->fld[0]).rtx);
  if (((enum rtx_code) (s1)->code) != PLUS || ((enum rtx_code) ((((s1)->fld[0]).rtx))->code) != REG
      || ((enum rtx_code) ((((s1)->fld[1]).rtx))->code) != CONST_INT)
    return 1;
  if (((enum rtx_code) (s2)->code) != PLUS || ((enum rtx_code) ((((s2)->fld[0]).rtx))->code) != REG
      || ((enum rtx_code) ((((s2)->fld[1]).rtx))->code) != CONST_INT)
    return 1;
  base1 = (((s1)->fld[0]).rtx);
  base2 = (((s2)->fld[0]).rtx);
  if (!rtx_equal_p (base1, base2))
    return 1;
  ofs1 += ((((((s1)->fld[1]).rtx))->fld[0]).rtwint);
  ofs2 += ((((((s2)->fld[1]).rtx))->fld[0]).rtwint);
  if (ofs1 >= ofs2 + size2 || ofs2 >= ofs1 + size1)
    return 0;
  return 1;
}
     ...
     bitmap new_deaths;
{
    ...
    if (insns)
      {
        ...
        if (bb->end == insn)
   bb->end = (((following)->fld[1]).rtx);
        for (ni = insns; ni != following; ni = (((ni)->fld[2]).rtx))
   {
     ((((ni)->fld[3]).bb) = bb);
      ...
      if (uid >= last_max_uid)
 {
   ...
   if (!set)
     ...
     {
       if (1 || ((enum rtx_code) ((((set)->fld[1]).rtx))->code) == MEM)
         delete_overlapping_slots (&slots, (((set)->fld[1]).rtx));
     }
 }
    }
}
     ...
     struct web *web1, *web2;
{
  ...
  size1 = web1->type == PRECOLORED
          ? 1 : mips_hard_regno_nregs (c1, ((enum machine_mode) ((cfun->emit->x_regno_reg_rtx)[web1->regno])->mode));
  size2 = web2->type == PRECOLORED
          ? 1 : mips_hard_regno_nregs (c2, ((enum machine_mode) ((cfun->emit->x_regno_reg_rtx)[web2->regno])->mode));
...
{
  do
    if (((live)->elms [(web->id) / ((unsigned) (8 * 8))] >> (web->id) % ((unsigned) (8 * 8)) & 1))
      return 1;
  while ((web = web->subreg_next));
  return 0;
}
# 794 "../../../kg++fe/gnu/ra-rewrite.c"
static void
     ...
     int add;
{
  ...
  if ((c = alias ((((web)->parent_web) ? find_web_for_subweb_1 ((web)->parent_web) : (web)))->color) < 0
      || c == an_unusable_color)
    return;
  size = mips_hard_regno_nregs (c, ((enum machine_mode) (web->orig_x)->mode));
  if ((((enum rtx_code) ((web)->orig_x)->code) == SUBREG))
    {
      c += subreg_regno_offset (c, ((enum machine_mode) ((((web->orig_x)->fld[0]).rtx))->mode),
    (((web->orig_x)->fld[1]).rtuint),
  ...
  if (add)
    for (; size--;)
      ((*in_use)[(c + size) / ((unsigned) (8 * 8))] |= ((HARD_REG_ELT_TYPE) (1)) << ((c + size) % ((unsigned) (8 * 8))));
  else
    for (; size--;)
      ((*in_use)[(c + size) / ((unsigned) (8 * 8))] &= ~(((HARD_REG_ELT_TYPE) (1)) << ((c + size) % ((unsigned) (8 * 8)))));
}
     ...
     struct web *web;
{
  ...
  size = web->type == PRECOLORED
         ? 1 : mips_hard_regno_nregs (c, ((enum machine_mode) ((cfun->emit->x_regno_reg_rtx)[web->regno])->mode));
  for (; size--;)
    if ((!!((*in_use)[(c + size) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((c + size) % ((unsigned) (8 * 8)))))))
      return 0;
  return 1;
}
     ...
     rtx last_block_insn;
{
 ...
 fancy_abort ("../../../kg++fe/gnu/ra-rewrite.c", 904, __FUNCTION__);
      ...
      if (!ri->need_load)
 {
   if (!supweb->spill_temp)
     ...
     ri->needed_loads[j] = 0;
      ...
      if (j < nl_first_reload && !((ri->live)->elms [(web->id) / ((unsigned) (8 * 8))] >> (web->id) % ((unsigned) (8 * 8)) & 1))
 continue;
      aweb = alias (supweb);
      aweb->changed = 1;
      start_sequence ();
      if (supweb->pattern)
 {
   ...
   if (aweb != supweb)
     fancy_abort ("../../../kg++fe/gnu/ra-rewrite.c", 931, __FUNCTION__);
   slot = copy_rtx (supweb->pattern);
   reg = copy_rtx (supweb->orig_x);


   if (reg != supweb->orig_x)
     fancy_abort ("../../../kg++fe/gnu/ra-rewrite.c", 937, __FUNCTION__);
 }
      else
 {
   allocate_spill_web (aweb);
      ...
      if (!before)
 {
   if ((((enum rtx_code) (last_block_insn)->code) == JUMP_INSN))
     ...
     after = last_block_insn;
      ...
      if (after)
 {
   rtx foll = (((after)->fld[2]).rtx);
   bb = (((after)->fld[3]).bb);
   emit_insn_after (ni, after);
   if (bb->end == after)
     bb->end = (((foll)->fld[1]).rtx);
   for (ni = (((after)->fld[2]).rtx); ni != foll; ni = (((ni)->fld[2]).rtx))
     {
       ((((ni)->fld[3]).bb) = bb);
       df_insn_modify (df, bb, ni);
     }
      ...
      else
 {
   ...
   spill_load_cost += bb->frequency + 1;
      ...
      ((ri->live)->elms [(web->id) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (web->id) % ((unsigned) (8 * 8))));
     ...
     struct web **ref2web;
{
 ...
 continue;
   ...
   && (!!((never_use_colors)[(supweb->color) / ((unsigned) (8 * 8))] & (((HARD_REG_ELT_TYPE) (1)) << ((supweb->color) % ((unsigned) (8 * 8)))))))
 continue;



      is_death = !((ri->live)->elms [(supweb->id) / ((unsigned) (8 * 8))] >> (supweb->id) % ((unsigned) (8 * 8)) & 1);
      is_death &= !((ri->live)->elms [(web->id) / ((unsigned) (8 * 8))] >> (web->id) % ((unsigned) (8 * 8)) & 1);
      if (is_death)
 {
   int old_num_r = num_reloads;
   bitmap_clear (ri->scratch);
   do { bitmap_element *ptr_ = (ri->need_reload)->first; unsigned int indx_ = (0) / ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))); unsigned bit_num_ = (0) % (unsigned) (8 * 4); unsigned word_num_ = (0) / (unsigned) (8 * 4) % ((128 + (8 * 4) - 1) / (8 * 4)); while (ptr_ != 0 && ptr_->indx < indx_) ptr_ = ptr_->next; if (ptr_ != 0 && ptr_->indx != indx_) { bit_num_ = 0; word_num_ = 0; } for (; ptr_ != 0; ptr_ = ptr_->next) { for (; word_num_ < ((128 + (8 * 4) - 1) / (8 * 4)); word_num_++) { BITMAP_WORD word_ = ptr_->bits[word_num_]; if (word_ != 0) { for (; bit_num_ < (unsigned) (8 * 4); bit_num_++) { BITMAP_WORD mask_ = ((BITMAP_WORD) 1) << bit_num_; if ((word_ & mask_) != 0) { word_ &= ~ mask_; (j) = (ptr_->indx * ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))) + word_num_ * (unsigned) (8 * 4) + bit_num_); { struct web *web2 = id2web[j]; struct web *aweb2 = alias ((((web2)->parent_web) ? find_web_for_subweb_1 ((web2)->parent_web) : (web2))); if (spill_is_free (&(ri->colors_in_use), aweb2) == 0) fancy_abort ("../../../kg++fe/gnu/ra-rewrite.c", 1068, __FUNCTION__); if (spill_same_color_p (supweb, aweb2) ) { if (!web2->in_load) { ri->needed_loads[ri->nl_size++] = web2; web2->in_load = 1; } bitmap_set_bit (ri->scratch, j); num_reloads--; } }; if (word_ == 0) break; } } } bit_num_ = 0; } word_num_ = 0; } } while (0);
# 1069 "../../../kg++fe/gnu/ra-rewrite.c"
     ...
     bitmap new_deaths;
{
  ...
  ri.need_reload = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
  ri.scratch = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
 ...
 break;
      ...
      do { HARD_REG_ELT_TYPE *scan_tp_ = (ri.colors_in_use); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
      do { bitmap_element *ptr_ = (live_at_end[i - 2])->first; unsigned int indx_ = (0) / ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))); unsigned bit_num_ = (0) % (unsigned) (8 * 4); unsigned word_num_ = (0) / (unsigned) (8 * 4) % ((128 + (8 * 4) - 1) / (8 * 4)); while (ptr_ != 0 && ptr_->indx < indx_) ptr_ = ptr_->next; if (ptr_ != 0 && ptr_->indx != indx_) { bit_num_ = 0; word_num_ = 0; } for (; ptr_ != 0; ptr_ = ptr_->next) { for (; word_num_ < ((128 + (8 * 4) - 1) / (8 * 4)); word_num_++) { BITMAP_WORD word_ = ptr_->bits[word_num_]; if (word_ != 0) { for (; bit_num_ < (unsigned) (8 * 4); bit_num_++) { BITMAP_WORD mask_ = ((BITMAP_WORD) 1) << bit_num_; if ((word_ & mask_) != 0) { word_ &= ~ mask_; (j) = (ptr_->indx * ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))) + word_num_ * (unsigned) (8 * 4) + bit_num_); { struct web *web = use2web[j]; struct web *aweb = alias ((((web)->parent_web) ? find_web_for_subweb_1 ((web)->parent_web) : (web))); if (aweb->type != SPILLED ) { ((ri.live)->elms [(web->id) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (web->id) % ((unsigned) (8 * 8))); if (aweb->type != SPILLED) update_spill_colors (&(ri.colors_in_use), web, 1); } }; if (word_ == 0) break; } } } bit_num_ = 0; } word_num_ = 0; } } while (0);
# 1131 "../../../kg++fe/gnu/ra-rewrite.c"
      ...
      if (flag_ra_ir_spilling)
 {
  ...
  if (aweb->type != SPILLED)
    continue;
         if (((!web->subreg_next) ? ((ri.live)->elms [(web->id) / ((unsigned) (8 * 8))] >> (web->id) % ((unsigned) (8 * 8)) & 1) : is_partly_live_1 (ri.live, web))
      ...
      for (; insn; insn = (((insn)->fld[1]).rtx))
 {
   ...
   if (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i') && (((insn)->fld[3]).bb) != last_bb)
     {
       int index = (((insn)->fld[3]).bb)->index + 2;
       do { bitmap_element *ptr_ = (live_at_end[index - 2])->first; unsigned int indx_ = (0) / ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))); unsigned bit_num_ = (0) % (unsigned) (8 * 4); unsigned word_num_ = (0) / (unsigned) (8 * 4) % ((128 + (8 * 4) - 1) / (8 * 4)); while (ptr_ != 0 && ptr_->indx < indx_) ptr_ = ptr_->next; if (ptr_ != 0 && ptr_->indx != indx_) { bit_num_ = 0; word_num_ = 0; } for (; ptr_ != 0; ptr_ = ptr_->next) { for (; word_num_ < ((128 + (8 * 4) - 1) / (8 * 4)); word_num_++) { BITMAP_WORD word_ = ptr_->bits[word_num_]; if (word_ != 0) { for (; bit_num_ < (unsigned) (8 * 4); bit_num_++) { BITMAP_WORD mask_ = ((BITMAP_WORD) 1) << bit_num_; if ((word_ & mask_) != 0) { word_ &= ~ mask_; (j) = (ptr_->indx * ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))) + word_num_ * (unsigned) (8 * 4) + bit_num_); { struct web *web = use2web[j]; struct web *aweb = alias ((((web)->parent_web) ? find_web_for_subweb_1 ((web)->parent_web) : (web))); if (aweb->type != SPILLED) { ((ri.live)->elms [(web->id) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (web->id) % ((unsigned) (8 * 8))); update_spill_colors (&(ri.colors_in_use), web, 1); } }; if (word_ == 0) break; } } } bit_num_ = 0; } word_num_ = 0; } } while (0);
# 1177 "../../../kg++fe/gnu/ra-rewrite.c"
       bitmap_clear (ri.scratch);
...
# 1194 "../../../kg++fe/gnu/ra-rewrite.c"
       bitmap_operation (ri.need_reload, ri.need_reload, ri.scratch,
    BITMAP_AND_COMPL);
       last_bb = (((insn)->fld[3]).bb);
       last_block_insn = insn;
       if (!((rtx_class[(int) (((enum rtx_code) (last_block_insn)->code))]) == 'i'))
  ...
  if (!((!supweb->subreg_next) ? ((ri.live)->elms [(supweb->id) / ((unsigned) (8 * 8))] >> (supweb->id) % ((unsigned) (8 * 8)) & 1) : is_partly_live_1 (ri.live, supweb)))
  ...
  if (web != supweb)
   ...
   bitmap_clear_bit (ri.need_reload, supweb->id);
    ...
    sweb = sweb->subreg_next)
        {
          ((ri.live)->elms [(sweb->id) / ((unsigned) (8 * 8))] &= ~((unsigned long long) 1 << (sweb->id) % ((unsigned) (8 * 8))));
  ...
  if (is_death)
    {
      ri.need_load = 1;
      bitmap_set_bit (new_deaths, (((insn)->fld[0]).rtint));
      break;
    }
       }

   if (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i') && ri.num_reloads)
     {
              int old_num_reloads = ri.num_reloads;
       reloads_to_loads (&ri, info.uses, info.num_uses, use2web);
# 1318 "../../../kg++fe/gnu/ra-rewrite.c"
       if (ri.num_reloads)
  ...
  ri.need_load = 1;
   ...
   if (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i') && flag_ra_ir_spilling)
     for (n = 0; n < info.num_uses; n++)
       {
  struct web *web = use2web[((info.uses[n])->id)];
  struct web *aweb = alias ((((web)->parent_web) ? find_web_for_subweb_1 ((web)->parent_web) : (web)));
      ...
      if (ri.num_reloads)
 {
   ...
   do { HARD_REG_ELT_TYPE *scan_tp_ = (cum_colors); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
   for (e = bb->pred; e && num < 5; e = e->pred_next, num++)
     {
       int j;
       do { HARD_REG_ELT_TYPE *scan_tp_ = (colors); scan_tp_[0] = 0; scan_tp_[1] = 0; scan_tp_[2] = 0; } while (0);
       ...
       do { HARD_REG_ELT_TYPE *scan_tp_ = (cum_colors), *scan_fp_ = (colors); scan_tp_[0] |= scan_fp_[0]; scan_tp_[1] |= scan_fp_[1]; scan_tp_[2] |= scan_fp_[2]; } while (0);
     }
   if (num == 5)
     in_ir = 1;
...
# 1422 "../../../kg++fe/gnu/ra-rewrite.c"
   bitmap_operation (ri.need_reload, ri.need_reload, ri.scratch,
      ...
      emit_loads (&ri, nl_first_reload, last_block_insn);
      if (ri.nl_size != 0 )
 fancy_abort ("../../../kg++fe/gnu/ra-rewrite.c", 1429, __FUNCTION__);
      if (!insn)
 break;
    }
  ...
  for (i = 0; i < web->num_uses; i++)
    {
      ...
      ((last_check_uses)->elms [(id) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (id) % ((unsigned) (8 * 8)));
...
detect_web_parts_to_rebuild ()
{
  ...
  uses_as_bitmap = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
  if (last_check_uses)
 ...
 for (; wl; wl = wl->next)
   {
     if (((already_webs)->elms [(wl->t->id) / ((unsigned) (8 * 8))] >> (wl->t->id) % ((unsigned) (8 * 8)) & 1))
       continue;
     ((already_webs)->elms [(wl->t->id) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (wl->t->id) % ((unsigned) (8 * 8)));
     mark_refs_for_checking (wl->t, uses_as_bitmap);
   }
 do { bitmap_element *ptr_ = (web->useless_conflicts)->first; unsigned int indx_ = (0) / ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))); unsigned bit_num_ = (0) % (unsigned) (8 * 4); unsigned word_num_ = (0) / (unsigned) (8 * 4) % ((128 + (8 * 4) - 1) / (8 * 4)); while (ptr_ != 0 && ptr_->indx < indx_) ptr_ = ptr_->next; if (ptr_ != 0 && ptr_->indx != indx_) { bit_num_ = 0; word_num_ = 0; } for (; ptr_ != 0; ptr_ = ptr_->next) { for (; word_num_ < ((128 + (8 * 4) - 1) / (8 * 4)); word_num_++) { BITMAP_WORD word_ = ptr_->bits[word_num_]; if (word_ != 0) { for (; bit_num_ < (unsigned) (8 * 4); bit_num_++) { BITMAP_WORD mask_ = ((BITMAP_WORD) 1) << bit_num_; if ((word_ & mask_) != 0) { word_ &= ~ mask_; (j) = (ptr_->indx * ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))) + word_num_ * (unsigned) (8 * 4) + bit_num_); { struct web *web2 = id2web[j]; if (((already_webs)->elms [(web2->id) / ((unsigned) (8 * 8))] >> (web2->id) % ((unsigned) (8 * 8)) & 1)) continue; ((already_webs)->elms [(web2->id) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (web2->id) % ((unsigned) (8 * 8))); mark_refs_for_checking (web2, uses_as_bitmap); }; if (word_ == 0) break; } } } bit_num_ = 0; } word_num_ = 0; } } while (0);
...
# 1552 "../../../kg++fe/gnu/ra-rewrite.c"
 ...
 for (link = df->regs[i].uses; link; link = link->next)
  ...
  if (rtl_dump_file && (debug_new_regalloc & 0x8000) != 0)
    {
...
static unsigned long long deleted_def_cost;
...
delete_useless_defs ()
{
  ...
  do { bitmap_element *ptr_ = (useless_defs)->first; unsigned int indx_ = (0) / ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))); unsigned bit_num_ = (0) % (unsigned) (8 * 4); unsigned word_num_ = (0) / (unsigned) (8 * 4) % ((128 + (8 * 4) - 1) / (8 * 4)); while (ptr_ != 0 && ptr_->indx < indx_) ptr_ = ptr_->next; if (ptr_ != 0 && ptr_->indx != indx_) { bit_num_ = 0; word_num_ = 0; } for (; ptr_ != 0; ptr_ = ptr_->next) { for (; word_num_ < ((128 + (8 * 4) - 1) / (8 * 4)); word_num_++) { BITMAP_WORD word_ = ptr_->bits[word_num_]; if (word_ != 0) { for (; bit_num_ < (unsigned) (8 * 4); bit_num_++) { BITMAP_WORD mask_ = ((BITMAP_WORD) 1) << bit_num_; if ((word_ & mask_) != 0) { word_ &= ~ mask_; (i) = (ptr_->indx * ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))) + word_num_ * (unsigned) (8 * 4) + bit_num_); { rtx insn = ((df->defs[i])->insn); rtx set = (((rtx_class[(int) (((enum rtx_code) (insn)->code))]) == 'i') ? (((enum rtx_code) ((((insn)->fld[5]).rtx))->code) == SET ? (((insn)->fld[5]).rtx) : single_set_2 (insn, (((insn)->fld[5]).rtx))) : (rtx) 0); struct web *web = (((def2web[i])->parent_web) ? find_web_for_subweb_1 ((def2web[i])->parent_web) : (def2web[i])); if (set && web->type == SPILLED && web->stack_slot == ((void *)0)) { deleted_def_insns++; deleted_def_cost += (((insn)->fld[3]).bb)->frequency + 1; ((insn)->code = (enum rtx_code) (NOTE)); (((insn)->fld[5]).rtint) = NOTE_INSN_DELETED; df_insn_modify (df, (((insn)->fld[3]).bb), insn); } }; if (word_ == 0) break; } } } bit_num_ = 0; } word_num_ = 0; } } while (0);
# 1607 "../../../kg++fe/gnu/ra-rewrite.c"
}
...
{
  int i;
  bitmap new_deaths = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
  reset_changed_flag ();
  spill_coalprop ();
  choose_spill_colors ();
  useless_defs = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
  ...
  do { bitmap_element *ptr_ = (new_deaths)->first; unsigned int indx_ = (0) / ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))); unsigned bit_num_ = (0) % (unsigned) (8 * 4); unsigned word_num_ = (0) / (unsigned) (8 * 4) % ((128 + (8 * 4) - 1) / (8 * 4)); while (ptr_ != 0 && ptr_->indx < indx_) ptr_ = ptr_->next; if (ptr_ != 0 && ptr_->indx != indx_) { bit_num_ = 0; word_num_ = 0; } for (; ptr_ != 0; ptr_ = ptr_->next) { for (; word_num_ < ((128 + (8 * 4) - 1) / (8 * 4)); word_num_++) { BITMAP_WORD word_ = ptr_->bits[word_num_]; if (word_ != 0) { for (; bit_num_ < (unsigned) (8 * 4); bit_num_++) { BITMAP_WORD mask_ = ((BITMAP_WORD) 1) << bit_num_; if ((word_ & mask_) != 0) { word_ &= ~ mask_; (i) = (ptr_->indx * ((unsigned) (((128 + (8 * 4) - 1) / (8 * 4)) * (unsigned) (8 * 4))) + word_num_ * (unsigned) (8 * 4) + bit_num_); { ((insns_with_deaths)->elms [(i) / ((unsigned) (8 * 8))] |= (unsigned long long) 1 << (i) % ((unsigned) (8 * 8)));}; if (word_ == 0) break; } } } bit_num_ = 0; } word_num_ = 0; } } while (0);
     ...
     struct df *df;
{
  ...
  regnos_coalesced_to_hardregs = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
 ...
 continue;
      if (web->reg_rtx || web->regno < 176)
 fancy_abort ("../../../kg++fe/gnu/ra-rewrite.c", 1713, __FUNCTION__);
      ...
      if (web->regno >= max_normal_pseudo)
 {
   ...
   if (web->color == an_unusable_color)
     {
       unsigned int inherent_size = (mode_size[(int) (((enum machine_mode) ((cfun->emit->x_regno_reg_rtx)[web->regno])->mode))]);
      ...
      else
 {
   ...
   if (web->num_uses == 0 && web->num_defs == 1)
     web->reg_rtx = gen_reg_rtx (((enum machine_mode) ((((enum rtx_code) ((web->defs[0])->reg)->code) == SUBREG ? ((((web->defs[0])->reg)->fld[0]).rtx) : ((web->defs[0])->reg)))->mode));
   else
     web->reg_rtx = gen_reg_rtx (((enum machine_mode) ((cfun->emit->x_regno_reg_rtx)[web->regno])->mode));

   if (web->type == COALESCED)
     bitmap_set_bit (regnos_coalesced_to_hardregs, (((web->reg_rtx)->fld[0]).rtuint));
 }
    }
...
# 1769 "../../../kg++fe/gnu/ra-rewrite.c"
  for (i = 0; i < df->use_id; i++)
    if (df->uses[i])
      {
 regset rs = (((((df->uses[i])->insn)->fld[3]).bb))->global_live_at_start;
 rtx regrtx;
 web = use2web[i];
 ...
 if (!regrtx)
   regrtx = web->reg_rtx;
 *(((enum rtx_code) ((df->uses[i])->reg)->code) == SUBREG ? &((((df->uses[i])->reg)->fld[0]).rtx) : ((df->uses[i])->loc)) = regrtx;
 if (bitmap_bit_p (rs, web->regno) && (((enum rtx_code) (regrtx)->code) == REG))
   {

     bitmap_set_bit (rs, (((regrtx)->fld[0]).rtuint));
   }
      }
  ...
  for (i = 0; i < df->def_id; i++)
    {
      ...
      if (!df->defs[i])
 continue;
      rs = (((((df->defs[i])->insn)->fld[3]).bb))->global_live_at_start;
      web = def2web[i];
      web = (((web)->parent_web) ? find_web_for_subweb_1 ((web)->parent_web) : (web));
      if (web->type != COLORED && web->type != COALESCED)
 continue;
      ...
      if (!regrtx)
 regrtx = web->reg_rtx;
      *(((enum rtx_code) ((df->defs[i])->reg)->code) == SUBREG ? &((((df->defs[i])->reg)->fld[0]).rtx) : ((df->defs[i])->loc)) = regrtx;
      if (bitmap_bit_p (rs, web->regno) && (((enum rtx_code) (regrtx)->code) == REG))
 {
 ...
 {
   int r = (((web->reg_rtx)->fld[0]).rtuint);
  ...
  old_regs = bitmap_initialize ((bitmap) xmalloc (sizeof (bitmap_head)), 1);
  for (si = 176; si < old_max_regno; si++)
     ...
     unsigned int level;
{
  ...
  ra_debug_msg (level, "%llu", spill_load_cost);
  ra_debug_msg (level, "\n  stores=%d cost=", emitted_spill_stores);
  ra_debug_msg (level, "%llu", spill_store_cost);
  ra_debug_msg (level, "\n  remat =%d cost=", emitted_remat);
  ra_debug_msg (level, "%llu", spill_remat_cost);
  ra_debug_msg (level, "\n removed:\n  moves =%d cost=", deleted_move_insns);
  ra_debug_msg (level, "%llu", deleted_move_cost);
  ra_debug_msg (level, "\n  others=%d cost=", deleted_def_insns);
  ra_debug_msg (level, "%llu", deleted_def_cost);
  ra_debug_msg (level, "\n");
}
