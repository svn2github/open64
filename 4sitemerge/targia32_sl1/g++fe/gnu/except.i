...
static void push_eh_cleanup (tree);
static _Bool decl_is_java_type (tree decl, int err);
static void initialize_handler_parm (tree, tree);
static tree do_allocate_exception (tree);
static tree stabilize_throw_expr (tree, tree *);
static tree wrap_cleanups_r (tree *, int *, void *);
static int complete_ptr_ref_or_void_ptr_p (tree, tree);
static _Bool is_admissible_throw_operand (tree);
...
init_exception_processing ()
{
  ...
  tmp = build_function_type (global_trees[TI_VOID_TYPE], global_trees[TI_VOID_LIST_NODE]);
  cp_global_trees[CPTI_TERMINATE] = build_cp_library_fn_ptr ("terminate", tmp);
  ((cp_global_trees[CPTI_TERMINATE])->common.volatile_flag) = 1;
  ((cp_global_trees[CPTI_TERMINATE])->common.nothrow_flag) = 1;
  pop_namespace ();


  tmp = tree_cons ((tree) ((void *)0), global_trees[TI_PTR_TYPE], global_trees[TI_VOID_LIST_NODE]);
  tmp = build_function_type (global_trees[TI_VOID_TYPE], tmp);
  cp_global_trees[CPTI_CALL_UNEXPECTED]
    = push_throw_library_fn ((__builtin_constant_p ("__cxa_call_unexpected") ? get_identifier_with_length (("__cxa_call_unexpected"), (unsigned) strlen ("__cxa_call_unexpected")) : (get_identifier) ("__cxa_call_unexpected")), tmp);

  (libfunc_table[LTI_eh_personality]) = init_one_libfunc (0
...
cp_protect_cleanup_actions ()
{
  ...
  return build_call (cp_global_trees[CPTI_TERMINATE], (tree) ((void *)0));
}
     ...
     tree type;
{
  ...
  if (decl_is_java_type (type, 0))
    exp = build_java_class_ref (((type)->common.type));
  else
    exp = get_tinfo_decl (type);

  mark_used (exp);
  exp = build1 (ADDR_EXPR, global_trees[TI_PTR_TYPE], exp);
...
build_exc_ptr ()
{
  return build (EXC_PTR_EXPR, global_trees[TI_PTR_TYPE]);
}
...
do_begin_catch ()
{
  ...
  fn = (__builtin_constant_p ("__cxa_begin_catch") ? get_identifier_with_length (("__cxa_begin_catch"), (unsigned) strlen ("__cxa_begin_catch")) : (get_identifier) ("__cxa_begin_catch"));
  if (namespace_binding ((fn), global_namespace))
    fn = namespace_binding ((fn), global_namespace);
  else
    {

      tree tmp = tree_cons ((tree) ((void *)0), global_trees[TI_PTR_TYPE], global_trees[TI_VOID_LIST_NODE]);
      fn = push_library_fn (fn, build_function_type (global_trees[TI_PTR_TYPE], tmp));
    }

  return build_function_call (fn, tree_cons ((tree) ((void *)0), build_exc_ptr (),
          (tree) ((void *)0)));
}
     ...
     tree type;
{
  ...
  return (((((((&((type)->type.lang_specific)->u.c)->methods))->vec.a[1])))->common.nothrow_flag);
}
     ...
     tree type;
{
  ...
  fn = (__builtin_constant_p ("__cxa_end_catch") ? get_identifier_with_length (("__cxa_end_catch"), (unsigned) strlen ("__cxa_end_catch")) : (get_identifier) ("__cxa_end_catch"));
  if (namespace_binding ((fn), global_namespace))
    fn = namespace_binding ((fn), global_namespace);
  else
    {

      fn = push_void_library_fn (fn, global_trees[TI_VOID_LIST_NODE]);

      ((fn)->common.nothrow_flag) = 0;
    }

  cleanup = build_function_call (fn, (tree) ((void *)0));
  ((cleanup)->common.nothrow_flag) = dtor_nothrow (type);
...
push_eh_cleanup (type)
     tree type;
{
  finish_decl_cleanup ((tree) ((void *)0), do_end_catch (type));
}
...
decl_is_java_type (decl, err)
     ...
     int err;
{
  _Bool r = (((enum tree_code) (decl)->common.code) == POINTER_TYPE
     && ((enum tree_code) (((decl)->common.type))->common.code) == RECORD_TYPE
     && ((((decl)->common.type))->type.lang_flag_3));

  if (err)
    {
      if (((enum tree_code) (decl)->common.code) == REFERENCE_TYPE
   && ((enum tree_code) (((decl)->common.type))->common.code) == RECORD_TYPE
   && ((((decl)->common.type))->type.lang_flag_3))
 {
 ...
 {
   tree jthrow_node
     = namespace_binding (((__builtin_constant_p ("jthrowable") ? get_identifier_with_length (("jthrowable"), (unsigned) strlen ("jthrowable")) : (get_identifier) ("jthrowable"))), global_namespace);

   if (jthrow_node == (tree) ((void *)0))
     fatal_error
       ("call to Java `catch' or `throw' with `jthrowable' undefined");

   jthrow_node = ((((jthrow_node)->common.type))->common.type);

   if (! (lookup_base ((((decl)->common.type)), jthrow_node, ba_any, ((void *)0)) != (tree) ((void *)0)))
     {
     ...
     enum languages lang;
{
 ...
 goto give_error;
  ...
  switch (lang)
    ...
    default:
      fancy_abort ("../../../kg++fe/gnu/cp/except.c", 317, __FUNCTION__);
    }
  return;
     ...
     tree exp;
{
  ...
  init_type = ((decl)->common.type);
  if (! (((enum tree_code) (init_type)->common.code) == POINTER_TYPE && ((enum tree_code) (((init_type)->common.type))->common.code) != OFFSET_TYPE)
      && ((enum tree_code) (init_type)->common.code) != REFERENCE_TYPE)
  ...
  if (((enum tree_code) (init_type)->common.code) == REFERENCE_TYPE
      && (((enum tree_code) (((init_type)->common.type))->common.code) == POINTER_TYPE && ((enum tree_code) (((((init_type)->common.type))->common.type))->common.code) != OFFSET_TYPE))
    exp = build_unary_op (ADDR_EXPR, exp, 1);

  exp = ocp_convert (init_type, exp, 1|64, 0);

  init = convert_from_reference (exp);



  if (((((decl)->common.type))->type.needs_constructing_flag))
    {


      init = ocp_convert (((decl)->common.type), init,
     1|64, 0);
      init = build1 (MUST_NOT_THROW_EXPR, ((init)->common.type), init);
    }
  ...
  start_decl_1 (decl);
  cp_finish_decl (decl, init, (tree) ((void *)0),
    (128)|(256));
}
     ...
     tree decl;
{
  ...
  if (decl && !complete_ptr_ref_or_void_ptr_p (((decl)->common.type), (tree) ((void *)0)))
    decl = (tree) ((void *)0);

  if (decl)
    type = prepare_eh_type (((decl)->common.type));
  else
    type = (tree) ((void *)0);
      ...
      if (decl_is_java_type (type, 1))
 {
   ...
   init = build (MINUS_EXPR, ((init)->common.type), init,
   ((((init)->common.type))->type.size_unit));
   init = build_indirect_ref (init, ((void *)0));
   is_java = 1;
 }
      else
 {
   ...
   init = do_begin_catch ();
      ...
      exp = create_temporary_var (global_trees[TI_PTR_TYPE]);
      ((exp)->decl.regdecl_flag) = 1;
      cp_finish_decl (exp, init, (tree) ((void *)0), (128));
...
expand_end_catch_block ()
{
  ...
  if ((cfun->language)->in_function_try_handler
   ...
   || (((current_function_decl)->decl.lang_specific)->decl_flags.destructor_attr)))
    finish_expr_stmt (build_throw ((tree) ((void *)0)));
}

tree
begin_eh_spec_block ()
{
  tree r = build_stmt (EH_SPEC_BLOCK, (tree) ((void *)0), (tree) ((void *)0));
  add_stmt (r);
  return r;
}
     ...
     tree eh_spec_block;
{
  ...
  do { (((eh_spec_block))->exp.operands[0]) = ((eh_spec_block)->common.chain); ((eh_spec_block)->common.chain) = (tree) ((void *)0); (current_stmt_tree ()->x_last_stmt) = eh_spec_block; } while (0);


  for (raises = (tree) ((void *)0);
       raw_raises && ((raw_raises)->list.value);
       raw_raises = ((raw_raises)->common.chain))
    raises = tree_cons ((tree) ((void *)0), prepare_eh_type (((raw_raises)->list.value)),
   raises);

  (((eh_spec_block))->exp.operands[1]) = raises;
}
     ...
     tree type;
{
  ...
  fn = (__builtin_constant_p ("__cxa_allocate_exception") ? get_identifier_with_length (("__cxa_allocate_exception"), (unsigned) strlen ("__cxa_allocate_exception")) : (get_identifier) ("__cxa_allocate_exception"));
  if (namespace_binding ((fn), global_namespace))
    fn = namespace_binding ((fn), global_namespace);
  else
    {

      tree tmp = tree_cons ((tree) ((void *)0), global_trees[TI_SIZE_TYPE], global_trees[TI_VOID_LIST_NODE]);
      fn = push_library_fn (fn, build_function_type (global_trees[TI_PTR_TYPE], tmp));
    }

  return build_function_call (fn, tree_cons ((tree) ((void *)0), size_in_bytes (type),
          (tree) ((void *)0)));
}
# 541 "../../../kg++fe/gnu/cp/except.c"
static tree
     ...
     void *data __attribute__ ((__unused__));
{
  ...
  if ((tree_code_type[(int) (((enum tree_code) (exp)->common.code))] == 't'))
    {
      *walk_subtrees = 0;
      return (tree) ((void *)0);
    }
  if (((enum tree_code) (exp)->common.code) != TARGET_EXPR)
    return (tree) ((void *)0);

  cleanup = (((exp))->exp.operands[2]);
  if (cleanup)
    {
      cleanup = build1 (MUST_NOT_THROW_EXPR, ((cleanup)->common.type), cleanup);
      (((exp))->exp.operands[2]) = cleanup;
    }


  return (tree) ((void *)0);
}
# 583 "../../../kg++fe/gnu/cp/except.c"
static tree
     ...
     tree *initp;
{
  ...
  if (((enum tree_code) (exp)->common.code) == TARGET_EXPR
      && ((enum tree_code) ((((exp))->exp.operands[1]))->common.code) == AGGR_INIT_EXPR
      && flag_elide_constructors)
    {
      tree aggr_init = ((((exp))->exp.operands[1]));
      tree args = ((aggr_init)->exp.operands[1]);
      tree newargs = (tree) ((void *)0);
      tree *p = &newargs;

      init_expr = c_global_trees[CTI_VOID_ZERO];
      for (; args; args = ((args)->common.chain))
 {
   tree arg = ((args)->list.value);
   tree arg_init_expr;

   arg = stabilize_expr (arg, &arg_init_expr);

   if (((arg_init_expr)->common.side_effects_flag))
     init_expr = build (COMPOUND_EXPR, global_trees[TI_VOID_TYPE], init_expr,
          arg_init_expr);
   *p = tree_cons ((tree) ((void *)0), arg, (tree) ((void *)0));
   p = &((*p)->common.chain);
 }
      ((aggr_init)->exp.operands[1]) = newargs;
    }
  else
    {
     ...
     tree exp;
{
  ...
  if (scope_chain->x_processing_template_decl)
    return build_min (THROW_EXPR, global_trees[TI_VOID_TYPE], exp);

  if (exp == cp_global_trees[CPTI_NULL])
  ...
  if (exp && decl_is_java_type (((exp)->common.type), 1))
    {
      tree fn = (__builtin_constant_p ("_Jv_Throw") ? get_identifier_with_length (("_Jv_Throw"), (unsigned) strlen ("_Jv_Throw")) : (get_identifier) ("_Jv_Throw"));
      if (namespace_binding ((fn), global_namespace))
 fn = namespace_binding ((fn), global_namespace);
      else
 {
   ...
   fn = push_throw_library_fn (fn, tmp);
      ...
      exp = build_function_call (fn, tree_cons ((tree) ((void *)0), exp, (tree) ((void *)0)));
    }
  else if (exp)
    {
      tree throw_type;
      ...
      fn = (__builtin_constant_p ("__cxa_throw") ? get_identifier_with_length (("__cxa_throw"), (unsigned) strlen ("__cxa_throw")) : (get_identifier) ("__cxa_throw"));
      if (namespace_binding ((fn), global_namespace))
 fn = namespace_binding ((fn), global_namespace);
      else
 {

   if (cp_global_trees[CPTI_CLEANUP_TYPE] == (tree) ((void *)0))
     {
       tmp = global_trees[TI_VOID_LIST_NODE];
       tmp = tree_cons ((tree) ((void *)0), global_trees[TI_PTR_TYPE], tmp);
       tmp = build_function_type (global_trees[TI_VOID_TYPE], tmp);
       cp_global_trees[CPTI_CLEANUP_TYPE] = build_pointer_type (tmp);
     }



   tmp = global_trees[TI_VOID_LIST_NODE];
   tmp = tree_cons ((tree) ((void *)0), cp_global_trees[CPTI_CLEANUP_TYPE], tmp);
   tmp = tree_cons ((tree) ((void *)0), global_trees[TI_PTR_TYPE], tmp);
   tmp = tree_cons ((tree) ((void *)0), global_trees[TI_PTR_TYPE], tmp);
   tmp = build_function_type (global_trees[TI_VOID_TYPE], tmp);
   fn = push_throw_library_fn (fn, tmp);
...
# 717 "../../../kg++fe/gnu/cp/except.c"
      exp = stabilize_throw_expr (exp, &temp_expr);


      allocate_expr = do_allocate_exception (((exp)->common.type));
      allocate_expr = get_target_expr (allocate_expr);
      ptr = (((allocate_expr))->exp.operands[0]);
      object = build1 (NOP_EXPR, build_pointer_type (((exp)->common.type)), ptr);
      object = build_indirect_ref (object, ((void *)0));


      exp = build_init (object, exp, (128));
      if (exp == global_trees[TI_ERROR_MARK])
 {
   ...
   return global_trees[TI_ERROR_MARK];
      ...
      exp = build1 (MUST_NOT_THROW_EXPR, ((exp)->common.type), exp);

      exp = build (COMPOUND_EXPR, ((exp)->common.type), allocate_expr, exp);
      if (temp_expr != c_global_trees[CTI_VOID_ZERO])
 {
   ...
   exp = build (COMPOUND_EXPR, ((exp)->common.type), temp_expr, exp);
   exp = build1 (CLEANUP_POINT_EXPR, ((exp)->common.type), exp);
 }

      throw_type = build_eh_type_type (prepare_eh_type (((object)->common.type)));

      if ((((((object)->common.type))->type.lang_flag_2)))
 {
   cleanup = lookup_fnfields (((((object)->common.type))->type.binfo),
         cp_global_trees[CPTI_COMPLETE_DTOR_IDENTIFIER], 0);
   cleanup = ((((cleanup))->exp.operands[1]));
   mark_used (cleanup);
   cxx_mark_addressable (cleanup);

   cleanup = build1 (ADDR_EXPR, cp_global_trees[CPTI_CLEANUP_TYPE], cleanup);
 }
      else
 {
   cleanup = build_int_2_wide ((unsigned long long) (0), (long long) (0));
   ((cleanup)->common.type) = cp_global_trees[CPTI_CLEANUP_TYPE];
 }

      tmp = tree_cons ((tree) ((void *)0), cleanup, (tree) ((void *)0));
      tmp = tree_cons ((tree) ((void *)0), throw_type, tmp);
      tmp = tree_cons ((tree) ((void *)0), ptr, tmp);

      tmp = build_function_call (fn, tmp);


      exp = build (COMPOUND_EXPR, ((tmp)->common.type), exp, tmp);
    }
  else
    {


      tree fn = (__builtin_constant_p ("__cxa_rethrow") ? get_identifier_with_length (("__cxa_rethrow"), (unsigned) strlen ("__cxa_rethrow")) : (get_identifier) ("__cxa_rethrow"));
      if (namespace_binding ((fn), global_namespace))
 fn = namespace_binding ((fn), global_namespace);
      else
 {
   ...
   fn = push_throw_library_fn
     (fn, build_function_type (global_trees[TI_VOID_TYPE], global_trees[TI_VOID_LIST_NODE]));
 }



      exp = build_function_call (fn, (tree) ((void *)0));
    }

  exp = build1 (THROW_EXPR, global_trees[TI_VOID_TYPE], exp);
     ...
     tree from;
{
  ...
  type = (complete_type_or_diagnostic ((type), (from), 0));
  if (!type)
    return 0;


  is_ptr = ((enum tree_code) (type)->common.code) == POINTER_TYPE;
  if (is_ptr || ((enum tree_code) (type)->common.code) == REFERENCE_TYPE)
    {
      tree core = ((type)->common.type);

      if (is_ptr && (((enum tree_code) (core)->common.code) == VOID_TYPE))
                ;
      else if (!(complete_type_or_diagnostic ((core), (from), 0)))
        return 0;
    }
  return 1;
}
...
# 871 "../../../kg++fe/gnu/cp/except.c"
# 1 "../../../kg++fe/gnu/cp/cfns.h" 1
     ...
     tree fn;
{
  ...
  if (((fn)->common.public_flag)
      && ((fn)->decl.external_flag)
      && (!(((fn)->decl.lang_flag_0) && (((enum tree_code) (fn)->common.code) == CONST_DECL || ((enum tree_code) (fn)->common.code) == PARM_DECL || ((enum tree_code) (fn)->common.code) == TYPE_DECL || ((enum tree_code) (fn)->common.code) == TEMPLATE_DECL)) && ((enum tree_code) ((((fn)->decl.context) ? ((fn)->decl.context) : global_namespace))->common.code) == NAMESPACE_DECL)
      && ((((fn)->decl.lang_specific) ? ((fn)->decl.lang_specific)->decl_flags.language : (((enum tree_code) (fn)->common.code) == FUNCTION_DECL ? lang_c : lang_cplusplus)) == lang_c))
            ;
  else
  ...
  return !!libc_name_p (((char *)(id)->identifier.id.str), ((id)->identifier.id.len));
}
     ...
     tree to, from;
{
  ...
  if (((enum tree_code) (to)->common.code) == POINTER_TYPE && ((enum tree_code) (from)->common.code) == POINTER_TYPE)
    {
      to = ((to)->common.type);
      from = ((from)->common.type);
...
check_handlers_1 (master, handlers)
     ...
     tree handlers;
{
  tree type = ((master)->common.type);
  tree handler;

  for (handler = handlers; handler; handler = ((handler)->common.chain))
    if (((handler)->common.type)
 && can_convert_eh (type, ((handler)->common.type)))
      {
 lineno = ((((handler))->exp.complexity));
 warning ("exception of type `%T' will be caught",
      ((handler)->common.type));
 lineno = ((((master))->exp.complexity));
     ...
     tree handlers;
{
  ...
  for (handler = handlers; handler; handler = ((handler)->common.chain))
    {
      if (((handler)->common.chain) == (tree) ((void *)0))
                                            ;
      else if (((handler)->common.type) == (tree) ((void *)0))
 {
   lineno = ((((handler))->exp.complexity));
   pedwarn
 ...
 check_handlers_1 (handler, ((handler)->common.chain));
    }
  lineno = save_line;
}
