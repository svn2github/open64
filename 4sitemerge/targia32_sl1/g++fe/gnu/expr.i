...
cplus_expand_constant (cst)
     tree cst;
{
  switch (((enum tree_code) (cst)->common.code))
    {
    case PTRMEM_CST:
      {
 ...
 member = (((ptrmem_cst_t)(cst))->member);
     ...
     int modifier;
{
  ...
  if (mode != (((target_flags & 0x00000002) && (target_flags & 0x00002000)) ? DImode : SImode) && modifier == EXPAND_SUM)
  ...
  switch (code)
    ...
    case OFFSET_REF:
      ...
      fancy_abort ("../../../kg++fe/gnu/cp/expr.c", 106, __FUNCTION__);
    ...
    case MUST_NOT_THROW_EXPR:
      ...
      ret = expand_expr (((exp)->exp.operands[0]), target, tmode, modifier);
      expand_eh_region_end_must_not_throw (build_call (cp_global_trees[CPTI_TERMINATE], 0));
      return ret;
  ...
  fancy_abort ("../../../kg++fe/gnu/cp/expr.c", 126, __FUNCTION__);
