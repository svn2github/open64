...
# 36 "../../../kg++fe/gnu/cp/rtti.c" 2
# 78 "../../../kg++fe/gnu/cp/rtti.c"
...
static _Bool typeid_ok_p (void);
static int qualifier_flags (tree);
static int target_incomplete_p (tree);
static tree tinfo_base_init (tree, tree);
static tree generic_initializer (tree, tree);
static tree ptr_initializer (tree, tree, int *);
static tree ptm_initializer (tree, tree, int *);
...
static tree get_pseudo_ti_init (tree, tree, int *);
static tree get_pseudo_ti_desc (tree);
static void create_tinfo_types (void);
static int typeinfo_in_lib_p (tree);

static int doing_runtime = 0;
# 112 "../../../kg++fe/gnu/cp/rtti.c"
void
init_rtti_processing ()
{
  push_namespace (cp_global_trees[CPTI_STD_IDENTIFIER]);
  cp_global_trees[CPTI_TYPE_INFO_TYPE]
    = xref_tag (class_type, (__builtin_constant_p ("type_info") ? get_identifier_with_length (("type_info"), (unsigned) strlen ("type_info")) : (get_identifier) ("type_info")),
                 (tree) ((void *)0), 1);
  pop_namespace ();
  cp_global_trees[CPTI_TYPE_INFO_PTR_TYPE] =
    build_pointer_type
     (build_qualified_type (cp_global_trees[CPTI_TYPE_INFO_TYPE], 0x1));
     ...
     tree exp;
{
  ...
  (void) (((((enum tree_code) (type)->common.code) == POINTER_TYPE) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/rtti.c", 140, __FUNCTION__), 0) : 0);
  ...
  index = build_int_2_wide ((unsigned long long) (-2 * 1), (long long) (-1));

  offset = build_vtbl_ref (build_indirect_ref (exp, ((void *)0)), index);

  type = build_qualified_type (global_trees[TI_PTR_TYPE],
          cp_type_quals (((exp)->common.type)));
  return build (PLUS_EXPR, type, exp,
  cp_convert (global_trees[TI_PTRDIFF_TYPE], offset));
}
...
throw_bad_cast ()
{
  tree fn = (__builtin_constant_p ("__cxa_bad_cast") ? get_identifier_with_length (("__cxa_bad_cast"), (unsigned) strlen ("__cxa_bad_cast")) : (get_identifier) ("__cxa_bad_cast"));
  if (namespace_binding ((fn), global_namespace))
    fn = namespace_binding ((fn), global_namespace);
  else
    fn = push_throw_library_fn (fn, build_function_type (global_trees[TI_PTR_TYPE],
        global_trees[TI_VOID_LIST_NODE]));

  return build_cxx_call (fn, (tree) ((void *)0), (tree) ((void *)0));
}

static tree
throw_bad_typeid ()
{
  tree fn = (__builtin_constant_p ("__cxa_bad_typeid") ? get_identifier_with_length (("__cxa_bad_typeid"), (unsigned) strlen ("__cxa_bad_typeid")) : (get_identifier) ("__cxa_bad_typeid"));
  if (namespace_binding ((fn), global_namespace))
    fn = namespace_binding ((fn), global_namespace);
  else
    {
      tree t = build_qualified_type (cp_global_trees[CPTI_TYPE_INFO_TYPE], 0x1);
      t = build_function_type (build_reference_type (t), global_trees[TI_VOID_LIST_NODE]);
     ...
     tree exp;
{
  ...
  if (!(((enum tree_code) (type)->common.code) == VOID_TYPE))
    type = (complete_type_or_diagnostic ((type), (exp), 0));

  if (!type)
    return global_trees[TI_ERROR_MARK];


  if ((((type)->common.lang_flag_2)) && ! resolves_to_fixed_type_p (exp, 0))
    {
      ...
      index = build_int_2_wide ((unsigned long long) (-1 * 1), (long long) (-1));
      t = build_vtbl_ref (exp, index);
      ((t)->common.type) = cp_global_trees[CPTI_TYPE_INFO_PTR_TYPE];
      return t;
    }


  return get_tinfo_ptr (((type)->type.main_variant));
}
...
typeid_ok_p ()
{
  ...
  if (!(((cp_global_trees[CPTI_TYPE_INFO_TYPE])->type.size) != (tree) ((void *)0)))
    {
...
build_typeid (exp)
     tree exp;
{
  tree cond = (tree) ((void *)0);
  int nonnull = 0;

  if (exp == global_trees[TI_ERROR_MARK] || !typeid_ok_p ())
    return global_trees[TI_ERROR_MARK];
  ...
  if (((enum tree_code) (exp)->common.code) == INDIRECT_REF
      && ((enum tree_code) (((((exp)->exp.operands[0]))->common.type))->common.code) == POINTER_TYPE
      && (((((exp)->common.type))->common.lang_flag_2))
      && ! resolves_to_fixed_type_p (exp, &nonnull)
      && ! nonnull)
    {
      exp = stabilize_reference (exp);
      cond = cp_convert (c_global_trees[CTI_BOOLEAN_TYPE], ((exp)->exp.operands[0]));
    }
  ...
  exp = build_indirect_ref (exp, ((void *)0));

  if (cond)
    {
      tree bad = throw_bad_typeid ();

      exp = build (COND_EXPR, ((exp)->common.type), cond, exp, bad);
    }
     ...
     tree type;
{
  ...
  if (((enum tree_code) (type)->common.code) == METHOD_TYPE)
    type = build_function_type (((type)->common.type),
    ((((type)->type.values))->common.chain));
  ...
  if ((((((enum tree_code) (type)->common.code)) == RECORD_TYPE || (((enum tree_code) (type)->common.code)) == UNION_TYPE) && (((enum tree_code) (type)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (type)->common.code) == TYPENAME_TYPE || ((enum tree_code) (type)->common.code) == TYPEOF_TYPE || ((enum tree_code) (type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((type)->type.lang_flag_5))))
    {
      d = ((&((((type)->type.main_variant))->type.lang_specific)->u.c)->typeinfo_var);
      if (d)
 return d;
    }
  ...
  d = namespace_binding ((name), global_namespace);
  if (!d)
    {
      tree var_desc = get_pseudo_ti_desc (type);

      d = build_lang_decl (VAR_DECL, name, ((var_desc)->common.type));

      ((d)->decl.artificial_flag) = 1;
      ((d)->common.readonly_flag) = 1;
      ((d)->common.static_flag) = 1;
      ((d)->decl.external_flag) = 1;
      ((d)->decl.assembler_name = (name));
      ((d)->decl.comdat_flag) = 1;

      pushdecl_top_level_and_finish (d, (tree) ((void *)0));

      if ((((((enum tree_code) (type)->common.code)) == RECORD_TYPE || (((enum tree_code) (type)->common.code)) == UNION_TYPE) && (((enum tree_code) (type)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (type)->common.code) == TYPENAME_TYPE || ((enum tree_code) (type)->common.code) == TYPEOF_TYPE || ((enum tree_code) (type)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((type)->type.lang_flag_5))))
 ((&((((type)->type.main_variant))->type.lang_specific)->u.c)->typeinfo_var) = d;


      ((name)->common.type) = type;
    }
     ...
     tree type;
{
  ...
  exp = ocp_convert (cp_global_trees[CPTI_TYPE_INFO_PTR_TYPE], exp, 8, 0);
...
get_typeid (type)
     tree type;
{
  if (type == global_trees[TI_ERROR_MARK] || !typeid_ok_p ())
    return global_trees[TI_ERROR_MARK];
...
ifnonnull (test, result)
     tree test, result;
{
  return build (COND_EXPR, ((result)->common.type),
  build (EQ_EXPR, c_global_trees[CTI_BOOLEAN_TYPE], test, global_trees[TI_INTEGER_ZERO]),
  cp_convert (((result)->common.type), global_trees[TI_INTEGER_ZERO]),
  result);
}
...
build_dynamic_cast_1 (type, expr)
     tree type, expr;
{
  enum tree_code tc = ((enum tree_code) (type)->common.code);
  tree exprtype = ((expr)->common.type);
  tree dcast_fn;
  tree old_expr = expr;
 ...
 break;
    case REFERENCE_TYPE:
      if (! (((enum tree_code) (((type)->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((type)->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((type)->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((type)->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((type)->common.type))->type.lang_flag_5)))
 {
   errstr = "target is not pointer or reference to class";
   goto fail;
 }
      if (!(((complete_type (((type)->common.type)))->type.size) != (tree) ((void *)0)))
 {
  ...
  if (((enum tree_code) (expr)->common.code) == OFFSET_REF)
    {
      ...
      exprtype = ((expr)->common.type);
    }

  if (tc == POINTER_TYPE)
    expr = convert_from_reference (expr);
  else if (((enum tree_code) (exprtype)->common.code) != REFERENCE_TYPE)
    {

      exprtype = build_reference_type (exprtype);
      expr = convert_to_reference (exprtype, expr, 1,
       (3), (tree) ((void *)0));
    }
  ...
  if (tc == POINTER_TYPE)
    {
      ...
      if (((enum tree_code) (exprtype)->common.code) != POINTER_TYPE)
 {
   errstr = "source is not a pointer";
   goto fail;
 }
      if (! (((enum tree_code) (((exprtype)->common.type))->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (((exprtype)->common.type))->common.code) == TYPENAME_TYPE || ((enum tree_code) (((exprtype)->common.type))->common.code) == TYPEOF_TYPE || ((enum tree_code) (((exprtype)->common.type))->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((((exprtype)->common.type))->type.lang_flag_5)))
 {
   errstr = "source is not a pointer to class";
   goto fail;
 }
      if (!(((complete_type (((exprtype)->common.type)))->type.size) != (tree) ((void *)0)))
 {
      ...
      if (!(((complete_type (((exprtype)->common.type)))->type.size) != (tree) ((void *)0)))
 {
  ...
  if (!at_least_as_qualified_p (((type)->common.type),
    ((exprtype)->common.type)))
    {
...

  {
    ...
    binfo = lookup_base (((exprtype)->common.type), ((type)->common.type),
    ba_not_special, ((void *)0));
 ...
 return expr;
  ...
  if ((((((exprtype)->common.type))->common.lang_flag_2)))
    {
      tree expr1;

      if (tc == POINTER_TYPE && (((enum tree_code) (((type)->common.type))->common.code) == VOID_TYPE))
 {

   if (((enum tree_code) (expr)->common.code) == ADDR_EXPR
       && ((enum tree_code) (((expr)->exp.operands[0]))->common.code) == VAR_DECL
       && ((enum tree_code) (((((expr)->exp.operands[0]))->common.type))->common.code) == RECORD_TYPE)
      ...
      else
 {
   ...
   if (tc == REFERENCE_TYPE)
     {
       if (((enum tree_code) (old_expr)->common.code) == VAR_DECL
    && ((enum tree_code) (((old_expr)->common.type))->common.code) == RECORD_TYPE)
  {
   ...
   else if (((enum tree_code) (expr)->common.code) == ADDR_EXPR)
     {
       tree op = ((expr)->exp.operands[0]);
       if (((enum tree_code) (op)->common.code) == VAR_DECL
    && ((enum tree_code) (((op)->common.type))->common.code) == RECORD_TYPE)
  {
    warning ("dynamic_cast of `%#D' to `%#T' can never succeed",
         op, type);
    retval = build_int_2_wide ((unsigned long long) (0), (long long) (0));
    ((retval)->common.type) = type;
    return retval;
  }
     }

   target_type = ((((type)->common.type))->type.main_variant);
   static_type = ((((exprtype)->common.type))->type.main_variant);
   ...
   elems = tree_cons
     ((tree) ((void *)0), expr1, tree_cons
      ((tree) ((void *)0), td3, tree_cons
       ((tree) ((void *)0), td2, tree_cons
        ((tree) ((void *)0), boff, (tree) ((void *)0)))));

   dcast_fn = cp_global_trees[CPTI_DCAST];
   if (!dcast_fn)
     {
       tree tmp;
       tree tinfo_ptr;
       tree ns = cp_global_trees[CPTI_ABI];
       const char *name;

       push_nested_namespace (ns);
       tinfo_ptr = xref_tag (class_type,
        (__builtin_constant_p ("__class_type_info") ? get_identifier_with_length (("__class_type_info"), (unsigned) strlen ("__class_type_info")) : (get_identifier) ("__class_type_info")),
                       (tree) ((void *)0),
        1);
  ...
  (build_qualified_type
   (tinfo_ptr, 0x1));
       name = "__dynamic_cast";
       tmp = tree_cons
  ((tree) ((void *)0), global_trees[TI_CONST_PTR_TYPE], tree_cons
   ((tree) ((void *)0), tinfo_ptr, tree_cons
    ((tree) ((void *)0), tinfo_ptr, tree_cons
     ((tree) ((void *)0), global_trees[TI_PTRDIFF_TYPE], global_trees[TI_VOID_LIST_NODE]))));
       tmp = build_function_type (global_trees[TI_PTR_TYPE], tmp);
       dcast_fn = build_library_fn_ptr (name, tmp);
              pop_nested_namespace (ns);
              cp_global_trees[CPTI_DCAST] = dcast_fn;
     }
     ...
     tree type;
{
  while (((enum tree_code) (type)->common.code) == POINTER_TYPE)
    if ((((enum tree_code) (type)->common.code) == POINTER_TYPE && ((enum tree_code) (((type)->common.type))->common.code) == OFFSET_TYPE))
      {
        if (!(((((((enum tree_code) (type)->common.code) == POINTER_TYPE && ((enum tree_code) (((type)->common.type))->common.code) == OFFSET_TYPE) ? ((((type)->common.type))->type.maxval) : (((((((((type)->type.values))->common.type)))->common.type))->type.maxval)))->type.size) != (tree) ((void *)0)))
          return 1;
        type = ((((enum tree_code) (type)->common.code) == POINTER_TYPE && ((enum tree_code) (((type)->common.type))->common.code) == OFFSET_TYPE) ? ((((type)->common.type))->common.type) : (((((((type)->type.values))->common.type)))->common.type));
      }
    else
      type = ((type)->common.type);
     ...
     tree target;
{
...

  {
    ...
    ((name_decl)->decl.artificial_flag) = 1;
    ((name_decl)->common.readonly_flag) = 1;
    ((name_decl)->common.static_flag) = 1;
    ((name_decl)->decl.external_flag) = 0;
    ((name_decl)->common.public_flag) = 1;
    comdat_linkage (name_decl);


    ((name_decl)->decl.assembler_name = (mangle_typeinfo_string_for_type (target)));

    ((name_decl)->decl.initial) = name_string;
    pushdecl_top_level_and_finish (name_decl, name_string);
  }

  vtable_ptr = ((desc)->list.value);
  if (!vtable_ptr)
    {
      tree real_type;

      push_nested_namespace (cp_global_trees[CPTI_ABI]);
      real_type = xref_tag (class_type, ((desc)->list.purpose),
                      (tree) ((void *)0), 1);
      pop_nested_namespace (cp_global_trees[CPTI_ABI]);

      if (!(((real_type)->type.size) != (tree) ((void *)0)))
 {



   ((&((real_type)->type.lang_specific)->u.c)->interface_unknown = 0);
   ((&((real_type)->type.lang_specific)->u.c)->interface_only) = 1;
 }
      ...
      vtable_ptr = build
 (PLUS_EXPR, ((vtable_ptr)->common.type), vtable_ptr,
  size_binop (MULT_EXPR,
       size_int_wide ((long long) (2 * 1), SIZETYPE),
       ((cp_global_trees[CPTI_VTABLE_ENTRY_TYPE])->type.size_unit)));
      ((vtable_ptr)->common.constant_flag) = 1;

      ((desc)->list.value) = vtable_ptr;
    }

  init = tree_cons ((tree) ((void *)0), vtable_ptr, init);

  init = tree_cons ((tree) ((void *)0), decay_conversion (name_decl), init);

  init = build (CONSTRUCTOR, (tree) ((void *)0), (tree) ((void *)0), nreverse (init));
  (((init)->common.lang_flag_4)) = ((init)->common.constant_flag) = ((init)->common.static_flag) = 1;
  init = tree_cons ((tree) ((void *)0), init, (tree) ((void *)0));
     ...
     tree target;
{
  ...
  init = build (CONSTRUCTOR, (tree) ((void *)0), (tree) ((void *)0), init);
  (((init)->common.lang_flag_4)) = ((init)->common.constant_flag) = ((init)->common.static_flag) = 1;
  return init;
}
     ...
     int *non_public_ptr;
{
  ...
  init = tree_cons ((tree) ((void *)0), build_int_2_wide ((unsigned long long) (flags), (long long) (0)), init);
  init = tree_cons ((tree) ((void *)0),
                    get_tinfo_ptr (((to)->type.main_variant)),
                    init);
...
{
  tree init = tinfo_base_init (desc, target);
  tree to = ((((enum tree_code) (target)->common.code) == POINTER_TYPE && ((enum tree_code) (((target)->common.type))->common.code) == OFFSET_TYPE) ? ((((target)->common.type))->common.type) : (((((((target)->type.values))->common.type)))->common.type));
  tree klass = ((((enum tree_code) (target)->common.code) == POINTER_TYPE && ((enum tree_code) (((target)->common.type))->common.code) == OFFSET_TYPE) ? ((((target)->common.type))->type.maxval) : (((((((((target)->type.values))->common.type)))->common.type))->type.maxval));
  int flags = qualifier_flags (to);
  int incomplete = target_incomplete_p (to);
  ...
  if (!(((klass)->type.size) != (tree) ((void *)0)))
    {
      flags |= 0x10;
      *non_public_ptr = 1;
    }
  ...
  if (((binfo)->common.static_flag))
    {
      if (((((((((enum tree_code) (basetype)->common.code)) == RECORD_TYPE || (((enum tree_code) (basetype)->common.code)) == UNION_TYPE) && (((enum tree_code) (basetype)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (basetype)->common.code) == TYPENAME_TYPE || ((enum tree_code) (basetype)->common.code) == TYPEOF_TYPE || ((enum tree_code) (basetype)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((basetype)->type.lang_flag_5))) ? (&((basetype)->type.lang_specific)->u.c)->marks : ((unsigned) ((basetype)->type.alias_set))) & (1 << (0))) != 0))
        *hint |= 1;
      if (((((((((enum tree_code) (basetype)->common.code)) == RECORD_TYPE || (((enum tree_code) (basetype)->common.code)) == UNION_TYPE) && (((enum tree_code) (basetype)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (basetype)->common.code) == TYPENAME_TYPE || ((enum tree_code) (basetype)->common.code) == TYPEOF_TYPE || ((enum tree_code) (basetype)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((basetype)->type.lang_flag_5))) ? (&((basetype)->type.lang_specific)->u.c)->marks : ((unsigned) ((basetype)->type.alias_set))) & (1 << (1))) != 0))
        *hint |= 2;
      ((((((enum tree_code) (basetype)->common.code)) == RECORD_TYPE || (((enum tree_code) (basetype)->common.code)) == UNION_TYPE) && (((enum tree_code) (basetype)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (basetype)->common.code) == TYPENAME_TYPE || ((enum tree_code) (basetype)->common.code) == TYPEOF_TYPE || ((enum tree_code) (basetype)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((basetype)->type.lang_flag_5))) ? (void) ((&((basetype)->type.lang_specific)->u.c)->marks |= (1 << (1))) : (void) (((basetype)->type.alias_set) |= (1 << (1))));
    }
  else
    {
      if (((((((((enum tree_code) (basetype)->common.code)) == RECORD_TYPE || (((enum tree_code) (basetype)->common.code)) == UNION_TYPE) && (((enum tree_code) (basetype)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (basetype)->common.code) == TYPENAME_TYPE || ((enum tree_code) (basetype)->common.code) == TYPEOF_TYPE || ((enum tree_code) (basetype)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((basetype)->type.lang_flag_5))) ? (&((basetype)->type.lang_specific)->u.c)->marks : ((unsigned) ((basetype)->type.alias_set))) & (1 << (0))) != 0) || ((((((((enum tree_code) (basetype)->common.code)) == RECORD_TYPE || (((enum tree_code) (basetype)->common.code)) == UNION_TYPE) && (((enum tree_code) (basetype)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (basetype)->common.code) == TYPENAME_TYPE || ((enum tree_code) (basetype)->common.code) == TYPEOF_TYPE || ((enum tree_code) (basetype)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((basetype)->type.lang_flag_5))) ? (&((basetype)->type.lang_specific)->u.c)->marks : ((unsigned) ((basetype)->type.alias_set))) & (1 << (1))) != 0))
        *hint |= 1;
      ((((((enum tree_code) (basetype)->common.code)) == RECORD_TYPE || (((enum tree_code) (basetype)->common.code)) == UNION_TYPE) && (((enum tree_code) (basetype)->common.code) == TEMPLATE_TYPE_PARM || ((enum tree_code) (basetype)->common.code) == TYPENAME_TYPE || ((enum tree_code) (basetype)->common.code) == TYPEOF_TYPE || ((enum tree_code) (basetype)->common.code) == BOUND_TEMPLATE_TEMPLATE_PARM || ((basetype)->type.lang_flag_5))) ? (void) ((&((basetype)->type.lang_specific)->u.c)->marks |= (1 << (0))) : (void) (((basetype)->type.alias_set) |= (1 << (0))));
    }
  if (!((binfo)->common.public_flag) && ((basetype)->type.binfo) != binfo)
    *hint |= 4;
  return (tree) ((void *)0);
};
     ...
     tree type;
{
  ...
  dfs_walk (((type)->type.binfo), dfs_class_hint_mark, ((void *)0), &hint_flags);
  dfs_walk (((type)->type.binfo), dfs_class_hint_unmark, ((void *)0), ((void *)0));

  for (i = 0; i < (((((((type)->type.binfo)))->vec.a[4]) ? (((((((type)->type.binfo)))->vec.a[4]))->vec.length) : 0)); ++i)
    {
      tree base_binfo = (((((((type)->type.binfo)))->vec.a[4]))->vec.a[(i)]);
     ...
     tree type;
{
  ...
  if (((enum tree_code) (type)->common.code) == POINTER_TYPE
      && (cp_type_quals (((type)->common.type)) == 0x1
   || cp_type_quals (((type)->common.type)) == 0x0))
    type = ((type)->common.type);
...
get_pseudo_ti_init (type, var_desc, non_public_p)
     ...
     int *non_public_p;
{
  (void) (((at_eof) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/rtti.c", 1033, __FUNCTION__), 0) : 0);
  switch (((enum tree_code) (type)->common.code))
    {
    case POINTER_TYPE:
      if ((((enum tree_code) (type)->common.code) == POINTER_TYPE && ((enum tree_code) (((type)->common.type))->common.code) == OFFSET_TYPE))
 ...
 return ptm_initializer (var_desc, type, non_public_p);
      else if (var_desc == cp_global_trees[CPTI_CLASS_DESC_TYPE])
        {
   if (!(((type)->type.size) != (tree) ((void *)0)))

     *non_public_p = 1;
   return class_initializer (var_desc, type, (tree) ((void *)0));
        }
      else if (var_desc == cp_global_trees[CPTI_SI_CLASS_DESC_TYPE])
 {
          tree base_binfos = (((((type)->type.binfo)))->vec.a[4]);
   tree base_binfo = ((base_binfos)->vec.a[0]);
   tree tinfo = get_tinfo_ptr (((base_binfo)->common.type));
   tree base_inits = tree_cons ((tree) ((void *)0), tinfo, (tree) ((void *)0));
   ...
   tree binfo = ((type)->type.binfo);
          int nbases = ((((binfo))->vec.a[4]) ? (((((binfo))->vec.a[4]))->vec.length) : 0);
          tree base_binfos = (((binfo))->vec.a[4]);
          tree base_inits = (tree) ((void *)0);
          int ix;


          for (ix = nbases; ix--;)
            {
              tree base_binfo = ((base_binfos)->vec.a[ix]);
              tree base_init = (tree) ((void *)0);
              int flags = 0;
              tree tinfo;
              tree offset;
              ...
              if (((base_binfo)->common.public_flag))
                flags |= 2;
              tinfo = get_tinfo_ptr (((base_binfo)->common.type));
       if (((base_binfo)->common.static_flag))
  {


    offset = ((binfo_for_vbase (((base_binfo)->common.type), type))->vec.a[5]);

    offset = convert (sizetype_tab[(int) SIZETYPE], offset);
    flags |= 1;
  }
       else
  offset = (((base_binfo))->vec.a[1]);


              offset = build_binary_op(LSHIFT_EXPR, offset, build_int_2_wide ((unsigned long long) (8), (long long) (0)), 1);

              offset = build_binary_op(BIT_IOR_EXPR, offset, build_int_2_wide ((unsigned long long) (flags), (long long) (0)), 1);

              base_init = tree_cons ((tree) ((void *)0), offset, base_init);
              base_init = tree_cons ((tree) ((void *)0), tinfo, base_init);
              base_init = build (CONSTRUCTOR, (tree) ((void *)0), (tree) ((void *)0), base_init);
       (((base_init)->common.lang_flag_4)) = 1;
              base_inits = tree_cons ((tree) ((void *)0), base_init, base_inits);
            }
   base_inits = build (CONSTRUCTOR,
         (tree) ((void *)0), (tree) ((void *)0), base_inits);
   (((base_inits)->common.lang_flag_4)) = 1;
   base_inits = tree_cons ((tree) ((void *)0), base_inits, (tree) ((void *)0));

   base_inits = tree_cons ((tree) ((void *)0),
      build_int_2_wide ((unsigned long long) (nbases), (long long) (0)), base_inits);

   base_inits = tree_cons ((tree) ((void *)0),
      build_int_2_wide ((unsigned long long) (hint), (long long) (0)), base_inits);
...
create_pseudo_type_info (const char *real_name, int ident, ...)
{
  ...
  { va_list ap; __builtin_va_start(ap,ident); { struct Qdmy;
  struct Qdmy;
  struct Qdmy;


  pseudo_name = (char *)__builtin_alloca(strlen (real_name) + 30);
  ...
  fields[0] = build_decl (FIELD_DECL, (tree) ((void *)0), cp_global_trees[CPTI_TI_DESC_TYPE]);


  for (ix = 0; (field_decl = __builtin_va_arg(ap,tree));)
    fields[++ix] = field_decl;


  pseudo_type = make_aggr_type (RECORD_TYPE);
  finish_builtin_type (pseudo_type, pseudo_name, fields, ix, global_trees[TI_PTR_TYPE]);
  ((&((pseudo_type)->type.lang_specific)->u.c)->as_base) = pseudo_type;

  result = tree_cons ((tree) ((void *)0), (tree) ((void *)0), (tree) ((void *)0));
  ((result)->list.purpose) = (__builtin_constant_p (real_name) ? get_identifier_with_length ((real_name), (unsigned) strlen (real_name)) : (get_identifier) (real_name));
  ((result)->common.type) =
    cp_build_qualified_type_real ((pseudo_type), (0x1), tf_error | tf_warning);
     ...
     tree type;
{
  switch (((enum tree_code) (type)->common.code))
    {
    case POINTER_TYPE:
      return (((enum tree_code) (type)->common.code) == POINTER_TYPE && ((enum tree_code) (((type)->common.type))->common.code) == OFFSET_TYPE) ? cp_global_trees[CPTI_PTM_DESC_TYPE] : cp_global_trees[CPTI_PTR_DESC_TYPE];
    case ENUMERAL_TYPE:
      return cp_global_trees[CPTI_ENUM_DESC_TYPE];
    case FUNCTION_TYPE:
      return cp_global_trees[CPTI_FUNC_DESC_TYPE];
    case ARRAY_TYPE:
      return cp_global_trees[CPTI_ARY_DESC_TYPE];
    case UNION_TYPE:
    case RECORD_TYPE:
      if ((((enum tree_code) (type)->common.code) == RECORD_TYPE && ((type)->type.lang_specific) && ((&((type)->type.lang_specific)->u.c)->ptrmemfunc_flag)))
 return cp_global_trees[CPTI_PTM_DESC_TYPE];
      else if (!(((type)->type.size) != (tree) ((void *)0)))
 {
   if (!at_eof)
     (cxx_incomplete_type_diagnostic (((tree) ((void *)0)), (type), 0));
   return cp_global_trees[CPTI_CLASS_DESC_TYPE];
 }
      else if (!(((((((type)->type.binfo)))->vec.a[4]) ? (((((((type)->type.binfo)))->vec.a[4]))->vec.length) : 0)))
 return cp_global_trees[CPTI_CLASS_DESC_TYPE];
      else
 {
   tree base_binfo =
     (((((((type)->type.binfo)))->vec.a[4]))->vec.a[0]);
   int num_bases = ((((((type)->type.binfo)))->vec.a[4]) ? (((((((type)->type.binfo)))->vec.a[4]))->vec.length) : 0);

   if (num_bases == 1
       && ((base_binfo)->common.public_flag)
       && !((base_binfo)->common.static_flag)
       && integer_zerop ((((base_binfo))->vec.a[1])))

     return cp_global_trees[CPTI_SI_CLASS_DESC_TYPE];
   else
     {
       tree var_desc;
       tree array_domain, base_array;

       if (((cp_global_trees[CPTI_VMI_CLASS_DESC_TYPE])->vec.length) <= num_bases)
  {
    int ix;
    tree extend = make_tree_vec (num_bases + 5);

    for (ix = ((cp_global_trees[CPTI_VMI_CLASS_DESC_TYPE])->vec.length); ix--;)
      ((extend)->vec.a[ix])
        = ((cp_global_trees[CPTI_VMI_CLASS_DESC_TYPE])->vec.a[ix]);
    cp_global_trees[CPTI_VMI_CLASS_DESC_TYPE] = extend;
  }
       var_desc = ((cp_global_trees[CPTI_VMI_CLASS_DESC_TYPE])->vec.a[num_bases]);
       if (var_desc)
  return var_desc;



       array_domain = build_index_type (size_int_wide ((long long) (num_bases), SIZETYPE));
       base_array =
  build_array_type (cp_global_trees[CPTI_BASE_DESC_TYPE], array_domain);

       push_nested_namespace (cp_global_trees[CPTI_ABI]);
       var_desc = create_pseudo_type_info
  ("__vmi_class_type_info", num_bases,
   build_decl (FIELD_DECL, (tree) ((void *)0), integer_types[itk_int]),
   build_decl (FIELD_DECL, (tree) ((void *)0), integer_types[itk_int]),
   build_decl (FIELD_DECL, (tree) ((void *)0), base_array),
   ((void *)0));
       pop_nested_namespace (cp_global_trees[CPTI_ABI]);

       ((cp_global_trees[CPTI_VMI_CLASS_DESC_TYPE])->vec.a[num_bases]) = var_desc;
       return var_desc;
     }
 }
    default:
      return cp_global_trees[CPTI_BLTN_DESC_TYPE];
    }
}
...
create_tinfo_types ()
{
  (void) (((!cp_global_trees[CPTI_TI_DESC_TYPE]) == 0) ? (fancy_abort ("../../../kg++fe/gnu/cp/rtti.c", 1283, __FUNCTION__), 0) : 0);
...

  {
    ...
    cp_global_trees[CPTI_TI_DESC_TYPE] = make_aggr_type (RECORD_TYPE);
    fields[0] = build_decl (FIELD_DECL, (tree) ((void *)0), global_trees[TI_CONST_PTR_TYPE]);
    fields[1] = build_decl (FIELD_DECL, (tree) ((void *)0), c_global_trees[CTI_CONST_STRING_TYPE]);
    finish_builtin_type (cp_global_trees[CPTI_TI_DESC_TYPE], "__type_info_pseudo",
                         fields, 1, global_trees[TI_PTR_TYPE]);
    (((cp_global_trees[CPTI_TI_DESC_TYPE])->type.lang_flag_1)) = 1;
  }


  cp_global_trees[CPTI_BLTN_DESC_TYPE] = create_pseudo_type_info
      ("__fundamental_type_info", 0,
       ((void *)0));


  cp_global_trees[CPTI_ARY_DESC_TYPE] = create_pseudo_type_info
      ("__array_type_info", 0,
       ((void *)0));
  cp_global_trees[CPTI_FUNC_DESC_TYPE] = create_pseudo_type_info
       ("__function_type_info", 0,
        ((void *)0));
  cp_global_trees[CPTI_ENUM_DESC_TYPE] = create_pseudo_type_info
       ("__enum_type_info", 0,
        ((void *)0));


  cp_global_trees[CPTI_CLASS_DESC_TYPE] = create_pseudo_type_info
        ("__class_type_info", 0,
         ((void *)0));



  cp_global_trees[CPTI_SI_CLASS_DESC_TYPE] = create_pseudo_type_info
           ("__si_class_type_info", 0,
            build_decl (FIELD_DECL, (tree) ((void *)0), cp_global_trees[CPTI_TYPE_INFO_PTR_TYPE]),
            ((void *)0));
...

  {
    ...
    fields[0] = build_decl (FIELD_DECL, (tree) ((void *)0), cp_global_trees[CPTI_TYPE_INFO_PTR_TYPE]);
    fields[1] = build_decl (FIELD_DECL, (tree) ((void *)0), integer_types[itk_long]);
    cp_global_trees[CPTI_BASE_DESC_TYPE] = make_aggr_type (RECORD_TYPE);
    finish_builtin_type (cp_global_trees[CPTI_BASE_DESC_TYPE], "__base_class_type_info_pseudo",
                         fields, 1, global_trees[TI_PTR_TYPE]);
    (((cp_global_trees[CPTI_BASE_DESC_TYPE])->type.lang_flag_1)) = 1;
  }


  cp_global_trees[CPTI_VMI_CLASS_DESC_TYPE] = make_tree_vec (10);




  cp_global_trees[CPTI_PTR_DESC_TYPE] = create_pseudo_type_info
  ...
  cp_global_trees[CPTI_PTM_DESC_TYPE] = create_pseudo_type_info
...
emit_support_tinfos ()
{
  static tree *const fundamentals[] =
  {
    ...
    &c_global_trees[CTI_BOOLEAN_TYPE],
    &c_global_trees[CTI_WCHAR_TYPE],
    &integer_types[itk_char], &integer_types[itk_signed_char], &integer_types[itk_unsigned_char],
    &integer_types[itk_short], &integer_types[itk_unsigned_short],
    &integer_types[itk_int], &integer_types[itk_unsigned_int],
    &integer_types[itk_long], &integer_types[itk_unsigned_long],
    &integer_types[itk_long_long], &integer_types[itk_unsigned_long_long],
    &global_trees[TI_FLOAT_TYPE], &global_trees[TI_DOUBLE_TYPE], &global_trees[TI_LONG_DOUBLE_TYPE],
    0
  };
  int ix;
  tree bltn_type, dtor;
  ...
  bltn_type = xref_tag (class_type,
   (__builtin_constant_p ("__fundamental_type_info") ? get_identifier_with_length (("__fundamental_type_info"), (unsigned) strlen ("__fundamental_type_info")) : (get_identifier) ("__fundamental_type_info")),
                  (tree) ((void *)0),
   1);
  pop_nested_namespace (cp_global_trees[CPTI_ABI]);
  if (!(((bltn_type)->type.size) != (tree) ((void *)0)))
    return;
  dtor = ((((&((bltn_type)->type.lang_specific)->u.c)->methods))->vec.a[1]);
  if (((dtor)->decl.external_flag))
    return;
  doing_runtime = 1;
  for (ix = 0; fundamentals[ix]; ix++)
    {
      ...
      tree bltn_const_ptr = build_pointer_type
              (build_qualified_type (bltn, 0x1));
      tree tinfo;

      tinfo = get_tinfo_decl (bltn);
      ((tinfo)->common.used_flag) = 1;
      ((decl_assembler_name (tinfo))->common.static_flag) = 1;
...
{
  if (
      ((enum tree_code) (t)->common.code) == VAR_DECL

      && namespace_binding ((((t)->decl.name)), global_namespace) == t

      && ((((t)->decl.name))->common.type)

      && ((enum tree_code) (((t)->common.type))->common.code) == RECORD_TYPE

      && ((((t)->common.type))->type.values)

      && ((((((t)->common.type))->type.values))->common.type) == cp_global_trees[CPTI_TI_DESC_TYPE])
    return 1;
  return 0;
}
# 1475 "../../../kg++fe/gnu/cp/rtti.c"
int
...
{
  tree decl = *decl_ptr;
  tree type = ((((decl)->decl.name))->common.type);
  int non_public;
  ...
  if ((((decl)->decl.external_flag) && ! (((decl)->decl.lang_specific)->decl_flags.not_really_extern)) || !((at_eof && ((decl)->common.public_flag) && !((decl)->decl.comdat_flag)) || (((decl)->decl.assembler_name != (tree) ((void *)0)) ) || (flag_syntax_only && ((decl)->common.used_flag))))
  ...
  if (((decl_assembler_name (decl))->common.static_flag))
    ((decl_assembler_name (decl))->common.not_emitted_by_gxx) = 0;
  else

    ((decl_assembler_name (decl))->common.not_emitted_by_gxx) = 1;
  ...
  ((decl)->common.public_flag) = !non_public;
  if (non_public)
    ((decl)->decl.comdat_flag) = 0;

  ((decl)->decl.initial) = var_init;
  cp_finish_decl (decl, var_init, (tree) ((void *)0), 0);



  ((((decl)->decl.name))->common.type) = (tree) ((void *)0);
