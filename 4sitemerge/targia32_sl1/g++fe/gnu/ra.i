     ...
     struct df *df;
{
 ...
 continue;
  ...
  if (refs_for_insn_df + (df->def_id + df->use_id) < act_refs)
    fancy_abort ("../../../kg++fe/gnu/ra.c", 282, __FUNCTION__);
}
     ...
     rtx reg;
{
  ...
  if (((enum rtx_code) (reg)->code) != SUBREG)
    fancy_abort ("../../../kg++fe/gnu/ra.c", 308, __FUNCTION__);
...
# 507 "../../../kg++fe/gnu/ra.c"
 ...
 if (mips_hard_regno_mode_ok[ (int)(i) ][ (reg) ]
  ...
  if (an_unusable_color == 176)
    fancy_abort ("../../../kg++fe/gnu/ra.c", 561, __FUNCTION__);
     ...
     struct df *df;
{
 ...
 for (link = ((df)->insns[(((insn)->fld[0]).rtint)].defs); link; link = link->next)
   if (!link->ref || bitmap_bit_p (empty_defs, ((link->ref)->id))
     ...
     fancy_abort ("../../../kg++fe/gnu/ra.c", 606, __FUNCTION__);
   else
 ...
 for (link = ((df)->insns[(((insn)->fld[0]).rtint)].uses); link; link = link->next)
   if (!link->ref || bitmap_bit_p (empty_uses, ((link->ref)->id))
       || bitmap_bit_p (b, ((link->ref)->id)))
     fancy_abort ("../../../kg++fe/gnu/ra.c", 614, __FUNCTION__);
   else
     bitmap_set_bit (b, ((link->ref)->id));
      }
 ...
 if (!link->ref || bitmap_bit_p (empty_defs, ((link->ref)->id))
     || bitmap_bit_p (b, ((link->ref)->id)))
   fancy_abort ("../../../kg++fe/gnu/ra.c", 626, __FUNCTION__);
 else
   bitmap_set_bit (b, ((link->ref)->id));
 ...
 if (!link->ref || bitmap_bit_p (empty_uses, ((link->ref)->id))
     || bitmap_bit_p (b, ((link->ref)->id)))
   fancy_abort ("../../../kg++fe/gnu/ra.c", 634, __FUNCTION__);
 else
   bitmap_set_bit (b, ((link->ref)->id));
    }
