...
% Treat -EL/EB the same a -mlittle/bigendian
% But note that some of the phases want one or the other.  So, each imply 
% the other, but have differ phase targets. -JMB
...
-version {show_version = TRUE; show_copyright = TRUE; execute_flag = FALSE;} A	N	""
...
-v	{show_flag = TRUE; show_version = TRUE;} A	p,a,l	self "-show"
	"show phases and version as they are being invoked"
-fullwarn toggle(&msglevel,2);		c,C,f,F,a  p	self "-m2"
...
-nostdinc++	nostdinc=TRUE;		C	p	self
	"Do not search for header files in the standard directories specific to C++"

-nostdlib	;	c,C,f,F	l	"-nostartfiles -nodefaultlibs"
	"no predefined libraries or startfiles"




-objectlist%s	source_kind=S_o;		c,C,f,F	l	"-objectlist %s"
...
% on 1, use g++ driver so don't need init and fini
...
-bigp_off toggle(&big_pages,FALSE);  A N ""
    "Disable large pages"

-bigp_on toggle(&big_pages,TRUE);  A N ""
    "Enable large pages"
...
-check_bounds	;		f,F	F	"-DEBUG:subscript_check"
	"check bounds"
...
% p is also a define, so trick the preprocessor
...
-mips1	toggle(&isa,ISA_MIPS1);		A	N	"-mips1" "-D__mips=1 -D_MIPS_ISA=1" "-TARG:isa=mips1"
	"compile for mips1 instruction set"
-mips2	toggle(&isa,ISA_MIPS2);		A	a,l	"-mips2" "-D__mips=2 -D_MIPS_ISA=2" "-TARG:isa=mips2"
	"compile for mips2 instruction set"
-mips3	toggle(&isa,ISA_MIPS3); 	A	a,l	"-mips3" "-D__mips=3 -D_MIPS_ISA=3" "-TARG:isa=mips3"
	"compile for mips3 instruction set"
-mips4	toggle(&isa,ISA_MIPS4); 	A	a,l	"-mips4"  "-TARG:isa=mips4"
	"compile for mips4 instruction set"

-mips64	toggle(&isa,ISA_MIPS64); 	f,F	l	"-mips64"  "-TARG:isa=mips64"
	"compile for mips64 instruction set"
-n32	toggle(&abi,ABI_N32); 	A	K	"-n32"  "-TARG:abi=n32"
	...
	""
-64     toggle(&abi,ABI_64);       A     K,K,M,a      "-64" "-D_ABI64=3 -D_MIPS_SIM=_ABI64 -D_MIPS_FPSET=32 -D_LP64 -Amodel(lp64)" "-TARG:abi=n64" "-mabi=64 -mlong64"
...
-itanium toggle(&isa,ISA_IA641);	A	N	"-i64"
	""
-32	toggle(&abi,ABI_I32); 	A	K,K,M	"-32" "-D_ILP32 -Amodel(ilp32)" "-TARG:abi=pr32"
        "compile for 32-bit PR ABI"
...
I-cpp_krdefs_linux	;		c,C	N	"-Dunix -Dlinux"
	""
-xansi	toggle(&ansi,EXTENDED_ANSI);		c	N	"" "-D__INLINE_INTRINSICS"  
	"extended ansi C"
-X*a	toggle(&ansi,EXTENDED_ANSI);		c	N	"" "-D__INLINE_INTRINSICS" 
	"extended ansi C"
-ansi	toggle(&ansi,STRICT_ANSI);		c,f,F	p,f	self "-Xansi -D__STRICT_ANSI__"
	"strict ansi"
-ansiE		;		f,F,c,C	f	self
	"issue error message for non standard features"
-ansiW		;		f,F,c,C	f	self
	"issue warning messages for non standard features"
-X*c	toggle(&ansi,STRICT_ANSI);		c	N	"-Xansi -D__STRICT_ANSI__"
	"strict ansi C"
-cckr	toggle(&ansi,KR_ANSI);		c	N	"-OPT:alias=cckr_default -Xcckr" "-D__INLINE_INTRINSICS" "-Dunix -Dsgi -DLANGUAGE_C"
	"K&R compatible C"
-X*t	toggle(&ansi,KR_ANSI);		c	N	"-OPT:alias=cckr_default -Xcckr" "-D__INLINE_INTRINSICS"  "-Dunix -Dsgi -DLANGUAGE_C"
	"K&R compatible C"
-ansiposix	warn_no_longer_supported(option_name); 	c	N	""
	""

-std=c89		;		c,C	p,f	self
	"-std option for gcc/g++"
-std=iso9899:1990	;		c,C	p,f	self
	"-std option for gcc/g++"
-std=iso9899:199409	;		c,C	p,f	self
	"-std option for gcc/g++"
-std=c99		;		c,C	p,f	self
	"-std option for gcc/g++"
-std=c9x		;		c,C	p,f	self
	"-std option for gcc/g++"
-std=iso9899:1999	;		c,C	p,f	self
	"-std option for gcc/g++"
-std=iso9899:199x	;		c,C	p,f	self
	"-std option for gcc/g++"
-std=gnu89		;		c,C	p,f	self
	"-std option for gcc/g++"
-std=gnu99		;		c,C	p,f	self
	"-std option for gcc/g++"
-std=gnu9x		;		c,C	p,f	self
	"-std option for gcc/g++"
...
I-D_LANGUAGE_C_PLUS_PLUS=1 ;	C	p	self
	""
%
% "-D__INLINE_INTRINSICS" is added to C compiles in add_special_option
...
I-DLANGUAGE_FORTRAN	;		f,F	p	self
	""
% f and a use p, but require k&r processing for things like # comments
...
-g2	toggle(&glevel,2);		A	f,i,b	self
	""
-g3	toggle(&glevel,3);		A	f,i,b	self
	"add some debugging to optimized code"
-gslim	toggle(&glevel,2);		c,C	f,i,b	self
	"put out slim class debug info"
-gslim0	toggle(&glevel,2);		c,C	f,i,b	self
	"put out slim class debug info"
-gslim2	toggle(&glevel,2);		c,C	f,i,b	self
	"put out slim class debug info"
-gdwarf	warn_ignored(option_name);	A	N	""
	"DWARF 1 format not supported"
...
-Nn%d	warn_no_longer_needed(option_name);  f,F	N self
	""
% use exact name a param rather than option_name for combined options 
...
-cray_mp    toggle(&mpkind,CRAY_MP);	 f,F F self 
...
-listing	keep_listing=TRUE;		f,F	F,K	""
...
-y%s		;		c,C,f,F	l	"-y %s"
	""
% the truth is, we use %x so this will b seen before %s;
% in other respects %x is the same a %d.
-D%x		;		c,C,f,F	l	"-D %d"
	""
-T%x		;		c,C,f,F	l	"-T %d"
	""
...
-flang-isoc9x	;		c,C	f	self
	""
-fallow-single-precision	;		c,C	f	self
	"Do not promote floats to double if using -traditional"
-fcond-mismatch	;		c,C	f	self
	""
-fno-cond-mismatch	;		c,C	f	self
	"Do not allow different types as args of ? operator"
-fasm	;		c,C	f	self
	""
...
-fmath-errno	toggle(&fmath_errno,1);		c,C	N	self
	""
-fno-math-errno	toggle(&fmath_errno,0);		c,C	N	self
...
-Wid-clash%d	;		c,C	f	self
	"Warn if 2 identifiers have the same first <num> chars"
-Wno-id-clash%d	;		c,C	f	self
	"Do not warn if 2 identifiers have the same first <num> chars"
...
-Wlarger-than-%d	;		c,C	f	self
	"Warn if an object is larger than <number> bytes"
-Wno-larger-than-%d	;		c,C	f	self
...
-Wdeprecated		;		c,C	p	self
	"Announce deprecation of compiler features"
-Wno-deprecated		;		c,C	p	self
...
-Wendif-labels	;		c,C	p	self
	"Warn if #if or #endif is followed by text"
-Wno-endif-labels	;		c,C	p	self
...
-Wno-globals	;		c,C	f	self
	"Do not warn about local/global variable inconsistencies"
...
-Wsurprising	;		c,C	f	self
	"Warn about code that might have unexpected semantics"
-Wno-surprising	;		c,C	f	self
	"Do not warn about code that might have unexpected semantics"
-Wsynth	;		C	f	self
...
-Wsystem-headers	;		c,C	f	self
	"Print warnings for constructs in system header files"
-Wno-system-headers	;		c,C	f	self
...
-fexpensive-optimizations	;		c,C	f	self
	""
-ffast-math	;		c,C	f	"-OPT:IEEE_arithmetic=2 -fno-math-errno"
	"Improve FP speed by violating ANSI & IEEE rules"
-ffloat-store	;		c,C	f	"-OPT:IEEE_arithmetic=1"
...
-MM	last_phase=P_any_cpp;	c,C,a	p	self
...
-fcall-used-%s	;		c,C	N	""
	""
-fexceptions	;	c,C	C	self
	"Enable exception handling"
-fhandle-exceptions	;	c,C	C	self
	"Enable exception handling"
-fno-exceptions	;	c,C	C	self
	"Disable exception handling"
-fno-handle-exceptions	;	c,C	C	self
	"Disable exception handling"
...
-mno-mmx	warn_ignored(option_name);	A	N	self
	"Disable MMX extensions"
-msse	;		A	N	self
	"Enable SSE extensions"
-mno-sse	warn_ignored(option_name);	A	N	self
	"Disable SSE extensions"
-msse2	;		A	N	self
	"Enable SSE2 extensions"
-mno-sse2	warn_ignored(option_name);	A	N	self
	"Disable SSE2 extensions"
-m3dnow	;		A	N	self
	"Enable 3Dnow extensions"
-mno-3dnow	warn_ignored(option_name);	A	N	self
...
-Wabi		warn_ignored(option_name);	A	C	self	""
	""
-###		{show_version = TRUE; show_but_not_run = TRUE;}	A	N	"-###"
