...
LOOP_DESCR_Add_BB(LOOP_DESCR *loop, BB *bb)
{
  ...
  do {
    /* We should first get the enclosing loop, and then add
     * the new BB to current loop, otherwise, we will not get
     * enclosing loop (since the newly added BB may not in 
     * enclosing loop)
     */
    LOOP_DESCR * enclosing = LOOP_DESCR_Next_Enclosing_Loop(loop);
    LOOP_DESCR_bbset(loop) = BB_SET_Union1D(LOOP_DESCR_bbset(loop), bb, loop->mem_pool);
    loop = enclosing;
  } while (loop);
}
