...
Pick_Compare_TOP (VARIANT *variant, TN **src1, TN **src2, OPS *ops)
{
  ...
  if (TN_has_value(*src2)) {
	...
	ISA_LC_Value_In_Class(TN_value(*src2), LC_pr_i16))
      cmp = cmp_i;
    else
  ...
  default:
    {
	  ...
	  if (TN_is_constant(src2) &&
	    ...
	    Build_OP (top, src1, Zero_TN, targ, ops);
	  } else {
	...
	switch (cond) {
	case V_BR_I8GE:	top = TOP_bgei; break;
	case V_BR_I8GT:	top = TOP_bgti; break;
	case V_BR_I8LE:	top = TOP_blei; break;
	case V_BR_I8LT:	top = TOP_blti; break;
	...
	case V_BR_I4GE:	top = TOP_bgei; break;
	case V_BR_I4GT:	top = TOP_bgti; break;
	case V_BR_I4LE:	top = TOP_blei; break;
	case V_BR_I4LT:	top = TOP_blti; break;
