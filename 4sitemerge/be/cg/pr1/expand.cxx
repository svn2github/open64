 ...
 * $Author: kevinlo $
 * $Source: /depot/CVSROOT/javi/src/sw/cmplr/be/cg/pr1/expand.cxx,v $
 *
 * Description:
 *
...
Expand_Copy (TN *result, TN *src, TYPE_ID mtype, OPS *ops)
{
  if (MTYPE_is_float(mtype)) {
    ...
    //    Build_OP (MTYPE_is_size_double(mtype)?TOP_mov_d: TOP_mov_s, result, src, ops);
  }
  else
    Build_OP(TOP_ori3, result, src, Zero_TN, ops);            
...
Expand_Convert_Length ( TN *dest, TN *src, TN *length_tn, TYPE_ID mtype, BOOL signed_extension, OPS *ops)
{
  ...
  if (val <= 15 && ! signed_extension) {
      Build_OP((val <=4) ? TOP_andi2 : TOP_andi3, dest, src, Gen_Literal_TN((1 << val) - 1, 4), ops);
  }
  else if (MTYPE_is_size_double(mtype) || val == 32) {
    Is_True(0, ("double not supported in PR1 isa"));
#ifndef TARG_PR
    if (val > 32) {
    ...
    TN *tmp = Build_TN_Of_Mtype(mtype);
    lit_tn = Gen_Literal_TN(32 - val, 4);
    Build_OP(TOP_shli3, tmp, src, lit_tn, ops);
    Build_OP(signed_extension ? TOP_shr3 : TOP_shl3, dest, tmp, lit_tn, ops);
  }
}
...
Exp_Immediate (TN *dest, TN *src, OPS *ops)
{
  ...
  if (TN_is_symbol(src) && TN_relocs(src) == TN_RELOC_PR_32) {
    Build_OP(TOP_movi, dest, Gen_Symbol_TN(TN_var(src), 0, TN_RELOC_PR_32), ops);
  }
  if (TN_is_symbol(src) && TN_relocs(src) == TN_RELOC_GPSUB) {
    Is_True(0,("wrong path"));
  }
  else if (ISA_LC_Value_In_Class (val, LC_pr_i4)) {
    Build_OP (TOP_ori2, dest, src, ops);
  }
  else if (ISA_LC_Value_In_Class (val, LC_pr_i14)) {
    Build_OP (TOP_addiu3, dest, Zero_TN, src, ops);
  }
  else if (val >= INT32_MIN && val <= INT32_MAX) {
    Build_OP (TOP_movi, dest, Gen_Literal_TN(val, 4), ops);
  }
  else if ((UINT64)val <= UINT32_MAX) {
	Is_True(0, ("large data NYI"));
	// Build_OP (TOP_ori, tmp, Zero_TN, Gen_Literal_TN((val >> 16) & 0xffff, 4), ops);
       //Build_OP (TOP_dsll, tmp, tmp, Gen_Literal_TN(16, 4), ops);
  }
  else {
    TCON tcon = Host_To_Targ (MTYPE_I4, val);
    ST *sym = New_Const_Sym (Enter_tcon (tcon), Be_Type_Tbl(MTYPE_I4));
    Allocate_Object(sym);
    FmtAssert((ST_gprel(sym)), 
	      ("sym not gp_rel: handle this"));
    Build_OP(TOP_ldw, dest, GP_TN, 
	     Gen_Symbol_TN(sym, 0, TN_RELOC_PR_GPREL), ops);
  } 
}
...
void
Expand_SR_Adj(BOOL isAdd, TN *result, TN *imm, OPS *ops)
{
  FmtAssert(TN_is_constant(imm), ("imm tn is non-const"));
  FmtAssert(TN_has_value(imm), ("imm tn has no value"));

  TN *addend = imm;
  INT32 val = TN_value(imm);
  if (!isAdd) {
    val = -val;
    addend = Gen_Literal_TN(val,4);
  }
  FmtAssert(ISA_LC_Value_In_Class(val, LC_pr_i14), ("imm value > 14 bits"));

  if (result == SP_TN)
    Build_OP(TOP_addsp3, result, result, addend, ops);
  else if (result == GP_TN)
    Build_OP(TOP_addgp3, result, result, addend, ops);
  else
    Is_True(0,("Illegal special register add"));
...
Expand_Add (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (TN_is_constant(src1)) {
	...
	return;
      ...
      if (ISA_LC_Value_In_Class (val, LC_pr_i14)) {
	if (ISA_LC_Value_In_Class (val, LC_pr_i4))
	  new_opcode = TOP_add2;
	new_opcode = TOP_add3;
      }
      else {
	...
	new_opcode = TOP_addgp3;
      if (TN_is_sp_reg(src2))
	new_opcode = TOP_addsp3;
      else {
	ST *base;
	INT64 val;
	TN *tmp = Build_TN_Of_Mtype (mtype);
	Base_Symbol_And_Offset_For_Addressing(TN_var(src1), TN_offset(src1), &base, &val);
	new_opcode = TOP_addiu3;
	if (ISA_LC_Value_In_Class (val, LC_pr_i4)) {
	  new_opcode = TOP_addiu2;
	}
	else if (val >= INT32_MIN && val <= INT32_MAX) {
	  Build_OP (TOP_movi, tmp, Gen_Literal_TN(val, 4), ops);
	  new_opcode = TOP_addu3;
	  src1 = tmp;
	} 
	else {
	  FmtAssert(0,(" 64 bit val NYI"));
	}  
      }     
      Build_OP (new_opcode, result, src2, src1, ops);
      return;
    ...
    Expand_Add (result, src2, src1, mtype, ops);
  	...
  	Build_OP (TOP_add3, result, src1, src2, ops);
  }
}
...
Expand_Sub (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (TN_is_constant(src2)) {
	...
	return;
      ...
      if (ISA_LC_Value_In_Class(val, LC_pr_i4))
	new_opcode = TOP_subi2;
      else if (ISA_LC_Value_In_Class(val, LC_pr_i14))
	new_opcode = TOP_subi3;
      else {
	src1 = Expand_Immediate_Into_Register(src1, is_64bit, ops);
	new_opcode = TOP_sub3;
      }
    ...
    // generate a negate
    Build_OP(TOP_subu3, result, Zero_TN, tmp, ops);
  } 
  else {
    Build_OP (TOP_subu3, result, src1, src2, ops);
  }
}
...
Expand_Neg (TN *result, TN *src, TYPE_ID mtype, OPS *ops)
{
  ...
  Build_OP (TOP_subiu3, result, Zero_TN, src, ops);
}
...
Expand_Abs (TN *dest, TN *src, TYPE_ID mtype, OPS *ops)
{
  ...
  Is_True(is_signed, ("Why abs of an unsigned?"));
  ...
  if (src != dest) {
    ...
    // tlti2 == tgei2 
    Build_OP(TOP_tgei2, p1, Zero_TN, src, ops);
    Build_OP(TOP_movtn, dest, src, p1, ops);
  }
...
Expand_Shift (TN *result, TN *src1, TN *src2, TYPE_ID mtype, SHIFT_DIRECTION kind, OPS *ops)
{
  ...
  if (is_64bit)
    Is_True(0,("64 bit NYI"));

  if (TN_is_constant(src1))
    src1 = Expand_Immediate_Into_Register(src1, MTYPE_is_size_double(mtype), ops);
  if (TN_has_value(src2)) {
    // only the low log2(wordsize) bits of the shift count are used. 
...
static void shladd(TN *r, TN *x1, INT s, TN *x2, OPS *ops)
{
  ...
  if (x2 == Zero_TN) {
    Build_OP(TOP_shl3, r, x1, Gen_Literal_TN(s, 4), ops);
  }
  new_opcode = TOP_shlad;
  if (TN_is_gp_reg(x2))
    new_opcode = TOP_shladgp;
  else if (TN_is_sp_reg(x2))
    new_opcode = TOP_shladsp;

  Build_OP (TOP_shlad,r, x1,Gen_Literal_TN (s, 4),x2,ops);
}
...
Expand_Multiply (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (! MTYPE_is_size_double(mtype))
    top = MTYPE_signed(mtype) ? TOP_mul3 : TOP_mulu3;
  else {
    Is_True(0,("double not supported in ISA"));
...
#else
  Is_True(0,("64 bit result not support in isa yet"));
#endif
}
...
#else
  Is_True(0,("64 bit result of high mult NYI"));
#endif
}

...
{
  /* dest = (src == 0) ? 1 : 0 */
  Build_OP ((src == dest) ? TOP_xori2 : TOP_xori3, dest, src, Gen_Literal_TN(1, 4), ops);
}

void
Expand_Logical_And (TN *dest, TN *src1, TN *src2, VARIANT variant, OPS *ops)
{
  Build_OP((src1 == dest) ? TOP_and2 : TOP_and3, dest, src1, src2, ops);
}

void
Expand_Logical_Or (TN *dest, TN *src1, TN *src2, VARIANT variant, OPS *ops)
{
  Build_OP((src1 == dest) ? TOP_or2 : TOP_or3, dest, src1, src2, ops);
}
...
{
  Build_OP(TOP_xori3, dest, src, Gen_Literal_TN(-1,4), ops);
}
...
Expand_Binary_And (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (TN_is_constant(src1)) {
    ...
    TOP new_opcode;
    if (ISA_LC_Value_In_Class ( val, LC_pr_u15)) {
      new_opcode = TOP_andi3;
      if (ISA_LC_Value_In_Class(val, LC_pr_u4))
	new_opcode = TOP_andi2;
    }
    else {
    ...
    Build_OP (TOP_and3, dest, src1, src2, ops);
  }
}
...
BOOL Expand_Special_And_Immed(TN *dest, TN *src1, INT64 imm, OPS *ops)
{
  ...
  if (start > 0 && start + len == 32) {
    ...
    if (extr_len == 8)
      Build_OP(TOP_bit_zext, dest, src1, ops);
    else {
      Build_OP(TOP_extru, dest, src1, Gen_Literal_TN(0, 4), Gen_Literal_TN(extr_len, 4), ops);
    }
  } else {
    Build_OP(TOP_depi, dest, Gen_Literal_TN(0, 4), src1, Gen_Literal_TN(start, 4), Gen_Literal_TN(len, 4), ops);
  }
  return TRUE;
}
...
Expand_Binary_Or (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (TN_is_constant(src1)) {
    ...
    else if (Expand_Special_And_Immed(dest, src1, val, ops)) {
      // call to Expand_... will finish the expansion if return true
      return;
    }
    else {
      src1 = Expand_Immediate_Into_Register(src1, MTYPE_bit_size(mtype) == 64, ops);
    ...
    Build_OP (TOP_or3, dest, src1, src2, ops);
  } 
}
...
Expand_Binary_Xor (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (TN_is_constant(src1)) {
    ...
    TOP new_opcode;
    if (ISA_LC_Value_In_Class ( val, LC_pr_u15)) {
      new_opcode = TOP_xori3;
      if (ISA_LC_Value_In_Class ( val, LC_pr_u15)) 
	new_opcode = TOP_xori2;
    }
    else {
      src1 = Expand_Immediate_Into_Register(src1, MTYPE_bit_size(mtype) == 64, ops);
      new_opcode = TOP_xor3;
    ...
    Build_OP (TOP_xor3, dest, src1, src2, ops);
  }
}
...
Expand_Binary_Nor (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  Expand_Binary_Xor (dest, tmp, Gen_Literal_TN(-1, 4), mtype, ops);
}
...
Expand_Int_Less (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (MTYPE_bit_size(mtype) == 64) {
    Is_True(0, ("64 bit data not supported"));
    return;
  }
  ...
  if (TN_is_constant(src2)) {
    ...
    TOP new_opcode;
    if (ISA_LC_Value_In_Class ( val, LC_pr_i13)) {
      if (MTYPE_signed(mtype))
	new_opcode == TOP_tlti3;
      else {
	Is_True(0,("Unsigned Int Less now supported in ISA"));
	return;
      }
    }
    else {
      src2 = Expand_Immediate_Into_Register(src2, 0, ops);
      if (MTYPE_signed(mtype))
	new_opcode == TOP_tlt3;
      else {
	Is_True(0, ("Unsigned Int Less now supported in ISA"));
    ...
    Build_OP (TOP_tlt3, dest, src1, src2, ops);
}
...
Expand_Int_Less_Equal (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (TN_is_constant(src2)) {
    ...
    if (ISA_LC_Value_In_Class ( val+1, LC_pr_i13)) {
      if (MTYPE_signed(mtype)) {
	Build_OP(TOP_tlti3, dest, src1, Gen_Literal_TN(val+1,4), ops);
      }
      else {
	Is_True(0, ("Unsigned tlt NYI in isa"));
	return;
      }
    }
    else {
    ...
    TN *tmp = Build_TN_Of_Mtype(mtype); 
    Build_OP(TOP_tle3, dest, src2, src1, ops);
  }
}
...
Expand_Int_Equal (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  Is_True(0,("tltiu NYI in isa (code using tlti3, wrong)"));
  Build_OP (TOP_tlti3, dest, tmp_tn, Gen_Literal_TN(1, 4), ops);
}

void
Expand_Int_Not_Equal (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  Build_OP(TOP_tne3, dest, src1, src2, ops);
}

void
Expand_Int_Greater_Equal (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  Build_OP(TOP_tge3, dest, src1, src2, ops);
}
...
Expand_Int_Greater (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (MTYPE_signed(mtype)) {
    ...
    Is_True(0,("unsigned tst NYI in isa"));
  }
}
...
#else
  Is_True(0, ("float arith NYI in isa"));
#endif
}
  ...
  OPS *ops)
{
  ...
  if (TN_register_class(cond_tn) == ISA_REGISTER_CLASS_integer) {
    ...
    Build_OP(TOP_tnei3, tmp_tn, cond_tn, Zero_TN, ops);
    Build_OP(TOP_movt, dest_tn, true_tn, ops);
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
    Build_OP(TOP_movf, dest_tn, false_tn, ops);
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
  }
  else {
    FmtAssert(FALSE, ("UNIMPLEMENTED"));
...
Expand_Min (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  BOOL is_signed = MTYPE_is_signed(mtype);
  if (!is_signed) {
    Is_True(0, ("unsigned NYI in isa"));
    return;
  }
  if (!MTYPE_is_float(mtype)) {
    TN *tmp_tn = Gen_Typed_Register_TN(MTYPE_I4, 4);
    if (src1 == dest) {
      Build_OP(TOP_tgem2, dest, dest, src2, ops);
    }
    else if (src2 == dest) {
      Build_OP(TOP_tgem2, dest, dest, src2, ops);
    }
    else {
      Build_OP(TOP_tgem2, dest, src2, src1, ops);
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
      Build_OP(TOP_movf, dest, src2, ops); 
    }
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
  }
}
        ...
        OPCODE compare, TN *cmp_kid1, TN *cmp_kid2, VARIANT variant, OPS *ops)
{
  ...
  if (result != true_tn && result != false_tn) {
    ...
    if (MTYPE_is_float(OPCODE_rtype(select))) {
      Build_OP (MTYPE_is_size_double(OPCODE_rtype(select))?TOP_mov_d:TOP_mov_s, result, true_tn, &new_ops);
    } 
    else 
#endif
    {
      Build_OP (TOP_or3, result, Zero_TN, true_tn, &new_ops);
    }
  }
  ...
  if (MTYPE_is_float(OPCODE_rtype(select))) {
    if (result != false_tn) {
      if (MTYPE_is_float(desc))
	Build_OP (MTYPE_is_size_double(OPCODE_rtype(select))?TOP_movf_d:TOP_movf_s, result, false_tn, p, &new_ops);
      else Build_OP (MTYPE_is_size_double(OPCODE_rtype(select))?TOP_movz_d:TOP_movz_s,
		     result, false_tn, p, &new_ops);
    } else {
      if (MTYPE_is_float(desc))
	Build_OP (MTYPE_is_size_double(OPCODE_rtype(select))?TOP_movt_d:TOP_movt_s,  result, true_tn, p, &new_ops);
...
Exp_COPY (TN *tgt_tn, TN *src_tn, OPS *ops)
{
  ...
  else
  {
    ...
    if (tgt_rc == src_rc && tgt_rc == ISA_REGISTER_CLASS_integer) {
      Build_OP( TOP_ori3, tgt_tn, src_tn, Zero_TN, ops );
      Set_OP_copy (OPS_last(ops));
    }
    else if (src_tn == tgt_tn)
    {
      ...
      Is_True(src_rc == ISA_REGISTER_CLASS_special, ("not supported reg class save"));
      TOP t;
      if (src_tn == LC_TN) t = TOP_movlc;
      else if (src_tn == RA_TN) t = TOP_movra;
      else if (src_tn == SP_TN) t = TOP_movsp;
      else if (src_tn == GP_TN) t = TOP_movgp;
      else { 
	Is_True(0,("unexpected special reg to save"));
      }
      Build_OP(t, tgt_tn, src_tn, ops);
    }
    else if (tgt_rc != ISA_REGISTER_CLASS_integer) {
      Is_True(tgt_rc == ISA_REGISTER_CLASS_special, ("not supported reg class save"));
      TOP t;
      if (tgt_tn == LC_TN) t = TOP_mov2lc;
      else if (tgt_tn == RA_TN) t = TOP_mov2ra;
      else if (tgt_tn == SP_TN) t = TOP_mov2sp;
      else if (tgt_tn == GP_TN) t = TOP_mov2gp;
      else { 
	Is_True(0,("unexpected special reg to save"));
      }
      Build_OP(t, tgt_tn, src_tn, ops);
...
void
Exp_Intrinsic_Op (INTRINSIC id, TN *result, TN *op0, TN *, TYPE_ID mtype, OPS *ops)
{
...
#else
  Is_True(0, ("Exp_Intrinsic_Call NYI"));
  return NULL;
#endif // TARG_PR
}
...
Exp_Is_Large_Stack_Sym(ST* sym,  INT64 ofst)
{
  ...
  if ((base_sym == SP_Sym || base_sym == FP_Sym) &&
      !ISA_LC_Value_In_Class(base_ofst, LC_pr_i14)) {
    return TRUE;
  }
...
#else
  DevWarn(("Please consult IA64 implementation"));
  Is_True(0, ("NYI"));
#endif
}
