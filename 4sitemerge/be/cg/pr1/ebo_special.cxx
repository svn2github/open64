 ...
 *  $Author: kevinlo $
 *  $Source: /depot/CVSROOT/javi/src/sw/cmplr/be/cg/pr1/ebo_special.cxx,v $
 *
 *  Revision comments:
 *
...
BOOL EBO_Can_Merge_Into_Offset (OP *op)
{
  ...
  if ((top != TOP_addi2) && (top != TOP_addi3) && 
      (top != TOP_addiu2) && (top != TOP_addiu3) &&
      (top != TOP_subi2) && (top != TOP_subi3) && 
      (top != TOP_subiu2) && (top != TOP_subiu3) &&
      (top != TOP_addlc3) && (top != TOP_addgp3) && 
      (top != TOP_addra3) && (top != TOP_addsp3))
    return FALSE;
...
/* NYI: The simulator does not yet support these instructions.  */
  ...
  return TRUE;
#else
  DevWarn(("Combined adjacent load NYI"));
  return FALSE;
#endif
}
                  ...
                  TN *tgt_tn, TN *src_tn, OPS *ops)
{
  ...
  TOP left_shift_op = TOP_shli2;
  INT left_shift_amt = reg_size - pos - bit_size;
  if (left_shift_amt > 16) {
    left_shift_op = TOP_shli3;
    left_shift_amt -= 32;
  }
  if (left_shift_amt >= 0)
...
#pragma mips_frequency_hint NEVER
  ...
  TOP extr_op = TOP_extr; /* Loads are always zero-filled on IA-64! */
		  ...
		  EBO_OP_INFO *opinfo)
{
  ...
  */
  if (OP_load(op) && OP_load(opinfo->in_op))
  {
    ...
    {
      DevWarn(("Revisit non-optimal case"));
      if (EBO_Trace_Optimization)
...
Convert_Imm_Add (OP *op, TN *tnr, TN *tn, INT64 imm_val, EBO_TN_INFO *tninfo)
{
  ...
  if (tnr == tn && imm_val == 0) {
    DevWarn(("adding self with imm 0"));
    return FALSE;
  }
  /* If 'imm_val' is 0, try a mov, otherwise try addi and addmi. */

  if (ISA_LC_Value_In_Class(imm_val, LC_pr_i16))
    top = TOP_is_2bytes(OP_code(op)) ? TOP_addiu2 : TOP_addiu3;
  else
    return FALSE;
                   ...
                   EBO_TN_INFO **opnd_tninfo)
{
  ...
  if (TOP_is_iadd(op)) {
    if (tnr == tn1) {
      if (imm_val == 0) {
	DevWarn(("Adding to itself with imm=0"));
	return FALSE;
      }
    }
...
Resolve_Conditional_Branch (OP *op, TN **opnd_tn)
{
  ...
  DevWarn(("Only handling beq and bne now"));
  if (top == TOP_beq) 
                          ...
                          EBO_TN_INFO **opnd_tninfo)
{
  ...
  {
    if (opcode == TOP_sub2 || opcode == TOP_sub3)
    {
      result_val = tn0_val - tn1_val;
      goto Constant_Created;
    }
  ...
  if (opcode == TOP_and2 || opcode == TOP_and3)
  {
    result_val = tn0_uval & tn1_uval;
    goto Constant_Created;
  }
  else if (opcode == TOP_or2 || opcode == TOP_or3)
  {
    result_val = tn0_uval | tn1_uval;
    goto Constant_Created;
  }
  else if (opcode == TOP_xor2 || opcode == TOP_xor3)
  {
    result_val = tn0_uval ^ tn1_uval;
    goto Constant_Created;
  }
  ...
  if (opcode == TOP_shl2 || opcode == TOP_shl3)
  {
    result_val = TRUNC_32(tn0_uval << tn1_uval);
    goto Constant_Created;
  }
  else if (opcode == TOP_shr2 || opcode == TOP_shr3)
  {
...
{
#ifdef TARG_PR
  Is_True(0,("PR isa does not support srl"));
  return NULL;
#else
  if (!srl_opinfo)
    return NULL;
...
INT EBO_Copy_Operand (OP *op)
{
  ...
  if (OP_copy(op)) {
    ...
    if (topcode == TOP_add2 || topcode == TOP_add3 ||
        topcode == TOP_addu2 || topcode == TOP_addu3 ||
        topcode == TOP_or2 || topcode == TOP_or3
        )
      return 0;    
  }
...
{  
  ...
  if (OP_code(op) == TOP_beq ||
	...
	opnd1_tninfo->in_op &&
	((OP_code(opnd1_tninfo->in_op) == TOP_addiu2) ||
	 (OP_code(opnd1_tninfo->in_op) == TOP_addiu3))) {
	...
	opnd0_tninfo->in_op &&
	(OP_code(opnd0_tninfo->in_op) == TOP_shr2 || 
	 OP_code(opnd0_tninfo->in_op) == TOP_shr3)) {
      EBO_TN_INFO *tninfo;
...
void Redundancy_Elimination()
{
  for( BB* bb = REGION_First_BB; bb != NULL; bb = BB_next( bb ) ){
	...
	continue; // end for this op
      ...
      // what about shift immediates?
      if (OP_code( op ) != TOP_shr2 && 
	  OP_code( op ) != TOP_shr3 && 
	  OP_code( op ) != TOP_shl2 && 
	  OP_code( op ) != TOP_shl3 )
	continue;
