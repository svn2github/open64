...
Expand_Power_Of_2_Divide (TN *result, TN *numer, INT64 dvsr, TYPE_ID mtype, OPS *ops)
{
  ...
  if (MTYPE_is_unsigned(mtype)) {
    ...
    Build_OP(TOP_tgei2, p1, Zero_TN, numer, ops);
    Expand_Add (t1, Gen_Literal_TN(absdvsr-1, is_double ? 8 : 4), 
		numer, mtype, ops);
    Build_OP(TOP_movti, t1, Zero_TN, numer, p1, ops);
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
    if (n > 31 && is_double) {
      Is_True(0,("64 bit power of 2 divide NYI"));
      //      Build_OP(TOP_dsra32, t2, t1, Gen_Literal_TN(n-32, 4), ops);
    }
    else 
      Build_OP(TOP_shr2, t2, t1, Gen_Literal_TN(n, 4), ops);
    if (dvsr < 0) 
      Expand_Neg(result, t2, mtype, ops);
  }
}
				  ...
				  TYPE_ID mtype, OPS *ops)
{
  ...
  if (is_signed) {
    ...
    if (is_double)
      Is_True(0,("pr1 has no double isa"));
      // Build_OP (TOP_dsubu, neg_tn, Zero_TN, numer_tn, ops);
    else
      Build_OP (TOP_subu2, neg_tn, Zero_TN, numer_tn, ops);      
    if (is_double) 
      Is_True(0,("pr1 has no double isa"));
      // Build_OP (TOP_dsrl32, p1, numer_tn, Gen_Literal_TN(31, 4), ops);
    else {
      Is_True(0,("pr1 has no logical shift right"));
      // Build_OP (TOP_srl, p1, numer_tn, Gen_Literal_TN(31, 4), ops);
    }
    Build_OP (TOP_movti, neg_tn, Zero_TN, numer_tn, p1, ops);
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
	...
	if (is_double) {
	  Is_True(0,("pr1 has no double isa"));
      //	  Build_OP (((n - 1)>31)?TOP_dsra32:TOP_dsra, shift_tn, mult_tn, Gen_Literal_TN ((n-1)%32, 4), ops);
      }
	else
	  Build_OP (TOP_shr2, 
	...
	Build_OP (TOP_subiu2, result, Zero_TN, shift_tn, ops);
      Build_OP (TOP_tgemi2, result, Zero_TN, shift_tn, p1, ops);
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
    } else {
	...
	if (is_double) {
	  Is_True(0,("pr1 has no double isa"));
      //	  Build_OP (((n - 1)>31)?TOP_dsra32:TOP_dsra, result, result, Gen_Literal_TN ((n-1)%32, 4), ops);
	}
	else
	  Build_OP (TOP_shr2, 
	...
	Build_OP (TOP_subiu2, mult_tn, Zero_TN, result, ops);
      Build_OP (TOP_tgemi2, result, Zero_TN, mult_tn, p1, ops);
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);      
    }
  } /* end Signed */
	...
	Is_True(0,("pr1 has no double isa"));
      ...
      Build_OP (tmp, tmp1_tn, numer_tn, Gen_Literal_TN (1, 4), ops);
      numer_tn = tmp1_tn;
    }
	...
	Build_OP (TOP_subu2, tmp1_tn, numer_tn, mult_tn, ops);
	Build_OP (TOP_shr2, tmp2_tn, tmp1_tn, Gen_Literal_TN (1, 4), ops);
	Build_OP (TOP_addu2, tmp3_tn, mult_tn, tmp2_tn, ops);
      }
      mult_tn = tmp3_tn;
    }
    ...
    if (is_double)
      Is_True(0,("pr1 has no double isa"));
      //      Build_OP (((n-1)>31)?TOP_dsrl32:TOP_dsrl, result, mult_tn, Gen_Literal_TN ((n-1)%32, 4), ops);    
    else
      Build_OP (TOP_shr2, result, mult_tn, Gen_Literal_TN ((n-1)%32, 4), ops);    
  } /* end Unsigned */
...
Expand_Power_Of_2_Rem (TN *result, TN *src1, INT64 src2_val, TYPE_ID mtype, OPS *ops)
{
  ...
  if (MTYPE_is_signed(mtype)) {
    ...
    Build_OP(TOP_tlti3, p1, src1, Zero_TN, ops);
    ...
    if (is_double) 
      ...
      Build_OP(TOP_subiu2, tmp1, Zero_TN, src1, ops);
    Build_OP(TOP_movti, tmp1, Zero_TN, src1, p1, ops);
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
    ...
    if (is_double) 
      ...
      Build_OP(TOP_subiu2, result, Zero_TN, tmp2, ops);
    Build_OP(TOP_movti, result, Zero_TN, tmp2, p1, ops);
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
  } else {
...
Expand_Divide (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (! MTYPE_is_size_double(mtype))
    top = MTYPE_signed(mtype) ? TOP_div2 : TOP_divu2;
  else {
      Is_True(0,("pr1 has no double isa"));
      //    top = MTYPE_signed(mtype) ? TOP_ddiv : TOP_ddivu;
  }
  if (TN_is_constant(src2))
    src2 = Expand_Immediate_Into_Register(src2, MTYPE_is_size_double(mtype), ops);
  Is_True(0, ("PR1 isa do not handle DIV/Mul properly"));
#if 0
...
Expand_Rem (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (const_src2) {
	...
	if (is_double)  {
	  ...
	  Build_OP(TOP_subu2, result, src1, mult_tn, ops);
	return;
      }
    }
  }
  ...
  Build_OP(TOP_mfhi, result, Hilo_TN(), ops);
    ...
    Is_True(0,("pr1 isa does not handle DIV/mul properly"));
  }
#endif
}
...
Expand_Mod (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  Build_OP(TOP_xor2, tmp2, tmp1, src2, ops);

  p1 = Gen_Typed_Register_TN(MTYPE_I4, 4);
  Build_OP(TOP_tgei2, p1, Zero_TN, tmp2, ops);

  /* result = divisor + remainder if p1
   * result = remainder if ~p1
   */
  tmp3 = Build_TN_Like(result);
  Build_OP(TOP_movfi, tmp3, Zero_TN, src2, p1, ops);
  Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
  Build_OP(TOP_movti, tmp3, Zero_TN, p1, ops);
  Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_COND_DEF);
  if (is_double) {
    Is_True(0,("pr1 has no double isa"));
    ...
    Build_OP(TOP_addu2, result, tmp1, tmp3, ops);
}
