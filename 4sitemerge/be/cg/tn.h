...
#endif
#ifdef TARG_PR
  TN_RELOC_PR_GPREL   = 0x30,
  TN_RELOC_PR_SCALL   = 0x31,
  TN_RELOC_PR_CALL    = 0x32,
  TN_RELOC_PR_SJ      = 0x33,
  TN_RELOC_PR_J       = 0x34,
  TN_RELOC_PR_SOFS    = 0x35,
  TN_RELOC_PR_OFS     = 0x36,

  TN_RELOC_PR_32     = 0x40,   // all 32 bits
#endif
#if defined(TARG_SL)
 ...
 *  Returns TRUE if TNs are identical register tn
 */
inline BOOL 
...
inline TN *Build_TN_Like(TN *tn)
{
  ...
  if (!TN_is_register(tn)) {
    ...
    if (tn == RA_TN || tn == GP_TN || tn == SP_TN || tn == LC_TN) {
