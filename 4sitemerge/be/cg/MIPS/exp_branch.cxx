...
#ifdef TARG_SL2
  extern OPS New_OPs; 
  extern BOOL  Is_C2_Scond_Instruction(TOP opcode);
  if( OPS_last(&New_OPs) && OP_c2_br_op(OPS_last(&New_OPs)) && 
         Is_C2_Scond_Instruction(OP_code(OPS_last(&New_OPs)))) {
     cmp = (cond == V_BR_I4EQ) ? TOP_c2_br_f:TOP_c2_br_t;
  }

#ifndef fork_joint
  else if(OPS_last(&New_OPs) && 
  	((OP_code(OPS_last(&New_OPs)) == TOP_c2_thread_major) ||
  	(OP_code(OPS_last(&New_OPs)) == TOP_c2_thread_minor)))
  {
     cmp = (OP_code(OPS_last(&New_OPs)) == TOP_c2_thread_major) ?  TOP_c2_fork_m : TOP_c2_fork_n;
     OP_Change_To_Noop(OPS_last(&New_OPs));
  }
#endif 
...
#ifdef TARG_SL2
       else if(cmp == TOP_c2_br_f || cmp == TOP_c2_br_t) {
	  TN *copc_tmp = Gen_Register_TN(ISA_REGISTER_CLASS_copc, 1);
	    Build_OP (cmp, copc_tmp, targ, ops);
       }
#endif 

#ifndef fork_joint
       else if(cmp == TOP_c2_fork_m || cmp == TOP_c2_fork_n) {
	   Build_OP (cmp, targ, ops);
       }
#endif 
