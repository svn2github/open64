...
extern ST *SP_Sym;
extern ANNOTATION *ANNOT_Get (ANNOTATION *list, ANNOTATION_KIND kind);
...
static BOOL Op_ENTRYINFO_sp_adj (OP *op) {
    ANNOTATION *ann = ANNOT_Get (BB_annotations(op->bb), ANNOT_ENTRYINFO);
    if (ann) {
	ENTRYINFO *ent_info = ANNOT_entryinfo(ann);
	if (op == ENTRYINFO_sp_adj(ent_info))
		return TRUE;
...
static void Set_ENTRYINFO_sp_adj (OP *replaced_op, OP* new_op) {
    ANNOTATION *ann = ANNOT_Get (BB_annotations(replaced_op->bb), ANNOT_ENTRYINFO);
    ENTRYINFO *ent_info = ANNOT_entryinfo(ann);
    ENTRYINFO_sp_adj(ent_info) = new_op;
}



static BOOL Op_EXITINFO_sp_adj (OP *op) {
	
   ANNOTATION *ann= ANNOT_Get (BB_annotations(op->bb), ANNOT_EXITINFO);
   if (ann) {   
	  EXITINFO *exit_info = ANNOT_exitinfo (ann);
	  if (op == EXITINFO_sp_adj(exit_info))
		return TRUE;
   } 
   return FALSE;	
}

static void Set_EXITINFO_sp_adj (OP *replaced_op, OP* new_op) {
	
    ANNOTATION *ann = ANNOT_Get (BB_annotations(replaced_op->bb), ANNOT_EXITINFO);
    EXITINFO *ent_info = ANNOT_exitinfo(ann);
    EXITINFO_sp_adj(ent_info) = new_op;
}
...
Repl_Size16_Instr::Replace_ResEqOper_OP (BOOL either = TRUE) {
  ...
  if (tn_registers_identical (rtn, op1tn)) {
    ...
    OP *new_op =NULL;
    if(!new_op){
      new_op = Mk_OP (newtop, rtn, op2tn,op1tn );
    }
  ...
  if ( oldtop == TOP_addu || oldtop == TOP_or || oldtop == TOP_subu ) {
    if (tn_registers_identical(op2tn, Zero_TN)) {
      ...
      new_op = Mk_OP (TOP_mv16, rtn, op1tn );
      Replace_CurOP(new_op);
      return TRUE;
    }
      ...
      new_op = Mk_OP (TOP_mv16, rtn, op2tn );
      Replace_CurOP(new_op);
      return TRUE;
    }
 ...
 if (OP_results(_cur_op) > 1 || OP_opnds(_cur_op) > 2) {
  ...
  if (op2value == 0) {
    ...
    if (!new_op) {
     ...
     OP *new_op = NULL;
     if (!new_op) {
	TN *tmp = Gen_Literal_TN((op2value>>2), 4);
       new_op = Mk_OP (TOP_add16_sp, rtn, op1tn, tmp);

       if (Op_ENTRYINFO_sp_adj(_cur_op)) {
	   Set_ENTRYINFO_sp_adj(_cur_op, new_op);
       } else if (Op_EXITINFO_sp_adj(_cur_op)) {
          Set_EXITINFO_sp_adj(_cur_op, new_op);
       }
     ...
     if(!new_op){
       ...
       new_op = Mk_OP (TOP_sub16_i, rtn, op1tn, tmp);
       Replace_CurOP(new_op);
     }
     return TRUE;
  }
...
Repl_Size16_Instr::Replace_Restricted_Imm_OP () {
  ...
  if ( oldtop == TOP_ori ) {
    if ( !TN_is_symbol(op2tn) && (op2value== 0) ) {
      if (rtn == op1tn) {
	 Is_True(0, ("or.i dest eq op1 && op2 is 0"));
      }
