...
Expand_Multiply (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (CG_sl2) {
    ...
    Build_OP(top, hi_tn, result, src1, src2, zero_tn, Zero_TN, zero_tn, ops);	
  } else {
...
void Expand_MulShift_Hi (TN *result, TN *src1, TN *src2, TN *src3, TYPE_ID mtype, OPS *ops)
{
  ...
  Build_OP(TOP_c3_muls, HI_TN, low_tn, src1, src2, src3, ops);
  Build_OP(TOP_c3_mvfs, result, HI_TN,  zero_tn, ops);
}
...
TN *Build_C3_Intrinsic_OP( INTRINSIC id, WN* intrncall, OPS *ops, TN* result ) 
 {
	 ...
	 case INTRN_MPY_32_16:
         ...
         case INTRN_C3_MULAD:
	     return Expand_C3_MULA(intrncall, result, ops, FALSE);
	 case INTRN_C3_MVFS:
     ...
     OPS* ops )
{
  ...
  Build_MC_OP(mc_op, dest_tn, cond_tn, true_tn, unsignedflag, ops,  OP_ALWAYS_COND_DEF);
  Build_MC_OP(op2, dest_tn, cond_tn, false_tn, unsignedflag, ops, OP_ALWAYS_COND_DEF);	 
  return;
}
       ...
       TN *result, TN *true_tn, TN *false_tn, TN *cmp_kid1, TN *cmp_kid2,  OPS *ops)
{
  ...
  int unsignedflag = (select_opc == OPC_U4SELECT) ? 1 : 0;
  ...
  if (tns_are_equals (true_tn, false_tn)) {
    ...
    if (!kid2_is_zero) {
      ...
      unsignedflag = MTYPE_signed(OPCODE_rtype(compare_opc)) ? 0 : 1;	  
    }
...
Analyze_SL2_Intrinsic( INTRINSIC id, WN* intrncall ) {
   ...
   switch (id) {
     case  INTRN_C2_MVGR_R2G:  //mode: opnds[2]   sign:opnds[3]
      ...
      break;
     case  INTRN_C2_MVGR_G2R: // op_mode:opnds[3]   dir:opnds[4]   sign:opnds[5]   size:opnds[6]  immed_bank:opnds[7]
      ...
      break;
     case  INTRN_C2_MVGR_G2S:
          return TOP_c2_mvgr_g2s;
      break;
     case  INTRN_C2_MVGR_S2G:
          return TOP_c2_mvgr_s2g;
      break;
     ...
     case  INTRN_C2_LD_G:
      ...
      break;
     case  INTRN_C2_LD_S:
        if(opnds[4]==2 && opnds[3]==1 && opnds[2]==1)
          return TOP_c2_ld_s2s_ga_p;
        else  if(opnds[4]==2 && opnds[3]==1 && opnds[2]==0)
          return TOP_c2_ld_s2s_ga;
        else  if(opnds[4]==3 && opnds[3]==1 && opnds[2]==1)
          return TOP_c2_ld_s2s_sa_p;
        else  if(opnds[4]==3 && opnds[3]==1 && opnds[2]==0)
          return TOP_c2_ld_s2s_sa;
        else { 
          FmtAssert(FALSE, ("invalid parameter in function c2_ld_s"));
        }
      break;
     case  INTRN_C2_ST_V:
     ...
     case  INTRN_C2_MMUL_S123:
          return TOP_c2_mmul_s123;
      break;
     case  INTRN_C2_MMUL:
        if(opnds[3]==0) // size: opnds[3] 
	 ...
	 switch(opnds[2]) 
      ...
      break;
     case  INTRN_C2_MVSEL:
          if( opnds[2] == 0 || opnds[2] == 2 )
            return TOP_c2_mvsel_zero_two;
          else 
            return TOP_c2_mvsel;
      break;
     case  INTRN_C2_BCST:
     ...
     case  INTRN_C2_ADDS_R:	    
        if(opnds[4]==1 && opnds[3]==0 && opnds[8] == 1) // immed: opnds[4]   size: opnds[3]  dir: opnds[8]
     ...
     case  INTRN_C2_MULS:
        return TOP_c2_muls;
     case  INTRN_C2_MULS_R:
        if(opnds[3]==0)
          return TOP_c2_muls_r_h;
        else  if(opnds[3]==1)
          return TOP_c2_muls_r_w;
        else { 
          FmtAssert(FALSE, ("invalid parameter in function c2_muls"));
        }
      break;
     case  INTRN_C2_MADS:
          return TOP_c2_mads;
     ...
     case INTRN_C2_MVGR_R2S:
	  return  TOP_c2_mvgr_r2s;
      case INTRN_C2_GSUMS:
	  return TOP_c2_gsums;
      case INTRN_C2_WRAP:
	  return TOP_c2_wrap;
      case INTRN_C2_CLZOB:
        if(opnds[2] == 0) //immed 
        {
          switch(opnds[3])
          {
            ...
            case 1:
              return TOP_c2_clzob_za;
            case 2:
              return TOP_c2_clzob_od;
            case 3:
              return TOP_c2_clzob_oa;
            default:
        ...
        else if(opnds[2] == 1) 
        {
          switch(opnds[3])
          {
            ...
            case 1:
              return TOP_c2_clzob_za_i;
            case 2:
              return TOP_c2_clzob_od_i;
            case 3:
              return TOP_c2_clzob_oa_i;
            default:
              FmtAssert(FALSE, ("invalid parameter in function c2_clzob"));	 
          };
        }
	...
	switch(opnds[0])
      ...
      case INTRN_C2_JOINT:
	  return TOP_c2_joint;
      case INTRN_C2_THREAD_MAJOR:
	  return TOP_c2_thread_major;
      case INTRN_C2_THREAD_MINOR:
	  return TOP_c2_thread_minor;
...
#ifdef TARG_SL
/* Get Build_OP_TYPE through topcode get from Analyze_SL_Intrinsic 
 * we need first get operands info indexed by topcode and return corresponding
 * BUILD_OP_TYPE */
BUILD_OP_TYPE
Get_Build_OP_Type( TOP topcode ) {
    SL_INTRN_INFO * info = SL_Intrn_Info(topcode);
    SL_INTRN_OPNDS_INFO * opnds_info = SL_Opnds_Info(info);
    return SL_INTRN_OPNDS_INFO_Build_Type(opnds_info);
}
/*
...
BOOL
SL2_Intrinsic_Need_Return(  INTRINSIC id) {
     switch(id) {  
...
BOOL
SL2_Intrinsic_Two_Results( INTRINSIC id ) {
  switch(id) {
    case INTRN_C2_MVSEL:
    case INTRN_C2_VLCS:
      return TRUE;
...
INT Build_Multi_Mode_Ldst( OP* op, OPS *ops )
{
   ...
   * operands.
   */
  if( TOP_is_c2_load(opcode) ){
    tn_c6 = OP_opnd( op, 1 );
  }else if( TOP_is_c2_store(opcode) ){
    tn_c6 = OP_opnd( op, 2 );
  }else{
    Is_True( 0, ("unknow instructions in Build_Multi_Mode_Ldst") );
  }
...
Build_LUT_Insn( OP* op )
{
  ...
  if( OP_code(op) == TOP_c2_vspel_mul_h ||
    ...
    tn = OP_opnd( op, 1 );
  } else if ( OP_code(op) == TOP_c2_vspel_mac_h ||
              OP_code(op) == TOP_c2_vspel_mac_w  ||
              OP_code(op) == TOP_c2_vspel_adds ) {
    tn = OP_opnd( op, 2 );
  } else {
    Is_True(0, ("invalid op for Build_LUT_Insn") );
  }
...
Build_SL_Intrinsic_OP( INTRINSIC id, WN* intrncall, OPS *ops, TN* result ) 
{
    ...
    /* mfc2 and cfc2 have return value so we need result_tn to record 
     * the return value */ 
    TN* result_tn = NULL; 
    ...
    if(id == INTRN_VBUF_OFFSET || id == INTRN_SBUF_OFFSET) {
       return Expand_Expr(WN_kid(intrncall, 0), intrncall, result, id);
    }
    ...
    SL_INTRN_INFO * info = SL_Intrn_Info(opcode);           
 
    SL_INTRN_OPNDS_INFO * opnds_info = SL_Opnds_Info(info);

/* for mfc2_v / mfc2_s / cfc2, as we need to return value for these three intrinsic
 * we need keep a operand position for result_tn which used to return value for these
 * intrinsic. That's the reason why we need add one to index of opnds array. */    
   for(int i = 0; i < WN_kid_count(intrncall); i++) {
     if(Need_Relocation(opcode, i)) {
/* if we find current intrinsic call include relocation symbol variable
 * we need record what relocation type ( %secrel_v or %secrel_s )
 * the symbol variable blongs to. 
 */
        rel_type = Get_TN_Rel_From_Opcode(opcode);
     }
     else {
       if( SL2_Intrinsic_Two_Results(id) ) {
         opnds[i+2] = WN_const_val(WN_kid0(WN_kid(intrncall, i)));
       } else if(SL2_Intrinsic_Need_Return(id)) {
         opnds[i+1] = WN_const_val(WN_kid0(WN_kid(intrncall, i)));
       } else {
         opnds[i] = WN_const_val(WN_kid0(WN_kid(intrncall, i)));
       }
     }
    ...
    /* opnds_num include some placeholder and actural opnds_info */
    UINT32 opnds_num = SL_INTRN_OPNDS_INFO_Num(opnds_info);

    BUILD_OP_TYPE build_type = Get_Build_OP_Type(opcode);
    
    OPNDS_ATTR_IDX* opnds_idx_array = SL_INTRN_OPNDS_INFO_Array(opnds_info);

    bool vlcs_int_reg_result = false;
    bool movpat_reg_result = false;
    bool ld_v_sw_got_c5 = false;
    bool sum4_skip_for_c2acc = false;
    for ( int i = 0 ; i < opnds_num; i++) {
       OPNDS_ATTR_IDX opnd_info_idx = opnds_idx_array[i];
          SL_OPND_INFO* opnd_info = SL_Opnd_Info(opnd_info_idx);
          OPND_ATTR_TYPE type = SL_OPND_INFO_Type(opnd_info);
          UINT32 pos = SL_OPND_INFO_Pos(opnd_info); 
          ST* st,  *base_st; INT64 base_ofst, ofst;
          switch(type) {
            case attr_undef:
          	  continue;
            case opnd_is_lit:
              if( sum4_skip_for_c2acc ) { 
                arg[pos] = Gen_Literal_TN(opnds[i-1], 4);
              } else {
                arg[pos] = Gen_Literal_TN(opnds[i], 4);
              }
              break;
            case opnd_is_cop_creg:
	       if( id ==INTRN_C2_THREAD_MAJOR || 
                   id == INTRN_C2_THREAD_MINOR) {
                  arg[pos] = Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_creg, 1, 0);
               } else if ( id == INTRN_C2_SUM4_R ||
                           ( id == INTRN_C2_MOV_R &&  
                             ( opnds[4]==3 || opnds[4]==1 ))) {
                  /* see c2 scheduling_rules.doc, it's about sum
                   * acc control register dependency
                   */
                  arg[pos] = Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_creg, 5, 0);	
               } else if ( id == INTRN_C2_LD_V && opnds[2] == 1) {
                 /* c2.ld.v.sw , access control register #5 and #6 */
                 if( !ld_v_sw_got_c5 ) {
                   arg[pos] = Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_creg, 6, 0);	
                   ld_v_sw_got_c5 = true;
                 } else {
                   arg[pos] = Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_creg, 7, 0);	
                 }  
               } else if( id == INTRN_C2_LD_V || id == INTRN_C2_ST_V ){
                 /* all c2.ld.v except .sw, access only ctrl reg #6  */
                 arg[pos] = Build_Dedicated_TN(
                               ISA_REGISTER_CLASS_cop_creg, 7, 0);
               } else if( id == INTRN_C2_SUBS_R ||
                          id == INTRN_C2_ADDS_R ||
                          id == INTRN_C2_SCOND_R ||
                          id == INTRN_C2_SCOND_R_WB ){
                 /* all c2_subs_r access only ctrl reg #2  */
                 arg[pos] = Build_Dedicated_TN(
                               ISA_REGISTER_CLASS_cop_creg, 3, 0);
               } else if( opcode == TOP_c2_mvsel_zero_two ){ 
                 /* opmode 0 or 2 of mvsel access ctrl reg #3  */
                 arg[pos] = Build_Dedicated_TN(
                               ISA_REGISTER_CLASS_cop_creg, 4, 0);
               } else {
                 arg[pos] = Build_Dedicated_TN(
               ...
               break;
            case opnd_is_cop_breg:
              arg[pos] = Build_Dedicated_TN(ISA_REGISTER_CLASS_cop_breg, opnds[i]+1, 0);
              break;
            case opnd_is_cop_vreg:
              if( vlcs_int_reg_result ){
                arg[pos] = Build_Dedicated_TN(ISA_REGISTER_CLASS_cop_vreg, opnds[i-1]+1, 0);
              } else if ( sum4_skip_for_c2acc ) {
                arg[pos] = Build_Dedicated_TN(ISA_REGISTER_CLASS_cop_vreg, opnds[i-1]+1, 0);
              } else {
                arg[pos] = Build_Dedicated_TN(ISA_REGISTER_CLASS_cop_vreg, opnds[i]+1, 0);
              }
              break;
            case opnd_is_cop_sreg:
              arg[pos] = Build_Dedicated_TN(ISA_REGISTER_CLASS_cop_sreg, opnds[i]+1, 0);
              break;
            case opnd_is_integer_reg:
              arg[pos] = Gen_Register_TN(ISA_REGISTER_CLASS_integer, 4);
              break;
            case opnd_is_expr:
               if( SL2_Intrinsic_Two_Results(id) ) {
                 arg[pos] = Expand_Expr(WN_kid(intrncall, i-2), 
                                        intrncall, NULL, id);
               } else if(SL2_Intrinsic_Need_Return(id)) {
                 arg[pos] = Expand_Expr(WN_kid(intrncall, i-1), 
                                        intrncall, NULL, id);
               } else if( movpat_reg_result && id==INTRN_C2_LD_V ) {
                 arg[pos] = Expand_Expr(WN_kid(intrncall, i-1), 
                                       intrncall, NULL, id);
               } else {
                 arg[pos] = Expand_Expr(WN_kid(intrncall, i), 
                                        intrncall, NULL, id);
              }
              break;
            case opnd_is_sym:
              if( SL2_Intrinsic_Two_Results(id) ) {
                st = WN_st(WN_kid0(WN_kid(intrncall, i-2)));
                ofst = WN_lda_offset(WN_kid0(WN_kid(intrncall,i-2)));
              } else if(SL2_Intrinsic_Need_Return(id)) {
                st = WN_st(WN_kid0(WN_kid(intrncall, i-1)));
                ofst = WN_lda_offset(WN_kid0(WN_kid(intrncall,i-1)));
              } else {
                st = WN_st(WN_kid0(WN_kid(intrncall, i)));
                ofst = WN_lda_offset(WN_kid0(WN_kid(intrncall,i)));
              }
              Is_True(st,  ("cannot calculate address including a variable at line %d\n",  WN_linenum(intrncall))); 
              arg[pos] = Gen_Symbol_TN(
                           st,
                           ofst,
                           rel_type);
              break;
#ifdef TARG_SL2
            case opnd_is_c2acc:
              arg[pos] = Build_Dedicated_TN(ISA_REGISTER_CLASS_c2accum, REGISTER_c2acc, 0);
              /* 3 c2.sum4 instructions need to skip back one 
               * position for opnd after result c2acc.
               */
              if( opcode == TOP_c2_sum4_r ||
                  opcode == TOP_c2_sum4_g ||
                  opcode == TOP_c2_sum4_sw || 
                  opcode == TOP_c2_sum3_saddr) 
                sum4_skip_for_c2acc = true;
              else
                sum4_skip_for_c2acc = false;
              break;
            case opnd_is_c2acc_ctrl:
              arg[pos] = Build_Dedicated_TN(ISA_REGISTER_CLASS_c2acc_ctrl, REGISTER_c2acc_ctrl, 0);
              break;
            case opnd_is_c2mvsel:
              arg[pos] = Build_Dedicated_TN(ISA_REGISTER_CLASS_c2mvsel, REGISTER_c2mvsel, 0);
              break;
            case opnd_is_c2vlcs:
              arg[pos] = Build_Dedicated_TN(ISA_REGISTER_CLASS_c2vlcs, REGISTER_c2vlcs, 0);
              vlcs_int_reg_result = true;
              break;
            case opnd_is_c2movpat:
              arg[pos] = Build_Dedicated_TN(ISA_REGISTER_CLASS_c2movpat, REGISTER_c2movpat, 0);
              if( id == INTRN_C2_LD_V )
                movpat_reg_result = true;
              break;
#endif
            default:
    ...
    TN* tn_copc;
    tn_copc = Gen_Register_TN(ISA_REGISTER_CLASS_copc, 1);
    ...
    switch(build_type) {
      case BUILD_OP_0:
        ...
        break;
      case BUILD_OP_1:
      	Build_OP(opcode, arg[0], ops);
      	break;
      case BUILD_OP_2:
        if( opcode == TOP_c2_vabs_w || opcode == TOP_c2_vabs_p ) {
          TN* rd1_plus_1 =  Build_Dedicated_TN(
                              ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          TN* rs2_plus_1 =  Build_Dedicated_TN(
                              ISA_REGISTER_CLASS_cop_vreg, opnds[2]+1+1, 0 );
          Build_OP(opcode, arg[0], rd1_plus_1, arg[1], rs2_plus_1, ops );
        } else if( opcode == TOP_c2_vabs_w_sm || 
                   opcode == TOP_c2_vabs_p_sm ) {
          TN* rd1_plus_1 =  Build_Dedicated_TN(
                              ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          TN* rs2_plus_1 =  Build_Dedicated_TN(
                              ISA_REGISTER_CLASS_cop_vreg, opnds[2]+1+1, 0 );
          TN* tn_c0 = Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_creg, 1, 0);	
          Build_OP(opcode, arg[0], rd1_plus_1, arg[1], rs2_plus_1, tn_c0, ops );
        } else if ( opcode == TOP_c2_vabs_h_sm ){
          TN* tn_c0 = Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_creg, 1, 0);	
          Build_OP(opcode, arg[0], arg[1], tn_c0, ops );
        } else if ( opcode == TOP_c2_vclp_p ){
          TN* rd1_plus_1 = Build_Dedicated_TN(
                              ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          TN* rs1_plus_1 =  Build_Dedicated_TN(
                              ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
          Build_OP(opcode, arg[0], rd1_plus_1, arg[1], rs1_plus_1, ops );
        } else if ( id == INTRN_C2_LD_V_IMM ||
                    id == INTRN_C2_ST_V_IMM ){
          /* c2.ldi.v and c2.sti.v have both word-size-mode and multi
           * mode. How many operands is decided by a insn sequence, and
           * the computation can be done more easily in  dependency 
           * graph creation. So I just expand a temporary instruction
           * here, and leave the TN list creation in Compute_BB_Graph, 
           * please see details in cg_dep_graph.cxx
           *
           * Build_OP uses Mk_OP, which cannot work for variable 
           * operands.
           */
          TN* tn_c6 = Build_Dedicated_TN( 
                          ISA_REGISTER_CLASS_cop_creg, 7, 0);
          TN* rs1_plus_1 = Build_Dedicated_TN( 
          ...
          TN* rd1_plus_1 = Build_Dedicated_TN( 
                               ...
                               opnds[0]+1+1,
                               0);
          /* So far, multi mode load or store is treated as word size
           * because more operands is ok for TOP_is_var_opnds, but less
           * operands will cause error.
           * I don't want to seperate word size mode in the multi mode
           * groups, So I create one more operands for all multi mode
           */
          if( TOP_is_c2_multi_mode_load(opcode) ) {
            TN* src_tns[2];
            TN* dst_tns[2];
            ...
            src_tns[1] = tn_c6;
            dst_tns[0] = arg[0];
            dst_tns[1] = rd1_plus_1;
            OPS_Append_Op(ops, Mk_VarOP(opcode, 2, 2, dst_tns, src_tns) );
          } else if( TOP_is_c2_multi_mode_store(opcode) ) {
            TN* src_tns[2];
            TN* dst_tns[2];
            src_tns[0] = arg[0];
            src_tns[1] = arg[1];
            src_tns[2] = tn_c6;
            src_tns[3] = rs1_plus_1;
            OPS_Append_Op(ops, Mk_VarOP(opcode, 0, 4, dst_tns, src_tns) );
          } else {
            /* For non multi-mode instruction, they have word size mode
             * and non word size mode
             */
            if( opcode == TOP_c2_sti_v_w ) {
              Build_OP(opcode, arg[0], arg[1], tn_c6, rs1_plus_1, ops);
            } else if ( opcode == TOP_c2_ldi_v_w ) {
              Build_OP(opcode, arg[0], rd1_plus_1, arg[1], tn_c6, ops);
            } else {
              Build_OP(opcode, arg[0], arg[1], tn_c6, ops);
            } 
          }
        } else if ( opcode == TOP_c2_mov_g ){
          TN* c2acc = Build_Dedicated_TN(ISA_REGISTER_CLASS_c2accum, 
                                         REGISTER_c2acc, 0);
          Build_OP(opcode, arg[0], c2acc, arg[1], ops);
        } else 
          Build_OP(opcode, arg[0], arg[1], ops);
        break;
      case BUILD_OP_3:
        if( id == INTRN_C2_SCOND_R ){ 
          if( opcode == TOP_c2_scond_r_w_eq ||
              opcode == TOP_c2_scond_r_w_lt ||
              opcode == TOP_c2_scond_r_w_le ||
              opcode == TOP_c2_scond_r_w_gt ||
              opcode == TOP_c2_scond_r_w_ge ) {
            /* word size and non-immed mode, needs two more source opnds*/
            TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
            TN* rs2_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
            Build_OP(opcode, tn_copc, arg[0], arg[1], arg[2], 
                     rs1_plus_1, rs2_plus_1, ops); 

          } else if ( opcode == TOP_c2_scond_r_w_eq_i ||
                      opcode == TOP_c2_scond_r_w_lt_i ||
                      opcode == TOP_c2_scond_r_w_le_i ||
                      opcode == TOP_c2_scond_r_w_gt_i ||
                      opcode == TOP_c2_scond_r_w_ge_i ) {
            /* word size and immed mode, needs one more source opnds*/
            TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
            Build_OP(opcode, tn_copc, arg[0], arg[1], arg[2], rs1_plus_1, ops); 
          } else {
            Build_OP(opcode, tn_copc, arg[0], arg[1], arg[2], ops); 
          }
        } else if(Is_SL2_Scond_With_Scalar_Destination(opcode)) {
          Build_OP(opcode, arg[0], tn_copc, arg[1], arg[2], ops); 
        } else if( opcode == TOP_c2_vshr_p ||
                   opcode == TOP_c2_vshl_p ||
                   opcode == TOP_c2_vshr_w ||
                   opcode == TOP_c2_vshl_w ){
          TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
          Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], rs1_plus_1, ops); 
        } else if( opcode == TOP_c2_vneg_p ||
                   opcode == TOP_c2_vneg_w ){
          TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
          TN* rs2_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[2]+1+1, 0 );
          Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], rs1_plus_1,
                   rs2_plus_1, ops); 
        } else if( opcode == TOP_c2_vclg_p_lt_and ||
                   opcode == TOP_c2_vclg_p_lt_or  ||
                   opcode == TOP_c2_vclg_p_le_and  ||
                   opcode == TOP_c2_vclg_p_le_or  ||
                   opcode == TOP_c2_vclg_p_eq_and  ||
                   opcode == TOP_c2_vclg_p_eq_or  ||
                   opcode == TOP_c2_vclg_p_ge_and  ||
                   opcode == TOP_c2_vclg_p_ge_or  ||
                   opcode == TOP_c2_vclg_p_gt_and  ||
                   opcode == TOP_c2_vclg_p_gt_or  ||
                   opcode == TOP_c2_vclg_p_and  ||
                   opcode == TOP_c2_vclg_p_or  ||
                   opcode == TOP_c2_vclg_p_le  ||
                   opcode == TOP_c2_vclg_p_eq  ||
                   opcode == TOP_c2_vclg_p_ge  ||
                   opcode == TOP_c2_vclg_p_gt ){
          TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
          ...
          Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], rs1_plus_1,
                   rs2_plus_1, ops); 
        } else if( opcode == TOP_c2_vclg_w_lt_and ||
                   opcode == TOP_c2_vclg_w_lt_or  ||
                   opcode == TOP_c2_vclg_w_le_and  ||
                   opcode == TOP_c2_vclg_w_le_or  ||
                   opcode == TOP_c2_vclg_w_eq_and  ||
                   opcode == TOP_c2_vclg_w_eq_or  ||
                   opcode == TOP_c2_vclg_w_ge_and  ||
                   opcode == TOP_c2_vclg_w_ge_or  ||
                   opcode == TOP_c2_vclg_w_gt_and  ||
                   opcode == TOP_c2_vclg_w_gt_or  ||
                   opcode == TOP_c2_vclg_w_and  ||
                   opcode == TOP_c2_vclg_w_or  ||
                   opcode == TOP_c2_vclg_w_le  ||
                   opcode == TOP_c2_vclg_w_lt  ||
                   opcode == TOP_c2_vclg_w_ge  ||
                   opcode == TOP_c2_vclg_w_gt  ||
                   opcode == TOP_c2_vcmpr_w_eq  ||
                   opcode == TOP_c2_vcmpr_w_lt  ||
                   opcode == TOP_c2_vcmpr_w_le  ||
                   opcode == TOP_c2_vcmpr_w_gt  ||
                   opcode == TOP_c2_vcmpr_w_ge  ){
          TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
          TN* rs2_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[2]+1+1, 0 );
          Build_OP(opcode, arg[0], arg[1], arg[2], rs1_plus_1, rs2_plus_1, ops); 
        } else if( opcode == TOP_c2_vmul_w ||
                   opcode == TOP_c2_ld_v_w ||
                   opcode == TOP_c2_vspel_mul_w ){
          TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], ops);
        } else if( opcode == TOP_c2_st_v_w ){
          TN* rs1_plus_1 =  Build_Dedicated_TN( 
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          Build_OP(opcode, arg[0], arg[1], arg[2], rs1_plus_1, ops);
        } else if( opcode == TOP_c2_vrnd_w ){
          TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
          Build_OP(opcode, arg[0], arg[1], arg[2], rs1_plus_1, ops);
        } else if( opcode == TOP_c2_vcmov_w_f ||
                   opcode == TOP_c2_vcmov_w_t ){
          TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          TN* rs2_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[2]+1+1, 0 );
          Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], rs2_plus_1, ops);
        } else if ( opcode == TOP_c2_vclp_2 ) {
          /* c2.vclp.2 uses its destination as one of its source */
          Build_OP(opcode, arg[0], arg[1], arg[2], arg[0], ops );
        } else if ( opcode == TOP_c2_mov_r ){
          TN* c2acc = Build_Dedicated_TN(ISA_REGISTER_CLASS_c2accum, 
                                         REGISTER_c2acc, 0);
          Build_OP(opcode, arg[0], c2acc, arg[1], arg[2], ops);
        } else {
          Build_OP(opcode, arg[0], arg[1], arg[2], ops);
        }
        break;
      case BUILD_OP_4:
        if(id==INTRN_C2_BDEP) {
         if( TN_is_zero(arg[0]))  {
           tmp = Build_TN_Like(arg[0]);
           Build_OP(TOP_add,  tmp,  Zero_TN,  arg[0],  ops);
           arg[0] = tmp;			   
          }		 	
	   Build_OP(opcode, arg[0],  arg[0],  arg[1], arg[2], arg[3], ops);
	 }
        else if( id == INTRN_C2_SCOND_R_WB ) {
          if( opcode == TOP_c2_scond_r_w_wb_eq ||
              opcode == TOP_c2_scond_r_w_wb_lt ||
              opcode == TOP_c2_scond_r_w_wb_le ||
              opcode == TOP_c2_scond_r_w_wb_gt ||
              opcode == TOP_c2_scond_r_w_wb_ge ) {
            /* word size and non-immed mode, needs two more source opnds*/
            TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
            TN* rs2_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[2]+1+1, 0 );
            Build_OP(opcode, arg[0], tn_copc, arg[1], arg[2], arg[3], 
                     rs1_plus_1, rs2_plus_1, ops); 

          } else if ( opcode == TOP_c2_scond_r_w_wb_eq_i ||
                      opcode == TOP_c2_scond_r_w_wb_lt_i ||
                      opcode == TOP_c2_scond_r_w_wb_le_i ||
                      opcode == TOP_c2_scond_r_w_wb_gt_i ||
                      opcode == TOP_c2_scond_r_w_wb_ge_i ) {
            /* word size and immed mode, needs one more source opnds*/
            TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
            Build_OP(opcode, arg[0], tn_copc, arg[1], arg[2], arg[3], 
            ...
            Build_OP(opcode, arg[0], tn_copc, arg[1], arg[2], arg[3], ops); 
          }
        }
        else if( id==INTRN_C2_SAD) 
          Build_OP(opcode, arg[1], arg[0], arg[2], arg[3], ops);
        else if( opcode == TOP_c2_vspas  ||
                 opcode == TOP_c2_vspel_mac_w ){
          TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
          Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], 
                   arg[3], rs1_plus_1, ops);
        } else if ( opcode == TOP_c2_sub_r_abs_w_i ) {
          /* needs one more result, and one more source opnd */
          TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
          Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], 
                   arg[3], rs1_plus_1, ops);
        } else if ( opcode == TOP_c2_sub_r_abs_w ) {
          /* needs one more result, and two more source opnd */
          TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
          TN* rs2_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[2]+1+1, 0 );
          Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], 
                   arg[3], rs1_plus_1, rs2_plus_1, ops);
        } else if(opcode == TOP_c2_vlcs_dc || opcode == TOP_c2_vlcs_ac) {
          TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], 
                   arg[3], ops);
        }
	 else if ( opcode == TOP_c2_mov_s_i ||
                    opcode == TOP_c2_mov_s ) {
          TN* c2acc = Build_Dedicated_TN(ISA_REGISTER_CLASS_c2accum, 
                                         REGISTER_c2acc, 0);
          Build_OP(opcode, arg[0], c2acc, arg[1], arg[2], arg[3], ops);
        } else 
          Build_OP(opcode, arg[0], arg[1], arg[2], arg[3], ops);
      	break;
      case BUILD_OP_5:
	 if(id==INTRN_C2_BDEP) {
           if( TN_is_zero(arg[0]))  {
	    ...
	    Build_OP(opcode, arg[0],  arg[0],  arg[1], arg[2], arg[3], ops);
	 } else if ( opcode == TOP_c2_vsubs_w || 
                     opcode == TOP_c2_vsubs_w_abs ||
                     opcode == TOP_c2_vsubs_p ||
                     opcode == TOP_c2_vsubs_p_abs ) {
           TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
           TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
           TN* rs2_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[2]+1+1, 0 );
           Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], arg[3], arg[4], 
                    rs1_plus_1, rs2_plus_1, ops );
	 } else if ( opcode == TOP_c2_vsubs_w_sm || 
                     opcode == TOP_c2_vsubs_w_abs_sm ||
                     opcode == TOP_c2_vsubs_p_sm ||
                     opcode == TOP_c2_vsubs_p_abs_sm ) {
           TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
           TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
           ...
           TN* tn_c0 = Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_creg, 1, 0);	
           Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], arg[3], arg[4], 
                    rs1_plus_1, rs2_plus_1, tn_c0, ops );
         } else if ( opcode == TOP_c2_vsubs_h_sm ||
                     opcode == TOP_c2_vsubs_h_abs_sm ){
           TN* tn_c0 = Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_creg, 1, 0);	
           Build_OP(opcode, arg[0], arg[1], arg[2], arg[3], arg[4], tn_c0, ops );
         } else if ( opcode == TOP_c2_vadds_w ||
                     opcode == TOP_c2_vadds_p ){
           TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
           TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
           ...
           Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], arg[3], arg[4], 
                    rs1_plus_1, rs2_plus_1, ops );
         } else if ( opcode == TOP_c2_vadds_w_c ||
                     opcode == TOP_c2_vadds_p_c ){
           TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
           TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
           ...
           if( (opnds[6]&6) == 6 )
             ...
             Is_True( 0, ("invalid shift mode of c2_vadds") );
           TN* tn_ctrl = Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_creg, ctrl_id, 0);	
           Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], arg[3], arg[4], 
                    rs1_plus_1, rs2_plus_1, tn_ctrl, ops );
         } else if ( opcode == TOP_c2_vadds_h_c ){
           int ctrl_id;
           if( (opnds[6]&6) == 6 )
             ctrl_id = 2;
           else if( (opnds[6]&6) == 2 )
           ...
           TN* tn_ctrl = Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_creg, ctrl_id, 0);	
           Build_OP(opcode, arg[0], arg[1], arg[2], arg[3], arg[4], tn_ctrl, ops);
         } else if ( opcode == TOP_c2_subs_r_w_i ) {
           /* needs one more result, and one more source opnd */
           TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
           TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
           Build_OP(opcode, arg[0], rd1_plus_1, 
                    arg[1], arg[2], arg[3], arg[4], rs1_plus_1, ops);
         } else if ( opcode == TOP_c2_subs_r_w ) {
           /* needs one more result, and two more source opnd */
           TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
           TN* rs1_plus_1 =  Build_Dedicated_TN(
           ...
           TN* rs2_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[2]+1+1, 0 );
           Build_OP(opcode, arg[0], rd1_plus_1, 
                    arg[1], arg[2], arg[3], arg[4], rs1_plus_1, rs2_plus_1, ops);
         } else if ( opcode == TOP_c2_satd ){
           /* rd1 may also be used as an operand , rs2 may be written */
           TN* rs_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1, 0 );
           Build_OP(opcode, arg[1], arg[0], arg[3], 
                            arg[2], arg[3], arg[4], rs_plus_1, ops);
         } else {
           Build_OP(opcode, arg[0], arg[1], arg[2], arg[3], arg[4], ops);
         }
        break;
      case BUILD_OP_6:
        if (opcode == TOP_c2_muls) {
          TN* hi_tn = Build_Dedicated_TN(ISA_REGISTER_CLASS_cop_creg, 8, 0);
          Build_OP(opcode, hi_tn, arg[0], arg[1], arg[2], arg[3], arg[4], arg[5], ops);
        } else {
          Build_OP(opcode, arg[0], arg[1], arg[2], arg[3], arg[4], arg[5], ops);
        }
        break;
      case BUILD_OP_7:
        if( opcode == TOP_c2_add_shl_r_w_i || 
            opcode == TOP_c2_add_shr_r_w_i ){
          TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
      	  Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], arg[3], 
                   arg[4], arg[5], arg[6], rs1_plus_1, ops);
        } else if ( opcode == TOP_c2_add_shl_r_w ||
                    opcode == TOP_c2_add_shr_r_w ){
          TN* rd1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[0]+1+1, 0 );
          TN* rs1_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[1]+1+1, 0 );
          TN* rs2_plus_1 =  Build_Dedicated_TN(
                                 ISA_REGISTER_CLASS_cop_vreg, opnds[2]+1+1, 0 );
      	  Build_OP(opcode, arg[0], rd1_plus_1, arg[1], arg[2], arg[3], 
                   arg[4], arg[5], arg[6], rs1_plus_1, rs2_plus_1, ops);
        } else {
      	  Build_OP(opcode, arg[0], arg[1], arg[2], arg[3], 
                   arg[4], arg[5], arg[6], ops);
        }
      	break;
      case BUILD_OP_8:
        Build_OP(opcode, arg[0], arg[1], arg[2], arg[3], arg[4], arg[5], arg[6], arg[7], ops);
        break;
      case BUILD_OP_9:
        Build_OP(opcode, arg[0], arg[1], arg[2], arg[3], arg[4], arg[5], arg[6], arg[7], arg[8], ops);
        break;
      default:
        FmtAssert(FALSE, ("unhandled build op "));
    }

    // For c2.ldi.v and c2.sti.v, they have both multimode
    // and word size mode, meaning containing extra opnds or results
    OP *op = OPS_last(ops);
    Is_True( OP_code(op) == opcode, ("opcode inconsistency") );
    INT row_count = -1;
    if( TOP_is_c2_multi_mode_load(OP_code(op)) ||
        TOP_is_c2_multi_mode_store(OP_code(op)) ){
        row_count = Build_Multi_Mode_Ldst(op, ops);
    }

    // for the following c2 instructions, it needs to access the LUT
    // to get their extra operands. There is a lut_index parameter
    // in the operand list of these opcode
    if( OP_code(op) == TOP_c2_vspel_mul_h ||
        OP_code(op) == TOP_c2_vspel_mul_w ||
        OP_code(op) == TOP_c2_vspel_adds  ||
    ...
    // store return value for such intrinsic which have return value;
#ifdef TARG_SL2
    if(SL2_Intrinsic_Need_Return(id)) {
        result_tn = arg[0];
    }
    switch(opcode) {
       case TOP_c2_bdep_l:
    ...
    if(id==INTRN_OP_C2_SCOND_BR ||id==INTRN_OP_C2_SCOND_BR_R) 
		 Set_OP_c2_br_op(OPS_last(ops));   

    if(id == INTRN_OP_C2_SCOND_BR) {
      return arg[2];
    }else if(id == INTRN_OP_C2_SCOND_BR_R){
      return arg[3];
    }else{
      return result_tn; 
    }
