...
{
  Fail_FmtAssertion (
    ("Is_Int_Output_Preg not applicable to PR targets"));
}
...
static BOOL Struct_Is_HFA ( const TY_IDX ty, Mtype_Return_Level level, TYPE_ID& hfa_mtype );
...
Get_Return_Info (TY_IDX rtype, Mtype_Return_Level level)
{
  ...
  switch (mtype) {
    ...
    case MTYPE_M:
      ...
      if (SIM_INFO.max_struct_result != 0) {
        ...
        if (size > 0 && 8 * size <= 2 * SIM_INFO.max_struct_result) {
          ...
          if (Struct_Is_HFA (rtype, level, hfa_mtype) && hfa_mtype != MTYPE_V &&
    ...
    default:
      info.count = 0;
      Fail_FmtAssertion ("Invalid return mtype %s encountered",(MTYPE_name(mtype)));
      break;
  } /* switch (mtype) */
