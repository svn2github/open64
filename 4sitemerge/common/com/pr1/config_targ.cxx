 ...
 * $Revision: 1.1 $
 * $Date: 2005/07/27 02:18:08 $
 * $Author: kevinlo $
 * $Source: /depot/CVSROOT/javi/src/sw/cmplr/common/com/pr1/config_targ.cxx,v $
 *
 *
 * Description:
 *
...
Prepare_Target ( void )
{
  ...
  if ( ABI_Name != NULL ) {
	...
	Use_32_Bit_Pointers = FALSE;
      ...
      if ( strcmp ( ABI_Name, "pr32" ) == 0 ) {
	Target_ABI = ABI_I32;
	isa_default = TARGET_ISA_PR;
	targ_default = TARGET_PR;
      } else {
  ...
  if ( Processor_Name != NULL ) {
    ...
    if ( strcasecmp ( Processor_Name, "sb1" ) == 0 ) {
      ...
      if ( strcasecmp ( Processor_Name, "r10000" ) == 0 ) {
	targ = TARGET_R10K;
	if ( strcasecmp ( Processor_Name, "pr1" ) == 0 ) {
	  targ = TARGET_PR;
	} else {
  ...
  Target_int64 = FALSE;
  Use_32_Bit_Pointers = ((Target_ABI == ABI_N32) || (Target_ABI == ABI_I32));
#if defined(FRONT_END_C) || defined(FRONT_END_CPLUSPLUS)
#ifndef KEY
...
Configure_Target ( void )
{
  ...
  if ( Use_32_Bit_Pointers ) {
    ...
    ErrMsg ( EC_Inv_TARG, "64_Bit_Pointers", ABI_Name );
    Pointer_Size = 8;
...
{
    if (Target_ABI == ABI_N32 || Target_ABI == ABI_I32) {
	 Pointer_Size = 4;
	 Pointer_Mtype  = WHIRL_Mtype_A_On ? MTYPE_A4 : MTYPE_U4;
	 Pointer_type   = Pointer_Mtype;
	 Pointer_Mtype2 = MTYPE_U4;
