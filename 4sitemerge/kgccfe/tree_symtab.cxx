...
Create_TY_For_Tree (tree type_tree, TY_IDX idx)
{
	...
	case COMPLEX_TYPE:
		switch (tsize) {
#ifdef KEY
		case 4: ErrMsg (EC_Unsupported_Type, "Complex short");
#endif
...
Create_ST_For_Tree (tree decl_node)
{
  ...
  if (DECL_NAME (decl_node)) {
    // handle cases like "int foo asm("myfoo") = 2;"
    if (TREE_CODE(decl_node) == VAR_DECL &&
	(TREE_STATIC(decl_node) || DECL_EXTERNAL(decl_node) ||
    ...
    case VAR_DECL:
      {
		...
		if (flag_no_common || DECL_SECTION_NAME(decl_node))
              	...
              	sclass = SCLASS_EXTERN;
#if !defined(TARG_PR) && !defined(TARG_MIPS)
              if(Use_Call_Shared_Link && ((Gp_Rel_Aggresive_Opt &&  sclass != SCLASS_EXTERN) || sclass == SCLASS_DGLOBAL)) 
#else
		if (1)
#endif
