			 ...
			 int   line)
{
...
#endif /* KEY */
    operand_constraint_array[i] = (char*)constraint_string;
    ++i;
  }
#ifdef KEY
  opnd_num_map[i] = -1;
  ...
  WN *asm_wn = WN_CreateAsm_Stmt (ninputs + 2,
				  (char*)TREE_STRING_POINTER (string));
  ...
  for (tail = clobbers; tail; tail = TREE_CHAIN (tail))
    {
      char *clobber_string = (char*)TREE_STRING_POINTER (TREE_VALUE (tail));
  ...
  for (tail = inputs;
	...
	if (addr_of_rvalue != NULL) {
      ...
      update_opnd_num(opnd_num_map, (char*)constraint_string);
#endif

      WN_kid (asm_wn, i) =
	WN_CreateAsm_Input( (char*)constraint_string, opnd_num, input_rvalue);
      ++i;
      ++opnd_num;
    }
	...
	if (plus_modifier)
	  {
	    ...
	    constraint_string = remove_plus_modifier((char*)constraint_string);
