...
#if defined(TARG_SL) && defined(__SL__) 
    typedef vector<INT32> value_type;
#else
...
#if defined(TARG_SL) && defined(__SL__) 
   INT32 right_taken_count;
   INT32 neither_taken_count;
#else
...
struct Loop_Profile {
#if defined(TARG_SL) && defined(__SL__) 
   INT32 invocation_count; // total times the loop is invoked
   INT32 total_trip_count; // total trip count from all invocations
   INT32 last_trip_count;  // trip count from last invocation
   INT32 min_trip_count;   // minimum trip count from previous invocations
   INT32 max_trip_count;   // maximum trip count from previous invocations
   INT32 num_zero_trips;   // Number of times the loop had a zero trip
#else
