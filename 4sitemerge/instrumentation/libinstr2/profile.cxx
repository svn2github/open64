...
PU_Profile_Handle:: PU_Profile_Handle(char *fname, char *pname, long current_pc, INT32 pusize, INT32 c_sum) {
	checksum = c_sum;
...
Profile_Switch_Init(PU_PROFILE_HANDLE pu_handle,
		    ...
		    INT32 num_case_values, INT32 *case_values)
#else
void
Profile_Switch_Init(PU_PROFILE_HANDLE pu_handle,
		    INT32 num_switches, INT32 *switch_num_targets,
...
Profile_Icall(PU_PROFILE_HANDLE pu_handle, INT32 icall_id, void * called_fun_address)
{
...
#if defined(TARG_SL) && defined(__SL__) 
  UINT32 value = (UINTPS)called_fun_address;
 #else
  UINT64 value = (UINTPS)called_fun_address;
       ...
       // clear_interval is the sum of _exec_counter of the middle two in the steady part.
        INT i, j;
 #if defined(TARG_SL) && defined(__SL__) 
        UINT32 clear_interval = ptnv->_counters[3] + ptnv->_counters[4]; 
 #else
        ...
        if (ptnv->_clear_counter >= clear_interval)
        {
        	...
        	//resort tnv
 #if defined(TARG_SL) && defined(__SL__) 
        	UINT32 tmpvalues[10], tmpcounters[10];
#else
...
#if defined(TARG_SL) && defined(__SL__) 
void Profile_Value( PU_PROFILE_HANDLE pu_handle, INT32 inst_id, INT32 value )
#else
...
#if defined(TARG_SL) && defined(__SL__) 
	const INT32 tmp_value = entry->value[j];
	const INT32 tmp_freq  = entry->freq[j];
#else
