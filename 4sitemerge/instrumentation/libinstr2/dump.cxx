...
namespace {
	...
	for (Loop_Profile_Vector::const_iterator first (src.begin ());
	  ...
	  INT32 zero_trips = first->num_zero_trips;
#else
...
#if defined(TARG_SL) && defined(__SL__) 
	  const INT32 positive = first->invocation_count - zero_trips;
	  const INT32 back = first->total_trip_count - first->invocation_count + zero_trips;
#else
