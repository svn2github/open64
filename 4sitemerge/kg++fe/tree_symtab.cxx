...
namespace {
  ...
  map<tree, TY_IDX     >     ty_idx_map;
  map<tree, ST*        >     st_map;
  map<tree, SYMTAB_IDX >     symtab_idx_map;
  map<tree, LABEL_IDX  >     label_idx_map;
  map<tree, ST*        >     string_st_map;
  map<tree, BOOL       >     bool_map;
  map<tree, INT32      >     field_id_map;
  map<tree, INT32      >     type_field_ids_used_map;
  map<tree, INT32      >     scope_number_map;
  map<tree, tree       >     label_scope_map;
  map<tree, DST_INFO_IDX>    decl_idx_map; 
  map<tree, DST_INFO_IDX>    decl_field_idx_map; 
  map<tree, DST_INFO_IDX>    decl_specification_idx_map; 
  map<tree, DST_INFO_IDX>    type_idx_map;
  map<tree, LABEL_IDX  >     handler_label_map;
  map<tree, DST_INFO_IDX>    abstract_root_map;
#ifdef KEY
  // Map PU to the PU-specific st_map.
  map<PU*, map<tree, ST*>* >     pu_map;
  // TRUE if ST is a decl that is being/already been expanded.
  map<tree, BOOL        >     expanded_decl_map;
  // TRUE if TREE is a DECL_FUNCTION whose PU should have PU_uplevel set.
  map<tree, BOOL >     func_PU_uplevel_map;
  map<tree, tree >	   parent_scope_map;
  ...
  map<ST*, INT32        >     weak_workaround_map;
#endif
}
...
set_DECL_ST(tree t, ST* st) {
  ...
  if (Current_scope != 0 &&
    ...
    if (Scope_tab[Current_scope].st != NULL) {
      ...
      map<PU*, map<tree, ST* >* >::iterator it =
	pu_map.find(pu);
      if (it == pu_map.end()) {
	// Create new PU-specific map.
	pu_map[pu] = new map<tree, ST* >;
      }
...
{ 
	map<tree, DST_INFO_IDX >::iterator it =
		decl_idx_map.find(t);
