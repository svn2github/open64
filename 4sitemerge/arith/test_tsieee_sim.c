...
main() {
	...
	file = fopen("results/tsieee_sim_data","r");
	if(file == NULL) {
		fprintf(stderr, "Could not open results/tsieee_sim_data\n");
		exit(0);
	}
	...
	while(fgets(line,256,file) != NULL) {
	  ...
	  if(func[0]=='C') {
	    if(func[1]=='X')
	      rsize=4;
	    else if(strcmp(func,"COS")!=0 && strcmp(func,"COSH")!=0)
	      rsize=2;
	    else 
	      rsize=1;
	  }
	  else if(func[0]=='X' && func[1] == 'D')
	    rsize=2;
	  ...
	  if(strncmp(func,"STRTO",5) == 0) {
	    ...
	    if(func[6] == 'I') {
	      rtype = AR_Int_46_S;
	      if(arg[0] & 0x80000000)
	      	arg[0] |= ((AR_HOST_SINT64)0xffffffff)<<32;
	      if(arg[1] & 0x80000000)
	      	arg[1] |= ((AR_HOST_SINT64)0xffffffff)<<32;
	    }
	    else if(func[6] == 'J')
	      rtype = AR_Int_64_S;
	    else if(func[6] == 'F')
	  ...
	  if(rsize > 1 && ((result[0]>>52)&0x7ff) == 0x7ff)
	     xor = result[0]^ans[0];
	  else
...
static char USMID [] = "\n%Z%%M%	%I%	%G% %U%\n";
static char rcsid [] = "$Id: test_tsieee_sim.c,v 1.1 2005/07/27 02:00:51 kevinlo Exp $";
