...
#else
    // For SL, we use slcc instead of gcc.
...
init_crt_paths (void)
{
	...
	char *tmp_name = create_temp_file_name("sl");
    char *slcc_name = concat_strings(get_phase_dir(P_ld), "/slcc");
	init_given_crt_path ("crtbegin.o", slcc_name, tmp_name);
	init_given_crt_path ("crt1.o", slcc_name, tmp_name);
#endif
}
...
#else
	  //??	case o_WlC:
#endif // 0 - mostly SGI specific stuff - sc
		return TRUE;
	default:
		return FALSE;
	}
...
static struct prof_lib prof_libs[] = {
    ...
    { "pscrt", 1 },
#else
    { OPEN64_CMPLR_NAME_PREFIX "fortran", 1 },
    { "crt", 1 },
#endif
    { NULL, 0 },
};
	...
	case O_l:
		if (strcmp(arg, "m") == 0 ||
		  ...
		  if (xpg_flag && invoked_lang == L_f77) {
		    ...
		    add_library(lib_objects, "m" OPEN64_CMPLR_NAME_PREFIX);
		  } else {
#if !defined(TARG_SL)
		    add_library(objects, "mv");
#endif
		    add_library(objects, "m" OPEN64_CMPLR_NAME_PREFIX);
		  }
		}
		else {
		  // non libm
		...
		if (invoked_lang == L_CC) {
		  ...
		  add_library(cxx_prelinker_objects, "m" OPEN64_CMPLR_NAME_PREFIX);
		}
#endif
...
find_crt_path (char *crtname)
{
	...
	if (option_was_seen(O_nostdlib) || option_was_seen(O_L)) {
		...
		if (use_h264libs == TRUE)
		  sprintf(buf, "%s/%s", get_phase_dir(P_sl2startup), crtname);	
		else	
