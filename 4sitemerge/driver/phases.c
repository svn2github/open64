	...
	FOREACH_OPTION_SEEN(flag) {
		FOREACH_IMPLIED_OPTION(iflag, flag) {
			    ...
			    && !is_object_option(iflag) )
			{
			    ...
			    {
				add_implied_string (phase_list, iflag, flag, phase);
			    }
			}
		}
	}
...
#else
set_library_paths(string_list_t *args, boolean native)
{
  char *root_prefix;
#if defined(TARG_SL)
...
#if !defined(TARG_SL)
		asprintf(&our_path, "%s/lib/" OPEN64_VERSION "/32",root_prefix);
#else
                if (use_h264libs == TRUE) {
		  asprintf(&gcc_v_path,
			 "%s/libsl2/gcc-lib/" OPEN64_TARGET "/"
			 OPEN64_GCC_VERSION "/32",
			 root_prefix);
		  asprintf(&gcc_path, "%s/libsl2/gcc-lib/" OPEN64_TARGET "/lib",
			 root_prefix);
		  asprintf(&lib_path, "%s/" OPEN64_TARGET "/libsl2/", root_prefix);			
                  asprintf(&our_path, "%s/usr/libsl2/",root_prefix);
                }				  
                else {
		  asprintf(&gcc_v_path,
			 "%s/lib/gcc-lib/" OPEN64_TARGET "/"
			 OPEN64_GCC_VERSION "/32",
			 root_prefix);
         	  asprintf(&gcc_path, "%s/lib/gcc-lib/" OPEN64_TARGET "/lib",
			 root_prefix);
	          asprintf(&lib_path, "%s/" OPEN64_TARGET "/lib/", root_prefix);			
		  asprintf(&our_path, "%s/usr/lib/",root_prefix);
                }
#endif
	} else {
		asprintf(&gcc_v_path,
			 "%s/lib/gcc-lib/" OPEN64_TARGET "/" OPEN64_GCC_VERSION,
			 root_prefix);
		asprintf(&gcc_path, "%s/lib/gcc-lib/" OPEN64_TARGET "/lib64",
			 root_prefix);
		asprintf(&lib_path, "%s/" OPEN64_TARGET "/lib64", root_prefix);
		asprintf(&our_path, "%s/lib/" OPEN64_VERSION, root_prefix);
#if defined(TARG_SL)
		strcpy(sl_crt, our_path);
		assert(strlen(our_path) > 1027);
#endif
	}
	
	add_string(args, concat_strings("-L", our_path));
	add_string(args, concat_strings("-L", gcc_v_path));
	add_string(args, concat_strings("-L", gcc_path));

#if !defined(TARG_SL)
	if (!option_was_seen(O_nostdlib)) {
		add_string(args, concat_strings("-Wl,-rpath-link,",
						gcc_v_path));
		add_string(args, concat_strings("-Wl,-rpath-link,",
						gcc_path));
		add_string(args, concat_strings("-Wl,-rpath-link,",
						lib_path));
		add_string(args, concat_strings("-Wl,-rpath,", gcc_v_path));
		add_string(args, concat_strings("-Wl,-rpath,", gcc_path));
	}
#endif
	
	if (native) {
		add_string(args, concat_strings("-L", lib_path));
		if (abi == ABI_N32) {
			add_string(args, "-L/lib");
			add_string(args, "-L/usr/lib");
		} else {
			add_string(args, "-L/lib64");
			add_string(args, "-L/usr/lib64");
		}
	}
...
add_file_args (string_list_t *args, phases_t index)
{
...
#ifndef TARG_IA64
		add_string(args,
			   concat_strings("-I", get_phase_dir (P_include)));
#endif
#endif

#ifdef KEY
		if (target_is_native() && !option_was_seen(O_nostdinc)) {
#if defined(PATHSCALE)
...
#else
                  sprintf (buf, "-include=%s/include/" OPEN64_VERSION, root);
#endif
                  add_string(args, buf);
                }
...
#if 0 //  dsm not supported, will be deprecated - SC
                /* create .rii file */
                if (option_was_seen(O_dsm)) {
                    if (outfile != NULL && !multiple_source_files
                                        && remember_last_phase != P_any_ld)
	...
	case P_ipl:
		...
		if (outfile != NULL && last_phase == current_phase
				    ...
				    && !(remember_last_phase == P_any_ld &&
#if 0 // -dsm no longer supported.  Bug 4406.
...
#else
		add_string(args, "-mips4"); // work around movz for now - SC
	        if (qwalign1 == TRUE) {
                  add_string(args, "-qwa1");
                } else {
                  // default pu 16-byte alignment
...
#else
		    ;
		set_library_paths(args, target_is_native());
#endif
		if (outfile != NULL) {
			add_string(args, "-o");
			add_string(args, outfile);
...
add_final_ld_args (string_list_t *args)
{
		...
		add_string(args, cmd_path);
	      } else if (use_h264libs == TRUE) {
	        add_string(args, "-T");
		asprintf(&cmd_path, "%s/usr/libsl2/ldscripts/sl2-h264-common.ld", cmp_tgt_root);
		add_string(args, cmd_path);
	      }
	    }
#endif // SL
...
#ifndef TARG_SL // jczhang: don't need these libs
        if (ipa == TRUE) {
          ...
          if ( fbuiltin != 0 ) {
            ...
            */
            ...
            if (invoked_lang != L_cc) {
              ...
              add_library(args, "m"  OPEN64_CMPLR_NAME_PREFIX);   // bug 3092
#else
              add_library(args, "mv");                  // bug 5527
	      add_library(args, "m" PSC_NAME_PREFIX);
#endif
            }
#else // SL needs no libm at this point, needs simulation library.
		// later on, may need to separate true native vs simulation build
            if (invoked_lang == L_CC) 
        ...
        if (ipa == TRUE) {
          if (shared != DSO_SHARED && shared != RELOCATABLE) {
            ...
            //add_string(args, find_crt_path("crtn.o"));  // jczhang: added above, so drop here.
          }
        }
}
...
init_phase_names ()
{
  ...
  /* Look in this special place for a link to the executable. This
     only works on Linux, but it should work since open64cc runs only on Linux. */
  ...
  //   prefix:   mips64el-key-linux-
  prefix = strdup(cmd);
  if ((x = strstr(prefix, "-" OPEN64_VERSION))) {
    *x = '\0';
  }
...
#if 0 // left for ref. this is troublesome, why Key removed this? - SC
			    ...
			    if (prelink_flag && source_lang == L_CC) {
		  		if (outfile != NULL && !multiple_source_files
				    ...
				    sprintf(ii_file_name,
				   	    "./ii_files/%s",
					    construct_file_with_extension(source_file, "ii"));
			        update_instantiation_info_file(
