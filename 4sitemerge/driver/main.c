...
#else
#include "lib_phase_dir.h"   // various version and paths
#endif
...
static void set_executable_dir (void) {
...
#else
    substitute_phase_dirs ("/usr/bin", basedir, "/" OPEN64_TARGET "/bin");
    substitute_phase_dirs ("/usr/lib", basedir, "/lib/" OPEN64_VERSION);
    substitute_phase_dirs ("/usr/lib/" OPEN64_CMPLR_NAME_PREFIX "cc-lib",
			   basedir, "/lib/" OPEN64_VERSION);
#endif
  ...
  if (ldir != 0) {
    if (ldir[12] == '/') {
      ...
      substitute_phase_dirs ("/usr/lib/" OPEN64_CMPLR_NAME_PREFIX "cc-lib", dir, "");
#endif
...
#else
		asprintf(&our_path, "%s/lib/" OPEN64_VERSION "/32",
			 root_prefix);
	} else {
