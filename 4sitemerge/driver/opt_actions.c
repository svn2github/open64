...
#else
#include "lib_phase_dir.h"   // for various VERSION etc
#endif
#include "phases.h"
#include "run.h"
...
static void Get_x86_ISA_extensions();
#endif
...
} P_to_I_Map[] =
{
  ...
  { PROC_SIMPLIGHT,	ISA_SL,  O_Unrecognized },
...
#endif
#if defined(TARG_X8664) && defined(PATHSCALE)
...
#elif TARG_MIPS
	// for SL1, we also use N32 internally
	toggle(&abi, ABI_N32);
    	add_option_seen ( O_n32 );
#elif TARG_X8664
...
check_dashdash (void)
{
#if 0	// Silently ignore dashdash options in case pathcc is called as
		// a linker.  Bug 4736.
...
print_file_path (char *fname, int exe)
{
  ...
  if (print_relative_path("lib/gcc-lib/" OPEN64_TARGET "/" OPEN64_GCC_VERSION, fname))
    return;

  if (print_relative_path("lib/gcc-lib/" OPEN64_TARGET "/lib64", fname))
    return;

  if (print_relative_path(OPEN64_TARGET "/lib64", fname))
    return;

  if (print_relative_path("lib/" OPEN64_VERSION, fname))
    return;
...
#if defined(TARG_X8664) && (PATHSCALE)
