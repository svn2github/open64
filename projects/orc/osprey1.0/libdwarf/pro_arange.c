...
)
{
	...
	*/
    ...
    int                 extension_word_size = 
			dbg->de_64bit_extension?4:0;
	...
	*/
    ...
    GET_CHUNK(dbg, dbg->de_elf_sects[DEBUG_ARANGES], 
		arange, (unsigned long)arange_num_bytes, 
	error);
    arange_ptr = arange;
    if (arange == NULL)
	...
	count = (3*dbg->de_arange_count)+1;
     ...
     /* the following is a small optimization: not
	...
	*/
    ...
    /* The arange address, length are pointer-size fields
    ...
    */
	...
	if(dbg->de_reloc_pair && 
	  ...
	  res = dbg->de_reloc_pair(dbg,
		...
		dwarf_drt_first_of_length_pair,upointer_size);
	  if(res != DW_DLV_OK) {
	    {_dwarf_p_error(dbg, error, DW_DLE_ALLOC_FAIL); return(0);}
	  }
