...
#ifdef SGI_MONGOOSE
  if (strcmp(input_filename, "<built-in>") != 0 && 
      strcmp(input_filename, "<command line>") != 0 && 
      skip_first != 1) 
    WFE_Set_Line_And_File (lineno, input_filename);
  skip_first = 0;
#endif /* SGI_MONGOOSE */
...
// to verify that the succeeding tokens are 'pragma omp'.
...
#else
	      // Bug 589
	      // gcc-3.3.2 (configured and built for x86-64) and SuseGcc have 
	      // similar problems when used to compile bug-589 with 
	      // -std=iso9899:1999. The problem somehow seems to be fixed with
	      // the RedHat version of Gcc for x86-64 (Gcc on any eng-xx 
	      // machine). Here is one possible way we could work around the 
	      // bug. The bug is that the constant 18446744073709551615 is 
	      // recognized to fit in 16-bytes because according to the C99 
	      // standard (as per the version stated few lines above) states
	      // all decimal constants are treated signed. Without this fix, 
	      // our front-end appears to comply with this standard but does 
	      // not match the output of Gcc on eng-xx machine.
	      if (1) 
#endif
		{
