	...
	Dwarf_Error *error)
{
	...
	if (dbg->eh_frame_cies == NULL) {
	    dbg->eh_frame_cies = (Dwarf_P_Cie) 
		_dwarf_p_get_alloc(dbg, sizeof(struct Dwarf_P_Cie_s));
	    if (dbg->eh_frame_cies == NULL) {
		DWARF_P_DBG_ERROR(dbg,DW_DLE_CIE_ALLOC,DW_DLV_NOCOUNT);
	    }
	    ...
	    dbg->eh_n_cie = 1;
 	    dbg->eh_last_cie = curcie;
	} 
	else {
	    curcie = dbg->eh_last_cie;
	    curcie->cie_next = (Dwarf_P_Cie) 
		_dwarf_p_get_alloc(dbg, sizeof(struct Dwarf_P_Cie_s));
	    if (curcie->cie_next == NULL) {
		DWARF_P_DBG_ERROR(dbg,DW_DLE_CIE_ALLOC,DW_DLV_NOCOUNT);
...
dwarf_add_ehframe_fde(
	...
	Dwarf_Error *error)
{
	return dwarf_add_ehframe_fde_b(dbg, fde, die, cie, virt_addr,
				     code_len, symidx, 0, 0, error);
}

/*ARGSUSED10*/
Dwarf_Unsigned
dwarf_add_ehframe_fde_b(
	Dwarf_P_Debug dbg,
	Dwarf_P_Fde fde,
	Dwarf_P_Die die,
	Dwarf_Unsigned cie,
	...
	Dwarf_Error *error)
{
	...
	curfde = dbg->eh_last_fde;
	if (curfde == NULL) {
	    dbg->eh_frame_fdes = fde;
	    dbg->eh_last_fde = fde;
	    dbg->eh_n_fde = 1;
	}
	else {
	...
	Dwarf_Error *error)
{
	...
	return dwarf_add_ehframe_info_b(
...
#if 0
/*
To allow a relocatable length.
Incomplete so far.
*/
Dwarf_P_Fde
dwarf_fde_add_inst_advance_loc(
    Dwarf_P_Debug       dbg,
    Dwarf_P_Fde         fde,
...
)
{
    ...
    {
            /* omitted till May 1999. Oops.
            */
            int uwordb_size =  fde->fde_uwordb_size;
            du =      val1;
            op = DW_CFA_set_loc;
            ptr = (char *) _dwarf_p_get_alloc(NULL, uwordb_size);
            if (ptr == NULL) {
                    _dwarf_p_error(NULL, error, DW_DLE_STRING_ALLOC);
                    return((Dwarf_P_Fde)DW_DLV_BADADDR);
            }
            WRITE_UNALIGNED(dbg,(void *)ptr,(const void *)&du,
            ...
            nbytes = uwordb_size;
                ...
                WRITE_UNALIGNED(dbg,(void *)ptr, (const void *)&dw,
			sizeof(dw),4);
                nbytes = 4;
            }
            else {
...
/* 
  DW_CFA_set_loc operation omitted till May 1999. Oops.
  Unused so far.
*/
Dwarf_P_Fde
dwarf_fde_add_inst_set_loc(
    Dwarf_P_Debug       dbg,
    Dwarf_P_Fde 	fde,
    Dwarf_Unsigned 	address,
    Dwarf_Unsigned 	sym_index,
...
)
{
    ...
    curinst->dfp_nbytes = nbytes;
    curinst->dfp_next = NULL;
    curinst->dfp_sym_index =  sym_index;
