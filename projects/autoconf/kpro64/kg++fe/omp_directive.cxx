...
void prepare_com_clause (tree  clause_tree, ST_list **clause_st)
{
     ...
     ST * st = NULL;
     ST_list * stlist = NULL, * last = *clause_st;
     ...
     if (!clause_tree)
     {
       ...
       if (!last)
       {
         FmtAssert (*clause_st == NULL, ("Invalid variable list"));
         *clause_st = last = stlist;
       }
       else
       {
         Is_True (last->next == NULL, ("Invalid variable list"));
         last->next = stlist;
         last = last->next;
       }
       return;
     ...
     // Keep the original order of variables, append entries to the end
     for (t = clause_tree; t; t = TREE_CHAIN (t))
       {      
         tree var = TREE_VALUE (t);
         tree tsubst_var = get_substituted (var, Current_Function_Decl ());
         if (tsubst_var)
           var = tsubst_var;
...
void prepare_reduction_clause (reduction_list *reduction_clause_list,
...
void
check_parallel_directive(parallel_clause_list * clause_list)
{
...
expand_start_parallel (struct parallel_clause_list * clause_list)
{
     ...
     for (cl = clause_list; cl != NULL; cl = cl->next)
       {
          if ( cl->type == p_private ) {
            prepare_com_clause (cl->node.var_list, &private_clause_st);
          }
       }
     ...
     for (cl = clause_list; cl != NULL; cl = cl->next)
       {
          if ( cl->type == p_firstprivate ) {
            prepare_com_clause (cl->node.var_list, &firstprivate_clause_st);
          }
       }
     ...
     for (cl = clause_list; cl != NULL; cl = cl->next)
       {
          if ( cl->type == p_shared ) {
            prepare_com_clause (cl->node.var_list, &shared_clause_st);
          }
       }
     ...
     for (cl = clause_list; cl != NULL; cl = cl->next)
       {
          if ( cl->type == p_copyin ) {
            prepare_com_clause (cl->node.var_list, &copyin_clause_st);
          }
       }
...
expand_start_for (struct for_clause_list * clause_list)
{
     ...
     enum schedule_kind_type  schedule_1_clause_value = SK_NONE;
     ...
     for (cl = clause_list; cl != NULL; cl = cl->next)
       {
          if ( cl->type == f_lastprivate ) {
            prepare_com_clause (cl->node.var_list, &lastprivate_clause_st);
          }
       }
     ...
     for (cl = clause_list; cl != NULL; cl = cl->next)
       {
          if ( cl->type == f_schedule_1) {
             schedule_1_clause_value = cl->node.schedule_kind;
             break;
          }
       }
     ...
     for_clause_wn.schedule_1_clause = schedule_1_clause_value;
...
{
     struct Sections_clause_wn_type sections_clause_wn;
     ...
     for (cl = clause_list; cl != NULL; cl = cl->next)
       {
          if ( cl->type == single_copyprivate ) {
            prepare_com_clause (cl->node.var_list, &copyprivate_clause_st);
          }
       }
...
expand_start_parallel_for (struct parallel_for_clause_list * clause_list)
{
     ...
     parallel_for_clause_wn.schedule_1_clause = schedule_1_clause_value;
...
///////// atomic directive ////////
//
// This function is now obsolete, and should be removed in future.
...
void expand_start_do_loop (tree init_expr, tree logical_expr, tree incr_expr)
{
    bool valid_for_expr = true;
    enum tree_code code, code1;
    tree temp; 
    ...
    switch (TREE_CODE (init_expr))
    {
      case EXPR_STMT:
        init_expr = EXPR_STMT_EXPR (init_expr);
        break;
      case DECL_STMT:
        init_expr = DECL_STMT_DECL (init_expr);
        break;
      default:
        break;
    }
...
*/
    ...
    if (valid_for_expr) 
    {
      WFE_expand_start_do_loop (index, start, end, step);
    }
   else
    {
      SRCPOS srcpos = Get_Srcpos();
