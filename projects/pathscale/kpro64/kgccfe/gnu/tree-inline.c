...
#ifdef KEY
// We cannot use any old ST that may be attached to the old decl.
// We hit this place while inlining a function, and we need to create
// new ST's for the inlined body.
	      if (DECL_ST (new_var))
	      	DECL_ST (new_var) = Create_ST_For_Tree (new_var);
#endif
	    }
	}
     ...
     void *data;
{
  ...
  if (!inlinable_function_p (fn, id))
	...
	{
#ifndef KEY	// Don't warn since we are suppressing the gcc inliner
		// (flag_really_no_inline = 1).  For bug 612.
