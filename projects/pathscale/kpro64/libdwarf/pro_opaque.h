...
struct Dwarf_P_Attribute_s {
    ...
    Dwarf_Unsigned	ar_rel_symidx;	/* when attribute has a relocatable
    ...
    char                ar_reloc_len;   /* Number of bytes that relocation
					   applies to. 4 or 8. Unused and
					   may be 0 if
				           if ar_rel_type is R_MIPS_NONE */
...
struct Dwarf_P_Simple_name_header_s {
	...
	Dwarf_Signed               sn_count;
		...
		/* length  that will be generated, not
		   counting fixed header or trailer */
	Dwarf_Signed               sn_net_len;
};
typedef int (*_dwarf_pro_reloc_name_func_ptr)(
        Dwarf_P_Debug dbg,
	int     sec_index,
...
struct Dwarf_P_Relocation_Block_s {
	...
	char *           rb_where_to_add_next; /* pointer to next
				slot (might be past end, depending
				on rb_next_slot_to_use) */
...
struct Dwarf_P_Per_Reloc_Sect_s {
	...
	unsigned long pr_reloc_total_count; /* total
		number of entries across all blocks */

	unsigned long pr_slots_per_block_to_alloc; /* at Block
		 	alloc, this is the 
			default number of slots to use */

	int          pr_sect_num_of_reloc_sect; /* sect number
		returned by de_func() or de_func_b() call,
		this is the sect number of the relocation section.
		*/
		

	/* singly-linked list. add at and ('last') 
           with count of blocks 
	*/
...
struct Dwarf_P_Debug_s {
    ...
    */
    ...
    Dwarf_Ptr           de_errarg;
	    ...
	    Provided by user. Only of these used per dbg.
	*/ 
    ...
    Dwarf_Unsigned		de_flags;    
	   ...
	   debug section stream output, including
           the stream data */
	...
	/* File entry information, null terminated 
	   singly-linked list */
    ...
    /* Pointer to the first section, to 
	   support reset_section_bytes */
    ...
    */
    ...
    /* relocation data. not all sections will actally
       have relocation info, of course
    */
    struct Dwarf_P_Per_Reloc_Sect_s   de_reloc_sect[NUM_DEBUG_SECTIONS];
    int                               de_reloc_next_to_return;/*iterator
		on reloc sections (SYMBOLIC output) */

    /* used in remembering sections */
    int             de_elf_sects[NUM_DEBUG_SECTIONS]; /* 
		        elf sect number
			of the section itself, DEBUG_LINE for example */

    Dwarf_Unsigned  de_sect_name_idx[NUM_DEBUG_SECTIONS]; /* section 
			name index or handle for the name of
			the symbol for DEBUG_LINE for example  */



    int de_offset_reloc;  /* offset reloc type,
		R_MIPS_32 for example. Specific to the ABI
		being produced. Relocates offset size field */
    int de_exc_reloc; /* reloc type 
		specific to exception table relocs. */
    int de_ptr_reloc;  /* standard reloc type,
		R_MIPS_32 for example. Specific to the ABI
		being produced. relocates pointer size field */

    unsigned char de_offset_size; /* section offset.
	Here to avoid test of abi in macro at run time 
  	MIPS -n32 4, -64 8.  */
    ...
    unsigned char de_relocation_record_size; /* reloc
		record size varies by ABI and relocation-output
		method (stream or symbolic) */

    unsigned char de_64bit_extension; /* non-zero if
	creating 64 bit offsets using dwarf2-99
	extension proposal */
    ...
    /* Add new fields at the END of this struct to preserve some
       hope of sensible behavior on dbg passing between DSOs
       linked with mismatched libdwarf producer versions.
    */
...
Dwarf_Unsigned _dwarf_add_simple_name_entry (
    Dwarf_P_Debug        dbg,
    Dwarf_P_Die          die,
    char                *entry_name,
