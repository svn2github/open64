...

/* ELF and ECOFF are incompatible and can't be mapped to each other, so give an
   error if we encounter an ECOFF file.  */
static boolean
elf64_alpha_copy_private_bfd_data (ibfd, obfd)
     bfd *ibfd;
     bfd *obfd;
{
...
static boolean
elf64_alpha_merge_private_bfd_data (ibfd, obfd)
     bfd *ibfd;
     bfd *obfd;
{
  if (bfd_get_flavour (ibfd) != bfd_target_elf_flavour
