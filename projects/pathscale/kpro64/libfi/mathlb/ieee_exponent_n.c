...
#pragma ident "@(#) libfi/mathlb/ieee_exponent_n.c	92.1	07/09/99 11:00:36"
#include <fmath.h>
#include <fortran.h>
#ifdef	__mips
#include <fp_class.h>
...
#ifndef	__mips
#ifdef _F_REAL4
/* IEEE_EXPONENT(X,Y_I4) returns integer(kind=4) result: */
#ifdef	_CRAYMPP
extern _f_int4 _IEEE_EXPONENT_I4_H(_f_real8 x);
#else	/* _CRAYMPP */
extern _f_int4 _IEEE_EXPONENT_I4_H(_f_real4 x);
#endif	/* _CRAYMPP */
extern _f_int4 _IEEE_EXPONENT_I4_R(_f_real8 x);
extern _f_int4 _IEEE_EXPONENT_I4_D(_f_real16 x);

/* _IEEE_EXPONENT_I4_H - IEEE EXPONENT returns the exponent part of the
 *                      32-bit argument in 32-bit integer.
 */
#ifdef	_CRAYMPP
static int __ieee_real4_i4(_f_real4 x);

_f_int4
_IEEE_EXPONENT_I4_H(_f_real8 x)
{
	int		i;
	REGISTER_8	s1;
...
#else	/* _CRAYMPP */
_f_int4
_IEEE_EXPONENT_I4_H(_f_real4 x)
{
	int		i;
	REGISTER_4	s1, s2;
...
/* _IEEE_EXPONENT_I4_R - IEEE EXPONENT returns the exponent part of the
 *                      64-bit argument in 32-bit integer.
 */
_f_int4
_IEEE_EXPONENT_I4_R(_f_real8 x)
{
	int		i;
	REGISTER_8	s1, s2;
...
#if _F_REAL16 == 1
/* _IEEE_EXPONENT_I4_D - IEEE EXPONENT returns the exponent part of the
 *                       128-bit argument in 32-bit integer.
 */
_f_int4
_IEEE_EXPONENT_I4_D(_f_real16 x)
{
...
_IEEE_EXPONENT_I4_H(_f_real4 x)
{
	...
	switch(fp_class_f(x)) {
		...
		case FP_NEG_NORM:
			...
			x_val.dword	= x;
			return((_f_int4)(x_val.parts.exponent -
				IEEE_32_EXPO_BIAS));
			}
		case FP_POS_DENORM:
		case FP_NEG_DENORM:
			 ...
			 * be counted.
			 */
			return((_f_int4)(-IEEE_32_EXPO_BIAS -
...
/* _IEEE_EXPONENT_I4_R - IEEE EXPONENT returns the exponent part of the
 *                      64-bit argument in 32bit integer.
 */
_f_int4
_IEEE_EXPONENT_I4_R(_f_real8 x)
{
	...
	switch(fp_class_d(x)) {
		...
		case FP_NEG_DENORM:
			 ...
			 * be counted.
			 */
			return((_f_int4)(-IEEE_64_EXPO_BIAS -
				(_leadz8(x_val.ull) +
				IEEE_64_EXPO_BITS)));
			}
		case FP_POS_ZERO:
...
/* _IEEE_EXPONENT_I4_D - IEEE EXPONENT returns the exponent part of the
 *                      128-bit argument in 32bit integer.
 */
_f_int4
_IEEE_EXPONENT_I4_D(_f_real16 x)
{
		...
		case FP_NEG_DENORM:
			...
			switch(fp_class_d(x_val.dbword[0])) {
				...
				case FP_NEG_NORM:
					...
					return((_f_int4)(db_x.parts.exponent -
					  IEEE_64_EXPO_BIAS));
					}
				case FP_POS_DENORM:
				case FP_NEG_DENORM:
