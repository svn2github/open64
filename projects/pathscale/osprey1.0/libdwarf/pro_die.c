	...
	Dwarf_Error *error)
{
	...
	if (new_die == NULL) {
	    DWARF_P_DBG_ERROR(dbg,DW_DLE_DIE_ALLOC,(Dwarf_P_Die)DW_DLV_BADADDR);
	}
	...
	new_die->di_tag = tag;
	ret_die = dwarf_die_link(new_die,parent,child,left,right,error);
	return ret_die;
}
	...
	Dwarf_Error *error)
{
	...
	if (child != NULL) {
	    ...
	    if (child->di_parent) {
		DWARF_P_DBG_ERROR(NULL,DW_DLE_PARENT_EXISTS,(Dwarf_P_Die)DW_DLV_BADADDR);
	    } else
	...
	if (left != NULL) {
	    ...
	    if (left->di_right) 	/* there's already a right sibl, lets insert */
	...
	if (n_nulls > 1) { 	/* multiple neighbors, error */
	    DWARF_P_DBG_ERROR(NULL,DW_DLE_EXTRA_NEIGHBORS,(Dwarf_P_Die)DW_DLV_BADADDR);
	}
	return new_die;
	...
	Dwarf_Error *error)
{
	...
	if (new_attr->ar_data == NULL) {
        ...
        {
          Dwarf_Unsigned du = 0;
	  WRITE_UNALIGNED(dbg,(void *)new_attr->ar_data,(const void *)&du,
	...
	Dwarf_Error *error)
{
	...
	if (die == NULL) {
	    DWARF_P_DBG_ERROR(NULL,DW_DLE_DIE_NULL,(Dwarf_P_Attribute)DW_DLV_BADADDR);
	}
	new_attr = (Dwarf_P_Attribute)
	    _dwarf_p_get_alloc(NULL,sizeof(struct Dwarf_P_Attribute_s));
	if (new_attr == NULL) {
	    DWARF_P_DBG_ERROR(NULL,DW_DLE_ATTR_ALLOC,(Dwarf_P_Attribute)DW_DLV_BADADDR);
	}
	...
	if (new_attr->ar_data == NULL) {
	    DWARF_P_DBG_ERROR(NULL,DW_DLE_STRING_ALLOC,(Dwarf_P_Attribute)DW_DLV_BADADDR);
	}
