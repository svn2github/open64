...
)
{
	...
	_dwarf_p_get_alloc(dbg, sizeof(struct Dwarf_P_Simple_nameentry_s));
    if (nameentry == NULL) {
    ...
    )
{
    ...
    Dwarf_Unsigned	      adjusted_length; /* count excluding 
				length field*/
    ...
    stream_bytes_count = extension_size +
	...
	stream_bytes, 
	(unsigned long)stream_bytes_count, error);
