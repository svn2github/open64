 ...
 */
typedef struct {
    UINT64 start;         //!< the starting PC of the corresponding .rta.data snippet
    UINT64 data_off;      //!< the offset in .rta.data section of the corresponding .rta.data snippet
    UINT32 size;          //!< the PC range of the corresponding .rta.data snippet
 ...
 */
typedef struct {
    UINT64 magic;         //!< the magic number of the annotation section
    INT32  pu_off;        //!< offset to the PU header table
    UINT16 type;          //!< the type of the annotation, whether static or dynamic
    UINT16 target;        //!< achitecture of target machine
    UINT16 pu_num;        //!< number of entries in PU header table
    UINT16 flags;         //!< lang, alias type, linkage rule, etc
    char     padding[4];    //!< padding for 64bit alignment
} Rta_Hdr;
 ...
 */
typedef struct {
    UINT64   start;   //!< beginning address of this PU (portion) in the .text section
    UINT32   size;    //!< size (in words) of PU (portion)
    UINT32   name;    //!< the name of this PU (portion), an index to strtab
    INT32    bb_off;  //!< offset (in bytes) from this PU header to its BB header table
    INT32    cfg_off; //!< offset to its CFG "string table"
    UINT32   maxregs; //!< max number of pseudo registers used in this PU (portion)
    UINT16   bb_num;  //!< number of BBs in this PU
    UINT16   bb_begin;//!< the smallest index of BB
    UINT16   entry;   //!< index of entry BB in the CFG, we assume there's only one
    UINT16   exit;    //!< index of exit BB in the CFG, we assume there's only one
    UINT16   flags;   //!< source code language, alias type, linkage rule, etc
    INT16    link;    //!< link to the next portion of the PU
    //  char       padding[4];  //!< padding for 64bit alignment
} Rta_Pu;
 ...
 */
typedef struct {
    UINT32 start;      //!< beginning addr offset of this BB based on the PU beginning addr
    UINT32 size;       //!< we assume a BB will not exceed 32bit size
    INT32  op_off;     //!< offset to the IR stmts of this BB
    UINT16 plink;      //!< index of the first element of predecessor list
    UINT16 slink;      //!< index of the first element of successor list
    UINT16 op_num;     //!< number of IR stmts in BB, we assume not exceed 16bits
    UINT16 flags;      //!< no-opt, IR-type, etc
    char     padding[4]; //!< padding for 64bit alignment
} Rta_Bb;
 ...
 */
typedef struct {
    INT32  extra;         //!< offset to extra information
    UINT16 opcode;        //!< opcode
    UINT16 result1;       //!< first result TN number
    UINT16 opnd1;         //!< first operand TN number
    UINT16 opnd2;         //!< second operand TN number
    UINT16 flags;         //!< flags
 ...
 */
typedef struct {
    INT32  offset;        //!< offset to extra information
    UINT16 type;          //!< type of the information
    UINT16 flags;         //!< flags (the most significant bit is reserved)
} Rta_Extra;
