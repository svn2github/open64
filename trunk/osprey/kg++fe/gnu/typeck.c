...
#if defined(TARG_SL2)  
// __v2buf char array[4][16] = { 0 } ; 
// assignment to element of vbuf array is forbiddent 
//  array[2][0] = 5;   
  if(TREE_CODE(lhs) == ARRAY_REF)
  {
     tree opnd0 = TREE_OPERAND(lhs, 0);  
     while(TREE_CODE(opnd0) == ARRAY_REF) 
	 opnd0 = TREE_OPERAND(opnd0, 0); 
     if(opnd0  && (TREE_CODE(opnd0) == VAR_DECL) && DECL_VBUF(opnd0))
     {
