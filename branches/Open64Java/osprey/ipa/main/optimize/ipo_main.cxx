	     ...
	     IPA_CALL_GRAPH *cg)
{
    ...
    {
      PU p = caller->Get_PU();
      if (((PU_src_lang (p) & PU_CXX_LANG)||(PU_src_lang (p) & PU_JAVA_LANG)) && PU_has_region (p))
...
#ifdef KEY
  if ((PU_src_lang (node->Get_PU()) & PU_CXX_LANG)||(PU_src_lang (node->Get_PU()) & PU_JAVA_LANG))
...
IPA_Remove_Regions (IPA_NODE_VECTOR v, IPA_CALL_GRAPH * cg)
{
    ...
    
    {
      ...
      if ((!(PU_src_lang (pu) & PU_CXX_LANG) && !(PU_src_lang (pu) & PU_JAVA_LANG)) || !PU_has_region (pu))
      	continue;
