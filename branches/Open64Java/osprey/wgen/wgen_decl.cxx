  ...
  switch (gs_tree_code(decl)) { 
    ...
    case GS_FUNCTION_DECL:
      ...
      // needed.
		...
		for(i = 1; i < level; i++)
		{
	  	...
	  	printf("transform a FUNCTION_DECL\n");
	#endif
	...
	return;
        ...
        if (body != NULL && 
	     ...
	     !gs_decl_maybe_in_charge_destructor_p(decl)) { //czw
    ...
    case GS_NAMESPACE_DECL: {
      ...
      /*      if (decl == std_node)
        ...
        printf("transform a NAMESPACE_DECL\n");
       #endif
...
//      fprintf(stderr, "NAMESPACE_DECL: Pop_Deferred_Function\n");
    ...
    case GS_TEMPLATE_DECL:
      {
		...
		printf("transform a TEMPLATE_DECL\n");
		#endif
    ...
    case GS_TYPE_DECL: {
      ...
      // because it may have side-effects.
		...
		printf("transform a TYPE_DECL\n");
		#endif
      if (!can_skip || gs_tree_side_effects(decl) || lang_java)   //czw
#endif
      {
...
AGGINIT::WGEN_Add_Aggregate_Init_Address (gs_t init)
{
  ...
  case GS_FUNCTION_DECL:
	...
	//czw
#ifdef KEY
	if (gs_decl_alias_target(init))	// Bug 4393.
		...
		if (WN_operator(wn) != OPR_ARRAY)
		  ...
		  if (const_indices) {
		    ...
		    WGEN_Add_Aggregate_Init_Symbol (WN_st(WN_array_base(wn)),
			      WN_offset(WN_array_base(wn)) + displace + iload_ofst);
		  }
  ...
  UINT field_id)
{
  ...
  if (TY_kind (ty) == KIND_ARRAY) {
    ...
    Traverse_Aggregate_Array (st, init_list, type, gen_initv, current_offset);//czw
  }
...
WGEN_Assemble_Alias (gs_t decl, gs_t target)
{
  ...
  if (!expanded_decl(base_decl)) {
    return 0;//czw//FALSE; // bugs 12602, 12704
  }
  expanded_decl(decl) = TRUE;
	...
	*/					        
	...
	gs_t decl = gs_operand(list, 0);
	  ...
	  if (gs_decl_thunk_p(decl))
	    ...
	    changed |= WGEN_Assemble_Alias(decl, gs_decl_alias_target(decl))?TRUE:FALSE;//czw
	  else        
