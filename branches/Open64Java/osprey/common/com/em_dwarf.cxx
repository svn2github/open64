...
Dwarf_P_Debug 
Em_Dwarf_Begin (BOOL is_64bit, BOOL dwarf_trace, DST_language dst_lang,		//czw
...
#define EXT_OP(v)  (DW_CFA_extended | v)
  ...
  // got removed.
  if (dst_lang == DW_LANG_C_plus_plus)/* may have eh info */		//czw
  {
  ...
  else if (dst_lang == DW_LANG_Java)/* may have eh info */		//czw
  {
    	if (Gen_PIC_Call_Shared || Gen_PIC_Shared)
	    augmenter = PIC_DW_CIE_AUGMENTER_STRING_V0;
	else
	    augmenter = DW_CIE_AUGMENTER_STRING_V0;
	personality = Save_Str ("__gcj_personality_v0");		//czw
        if (Gen_PIC_Call_Shared || Gen_PIC_Shared)
        {
            ST * pic_personality_st = New_ST (GLOBAL_SYMTAB);
            STR_IDX name = Save_Str ("DW.ref.__gcj_personality_v0");		//czw
            ST_Init(pic_personality_st, name, CLASS_VAR, SCLASS_DGLOBAL, EXPORT_HIDDEN, MTYPE_TO_TY_array[MTYPE_U8]);
            Set_ST_is_weak_symbol (pic_personality_st);
	    Set_ST_is_initialized (pic_personality_st);
            ST_ATTR_IDX st_attr_idx;
            ST_ATTR&    st_attr = New_ST_ATTR (GLOBAL_SYMTAB, st_attr_idx);
            ST_ATTR_Init (st_attr, ST_st_idx (pic_personality_st), ST_ATTR_SECTION_NAME, Save_Str (".gnu.linkonce.d.DW.ref.__gcj_personality_v0"));		//czw

	    ST * personality_st = New_ST (GLOBAL_SYMTAB);
	    ST_Init (personality_st, Save_Str("__gcj_personality_v0"), CLASS_VAR, SCLASS_EXTERN, EXPORT_PREEMPTIBLE, MTYPE_TO_TY_array[MTYPE_U8]);	//czw
	    INITV_IDX iv = New_INITV();
	    INITV_Init_Symoff (iv, personality_st, 0, 1);
	    New_INITO (ST_st_idx (pic_personality_st), iv);
	    // bug 8315: With -ipa, if this file is not symtab.s (actually
  ...
  if ((dst_lang == DW_LANG_C_plus_plus) || (dst_lang == DW_LANG_Java) || DEBUG_Emit_Ehframe)		//czw
