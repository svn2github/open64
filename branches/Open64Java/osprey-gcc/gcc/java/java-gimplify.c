...
static tree transform_statement(tree);
...
java_genericize (tree fndecl)
{
  ...
  if(flag_spin_file)
  {
	...
	DECL_SAVED_TREE (fndecl) = transform_statement(DECL_SAVED_TREE (fndecl));
  ...
  while (handler)
    {
      ...
      tree expr;    //czw
      tree prepare_eh_table_type_return = prepare_eh_table_type (catch_type);
	 // if(!flag_spin_file)      //czw
      expr = build2 (CATCH_EXPR, void_type_node,
			  prepare_eh_table_type_return,
			  handler);
	/*  else
      expr = build2 (CATCH_EXPR, void_type_node,
			  catch_type,
...
static tree transform_statement(tree statement)
{
	tree arglist;
	...
	switch(TREE_CODE (statement))
	{
		...
		case CALL_EXPR:
			for (arglist = TREE_OPERAND (statement, 1); arglist;
		...
		case SAVE_EXPR:
		      /* Note that we can see <save_expr NULL> if the save_expr was
			 already handled by gimplify_save_expr.  */
		      if (TREE_OPERAND (statement, 0) != NULL_TREE
			  && TREE_CODE (TREE_OPERAND (statement, 0)) == VAR_DECL)
			TREE_OPERAND (statement, 0) 
			  = java_replace_reference (TREE_OPERAND (statement, 0), 
			       /* want_lvalue */ false);
			TREE_OPERAND (statement, 0)  = transform_statement(TREE_OPERAND (statement, 0) );
			break;
