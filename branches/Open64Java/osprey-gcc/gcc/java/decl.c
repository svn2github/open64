...
pushdecl (tree x)
{
  ...
  if (TREE_CODE (x) != TYPE_DECL)
  {
#ifdef KEY 
	//yzm
	//debugging position: print out the name of function level 
	//objects
	/*
	tree name = DECL_NAME(x);
	if (name != NULL)
	{
	  ...
	  {
		  printf("Some Identifier Node Here.");
		  printf("%s\n", IDENTIFIER_POINTER(name));

//		  if (strcmp("Test002.class", IDENTIFIER_POINTER(name)) == 0)
		  if (current_function_decl != NULL)
		  {
			tree func_name = DECL_NAME(current_function_decl);
			if (func_name != NULL)
			{
