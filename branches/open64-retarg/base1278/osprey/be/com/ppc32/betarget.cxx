...
INT Max_Small_Frame_Offset = 0x7fff;    // 16 bits
...
/*      if (rtype == MTYPE_F4 || rtype == MTYPE_F8 || rtype == MTYPE_F10)
    ...
    case OPR_TRAP:
      return TOP_UNDEFINED;//TOP_break;
...
Multiply_Limit( BOOL is_64bit, INT64 val)
{
  ...
  if (is_64bit)	{ /* 64-bit multiply */
    switch( Target ) {
    case TARGET_POWERPC:	limit = 14; break;
...
Divide_Limit ( BOOL is_64bit)
{
  ...
  if (is_64bit)	{ /* 64-bit divide */
    switch( Target ) {
    case TARGET_POWERPC:	limit = 50; break;
...
INT Copy_Quantum_Ratio(void)
{
  ...
  switch(Target) {
  case TARGET_POWERPC:	ratio=	4; break;
