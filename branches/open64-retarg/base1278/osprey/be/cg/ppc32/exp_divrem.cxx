...
FmtAssert( FALSE, ("Not IMP")); 
  ...
  if (MTYPE_is_unsigned(mtype)) {
    ...
    Expand_Add (t1, Gen_Literal_TN(absdvsr-1, is_double ? 8 : 4), 
		numer, mtype, ops);
    Build_OP(TOP_mc_z_lt, tmp_result, t1, numer, numer, ops);
#else
    TN *p1 = Build_TN_Of_Mtype(MTYPE_I4);
    Build_OP(TOP_cmp, p1, numer, Zero_TN, ops);
    Expand_Add (t1, Gen_Literal_TN(absdvsr-1, is_double ? 8 : 4), 
		numer, mtype, ops);
...
#ifdef TARG_PPC32
    Build_OP(is_double ? TOP_dsra : TOP_srawi, t2, tmp_result, Gen_Literal_TN(n, 4), ops);
    if (dvsr < 0) Expand_Neg(tmp_result, t2, mtype, ops);
    if (tmp_result != result) {
      Expand_Copy(result, tmp_result, mtype, ops);
    }
#else 
    Build_OP(is_double ? TOP_dsra : TOP_srawi, t2, t1, Gen_Literal_TN(n, 4), ops);
...
FmtAssert( FALSE, ("Not IMP")); 
  ...
  if (is_signed) {
    ...
    if (is_double) 
      ...
      Build_OP (TOP_srwi, p1, numer_tn, Gen_Literal_TN(31, 4), ops);
    ...
    if (is_odd) {
	 ...
	 multiply operation. */
      ...
      Build_OP (is_double?TOP_dsrl:TOP_srwi, tmp1_tn, numer_tn, Gen_Literal_TN (1, 4), ops);
      numer_tn = tmp1_tn;
    }
	...
	Build_OP (TOP_srwi, tmp2_tn, tmp1_tn, Gen_Literal_TN (1, 4), ops);
    ...
    if (is_double)
      ...
      Build_OP (TOP_srwi, result, mult_tn, Gen_Literal_TN ((n-1)%32, 4), ops);    
  } /* end Unsigned */
#endif
  return TRUE;
}
...
FmtAssert( FALSE, ("Not IMP")); 
  ...
  if (MTYPE_is_signed(mtype)) {
    ...
    Build_OP(TOP_mc_zn_ge, tmp1, src1, src1, src1, ops);
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_UNC_DEF);
#else
    /* Test sign of src1
     */
    p1 = Build_TN_Of_Mtype(MTYPE_I4);
    Build_OP(TOP_cmp, p1, src1, Zero_TN, ops);
    ...
    Build_OP(TOP_mc_zn_ge, result, tmp2, tmp2, src1,ops); 
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_UNC_DEF);
#else
    if (is_double) 
...
Expand_Divide (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  Build_OP(div, result, src1, src2, ops);
  return result;
#if 0
  ...
  if (Is_Target_Sb1()) {
	...
	tmp = Expand_Immediate_Into_Register (src2, mtype, ops);
      ...
      Build_OP(TOP_fdiv, t5, t3, t4, ops);
      Build_OP(TOP_trunc_l_d, t6, t5, ops);
      Build_OP(TOP_dmfc1, result, t6, ops);
      Build_OP(TOP_tweq, (success == 1)? src2: tmp, 
...
#if !defined(TARG_PPC32)
  Build_OP(TOP_tweq, src2, Zero_TN, Gen_Literal_TN(7, 4), ops);
#else
...
// FmtAssert( FALSE, ("Not IMP")); 
  ...
  if (MTYPE_is_unsigned(mtype)) div = TOP_divwu;
  Build_OP(div, result, src1, src2, ops);
  Build_OP(TOP_mullw, result, result, src2, ops);
  Build_OP(TOP_subf, result, result, src1, ops);
#if 0
  TOP top;
  BOOL is_double = MTYPE_is_size_double(mtype);
...
Expand_Mod (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  Build_OP(TOP_cmp, p1, tmp2, Zero_TN, ops);
...
// FmtAssert( FALSE, ("Not IMP")); 
  ...
  if (MTYPE_is_unsigned(mtype)) div = TOP_divwu;
  Build_OP(div, result, src1, src2, ops);
  Build_OP(TOP_mullw, result2, result, src2, ops);
  Build_OP(TOP_subf, result2, result2, src1, ops);
#if 0
  BOOL is_double = MTYPE_is_size_double(mtype);
