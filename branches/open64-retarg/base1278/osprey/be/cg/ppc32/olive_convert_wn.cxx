...
void Handle_Cond_Move_Int(OPERATOR opr, TOP top_cmp, TN *dest, TN *src1, TN *src2, OPS *ops)
{
    ...
    {
        case OPR_GT:
            Build_OP(TOP_rlwinm, dest, dest, Gen_Literal_TN(2, 4), Gen_Literal_TN(31, 4), Gen_Literal_TN(31, 4), dest, ops);
            break;
        case OPR_GE:
            Build_OP(TOP_rlwinm, dest, dest, Gen_Literal_TN(1, 4), Gen_Literal_TN(31, 4), Gen_Literal_TN(31, 4), dest, ops);
            Build_OP(TOP_xori, dest, dest, Gen_Literal_TN(1, 4), ops);
            break;
        case OPR_EQ:
            Build_OP(TOP_rlwinm, dest, dest, Gen_Literal_TN(3, 4), Gen_Literal_TN(31, 4), Gen_Literal_TN(31, 4), dest, ops);
            break;
        case OPR_NE:
            Build_OP(TOP_rlwinm, dest, dest, Gen_Literal_TN(3, 4), Gen_Literal_TN(31, 4), Gen_Literal_TN(31, 4), dest, ops);
            Build_OP(TOP_xori, dest, dest, Gen_Literal_TN(1, 4), ops);
...
{
    TN * cr = Gen_CR_TN(1);
    Build_OP(TOP_fcmpu,  cr/*Gen_Literal_TN(1, 4)*/, src1, src2, ops);
    switch (opr)
    {
        case OPR_GT:
            Build_OP(TOP_mfcr, dest, cr, ops);
            Build_OP(TOP_rlwinm, dest, dest, Gen_Literal_TN(6, 4), Gen_Literal_TN(31, 4), Gen_Literal_TN(31, 4), dest, ops);
            break;
        case OPR_GE:
	     Build_OP(TOP_cror, cr, Gen_Literal_TN(0, 4), Gen_Literal_TN(5, 4), Gen_Literal_TN(6, 4), cr, ops);
            Build_OP(TOP_mfcr, dest, cr, ops);
            Build_OP(TOP_rlwinm, dest, dest, Gen_Literal_TN(1, 4), Gen_Literal_TN(31, 4), Gen_Literal_TN(31, 4), dest, ops);
            break;
        case OPR_EQ:
            Build_OP(TOP_mfcr, dest, cr, ops);
            Build_OP(TOP_rlwinm, dest, dest, Gen_Literal_TN(7, 4), Gen_Literal_TN(31, 4), Gen_Literal_TN(31, 4), dest, ops);
            break;
        case OPR_NE:
	     Build_OP(TOP_cror, cr, Gen_Literal_TN(0, 4), Gen_Literal_TN(4, 4), Gen_Literal_TN(5, 4), cr, ops);
            Build_OP(TOP_mfcr, dest, cr, ops);
            Build_OP(TOP_rlwinm, dest, dest, Gen_Literal_TN(1, 4), Gen_Literal_TN(31, 4), Gen_Literal_TN(31, 4), dest, ops);
            break;
        case OPR_LE:
            Build_OP(TOP_cror, cr, Gen_Literal_TN(0, 4), Gen_Literal_TN(4, 4), Gen_Literal_TN(6, 4), cr, ops);
            Build_OP(TOP_mfcr, dest, cr, ops);
            Build_OP(TOP_rlwinm, dest, dest, Gen_Literal_TN(1, 4), Gen_Literal_TN(31, 4), Gen_Literal_TN(31, 4), dest, ops);
            break;
        case OPR_LT:
            Build_OP(TOP_mfcr, dest, cr, ops);
            Build_OP(TOP_rlwinm, dest, dest, Gen_Literal_TN(5, 4), Gen_Literal_TN(31, 4), Gen_Literal_TN(31, 4), dest, ops);
            break;
    }
}
...
void Handle_Float_Int_Cvt(Cvt_Type cvtType, TYPE_ID srcType, TN* dest, TN* src, OPS* ops)
{
    ...
    /*
    Build_OP(TOP_stfd, tn, SP_TN, Gen_Literal_TN(base_ofst, 4), ops);
    Build_OP(TOP_lwz, dest, SP_TN, Gen_Literal_TN(base_ofst + 4, 4), ops);
    */
}
/*
    # FR0 = 0.0
...
void Handle_Float_Uint_Cvt(Cvt_Type cvtType, TYPE_ID srcType, TN* dest, TN* src, OPS* ops)
{
    ...
    BB* bb_entry = Cur_BB;
    BB* bb_flip_entry = Gen_And_Append_BB(bb_entry);
    BB* bb_exit = Gen_And_Append_BB(bb_flip_entry);
    const LABEL_IDX label_bb_exit = Gen_Label_For_BB(bb_exit);
    ...
    Build_OP(TOP_fcmpu, Gen_CR_TN(0) /*Gen_Literal_TN(0, 4)*/, fr2, fr4, ops);
    ...
    /* Build_OP(TOP_stfd, fr2, SP_TN, Gen_Literal_TN(base_ofst, 4), ops);
    Build_OP(TOP_lwz, dest, SP_TN, Gen_Literal_TN(base_ofst + 4, 4), ops); */
    Build_OP(TOP_blt, Gen_CR_TN(0), Gen_Label_TN(label_bb_exit, 0), ops);
    ...
    OPS* bb_flip_entry_ops = &New_OPs;
    Build_OP(TOP_xoris, dest, dest, Gen_Literal_TN(0x8000, 4), bb_flip_entry_ops);
    
    total_bb_insts = 0;
    Last_Processed_OP = NULL;
    Process_New_OPs();
    BB_Append_Ops(bb_flip_entry, &New_OPs);
    OPS_Init(&New_OPs);
    OPS_Init(ops);
    Cur_BB = bb_exit;
}
...
void Handle_Int_Float_Cvt(TN * dest, TN * src, OPS * ops, bool isUnsigned, bool isDouble)
{
    ...
    // Build_OP(TOP_stw, tmp1, SP_TN, Gen_Literal_TN(ofst, Pointer_Size), ops);
    Exp_Store(MTYPE_I4, tmp1, sym1, 0, ops, V_NONE);

    if (!isUnsigned) {
        Build_OP(TOP_xoris, src, src, Gen_Literal_TN(0x8000, Pointer_Size), ops);    
    }
    // Build_OP(TOP_stw, src, SP_TN, Gen_Literal_TN(ofst+4, Pointer_Size), ops);
    Exp_Store(MTYPE_I4, src, sym1, 4, ops, V_NONE);

    Exp_Load(MTYPE_F8, MTYPE_F8, dest, sym1, 0, ops, V_NONE);    
    // Build_OP(TOP_lfd, dest, SP_TN, Gen_Literal_TN(ofst, Pointer_Size), ops);

    Build_OP(TOP_fsub, dest, dest, tmp, ops);
    if (!isDouble) {
        Build_OP(TOP_frsp, dest, dest, ops);
    }
...
void Handle_ULonglong_Float_Cvt(TN* dest, TN* src_high, TN* src_low, OPS* ops, bool isDouble)
{
    ...
    Build_OP(TOP_oris, tmp1, tmp1, Gen_Literal_TN(0x43F0, 4), ops);
    // Build_OP(TOP_stw, tmp1, SP_TN, Gen_Literal_TN(ofst, 4), ops);
    Exp_Store(MTYPE_I4, tmp1, sym1, 0, ops, V_NONE);
    // Build_OP(TOP_stw, src_low, SP_TN, Gen_Literal_TN(ofst+4, 4), ops);
    Exp_Store(MTYPE_I4, src_low, sym1, 4, ops, V_NONE);
    // Build_OP(TOP_lfd, dest, SP_TN, Gen_Literal_TN(ofst, 4), ops);
    Exp_Load(MTYPE_F8, MTYPE_F8, dest, sym1, 0, ops, V_NONE);    
    Build_OP(TOP_fsub, dest, dest, tmp, ops);
    if (!isDouble) {
        Build_OP(TOP_frsp, dest, dest, ops);
...
void Handle_Cond_Longlong(OPERATOR opr, TOP top_cmp, TN *dest, TN *src1_high, TN* src1_low, TN *src2_high,  TN *src2_low, OPS *ops)
{
    if (opr == OPR_EQ)
    {
        ...
        LABEL_IDX l_set_true  = Gen_Label_For_BB(bb_set_true);
        LABEL_IDX l_exit      = Gen_Label_For_BB(bb_exit);
        
        BB_branch_wn(bb_entry) = WN_Create(OPC_TRUEBR,1);
        WN_kid0(BB_branch_wn(bb_entry)) = NULL;
        WN_label_number(BB_branch_wn(bb_entry)) = l_exit;
        
        Build_OP(TOP_li, dest, Gen_Literal_TN(0, 4), ops);
        TN * cr = Gen_CR_TN(0);
        Build_OP(top_cmp, cr, src1_high, src2_high, ops);
        Build_OP(TOP_bne, cr, Gen_Label_TN(l_exit, 0), ops);
        // Build_OP(top_cmp, Gen_Literal_TN(0, 4), src1_high, src2_high, ops);
        // Build_OP(TOP_bne, Gen_Literal_TN(0, 4), Gen_Label_TN(l_exit, 0), ops);
        ...
        WN_label_number(BB_branch_wn(bb_cmp_low)) = l_exit;

        OPS_Init(&New_OPs);
        OPS * bb_cmp_low_ops = &New_OPs;
        Build_OP(top_cmp, Gen_CR_TN(0) , src1_low, src2_low, bb_cmp_low_ops);
        Build_OP(TOP_bne, Gen_CR_TN(0) , Gen_Label_TN(l_exit, 0), bb_cmp_low_ops);
        ...
        Build_OP(TOP_li, dest, Gen_Literal_TN(1, 4), bb_set_true_ops);
        Process_New_OPs();
    ...
    if (opr == OPR_NE)
    {
        BB* bb_entry = Cur_BB;
        BB* bb_cmp_low = Gen_And_Append_BB(bb_entry);
        BB* bb_set_false = Gen_And_Append_BB(bb_cmp_low);
        ...
        Build_OP(TOP_li, dest, Gen_Literal_TN(1, 4), ops);
        Build_OP(top_cmp, Gen_CR_TN(0) /*Gen_Literal_TN(0, 4)*/, src1_high, src2_high, ops);
        Build_OP(TOP_bne, Gen_CR_TN(0) /*Gen_Literal_TN(0, 4)*/, Gen_Label_TN(label_bb_exit, 0), ops);
        ...
        BB_branch_wn(bb_cmp_low) = WN_Create(OPC_TRUEBR,1);
        WN_kid0(BB_branch_wn(bb_cmp_low)) = NULL;
        WN_label_number(BB_branch_wn(bb_cmp_low)) = label_bb_exit;
        
        OPS* bb_cmp_low_ops = &New_OPs;
        Build_OP(top_cmp, Gen_CR_TN(0) , src1_low, src2_low, bb_cmp_low_ops);
        Build_OP(TOP_bne, Gen_CR_TN(0) , Gen_Label_TN(label_bb_exit, 0), bb_cmp_low_ops);
        ...
        Build_OP(TOP_li, dest, Gen_Literal_TN(0, 4), bb_set_false_ops);
    ...
    BB* bb_set_false = Gen_And_Append_BB(bb_cmp_low);
    BB* bb_exit = Gen_And_Append_BB(bb_set_false);
    const LABEL_IDX label_bb_set_false = Gen_Label_For_BB(bb_set_false);
    const LABEL_IDX label_bb_exit = Gen_Label_For_BB(bb_exit);
    ...
    Build_OP(TOP_li, dest, Gen_Literal_TN(1, 4), ops);
    Build_OP(top_cmp, Gen_CR_TN(0) /*Gen_Literal_TN(0, 4)*/, src1_high, src2_high, ops);
    Build_OP(top_br1, Gen_CR_TN(0) /*Gen_Literal_TN(0, 4)*/, Gen_Label_TN(label_bb_exit, 0), ops);
    ...
    // we should remove this inst if this BB follow's bb_entry
    Build_OP(top_cmp, Gen_CR_TN(0) , src1_high, src2_high, bb_cmp_high_ops);
    Build_OP(top_br2, Gen_CR_TN(0) , Gen_Label_TN(label_bb_set_false, 0), bb_cmp_high_ops);
    ...
    OPS* bb_cmp_low_ops = &New_OPs;
    Build_OP(top_cmp, Gen_CR_TN(0) , src1_low, src2_low, bb_cmp_low_ops);
    Build_OP(top_br3, Gen_CR_TN(0) , Gen_Label_TN(label_bb_exit, 0), bb_cmp_low_ops);
...
void Handle_MaxMin_Longlong(bool max, TOP top_cmp, TN* dest_high, TN* dest_low, TN* src1_high, TN* src1_low, TN* src2_high, TN* src2_low, OPS* ops)
{
    ...
    OPS* bb_cmp_low_ops = &New_OPs;
    TN * cr = Gen_CR_TN(0);        
    Build_OP(TOP_cmplw, cr /*Gen_Literal_TN(0, 4)*/, src1_low, src2_low, bb_cmp_low_ops);
    Build_OP(top_br3, cr/*Gen_Literal_TN(0, 4)*/, Gen_Label_TN(label_bb_assign1, 0), bb_cmp_low_ops);
