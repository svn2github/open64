...
Expand_64Bit_Branch(TOP cmp, TN* targ, TN* src1_high, TN* src1_low, TN* src2_high, TN* src2_low, VARIANT variant, OPS *ops)
{
  ...
  TN* cr = Gen_CR_TN(2); // Gen_Literal_TN(2, 4);
...
{
  TN * src2_high  = src2;
  TN * src2_low   = Get_TN_Pair(src2_high);
  TN * src1_high  = src1;
  TN * src1_low   = Get_TN_Pair(src1);

  if (TN_has_value(src2)) {
    src2_high = Expand_Immediate_Into_Register(NULL, src2, MTYPE_I8, ops);
    src2_low  = Get_TN_Pair(src2_high);
  }
  else {
    if (!src2_low) {
      src2_low  = src2;
  ...
  cmp = Pick_Compare_TOP(&variant, &src1_high, &src2_high, ops);
...
Expand_Branch (TN *targ, TN *src1, TN *src2, VARIANT variant, OPS *ops)
{
  ...
  default:
      ...
      if (Get_TN_Pair(src1)) src1 = Get_TN_Pair(src1);
      if (Get_TN_Pair(src2)) src2 = Get_TN_Pair(src2);
      ...
      TN * cr = Gen_CR_TN(7);// Gen_Literal_TN(7, 4);
      ...
      if (!TOP_is_flop(cmp)) {  // integer
        ...
        switch (cond) {
          ...
          case V_BR_FGE:
          case V_BR_DGE:
            Build_OP(TOP_cror, cr, Gen_Literal_TN(2, 4), Gen_Literal_TN(29, 4), 
              Gen_Literal_TN(30, 4), cr, ops);
            break;
          case V_BR_FLE:
          case V_BR_DLE:
            Build_OP(TOP_cror, cr, Gen_Literal_TN(2, 4), Gen_Literal_TN(28, 4), 
              Gen_Literal_TN(30, 4), cr, ops);
            break;
          /* case V_BR_FNE:
          case V_BR_DNE:
            Build_OP(TOP_cror, Gen_Literal_TN(2, 4), Gen_Literal_TN(28, 4), Gen_Literal_TN(29, 4), ops);
            break;*/
          default:
        ...
        if (false_br) {
          Build_OP (TOP_bne, Gen_CR_TN(0), targ, ops);
        }
        else {
          Build_OP (TOP_beq, Gen_CR_TN(0), targ, ops);
        }
      }      
    }
}
...
void Exp_Call (OPERATOR opr, TN *return_address, TN *target, OPS *ops)
{
  ...
  case OPR_PICCALL:
    ...
    Build_OP(TOP_creqv, Gen_CR_TN(0), cnd, cnd, cnd, Gen_CR_TN(0), ops);
