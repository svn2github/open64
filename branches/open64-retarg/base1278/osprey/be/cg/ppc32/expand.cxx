...
static void  
Expand_64Bit_Int_Cmp(OPERATOR opr, TN* result, TN* src1_hi, 
  TN* src2_hi, TYPE_ID mtype, OPS* ops)
{
  FmtAssert((result != src1_hi) && (result != src2_hi), ("NYI"));

  TN* src1_lo = Get_TN_Pair(src1_hi);
  TN* src2_lo = Get_TN_Pair(src2_hi);
  TOP cmp;
  
  FmtAssert(src1_lo, ("Expand_64Bit_Int_Cmp src1_hi is NULL"));
    
  if (TN_has_value(src2_hi)) {
    src2_hi = Expand_Immediate_Into_Register(NULL, src2_hi, mtype, ops);
    src2_lo = Get_TN_Pair(src2_hi);
  }
  else {
    if (!src2_lo) {
      src2_lo = src2_hi;
      src2_hi = Get_64Bit_High_TN(src2_lo, MTYPE_is_signed(mtype), ops);
    }
  }
  
  cmp = MTYPE_is_signed(mtype) ? TOP_cmpw : TOP_cmplw;
  Handle_Cond_Longlong(opr, cmp, result, src1_hi, src1_lo, 
    src2_hi, src2_lo, ops);
}


static void Expand_64Bit_Cvtl(TN * result_h, TN * src_h, TN * len, 
  TYPE_ID mtype, TYPE_ID desc, OPS* ops)
{
  TN * result_l = Get_TN_Pair(result_h);
  TN * src_l    = Get_TN_Pair(src_h);
  BOOL signedex = MTYPE_is_signed(desc) || (MTYPE_bit_size(desc) > MTYPE_bit_size(mtype));

  INT64 l = TN_value(len);
  if (l == 32) {
    TN * low    = src_h;
    if (OP_NEED_PAIR(desc) && src_l) 
      low = src_l;
    
    Expand_Copy(result_l, low, MTYPE_I4, ops);
  }
  else if (l < 32) {
    TN * low    = src_h;
    if (OP_NEED_PAIR(desc) && src_l) 
    ...
    Expand_Convert_Length(result_l, low, len, mt, FALSE, ops);  // treat as unsigned
  }
  else {  // len > 32
    if (src_l) {  
      Expand_Copy(result_l, src_l, MTYPE_I4, ops);
      Expand_Convert_Length(result_h, src_h, Gen_Literal_TN(l-32, 4), MTYPE_I4, desc, ops);
    }
    else {  // 4byte->8byte
      Expand_Copy(result_l, src_h, MTYPE_I4, ops);      
    }
  }
  
  if (signedex) {
    if ((l <= 32) || (src_l == NULL)) {
      Build_OP(TOP_srawi, result_h, result_l, Gen_Literal_TN(31, 4), ops);
    }
  }
  else {
...
void Expand_64Bit_Unary_OP(OPERATOR opr, TYPE_ID mtype,
           TN * result_h, TN * src_h,
           OPS* ops)
{
  TN * result_l = Get_TN_Pair(result_h);
  TN * src_l    = Get_TN_Pair(src_h);
  FmtAssert(result_l != NULL, ("Expand_64Bit_Unary_OP result tn pair not setup"));  
  
  switch (opr){
  case OPR_INTCONST:
  {   
    INT64 val = TN_value(src_h);  // treat as signed value
  ...
  case OPR_NEG:
    ...
    Build_OP(TOP_subfic, result_l, result_l, Gen_Literal_TN(0, 4), ops);
    Build_OP(TOP_subfze, result_h, result_h, ops);
    break;
  case OPR_BNOT:
...
static void Expand_64Bit_Binary_OP(OPERATOR opr, TYPE_ID mtype,
            TN * result_h, TN * src1_h, TN * src2_h,
            OPS * ops)
{ 
  TN* result_l = Get_TN_Pair(result_h);
  TN* src1_l   = Get_TN_Pair(src1_h);
  TN* src2_l   = TN_has_value(src2_h) ? NULL : Get_TN_Pair(src2_h);

  if (TN_has_value(src1_h)) {
    src1_h = Expand_Immediate_Into_Register(NULL, src1_h, mtype, ops);
    src1_l = Get_TN_Pair(src1_h);
  }
  else if (src1_l == NULL){
    src1_l = src1_h;  
  ...
  if (TN_has_value(src2_h)){
    src2_h  = Expand_Immediate_Into_Register(NULL, src2_h, mtype, ops);
    src2_l  = Get_TN_Pair(src2_h);
  }
  else if (src2_l == NULL) {
    src2_l = src2_h;
  ...
  FmtAssert(result_l && src1_l && src2_l, ("Expand_64Bit_Binary_OP"));  
  ...
  case OPR_BXOR:
    ...
    break;
  case OPR_BNOR:
    Build_OP(TOP_nor, result_l, result_l, result_l, ops);
    Build_OP(TOP_nor, result_h, result_h, result_h, ops);
    break;
  default:
...
static void Expand_64Bit_Multiply(TN* result_h, TN* src1_h, TN* src2_h, TYPE_ID mtype, OPS* ops)
{
  TN* result_l = Get_TN_Pair(result_h);
  TN* src1_l   = Get_TN_Pair(src1_h);
  TN* src2_l   = TN_has_value(src2_h) ? NULL : Get_TN_Pair(src2_h);
  ...
  Build_OP(TOP_mullw,  result_h,  src1_h,    src2_l, ops);
  Build_OP(TOP_mullw,  result_l,  src1_l,    src2_h, ops);
  Build_OP(TOP_add,    result_h,  result_h,  result_l, ops);
  Build_OP(TOP_mulhwu, result_l,  src1_l,    src2_l, ops);
  Build_OP(TOP_add,    result_h,  result_h,  result_l, ops);
...
static void Expand_64Bit_Shift(SHIFT_DIRECTION shift_dir,
        TN* result_h, TN* src_h, TN* shift, TYPE_ID mtype, OPS* ops)
{ 
  TN * result_l = Get_TN_Pair(result_h);
  TN * src_l    = Get_TN_Pair(src_h);
  ...
  FmtAssert(result_l && src_h && src_l, ("Expand_64Bit_Shift result or operand1 TN pair not setup"));
  ...
  if (1){
    ...
    if (shift_amt > 32){
      ...
      TN * tmp = Build_TN_Like(result_l);
      switch (shift_dir){
      case shift_left:
        Build_OP(TOP_srwi, tmp, src_l, Gen_Literal_TN(32-shift_amt, 4), ops);
        Build_OP(TOP_slwi, result_h, src_h, Gen_Literal_TN(shift_amt, 4), ops);
        Build_OP(TOP_or,   result_h, result_h, tmp, ops);
      ...
      case shift_lright:
        ...
        Build_OP(TOP_srwi, result_l, src_l, Gen_Literal_TN(shift_amt, 4), ops);
        Build_OP(TOP_rlwimi, result_l, src_h, Gen_Literal_TN(32-shift_amt, 4), 
          Gen_Literal_TN(0, 0), Gen_Literal_TN(shift_amt-1, 4), result_l, ops);        
        break;
      }
    }
  } 
...
void Expand_Vararg_Float_Flag(BOOL bfloat, OPS * ops)
{
  ...
  TN * cr = Gen_CR_TN(0);
  Build_OP(top, cr, tn, tn, tn, cr, ops);
}
...
void
Expand_Copy (TN *result_h, TN *src_h, TYPE_ID mtype, OPS *ops)
{
  if (OP_NEED_PAIR(mtype)) {    
    ...
    else if (result_l == NULL) {  // I8/U8 ==>> U4/I4
      DevWarn("Expand_Copy : result TN pair not setup");
      FmtAssert(src_l, ("Expand_Copy : src TN pair not setup"));
      Build_OP(TOP_mr, result_h, src_l, ops);
      Set_OP_copy(OPS_last(ops));
    }
    else {  // U4/I4 ==>> U8/I8 
      Build_OP(TOP_mr, result_l, src_h, ops);
      Set_OP_copy(OPS_last(ops));
      result_h = Get_64Bit_High_TN(src_h, MTYPE_signed(mtype), ops);
    }
  }
  else if (MTYPE_is_float(mtype)) {
    Build_OP(TOP_fmr, result_h, src_h, ops);
    Set_OP_copy(OPS_last(ops));
  }
  else {
    Build_OP(TOP_mr, result_h, src_h, ops);
...
Expand_Convert_Length (TN *dest, TN *src, TN *length_tn, TYPE_ID mtype, TYPE_ID desc, OPS *ops)
{
  ...
  BOOL signed_extension = MTYPE_is_signed(desc) || (MTYPE_bit_size(desc) > MTYPE_bit_size(mtype));
  ...
  if (OP_NEED_PAIR(mtype)) {    // 64bit result
    ...
    if (Get_TN_Pair(src)) // 64bit -> 32bit...
      src = Get_TN_Pair(src);
    
    if (val == 32) {
      Build_OP(TOP_mr, dest, src, ops);
    }    
    else if (!signed_extension && val <= 16) {  // vale == 16      
      Build_OP(TOP_andi_, dest, src, Gen_Literal_TN((1 << val) - 1, 4), ops);  
    }
    else {
      Build_OP(TOP_slwi, dest, src, Gen_Literal_TN(32-val, 4), ops);
      TOP srw = signed_extension ? TOP_srawi : TOP_srwi;
      Build_OP(srw, dest, dest, Gen_Literal_TN(32-val, 4), ops);
    }
  }
}
...
Exp_Immediate (TN *dest, TN *src, TYPE_ID mtype, OPS *ops)
{
  ...
  if (OP_NEED_PAIR(mtype) && Get_TN_Pair(dest)){
    ...
    if (!Get_TN_Pair(dest))Add_TN_Pair(dest, Build_TN_Like(dest));
    Expand_64Bit_Unary_OP(OPR_INTCONST, MTYPE_I8, dest, src, ops );   
  }  
    ...
    if(ST_gprel(sym)) {
      ...
      Build_OP(TOP_li, dest, Gen_Literal_TN((val >> 48) & 0xffff, 8), ops);
      Build_OP(TOP_ori, dest, dest, Gen_Literal_TN((val >> 32) & 0xffff, 8), ops);
      DevWarn("Long Long value is not supported, constant value has been truncated");            
   }
 } 
}
...
{ 
  ...
  if (TN_has_value(imm)) {
    INT64 val = -TN_value(imm);
    FmtAssert((val < 0), ("SR adjustment value wrong!"));    
    TN *adj = Gen_Literal_TN(val, TN_size(imm));    
    if (ISA_LC_Value_In_Class (val, LC_simm16)) {
      Build_OP(TOP_stwu, result, result, adj, ops);
    } else {
      FmtAssert(FALSE, ("NOT Verify"));
      TN * ofst = Gen_Register_TN(ISA_REGISTER_CLASS_integer, Pointer_Size);
      short hi = (short)((val >> 16) & 0xffff);
      unsigned short low = (val & 0xffff);
      Build_OP(TOP_lis, ofst, Gen_Literal_TN(hi, Pointer_Size), ops);
      Build_OP(TOP_ori, ofst, ofst, Gen_Literal_TN(low, Pointer_Size), ops);
      Build_OP(TOP_stwux,result, result, ofst, ops);
    }
  }else{
    FmtAssert((TN_register_class(imm) == ISA_REGISTER_CLASS_integer), ("SR reg class error"));
    Expand_Neg(imm, imm, MTYPE_I4, ops);
...
Expand_Add (TN *result, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (OP_NEED_PAIR(mtype)) {
    if  (!Get_TN_Pair(result)) Add_TN_Pair(result, Build_TN_Like(result));
...
Expand_Abs (TN *dest, TN *src, TYPE_ID mtype, OPS *ops)
{
  if (MTYPE_is_float(mtype)) {
    ...
    if (OP_NEED_PAIR(mtype)) {
      ...
      TN * dest_low = Get_TN_Pair(dest);
      TN * src_low = Get_TN_Pair(src); 
      TN * dest_hi = dest;
      TN * src_hi  = src;
      FmtAssert(dest_low, ("Expand_Abs result tn pair not setup"));
      FmtAssert(src_low , ("Expand_Abs src tn pair not setup"));      
      
      Build_OP(TOP_srawi, mask,     src_hi, Gen_Literal_TN(31, 4), ops);
      Build_OP(TOP_xor,   dest_hi,  src_hi, mask, ops);
      Build_OP(TOP_xor,   dest_low, src_low, mask, ops);
      Build_OP(TOP_subfc, dest_low, mask, dest_low, ops);
      Build_OP(TOP_subfe, dest_hi,  mask, dest_hi, ops);      
    }
    else {
...
Expand_Cond(TOP top, TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (TN_is_constant(src2)) {
    ...
    if (ISA_LC_Value_In_Class (val, LC_simm16)) 
      opcmp = MTYPE_signed(mtype) ? TOP_cmpwi : TOP_cmplwi;
    else {
      src2 = Expand_Immediate_Into_Register(src2, FALSE, ops);
      opcmp = MTYPE_signed(mtype) ? TOP_cmpw : TOP_cmplw;
    }
  }
  else { 
  ...
  Build_OP(opcmp, Gen_Literal_TN(0, 4), src1, src2, ops);
  Build_OP(top, Gen_Literal_TN(0, 4), Gen_Label_TN(label_bb_exit, 0), ops);
...
Expand_Int_Cmp(OPERATOR opr, TOP top, TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  if (dest_hi) {
    Build_OP(TOP_li, dest_hi, Gen_Literal_TN(0, 4), ops);
  }
}
...
Expand_Int_Less (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  // Expand_Cond(TOP_blt, dest, src1, src2, mtype, ops);
}
...
Expand_Int_Less_Equal (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  // Expand_Cond(TOP_ble, dest, src1, src2, mtype, ops);
}
...
Expand_Int_Equal (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  // Expand_Cond(TOP_beq, dest, src1, src2, mtype, ops);
}
...
Expand_Int_Not_Equal (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  // Expand_Cond(TOP_bne, dest, src1, src2, mtype, ops);
}
...
Expand_Int_Greater_Equal (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  // Expand_Cond(TOP_bge, dest, src1, src2, mtype, ops);
}
...
Expand_Int_Greater (TN *dest, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{
  ...
  /*
  if (Get_TN_Pair(dest) || OP_NEED_PAIR(mtype)) {
    FmtAssert(Get_TN_Pair(dest), ("Expand_Int_Equal tn result pair not setup"));
    if (Get_TN_Pair(src1))
      Expand_64Bit_Int_Cmp(OPR_GT, dest, src1, src2, mtype, ops);
    else
      Expand_Cond(TOP_bgt, dest, src1, src2, mtype, ops);
    Build_OP(TOP_li, Get_TN_Pair(dest), Gen_Literal_TN(0, 4), ops);
  }
  else {  
    Expand_Cond(TOP_bgt, dest, src1, src2, mtype, ops);
  }*/
...
Expand_Float_To_Float (TN *dest, TN *src, TYPE_ID rtype, TYPE_ID desc, OPS *ops)
{
  ...
  {
    FmtAssert(FALSE, ("PPC F8->F4 need do nothing"));
  }
}


void
Expand_Int_To_Float (TN *dest, TN *src_h, TYPE_ID imtype, TYPE_ID fmtype, OPS *ops)
{
  if (OP_NEED_PAIR(imtype)) {
    TN * src_l = Get_TN_Pair(src_h);
    FmtAssert(src_l, ("Expand_Int_To_Float U8 src tn pair not setup"));
    FmtAssert(MTYPE_is_unsigned(imtype), ("Expand_Int_To_Float can not handle I8"));
    
    Handle_ULonglong_Float_Cvt(dest, src_h, src_l, ops, MTYPE_is_size_double(fmtype));
  }
  else {
    Handle_Int_Float_Cvt(dest, src_h, ops, MTYPE_is_unsigned(imtype), fmtype == MTYPE_F8);
  }
}
...
void Expand_Select_Def(TOP top_cmp, TOP branch, TN * dest_tn, 
  TN * cmp1, TN * cmp2, TN * true_tn, TN * false_tn, TYPE_ID mtype, OPS * ops)
{  
  BB* bb_entry        = Cur_BB;
  BB* bb_set_true     = Gen_And_Append_BB(bb_entry);
  BB* bb_exit         = Gen_And_Append_BB(bb_set_true);
  ...
  Build_OP(top_cmp, Gen_CR_TN(3) /*Gen_Literal_TN(3, 4)*/, cmp1, cmp2, ops);
  Build_OP(branch, Gen_CR_TN(3) /*Gen_Literal_TN(3, 4)*/, Gen_Label_TN(label_bb_exit, 0), ops);
  ...
  OPS* bb_set_ops = &New_OPs;
  Expand_Copy(dest_tn, false_tn, mtype, bb_set_ops);
  total_bb_insts = 0;
  Last_Processed_OP = NULL;
  Process_New_OPs();
  BB_Append_Ops(bb_set_true, bb_set_ops);
...
Expand_Select (TN *dest_tn, TN *cond_tn, TN *true_tn, TN *false_tn, 
  TYPE_ID mtype, VARIANT variant, OPS *ops)
{
  // FmtAssert(FALSE, ("Expand_Select : NYI"));  
  DevWarn("Expand_Select : not verified");
#if 1
  TN * zo    = NULL;
  TOP cmp    = TOP_UNDEFINED;
  TOP branch = TOP_bne; // Cond_To_Top(variant);
;
  ...
  Expand_Select_Def(cmp, branch, dest_tn, cond_tn, zo, true_tn, false_tn, mtype, ops);
#if 0    
    ...
    //Build_OP(TOP_mc_z_ne, dest_tn, true_tn, false_tn, tmp_tn, ops); 
      Build_OP(TOP_mc_z_ne, dest_tn, tmp_tn, true_tn, ops);
      Build_OP(TOP_mc_z_eq, dest_tn, tmp_tn, false_tn, ops);
      Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_UNC_DEF);
    }
  } else {
    FmtAssert(FALSE, ("UNIMPLEMENTED"));
...
static void
Expand_MinMax_All (TN *dest, TN *dest2, TN *src1, TN *src2, TYPE_ID mtype, OPS *ops)
{ 
  if (MTYPE_is_float(mtype)) {
    FmtAssert(FALSE, ("Not IMP"));
    Set_OP_cond_def_kind(OPS_last(ops), OP_ALWAYS_UNC_DEF);
    ...
    TN * op1, * op2;
    op1 = Gen_Register_TN(ISA_REGISTER_CLASS_integer, 4);
    op2 = Gen_Register_TN(ISA_REGISTER_CLASS_integer, 4);
    // Build_OP(TOP_mr, op1, src1, ops);
    // Build_OP(TOP_mr, op2, src2, ops);
    if (is_signed){      
    ...
    Build_OP(TOP_subfc, op2, op2, op1, ops);
    Build_OP(TOP_subfe, op1, op1, op1, ops);
    if (dest){
      // TN * tmp3 = Gen_Register_TN(ISA_REGISTER_CLASS_integer, 4);
      Build_OP(TOP_and, op2, op2, op1, ops);
      Build_OP(TOP_add, dest, op2, src2, ops); 
    }
    if (dest2){
      Build_OP(TOP_andc, op2, op2, op1, ops);
      Build_OP(TOP_add, dest2, op2, src2, ops); 
    } 
  ...
  TYPE_ID mtype, OPS * ops)
{
  ...
  if (destMin)
    Exp_COPY(destMin, src1, ops);
  if (destMax)
    Exp_COPY(destMax, src2, ops);
  
  Build_OP(opcmp, Gen_CR_TN(4) /*Gen_Literal_TN(0, 4)*/, src1, src2, ops);
  Build_OP(TOP_blt, Gen_CR_TN(4) /*Gen_Literal_TN(0, 4)*/, Gen_Label_TN(label_bb_exit, 0), ops);
  ...
  OPS* bb_set_ops = &New_OPs;
  if (destMin)
    Exp_COPY(destMin, src2, ops);
  if (destMax)
    Exp_COPY(destMax, src1, ops);
  ...
  if (destMin) 
    Exp_COPY(destMin, src1, &new_ops);
  if (destMax)
    Exp_COPY(destMax, src2, &new_ops);
  
  TN * fcr      = Gen_Literal_TN(4, 4);
  Build_OP(cmp, fcr, src1, src2, &new_ops);
  
  TOP branch = TOP_blt;

  BB * bb_entry         = Cur_BB;
  BB * bb_false         = Gen_And_Append_BB(bb_entry);
  BB * bb_exit          = Gen_And_Append_BB(bb_false);
  LABEL_IDX  lab_false  = Gen_Label_For_BB(bb_false);
  LABEL_IDX  lab_exit   = Gen_Label_For_BB(bb_exit);

  BB_branch_wn(bb_entry) = WN_Create(OPC_TRUEBR,1);
  WN_kid0(BB_branch_wn(bb_entry)) = NULL;
  WN_label_number(BB_branch_wn(bb_entry)) = lab_exit;
    
  
  TN * tn_lab_exit      = Gen_Label_TN(lab_exit, 0);
  Build_OP(branch, fcr, tn_lab_exit, &new_ops);
  ...
  OPS_Init(&New_OPs);
  if (destMin) 
    Exp_COPY(destMin, src2, &New_OPs);
  if (destMax)
    Exp_COPY(destMax, src1, &New_OPs);

  total_bb_insts = 0;
  Last_Processed_OP = NULL;
  Process_New_OPs();
  BB_Append_Ops(bb_false, &New_OPs);
...
{ 
  ...
  if (MTYPE_is_float(mtype)) {
    Expand_Float_MinMax(dest, dest2, src1, src2, mtype, ops);
  }
  else {
    Expand_MinMax_All(dest, dest2, src1, src2, mtype, ops);
  }
}
...
{
  DevWarn("Exp_Select_And_Condition : HERE");
  ...
  // Is_True(select == OPC_U4SELECT || select == OPC_I4SELECT, ("Exp_Select_And_Condition:: select opr must be OPC_U4SELECT or OPC_I4SELECT"));
  ...
  Exp_COPY(result, true_tn, ops);
  
  Build_OP(cmp, Gen_CR_TN(3), cmp_kid1, cmp_kid2, ops);
  Build_OP(Branch_OPR2TOP(OPCODE_operator(compare)), Gen_CR_TN(3), 
    Gen_Label_TN(label_bb_exit, 0), ops);
  ...
  OPS* bb_set_ops = &New_OPs;
  Exp_COPY(result, false_tn, &New_OPs);
...
Expand_Sqrt (TN *result, TN *src, TYPE_ID mtype, OPS *ops)
{
  ...
  DevWarn("Expand_Sqrt : HERE ");
  
  TOP top = MTYPE_is_size_double(mtype) ? TOP_frsqrt:TOP_frsqrts;
...
{ 
  ...
  if (OP_code(br_op) == TOP_beq || OP_code(br_op) == TOP_bne) {
    ...
    Build_OP(TOP_xor, tmp_tn, OP_opnd(br_op, 0), OP_opnd(br_op, 1), 
       &new_ops);
    Build_OP(TOP_cmplwi, false_tn, tmp_tn, Gen_Literal_TN(1, 4), &new_ops);
    Build_OP(TOP_cmplw, true_tn, Zero_TN, tmp_tn, &new_ops);
  ...
  break;
    ...
    last_false_tn = false_tn;
    return;           
  } else if (OP_code(br_op) == TOP_bge || OP_code(br_op) == TOP_bltz) {
    OPS new_ops;
    OPS_Init(&new_ops);
    true_tn = Gen_Register_TN(ISA_REGISTER_CLASS_integer, 4);
    false_tn = Gen_Register_TN(ISA_REGISTER_CLASS_integer, 4);
    TN *tmp_tn = Gen_Register_TN(ISA_REGISTER_CLASS_integer, 4);
    TN *tmp1_tn;
    Build_OP(TOP_cmpw, tmp_tn, OP_opnd(br_op, 0), Zero_TN,
       &new_ops);
    Build_OP(TOP_xori, true_tn, tmp_tn, Gen_Literal_TN(1, 4), &new_ops);
    Build_OP(TOP_cmpwi, false_tn, OP_opnd(br_op, 0), Gen_Literal_TN(0, 4), 
       &new_ops);
    if (OP_code(br_op) == TOP_bge) {
      tmp1_tn = true_tn;
      true_tn = false_tn;
      false_tn = tmp1_tn;
    }
