                  ...
                  TN *tgt_tn, TN *src_tn, OPS *ops)
{
  ...
  if (pos == 0 && bit_size <= 16 && ! MTYPE_signed(rtype)) {
    Build_OP(TOP_andi_, tgt_tn, src_tn,
  ...
  if (right_shift_amt > 31) {
    ...
    Is_True(0,("shift > 31 bits"));
#endif
    // right_shift_op = TOP_dsra32; // ? YU
    right_shift_amt -= 32;
  }
  if (!MTYPE_signed(rtype)) {
#if defined(TARG_PPC32)
    if (right_shift_op == TOP_srawi)
      right_shift_op = TOP_srwi;
    else
      right_shift_op = TOP_srwi; // ???? SC
#else
...
#pragma mips_frequency_hint NEVER
  ...
  if (offset_succ == offset_pred &&
      size_pred == size_succ) {
    Build_OP(TOP_mr, OP_result(op, 0), OP_opnd(pred_op, 0), &ops);
  ...
  TYPE_ID rtype, desc;
  if (OP_code(pred_op) == TOP_stb)
    desc = MTYPE_I1;
  else if (OP_code(pred_op) == TOP_sth)
    desc = MTYPE_I2;
  else if (OP_code(pred_op) == TOP_stw)
    desc = MTYPE_I4;
  else 
    desc = MTYPE_I8;
  BOOL unsigned_op = (OP_code(op) == TOP_lbz ||
		      OP_code(op) == TOP_lhz ||
		      OP_code(op) == TOP_lwz);
  if (desc == MTYPE_I1) {
    if (unsigned_op)
...
Convert_Imm_Add (OP *op, TN *tnr, TN *tn, INT64 imm_val, EBO_TN_INFO *tninfo)
{
  ...
  static int pw = 1;
  if (pw) {
    DevWarn("ebo_special.cxx:Convert_Imm_Add not verified!");
    pw  = 0;
  }
...
Resolve_Conditional_Branch (OP *op, TN **opnd_tn)
{
  ...
  if (taken)
  {
    ...
    Build_OP (TOP_b, OP_opnd(op,TOP_Find_Operand_Use(OP_code(op),OU_target)), &ops);
    BB_Insert_Ops(OP_bb(op), op, &ops, FALSE);
                          ...
                          EBO_TN_INFO **opnd_tninfo)
{
  ...
  {
    if (opcode == TOP_subf) // careful it is subf
    {
      result_val = tn1_val - tn0_val; // 
      goto Constant_Created;
    }
  }
  ...
  else if (opcode == TOP_srawi)
  {
    result_val = tn0_val >> tn1_uval;
    goto Constant_Created;
  }
  else if (opcode == TOP_srwi)
  {
...
{  
  ...
  if (OP_copy(op)) {
    ...
    if (topcode == TOP_add || topcode == TOP_fadd ||        
        topcode == TOP_or || 
        topcode == TOP_mr || topcode== TOP_fmr)
      return 0;    
  }
	     ...
	     BS *in_defined_tns, BS **out_defined_tns)
{
  ...
  *len = BB_length(bb);
  ...
  if (jump && ((*len == 1) || (OP_code(jump) != TOP_b)))
    return NULL;
...
void Redundancy_Elimination()
{
  ...
  static int pw = 1;
  if (pw) {
    DevWarn("ebo_special.cxx:Redundancy_Elimination not verified.");
    pw  = 0;
  }
  ...
  for( BB* bb = REGION_First_BB; bb != NULL; bb = BB_next( bb ) ){
	...
	continue; // end for this op
      ...
      if (OP_code( op ) != TOP_srwi && 
	  OP_code( op ) != TOP_srawi && 
	  OP_code( op ) != TOP_slwi /*&& 
	  OP_code( op ) != TOP_dsrl && 
	  OP_code( op ) != TOP_dsra && 
	  OP_code( op ) != TOP_dsll && 
	...
	for( int i = 0; i < OP_opnds( dw_op ); i++ ){
	  if( TN_is_register(OP_opnd( dw_op, i)) && 
	    ...
	    Set_OP_opnd( dw_op, i, Gen_Literal_TN(0, 4));
	    done = TRUE;
	  }
	}
