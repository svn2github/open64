...
Handle_Misc_MType(TYPE_ID rtype, TYPE_ID mtype, TN * result, OPS * ops)
{
  if ((rtype == MTYPE_I4) && (mtype == MTYPE_I1)) {
    TN * t = Build_TN_Like(result);
    Build_OP(TOP_extsb, t, result, ops);
    Exp_COPY(result, t, ops);
  }
    ...
    TN* result, TN* base, TN* ofst, OPS* ops)
{
  ...
  TN * tn_h = Get_TN_Pair(result);
  FmtAssert(tn_h, ("Expand_64Bit_Load : result tn pair not setup"));  
  
  if (!tn_h) {
    tn_h = Create_TN_Pair(result, new_mtype);
  }
  ...
  if (MTYPE_is_size_double(desc)){
    ...
    if (TN_is_register(ofst)) { 
      Expand_Add(ofst, ofst, Gen_Literal_TN(4, Pointer_Size), MTYPE_I4, ops);
      ofst_h = ofst;
    }
    else {
    ...
    Build_OP(new_top, tn_h, base, ofst, ops);
    Handle_Misc_MType(new_mtype, new_desc, tn_h, ops);
    if (rtype == MTYPE_I8)
      Build_OP(TOP_srawi, result, tn_h, Gen_Literal_TN(31, 4), ops);    
    else
      Build_OP(TOP_li, result, Gen_Literal_TN(0, 4), ops);
  }
}
...
Expand_Load (OPCODE opcode, TN *result, TN *base, TN *ofst, OPS *ops)
{
  ...
  if (notFixImm) {
    ...
    Build_OP(top, result, base, tmp_tn, ops);
  }
  else {    
    Build_OP(top, result, base, ofst, ops);
  }
  
  Handle_Misc_MType(rtype, mtype, result, ops);
}
...
Expand_Store (TYPE_ID mtype, TN *src, TN *base, TN *ofst, OPS *ops)
{
  ...
  TOP        top = Pick_Store_Instruction(mtype, bNeedX);
	...
	if (Get_TN_Pair(src))src = Get_TN_Pair(src);
  FmtAssert(src, ("NYI"));
	if (notFixImm) {
    TN * tmp_tn = Expand_Immediate_Into_Register(NULL, ofst, MTYPE_U4, ops);
    ...
    switch (desc) {
      ...
      case MTYPE_U8:
        ...
        Build_OP(TOP_rlwinm, result, result, Gen_Literal_TN(24, 4), 
            Gen_Literal_TN(0, 4), Gen_Literal_TN(7, 4), result, ops);
        ...
        /* if (!res_h) {
          ...
          DevWarn("Expand_Composed_Load : TN pair is null");
...
{
  DevWarn("Expand_Composed_Store Not verified");
...
#endif   
{
  ...
  if (base_sym == SP_Sym || base_sym == FP_Sym) {
    ...
    if (sym == base_sym) {
      ...
      if (ISA_LC_Value_In_Class(base_ofst, LC_simm16))
        ...
        Build_OP(TOP_li, tmp_tn, 
             Gen_Literal_TN(base_ofst >> 16, 4), &newops);
        Build_OP(TOP_addi, tmp_tn, tmp_tn, base_tn, &newops);
        base_tn = tmp_tn;
        ofst_tn = Gen_Literal_TN (base_ofst & 0xffff, 4); */
        if (is_lda) {
          tmp_tn = Build_TN_Of_Mtype(Pointer_Mtype);
          Expand_Add(tmp_tn, base_tn, Gen_Literal_TN(ofst, Pointer_Size), MTYPE_U4, &newops);
          base_tn = tmp_tn;
          ofst_tn = Gen_Literal_TN(0, Pointer_Size);
        }
        else {
          ofst_tn = Expand_Immediate_Into_Register(NULL, Gen_Literal_TN(ofst, Pointer_Size), MTYPE_U4, &newops);
        }
      }
    }
    else {
      ...
      if (ISA_LC_Value_In_Class(base_ofst, LC_simm16))
        ...
        /*
        tmp_tn = Build_TN_Of_Mtype(Pointer_Mtype);
        Build_OP(TOP_li, tmp_tn, Gen_Symbol_TN(sym, ofst, TN_RELOC_HIGH16), &newops);
        Build_OP(TOP_addi, tmp_tn, tmp_tn, base_tn, &newops);
        base_tn = tmp_tn;
  ...
  if (is_store) {
    ...
    if (TN_is_symbol(ofst_tn)) 
      Build_OP(TOP_addi, tn, base_tn, ofst_tn, &newops);
          ...
          TN *tgt_tn, TN *src_tn, OPS *ops)
{
  ...
  if (OP_NEED_PAIR(rtype)) {
    TN * tgt_low = Get_TN_Pair(tgt_tn);
    FmtAssert(tgt_low, ("Exp_Extract_Bits : I8 <- I8 result tn pair not setup"));
    
    if (OP_NEED_PAIR(desc)) {  // I8 <- I8
      TN * src_low = Get_TN_Pair(src_tn);
      FmtAssert(src_low, ("Exp_Extract_Bits : I8 <- I8 source tn pair not setup"));

      if (bit_offset< 32){
        if (bit_offset + bit_size <= 32){        
          UINT adjust_offset = (bit_offset + bit_size) % 32;
          Build_OP(TOP_rlwinm, tgt_low, src_tn, Gen_Literal_TN(adjust_offset, 4), 
            Gen_Literal_TN(32-bit_size, 4), Gen_Literal_TN(31, 4), tgt_low, ops);
        }
        else if (bit_size <= 32){
          UINT size_low = 32 - bit_offset;        
          Build_OP(TOP_rlwinm, tgt_low, src_tn, Gen_Literal_TN(bit_offset, 4), 
            Gen_Literal_TN(32-bit_size, 4), Gen_Literal_TN(32-bit_size+size_low-1, 4), tgt_low, ops);
          Build_OP(TOP_rlwimi, tgt_low, src_low, Gen_Literal_TN(bit_size-size_low, 4),
            Gen_Literal_TN(32-(bit_size-size_low), 4), Gen_Literal_TN(31, 4), tgt_low, ops);
          
        }
        else {
          UINT size_low = 32;   
          UINT adj_off  = (bit_offset + bit_size - 32) % 32;
          Build_OP(TOP_rlwinm, tgt_low, src_low, Gen_Literal_TN(adj_off, 4), 
            Gen_Literal_TN(64-(bit_offset+bit_size), 4), Gen_Literal_TN(31, 4), tgt_low, ops);

          if (adj_off > 0)
            Build_OP(TOP_rlwimi, tgt_low, src_tn, Gen_Literal_TN(adj_off, 4),
              Gen_Literal_TN(0, 4), Gen_Literal_TN(31-adj_off, 4), tgt_low, ops);
         
          Build_OP(TOP_rlwinm, tgt_tn, src_tn, Gen_Literal_TN(adj_off, 4), 
            Gen_Literal_TN(64-bit_size, 4), Gen_Literal_TN(31, 4), tgt_tn, ops);
        }
      }
      else {  // bit_off >= 32
        UINT adjust_offset = (bit_offset+bit_size - 32) % 32;
        Build_OP(TOP_rlwinm, tgt_low, src_tn, Gen_Literal_TN(adjust_offset, 4), 
          Gen_Literal_TN(32-bit_size, 4), Gen_Literal_TN(31, 4), tgt_low, ops);       
      }      
    }
    else {  // I8 <- I4
      FmtAssert(bit_offset < 32, ("Exp_Extract_Bits : I8 <- I4 bit offset out of limit"));
      FmtAssert((bit_offset + bit_size) < 32, ("Exp_Extract_Bits : I8 <- I4 bits out of limit"));
      
      UINT adjust_offset = (bit_offset + bit_size) % 32;
      Build_OP(TOP_rlwinm, tgt_low, src_tn, Gen_Literal_TN(adjust_offset, 4), 
        Gen_Literal_TN(32-bit_size, 4), Gen_Literal_TN(31, 4), tgt_low, ops);

      if (MTYPE_is_signed(rtype) && bit_size < 32) {
        Build_OP(TOP_slwi,  tgt_low, tgt_low, Gen_Literal_TN(32-bit_size, 4), ops);
        Build_OP(TOP_srawi, tgt_low, tgt_low, Gen_Literal_TN(32-bit_size, 4), ops);
      }
    }

    if (MTYPE_is_signed(rtype)) {
      if (bit_size < 32) {
        Build_OP(TOP_slwi,  tgt_tn, tgt_low, Gen_Literal_TN(32-bit_size, 4), ops);
        Build_OP(TOP_srawi, tgt_tn, tgt_tn, Gen_Literal_TN(32-bit_size, 4), ops);
      }
      else if (bit_size == 32) {
        Build_OP(TOP_srawi, tgt_tn, tgt_low, Gen_Literal_TN(31, 4), ops);
      }
      else  {
        Build_OP(TOP_slwi, tgt_tn, tgt_tn, Gen_Literal_TN(64-bit_size, 4), ops);
        Build_OP(TOP_srawi, tgt_tn, tgt_tn, Gen_Literal_TN(64-bit_size, 4), ops);
      }
    }
    else {
      Build_OP(TOP_li, tgt_tn, Gen_Literal_TN(0, 4), ops);
    }
  } // end if (OP_NEED_PAIR(rtype))
  else {
    if (OP_NEED_PAIR(desc)) { // I4 <- I8
      TN * src_low = Get_TN_Pair(src_tn);
      FmtAssert(src_low, ("Exp_Extract_Bits : I4 <- I8 src tn pair not setup"));
      FmtAssert(bit_size <= 32, ("Exp_Extract_Bits : I4 <- I8  result bit size out of limit"));

      if (bit_offset + bit_size <= 32) {
        UINT adjust_offset = (bit_offset + bit_size) % 32;
        Build_OP(TOP_rlwinm, tgt_tn, src_low, Gen_Literal_TN(adjust_offset, 4), 
          Gen_Literal_TN(32-bit_size, 4), Gen_Literal_TN(31, 4), tgt_tn, ops);
      }
      else {
        UINT size_low = 32 - bit_offset;
        if (bit_offset == 32 && bit_size == 32) {
          Expand_Copy(tgt_tn, src_tn, rtype, ops);
        }
        else {
          Build_OP(TOP_rlwinm, tgt_tn, src_low, Gen_Literal_TN(0, 4), 
            Gen_Literal_TN(32-bit_size, 4), Gen_Literal_TN(32-bit_size+size_low, 4), tgt_tn, ops);
          Build_OP(TOP_rlwimi, tgt_tn, src_tn, Gen_Literal_TN(bit_size-size_low, 4),
            Gen_Literal_TN(31-(bit_size-size_low), 4), Gen_Literal_TN(31, 4), tgt_tn, ops);
        }
      }
    }
    else {  // I4 <- I4
      ...
      UINT adjust_offset = (bit_offset+bit_size+rsize-dsize) % 32;
      Build_OP(TOP_rlwinm, tgt_tn, src_tn, Gen_Literal_TN(adjust_offset, 4), 
        Gen_Literal_TN(32-bit_size, 4), Gen_Literal_TN(31, 4), tgt_tn, ops);
    }
  ...
  if (bit_offset < 32 ) {
    UINT adjust_offset = (bit_offset + bit_size) % 32;
    UINT bits = (bit_size <= 32) ? bit_size : (bit_size - 32);
    Build_OP(TOP_rlwinm, tgt_tn, src_tn, Gen_Literal_TN(adjust_offset, 4), 
      Gen_Literal_TN(32-bits, 4), Gen_Literal_TN(31, 4), ops);

    if (MTYPE_is_signed(rtype)) {
      Build_OP(TOP_slwi,  tgt_tn, tgt_tn, Gen_Literal_TN(32 - bits, 4), ops);
      Build_OP(TOP_srawi, tgt_tn, tgt_tn, Gen_Literal_TN(32 - bits, 4), ops);
    }
  }

  if ((bit_size + bit_offset) > 32) {
    FmtAssert(OP_NEED_PAIR(rtype), ("Exp_Extract_Bits: result type seem wrong"));
    TN * tgt_high = Get_TN_Pair(tgt_tn);
    TN * src_high = Get_TN_Pair(src_tn);
    FmtAssert(tgt_high && src_high, ("Exp_Extract_Bits: TN pair not setup"));

    if (bit_offset >= 32) {
      bit_offset = bit_offset - 32;
    } 
    else {
      bit_size   = bit_size + bit_offset - 32;
      bit_offset = 0;
    } 

    UINT adjust_offset = (bit_offset + bit_size) % 32;
    Build_OP(TOP_rlwinm, tgt_high, src_high, Gen_Literal_TN(adjust_offset, 4), 
      Gen_Literal_TN(32-bit_size, 4), Gen_Literal_TN(31, 4), ops);
    if (MTYPE_is_signed(rtype)) {
      Build_OP(TOP_slwi,  tgt_tn, tgt_tn, Gen_Literal_TN(32 - bit_size, 4), ops);
      Build_OP(TOP_srawi, tgt_tn, tgt_tn, Gen_Literal_TN(32 - bit_size, 4), ops);
    }
  ...
  TN *tgt_tn, TN *src1_tn, TN *src2_tn, OPS *ops)
{
  UINT rsize = MTYPE_bit_size(rtype) <= 32 ? MTYPE_bit_size(rtype) : 32;
  UINT dsize = MTYPE_bit_size(desc) <= 32 ? MTYPE_bit_size(desc) : 32;
  FmtAssert(bit_offset + bit_size <= MTYPE_bit_size(desc), ("Exp_Deposit_Bits : bit out of range"));

  if (!OP_NEED_PAIR(rtype) && !OP_NEED_PAIR(desc)) { // I4 <- I4
    UINT adjust_offset = (dsize - (bit_offset + bit_size)) % dsize;
    Build_OP(TOP_rlwimi, src1_tn, src2_tn, Gen_Literal_TN(adjust_offset, 4),
      Gen_Literal_TN(rsize-dsize+bit_offset, 4), Gen_Literal_TN(rsize-dsize+bit_size+bit_offset-1, 4), src1_tn, ops);
  }
  else if (!OP_NEED_PAIR(rtype) && OP_NEED_PAIR(desc)) { // I4 <- I8
    TN * src2_low = Get_TN_Pair(src2_tn);
    FmtAssert(src2_low, ("Exp_Deposit_Bits : I4 <- I8 source2 tn pair not setup"));

#if 0    
    if (bit_size + bit_offset > dsize) {
      DevWarn("Exp_Deposit_Bits : I4 <- I8 bit size out of range");
      bit_size = dsize - bit_offset;
    }
#endif    
    
    UINT adjust_offset = (dsize - (bit_offset + bit_size)) % dsize;
    Build_OP(TOP_rlwimi, src1_tn, src2_low, Gen_Literal_TN(adjust_offset, 4),
      Gen_Literal_TN(rsize-dsize+bit_offset, 4), Gen_Literal_TN(rsize-dsize+bit_size+bit_offset-1, 4), src1_tn, ops);  
  }
  else if (OP_NEED_PAIR(rtype) && !OP_NEED_PAIR(desc)) { // I8 <- I4
    TN * src1_low = Get_TN_Pair(src1_tn);
    FmtAssert(src1_low, ("Exp_Deposit_Bits : I8 <- I4 source1 tn pair not setup"));
    ...
    if (bit_offset < 32) {
      ...
      else /*if (bit_size < 32)*/{// bit_offset < 32, bit_offset + bit_size > 32
        UINT bit_size1 = 32 - bit_offset;
        UINT adjust_offset = (dsize - (bit_offset + bit_size1 - 1)) % dsize;
        Build_OP(TOP_rlwimi, src1_low, src2_tn, Gen_Literal_TN(adjust_offset, 4),
          Gen_Literal_TN(rsize-dsize+bit_offset, 4), Gen_Literal_TN(rsize-dsize+bit_size1+bit_offset-1, 4), src1_low, ops); 

        Build_OP(TOP_rlwimi, src1_tn, src2_tn, Gen_Literal_TN(32-bit_size+bit_size1, 4),
          Gen_Literal_TN(0, 4), Gen_Literal_TN(bit_size-bit_size1-1, 4), src1_tn, ops);
      }      
    }
    else {  // bit_offset > 32      
      UINT adjust_offset = (dsize - (bit_offset + bit_size - 1)) % dsize;
      Build_OP(TOP_rlwimi, src1_tn, src2_tn, Gen_Literal_TN(adjust_offset, 4),
        Gen_Literal_TN(rsize-dsize+bit_offset, 4), Gen_Literal_TN(rsize-dsize+bit_size+bit_offset-1, 4), src1_tn, ops);  
    }
  }
    ...
    TN * src1_low = Get_TN_Pair(src1_tn);
    TN * src2_low = Get_TN_Pair(src2_tn);
    FmtAssert(src1_low, ("Exp_Deposit_Bits : I8 <- I8 source1 tn pair not setup"));
    FmtAssert(src2_low, ("Exp_Deposit_Bits : I8 <- I8 source2 tn pair not setup"));
    
    if (bit_offset < 32) {
      if (bit_offset + bit_size <= 32) {
        UINT adjust_offset = (dsize - (bit_offset + bit_size)) % dsize;
        Build_OP(TOP_rlwimi, src1_tn, src2_low, Gen_Literal_TN(adjust_offset, 4),
          Gen_Literal_TN(bit_offset, 4), Gen_Literal_TN(bit_size+bit_offset-1, 4), src1_tn, ops);    
      }    
      else if (bit_size <= 32){
        UINT bit_size1 = 32 - bit_offset;
        UINT adjust_offset = (2*dsize - (bit_offset + bit_size1)) % dsize;
        Build_OP(TOP_rlwimi, src1_tn, src2_low, Gen_Literal_TN(32-bit_offset, 4),
          Gen_Literal_TN(bit_offset, 4), Gen_Literal_TN(bit_size1+bit_offset-1, 4), src1_tn, ops); 
        Build_OP(TOP_rlwimi, src1_low, src2_low, Gen_Literal_TN(32-bit_size+bit_size1, 4),
          Gen_Literal_TN(0, 4), Gen_Literal_TN(bit_size-bit_size1-1, 4), src1_low, ops);
      }      
      else {
        UINT adj_off = 64 - bit_offset - bit_size;
        UINT last_bit = bit_offset + bit_size - 33;
        Build_OP(TOP_rlwimi, src1_tn, src2_tn, Gen_Literal_TN(adj_off, 4),
          Gen_Literal_TN(bit_offset, 4), Gen_Literal_TN(last_bit, 4), src1_tn, ops);           

        if (last_bit < 31)
          Build_OP(TOP_rlwimi, src1_tn, src2_low, Gen_Literal_TN(adj_off, 4),
            Gen_Literal_TN(last_bit+1, 4), Gen_Literal_TN(31, 4), src1_tn, ops);
        Build_OP(TOP_rlwimi, src1_low, src2_low, Gen_Literal_TN(adj_off, 4),
          Gen_Literal_TN(0, 4), Gen_Literal_TN(last_bit, 4), src1_low, ops);    
      }
    }
    else {  // bit_offset >= 32      
      UINT adj_off = 64 - bit_offset - bit_size;
      Build_OP(TOP_rlwimi, src1_tn, src2_low, Gen_Literal_TN(adj_off, 4),
        Gen_Literal_TN(bit_offset-32, 4), Gen_Literal_TN(bit_size+bit_offset-33, 4), src1_tn, ops);  
    }
  }
  if (tgt_tn != src1_tn) {
...
#if 0
  if (bit_offset < 32 ) {
    UINT adjust_offset = (63 - (bit_offset + bit_size - 1)) % 32;
    Build_OP(TOP_rlwimi, src1_tn, src2_tn, Gen_Literal_TN(adjust_offset, 4),
      Gen_Literal_TN(bit_offset % 32, 4), Gen_Literal_TN((bit_size + bit_offset -1) % 32, 4), ops);
  }
  
  if ((bit_size + bit_offset) > 32) {
    FmtAssert(OP_NEED_PAIR(rtype), ("Exp_Deposit_Bits: result type seem wrong"));
    TN * src1_high = Get_TN_Pair(src1_tn);
    TN * src2_high = Get_TN_Pair(src2_tn);
    FmtAssert(src1_high && src2_high, ("Exp_Deposit_Bits: TN pair not setup"));
    ...
    UINT adjust_offset = (63 - (bit_offset + bit_size - 1)) % 32;
    Build_OP(TOP_rlwimi, src1_high, src2_high, Gen_Literal_TN(adjust_offset, 4),
      Gen_Literal_TN(bit_offset % 32, 4), Gen_Literal_TN((bit_size + bit_offset -1) % 32, 4), ops);
  }
