...
parse_args (unsigned argc, char **argv)
{
  ...
  for (i = 1; i < argc; i++)
      ...
      if (strncmp("-Wl,", argv[i], 4) == 0)
	{
      ...
      // ipa_link will pass -static-libgcc and shared-libgcc directly to the
      // final linker.  In the meantime, change them to harmless -g.  Bug 11527.
...
#ifdef KEY
	      ld_ipa_opt[LD_IPA_SHARABLE].flag =
		ld_ipa_opt[LD_IPA_SHARABLE].set = F_MAKE_SHARABLE;
#endif
