...
void extern WGEN_CS_push( WGEN_CHECK_KIND kind, int lnum,int fnum)
{
  int new_stack_size;
  ...
  if (Trace_Omp)
    printf("Pushing %s ok! Stack now: %d element\n",
            WGEN_omp_name (omp_check_sp->kind), omp_check_num);
}
...
extern WGEN_CHECK_KIND WGEN_CS_pop(WGEN_CHECK_KIND kind)
{
  FmtAssert (omp_check_sp >= omp_check_stack,
             ("no more entries on stack in function WGEN_cs_Pop"));
  ...
  if (Trace_Omp)
    printf("Poping %s ok! Stack now: %d element\n",
           WGEN_omp_name (omp_check_sp->kind), omp_check_num-1);
  omp_check_sp--;
  omp_check_num--;
...
{
  if (omp_check_sp < omp_check_stack)
    return NULL;
...
// Returns NULL if there is no enclosing region.
extern CHECK_STMT *WGEN_CS_enclose(void)
{
  if (omp_check_sp-1 < omp_check_stack)
    return NULL;
...
{
  FmtAssert (omp_check_sp >= omp_check_stack,
             ("no entry on stack in function WGEN_Set_Prag"));
...
{
  FmtAssert (omp_check_sp >= omp_check_stack,
             ("no entry on stack in function WGEN_Set_Intflag"));
...
extern void WGEN_Set_LFnum(CHECK_STMT* cs,int lnum,int fnum)
{
  ...
  FmtAssert (omp_check_sp >= omp_check_stack,
             ("no entriy on stack in function WGEN_Set_LFnum"));
...
// set region node
extern void WGEN_Set_Region (WN * region)
{
  FmtAssert (omp_check_sp >= omp_check_stack,
             ("no entry on stack in function WGEN_Set_Region"));
...
extern int WGEN_CS_Find_depth(WGEN_CHECK_KIND kind,int depth)
{
  int count=0;
  int find=0, rval=-1;
  CHECK_STMT* temp;
...
extern bool WGEN_bind_to_same(WGEN_CHECK_KIND sub1,WGEN_CHECK_KIND sub2,WGEN_CHECK_KIND bind)
{
	...
	find1=WGEN_CS_Find(sub1);
	find2=WGEN_CS_Find(sub2);
	find3=WGEN_CS_Find(bind);
	
	if(find1<0||find2<0||find3<0)  // one or more of the three kind are not in stack
	   rval=false;
	else if(find1==find2)   //sub1 and sub2 are same kind
	{  
		if(WGEN_CS_Find_depth(sub1,2)>-1)
		{
		   tmp=WGEN_CS_Find_depth(sub1,2);
...
void WGEN_add_pragma_to_enclosing_regions (WN_PRAGMA_ID id, ST * st,
                                           BOOL parallel_only)
{
  WN_VECTOR regions;
