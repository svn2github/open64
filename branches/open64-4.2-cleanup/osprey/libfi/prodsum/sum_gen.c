...
DopeVectorType	*mask)
{
  ...
  if (!HAVE_MASK(mask))
  {
    ...
    while (counter[src_rank] < src_extent[src_rank] ) {
      ...
      for ( i = 0 ; i < a_size ; i ++ ) {
	accum.r += (*(c8 *)array_p).r ;
	accum.i +=(*(c8 *)array_p).i ;

	array_p += a_stride ;
      }
      (*(c8 *) result_p).r = accum.r ;
      (*(c8 *) result_p).i = accum.i ;
      counter[0] = a_size  ;
      j = 0 ;
      while ((counter[j] == src_extent[j]) && (j < src_rank)) {
	array_p += src_offset[j] ;
...
DopeVectorType	*mask)
{
  ...
  if (!HAVE_MASK(mask))
  {
    ...
    while (counter[src_rank] < src_extent[src_rank] ) {
      ...
      for ( i = 0 ; i < a_size ; i ++ ) {
	accum.r += (*(c16 *)array_p).r ;
	accum.i +=(*(c16 *)array_p).i ;

	array_p += a_stride ;
      }
      (*(c16 *) result_p).r = accum.r ;
      (*(c16 *) result_p).i = accum.i ;
      counter[0] = a_size  ;
      j = 0 ;
      while ((counter[j] == src_extent[j]) && (j < src_rank)) {
	array_p += src_offset[j] ;
...
DopeVectorType	*mask)
{
  ...
  if (!HAVE_MASK(mask))
  {
    ...
    while (counter[src_rank] < src_extent[src_rank] ) {
      ...
      for ( i = 0 ; i < a_size ; i ++ ) {
	accum.r += (*(c32 *)array_p).r ;
	accum.i +=(*(c32 *)array_p).i ;

	array_p += a_stride ;
      }
      (*(c32 *) result_p).r = accum.r ;
      (*(c32 *) result_p).i = accum.i ;
      counter[0] = a_size  ;
      j = 0 ;
      while ((counter[j] == src_extent[j]) && (j < src_rank)) {
	array_p += src_offset[j] ;
