...
QUAD_TYPE QUAD_TYPE::operator=(const double d)
{
	INT err;
	QUAD_TYPE q = RQ_To_R16(__c_q_extd(d,&err));
	*this = q;
	return q;
}
QUAD_TYPE QUAD_TYPE::operator+=(QUAD_TYPE q)
{
	INT err;
	QUAD_TYPE res = *this + q;
       *this = res;
	return res;
}
QUAD_TYPE QUAD_TYPE::operator-=(QUAD_TYPE q)
...
{
	INT err;
	return RQ_To_R16(__c_q_add(R16_To_RQ(*this),R16_To_RQ(q),&err));
}
QUAD_TYPE QUAD_TYPE::operator+(double d) {
	INT err;
       return *this+RQ_To_R16(__c_q_extd(d, &err));
}
QUAD_TYPE QUAD_TYPE::operator-(QUAD_TYPE q)
{
	INT err;
	return RQ_To_R16(__c_q_sub(R16_To_RQ(*this),R16_To_RQ(q),&err));
}
QUAD_TYPE QUAD_TYPE::operator-(double d) {
	INT err;
       return *this-RQ_To_R16(__c_q_extd(d, &err));
}
QUAD_TYPE QUAD_TYPE::operator-()
{
	INT err;
	return RQ_To_R16(__c_q_neg(R16_To_RQ(*this),&err));
}
QUAD_TYPE QUAD_TYPE::operator*(QUAD_TYPE q)
{
	INT err;
	return RQ_To_R16(__c_q_mul(R16_To_RQ(*this),R16_To_RQ(q),&err));
}
QUAD_TYPE QUAD_TYPE::operator*(double d) {
	INT err;
       return *this*RQ_To_R16(__c_q_extd(d, &err));
...
{
	INT err;
	return RQ_To_R16(__c_q_div(R16_To_RQ(*this),R16_To_RQ(q),&err));
}
QUAD_TYPE QUAD_TYPE::operator/(double d) {
	INT err;
       return *this/RQ_To_R16(__c_q_extd(d, &err));
}
INT QUAD_TYPE::operator>(QUAD_TYPE q)
{
	INT err;
	return __c_q_gt(R16_To_RQ(*this),R16_To_RQ(q),&err);
}
INT QUAD_TYPE::operator>(double d) {
	INT err;
       return *this>RQ_To_R16(__c_q_extd(d, &err));
}
INT QUAD_TYPE::operator<(QUAD_TYPE q)
{
	INT err;
	return __c_q_lt(R16_To_RQ(*this),R16_To_RQ(q),&err);
}
INT QUAD_TYPE::operator<(double d) {
	INT err;
       return *this<RQ_To_R16(__c_q_extd(d, &err));
...
{
	INT err;
	return __c_q_eq(R16_To_RQ(*this),R16_To_RQ(q),&err);
}
INT QUAD_TYPE::operator==(double d) {
	INT err;
       return *this==RQ_To_R16(__c_q_extd(d, &err));
...
{
	INT err;
	return __c_q_ne(R16_To_RQ(*this),R16_To_RQ(q),&err);
}
INT QUAD_TYPE::operator!=(double d) {
	INT err;
       return *this!=RQ_To_R16(__c_q_extd(d, &err));
...
{
	INT err;
	return __c_q_ge(R16_To_RQ(*this),R16_To_RQ(q),&err);
}
INT QUAD_TYPE::operator>=(double d) {
	INT err;
       return *this>=RQ_To_R16(__c_q_extd(d, &err));
...
{
	INT err;
	return __c_q_le(R16_To_RQ(*this),R16_To_RQ(q),&err);
}
INT QUAD_TYPE::operator<=(double d) {
	INT err;
       return *this<=RQ_To_R16(__c_q_extd(d, &err));
   ...
   switch (TCON_ty(c0)) {
    ...
    case MTYPE_CQ:
	 ...
	 qhypot function, when we figure out how to make it available*/
      /* qmag = qhypot(qr,qi); */
#endif
      qmag = RQ_To_R16(__c_q_sqrt( R16_To_RQ(qi*qi + qr*qr) ,&err));
      if (qmag == q0){
	 rqr = q0;
	 rqi = q0;
      }
      else if (qr > q0) {
	rqr = RQ_To_R16(__c_q_sqrt( R16_To_RQ(q05 * (qmag+qr)),&err));
	rqi = q05 * (qi / rqr );
      } else {
	rqi = RQ_To_R16(__c_q_sqrt( R16_To_RQ(q05 * (qmag - qr )),&err));
	if (qi < q0) rqi = -rqi;
	rqr = q05 * (qi / rqi);
      }
      TCON_R16(r) = rqr;
...
#undef DEBUG_FOLD
   ...
   *folded = TRUE;
    ...
    */
    ...
    case OPC_I4FQRND:
    if (TCON_R16(c0) >= 0.0){
      TCON_R16(c0) = (INT32)__c_ji_qint(R16_To_RQ(TCON_R16(c0) + 0.5),&err);
    } else {
      TCON_R16(c0) = (INT32)__c_ji_qint(R16_To_RQ(TCON_R16(c0) - 0.5),&err);
    }
    ...
    case OPC_I8FQRND:
    if (TCON_R16(c0) >= 0.0 ){
      TCON_R16(c0) = (INT64)__c_ki_qint(R16_To_RQ(TCON_R16(c0) + 0.5),&err);
    } else {
      TCON_R16(c0) = (INT64)__c_ki_qint(R16_To_RQ(TCON_R16(c0) - 0.5),&err);
    }     
    c0 = Targ_Conv(MTYPE_I8,c0);
...
#ifdef TARG_NEEDS_QUAD_OPS
    case OPC_U4FQRND:
      TCON_R16(c0) = (UINT32)__c_ji_quint(R16_To_RQ(TCON_R16(c0) + 0.5),&err);
...
#ifdef TARG_NEEDS_QUAD_OPS
    case OPC_U8FQRND:
      TCON_R16(c0) = (UINT64)__c_ki_quint(R16_To_RQ(TCON_R16(c0) + 0.5),&err);
//      TCON_R16(c0) = (UINT64)(TCON_R16(c0) + 0.5);
	...
	*folded = FALSE;
    ...
    case OPC_FQSQRT:
    if (TCON_R16(c0) >= 0) {
      TCON_R16(c0) = RQ_To_R16(__c_q_sqrt(R16_To_RQ(TCON_R16(c0)),&err)); 
    } else {
      *folded = FALSE;
    }
    break;
  ...
  */
  switch ( FROM_TO(ty_from, ty_to) ) {
    ...
    case FROM_TO(MTYPE_CQ, MTYPE_C4):
      TCON_R4(r) = __c_sngl_q(R16_To_RQ(TCON_R16(c)),&err);
      TCON_IR4(r) = __c_sngl_q(R16_To_RQ(TCON_IR16(c)),&err);
      break;
#endif /* TARG_NEEDS_QUAD_OPS */
...
#ifdef TARG_NEEDS_QUAD_OPS
   case MTYPE_FQ:
      return __c_ji_qint(R16_To_RQ(TCON_R16(fvalue)),&err);
#endif
