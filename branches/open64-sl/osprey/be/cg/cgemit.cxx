...
#ifdef TARG_SL
BOOL OP_SL1_COMMON_LD(OP *op) 
{
  FmtAssert((Is_Target_Sl1_pcore() || Is_Target_Sl1_dsp()), ("OP_SL1_COMMON_LD: FYI"));
...
BOOL OP_SL1_COMMON_ST(OP *op) 
{
  FmtAssert((Is_Target_Sl1_pcore() || Is_Target_Sl1_dsp()), ("OP_SL1_COMMON_ST: FYI"));
...
TN *Get_Common_LD_ST_Base_TN(OP *op)
{
  FmtAssert((OP_SL1_COMMON_LD(op) || OP_SL1_COMMON_ST(op)), ("Get_Common_LD_ST_Base_TN: FYI"));
  if (OP_SL1_COMMON_LD(op))
    return OP_opnd(op, 0);
  else if (OP_SL1_COMMON_ST(op)) 
    return OP_opnd(op, 1);
}

// just for bank check, the offset is divided by 4
INT32 Get_Common_LD_ST_Offset(OP *op) 
{
  FmtAssert((OP_SL1_COMMON_LD(op) || OP_SL1_COMMON_ST(op)), ("Get_Common_LD_ST_Offset: FYI"));
...
static BOOL Can_Sort_Pop16_Sequence(vector<OP*> *pop16_vector)
{
  INT32 flag[60] = {0};
  ...
  for(temp = pop16_vector->begin(); temp != pop16_vector->end(); temp ++) {
    ...
    FmtAssert(TN_is_register(tn), ("Can_Sort_Pop16_Sequence: FYI"));
    tn_num = TN_register(tn);
    if (flag[tn_num] == 1)
      return FALSE;
      //FmtAssert(FALSE, ("Can_Sort_Pop16_Sequence: Can not Sort Pop"));
    else
      flag[tn_num] = 1;    
  }
  return TRUE;
...
static void CG_Resort_Pop16(BB *bb, OP *begin_op, OP *end_op, vector<OP*> *pop16_vector) 
{
  if (pop16_vector->size() <= 2) return;
  if (Can_Sort_Pop16_Sequence(pop16_vector) == FALSE) return;
  ...
  for(temp = pop16_vector->begin(); temp != pop16_vector->end(); temp ++) {
    ...
    FmtAssert(TN_is_constant(tn), ("CG_Resort_Pop16: The Offset of Pop16 is not constant"));
    INT64 offset = Get_Common_LD_ST_Offset(*temp);
    ...
    switch (offset & 0x3) {
      ...
      default:
        FmtAssert(FALSE, ("CG_Resort_Pop16: FYI"));      
    }    
  }
  pop16_vector->clear();
  OP* op = begin_op;
...
/* resorted pop16 sequence for fewer bank conflict access */
static void CG_Resort_Pop16_In_BB(BB *bb)
{
  OP *op;
  ...
  FOR_ALL_BB_OPs_FWD(bb, op) {
    if (OP_code(op) == TOP_pop16) {
      ...
      if (pop16_vector->size() > 2) CG_Resort_Pop16(bb, OP_prev(op), op1, pop16_vector);
      op = op1;
      if (op == NULL) break;
    }
  }
...
{  
  ...
  for (op_t = OP_next(op); (op_t != NULL) && (num_nop16 > 0); op_t = OP_next(op_t)) {
    if (OP_SL1_COMMON_LD(op_t) || OP_SL1_COMMON_ST(op_t)) 
      break;
  ...
  if ((num_nop16 > 0) 
    && (((op_t != NULL) && (OP_SL1_COMMON_LD(op_t) || OP_SL1_COMMON_ST(op_t)))
      || (op_t == NULL)))
...
{
  if ((OP_SL1_COMMON_LD(op1) || OP_SL1_COMMON_ST(op1))
    && OP_SL1_COMMON_LD(op2)

    && (!((Get_Common_LD_ST_Base_TN(op1) == Get_Common_LD_ST_Base_TN(op2)) 
       && ((Get_Common_LD_ST_Offset(op1) & 0x3) != (Get_Common_LD_ST_Offset(op2) & 0x3)))))
    return TRUE;
  else
    return FALSE;
}
...
static inline BOOL top_is_ld_st_jplnk_jrlnk(OP *op)
{
  ...
  TOP top = OP_code(op);
  if (OP_SL1_COMMON_LD(op) || OP_SL1_COMMON_ST(op)
 ...
 */
static void CG_Add_Nop16_Workaround(BB* bb)
{
  OP *op;
  INT32 num_nop16;
#if 0
/* Disable following codes for compiling speed */
  ...
  if (first_op) {
    ...
    if ((second_op) && OP_SL1_COMMON_LD(second_op) 
