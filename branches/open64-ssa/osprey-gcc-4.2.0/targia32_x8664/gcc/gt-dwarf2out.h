...
{
  struct pubname_struct * const x = (struct pubname_struct *)x_p;
  if (gt_pch_note_object (x, x, gt_pch_p_14pubname_struct, gt_ggc_e_14pubname_struct))
    {
      gt_pch_n_10die_struct ((*x).die);
      gt_pch_n_S ((*x).name);
    }
...
{
  struct die_struct * const x = (struct die_struct *)x_p;
  if (gt_pch_note_object (x, x, gt_pch_p_10die_struct, gt_ggc_e_10die_struct))
    {
...
gt_pch_na_pubname_table (ATTRIBUTE_UNUSED void *x_p)
{
  if (pubname_table != NULL) {
    ...
    gt_pch_note_object (pubname_table, &pubname_table, gt_pch_pa_pubname_table, gt_types_enum_last);
  }
}
