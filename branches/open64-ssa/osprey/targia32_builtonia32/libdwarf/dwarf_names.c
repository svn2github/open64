...
get_FRAME_name (Dwarf_Debug dbg, Dwarf_Half val)
{
	...
	case DW_FRAME_REG34:
		if (ellipsis)
			...
			return "DW_FRAME_REG34";
	case DW_FRAME_REG35:
		if (ellipsis)
			return "REG35";
		else
			return "DW_FRAME_REG35";
#endif
	case DW_FRAME_FREG0:
		if (ellipsis)
			return "FREG0";
...
get_ADDR_name (Dwarf_Debug dbg, Dwarf_Half val)
{
	...
	case DW_ADDR_reg_space:
		if (ellipsis)
			return "ADDR_reg_space";
		else
			return "DW_ADDR_reg_space";
	case DW_ADDR_sreg_space:
		if (ellipsis)
			return "ADDR_sreg_space";
		else
			return "DW_ADDR_sreg_space";
	case DW_ADDR_const_space:
		if (ellipsis)
			return "ADDR_const_space";
		else
			return "DW_ADDR_const_space";
	case DW_ADDR_global_space:
		if (ellipsis)
			return "ADDR_global_space";
		else
			return "DW_ADDR_global_space";
	case DW_ADDR_local_space:
		if (ellipsis)
			return "ADDR_local_space";
		else
			return "DW_ADDR_local_space";
	case DW_ADDR_param_space:
		if (ellipsis)
			return "ADDR_param_space";
		else
			return "DW_ADDR_param_space";
	case DW_ADDR_shared_space:
		if (ellipsis)
			return "ADDR_shared_space";
		else
			return "DW_ADDR_shared_space";
	case DW_ADDR_surf_space:
		if (ellipsis)
			return "ADDR_surf_space";
		else
			return "DW_ADDR_surf_space";
	case DW_ADDR_tex_space:
		if (ellipsis)
			return "ADDR_tex_space";
		else
			return "DW_ADDR_tex_space";
	case DW_ADDR_code_space:
		if (ellipsis)
			return "ADDR_code_space";
		else
			return "DW_ADDR_code_space";
#endif
	default:
