...
#ifdef KEY
static int _dwarf_pro_generate_ehframe (Dwarf_P_Debug dbg, Dwarf_Error *error);
#endif
...
)
{
	/* 
	    ...
	    sections. get_section_bytes() returns 
	    pointers to these buffers one at a time.
	*/
    int 	nbufs;	
    int 	sect;	
    int 	   name_idx;	
	...
	if(dbg->de_func_b) {
           new_base_elf_sect = dbg->de_func_b(_dwarf_sectnames[sect], 
		/* rec size*/1, 
		SECTION_TYPE,
	    flags,
	    SHN_UNDEF, 0, &du, &err);
	...
	*/
...
#ifdef KEY
    if (dbg->eh_frame_cies) {
	nbufs = _dwarf_pro_generate_ehframe(dbg,error);
	if (nbufs < 0) {
	    DWARF_P_DBG_ERROR(dbg, DW_DLE_DEBUGFRAME_ERROR, DW_DLV_NOCOUNT);
	}
    }
...

    {
      ...
      int res;
      res = dbg->de_transform_relocs_to_disk(dbg,
		       &new_secs);
      if(res != DW_DLV_OK) {
	      DWARF_P_DBG_ERROR(dbg, DW_DLE_DEBUGINFO_ERROR, DW_DLV_NOCOUNT);
      }
...
_dwarf_pro_generate_debugline(Dwarf_P_Debug dbg, Dwarf_Error *error)
{
	...
	prolog_size +=	
			extension_size +  /* DISTINGUISHED VALUE */
	...
	*data = '\0';
	...
	while(curline) {
	  ...
	  int line_adv; /* supposed to be a reasonably small number,
	  ...
	  if (curline->dpl_opc != 0) {
		...
		arg = buff1;
		GET_CHUNK(dbg,elfsectno,data,nbytes+sizeof(Dwarf_Ubyte),
					error);
		WRITE_UNALIGNED(dbg,(void *)data,(const void *)&db,
			sizeof(db), sizeof(Dwarf_Ubyte));
		data += sizeof(Dwarf_Ubyte);
	  ...
	  if (no_lns_copy == 0) {	/* if not a special or 
					dw_lne_end_seq
	...
	du = sum_bytes - uwordb_size
		- extension_size;	/* subtract length field */
        {
...
---------------------------------------------------------------*/
static int 
_dwarf_pro_generate_ehframe(Dwarf_P_Debug dbg, Dwarf_Error *error)
{
	int elfsectno;
	int i;
	int firsttime = 1;
	...
	curcie = dbg->eh_frame_cies;
	cie_length = 0;
	cur_off = 0;
	cie_offs = (long *)
	    _dwarf_p_get_alloc(dbg, sizeof(long)*dbg->eh_n_cie);
	if (cie_offs == NULL) {
	    DWARF_P_DBG_ERROR(dbg,DW_DLE_CIE_OFFS_ALLOC,-1);
	}
	/* generate cie number as we go along */
...
_dwarf_pro_generate_debuginfo(Dwarf_P_Debug dbg,Dwarf_Error *error)
{
	...
	/* write cu header */
	cu_header_size = extension_size+
			 uword_size +	     /* length of info section */
	...
	version = CURRENT_VERSION_STAMP;	/* assume 
		this length will not change */
	...
	/* We have filled the chunk we got with GET_CHUNK.
           At this point we no longer dare use "data"
	   or "start_info_sec" as a pointer any longer except
	   to refer to that first small chunk for the cu header.
	*/
	...
	*/
        res = dbg->de_reloc_name(dbg,
	    	      ...
	    	      extension_size +
		      uword_size+sizeof(Dwarf_Half),  /* r_offset */
	...
	/* Pass 2: Write out the die information 
	   Here 'data' is a temporary, one block for each
	   GET_CHUNK.  'data' is overused. 
        */
	curdie = dbg->de_dies;
	while (curdie != NULL) {
	    Dwarf_P_Attribute curattr;
		...
		switch (curattr->ar_attribute_form) {
			...
			char buff1[ENCODE_SPACE_NEEDED];
                        ...
                        res = _dwarf_pro_encode_leb128_nm(
				curattr->ar_ref_die->di_offset,&nbytes,
                                    buff1,sizeof(buff1));
                        if (res != DW_DLV_OK) {
                            DWARF_P_DBG_ERROR(dbg,DW_DLE_ABBREV_ALLOC, -1);
                        }
	...
	Dwarf_Error *error)
{
	...
	*length = dbg->de_debug_sects->ds_nbytes;
	...
	/* We may want to call the section stuff more than once:
	   see dwarf_reset_section_bytes()
	   do not do: dbg->de_n_debug_sect--; 
	*/
...
)
{
    ...
    /* By using MAGIC_SECT_NO we allow the following
	MAGIC_SECT_NO must not match any legit section number.
    ...
    */
	...
	/* Either the elf section has changed
	   or there is not enough space in the current section.

           Create a new Dwarf_P_Section_Data_s  for the chunk.
	   and have space 'on the end' for the buffer itself
	   so we just do one malloc (not two).

	*/
	unsigned long space = nbytes;

	if( nbytes < CHUNK_SIZE)  space = CHUNK_SIZE;
	...
	cursect->ds_nbytes = nbytes; /* reserve this number of
             bytes of space for caller to fill in
             */

	/* Now link on the end of the list, and mark
	   this one as the current one
	*/

        if (dbg->de_debug_sects->ds_elf_sect_no == MAGIC_SECT_NO ) {
	    /* the only entry is the special one for 'no entry'
	       so delete that phony one while adding this initial
	       real one.
	    */
