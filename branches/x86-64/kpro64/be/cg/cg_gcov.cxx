...
CG_Instrument_Arcs()
{
...
#else
    Expand_Load( OPCODE_make_op (OPR_LDID, rtype, rtype),ld_2nd_result_tn, ld_result_tn, Gen_Literal_TN(count*8,4), V_NONE, &new_ops);
#endif
...
#else
    Expand_Store (OPCODE_desc(OPCODE_make_op(OPR_STID, MTYPE_V, rtype)),result_tn, ld_result_tn, Gen_Literal_TN(count*8,4), V_NONE, &new_ops);
#endif
  ...
  for (bb = REGION_First_BB; bb; bb = BB_next(bb)) 
  {
	...
	if (BB_Fall_Thru_Successor(bb) == bb_succ)
	  ...
	  Build_OP( TOP_br, Gen_Label_TN(tgt_label, 0), &new_ops);
#else
	  // mips
