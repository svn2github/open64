...
#pragma ident "@(#) libu/util/i3e/ieee_binary_scale_r_i8.c	92.1	07/07/99 14:37:09"
...
extern double _IEEE_BINARY_SCALE(double x, int n);

/* BINARY_SCALE - computes x * 2**n efficiently by not computing 2**n
 *	INPUT:   64 bit float x
 *	         64 bit integer n
 *	OUTPUT:  64 bit float result
 */

#pragma _CRI duplicate _IEEE_BINARY_SCALE as _IEEE_BINARY_SCALE_I8
double
_IEEE_BINARY_SCALE(double x, int n)
{  
	/* Union defined to work with IEEE 64-bit floating point. */
	union _ieee_double {
		double	dword;
		long long	lword;
		unsigned long long ulword;
		struct {
