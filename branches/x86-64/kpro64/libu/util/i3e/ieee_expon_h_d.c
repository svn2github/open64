...
#pragma ident "@(#) libu/util/i3e/ieee_expon_h_d.c	92.1	07/07/99 14:37:09"
#include <fenv.h>
#include <fp.h>
#include "i3eintrin.h"

#ifndef _LD64
extern float _IEEE_EXPONENT_H_D(long double x);
/* _IEEE_EXPONENT_H_D - IEEE EXPONENT returns the exponent part of the
 *                       128-bit argument in 32-bit float.
 *
 * IEEE_EXPONENT(X,Y_R4) returns real(kind=4)result
 */
#pragma _CRI duplicate _IEEE_EXPONENT_H_D as _IEEE_LOGB_H_D
float
_IEEE_EXPONENT_H_D(long double x)
{
	...
	switch (_fpclassifyl(x)) {
		case FP_NAN:
			...
			x_val.lwrd	= _HALF_NaN;
			return(x_val.dwrd);
			}
		case FP_INFINITE:
			{
			union _ieee_fdouble x_val;
			x_val.lwrd	= IEEE_32_INFINITY;
			return(x_val.dwrd);
			}
		case FP_NORMAL:
			{
