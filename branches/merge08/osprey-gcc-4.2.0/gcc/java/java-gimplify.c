...
// we do not need to recorde 'outer'
  outer = gimple_current_bind_expr ();
  tree block;
...
static tree transform_builtin(tree statement)
{
  tree caller_addr;
  tree caller_decl;
  caller_addr = TREE_OPERAND (statement, 0);
  for(;TREE_CODE(caller_addr)==NOP_EXPR;caller_addr = TREE_OPERAND (caller_addr, 0));
    if(caller_addr==NULL || TREE_CODE(caller_addr)!=ADDR_EXPR)
      return NULL;
  caller_decl = TREE_OPERAND (caller_addr, 0);
  for(;TREE_CODE(caller_decl)==NOP_EXPR;caller_decl = TREE_OPERAND (caller_decl, 0));
    if(caller_decl==NULL || TREE_CODE(caller_decl)!=FUNCTION_DECL)
      return NULL;
 
  return check_for_builtin2(caller_decl, statement);
}
 
tree transform_statement(tree statement)
{
  tree arglist;
  int e;
 
  if(GS_TRANSFORMED_NODE(statement) != NULL)
    return GS_TRANSFORMED_NODE(statement);
 
  switch(TREE_CODE (statement))
  {
    case BLOCK:
      statement = java_gimplify_block (statement);
      if(BIND_EXPR_BODY (statement))
        BIND_EXPR_BODY (statement) = transform_statement(BIND_EXPR_BODY (statement));
      break;
    case FLOAT_EXPR:
    ...
    case CALL_EXPR:
    {
      tree builtin = transform_builtin(statement);
      if (builtin != NULL && TREE_CODE(builtin) == CALL_EXPR)
        for (arglist = TREE_OPERAND (builtin, 1); arglist; arglist = TREE_CHAIN (arglist))
          TREE_VALUE (arglist) = transform_statement(TREE_VALUE (arglist));
      else if (builtin != NULL)
        statement = builtin;
      else
      {
        for (arglist = TREE_OPERAND (statement, 1); arglist; arglist = TREE_CHAIN (arglist))
          TREE_VALUE (arglist) = transform_statement(TREE_VALUE (arglist));
      }
      break;
    }
    ...
    case COND_EXPR:
      TREE_OPERAND (statement, 0) = transform_statement(TREE_OPERAND (statement, 0));
      TREE_OPERAND (statement, 1) = transform_statement(TREE_OPERAND (statement, 1));
      TREE_OPERAND (statement, 2) = transform_statement(TREE_OPERAND (statement, 2));
      break;
    case LOOP_EXPR:
      LOOP_EXPR_BODY(statement) = transform_statement(LOOP_EXPR_BODY(statement));
      break;
    case EXPR_WITH_FILE_LOCATION:
#ifdef USE_MAPPED_LOCATION
      input_location = EXPR_LOCATION (statement);
#else
      input_location.file = EXPR_WFL_FILENAME (statement);
      input_location.line = EXPR_WFL_LINENO (statement);
#endif
      statement = EXPR_WFL_NODE (statement);
      if (EXPR_P (statement))
        SET_EXPR_LOCATION (statement, input_location);
      statement = transform_statement(statement);
      break;
    case LABELED_BLOCK_EXPR:
        statement = java_gimplify_labeled_block_expr (statement);
      if(TREE_CODE(statement) == COMPOUND_EXPR)
        statement = transform_statement(statement);
       break;
    case EXIT_BLOCK_EXPR:
      statement = java_gimplify_exit_block_expr (statement);
      break;
    case CASE_EXPR:
      statement = java_gimplify_case_expr (statement);
      TREE_OPERAND (statement, 0) = transform_statement(TREE_OPERAND (statement, 0));
      break;
    case SWITCH_EXPR:
      SWITCH_BODY (statement) = transform_statement(SWITCH_BODY (statement));
      break;
    case DEFAULT_EXPR:
      statement = java_gimplify_default_expr (statement);
      break;
    case NEW_ARRAY_INIT:
      statement = java_gimplify_new_array_init (statement);
      statement = transform_statement(statement);
      break;
    case TRY_EXPR:
      statement = java_gimplify_try_expr (statement);
      //deal with the TRY_BODY which maybe BLOCK or COMPOUND_EXPR
      TREE_OPERAND (statement, 0) = transform_statement(TREE_OPERAND (statement, 0));
      //deal with the JAVA_CATCH_EXPR in CATCH_EXPR
      TREE_OPERAND (statement, 1) = transform_statement(TREE_OPERAND (statement, 1));
      break;
    ...
    case TRY_CATCH_EXPR:
     ...
     //deal with the JAVA_CATCH_EXPR in CATCH_EXPR
      ...
      break ;
    case JAVA_EXC_OBJ_EXPR:
      statement = build_exception_object_ref (TREE_TYPE (statement));
      break;
    case VAR_DECL:
      statement = java_replace_reference (statement, /* want_lvalue */ false);
      break;
    case MODIFY_EXPR:
      e = java_gimplify_modify_expr (&statement, NULL, NULL);
      if(TREE_CODE(statement) == MODIFY_EXPR)
      {
        if(TREE_OPERAND (statement, 0))
          TREE_OPERAND (statement, 0) = transform_statement(TREE_OPERAND (statement, 0));
      ...
      if(TREE_CODE(statement) == NOP_EXPR)
        if(TREE_OPERAND (statement, 0))
          TREE_OPERAND (statement, 0)  = transform_statement(TREE_OPERAND (statement, 0) );
      break;
    case SAVE_EXPR:
      /* Note that we can see <save_expr NULL> if the save_expr was
         already handled by gimplify_save_expr.  */
      if (TREE_OPERAND (statement, 0) != NULL_TREE && TREE_CODE (TREE_OPERAND (statement, 0)) == VAR_DECL)
        TREE_OPERAND (statement, 0)
                  = java_replace_reference (TREE_OPERAND (statement, 0), /* want_lvalue */ false);
      TREE_OPERAND (statement, 0)  = transform_statement(TREE_OPERAND (statement, 0) );
      break;
    case BIND_EXPR:
      if(TREE_OPERAND (statement, 1))
    ...
    default:
      ...
      {
        if(is_gimple_formal_tmp_rhs (TREE_OPERAND (statement, 0)))
        {
          TREE_OPERAND (statement, 0) = Replace_Left_with_temp(TREE_OPERAND (statement, 0));
        }
        TREE_OPERAND (statement, 0)  = transform_statement(TREE_OPERAND (statement, 0) );
        TREE_OPERAND (statement, 1)  = transform_statement(TREE_OPERAND (statement, 1) );
     }
   }
   GS_TRANSFORMED_NODE(statement) = statement;
   return statement;
}
