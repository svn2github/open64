...
/*WARNING: This file is automatically generated from kgccfe/gnu/c-parse.in!
  ***WARNING***: And then modified to handle OpenMP constructs!*/

/*
   Copyright 2004, 2005, 2006 PathScale, Inc.  All Rights Reserved.
   File modified February 17, 2005 by PathScale, Inc. to add OpenMP support.
 */
...
%type <ttype> do_stmt_start poplevel stmt label structured_block
...
%type <ttype> variable_list critical_directive region_phrase
...
%type <ttype> barrier_directive flush_directive
...
/* Note how the declaration of the variable is in effect while its init is parsed! */
		{ finish_init ();
		  ...
		  if (In_MP_Region && Is_shared_mp_var ($<ttype>5))
		    mp_locals[mp_nesting] = chainon (mp_locals[mp_nesting], build_tree_list (NULL, $<ttype>5));
		}
	| declarator maybeasm maybe_attribute
...
exec_freq_directive_ignore:
	PRAGMA_EXEC_FREQ freq_hint_ignore IDENTIFIER '\n'
	{ $$ = NULL; }
	;
...
parallel_directive:
        PRAGMA_OMP PARALLEL '\n'
        { $$ = NULL; }
        | PRAGMA_OMP PARALLEL  parallel_clause_list '\n'
        { $$ = $3; }
        ;
...
parallel_clause:
      IF '(' expr_no_commas ')'
        ...
        |  DEFAULT '(' SHARED ')'
        { $$ = build_parallel_clause_list(NULL, p_default, default_shared, 0); }
        |  DEFAULT '(' NONE ')'
...
for_directive:
        PRAGMA_OMP FOR '\n'
        { $$ = NULL; }
        | PRAGMA_OMP FOR for_clause_list '\n'
        { $$ = $3; }
        ;
...
for_clause:
        ...
        |  SCHEDULE '(' schedule_kind ',' expr_no_commas ')'
...
sections_directive:
        PRAGMA_OMP SECTIONS '\n'
        { $$ = NULL; }
        | PRAGMA_OMP SECTIONS sections_clause_list '\n'
        { $$ = $3; }
        ;
...
single_directive:
        PRAGMA_OMP SINGLE '\n'
        {$$ = NULL;}
        | PRAGMA_OMP SINGLE single_clause_list '\n'
        {$$ = $3;}
        ;
...
parallel_for_directive:
        ...
        {$$ = NULL;}
        | PRAGMA_OMP PARALLEL FOR parallel_for_clause_list '\n'
        {$$ = $4;}
        ;
...
parallel_for_clause:
        ...
        | NUM_THREADS '(' expr_no_commas ')'
        { $$ = build_parallel_for_clause_list($3, p_for_num_threads, 0, 0, 0); }
        | PRIVATE '(' variable_list ')'
        { $$ = build_parallel_for_clause_list($3, p_for_private, 0, 0, 0); }
        |  COPYPRIVATE '(' variable_list ')'
        { $$ = build_parallel_for_clause_list($3, p_for_copyprivate, 0, 0, 0); }
        |  FIRSTPRIVATE '(' variable_list ')'
        { $$ = build_parallel_for_clause_list($3, p_for_firstprivate, 0, 0, 0); }
        |  LASTPRIVATE '(' variable_list ')'
        { $$ = build_parallel_for_clause_list($3, p_for_lastprivate, 0, 0, 0); }
        |  SHARED '(' variable_list ')'
        ...
        |  SCHEDULE '(' schedule_kind ',' expr_no_commas ')'
        { $$ = build_parallel_for_clause_list($5, p_for_schedule_2, 0, $3, 0); }
        |  ORDERED
...
parallel_sections_directive:
        ...
        {$$ = NULL;}
        | PRAGMA_OMP PARALLEL SECTIONS parallel_sections_clause_list '\n'
        {$$ = $4;}
        ;
...
critical_directive:
        PRAGMA_OMP CRITICAL '\n'
        { $$ = NULL; }
        | PRAGMA_OMP CRITICAL '(' region_phrase ')'  '\n'
        { $$ = $4; }
        ;
...
atomic_construct:
        atomic_directive expr_no_commas ';'
        {
	    add_stmt (build_omp_stmt (atomic_cons, $2));
	    $$ = NULL;
        }
        ;
...
flush_directive:
        ...
        | PRAGMA_OMP FLUSH '(' variable_list ')' '\n'
...
threadprivate_directive:
        PRAGMA_OMP THREADPRIVATE '(' variable_list ')' '\n'
...
variable_list:
        IDENTIFIER
        {
          ...
          $$ = build_tree_list (NULL_TREE, lookup_name ($1));
        }
        |variable_list ',' IDENTIFIER
    {
      if (lookup_name ($3) == 0)
        error ("Undefined variable %s", IDENTIFIER_POINTER($3));
      $$ = chainon ($1, build_tree_list (NULL_TREE, lookup_name ($3)));
    }
    ;
