...
Can_Mem_Op_Be_Moved(OP *mem_op, BB *cur_bb, BB *src_bb, BB *dest_bb, 
		    mINT32 motion_type, mUINT32 *cur_spec_type)
{
...
// ======================================================================
static void
Update_GRA_Live_Sets(OP *cand_op, BB *bb, BB *cand_bb, BB_SET **pred_bbs, mINT32 motion_type)
{
  ...
  for (i = 0; i < OP_results(cand_op); ++i) {
    ...
    if (result) {
      if( TN_is_global_reg(result) ) {
        if( use_before_def || !re_def )
          ...
          DevWarn("there should be double definition, without usage");
      }
    ...
    FOR_ALL_BB_SET_members (*pred_bbs, cur_bb) {
      if( ( cand_bb!=cur_bb || Motion_Is_CIRC_ABOVE(motion_type) ) && result ) {
...
#pragma mips_frequency_hint NEVER
      ...
      if(!Ignore_TN_Dep) 
        Update_GRA_Live_Sets(cand_op, bb, cand_bb, pred_bbs, motion_type);
...
OP_To_Move (BB *bb, BB *tgt_bb, BB_SET **pred_bbs, mINT32 motion_type, mUINT8 *spec_type)
{
  ...
  for (cur_op = (forw) ? BB_first_op(bb) : BB_last_op(bb); cur_op; 
    ...
    if( OP_xfer(cur_op) || OP_noop(cur_op) || OP_is_loop(cur_op) ) {
      if( Trace_GCM ){
  ...
  for (cloop = loop_list; cloop != NULL; cloop = LOOP_DESCR_next(cloop))
  {
    ...
    if( CG_GCM_enable_licm && LOOP_DESCR_loophead(cloop) 
      ...
      if( !GCM_LICM_Skip_Loop_Binary_Search(loop_id) )
        Perform_Loop_Invariant_Code_Motion( cloop, 
                                            &gcm_loop_pool, 
                                            Ignore_TN_Dep );
    }
