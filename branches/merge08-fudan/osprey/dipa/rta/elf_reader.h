...
class ProgHdrTable
{
 ...
 private: // private utility functions
  void _print_one(FILE* stream,ProgHdr* phdr,UINT phNum);
  
 public: // Constructors
  ProgHdrTable(unsigned char* address,ELFHdr* eh);

 public:
  ProgHdr* getPHdrByIndex(UINT phNum);
...
class SecHdrTable
{
 ...
 private: // private utility functions
  void _print_one(FILE* stream,SecHdr* shdr,UINT secNum);
 ...
 public:
  ...
  SecHdr* getSHdrByIndex(UINT secNum);
