...
void Try_Monitor::Set_Index_Last()
{
  current_index = try_vector.size() - 1;
}
void Try_Monitor::Set_Index_Prev()
{
  current_index--;
}
void Try_Monitor::PopBack()	
{
  try_vector.pop_back();
}
bool Try_Monitor::Empty()	
{
  return try_vector.size() == 0;
}
bool Try_Monitor::LastIsCurrent()
{
  if(state_stack.size() == 0)
    return false;
  return try_vector[try_vector.size() - 1].stmt == try_vector[state_stack.top().index].stmt;
}
