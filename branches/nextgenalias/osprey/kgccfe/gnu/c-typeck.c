...
#ifdef KEY
	{
	  // Use floorf(x) instead of floor(x) whenever x is float.  In general
	  // this optimization can be done for operations besides MINUS, but we
	  // do it only here as a hack.
