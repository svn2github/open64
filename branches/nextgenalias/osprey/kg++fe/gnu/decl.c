...
#if defined(TARG_SL2)
  if(TREE_CODE(decl) == VAR_DECL && DECL_VBUF(decl) && !initialized)
  	error("must initialize vbuf variables \"%s\"",  IDENTIFIER_POINTER(DECL_NAME(decl))); 
#endif 
...
#if defined(TARG_SL2)
  if(RIDBIT_SETP(RID_SBUF, specbits))
  	DECL_SBUF(decl) = 1; 
  else if(RIDBIT_SETP(RID_V1BUF, specbits))
  	DECL_V1BUF(decl) = 1; 
  else if(RIDBIT_SETP(RID_V2BUF, specbits))
  	DECL_V2BUF(decl) = 1;
  else if(RIDBIT_SETP(RID_V4BUF, specbits))
  	DECL_V4BUF(decl) = 1; 
#endif
...
#if defined(TARG_SL2)
  if(RIDBIT_SETP(RID_SBUF, specbits))
   type_quals |= TYPE_QUAL_SBUF; 
  if(RIDBIT_SETP(RID_V1BUF, specbits))
   type_quals |= TYPE_QUAL_V1BUF; 
  if(RIDBIT_SETP(RID_V2BUF, specbits))
   type_quals |= TYPE_QUAL_V2BUF; 
  if(RIDBIT_SETP(RID_V4BUF, specbits))
   type_quals |= TYPE_QUAL_V4BUF; 
#endif
