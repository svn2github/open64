...
static bfd_boolean elf_add_default_symbol
  ...
  PARAMS ((bfd *, struct bfd_link_info *, struct elf_link_hash_entry *,
	   ...
	   bfd_boolean *, bfd_boolean, bfd_boolean, flagword,
...
#ifdef IPA_LINK /* work around a seg fault through a null pointer (by KEY) */
...
#ifdef IPA_LINK
		  /* bug#2051
		     If <abfd> is not WHIRL object symbols, the <old_bfd> is
		     NULL because function _bfd_generic_link_add_one_symbol
		     "only changes definitions for WHIRL object symbols only."
		   */
		  && ipa_is_whirl(abfd)
#endif
		  )
