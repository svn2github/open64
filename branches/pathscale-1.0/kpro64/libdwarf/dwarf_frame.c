...
)
{
    ...
    Dwarf_Unsigned  adv_loc; /* must be min de_pointer_size bytes 
			 and must be at least sizeof Dwarf_ufixed */
...
{
    Dwarf_Unsigned cie_id; /* must be 
				min de_length_size bytes in size */
    Dwarf_Small    version;
	...
	/* egcs-1.1.2 .eh_frame uses 0 as the distinguishing id.
		sgi uses -1 in .debug_frame.
	   .eh_frame  not quite identical to .debug_frame
	*/
...
)
{
    ...
    Dwarf_Unsigned  length = 0;	/* READ_UNALIGNED needs 
				min de_length_size byte dest */
    Dwarf_Unsigned  cie_base_offset = 0; /* needs to be
				min de_length_size byte dest */
    ...
    */
    ...
    Dwarf_Addr      initial_location; /* must be
			min de_pointer_size bytes in size */
    Dwarf_Addr      address_range; /* must be
			min de_pointer_size bytes in size */
    ...
    */
	...
	int local_length_size = 0;
        ...
        READ_AREA_LENGTH(dbg,length,Dwarf_Unsigned,
          frame_ptr,local_length_size,local_extension_size);
	...
	if(length == 0) {
		/* nul bytes at end of section, seen at end
		   of egcs eh_frame sections (in a.out).
		   Take this as meaning no more CIE/FDE data.
		   We should be very close to end of section.
		*/
		break;
        }
	...
	SIGN_EXTEND(cie_id, local_length_size);
	cie_base_offset = cie_id; /* if this is a CIE,
		this is ignored.  If it is an FDE, this
		is the section offset that allows us to get to
	        the cie of this fde. Save it for the fde
		part of the 'if' below */
		...
		See the dwarf spec, section 6.4.1 
	    */
		...
		data_alignment_factor = 
			(Dwarf_Sword)_dwarf_decode_s_leb128(
		...
		READ_UNALIGNED(dbg,exception_table_addr, Dwarf_Unsigned,
                        frame_ptr, local_length_size);
		frame_ptr += local_length_size;
	   ...
	   */
	    ...
	    READ_UNALIGNED(dbg,address_range, Dwarf_Addr,
			frame_ptr, dbg->de_pointer_size);
            frame_ptr += dbg->de_pointer_size;
	    /* Get the augmentation string from Cie to identify the layout
	       of this Fde.  */
	    if(use_gnu_cie_calc) {
	      /* cie_id value is offset, in section, of the cie_id itself, to
		 use vm ptr  of the value, less the value, to
	         get to the cie itself.
		 In addition, munge cie_base_offset to look
	         *as if* it was from real dwarf.
	      */
	      ...
	      */
              cieptr = (Dwarf_Small *) (section_ptr + cie_base_offset);
	    }
	    ...
	    if ((strcmp((char *) augmentation, 
				DW_DEBUG_FRAME_AUGMENTER_STRING) == 0) ||
		(strcmp((char *) augmentation, DW_EMPTY_STRING) == 0)) {
		/* We are pointing at the start of instructions. Do nothing. */
	    }
	    else if (augmentation[0] == 'z') {
                Dwarf_Unsigned lreg;
                DECODE_LEB128_UWORD(frame_ptr, lreg)
		...
		saved_frame_ptr = frame_ptr;
		    ...
		    /* ?? THis presumes that the offset is always 32 bits */
		    READ_UNALIGNED(dbg, offset_into_exception_tables, Dwarf_Addr,
				frame_ptr, sizeof(Dwarf_sfixed));
		...
		*/
		...
		/* We do not understand the augmentation string. No
		   assumption can be made about if the instructions
		   is present. */
		/* FIX -- The old code assumed that the instruction table
		   starts at the location pointed to by  frame_ptr, clearly 
		   incorrect. */
	    }
    ...
    *fde_element_count = fde_count;
	...
	/* we assume that the next fde has the same cie as the
	** last fde and resume the search where we left off
	*/
	...
	*/
    ...
    qsort((void *) fde_list_ptr, fde_count, sizeof(Dwarf_Ptr), qsort_compare);
...
)
{
    ...
    Dwarf_Unsigned  	length; /* must be
				min de_length_size bytes */
    Dwarf_Signed 	signed_offset; /* must be 
				min de_length_size bytes */
    Dwarf_Addr 		initial_location; /* must be 
				min de_pointer_size bytes */
    Dwarf_Addr 		address_range; /* must be 
				min de_pointer_size bytes */
    Dwarf_Fde 		new_fde;
    ...
    READ_AREA_LENGTH(dbg,length,Dwarf_Unsigned,
        fde_ptr,fde_local_length_size,fde_local_extension_size);
    ...
    res = get_augmentation_string(dbg, 
		      (Dwarf_Small *) (dbg->de_debug_frame+signed_offset),
			DW_CIE_ID,
		      &augmentation, error);
    if (res != DW_DLV_OK) {
	return res;
    ...
    if ((strcmp((char *) augmentation,DW_DEBUG_FRAME_AUGMENTER_STRING) == 0) ||
	(strcmp((char *) augmentation, DW_EMPTY_STRING) == 0)) {
	/* Do nothing. The fde_ptr is pointing at start of instructions. */
    }
    else if (augmentation[0] == 'z') {
	/* The augmentation starts with a known prefix. See the
	   dwarf_frame.h for details on the layout. */
	...
	if (strcmp((char *) augmentation, DW_CIE_AUGMENTER_STRING_V0) == 0) {
	    /* The first word is an offset into execption tables. */
	    READ_UNALIGNED(dbg,offset_into_exception_tables, Dwarf_Signed,
			fde_ptr, 
			sizeof(Dwarf_sfixed));
	    SIGN_EXTEND(offset_into_exception_tables,
					sizeof(Dwarf_sfixed));
    ...
    /* READ_AREA_LENGTH updates cie_ptr for consumed bytes*/
    READ_AREA_LENGTH(dbg,length,Dwarf_Unsigned,
        cie_ptr,cie_local_length_size,cie_local_extension_size);
...
)
{
    ...
    *offset_into_exception_tables = fde->fd_offset_into_exception_tables;
    return DW_DLV_OK;
}
...
static int _dwarf_get_fde_info_for_a_pc_row(
	Dwarf_Fde       fde,
...
)
{
    ...
    for (i = 0; i < DW_REG_TABLE_SIZE; i++) {
        reg_table->rules[i].dw_offset_relevant = fde_table.fr_reg[i].ru_is_off;
...
)
{
    ...
    /* _dwarf_get_fde_info_for_a_pc_row will perform more sanity checks */
    res= _dwarf_get_fde_info_for_a_pc_row(fde, pc_requested, &fde_table, error);
    if (res != DW_DLV_OK) {
	return res;
    }
...
/*
	...
	fde.
*/
int dwarf_get_fde_instr_bytes(Dwarf_Fde inFde, Dwarf_Ptr *outinstraddr,
             Dwarf_Unsigned *outaddrlen, Dwarf_Error *error)
{
    Dwarf_Unsigned len;
    ...
    len = (inFde->fd_fde_start + inFde->fd_length + 
		inFde->fd_length_size +
		inFde->fd_extension_size)
		- instrs;
...
)
{
    ...
    {
	/* The fde's are sorted by their addresses. Binary search to find
	    correct fde.
	*/
	int low = 0;
...
)
{
	...
	returned_op_list,
		...
		/*search_pc_val*/0, 
		/*location*/0,
        instruction, (Dwarf_Ptr)((char *)instruction) + i_length,
        NULL, NULL, dbg,&instr_count,&dw_err);
    if (res != DW_DLV_OK) {
	if(res == DW_DLV_ERROR) {
	  _dwarf_error(dbg,error,dw_err);
	}
	...
	Dwarf_Off  **offsetlist,Dwarf_Signed *returncount,Dwarf_Error *err)
{
  ...
  for(i = 0;  i < cie_count;  i++) {
	...
	if(finst->fp_base_op == 0 &&
		finst->fp_extended_op == 1)  {
	  /* is DW_CFA_set_loc */
  ...
  for(i = 0;  i < fde_count; i++) {
	...
	if(finst2->fp_base_op == 0 &&
		finst2->fp_extended_op == 1)  {
	  /* is DW_CFA_set_loc */
